...
public class SctpTransferTest {
	 ...
	 */
	@Test
	public void testSocketBrokenLink() throws Exception {
		...
		for (int i = 0; i < ITERATIONS; i++) {
			System.out.println("Testing broken link, iteration " + (i + 1) + " of " + ITERATIONS
					+ ". NOTE: IOExceptions may be " + "visible during this test, and are expected.");
			try {
	...
	private void testTransferPart(SctpSocket sender, SctpSocket receiver, byte[] testData, long timeoutInSeconds)
			throws Exception {
		...
		try {
			noTimeoutOccurred = dataReceivedLatch.await(timeoutInSeconds, TimeUnit.SECONDS);
			if (noTimeoutOccurred) {
				assertArrayEquals(testData, receivedData);
			} else {
				fail("Test data did not get received within " + timeoutInSeconds + " seconds.");
			}
