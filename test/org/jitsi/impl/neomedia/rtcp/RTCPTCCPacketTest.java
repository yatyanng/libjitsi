...
public class RTCPTCCPacketTest {
	...
	private static final byte[] fci = new byte[] {
			...
			// (7 received)
			(byte) 0xe5, (byte) 0x55,
			// vector, 1-bit symbols, 3xR + 2NR + 1R + 1NR + 1R [packets over, 6 remaining 0
			// bits]
			// (5 received)
			(byte) 0xb9, (byte) 0x40,
			...
			(byte) 0x2c, (byte) 0x78,
			// the large one
			(byte) 0xff, (byte) 0x64, (byte) 0x04, (byte) 0x04, (byte) 0x00, (byte) 0x00, (byte) 0x04, (byte) 0x00,
			(byte) 0x04, (byte) 0x04, (byte) 0x00, (byte) 0x1c, (byte) 0x34 };

	@Test
	public void parse() throws Exception {
		// Note that this excludes packets reported as not received.
		RTCPTCCPacket.PacketMap packetMap = RTCPTCCPacket.getPacketsFromFci(new ByteArrayBufferImpl(fci));
		...
		assertEquals(base + received + notReceived - 1, (int) packetMap.lastKey());
	...
	@Test
	public void parseWithNR() throws Exception {
		// Note that this excludes packets reported as not received.
		RTCPTCCPacket.PacketMap packetMap = RTCPTCCPacket.getPacketsFromFci(new ByteArrayBufferImpl(fci),
	...
	public void createAndParse() throws Exception {
		...
		RTCPTCCPacket packet = new RTCPTCCPacket(0, 0, before, (byte) fbPacketCount, new DiagnosticContext());

		RTCPTCCPacket.PacketMap after = RTCPTCCPacket.getPacketsFromFci(new ByteArrayBufferImpl(packet.fci));
	...
	@Test
	public void createAndParseWithNR() throws Exception {
		long now = 1489968000021L;
		RTCPTCCPacket.PacketMap before = new RTCPTCCPacket.PacketMap();
		before.put(120, now);
		before.put(121, now + 1);
	...
	@Test
	public void createAndParse2() throws Exception {
		...
		assertEquals(referenceTime250us, RTCPTCCPacket.getReferenceTime250us(afterBaf));
