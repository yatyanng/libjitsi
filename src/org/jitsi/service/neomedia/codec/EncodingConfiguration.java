...
public abstract class EncodingConfiguration {
	/**
	 * The <tt>Logger</tt> used by this <tt>EncodingConfiguration</tt> instance for
	 * logging output.
	 */
	...
	/**
	 * That's where we keep format preferences matching SDP formats to integers. We
	 * keep preferences for both audio and video formats here in case we'd ever need
	 * to compare them to one another. In most cases however both would be
	 * decorelated and other components (such as the UI) should present them
	 * separately.
	 */
	...
	private void updateSupportedEncodings() {
		/*
		 * If they need updating, their caches are invalid and need rebuilding next time
		 * they are requested.
		 */
	 ...
	 * @return a <tt>Set</tt> of enabled available <tt>MediaFormat</tt>s with the
	 *         specified <tt>MediaType</tt> sorted in decreasing priority
	 */
	...
	/**
	 * Sets <tt>pref</tt> as the preference associated with <tt>encoding</tt>. Use
	 * this method for both audio and video encodings and don't worry if preferences
	 * are equal since we rarely need to compare prefs of video encodings to those
	 * of audio encodings.
	 *
	...
	/**
	 * Sets <tt>priority</tt> as the preference associated with <tt>encoding</tt>.
	 * Use this method for both audio and video encodings and don't worry if the
	 * preferences are equal since we rarely need to compare the preferences of
	 * video encodings to those of audio encodings.
	 *
	...
	public void setPriority(MediaFormat encoding, int priority) {
		...
		/*
		 * Since we'll be remembering the priority in the ConfigurationService by
		 * associating it with a property name/key based on encoding and clock rate
		 * only, it does not make sense to store the MediaFormat in encodingPreferences
		 * because MediaFormat is much more specific than just encoding and clock rate.
		 */
	...
	public int getPriority(MediaFormat encoding) {
		 ...
		 * NullPointerException if encodingPreferences does not contain a mapping for
		 * encoding.
		 */
	...
	/**
	 * Returns all the available encodings for a specific <tt>MediaType</tt>. This
	 * includes disabled ones (ones with priority 0).
	 *
	 ...
	 *
	 * @param type the <tt>MediaType</tt> of the supported <tt>MediaFormat</tt>s to
	 *             get
	...
	/**
	 * Compares the two formats for order. Returns a negative integer, zero, or a
	 * positive integer as the first format has been assigned a preference higher,
	 * equal to, or greater than the one of the second.
	 *
	 * @param enc1 the first format to compare for preference.
	 * @param enc2 the second format to compare for preference
	 * @return a negative integer, zero, or a positive integer as the first format
	 *         has been assigned a preference higher, equal to, or greater than the
	 *         one of the second
	 */
	 ...
	 * @return the key in {@link #encodingPreferences} which is associated with the
	 *         priority of the specified <tt>encoding</tt>
	 */
	 ...
	 * (prefix+formatName, formatPriority) will be added to <tt>properties</tt>, one
	 * for each available format. Note that a "." is not automatically added to
	 * <tt>prefix</tt>.
	 *
	 ...
	 * <tt>prefix</tt>, its ignored. If the key begins with <tt>prefix</tt>, look
	 * for an encoding name after the last ".", and interpret the key value as
	 * preference.
	 *
	 * @param properties The <tt>Map</tt> to parse.
	 * @param prefix     The prefix to use.
	...
	/**
	 * Returns <tt>true</tt> if there is at least one enabled format for media type
	 * <tt>type</tt>.
	 *
	 * @param mediaType The media type, MediaType.AUDIO or MediaType.VIDEO
	 * @return <tt>true</tt> if there is at least one enabled format for media type
	 *         <tt>type</tt>.
	 */
