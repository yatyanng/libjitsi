...
public interface MediaService {
	 ...
	 * {@link #getDefaultDevice(MediaType, MediaUseCase)}. The <tt>oldValue</tt> and
	 * the <tt>newValue</tt> of the fired <tt>PropertyChangeEvent</tt> are not to be
	 * relied on and instead a call to <tt>getDefaultDevice</tt> is to be performed
	 * to retrieve the new value.
	 */
	public static final String DEFAULT_DEVICE = "defaultDevice";

	/**
	 * The name of the property which controls whether the libjitsi codecs which
	 * depend on ffmpeg (currently mp3, h264 and amrwb) will be enabled.
	 */
	public static final String ENABLE_FFMPEG_CODECS_PNAME = MediaService.class.getName() + ".ENABLE_FFMPEG_CODECS";

	/**
	 * The name of the property which controls whether the h264 formats will be
	 * registered in libjitsi even if the ffmpeg codec is missing.
	 */
	public static final String ENABLE_H264_FORMAT_PNAME = MediaService.class.getName() + ".ENABLE_H264_FORMAT";

	/**
	 * Adds a <tt>PropertyChangeListener</tt> to be notified about changes in the
	...
	/**
	 * Initializes a new <tt>MediaStream</tt> of a specific <tt>MediaType</tt>. The
	 * new instance will not have a <tt>MediaDevice</tt> at the time of its
	 * initialization and a <tt>MediaDevice</tt> may be specified later on with the
	 * constraint that {@link MediaDevice#getMediaType()} equals <tt>mediaType</tt>.
	 *
	 ...
	 *                  sending and receiving media or <tt>null</tt> if the stream
	 *                  is to not have a <tt>StreamConnector</tt> configured at
	 *                  initialization time and a <tt>StreamConnector</tt> is to be
	 *                  specified later on
	 * @param device    the device to be used for both capture and playback of media
	...
	/**
	 * Initializes a new <tt>MediaStream</tt> instance which is to exchange media of
	 * a specific <tt>MediaType</tt> via a specific <tt>StreamConnector</tt>.
	 *
	 * @param connector the <tt>StreamConnector</tt> the stream should use for
	 *                  sending and receiving media or <tt>null</tt> if the stream
	 *                  is to not have a <tt>StreamConnector</tt> configured at
	 *                  initialization time and a <tt>StreamConnector</tt> is to be
	 *                  specified later on
	 * @param mediaType the <tt>MediaType</tt> of the media to be exchanged by the
	 *                  new instance via the specified <tt>connector</tt>
	 * @return a new <tt>MediaStream</tt> instance which is to exchange media of the
	 *         specified <tt>mediaType</tt> via the specified <tt>connector</tt>
	 */
	 ...
	 *                    sending and receiving media or <tt>null</tt> if the stream
	 *                    is to not have a <tt>StreamConnector</tt> configured at
	 *                    initialization time and a <tt>StreamConnector</tt> is to
	...
	/**
	 * Initializes a new <tt>MediaStream</tt> instance which is to exchange media of
	 * a specific <tt>MediaType</tt> via a specific <tt>StreamConnector</tt>. The
	 * security of the media exchange is to be controlled by a specific
	 * <tt>SrtpControl</tt>.
	 *
	 * @param connector   the <tt>StreamConnector</tt> the stream should use for
	 *                    sending and receiving media or <tt>null</tt> if the stream
	 ...
	 * @param mediaType   the <tt>MediaType</tt> of the media to be exchanged by the
	 *                    new instance via the specified <tt>connector</tt>
	 * @param srtpControl the <tt>SrtpControl</tt> to control the security of the
	 *                    media exchange
	 * @return a new <tt>MediaStream</tt> instance which is to exchange media of the
	 *         specified <tt>mediaType</tt> via the specified <tt>connector</tt>
	 */
	...
	/**
	 * Returns a list containing all devices known to this service implementation
	 * and handling the specified <tt>MediaType</tt>.
	 *
	 * @param mediaType the media type (i.e. AUDIO or VIDEO) that we'd like to
	 *                  obtain the device list for.
	 * @param useCase   <tt>MediaUseCase</tt> value indicating for the use-case of
	 *                  device that we are trying to obtain.
	 *
	 ...
	 * statically assigned the 101 payload type by some legacy systems. Signalling
	 ...
	 * changes in the values of the properties of this instance.
	 *
	 * @param listener the <tt>PropertyChangeListener</tt> to no longer be notified
	...
	/**
	 * Returns the value which will be used for the canonical end-point identifier
	 * (CNAME) in RTCP packets sent by this running instance of libjitsi.
	 * 
	 * @return the value which will be used for the canonical end-point identifier
	 *         (CNAME) in RTCP packets sent by this running instance of libjitsi.
	 */
	public String getRtpCname();

	/**
	 * Creates a <tt>RecorderEventHandler</tt> instance that saves received events
	 * in JSON format.
	 * 
	 * @param filename the filename into which the created
	 *                 <tt>RecorderEventHandler</tt> will save received events.
	 * @return a <tt>RecorderEventHandler</tt> instance that saves received events
	 *         in JSON format.
	 * @throws IOException if a <tt>RecorderEventHandler</tt> could not be created
	 *                     for <tt>filename</tt>.
	 */
