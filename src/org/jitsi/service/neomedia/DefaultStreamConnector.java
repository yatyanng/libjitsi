...
public class DefaultStreamConnector implements StreamConnector {
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>DefaultStreamConnector</tt> class and its
	...
	/**
	 * The name of the property containing the number of binds that a Media Service
	 * Implementation should execute in case a port is already bound to (each retry
	 * would be on a new port in the allowed boundaries).
	 */
	public static final String BIND_RETRIES_PROPERTY_NAME = "net.java.sip.communicator.service.media.BIND_RETRIES";

	/**
	 * The name of the property that contains the maximum port number that we'd like
	 * our RTP managers to bind upon.
	 */
	...
	/**
	 * The name of the property that contains the minimum port number that we'd like
	 * our RTP managers to bind upon.
	 */
	 ...
	 * specified local <tt>InetAddress</tt> and its port is within the range defined
	 * by the <tt>ConfigurationService</tt> properties
	...
	private static synchronized DatagramSocket createDatagramSocket(InetAddress bindAddr) {
		...
		for (int i = 0; i < bindRetries; i++) {
			...
			try {
				...
				logger.warn("Retrying a bind because of a failure to bind to address " + bindAddr + " and port " + port,
						se);
			}
		}
		return null;
	...
	/**
	 * The local <tt>InetAddress</tt> this <tt>StreamConnector</tt> attempts to bind
	 * to on demand.
	 */
	private final InetAddress bindAddr;

	/**
	 * The <tt>DatagramSocket</tt> that a stream should use for control data (e.g.
	 * RTCP) traffic.
	 */
	...
	/**
	 * Initializes a new <tt>DefaultStreamConnector</tt> instance with no control
	 * and data <tt>DatagramSocket</tt>s.
	 * <p>
	 * Suitable for extenders willing to delay the creation of the control and data
	 * sockets. For example, they could override {@link #getControlSocket()} and/or
	 * {@link #getDataSocket()} and create them on demand.
	 */
	...
	/**
	 * Initializes a new <tt>DefaultStreamConnector</tt> instance with a specific
	 * bind <tt>InetAddress</tt>. The new instance is to attempt to bind on demand
	 * to the specified <tt>InetAddress</tt> in the port range defined by the
	 * <tt>ConfigurationService</tt> properties
	 * {@link #MIN_PORT_NUMBER_PROPERTY_NAME} and
	 * {@link #MAX_PORT_NUMBER_PROPERTY_NAME} at most
	 * {@link #BIND_RETRIES_PROPERTY_NAME} times.
	 *
	 * @param bindAddr the local <tt>InetAddress</tt> the new instance is to attempt
	 *                 to bind to
	 */
	 ...
	 *                      RTP) traffic
	 * @param controlSocket the <tt>DatagramSocket</tt> to be used for control data
	 *                      (e.g. RTCP) traffic
	 */
	...
	/**
	 * Returns a reference to the <tt>DatagramSocket</tt> that a stream should use
	 * for control data (e.g. RTCP) traffic.
	 *
	 * @return a reference to the <tt>DatagramSocket</tt> that a stream should use
	 *         for control data (e.g. RTCP) traffic
	...
	/**
	 * Returns a reference to the <tt>Socket</tt> that a stream should use for data
	 * (e.g. RTP) traffic.
	 *
	 * @return a reference to the <tt>Socket</tt> that a stream should use for data
	 *         (e.g. RTP) traffic.
	 */
	@Override
	...
	/**
	 * Notifies this instance that utilization of its <tt>DatagramSocket</tt>s for
	 * data and/or control traffic has started.
	 *
	...
	/**
	 * Notifies this instance that utilization of its <tt>DatagramSocket</tt>s for
	 * data and/or control traffic has temporarily stopped. This instance should be
