...
public class BasicVolumeControl implements VolumeControl, GainControl {
	...
	/**
	 * Applies the gain specified by <tt>gainControl</tt> to the signal defined by
	 * the <tt>length</tt> number of samples given in <tt>buffer</tt> starting at
	 * <tt>offset</tt>.
	 *
	...
	/**
	 * Returns the decibel value for a ratio between a power level required and the
	 * reference power level.
	 *
	 * @param powerLevelRequired  The power level wished for the signal (corresponds
	 *                            to the measured power level).
	...
	protected static float getDefaultVolumeLevel() {
		return MIN_VOLUME_LEVEL
				+ (MAX_VOLUME_LEVEL - MIN_VOLUME_LEVEL) / ((MAX_VOLUME_PERCENT - MIN_VOLUME_PERCENT) / 100);
	}
	 ...
	 * @param referencePowerLevel The reference power level.
	 * @return The power level the signal, which corresponds to the measured power
	 *         level.
	 */
	...
	/**
	 * The <tt>VolumeChangeListener</tt>s interested in volume change events through
	 * the <tt>VolumeControl</tt> interface.
	 * <p>
	...
	/**
	 * Creates volume control instance and initializes initial level value if stored
	 ...
	 *                                             value of the volume level of the
	 *                                             new instance
	 */
	...
	/**
	 * Fires a new <tt>VolumeChangeEvent</tt> to the <tt>VolumeChangeListener</tt>s
	 * added to this instance to notify about a change in the volume (level) of this
	 * <tt>VolumeControl</tt>.
	 */
	...
	/**
	 * Set the gain in decibels. Setting the gain to 0.0 (the default) implies that
	 * the audio signal is neither amplified nor attenuated. Positive values amplify
	 * the audio signal and negative values attenuate the signal.
	 *
	 ...
	 * 0.0 is silence; 1.0 is the loudest useful level that this
	 ...
	 * Media Service.
	 *
	 * @param value the new value, changed if different from current volume
	 *              settings.
	...
	private float setVolumeLevel(float value) {
		...
		/*
		 * Save the current volume level in the ConfigurationService so that we can
		 * restore it on the next application run.
		 */
		ConfigurationService cfg = LibJitsi.getConfigurationService();
	...
	/**
	 * Modifies the hardware microphone sensibility (hardware amplification). This
	 * is a void function for AbstractVolumeControl since it does not have any
	 * connection to hardware volume. But, this function must be redefined by any
	 * extending class.
	 */
	protected void updateHardwareVolume() {
		/*
		 * AbstractVolumeControl does not implement such functionality, the method is
		 * defined and invoked in order to allow extenders to provide such
		 * functionality.
		 */
	}
}
