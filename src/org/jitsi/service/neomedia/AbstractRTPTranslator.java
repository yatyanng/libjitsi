...
public abstract class AbstractRTPTranslator implements RTPTranslator {
	/**
	 * An empty array with element type <tt>WriteFilter</tt>. Explicitly defined in
	 * order to reduce unnecessary allocations and the consequent effects of the
	 * garbage collector.
	 */
	 ...
	 * @param pkt         the packet from <tt>source</tt> which is to be written
	 ...
	 *         writing is to abort
	 */
	protected boolean willWrite(WriteFilter writeFilter, MediaStream source, RawPacket pkt, MediaStream destination,
			boolean data) {
		boolean accept;
