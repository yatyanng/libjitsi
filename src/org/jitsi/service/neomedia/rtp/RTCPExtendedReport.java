...
public class RTCPExtendedReport extends RTCPPacket {
	...
	public static abstract class ReportBlock {
		...
		/**
		 * Serializes/writes the binary representation of this <tt>ReportBlock</tt> into
		...
		/**
		 * Computes the length in <tt>byte</tt>s of this <tt>ReportBlock</tt>, including
		 * the header and any padding.
		 * <p>
		 * The implementation of <tt>ReportBlock</tt> returns the length in
		 * <tt>byte</tt>s of the header of an extended report block i.e. <tt>4</tt>. The
		 * implementation is provided as a convenience because RFC 3611 defines that the
		 * type-specific block contents of an extended report block may be zero bits
		 * long if the block type definition permits.
		 * </p>
		 *
		 * @return the length in <tt>byte</tt>s of this <tt>ReportBlock</tt>, including
		 *         the header and any padding.
		 */
	...
	public static class VoIPMetricsReportBlock extends ReportBlock {
		/**
		 * The jitter buffer size is being dynamically adjusted to deal with varying
		 * levels of jitter.
		 */
		...
		/**
		 * An enhanced interpolation algorithm is being used; algorithms of this type
		 * are able to conceal high packet loss rates effectively.
		 */
		...
		/**
		 * A simple replay or interpolation algorithm is being used to fill-in the
		 * missing packet; this approach is typically able to conceal isolated lost
		...
		/**
		 * No information is available concerning the use of packet loss concealment
		 ...
		 * reception that were either lost or discarded. The value is expressed as a
		 * fixed point number with the binary point at the left edge of the field. It is
		 * calculated by dividing the total number of packets lost or discarded
		 * (excluding duplicate packet discards) within burst periods by the total
		 * number of packets expected within the burst periods, multiplying the result
		 * of the division by 256, limiting the maximum value to 255 (to avoid
		 * overflow), and taking the integer part. The field MUST be populated and MUST
		 * be set to zero if no packets have been received.
		 */
		 ...
		 * since the beginning of reception, due to late or early arrival, under-run or
		 * overflow at the receiving jitter buffer. The value is expressed as a fixed
		 * point number with the binary point at the left edge of the field. It is
		 * calculated by dividing the total number of packets discarded (excluding
		 * duplicate packet discards) by the total number of packets expected,
		 * multiplying the result of the division by 256, limiting the maximum value to
		...
		/**
		 * The fraction of RTP data packets within inter-burst gaps since the beginning
		 * of reception that were either lost or discarded. The value is expressed as a
		 * fixed point number with the binary point at the left edge of the field. It is
		 * calculated by dividing the total number of packets lost or discarded
		 * (excluding duplicate packet discards) within gap periods by the total number
		 * of packets expected within the gap periods, multiplying the result of the
		 * division by 256, limiting the maximum value to 255 (to avoid overflow), and
		 * taking the integer part. The field MUST be populated and MUST be set to zero
		...
		/**
		 * The implementation specific adjustment rate of a jitter buffer in adaptive
		 * mode. Defined in terms of the approximate time taken to fully adjust to a
		 * step change in peak to peak jitter from 30 ms to 100 ms such that:
		 * <tt>adjustment time = 2 * J * frame size (ms)<tt>
		 ...
		 * reception, expressed as a fixed point number with the binary point at the
		 * left edge of the field. This value is calculated by dividing the total number
		 * of packets lost (after the effects of applying any error protection such as
		 * FEC) by the total number of packets expected, multiplying the result of the
		 * division by 256, limiting the maximum value to 255 (to avoid overflow), and
		 * taking the integer part. The numbers of duplicated packets and discarded
		 * packets do not enter into this calculation. Since receivers cannot be
		 * required to maintain unlimited buffers, a receiver MAY categorize
		 * late-arriving packets as lost. The degree of lateness that triggers a loss
		 * SHOULD be significantly greater than that which triggers a discard.
		 */
		...
		/**
		 * The synchronization source identifier (SSRC) of the RTP data packet source
		 * being reported upon by this report block.
		 */
		 ...
		 *
		 * @param blockLength     the length of the extended report block to read, not
		 *                        including the header, in bytes.
		 * @param datainputstream the binary representation from which the new instance
		 *                        is to be initialized. The <tt>datainputstream</tt> is
		 *                        asumed to contain type-specific block contents without
		 *                        extended report block header i.e. no block type (BT),
		 *                        type-specific, and block length fields will be read
		 *                        from <tt>datainputstream</tt>.
		...
		protected void assemble(DataOutputStream dataoutputstream) throws IOException {
			...
			dataoutputstream.writeByte(((getPacketLossConcealment() & 0x03) << 6)
					| ((getJitterBufferAdaptive() & 0x03) << 4) | (getJitterBufferRate() & 0x0F));
			// reserved
			dataoutputstream.writeByte(0);
			// jitter buffer nominal delay (JB nominal)
		 ...
		 * <tt>byte</tt>s equal to <tt>36</tt>, including the extended report block
		 * header.
		 */
		@Override
		public int calcLength() {
		 ...
		 *
		 * @return the fraction of RTP data packets within burst periods since the
		 *         beginning of reception that were either lost or discarded
		 */
		 ...
		 * discarded since the beginning of reception, due to late or early arrival,
		 * under-run or overflow at the receiving jitter buffer.
		 *
		 * @return the fraction of RTP data packets from the source that have been
		...
		/**
		 * Get the fraction of RTP data packets within inter-burst gaps since the
		 * beginning of reception that were either lost or discarded.
		 *
		 * @return the fraction of RTP data packets within inter-burst gaps since the
		 *         beginning of reception that were either lost or discarded
		 */
		...
		/**
		 * Gets the implementation specific adjustment rate of a jitter buffer in
		 * adaptive mode.
		 *
		 * @return the implementation specific adjustment rate of a jitter buffer in
		 *         adaptive mode
		 */
		 ...
		 *
		 * @return the fraction of RTP data packets from the source lost since the
		 *         beginning of reception
		 * @see #lossRate
		 */
		...
		/**
		 * Gets the synchronization source identifier (SSRC) of the RTP data packet
		 * source being reported upon by this report block.
		 *
		 * @return the synchronization source identifier (SSRC) of the RTP data packet
		 *         source being reported upon by this report block
		 */
		 ...
		 * beginning of reception that were either lost or discarded.
		 *
		 * @param burstDensity the fraction of RTP data packets within burst periods
		 ...
		 * under-run or overflow at the receiving jitter buffer.
		 *
		 * @param discardRate the fraction of RTP data packets from the source that have
		...
		/**
		 * Sets the fraction of RTP data packets within inter-burst gaps since the
		 * beginning of reception that were either lost or discarded.
		 *
		 * @param gapDensity the fraction of RTP data packets within inter-burst gaps
		 *                   since the beginning of reception that were either lost or
		 *                   discarded
		 */
		 ...
		 *                                  <tt>jitterBufferAdapter</tt> is not one of
		 *                                  the constants
		 *                                  <tt>ADAPTIVE_JITTER_BUFFER_ADAPTIVE</tt>,
		 *                                  <tt>NON_ADAPTIVE_JITTER_BUFFER_ADAPTIVE</tt>,
		 *                                  <tt>RESERVED_JITTER_BUFFER_ADAPTIVE</tt>,
		 *                                  and <tt>UNKNOWN_JITTER_BUFFER_ADAPTIVE</tt>
		 */
		...
		/**
		 * Sets the implementation specific adjustment rate of a jitter buffer in
		 * adaptive mode.
		 *
		 * @param jitterBufferRate the implementation specific adjustment rate of a
		 ...
		 * beginning of reception.
		 *
		 * @param lossRate the fraction of RTP data packets from the source lost since
		 ...
		 *
		 * @param packetLossConcealment {@link #STANDARD_PACKET_LOSS_CONCEALMENT},
		 *                              {@link #ENHANCED_PACKET_LOSS_CONCEALMENT},
		 *                              {@link #DISABLED_PACKET_LOSS_CONCEALMENT}, or
		 *                              {@link #UNSPECIFIED_PACKET_LOSS_CONCEALMENT}
		 * @throws IllegalArgumentException if the specified
		 *                                  <tt>packetLossConcealment</tt> is not one of
		 *                                  the constants
		 *                                  <tt>STANDARD_PACKET_LOSS_CONCEALMENT</tt>,
		 *                                  <tt>ENHANCED_PACKET_LOSS_CONCEALMENT</tt>,
		 *                                  <tt>DISABLED_PACKET_LOSS_CONCEALMENT</tt>,
		 *                                  and
		...
		/**
		 * Sets the synchronization source identifier (SSRC) of the RTP data packet
		 * source being reported upon by this report block.
		 *
		 * @param sourceSSRC the synchronization source identifier (SSRC) of the RTP
		 *                   data packet source being reported upon by this report block
		 */
	 ...
	 * <tt>RTCPExtendedReport</tt> has been received or sent by the local endpoint.
	 */
	 ...
	 * @throws IOException if an input/output error occurs while
	 *                     deserializing/reading the new instance from <tt>buf</tt>
	 ...
	 *                     deserializing/reading the new instance from
	 *                     <tt>datainputstream</tt> or the binary representation
	 *                     does not parse into an <tt>RTCPExtendedReport</tt>
	 *                     instance.
	 */
	...
	/**
	 * Initializes a new <tt>RTCPExtendedReport</tt> instance by
	 * deserializing/reading a binary representation of part of the packet from a
	 * <tt>DataInputStream</tt>, and taking the values found in the first 4 bytes of
	 * the binary representation as arguments.
	 *
	 ...
	 * @param datainputstream the binary representation from which the new instance
	 *                        is to be initialized, excluding the first 4 bytes.
	 * @throws IOException if an input/output error occurs while
	 *                     deserializing/reading the new instance from
	 *                     <tt>datainputstream</tt> or the binary representation
	 *                     does not parse into an <tt>RTCPExtendedReport</tt>
	 ...
	 * Initializes a new <tt>RTCPExtendedReport</tt> instance by
	 * deserializing/reading a binary representation of part of the packet from a
	 * <tt>DataInputStream</tt>, and taking the values normally found in the first 4
	 * bytes of the binary representation as arguments.
	 *
	 * @param b0              the first byte of the binary representation.
	 * @param pt              the value of the {@code packet type} field.
	 * @param length          the length in bytes of the RTCP packet, including all
	 *                        of it's headers and excluding any padding.
	 * @param datainputstream the binary representation from which the new instance
	 *                        is to be initialized, excluding the first 4 bytes.
	 * @throws IOException if an input/output error occurs while
	 *                     deserializing/reading the new instance from
	 ...
	 *
	 * @param reportBlock the extended report block to add to this extended report
	 * @return <tt>true</tt> if the list of extended report blocks carried by this
	 *         extended report changed because of the method invocation; otherwise,
	 *         <tt>false</tt>
	...
	/**
	 * Gets the synchronization source identifier (SSRC) of the originator of this
	 * XR packet.
	 *
	...
	/**
	 * Sets the synchronization source identifier (SSRC) of the originator of this
	 * XR packet.
	 *
	 * @param ssrc the synchronization source identifier (SSRC) of the originator of
	 *             this XR packet
	 */
	 ...
	 *                        <tt>RTCPExtendedReport</tt> has been received or sent
