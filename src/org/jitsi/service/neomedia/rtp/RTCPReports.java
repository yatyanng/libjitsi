...
public class RTCPReports {
	/**
	 * The <tt>Logger</tt> used by the <tt>RTCPReports</tt> class and its instances
	...
	/**
	 * The RTCP extended reports (XR) received by the local endpoint represented by
	 * this instance associated with the synchronization source identifiers of their
	 * respective originator (SSRC defined by RFC 3611).
	 */
	private final Map<Integer, RTCPExtendedReport> receivedExtendedReports = new HashMap<>();

	/**
	 * The RTCP sender report (SR) and/or receiver report (RR) blocks received by
	 * the local endpoint represented by this instance associated with the
	 * synchronization source identifiers of their respective source (SSRC of source
	 * defined by RFC 3550).
	 */
	 ...
	 * synchronization source identifiers of their respective originator (SSRC of
	 * sender defined by RFC 3550).
	 */
	 ...
	 * source identifiers of their respective source (SSRC of source defined by RFC
	 * 3611).
	 */
	private final Map<Integer, RTCPExtendedReport.VoIPMetricsReportBlock> receivedVoIPMetrics = new HashMap<>();

	/**
	 * The RTCP extended reports (XR) sent by the local endpoint represented by this
	 * instance associated with the synchronization source identifiers of their
	 * respective originator (SSRC defined by RFC 3611).
	 */
	private final Map<Integer, RTCPExtendedReport> sentExtendedReports = new HashMap<>();

	/**
	 * The RTCP sender report (SR) and/or receiver report (RR) blocks sent by the
	 * local endpoint represented by this instance associated with the
	 * synchronization source identifiers of their respective source (SSRC of source
	 * defined by RFC 3550).
	 */
	...
	/**
	 * The RTCP sender reports (SR) and/or receiver reports (RR) sent by the local
	 * endpoint represented by this instance associated with the synchronization
	 * source identifiers of their respective originator (SSRC of sender defined by
	 * RFC 3550).
	 */
	 ...
	 * represented by this instance associated with the synchronization source
	 * identifiers of their respective source (SSRC of source defined by RFC 3611).
	 */
	private final Map<Integer, RTCPExtendedReport.VoIPMetricsReportBlock> sentVoIPMetrics = new HashMap<>();

	/**
	 * Adds a new <tt>RTCPReportListener</tt> to be notified by this instance about
	 * the receiving and sending of RTCP RR, SR and XR.
	 *
	...
	/**
	 * Gets the RTCP sender report (SR) and/or receiver report (RR) blocks received
	...
	/**
	 * Gets the RTCP sender reports (SR) and/or receiver reports (RR) received by
	 * the local endpoint.
	 *
	 * @return the RTCP sender reports (SR) and/or receiver reports (RR) received by
	 *         the local endpoint
	 */
	...
	/**
	 * Gets the RTCP extended report (XR) VoIP Metrics blocks received by the local
	 * endpoint.
	 *
	...
	/**
	 * Gets the latest RTCP extended report (XR) VoIP Metrics block received from a
	 * remote originator for a local source.
	 *
	 * @param sourceSSRC the SSRC of the local source
	...
	/**
	 * Gets the RTCP sender report (SR) and/or receiver report (RR) blocks sent by
	 * the local endpoint.
	 *
	 * @return the RTCP sender report (SR) and/or receiver report (RR) blocks sent
	 *         by the local endpoint
	 */
	...
	/**
	 * Gets the RTCP sender reports (SR) and/or receiver reports (RR) sent by the
	 * local endpoint.
	 *
	 * @return the RTCP sender reports (SR) and/or receiver reports (RR) sent by the
	 *         local endpoint
	 */
	 ...
	 * endpoint.
	 *
	 * @return the RTCP extended report (XR) VoIP Metrics blocks sent by the local
	 *         endpoint
	 */
	 ...
	 *
	 * @param sourceSSRC the SSRC of the remote source
	 * @return the RTCP extended report (XR) VoIP Metrics block sent from a local
	 *         originator for the specified <tt>sourceSSRC</tt>
	 */
	...
	/**
	 * Removes an existing <tt>RTCPReportListener</tt> to no longer be notified by
	 * this instance about the receiving and sending of RTCP RR, SR and XR.
	 *
	 ...
	 * <tt>extendedReport</tt> and notifies the <tt>RTCPReportListener</tt>s added
	 * to this instance.
	 *
	...
	/**
	 * Notifies this instance that a specific <tt>RTCPExtendedReport</tt> was sent
	 * by the local endpoint. Remembers the sent <tt>extendedReport</tt> and
	...
	/**
	 * Notifies this instance that a specific <tt>RTCPReport</tt> was received by
	 * the local endpoint. Remembers the received <tt>report</tt> and notifies the
			...
			if (report.equals(oldValue)) {
				...
				if (feedbacks != null) {
					...
					if (!feedbacks.isEmpty() && logger.isTraceEnabled()) {
						...
						s.append("Received RTCP RR blocks from SSRC ").append(report.getSSRC() & 0xFFFFFFFFL)
								.append(" at time (ms) ").append(report.getSystemTimeStamp()).append(" for SSRC(s):");
						for (RTCPFeedback feedback : feedbacks) {
							s.append(' ').append(feedback.getSSRC() & 0xFFFFFFFFL).append(',');
						}
						logger.trace(s);
					}
				}
	...
	/**
	 * Notifies this instance that a specific <tt>RTCPReport</tt> was sent by the
	 * local endpoint. Remembers the sent <tt>report</tt> and notifies the
	 * <tt>RTCPReportListener</tt>s added to this instance.
	 *
