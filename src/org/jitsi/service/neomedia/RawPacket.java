...
public class RawPacket implements ByteArrayBuffer {
	 ...
	 * parameters. Note that because we're creating a packet filled with padding,
	 * the length must not exceed 12 + 0xFF.
	 *
	...
	public boolean setPaddingSize(int len) {
		if (buffer == null || buffer.length < offset + FIXED_HEADER_SIZE + len || len < 0 || len > 0xFF) {
			return false;
		}
	...
	public String toString() {
		...
		// RTP packet.
		StringBuilder sb = new StringBuilder("RawPacket[off=").append(offset).append(", len=").append(length)
	...
	public class HeaderExtensions implements Iterator<HeaderExtension> {
		...
		private void reset() {
			...
			nextOff = offset + FIXED_HEADER_SIZE + getCsrcCount(buffer, offset, length) * 4 + EXT_HEADER_SIZE;
			remainingLen = len;
		}
