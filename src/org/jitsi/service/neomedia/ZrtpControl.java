...
public interface ZrtpControl extends SrtpControl {
	 ...
	 *         immediately after class instantiation. Returns <tt>null</tt> if ZRTP
	 *         is not available.
	 */
	 ...
	 *         peer's hello hash is not available, returns <tt>null</tt>.
	 */
	public String getPeerHelloHash();

	/**
	 * Gets other party's ZID (ZRTP Identifier) data that was received during ZRTP
	 * processing. The ZID data can be retrieved after ZRTP receives the first Hello
	...
	/**
	 * Gets other party's ZID (ZRTP Identifier) data that was received during ZRTP
	 * processing as a <tt>String</tt>. The ZID data can be retrieved after ZRTP
	 * receives the first Hello packet from the other party.
	 *
	...
	/**
	 * Returns the timeout value in milliseconds that we will wait and fire timeout
	 * secure event if call is not secured.
	 *
	 * @return the timeout value in milliseconds that we will wait and fire timeout
	 *         secure event if call is not secured.
	 */
