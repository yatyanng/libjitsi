	...
	public void addKeyFrameRequestee(int index, KeyFrameRequestee keyFrameRequestee) {
		...
		synchronized (this) {
			if (!keyFrameRequestees.contains(keyFrameRequestee)) {
				...
				/*
				 * If this KeyFrameControl is to determine the index at which keyFrameRequestee
				 * is to be added according to its own internal logic, then it will prefer
				 * KeyFrameRequestee implementations from outside of neomedia rather than from
				 * its inside.
				 */
				if (-1 == index) {
	...
	public void addKeyFrameRequester(int index, KeyFrameRequester keyFrameRequester) {
		...
		synchronized (this) {
			if (!keyFrameRequesters.contains(keyFrameRequester)) {
				...
				/*
				 * If this KeyFrameControl is to determine the index at which keyFrameRequester
				 * is to be added according to its own internal logic, then it will prefer
				 * KeyFrameRequester implementations from outside of neomedia rather than from
				 * its inside.
				 */
				if (-1 == index) {
	...
	public boolean keyFrameRequest() {
		for (KeyFrameRequestee keyFrameRequestee : getKeyFrameRequestees()) {
			try {
				...
				/*
				 * A KeyFrameRequestee has malfunctioned, do not let it interfere with the
				 * others.
				 */
			}
		}
