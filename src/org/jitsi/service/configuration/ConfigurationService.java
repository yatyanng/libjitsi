...
public interface ConfigurationService {
	/**
	 * The name of the property that indicates the name of the directory where Jitsi
	 * is to store user specific data such as configuration files, message and call
	 * history.
	 */
	public static final String PNAME_SC_HOME_DIR_NAME = "net.java.sip.communicator.SC_HOME_DIR_NAME";

	/**
	 * The name of the property that indicates the location of the directory where
	 * Jitsi is to store user specific data such as configuration files, message and
	 * call history.
	 */
	 ...
	 * this method would first trigger a PropertyChangeEvent that will be dispatched
	 * to all VetoableChangeListeners. In case no complaints (PropertyVetoException)
	 ...
	 * to all VetoableChangeListeners. In case no complaints (PropertyVetoException)
	 * have been received, the property will be actually changed and a
	 * PropertyChangeEvent will be dispatched. This method also allows the caller to
	 * specify whether or not the specified property is a system one.
	 * <p>
	 * 
	 * @param propertyName the name of the property to change.
	 * @param property     the new value of the specified property.
	 * @param isSystem     specifies whether or not the property being set is a
	 *                     System property and should be resolved against the system
	 *                     property set.
	 *
	 * @throws ConfigPropertyVetoException in case the changed has been refused by
	 *                                     at least one propertychange listener.
	 ...
	 * meaning that first <code>VetoableChangeListener</code>s are asked to approve
	 * the modifications of the specified properties to the specified values, then
	 * the modifications are performed if no complaints have been raised in the form
	 * of <code>PropertyVetoException</code> and finally
	 * <code>PropertyChangeListener</code>s are notified about the changes of each
	 * of the specified properties. The batch operations allows the
	 * <code>ConfigurationService</code> implementations to optimize, for example,
	 * the saving of the configuration which in this case can be performed only once
	 * for the setting of multiple properties.
	 *
	 * @param properties a {@link Map} of property names to their new values to be
	 *                   set.
	 * @throws ConfigPropertyVetoException if a change in at least one of the
	 *                                     properties has been refused by at least
	 *                                     one of the
	...
	/**
	 * Returns the value of the property with the specified name or null if no such
	 * property exists.
	 * 
	...
	/**
	 * Returns a {@link List} of <tt>String</tt>s containing all property names that
	 * have the specified prefix. The return value will include property names that
	 * have prefixes longer than specified, unless the <tt>exactPrefixMatch</tt>
	 ...
	 * that have the specified suffix. A suffix is considered to be everything after
	 * the last dot in the property name.
	 * <p>
	 ...
	 * </code>
	 * <p>
	 * A call to this method with <tt>suffix</tt> equal to "PROP1" will return both
	 * properties, whereas the call with <tt>suffix</tt> equal to
	 * "communicator.PROP1" or "PROP2" will return an empty <tt>List</tt>. Thus, if
	 * the <tt>suffix</tt> argument contains a dot, nothing will be found.
	 * </p>
	 *
	 ...
	 * property value was mapped against the specified propertyName, or in case the
	 * returned property string had zero length or contained whitespaces only.
	 *
	 * @param propertyName the name of the property that is being queried.
	 ...
	 *
	 * @param propertyName the name of the property that is being queried.
	 * @param defaultValue the value to be returned if the specified property name
	...
	/**
	 * Gets the value of a specific property as a boolean. If the specified property
	 * name is associated with a value in this <code>ConfigurationService</code>,
	 * the string representation of the value is parsed into a boolean according to
	 * the rules of {@link Boolean#parseBoolean(String)} . Otherwise,
	 ...
	 *         <code>defaultValue</code> if the property with the specified name is
	 *         not associated with a value in this <code>ConfigurationService</code>
	 */
	 ...
	 * {@link Integer#parseInt(String)} . If parsing the value as a signed decimal
	 * integer fails or there is no value associated with the specified property
	 * name, <tt>defaultValue</tt> is returned.
	 *
	 * @param propertyName the name of the property to get the value of as a signed
	 *                     decimal integer
	 * @param defaultValue the value to be returned if parsing the value of the
	 *                     specified property name as a signed decimal integer fails
	 *                     or there is no value associated with the specified
	 *                     property name in this <tt>ConfigurationService</tt>
	 * @return the value of the property with the specified name in this
	 *         <tt>ConfigurationService</tt> as a signed decimal integer;
	 *         <tt>defaultValue</tt> if parsing the value of the specified property
	 *         name fails or no value is associated in this
	 *         <tt>ConfigurationService</tt> with the specified property name
	 */
	public int getInt(String propertyName, int defaultValue);

	/**
	 * Gets the value of a specific property as a double. If the specified property
	 * name is associated with a value in this <tt>ConfigurationService</tt>, the
	 * string representation of the value is parsed into a double according to the
	 * rules of {@link Double#parseDouble(String)}. If there is no value, or parsing
	 * of the value fails, <tt>defaultValue</tt> is returned.
	 *
	...
	/**
	 * Gets the value of a specific property as a signed decimal long integer. If
	 * the specified property name is associated with a value in this
	 * <tt>ConfigurationService</tt>, the string representation of the value is
	 ...
	 * @param defaultValue the value to be returned if parsing the value of the
	 *                     specified property name as a signed decimal long integer
	 *                     fails or there is no value associated with the specified
	 *                     property name in this <tt>ConfigurationService</tt>
	 * @return the value of the property with the specified name in this
	 ...
	 * In case a property with the specified name does not exist the listener is
	 * still added and would only be taken into account from the moment such a
	 * property is set by someone.
	 * <p>
	 * 
	 ...
	 * were registered for a specific property. The method has no effect when called
	 * for a listener that was not registered for that specific property.
	 * <p>
	 *
	...
	/**
	 * Store the current set of properties back to the configuration file. The name
	 * of the configuration file is queried from the system property
	 * net.java.sip.communicator.PROPERTIES_FILE_NAME, and is set to
	 * sip-communicator.xml in case the property does not contain a valid file name.
	 * The location might be one of three possible, checked in the following order:
	 * <br>
	 ...
	 * The name of the configuration file is queried from the system property
	 * net.java.sip.communicator.PROPERTIES_FILE_NAME, and is set to
	 * sip-communicator.xml in case the property does not contain a valid file name.
	 * The location might be one of three possible, checked in the following order:
	 * <br>
	 ...
	 * Implementations that use a file for storing properties may simply delete it
	 * when this method is called.
	 */
	public void purgeStoredConfiguration();

	/**
	 * Prints all configuration properties on 'INFO' logging level *except* that
	 * properties which name matches given regular expression will have their values
	 * masked with ***.
	 *
	...
	/**
	 * Returns the name of the directory where Jitsi is to store user specific data
	 * such as configuration files, message and call history as well as is bundle
	 * repository.
	 *
	 * @return the name of the directory where Jitsi is to store user specific data
	 *         such as configuration files, message and call history as well as is
	 *         bundle repository.
	 */
	public String getScHomeDirName();

	/**
	 * Returns the location of the directory where Jitsi is to store user specific
	 * data such as configuration files, message and call history as well as is
	 * bundle repository.
	 *
	 * @return the location of the directory where Jitsi is to store user specific
	 *         data such as configuration files, message and call history as well as
	 *         is bundle repository.
	 */
	public String getScHomeDirLocation();

	/**
	 * Use with caution! Returns the name of the configuration file currently used.
	 * Placed in HomeDirLocation/HomeDirName {@link #getScHomeDirLocation()}
