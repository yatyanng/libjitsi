...
public class ConfigurationServiceImpl implements ConfigurationService {
	/**
	 * The <tt>Logger</tt> used by this <tt>ConfigurationServiceImpl</tt> instance
	 * for logging output.
	 */
	private final Logger logger = Logger.getLogger(ConfigurationServiceImpl.class);

	/**
	 * The name of the <tt>ConfigurationStore</tt> class to be used as the default
	 * when no specific <tt>ConfigurationStore</tt> class is determined as
	 * necessary.
	 */
	...
	/**
	 * Name of the file containing overrides (possibly set by the deployer) for any
	 ...
	 * system properties keys whose values should be masked when printed out to the
	 * logs.
	 */
	...
	/**
	 * A set of immutable properties deployed with the application during install
	 * time. The properties in this file will be impossible to override and attempts
	 * to do so will simply be ignored.
	 * 
	 ...
	 * Contrary to the properties in {@link #immutableDefaultProperties} the ones in
	 * this map can be overridden with call to the <tt>setProperty()</tt> methods.
	 * Still, re-setting one of these properties to <tt>null</tt> would cause for
	 * its initial value to be restored.
	 */
	...
	/**
	 * A (cached) reference to a <tt>FileAccessService</tt> implementation used by
	 * this <tt>ConfigurationService</tt> implementation.
	 */
	private FileAccessService faService;

	/**
	 * The indicator which determines whether this instance has assigned a value to
	 * {@link #faService}. Introduced in order to avoid multiple attempts to query
	 * for a <tt>FileAccessService</tt> implementation while still delaying the
	 * initial query.
	 */
	 ...
	 * property name-value associations of this <code>ConfigurationService</code>
	 * and performs their actual storing in <code>configurationFile</code>.
	 */
	private ConfigurationStore store;

	public ConfigurationServiceImpl() {
		/*
		 * XXX We explicitly delay the query for the FileAccessService implementation
		 * because FileAccessServiceImpl looks for properties set by methods of
		 * ConfigurationServiceImpl and we want to make sure that we have given the
		 * chance to this ConfigurationServiceImpl to set these properties before
		 * FileAccessServiceImpl looks for them.
		 */
	...
	/**
	 * Performs the actual setting of a property with a specific name to a specific
	 * new value without asking <code>VetoableChangeListener</code>, storing into
	 * the configuration file and notifying {@link PropertyChangeListener}s.
	 *
	 * @param propertyName the name of the property which is to be set to a specific
	 *                     value.
	...
	/**
	 * Removes the property with the specified name. Calling this method would first
	 * trigger a PropertyChangeEvent that will be dispatched to all
	 * VetoableChangeListeners. In case no complaints (PropertyVetoException) have
	 * been received, the property will be actually changed and a
	...
	public List<String> getAllPropertyNames() {
		return Collections.unmodifiableList(Arrays.asList(store.getPropertyNames()));
	}
	...
	public List<String> getPropertyNamesByPrefix(String prefix, boolean exactPrefixMatch) {
		...
		getPropertyNamesByPrefix(prefix, exactPrefixMatch, store.getPropertyNames(), resultKeySet);
	 ...
	 * property names in the <tt>names</tt> array that partially or completely match
	 * the specified prefix. Depending on the value of the <tt>exactPrefixMatch</tt>
	 * parameter the method will (when false) or will not (when exactPrefixMatch is
	 * true) include property names that have prefixes longer than the specified
	 * <tt>prefix</tt> param.
	 *
	 * @param prefix           a String containing the prefix (the non dotted
	 *                         non-caps part of a property name) that we're looking
	 *                         for.
	 * @param exactPrefixMatch a boolean indicating whether the returned property
	 *                         names should all have a prefix that is an exact match
	 *                         of the the <tt>prefix</tt> param or whether
	 *                         properties with prefixes that contain it but are
	 *                         longer than it are also accepted.
	 * @param names            the list of names that we'd like to search.
	 *
	 * @return a reference to the updated result set.
	 */
	private Set<String> getPropertyNamesByPrefix(String prefix, boolean exactPrefixMatch, String[] names,
	 ...
	 *
	 * @param file a reference to the configuration file where properties should be
	 *             stored.
	...
	/**
	 * Returns the configuration file currently used by the implementation. If there
	 * is no such file or this is the first time we reference it a new one is
	 * created.
	 * 
	...
	private File getConfigurationFile() throws IOException {
		if (this.configurationFile == null) {
			 ...
			 * available in the store of this instance so that users don't have to ask the
			 * system properties again.
			 */
	...
	/**
	 * Determines the name and the format of the configuration file to be used and
	 * initializes the {@link #configurationFile} and {@link #store} fields of this
	 * instance.
	 */
	private void createConfigurationFile() throws IOException {

		/*
		 * Choose the format of the configuration file so that the performance-savvy
		 * properties format is used whenever possible and only go with the slow and fat
		 * XML format when necessary.
		 */
		File configurationFile = getConfigurationFile("xml", false);

		if (configurationFile == null) {
			/*
			 * It's strange that there's no configuration file name but let it play out as
			 * it did when the configuration file was in XML format.
			 */
			...
			/*
			 * Obviously, a file with the .properties extension is in the properties format.
			 * Since there's no file with the .xml extension, the case is simple.
			 */
			if (".properties".equalsIgnoreCase(extension)) {
				...
				/*
				 * But if we're told that the configuration file name is with the .xml
				 * extension, we may also have a .properties file or the .xml extension may be
				 * only the default and not forced on us so it may be fine to create a
				 * .properties file and use the properties format anyway.
				 */
				...
				/*
				 * If there's an actual file with the .properties extension, then we've
				 * previously migrated the configuration from the XML format to the properties
				 * format. We may have failed to delete the migrated .xml file but it's fine
				 * because the .properties file is there to signal that we have to use it
				...
				/*
				 * Otherwise, the lack of an existing .properties file doesn't help us much and
				 * we have the .xml extension for the file name so we have to determine whether
	...
	/**
	 * Returns a reference to the configuration file that the service should load.
	 * The method would try to load a file with the name sip-communicator.xml unless
	 * a different one is specified in the system property
	 * net.java.sip.communicator.PROPERTIES_FILE_NAME . The method would first try
	 * to load the file from the current directory if it exists this is not the case
	 * a load would be attempted from the $HOME/.sip-communicator directory. In case
	 * it was not found there either we'll look for it in all locations currently
	 * present in the $CLASSPATH. In case we find it in there we will copy it to the
	 * $HOME/.sip-communicator directory in case it was in a jar archive and return
	 * the reference to the newly created file. In case the file is to be found
	 * nowhere - a new empty file in the user home directory and returns a link to
	 * that one.
	 *
	 * @param extension the extension of the file name of the configuration file.
	 *                  The specified extension may not be taken into account if the
	 *                  the configuration file name is forced through a system
	 *                  property.
	 * @param create    <tt>true</tt> to create the configuration file with the
	...
	private File getConfigurationFile(String extension, boolean create) throws IOException {
		...
		if (configFileInUserHomeDir.exists()) {
			if (logger.isDebugEnabled())
				logger.debug(
						"Using config file in $HOME/.sip-communicator: " + configFileInUserHomeDir.getAbsolutePath());
		...
		try {
			...
			try {
				...
				/*
				 * Ignore it because it doesn't matter and, most importantly, it shouldn't
				 * prevent us from using the configuration file.
				 */
			}
		}
		return configFileInUserHomeDir;
	...
	/**
	 * Gets the <tt>ConfigurationStore</tt> <tt>Class</tt> to be used as the default
	 * when no specific <tt>ConfigurationStore</tt> <tt>Class</tt> is determined as
	 * necessary.
	 *
	 * @return the <tt>ConfigurationStore</tt> <tt>Class</tt> to be used as the
	 *         default when no specific <tt>ConfigurationStore</tt> <tt>Class</tt>
	...
	/**
	 * Copies the contents of a specific <code>InputStream</code> as bytes into a
	 * specific output <code>File</code>.
	 *
	 * @param inputStream the <code>InputStream</code> the contents of which is to
	 *                    be output in the specified <code>File</code>
	 * @param outputFile  the <code>File</code> to write the contents of the
	 *                    specified <code>InputStream</code> into
	 * @throws IOException
	 */
	...
	/**
	 * Returns the value of the specified java system property. In case the value
	 * was a zero length String or one that only contained whitespaces, null is
	 * returned. This method is for internal use only. Users of the configuration
	 * service are to use the getProperty() or getString() methods which would
	 * automatically determine whether a property is system or not.
	 * 
	 * @param propertyName the name of the property whose value we need.
	 * @return the value of the property with name propertyName or null if the value
	 *         had length 0 or only contained spaces tabs or new lines.
	 */
	...
	/**
	 * Returns the String value of the specified property (minus all encompassing
	 * whitespaces) and null in case no property value was mapped against the
	 * specified propertyName, or in case the returned property string had zero
	 * length or contained whitespaces only.
	 *
	 * @param propertyName the name of the property that is being queried.
	 * @return the result of calling the property's toString method and null in case
	 *         there was no value mapped against the specified.
	...
	/**
	 * Gets a (cached) reference to a <tt>FileAccessService</tt> implementation to
	 * be used by this <tt>ConfigurationService</tt> implementation.
	 *
	...
	public double getDouble(String propertyName, double defaultValue) {
		...
		if ((stringValue != null) && (stringValue.length() > 0)) {
			try {
				...
				logger.error(propertyName + " does not appear to be a double. " + "Defaulting to " + defaultValue + ".",
						ex);
			}
		}
		return doubleValue;
	...
	public long getLong(String propertyName, long defaultValue) {
		...
		if ((stringValue != null) && (stringValue.length() > 0)) {
			try {
				...
				logger.error(propertyName + " does not appear to be a long integer. " + "Defaulting to " + defaultValue
						+ ".", ex);
			}
		}
		return longValue;
	...
	/**
	 * Determines whether the property with the specified <tt>propertyName</tt> has
	 * been previously declared as System
	 *
	 * @param propertyName the name of the property to verify
	 * @return true if someone at some point specified that property to be system.
	 *         (This could have been either through a call to setProperty(string,
	 *         true)) or by setting the system attribute in the xml conf file to
	 *         true.
	 */
	...
	/**
	 * Goes over all system properties and outputs their names and values for debug
	 * purposes. The method has no effect if the logger is at a log level other than
	 * DEBUG or TRACE (FINE or FINEST). * Changed that system properties are printed
	 * in INFO level and this way they are included in the beginning of every users
	 * log file.
	 */
	...
	/**
	 * The method scans the contents of the SYS_PROPS_FILE_NAME_PROPERTY where it
	 * expects to find a comma separated list of names of files that should be
	 * loaded as system properties. The method then parses these files and loads
	 * their contents as system properties. All such files have to be in a location
	 ...
	 *
	 * @param clazz the {@link ConfigurationStore} that this configuration service
	 ...
	 * overrides them with the default override values.
	 */
	 ...
	 * directory. Typically this file is to be called for the default properties and
	 * the admin overrides.
	 *
