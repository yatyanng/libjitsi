...
public class MediaServiceImpl extends PropertyChangeNotifier implements MediaService {
	...
	/**
	 * The name of the <tt>boolean</tt> <tt>ConfigurationService</tt> property which
	 * indicates whether the detection of audio <tt>CaptureDevice</tt>s is to be
	 * disabled. The default value is <tt>false</tt> i.e. the audio
	...
	/**
	 * The name of the <tt>boolean</tt> <tt>ConfigurationService</tt> property which
	 * indicates whether the detection of video <tt>CaptureDevice</tt>s is to be
	 * disabled. The default value is <tt>false</tt> i.e. the video
	 * <tt>CaptureDevice</tt>s are detected.
	 */
	...
	/**
	 * The value of the <tt>devices</tt> property of <tt>MediaServiceImpl</tt> when
	 * no <tt>MediaDevice</tt>s are available. Explicitly defined in order to reduce
	 * unnecessary allocations.
	 */
	private static final List<MediaDevice> EMPTY_DEVICES = Collections.emptyList();

	/**
	 * The name of the <tt>System</tt> boolean property which specifies whether the
	 * committing of the JMF/FMJ <tt>Registry</tt> is to be disabled.
	 */
	private static final String JMF_REGISTRY_DISABLE_COMMIT = "net.sf.fmj.utility.JmfRegistry.disableCommit";

	/**
	 * The name of the <tt>System</tt> boolean property which specifies whether the
	 * loading of the JMF/FMJ <tt>Registry</tt> is to be disabled.
	 */
	 ...
	 * {@link #postInitializeOnce(MediaServiceImpl)} has been executed in order to
	...
	/**
	 * The value which will be used for the canonical end-point identifier (CNAME)
	 * in RTCP packets sent by this running instance of libjitsi.
	 */
	private static final String rtpCname = UUID.randomUUID().toString();

	/**
	 * The <tt>CaptureDevice</tt> user choices such as the default audio and video
	 * capture devices.
	 */
	 ...
	 * {@link MediaService#getDevices(MediaType, MediaUseCase)} method is called
	 * with an argument {@link MediaType#AUDIO}.
	 */
	...
	/**
	 * The <tt>MediaFormatFactory</tt> through which <tt>MediaFormat</tt> instances
	 * may be created for the purposes of working with the <tt>MediaStream</tt>s
	 * created by this <tt>MediaService</tt>.
	 */
	private MediaFormatFactory formatFactory;

	/**
	 * The one and only <tt>MediaDevice</tt> instance with <tt>MediaDirection</tt>
	 * not allowing sending and <tt>MediaType</tt> equal to <tt>AUDIO</tt>.
	 */
	private MediaDevice nonSendAudioDevice;

	/**
	 * The one and only <tt>MediaDevice</tt> instance with <tt>MediaDirection</tt>
	 * not allowing sending and <tt>MediaType</tt> equal to <tt>VIDEO</tt>.
	 */
	 ...
	 * {@link MediaService#getDevices(MediaType, MediaUseCase)} method is called
	 * with an argument {@link MediaType#VIDEO}.
	 */
	private final List<MediaDeviceImpl> videoDevices = new ArrayList<MediaDeviceImpl>();

	/**
	 * A {@link Map} that binds indicates whatever preferences this media service
	 * implementation may have for the RTP payload type numbers that get dynamically
	 * assigned to {@link MediaFormat}s with no static payload type. The method is
	 * useful for formats such as "telephone-event" for example that is statically
	 * assigned the 101 payload type by some legacy systems. Signalling protocol
	 * implementations such as SIP and XMPP should make sure that, whenever this is
	 * possible, they assign to formats the dynamic payload type returned in this
	 * {@link Map}.
	 */
	...
	/**
	 * Listeners interested in Recorder events without the need to have access to
	 * their instances.
	 */
	...
	public MediaServiceImpl() {
		/*
		 * XXX The deviceConfiguration is initialized and referenced by this instance so
		 * adding deviceConfigurationPropertyChangeListener does not need a matching
		 * removal.
		 */
	...
	/**
	 * Create a <tt>MediaStream</tt> which will use a specific <tt>MediaDevice</tt>
	 * for capture and playback of media. The new instance will not have a
	 * <tt>StreamConnector</tt> at the time of its construction and a
	 * <tt>StreamConnector</tt> will be specified later on in order to enable the
	 * new instance to send and receive media.
	 *
	 ...
	 * {@inheritDoc}
	 *
	 * Implements {@link MediaService#createMediaStream(MediaType)}. Initializes a
	 * new <tt>AudioMediaStreamImpl</tt> or <tt>VideoMediaStreamImpl</tt> in accord
	 * with <tt>mediaType</tt>
	 */
	@Override
	 ...
	 * <tt>MediaDevice</tt> for both capture and playback of media exchanged via the
	 * specified <tt>StreamConnector</tt>.
	 *
	 * @param connector the <tt>StreamConnector</tt> that the new
	 *                  <tt>MediaStream</tt> instance is to use for sending and
	 *                  receiving media
	 ...
	 *                    instance is to use for both capture and playback of media
	 *                    exchanged via the specified <tt>connector</tt>
	 * @param srtpControl a control which is already created, used to control the
	 *                    SRTP operations.
	 *
	 * @return a new <tt>MediaStream</tt> instance
	 * @see MediaService#createMediaStream(StreamConnector, MediaDevice)
	 ...
	 *                    instance to be initialized. If <tt>null</tt>,
	 *                    <tt>device</tt> must be non-<tt>null</tt> and its
	 *                    {@link MediaDevice#getMediaType()} will be used to
	 *                    determine the <tt>MediaType</tt> of the new instance. If
	 *                    non-<tt>null</tt>, <tt>device</tt> may be <tt>null</tt>.
	 *                    If non-<tt>null</tt> and <tt>device</tt> is
	 *                    non-<tt>null</tt>, the <tt>MediaType</tt> of
	 ...
	 *                    non-<tt>null</tt>
	 * @param srtpControl the <tt>SrtpControl</tt> to be used by the new instance if
	 *                    non-<tt>null</tt>
	 * @return a new <tt>MediaStream</tt> instance
	 */
	private MediaStream createMediaStream(MediaType mediaType, StreamConnector connector, MediaDevice device,
	...
	/**
	 * Creates a new <tt>MediaDevice</tt> which uses a specific <tt>MediaDevice</tt>
	 * to capture and play back media and performs mixing of the captured media and
	 * the media played back by any other users of the returned
	 * <tt>MediaDevice</tt>. For the <tt>AUDIO</tt> <tt>MediaType</tt>, the returned
	 * device is commonly referred to as an audio mixer. The <tt>MediaType</tt> of
	 * the returned <tt>MediaDevice</tt> is the same as the <tt>MediaType</tt> of
	 * the specified <tt>device</tt>.
	 *
	 * @param device the <tt>MediaDevice</tt> which is to be used by the returned
	 ...
	 *         capture and play back media and performs mixing of the captured media
	 *         and the media played back by any other users of the returned
	 ...
	 *
	 * @param mediaType a <tt>MediaType</tt> value indicating the type of media to
	 *                  be handled by the <tt>MediaDevice</tt> to be obtained
	 * @param useCase   the <tt>MediaUseCase</tt> to obtain the <tt>MediaDevice</tt>
	 *                  list for
	 * @return the default <tt>MediaDevice</tt> for the specified <tt>mediaType</tt>
	 *         if such a <tt>MediaDevice</tt> exists; otherwise, <tt>null</tt>
	...
	/**
	 * Gets the <tt>CaptureDevice</tt> user choices such as the default audio and
	 * video capture devices.
	 *
	 * @return the <tt>CaptureDevice</tt> user choices such as the default audio and
	 *         video capture devices.
	 */
	...
	/**
	 * Gets a list of the <tt>MediaDevice</tt>s known to this <tt>MediaService</tt>
	 * and handling the specified <tt>MediaType</tt>.
	 *
	 * @param mediaType the <tt>MediaType</tt> to obtain the <tt>MediaDevice</tt>
	 *                  list for
	 * @param useCase   the <tt>MediaUseCase</tt> to obtain the <tt>MediaDevice</tt>
	 *                  list for
	 ...
	 *         The returned <tt>List</tt> is a copy of the internal storage and,
	 *         consequently, modifications to it do not affect this instance.
	 *         Despite the fact that a new <tt>List</tt> instance is returned by
	 *         each call to this method, the <tt>MediaDevice</tt> instances are the
	 *         same if they are still known to this <tt>MediaService</tt> to be
	 *         available.
	...
	public List<MediaDevice> getDevices(MediaType mediaType, MediaUseCase useCase) {
		...
		/*
		 * If there are no MediaDevice instances of the specified mediaType, make sure
		 * that there is at least one MediaDevice which does not allow sending.
		 */
	...
	/**
	 * Returns the current encoding configuration -- the instance that contains the
	 * global settings. Note that any changes made to this instance will have
	 * immediate effect on the configuration.
	 *
	 * @return the current encoding configuration -- the instance that contains the
	 *         global settings.
	 */
	@Override
	 ...
	 *
	 * @return the <tt>MediaFormatFactory</tt> through which <tt>MediaFormat</tt>
	 *         instances may be created for the purposes of working with the
	 *         <tt>MediaStream</tt>s created by this <tt>MediaService</tt>
	 ...
	 * <tt>MediaDirection</tt> not allowing sending and <tt>MediaType</tt> equal to
	 * <tt>AUDIO</tt>.
	 *
	 * @return the one and only <tt>MediaDevice</tt> instance with
	 *         <tt>MediaDirection</tt> not allowing sending and <tt>MediaType</tt>
	 *         equal to <tt>AUDIO</tt>
	 */
	 ...
	 * output/playback.
	 *
	 * @return the <tt>VolumeControl</tt> which controls the volume level of audio
	 *         output/playback
	...
	/**
	 * Creates a new <tt>Recorder</tt> instance that can be used to record a call
	 * which captures and plays back media using a specific <tt>MediaDevice</tt>.
	 *
	 * @param device the <tt>MediaDevice</tt> which is used for media capture and
	 *               playback by the call to be recorded
	 * @return a new <tt>Recorder</tt> instance that can be used to record a call
	 *         which captures and plays back media using the specified
	 *         <tt>MediaDevice</tt>
	...
	/**
	 * Returns a {@link Map} that binds indicates whatever preferences this media
	 * service implementation may have for the RTP payload type numbers that get
	 * dynamically assigned to {@link MediaFormat}s with no static payload type. The
	 * method is useful for formats such as "telephone-event" for example that is
	 * statically assigned the 101 payload type by some legacy systems. Signaling
	 * protocol implementations such as SIP and XMPP should make sure that, whenever
	 * this is possible, they assign to formats the dynamic payload type returned in
	 * this {@link Map}.
	 *
	 * @return a {@link Map} binding some formats to a preferred dynamic RTP payload
	 *         type number.
	 */
	@Override
	public Map<MediaFormat, Byte> getDynamicPayloadTypePreferences() {
		if (dynamicPayloadTypePreferences == null) {
			...
			/*
			 * Set the dynamicPayloadTypePreferences to their default values. If the user
			 * chooses to override them through the ConfigurationService, they will be
	...
	/**
	 * Creates a preview component for the specified device(video device) used to
	 * show video preview from that device.
	 *
	...
	public Object getVideoPreviewComponent(MediaDevice device, int preferredWidth, int preferredHeight) {
		...
		try {
			...
			if ((device != null) && ((captureDeviceInfo = ((MediaDeviceImpl) device).getCaptureDeviceInfo()) != null)) {
				...
				Processor player = ForbiddenManager.createProcessor(dataSource);
				final VideoContainerHierarchyListener listener = new VideoContainerHierarchyListener(videoContainer,
						player);
	 ...
	 * @param listener       the hierarchy listener we created for the video
	 *                       container.
	 */
	private static void controllerUpdateForPreview(ControllerEvent event, JComponent videoContainer,
			MediaLocator locator, VideoContainerHierarchyListener listener) {
	...
	public MediaDevice getMediaDeviceForPartialDesktopStreaming(int width, int height, int x, int y) {
		...
		Format formats[] = new Format[] {
				new AVFrameFormat(size, Format.NOT_SPECIFIED, FFmpeg.PIX_FMT_ARGB, Format.NOT_SPECIFIED),
		...
		CaptureDeviceInfo devInfo = new CaptureDeviceInfo(name + " " + display,
				new MediaLocator(DeviceSystem.LOCATOR_PROTOCOL_IMGSTREAMING + ":" + display + "," + x + "," + y),
				formats);
	 ...
	 * @return true if <tt>MediaDevice</tt> is a partial desktop streaming device,
	 *         false otherwise
	 */
	@Override
	...
	/**
	 * Those interested in Recorder events add listener through MediaService. This
	 * way they don't need to have access to the Recorder instance. Adds a new
	 * <tt>Recorder.Listener</tt> to the list of listeners interested in
	 * notifications from a <tt>Recorder</tt>.
	 *
	 * @param listener the new <tt>Recorder.Listener</tt> to be added to the list of
	 *                 listeners interested in notifications from
	 *                 <tt>Recorder</tt>s.
	 */
	@Override
	 ...
	 *
	 * @param listener the existing <tt>Listener</tt> to be removed from the list of
	 ...
	 *
	 * @param event a <tt>PropertyChangeEvent</tt> which specifies the name of the
	...
	private void deviceConfigurationPropertyChange(PropertyChangeEvent event) {
		 ...
		 * AUDIO_PLAYBACK_DEVICE is not. Anyway, MediaDevice is supposed to represent
		 * the device to be used for capture AND playback (though its current
		 * implementation MediaDeviceImpl may be incomplete with respect to the playback
		 * representation). Since it is not clear at this point of the execution whether
		 * AUDIO_PLAYBACK_DEVICE really affects the DEFAULT_DEVICE and for the sake of
		 * completeness, throw in the changes to the AUDIO_NOTIFY_DEVICE as well.
		 */
		if (DeviceConfiguration.AUDIO_CAPTURE_DEVICE.equals(propertyName)
			...
			/*
			 * We do not know the old value of the property at the time of this writing. We
			 * cannot report the new value either because we do not know the MediaType and
			 * the MediaUseCase.
			 */
	 ...
	 *
	 * @return a new <tt>RTPTranslator</tt> which is to forward RTP and RTCP traffic
	 *         between multiple <tt>MediaStream</tt>s
	 ...
	 *                         caused the need to perform the one-time
	 *                         initialization
	 */
	private static void postInitializeOnce(MediaServiceImpl mediaServiceImpl) {
		/*
		 * Some SecureRandom() implementations like SHA1PRNG call /dev/random to seed
		 * themselves on first use. Call SecureRandom early to avoid blocking when
		 * establishing a connection for exemple.
		 */
	...
	/**
	 * Sets up FMJ for execution. For example, sets properties which instruct FMJ
	 * whether it is to create a log, where the log is to be created.
	 */
	private static void setupFMJ() {
		/*
		 * FMJ now uses java.util.logging.Logger, but only logs if allowLogging is set
		 * in its registry. Since the levels can be configured through properties for
		 * the net.sf.fmj.media.Log class, we always enable this (as opposed to only
		 * enabling it when this.logger has debug enabled).
		 */
		Registry.set("allowLogging", true);

		/*
		 * Disable the loading of .fmj.registry because Kertesz Laszlo has reported that
		 * audio input devices duplicate after restarting Jitsi. Besides, Jitsi does not
		 * really need .fmj.registry on startup.
		 */
		...
		if (cfg != null) {
			...
			for (String prop : cfg.getPropertyNamesByPrefix(
					"net.java.sip.communicator.impl.neomedia" + ".adaptive_jitter_buffer", true)) {
	...
	/**
	 * Determines whether the support for a specific <tt>MediaType</tt> is enabled.
	 * The <tt>ConfigurationService</tt> and <tt>System</tt> properties
	 * {@link #DISABLE_AUDIO_SUPPORT_PNAME} and {@link #DISABLE_VIDEO_SUPPORT_PNAME}
	 * allow disabling the support for, respectively, {@link MediaType#AUDIO} and
	 * {@link MediaType#VIDEO}.
	 *
	 * @param mediaType the <tt>MediaType</tt> to be determined whether the support
	 *                  for it is enabled
	 * @return <tt>true</tt> if the support for the specified <tt>mediaType</tt> is
	 ...
	 * Whether the container is displayable or not we will stop the player or start
	 * it.
	 */
	private static class VideoContainerHierarchyListener implements HierarchyListener {
		...
		/**
		 * The preview component of the player, must be set once the player has been
		 * realized.
		 */
		...
		public void dispose() {
			...
			/*
			 * We've just disposed the player which created the preview component so the
			 * preview component is of no use regardless of whether the Media configuration
			 * form will be redisplayed or not. And since the preview component appears to
			 * be a huge object even after its player is disposed, make sure to not
			 * reference it.
			 */
			if (preview != null)
