...
class PushSourceStreamDesc {
	...
	/**
	 * <tt>true</tt> if this instance represents a data/RTP stream or <tt>false</tt>
	 * if this instance represents a control/RTCP stream
	 */
	 ...
	 * which may provide Buffer properties other than <tt>data</tt>, <tt>length</tt>
	 * and <tt>offset</tt> such as <tt>flags</tt>.
	 */
	public final PushBufferStream streamAsPushBufferStream;

	/**
	 * Initializes a new <tt>PushSourceStreamDesc</tt> instance which is to describe
	 * a specific endpoint <tt>PushSourceStream</tt> for an
	 * <tt>RTPTranslatorImpl</tt>.
	 *
	 * @param connectorDesc the endpoint <tt>RTPConnector</tt> which owns the
	 *                      specified <tt>stream</tt>
	 * @param stream        the endpoint <tt>PushSourceStream</tt> to be described
	 *                      by the new instance for an <tt>RTPTranslatorImpl</tt>
	 * @param data          <tt>true</tt> if the specified <tt>stream</tt> is a
	 *                      data/RTP stream or <tt>false</tt> if the specified
	 *                      <tt>stream</tt> is a control/RTCP stream
	 */
	public PushSourceStreamDesc(RTPConnectorDesc connectorDesc, PushSourceStream stream, boolean data) {
		...
		streamAsPushBufferStream = (PushBufferStream) stream
				.getControl(AbstractPushBufferStream.PUSH_BUFFER_STREAM_CLASS_NAME);
	}
}
