...
public class RTCPFeedbackMessageSender {
	/**
	 * The <tt>Logger</tt> used by the <tt>RTCPFeedbackMessageSender</tt> class and
	 ...
	 * initialization of the class {@code RTCPFeedbackMessageSender} cached for the
	 * purposes of performance.
	 */
	private static final boolean TRACE = logger.isTraceEnabled();

	/**
	 * The interval in milliseconds at which we re-send an FIR, if the previous one
	 * was not satisfied.
	 */
	 ...
	 * <tt>RTCPFeedbackMessageSender</tt> sends RTCP feedback message packets. The
	 * synchronization source identifier (SSRC) of <tt>rtpTranslator</tt> is used as
	 * the SSRC of packet sender.
	 */
	 ...
	 *                      instance is to send RTCP feedback message packets and
	 *                      the SSRC of which is to be used as the SSRC of packet
	 *                      sender
	 */
	...
	/**
	 * Sends an RTCP Full Intra Request (FIR) or Picture Loss Indication (PLI), to
	 * the media sender/source with a specific synchronization source identifier
	 * (SSRC). Whether to send a FIR or a PLI message is decided based on whether
	 * the {@link MediaStream} associated with the SSRC supports FIR or PLI.
	 *
	...
	public boolean requestKeyframe(long mediaSenderSSRC) {
		...
		if (keyframeRequester == null) {
			...
			KeyframeRequester existingKfRequester = kfRequesters.putIfAbsent(mediaSenderSSRC, keyframeRequester);
	...
	/**
	 * Sends an RTCP Full Intra Request (FIR) or Picture Loss Indication (PLI), to
	 * media senders/sources with a specific synchronization source identifiers
	 * (SSRCs). Whether to send a FIR or a PLI message is decided based on whether
	 * the {@link MediaStream} associated with the SSRC supports FIR or PLI.
	 *
	 ...
	 *
	 * @param streamRTPManager a <tt>StreamRTPManagerDesc</tt> which identifies the
	 *                         peer from which an RTP packet has been received
	 * @param buf              the buffer which contains the bytes of the received
	 *                         RTP or RTCP packet
	 * @param off              the zero-based index in <tt>buf</tt> at which the
	 *                         bytes of the received RTP or RTCP packet begin
	...
	/**
	 * The <tt>KeyframeRequester</tt> is responsible for sending FIR requests to a
	...
	class KeyframeRequester extends PeriodicRunnable {
		 ...
		 *
		 * @param allowResetRemainingRetries true if it's allowed to reset the remaining
		...
		public boolean maybeRequest(boolean allowResetRemainingRetries) {
			synchronized (this) {
				...
				logger.info("Sending a FIR to ssrc=" + mediaSenderSSRC + " remainingRetries=" + remainingRetries);
			}
			...
			StreamRTPManager streamRTPManager = rtpTranslator.findStreamRTPManagerByReceiveSSRC((int) mediaSenderSSRC);
			...
			if (!videoMediaStream.supportsPli() && videoMediaStream.supportsFir()) {
				request = new RTCPFeedbackMessagePacket(RTCPFeedbackMessageEvent.FMT_FIR,
						RTCPFeedbackMessageEvent.PT_PS, senderSSRC, mediaSenderSSRC);
				request.setSequenceNumber(sequenceNumber.incrementAndGet());
			} else {
				request = new RTCPFeedbackMessagePacket(RTCPFeedbackMessageEvent.FMT_PLI,
						RTCPFeedbackMessageEvent.PT_PS, senderSSRC, mediaSenderSSRC);
