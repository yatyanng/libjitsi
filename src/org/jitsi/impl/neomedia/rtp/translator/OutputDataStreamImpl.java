...
class OutputDataStreamImpl implements OutputDataStream, Runnable {
	/**
	 * The <tt>Logger</tt> used by the <tt>OutputDataStreamImpl</tt> class and its
	 ...
	 * indicates whether the RTP header extension(s) are to be removed from received
	 * RTP packets prior to relaying them. The default value is <tt>false</tt>.
	 */
	private static final String REMOVE_RTP_HEADER_EXTENSIONS_PNAME = RTPTranslatorImpl.class.getName()
	...
	/**
	 * The indicator which determines whether the RTP header extension(s) are to be
	 * removed from received RTP packets prior to relaying them. The default value
	 * is <tt>false</tt>.
	 */
	 ...
	 * risks. I do NOT want to use {@code CopyOnWriteArrayList} because I want to
	 * (1) avoid {@code Iterator}s and (2) reduce synchronization. The access to
	...
	public OutputDataStreamImpl(RTPConnectorImpl connector, boolean data) {
		...
		_removeRTPHeaderExtensions = ConfigUtils.getBoolean(LibJitsi.getConfigurationService(),
				REMOVE_RTP_HEADER_EXTENSIONS_PNAME, false);
	...
	/**
	 * Adds a new {@code OutputDataStream} to the list of {@code OutputDataStream}s
	 * into which this {@code OutputDataStream} copies written data/packets. If this
	 * instance contains the specified {@code stream} already, does nothing.
	 *
	...
	private int doWrite(byte[] buf, int off, int len, Format format, StreamRTPManagerDesc exclusion) {
		...
		for (int i = 0, end = streams.size(); i < end; ++i) {
			...
			if (write) {
				...
				write = translator.willWrite(/* source */ exclusion, new RawPacket(buf, off, len),
						/* destination */ streamRTPManager, _data);
			}
	 ...
	 *
	 * @param buf the <tt>byte</tt>s of a datagram packet which may contain an RTP
	 *            packet
	 ...
	 * @return the number of <tt>byte</tt>s in <tt>buf</tt> starting at <tt>off</tt>
	 *         comprising the actual data after the possible removal of the RTP
	 *         header extension(s)
	 */
	 ...
	 * {@code RTPConnector} from the list of {@code OutputDataStream}s into which
	 * this {@code OutputDataStream} copies written data/packets.
	 *
	...
	/**
	 * Notifies this instance that a specific <tt>byte</tt> buffer will be written
	 * into the control <tt>OutputDataStream</tt> of a specific
	 * <tt>StreamRTPManagerDesc</tt>.
	 *
	 * @param destination the <tt>StreamRTPManagerDesc</tt> which is the destination
	 *                    of the write
	 ...
	 * @param length      the number of <tt>byte</tt>s in <tt>buffer</tt> beginning
	 *                    at <tt>offset</tt> which constitute the data to the
	 *                    written into <tt>destination</tt>
	 * @param format      the FMJ <tt>Format</tt> of the data to be written into
	 *                    <tt>destination</tt>
	 * @param exclusion   the <tt>StreamRTPManagerDesc</tt> which is exclude from
	 *                    the write batch, possibly because it is the cause of the
	 *                    write batch in the first place
	 ...
	 */
	private boolean willWriteControl(StreamRTPManagerDesc destination, byte[] buffer, int offset, int length,
					...
					if (rtcpLength <= length) {
						...
						if (destination.containsReceiveSSRC(ssrcOfMediaSource)) {
							if (logger.isTraceEnabled()) {
								...
								String message = getClass().getName() + ".willWriteControl: FMT " + fmt + ", PT " + pt
										+ ", SSRC of packet sender " + Long.toString(ssrcOfPacketSender & 0xffffffffl)
										+ ", SSRC of media source " + Long.toString(ssrcOfMediaSource & 0xffffffffl);
	...
	/**
	 * Notifies this instance that a specific <tt>byte</tt> buffer will be written
	 * into the data <tt>OutputDataStream</tt> of a specific
	 * <tt>StreamRTPManagerDesc</tt>.
	 *
	 * @param destination the <tt>StreamRTPManagerDesc</tt> which is the destination
	 *                    of the write
	 ...
	 *         data into the specified <tt>destination</tt>
	 */
	private boolean willWriteData(StreamRTPManagerDesc destination, byte[] buf, int off, int len, Format format,
			StreamRTPManagerDesc exclusion) {
