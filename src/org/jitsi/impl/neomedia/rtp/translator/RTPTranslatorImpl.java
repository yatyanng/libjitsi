...
public class RTPTranslatorImpl extends AbstractRTPTranslator implements ReceiveStreamListener {
	 ...
	 * @param methodName the name of the method on <tt>obj</tt> which is the source
	 *                   of the log request
	 ...
	 *                   begins
	 * @param len        the number of bytes in <tt>buf</tt> which constitute the
	 *                   valid data
	 */
				...
				if (pt == 203 /* BYE */) {
					...
					if (rtcpLength <= len) {
						...
						for (int i = 0, end = off + len; i < sc && o + 4 <= end; ++i, o += 4) {
							...
							LOGGER.trace(obj.getClass().getName() + '.' + methodName + ": RTCP BYE SSRC/CSRC "
	...
	/**
	 * The <tt>RTPManager</tt> which implements the actual RTP management of this
	 * instance.
	 */
	...
	/**
	 * The list of <tt>StreamRTPManager</tt>s i.e. <tt>MediaStream</tt>s which this
	 * instance forwards RTP and RTCP traffic between.
	 */
	 ...
	 * <tt>Format</tt>. The association between the specified <tt>format</tt> and
	 * the specified <tt>payloadType</tt> is being added by a specific
	 * <tt>StreamRTPManager</tt> but effects the <tt>RTPTranslatorImpl</tt>
	 * globally.
	 * 
	 * @param streamRTPManager the <tt>StreamRTPManager</tt> that is requesting the
	 *                         association of <tt>format</tt> to
	 *                         <tt>payloadType</tt>
	 * @param format           the <tt>Format</tt> which is to be associated with
	 *                         the specified RTP payload type (number)
	 ...
	 *
	 * @param streamRTPManager the <tt>StreamRTPManager</tt> which specifies the
	 *                         neomedia <tt>MediaStream</tt> with which the
	 *                         <tt>ReceiveStreamEvent</tt>s delivered to the
	 *                         specified <tt>listener</tt> are to be related. In
	 *                         other words, a <tt>ReceiveStreamEvent</tt> received
	 *                         by <tt>RTPTranslatorImpl</tt> is first examined to
	 *                         determine which <tt>StreamRTPManager</tt> it is
	 *                         related to and then it is delivered to the
	 *                         <tt>ReceiveStreamListener</tt>s which have been added
	 *                         to this <tt>RTPTranslatorImpl</tt> by that
	 *                         <tt>StreamRTPManager</tt>.
	 * @param listener         the <tt>ReceiveStreamListener</tt> to be notified
	 *                         about <tt>ReceiveStreamEvent</tt>s related to the
	 ...
	 * received by this <tt>RTPTranslatorImpl</tt>. Though the request is being made
	 * by a specific <tt>StreamRTPManager</tt>, the addition of the specified
	 * <tt>listener</tt> and the deliveries of the <tt>RemoteEvent</tt>s are
	 * performed irrespective of any <tt>StreamRTPManager</tt>.
	 *
	 * @param streamRTPManager the <tt>StreamRTPManager</tt> which is requesting the
	 *                         addition of the specified <tt>RemoteListener</tt>
	 ...
	 *
	 * @param sendStreamDesc a <tt>SendStreamDesc</tt> instance that specifies the
	 ...
	 *
	 * @param streamRTPManager the <tt>StreamRTPManager</tt> which is requesting the
	 *                         creation of a <tt>SendStream</tt>. Since multiple
	 *                         <tt>StreamRTPManager</tt> may request the creation of
	 *                         a <tt>SendStream</tt> from one and the same
	 *                         combination of <tt>dataSource</tt> and
	 *                         <tt>streamIndex</tt>, the method may not create a
	 *                         completely new <tt>SendStream</tt> but may return a
	 *                         <tt>StreamRTPManager</tt>-specific view of an
	 *                         existing <tt>SendStream</tt>.
	 * @param dataSource       the <tt>DataSource</tt> which provides the stream
	 *                         from which a <tt>SendStream</tt> is to be created
	 * @param streamIndex      the zero-based position within the array/list of
	 *                         streams of the specified <tt>dataSource</tt> of the
	 *                         stream from which a <tt>SendStream</tt> is to be
	 *                         created
	 * @return a <tt>SendStream</tt> created from the specified <tt>dataSource</tt>
	 *         and <tt>streamIndex</tt>. The returned <tt>SendStream</tt>
	 *         implementation is a <tt>streamRTPManager</tt>-dedicated view to an
	 *         actual <tt>SendStream</tt> which may have been created during a
	...
	/**
	 * Notifies this instance that an RTP or RTCP packet has been received from a
	 * peer represented by a specific <tt>PushSourceStreamDesc</tt>.
	 *
	 * @param streamDesc a <tt>PushSourceStreamDesc</tt> which identifies the peer
	 *                   from which an RTP or RTCP packet has been received
	 ...
	 *                   <tt>off</tt> which represent the received RTP or RTCP
	 *                   packet
	 * @param flags      <tt>Buffer.FLAG_XXX</tt>
	 * @return the number of bytes in <tt>buf</tt> beginning at <tt>off</tt> which
	 *         represent the received RTP or RTCP packet
	...
	int didRead(PushSourceStreamDesc streamDesc, byte[] buf, int off, int len, int flags) throws IOException {
		...
		try {
			...
			if (data) {
				...
				// RTP packet?
				if ((len >= RTPHeader.SIZE) && (/* v */ ((buf[off] & 0xc0) >>> 6) == RTPHeader.VERSION)) {
					...
					rtcpFeedbackMessageSender.maybeStopRequesting(streamRTPManager, ssrc & 0xffff_ffffL, buf, off, len);
				}
			} else if (LOGGER.isTraceEnabled()) {
				logRTCP(this, "read", buf, off, len);
			}

			OutputDataStreamImpl outputStream = data ? connector.getDataOutputStream()
	 ...
	 * execution and prepares that <tt>StreamRTPManager</tt> to be garbage collected
	 * (as far as this <tt>RTPTranslatorImpl</tt> is concerned).
	 */
	...
	public StreamRTPManager findStreamRTPManagerByReceiveSSRC(int receiveSSRC) {
		StreamRTPManagerDesc desc = findStreamRTPManagerDescByReceiveSSRC(receiveSSRC, /* exclusion */ null);
	 ...
	 * @param exclusion   the <tt>StreamRTPManager</tt>, if any, to be excluded from
	 *                    the search
	 ...
	 * <tt>MediaStream</tt> (specified in the form of a <tt>StreamRTPManager</tt>
	 * instance for the purposes of and in the terms of <tt>RTPManagerImpl</tt>).
	 *
	 * @param streamRTPManager the <tt>StreamRTPManager</tt> to which the returned
	...
	/**
	 * Gets the <tt>RTCPFeedbackMessageSender</tt> which should be used for sending
	 * RTCP Feedback Messages from this <tt>RTPTranslator</tt>.
	 * 
	 ...
	 *                         neomedia <tt>MediaStream</tt> with which the
	 *                         <tt>ReceiveStreamEvent</tt>s delivered to the
	 *                         specified <tt>listener</tt> are to be related
	 * @param listener         the <tt>ReceiveStreamListener</tt> to no longer be
	 *                         notified about <tt>ReceiveStreamEvent</tt>s related
	 *                         to the specified <tt>streamRTPManager</tt>
	 */
	 ...
	 * <tt>RemoteEvent</tt>s received by this <tt>RTPTranslatorImpl</tt>. Though the
	 * request is being made by a specific <tt>StreamRTPManager</tt>, the addition
	 * of the specified <tt>listener</tt> and the deliveries of the
	 * <tt>RemoteEvent</tt>s are performed irrespective of any
	 * <tt>StreamRTPManager</tt> so the removal follows the same logic.
	 *
	 * @param streamRTPManager the <tt>StreamRTPManager</tt> which is requesting the
	 *                         removal of the specified <tt>RemoteListener</tt>
	 * @param listener         the <tt>RemoteListener</tt> to no longer be notified
	 *                         about <tt>RemoteEvent</tt>s received by this
	 *                         <tt>RTPTranslatorImpl</tt>
	 */
	 ...
	 * {@link #addSendStreamListener(StreamRTPManager, SendStreamListener)} is not
	...
	/**
	 * Notifies this <tt>ReceiveStreamListener</tt> about a specific event related
	 * to a <tt>ReceiveStream</tt>.
	 *
	 * @param event a <tt>ReceiveStreamEvent</tt> which contains the specifics of
	 *              the event this <tt>ReceiveStreamListener</tt> is being notified
	 *              about
	...
	public void update(ReceiveStreamEvent event) {
		/*
		 * Because NullPointerException was seen during testing, be thorough with the
		 * null checks.
		 */
		if (event != null) {
	 ...
	 * @param pkt         the packet from the <tt>source</tt> which are to be
	 *                    written into the <tt>destination</tt>
	 * @param destination the destination into which <tt>buffer</tt> is to be
	 *                    written
	 * @param data        <tt>true</tt> for data/RTP or <tt>false</tt> for
	 *                    control/RTCP
	 * @return <tt>true</tt> if the writing is to continue or <tt>false</tt> if the
	 *         writing is to abort
	 */
