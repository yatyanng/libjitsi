...
class PushSourceStreamImpl implements PushSourceStream, Runnable, SourceTransferHandler {
	/**
	 * The <tt>Logger</tt> used by the <tt>PushSourceStreamImpl</tt> class and its
	 ...
	 * <tt>SourcePacket</tt> from {@link #readQ} after a <tt>SourcePacket</tt> was
	 * written there.
	 */
	private boolean _read = false;

	/**
	 * The <tt>Queue</tt> of <tt>SourcePacket</tt>s to be read out of this instance
	 * via {@link #read(byte[], int, int)}.
	 */
	...
	/**
	 * The pool of <tt>SourcePacket</tt> instances to reduce their allocations and
	 * garbage collection.
	 */
	private final Queue<SourcePacket> sourcePacketPool = new LinkedBlockingQueue<>(
			RTPConnectorOutputStream.POOL_CAPACITY);
	 ...
	 * {@inheritDoc}
	 *
	 * Implements {@link SourceTransferHandler#transferData(PushSourceStream)}. This
	 * instance sets itself as the <tt>transferHandler</tt> of all
	 * <tt>PushSourceStream</tt>s that get added to it (i.e. {@link #streams}). When
	 * either one of these pushes media data, this instance pushes that media data.
	 */
	@Override
				...
				synchronized (readQ) {
					...
					if (readQ.size() >= readQCapacity) {
						...
						if (RTPConnectorOutputStream.logDroppedPacket(numDroppedPackets)) {
							logger.warn(
									"Dropped " + numDroppedPackets + " packets " + "hashCode=" + hashCode() + "): ");
						}
					}
