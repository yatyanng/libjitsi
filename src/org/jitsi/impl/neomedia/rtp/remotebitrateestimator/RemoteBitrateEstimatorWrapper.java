...
public class RemoteBitrateEstimatorWrapper extends SinglePacketTransformerAdapter
	...
	/**
	 * The {@link Logger} used by the {@link RemoteBitrateEstimatorWrapper} class to
	 ...
	 */
	public final static String ENABLE_AST_RBE_PNAME = "org.jitsi.impl.neomedia.rtp.ENABLE_AST_RBE";
	 ...
	 * estimator.
	 */
	private static final boolean ENABLE_AST_RBE = cfg != null
			? cfg.getBoolean(ENABLE_AST_RBE_PNAME, ENABLE_AST_RBE_DEFAULT)
			: ENABLE_AST_RBE_DEFAULT;
	...
	/**
	 * Counts packets without the AST header extension. After {@link #SS_THRESHOLD}
	 * many packets we switch back to the single stream RBE.
	 */
	 ...
	 *                          changes.
	 * @param diagnosticContext the {@link DiagnosticContext} to be used by this
	 *                          instance.
	 */
	...
	public RawPacket reverseTransform(RawPacket pkt) {
		...
		if (ext != null) {
			...
			if (!usingAbsoluteSendTime) {
				...
				rbe = new RemoteBitrateEstimatorAbsSendTime(observer, diagnosticContext);
		...
		if (!usingAbsoluteSendTime) {
			incomingPacketInfo(System.currentTimeMillis(), pkt.getTimestamp(), pkt.getPayloadLength(),
					pkt.getSSRCAsLong());

			return pkt;
		}

		long sendTime24bits = astExtensionID == -1 ? -1 : AbsSendTimeEngine.getAbsSendTime(pkt, (byte) astExtensionID);

		if (usingAbsoluteSendTime && sendTime24bits != -1) {
			incomingPacketInfo(System.currentTimeMillis(), sendTime24bits, pkt.getPayloadLength(), pkt.getSSRCAsLong());
		}
