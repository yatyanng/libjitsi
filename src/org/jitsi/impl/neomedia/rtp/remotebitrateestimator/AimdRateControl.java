...
class AimdRateControl {
	...
	private long additiveRateIncrease(long nowMs, long lastMs, long responseTimeMs) {
		...
		double bitsPerFrame = currentBitrateBps / 30.0;
	...
	private long changeBitrate(long currentBitrateBps, long incomingBitrateBps, long nowMs) {
		...
		case kRcIncrease: {
			if (avgMaxBitrateKbps >= 0F && incomingBitrateKbps > avgMaxBitrateKbps + 3F * stdMaxBitRate) {
			...
			{
				long multiplicativeIncreaseBps = multiplicativeRateIncrease(nowMs, timeLastBitrateChange,
						currentBitrateBps);
	...
	private void changeRegion(RateControlRegion region, long nowMs) {
		...
		if (logger.isTraceEnabled()) {
			logger.trace(diagnosticContext.makeTimeSeriesPoint("aimd_region", nowMs).addField("aimd_id", hashCode())
	...
	private void changeState(RateControlState newState, long nowMs) {
		...
		if (logger.isTraceEnabled()) {
			logger.trace(diagnosticContext.makeTimeSeriesPoint("aimd_state", nowMs).addField("aimd_id", hashCode())
	...
	/**
	 * Returns <tt>true</tt> if the bitrate estimate hasn't been changed for more
	 * than an RTT, or if the <tt>incomingBitrate</tt> is more than 5% above the
	 * current estimate. Should be used to decide if we should reduce the rate
	 * further when over-using.
	 *
	 * @param timeNow
	...
	/**
	 * Returns <tt>true</tt> if there is a valid estimate of the incoming bitrate,
	...
	public void setRtt(long rtt) {
		if (logger.isTraceEnabled()) {
			logger.trace(diagnosticContext.makeTimeSeriesPoint("aimd_rtt", System.currentTimeMillis())
					.addField("aimd_id", hashCode()).addField("rtt", rtt));
		}
	...
	public void update(RateControlInput input, long nowMs) {
		...
		if (!bitrateIsInitialized) {
			...
			} else if (nowMs - timeFirstIncomingEstimate > kInitializationTimeMs && input.incomingBitRate > 0L) {
	...
	public long updateBandwidthEstimate(long nowMs) {
		...
		if (logger.isTraceEnabled() && isValidEstimate()) {
			logger.trace(diagnosticContext.makeTimeSeriesPoint("aimd_estimate", nowMs).addField("aimd_id", hashCode())
	...
	private void updateMaxBitRateEstimate(float incomingBitrateKbps) {
		...
		varMaxBitrateKbps = (1 - alpha) * varMaxBitrateKbps
				+ alpha * (avgMaxBitrateKbps - incomingBitrateKbps) * (avgMaxBitrateKbps - incomingBitrateKbps) / norm;
