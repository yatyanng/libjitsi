...
class OveruseDetector {
	 ...
	 * offset. {@code timestampDelta} is the delta between the last timestamp which
	 * the estimated offset is based on and the last timestamp on which the last
	 * offset was based on, representing the time between detector updates.
	 * {@code numOfDeltas} is the number of deltas the offset estimate is based on.
	...
	public BandwidthUsage detect(double offset, double tsDelta, int numOfDeltas, long nowMs) {
		...
		if (newHypothesis && logger.isTraceEnabled()) {
			logger.trace(diagnosticContext.makeTimeSeriesPoint("utilization_hypothesis", nowMs)
					.addField("detector", hashCode()).addField("offset", offset).addField("prev_offset", prev_offset)
					.addField("T", T).addField("threshold", threshold).addField("hypothesis", hypothesis.getValue()));
		}
