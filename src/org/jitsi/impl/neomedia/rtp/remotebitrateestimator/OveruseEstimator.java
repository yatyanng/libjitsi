...
class OveruseEstimator {
	 ...
	 *
	 * @param matrix the <tt>double</tt> two-dimensional matrix to create and return
	 *               a deep copy of
	 ...
	 * Original c++ code uses std::list<double> but in Java this translate to
	 * {@code List<Double>} which causes a lot of autoboxing
	 */
	...
	public OveruseEstimator(OverUseDetectorOptions options, @NotNull DiagnosticContext diagnosticContext) {
		...
		/**
		 * Initialize {@link tsDeltaHist} with {@code Double.MAX_VALUE} to simplify
		 * {@link updateMinFramePeriod}
		 */
	 ...
	 * {@code currentHypothesis} should be the hypothesis of the over-use detector
	 * at this time.
	 *
	 * @param tDelta
	 * @param tsDelta
	...
	public void update(long tDelta, double tsDelta, int sizeDelta, BandwidthUsage currentHypothesis,
		...
		boolean positiveSemiDefinite = E[0][0] + E[1][1] >= 0 && E[0][0] * E[1][1] - E[0][1] * E[1][0] >= 0
				&& E[0][0] >= 0;
		...
		if (logger.isTraceEnabled()) {
			logger.trace(diagnosticContext.makeTimeSeriesPoint("delay_variation_estimation", systemTimeMs)
					.addField("estimator", hashCode()).addField("time_delta", tDelta).addField("ts_delta", tsDelta)
					.addField("tts_delta", tTsDelta).addField("offset", offset)
	...
	private double updateMinFramePeriod(double tsDelta) {
		/**
		 * Change from C++ version: We use {@link tsDeltaHist} as a circular buffer
		 * initialized with {@code Double.MAX_VALUE}, so we insert new {@link tsDelta}
		 * value at {@link tsDeltaHistInsIdx} and we search for the minimum value in
		 * {@link tsDeltaHist}
		 */
