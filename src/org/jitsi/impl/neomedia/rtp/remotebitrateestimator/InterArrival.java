...
class InterArrival {
	 ...
	 * most {@code timestampGroupLengthTicks} older than the first timestamp in that
	 * group.
	 *
	...
	/**
	 * Returns {@code true} if a delta was computed, or {@code false} if the current
	 ...
	 *                      {@code arrivalTimeDeltaMs} is the computed arrival-time
	 *                      delta, {@code packetSizeDelta} is the computed size
	 *                      delta.
	 * @return
	 *
	 * @Note: We have two {@code computeDeltas}. One with a valid
	 *        {@code systemTimeMs} according to webrtc implementation as of June
	 *        12,2017 and a previous one with a default systemTimeMs (-1L). the
	 *        later may be removed or deprecated.
	 */
	...
	public boolean computeDeltas(long timestamp, long arrivalTimeMs, int packetSize, long[] deltas, long systemTimeMs) {
		...
		if (currentTimestampGroup.isFirstPacket()) {
			...
			if (prevTimestampGroup.completeTimeMs >= 0) {
				/* long timestampDelta */ deltas[0] = TimestampUtils
						.subtractAsUnsignedInt32(currentTimestampGroup.timestamp, prevTimestampGroup.timestamp);

				long arrivalTimeDeltaMs = deltas[1] = currentTimestampGroup.completeTimeMs
				...
				if (prevTimestampGroup.lastSystemTimeMs != -1L && currentTimestampGroup.lastSystemTimeMs != -1L
						&& arrivalTimeDeltaMs - systemTimeDeltaMs >= kArrivalTimeOffsetThresholdMs) {
				...
				/* int packetSizeDelta */ deltas[2] = (int) (currentTimestampGroup.size - prevTimestampGroup.size);

				if (logger.isTraceEnabled()) {
					logger.trace(diagnosticContext.makeTimeSeriesPoint("computed_deltas")
							.addField("inter_arrival", hashCode()).addField("arrival_time_ms", arrivalTimeMs)
							.addField("timestamp_delta", deltas[0]).addField("arrival_time_ms_delta", deltas[1])
	...
	/**
	 * Returns {@code true} if the last packet was the end of the current batch and
	 * the packet with {@code timestamp} is the first of a new batch.
	 *
	 * @param arrivalTimeMs
	 * @param timestamp
	 * @return
	...
	private boolean isNewTimestampGroup(long arrivalTimeMs, long timestamp) {
		if (currentTimestampGroup.isFirstPacket()) {
			...
			long timestampDiff = TimestampUtils.subtractAsUnsignedInt32(timestamp,
					currentTimestampGroup.firstTimestamp);
	...
	/**
	 * Returns {@code true} if the packet with timestamp {@code timestamp} arrived
	 * in order.
	 *
	 * @param timestamp
	 * @return
	...
	private boolean isPacketInOrder(long timestamp) {
		if (currentTimestampGroup.isFirstPacket()) {
			...
			if (!inOrder && logger.isTraceEnabled()) {
				logger.trace(diagnosticContext.makeTimeSeriesPoint("reordered_packet")
						.addField("inter_arrival", hashCode()).addField("ts_delta_ms", tsDeltaMs));
			}
	...
	private static class TimestampGroup {
		 ...
		 * of this {@code TimestampGroup}.
		 *
		 * @param source the {@code TimestampGroup} the values of the fields of which
