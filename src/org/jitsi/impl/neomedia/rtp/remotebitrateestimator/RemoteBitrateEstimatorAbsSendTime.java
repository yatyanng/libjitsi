...
/**
 * webrtc.org abs_send_time implementation as of June 26, 2017. commit ID:
 * 23fbd2aa2c81d065b84d17b09b747e75672e1159
 *
...
public class RemoteBitrateEstimatorAbsSendTime implements RemoteBitrateEstimator {
	/**
	 * The <tt>Logger</tt> used by the <tt>RemoteBitrateEstimatorAbsSendTime</tt>
	 * class and its instances for logging output.
	 */
	private static final Logger logger = Logger.getLogger(RemoteBitrateEstimatorAbsSendTime.class);
	...
	private static final TimeSeriesLogger timeSeriesLogger = TimeSeriesLogger
			.getTimeSeriesLogger(RemoteBitrateEstimatorAbsSendTime.class);

	/**
	 * Defines the number of digits in the AST representation (24 bits, 6.18 fixed
	 * point) after the radix.
	 */
	private final static int kAbsSendTimeFraction = 18;

	/**
	 * Defines the upshift (left bit-shift) to apply to AST (24 bits, 6.18 fixed
	 * point) to make it inter-arrival compatible (expanded AST, 32 bits, 6.26 fixed
	 * point).
	 */
	private final static int kAbsSendTimeInterArrivalUpshift = 8;

	/**
	 * This is used in the {@link InterArrival} computations. In this estimator a
	 * timestamp group is defined as all packets with a timestamp which are at most
	 * 5ms older than the first timestamp in that group.
	 */
	...
	/**
	 * Converts the {@link #kTimestampGroupLengthMs} into "ticks" for use with the
	 * {@link InterArrival}.
	 */
	private static final long kTimestampGroupLengthTicks = (kTimestampGroupLengthMs << kInterArrivalShift) / 1000;

	/**
	 * Defines the expanded AST (32 bits) to millis conversion rate. Units are ms
	 * per timestamp
	 */
	private static final double kTimestampToMs = (double) 1000 / (1 << kInterArrivalShift);

	/**
	 * Defines the maximum distance between the send time delta (of a probe packet
	 * from the previous probe packet) and the cluster mean send time delta above
	 * which the probe is no longer considered part of the cluster.
	 */
	private final static float kMaxSendTimeDeltaMsDistance = 2.5f;

	/**
	 * Defines the minimum size of a packet to be considered a probe. We currently
	 * assume that only packets larger than 200 bytes are paced by the sender.
	 */
	 ...
	 * default values used to initialize the field are of no importance because they
	 * will be overwritten before they are actually used.)
	 */
	 ...
	 * {@link #ssrcsMap} suitable for public access and introduced for the purposes
	 * of reducing the number of allocations and the effects of garbage collection.
	 */
	...
	/**
	 * The time (in millis) when we saw the first packet. Useful to determine the
	 * probing period.
	 */
	...
	/**
	 * The rate control implementation based on additive increases of bitrate when
	 * no over-use is detected and multiplicative decreases when over-uses are
	 * detected.
	 */
	 ...
	 * Ctor.
	 *
	 * @param observer          the observer to notify on bitrate estimation
	 *                          changes.
	...
	/**
	 * Determines whether a {@link Probe} belongs to a cluster or not, by examining
	 * its send time delta distance from the cluster mean.
	 *
	...
	private static boolean isWithinClusterBounds(long sendDeltaMs, Cluster clusterAggregate) {
		...
		double clusterMean = clusterAggregate.meanSendDeltaMs / clusterAggregate.count;

		return Math.abs(sendDeltaMs - clusterMean) < kMaxSendTimeDeltaMsDistance;
	}

	/**
	 * Finalizes the cluster (computes the means from the sums) and adds it to the
	 * clusters list.
	 *
		...
		for (Cluster cluster : clusters) {
			...
			if (cluster.numAboveMinDelta > cluster.count / 2
				...
				if (logger.isDebugEnabled()) {
					logger.debug("Probe failed, sent at " + sendBitrateBps + " bps, received at " + recvBitrateBps
							+ " bps. Mean send delta: " + cluster.meanSendDeltaMs + " ms, mean recv delta: "
							+ cluster.meanRecvDeltaMs + " ms, num probes: " + cluster.count);
				}
				break;
			}
		}
	...
	/**
	 * Processes the received clusters and maybe updates the remote bitrate. It
	...
	private synchronized boolean processClusters(long nowMs) {
		...
		if (isBitrateImproving(probeBitrateBps)) {
			if (logger.isDebugEnabled()) {
				logger.debug("Probe successful, sent at " + bestProbe.getSendBitrateBps() + " bps, received at "
						+ bestProbe.getRecvBitrateBps() + " bps. Mean send delta: " + bestProbe.meanSendDeltaMs
						+ " ms, mean recv delta: " + bestProbe.meanRecvDeltaMs + " ms, num probes: " + bestProbe.count);
			}
	...
	/**
	 * Determines whether or not the specified bitrate is an improvement over our
	 * current estimate.
	 *
	 ...
	 * @param sendTime24bits the send time of the packet in AST format (24 bits,
	 *                       6.18 fixed point).
	...
	public void incomingPacketInfo(long arrivalTimeMs, long sendTime24bits, int payloadSize, long ssrc) {
		...
		if (timeSeriesLogger.isTraceEnabled()) {
			timeSeriesLogger.trace(diagnosticContext.makeTimeSeriesPoint("in_pkt", nowMs).addField("rbe_id", hashCode())
					.addField("recv_ts_ms", arrivalTimeMs).addField("send_ts_ms", sendTimeMs)
					.addField("pkt_sz_bytes", payloadSize).addField("ssrc", ssrc));
		}
		...
		synchronized (this) {
			...
			if (payloadSize > kMinProbePacketSize
				...
				if (totalProbesReceived < kMaxProbePackets) {
					...
					if (logger.isDebugEnabled()) {
						logger.debug(
								"Probe packet received: send time=" + sendTimeMs + " ms, recv time=" + arrivalTimeMs
										+ " ms, send delta=" + sendDeltaMs + " ms, recv delta=" + recvDeltaMs + " ms.");
					}
				}
			...
			if (detector.interArrival.computeDeltas(timestamp, arrivalTimeMs, payloadSize, deltas, nowMs)) {
				...
				detector.estimator.update(/* timeDelta */ deltas[1], /* timestampDelta */ tsDeltaMs,
						/* sizeDelta */ (int) deltas[2], detector.detector.getState(), nowMs);

				detector.detector.detect(detector.estimator.getOffset(), tsDeltaMs, detector.estimator.getNumOfDeltas(),
						arrivalTimeMs);
			}
	...
	/**
	 * Timeouts SSRCs that have not received any data for kTimestampGroupLengthMs
	 * millis.
	 *
	 * @param nowMs the current time in millis.
	 */
	...
	private class Detector {
		...
		public Detector(OverUseDetectorOptions options, boolean enableBurstGrouping) {
			this.interArrival = new InterArrival(kTimestampGroupLengthTicks, kTimestampToMs, enableBurstGrouping,
					diagnosticContext);
