...
public class RTPEncodingDesc {
	/**
	 * The {@link Logger} used by the {@link RTPEncodingDesc} class to print debug
	 * information.
	 */
	...
	/**
	 * The maximum time interval (in millis) an encoding can be considered active
	 * without new frames. This value corresponds to 4fps + 50 millis to compensate
	 * for network noise. If the network is clogged and we don't get a new frame
	 * within 300 millis, and if the encoding is being received, then we will ask
	 * for a new key frame (this is done in the JVB in SimulcastController).
	 */
	...
	/**
	 * The ssrcs associated with this encoding (for example, RTX or FLEXFEC) Maps
	 * ssrc -> type {@link Constants} (rtx, etc.)
	 */
	 ...
	 * The actual frame rate may be less due to bad network or system load.
	 */
	...
	/**
	 * The {@link MediaStreamTrackDesc} that this {@link RTPEncodingDesc} belongs
	 * to.
	 */
	private final MediaStreamTrackDesc track;

	/**
	 * The {@link RateStatistics} instance used to calculate the receiving bitrate
	 * of this RTP encoding.
	 */
	private final RateStatistics rateStatistics = new RateStatistics(AVERAGE_BITRATE_WINDOW_MS);

	/**
	 * The {@link TreeMap} that holds the seen {@link FrameDesc}, keyed by their RTP
	 * timestamps.
	 */
	private final TreeMap<Long, FrameDesc> streamFrames = new TreeMap<Long, FrameDesc>() {
		 ...
		 * {@inheritDoc}
		 *
		 * It also removes the eldest entry each time a new one is added and the total
		 * number of entries exceeds FRAMES_HISTORY_SZ.
		 */
		@Override
	...
	public RTPEncodingDesc(MediaStreamTrackDesc track, long primarySSRC) {
		this(track, 0, primarySSRC, -1 /* tid */, -1 /* sid */, NO_HEIGHT /* height */, NO_FRAME_RATE /* frame rate */,
				null /* dependencies */);
	}
	 ...
	 * Ctor.
	 *
	 * @param track               the {@link MediaStreamTrackDesc} that this
	 *                            instance belongs to.
	 ...
	 * @param dependencyEncodings The {@link RTPEncodingDesc} on which this layer
	 *                            depends.
	 */
	public RTPEncodingDesc(MediaStreamTrackDesc track, int idx, long primarySSRC, int tid, int sid, int height,
			double frameRate, RTPEncodingDesc[] dependencyEncodings) {
	...
	/**
	 * Applies frame boundaries heuristics to frames olderFrame and newerFrame,
	 * assuming olderFrame predates/is older than newerFrame. Depending on the
	 * relationship of olderFrame and newerFrame, and what we know about olderFrame
	 * and newerFrame, we may be able to deduce the last expected sequence number
	 * for olderFrame and/or the first expected sequence number of newerFrame.
	 *
	...
	private static void applyFrameBoundsHeuristics(FrameDesc olderFrame, FrameDesc newerFrame) {
		...
		boolean framesSupportFrameBoundaries = olderFrame.supportsFrameBoundaries()
		...
		if (guessed) {
			if (logger.isDebugEnabled()) {
				logger.debug("Guessed frame boundaries ts=" + olderFrame.getTimestamp() + ",start="
						+ olderFrame.getStart() + ",end=" + olderFrame.getEnd() + ",ts=" + newerFrame.getTimestamp()
						+ ",start=" + newerFrame.getStart() + ",end=" + newerFrame.getEnd());
			}
		}
	}
	 ...
	 * @return the ssrc for the stream that corresponds to the given type, if it
	 *         exists; otherwise -1
	 */
	...
	public boolean isActive(long nowMs) {
		if (lastReceivedFrame == null) {
			...
			boolean nextIsActive = encodings != null && encodings.length > idx + 1
	...
	public String toString() {
		return "subjective_quality=" + idx + ",primary_ssrc=" + getPrimarySSRC() + ",secondary_ssrcs=" + secondarySsrcs
				+ ",temporal_id=" + tid + ",spatial_id=" + sid + ",last_stable_bitrate_bps=" + lastStableBitrateBps;
	}
	...
	/**
	 * Returns a boolean that indicates whether or not this {@link RTPEncodingDesc}
	 * depends on the subjective quality index that is passed as an argument.
	 *
	...
	/**
	 * Gets a boolean indicating whether or not the specified packet matches this
	 * encoding or not. Assumes that the packet is valid.
	 *
	...
	boolean matches(RawPacket pkt) {
		...
		int tid = this.tid != -1 ? track.getMediaStreamTrackReceiver().getStream().getTemporalID(pkt) : -1;
		int sid = this.sid != -1 ? track.getMediaStreamTrackReceiver().getStream().getSpatialID(pkt) : -1;

		return (tid == -1 && sid == -1 && idx == 0) || (tid == this.tid && sid == this.sid);
	}

	/**
	 * Gets a boolean indicating whether or not the SSRC specified in the arguments
	 * matches this encoding or not.
	 *
	...
	void update(RawPacket pkt, long nowMs) {
		...
		if (frame == null) {
			...
			if (lastReceivedFrame == null || RTPUtils.isNewerTimestampThan(ts, lastReceivedFrame.getTimestamp())) {
		...
		if (frameChanged) {
			...
			Map.Entry<Long, FrameDesc> ceilingEntry = base.streamFrames.ceilingEntry((ts + 1) & 0xFFFFFFFFL);
			...
			Map.Entry<Long, FrameDesc> floorEntry = base.streamFrames.floorEntry((ts - 1) & 0xFFFFFFFFL);
	...
	/**
	 * Gets the cumulative bitrate (in bps) of this {@link RTPEncodingDesc} and its
	 * dependencies.
	 *
	 * @param nowMs
	 * @return the cumulative bitrate (in bps) of this {@link RTPEncodingDesc} and
	 *         its dependencies.
	 */
	...
	/**
	 * Recursively adds the bitrate (in bps) of this {@link RTPEncodingDesc} and its
	 * dependencies in the array passed in as an argument.
	 *
	 * @param nowMs
	 */
	...
	/**
	 * Finds the {@link FrameDesc} that matches the RTP packet specified in the
	 ...
	 * @param len the number of <tt>byte</tt>s in <tt>buf</tt> which constitute the
	 *            actual data.
	 *
	 * @return the {@link FrameDesc} that matches the RTP packet specified in the
	 *         buffer passed in as a parameter, or null if there is no matching
	 *         {@link FrameDesc}.
	 */
...
//    }
	 ...
	 *
	 * @return the {@link FrameDesc} that matches the RTP timestamp given, or null
	 *         if there is no matching frame {@link FrameDesc}.
	 */
	 ...
	 * 
	 * @return last frame from this encoding that has been received, otherwise null.
	 */
	...
	public void incrReceivers() {
		...
		if (logger.isTraceEnabled()) {
			logger.trace("increment_receivers,hash=" + track.getMediaStreamTrackReceiver().getStream().hashCode()
					+ ",idx=" + idx + ",receivers=" + numOfReceivers);
		}
	}
	...
	public void decrReceivers() {
		...
		if (logger.isTraceEnabled()) {
			logger.trace("decrement_receivers,hash=" + track.getMediaStreamTrackReceiver().getStream().hashCode()
					+ ",idx=" + idx + ",receivers=" + numOfReceivers);
		}
	}
}
