...
public class RawPacketCache implements AutoCloseable {
	/**
	 * The <tt>Logger</tt> used by the <tt>RawPacketCache</tt> class and its
	 ...
	 */
	public final static String NACK_CACHE_SIZE_STREAMS = "org.jitsi.impl.neomedia.transform.CachingTransformer.CACHE_SIZE_STREAMS";

	/**
	 * Configuration property number of packets to cache.
	 */
	public final static String NACK_CACHE_SIZE_PACKETS = "org.jitsi.impl.neomedia.transform.CachingTransformer.CACHE_SIZE_PACKETS";

	/**
	 * Configuration property for nack cache size in milliseconds.
	 */
	public final static String NACK_CACHE_SIZE_MILLIS = "org.jitsi.impl.neomedia.transform.CachingTransformer.CACHE_SIZE_MILLIS";
	...
	/**
	 * The amount of time, after which the cache for an SSRC will be cleared, unless
	 * new packets have been inserted.
	 */
	...
	/**
	 * Counts the number of requests (calls to {@link #get(long, int)}) which the
	 * cache was able to answer.
	 */
	private AtomicInteger totalHits = new AtomicInteger(0);

	/**
	 * Counts the number of requests (calls to {@link #get(long, int)}) which the
	 * cache was not able to answer.
	 */
	...
	/**
	 * The hash code or other identifier of the owning stream, if any. Only used for
	 * logging.
	 */
	...
	public void close() throws Exception {
		if (totalPacketsAdded.get() > 0) {
			logger.info(Logger.Category.STATISTICS,
					"closed,stream=" + streamId + " max_size_bytes=" + maxSizeInBytes + ",max_size_packets="
							+ maxSizeInPackets + ",total_hits=" + totalHits.get() + ",total_misses=" + totalMisses.get()
							+ ",total_packets=" + totalPacketsAdded.get() + ",oldest_hit_ms=" + oldestHit);
		}
	...
	/**
	 * Gets the packet, encapsulated in a {@link Container} with the given SSRC and
	 * RTP sequence number from the cache. If no such packet is found, returns
	 * <tt>null</tt>.
	 * 
	 * @param ssrc The SSRC of the packet.
	 * @param seq  the RTP sequence number of the packet.
	 * @return the packet, encapsulated in a {@link Container} with the given SSRC
	 ...
	 * If no such packet is found, returns <tt>null</tt>.
	 * 
	 * @param ssrc The SSRC of the packet.
	 * @param seq  the RTP sequence number of the packet.
	...
	/**
	 * Gets the {@link Cache} instance which caches packets with SSRC <tt>ssrc</tt>,
	 * creating if <tt>create</tt> is set and creation is possible (the maximum
	 * number of caches hasn't been reached).
	 * 
	 * @param ssrc   the SSRC.
	...
	/**
	 * Gets an unused <tt>RawPacket</tt> with at least <tt>len</tt> bytes of buffer
	 * space.
	 * 
	...
	/**
	 * Checks for {@link Cache} instances which have not received new packets for a
	 * period longer than {@link #SSRC_TIMEOUT_MILLIS} and removes them.
	 */
	...
	/**
	 * Returns a {@link Container} and its {@link RawPacket} to the list of free
	...
	/**
	 * Gets the most recent packets from the cache that pertains to the SSRC that is
	 * specified as an argument, not exceeding the number of bytes specified as an
	 * argument.
	 *
	 ...
	 *         number of bytes specified as an argument, or null if there are no
	...
	/**
	 * Updates the timestamp of the packet in the cache with SSRC {@code ssrc} and
	 * sequence number {@code seq}, if such a packet exists in the cache, setting it
	 * to {@code ts}.
	 * 
	 * @param ssrc the SSRC of the packet.
	...
	private class Cache {
		/**
		 * The underlying container. It maps a packet index (based on its RTP sequence
		 * number, in the same way as used in SRTP (RFC3711)) to the <tt>RawPacket</tt>
		...
		/**
		 * Calculates the index of an RTP packet based on its RTP sequence number and
		 * updates the <tt>s_l</tt> and <tt>ROC</tt> fields. Based on the procedure
		 * outlined in RFC3711
		 * 
		...
		/**
		 * Returns a copy of the RTP packet with sequence number {@code seq} from the
		 * cache, or {@code null} if the cache does not contain a packet with this
		 * sequence number.
		 * 
		 * @param seq the RTP sequence number of the packet to get.
		 * @return a copy of the RTP packet with sequence number {@code seq} from the
		 *         cache, or {@code null} if the cache does not contain a packet with
		...
		private synchronized Container get(int seq) {
			...
			return container == null ? null
					: new Container(new RawPacket(container.pkt.getBuffer().clone(), container.pkt.getOffset(),
							container.pkt.getLength()), container.timeAdded);
		}

		/**
		 * Returns the RTP packet with sequence number {@code seq} from the cache, or
		 * {@code null} if the cache does not contain a packet with this sequence
		 * number.
		 * 
		 * @param seq the RTP sequence number of the packet to get.
		 * @return the RTP packet with sequence number {@code seq} from the cache, or
		 *         {@code null} if the cache does not contain a packet with this
		 *         sequence number.
		 */
		...
		/**
		 * Drops the oldest packets from the cache until: 1. The cache contains at most
		 * {@link #MAX_SIZE_PACKETS} packets, and 2. The cache only contains packets at
		 * most {@link #SIZE_MILLIS} milliseconds older than the newest packet in the
		 * cache.
		 */
		...
		/**
		 * Gets the most recent packets from this cache, not exceeding the number of
		...
		public synchronized Set<Container> getMany(int bytes) {
			...
			Iterator<Map.Entry<Integer, Container>> it = cache.descendingMap().entrySet().iterator();
	...
	public class Container {
		...
		/**
		 * The time (in milliseconds since the epoch) that the packet was added to the
		 * cache.
		 */
