 ...
 * intention is to have the same instance shared between all media streams of a
 * transport channel, so we expect it will be accessed by multiple threads. See
...
public class TransportCCEngine extends RTCPPacketListenerAdapter implements RemoteBitrateObserver, CallStatsObserver {
	...
	private static final TimeSeriesLogger timeSeriesLogger = TimeSeriesLogger
			.getTimeSeriesLogger(TransportCCEngine.class);

	/**
	 * The engine which handles incoming RTP packets for this instance. It reads
	 * transport-wide sequence numbers and registers arrival times.
	 */
	private final IngressEngine ingressEngine = new IngressEngine();

	/**
	 * The engine which handles outgoing RTP packets for this instance. It adds
	 * transport-wide sequence numbers to outgoing RTP packets.
	 */
	...
	/**
	 * Some {@link VideoMediaStream} that utilizes this instance. We use it to get
	 * the sender/media SSRC of the outgoing RTCP TCC packets.
	 */
	...
	/**
	 * The time (in milliseconds since the epoch) at which the first received packet
	 * in {@link #incomingPackets} was received (or -1 if the map is empty). Kept
	 * here for quicker access, because the map is ordered by sequence number.
	 */
	private long firstIncomingTs = -1;

	/**
	 * The reference time of the remote clock. This is used to rebase the arrival
	 * times in the TCC packets to a meaningful time base (that of the sender). This
	 * is technically not necessary and it's done for convenience.
	 */
	private long remoteReferenceTimeMs = -1;

	/**
	 * Local time to map to the reference time of the remote clock. This is used to
	 * rebase the arrival times in the TCC packets to a meaningful time base (that
	 * of the sender). This is technically not necessary and it's done for
	 * convinience.
	 */
	private long localReferenceTimeMs = -1;

	/**
	 * Holds a key value pair of the packet sequence number and an object made up of
	 * the packet send time and the packet size.
	 */
	...
	private void packetReceived(int seq, int pt, boolean marked) {
		...
		if (timeSeriesLogger.isTraceEnabled()) {
			timeSeriesLogger.trace(diagnosticContext.makeTimeSeriesPoint("ingress_tcc_pkt", now).addField("seq", seq)
					.addField("pt", pt));
		}
	...
	/**
	 * Examines the list of received packets for which we have not yet sent feedback
	 * and determines whether we should send feedback at this point. If so, sends
	 * the feedback.
	 * 
	 * @param marked whether the last received RTP packet had the "marked" bit set.
	...
	private void maybeSendRtcp(boolean marked, long now) {
		...
		synchronized (incomingPacketsSyncRoot) {
			...
			int packetCount = 1 + RTPUtils.subtractNumber(incomingPackets.lastKey(), incomingPackets.firstKey());
			...
			if (delta > 100 || (delta > 20 && marked) || incomingPackets.size() > 100
		...
		if (packets != null) {
			...
			try {
				...
				RTCPTCCPacket rtcpPacket = new RTCPTCCPacket(senderSSRC, sourceSSRC, packets,
	...
	public void tccReceived(RTCPTCCPacket tccPacket) {
		...
		for (Map.Entry<Integer, Long> entry : packetMap.entrySet()) {
			...
			if (remoteReferenceTimeMs == -1) {
				remoteReferenceTimeMs = RTCPTCCPacket
						.getReferenceTime250us(new ByteArrayBufferImpl(tccPacket.fci, 0, tccPacket.fci.length)) / 4;
			...
			if (timeSeriesLogger.isTraceEnabled()) {
				if (previousArrivalTimeMs != -1) {
					...
					timeSeriesLogger.trace(
							diagnosticContext.makeTimeSeriesPoint("ingress_tcc_ack").addField("seq", entry.getKey())
									.addField("arrival_time_ms", arrivalTimeMs).addField("diff_ms", diff_ms));
				} else {
					timeSeriesLogger.trace(diagnosticContext.makeTimeSeriesPoint("ingress_tcc_ack")
							.addField("seq", entry.getKey()).addField("arrival_time_ms", arrivalTimeMs));
				}
			}
			...
			bitrateEstimatorAbsSendTime.incomingPacketInfo(arrivalTimeMs, sendTime24bits, packetDetail.packetLength,
	...
	/**
	 * Adds a {@link MediaStream} to the list of {@link MediaStream}s which use this
	 * {@link TransportCCEngine}.
	 * 
	...
	/**
	 * Removes a {@link MediaStream} from the list of {@link MediaStream}s which use
	 * this {@link TransportCCEngine}.
	 * 
	...
	/**
	 * {@link PacketDetail} is an object that holds the length(size) of the packet
	 * in {@link #packetLength} and the time stamps of the outgoing packet in
	 * {@link #packetSendTimeMs}
	 */
	...
	public class EgressEngine extends SinglePacketTransformerAdapter implements TransformEngine {
		 ...
		 * <p>
		 * </p>
		 * If the transport-cc extension is configured, update the transport-wide
		 * sequence number (adding a new extension if one doesn't exist already).
		 */
		@Override
		public RawPacket transform(RawPacket pkt) {
			if (extensionId != -1) {
				...
				if (timeSeriesLogger.isTraceEnabled()) {
					timeSeriesLogger.trace(diagnosticContext.makeTimeSeriesPoint("egress_tcc_pkt")
							.addField("rtp_seq", pkt.getSequenceNumber()).addField("pt", RawPacket.getPayloadType(pkt))
