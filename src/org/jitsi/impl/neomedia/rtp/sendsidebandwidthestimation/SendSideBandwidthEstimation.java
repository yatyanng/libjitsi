 ...
 * https://tools.ietf.org/html/draft-ietf-rmcat-gcc-01 Heavily based on code
 * from webrtc.org (send_side_bandwidth_estimation.cc, commit ID
...
class SendSideBandwidthEstimation extends RTCPPacketListenerAdapter implements BandwidthEstimator {
	/**
	 * The name of the property that specifies the low-loss threshold (expressed as
	 * a proportion of lost packets). See {@link #low_loss_threshold_}.
	 */
	public final static String LOW_LOSS_THRESHOLD_PNAME = SendSideBandwidthEstimation.class.getName()
			+ ".lowLossThreshold";

	/**
	 * The name of the property that specifies the high-loss threshold (expressed as
	 * a proportion of lost packets). See {@link #high_loss_threshold_}.
	 */
	public final static String HIGH_LOSS_THRESHOLD_PNAME = SendSideBandwidthEstimation.class.getName()
			+ ".highLossThreshold";

	/**
	 * The name of the property that specifies the bitrate threshold (in kbps). See
	 * {@link #bitrate_threshold_bps_}.
	 */
	public final static String BITRATE_THRESHOLD_KBPS_PNAME = SendSideBandwidthEstimation.class.getName()
			+ ".bitrateThresholdKbps";

	/**
	 * The name of the property that specifies the probability of enabling the
	 * loss-based experiment.
	 */
	public final static String LOSS_EXPERIMENT_PROBABILITY_PNAME = SendSideBandwidthEstimation.class.getName()
	 ...
	 * timeout experiment.
	 */
	public final static String TIMEOUT_EXPERIMENT_PROBABILITY_PNAME = SendSideBandwidthEstimation.class.getName()
	...
	/**
	 * The <tt>Logger</tt> used by the {@link SendSideBandwidthEstimation} class and
	...
	private static final TimeSeriesLogger timeSeriesLogger = TimeSeriesLogger
			.getTimeSeriesLogger(SendSideBandwidthEstimation.class);
	...
	SendSideBandwidthEstimation(MediaStreamImpl stream, long startBitrate) {
		...
		float lossExperimentProbability = (float) cfg.getDouble(LOSS_EXPERIMENT_PROBABILITY_PNAME,
				kDefaultLossExperimentProbability);

		if (kRandom.nextFloat() < lossExperimentProbability) {
			low_loss_threshold_ = (float) cfg.getDouble(LOW_LOSS_THRESHOLD_PNAME, kDefaultLowLossThreshold);
			high_loss_threshold_ = (float) cfg.getDouble(HIGH_LOSS_THRESHOLD_PNAME, kDefaultHighLossThreshold);
			bitrate_threshold_bps_ = 1000 * cfg.getInt(BITRATE_THRESHOLD_KBPS_PNAME, kDefaultBitrateThresholdKbps);
		} else {
		...
		float timeoutExperimentProbability = (float) cfg.getDouble(TIMEOUT_EXPERIMENT_PROBABILITY_PNAME,
	...
	protected synchronized void updateEstimate(long now) {
		...
		if (time_since_packet_report_ms < kPacketReportTimeoutIntervals * kFeedbackIntervalMs) {
			...
			if (time_since_feedback_ms > kFeedbackTimeoutIntervals * kFeedbackIntervalMs
					&& (last_timeout_ms_ == -1 || now - last_timeout_ms_ > kTimeoutIntervalMs)) {
	...
	private synchronized void updateMinHistory(long now_ms) {
		...
		while (!min_bitrate_history_.isEmpty() && bitrate_ <= min_bitrate_history_.getLast().second) {
	...
	private synchronized long getRtt() {
		...
		if (rtt < 0 || rtt > 1000) {
			logger.warn(
					"RTT not calculated, or has a suspiciously high value (" + rtt + "). Using the default of 100ms.");
			rtt = 100;
		}
	...
	/**
	 * Notifies registered listeners that the estimation of the available bandwidth
	 * has changed.
	 * 
	...
	/**
	 * This class records statistics information about how much time we spend in
	 * different loss-states (loss-free, loss-limited and loss-degraded).
	 */
	public class StatisticsImpl implements Statistics {
		...
		/**
		 * Computes the min/max/avg/sd of the bitrate while in {@link #currentState}.
		 */
		private LongSummaryStatistics currentStateBitrateStatistics = new LongSummaryStatistics();

		/**
		 * Computes the min/max/avg/sd of the loss while in {@link #currentState}.
		 */
		private IntSummaryStatistics currentStateLossStatistics = new IntSummaryStatistics();

		/**
		 * True when the fields of this class have changed from their default value. The
		 * purpose is to avoid creating new IntSummaryStatistics and
		...
		private final LongSummaryStatistics lossFreeMsStats = new LongSummaryStatistics(),
				lossDegradedMsStats = new LongSummaryStatistics(), lossLimitedMsStats = new LongSummaryStatistics();
		...
		public void update(long nowMs) {
			synchronized (SendSideBandwidthEstimation.this) {
				...
				boolean currentStateHasTimedOut = time_since_packet_report_ms < kPacketReportTimeoutIntervals
						* kFeedbackIntervalMs;
		 ...
		 * @param currentStateHasTimedOut true if the current state has timed out, i.e.
		 *                                we haven't received receiver reports "in a
		 *                                while".
		...
		void update(long nowMs, boolean currentStateHasTimedOut, LossRegion nextState) {
			synchronized (SendSideBandwidthEstimation.this) {
				...
				if (this.currentState != null) {
					...
					case LossDegraded:
						lossDegradedMsStats.accept(currentStateCumulativeDurationMs);
						break;
					case LossFree:
						lossFreeMsStats.accept(currentStateCumulativeDurationMs);
						break;
					case LossLimited:
						lossLimitedMsStats.accept(currentStateCumulativeDurationMs);
						break;
					}

					if (timeSeriesLogger.isTraceEnabled()) {
						timeSeriesLogger.trace(diagnosticContext.makeTimeSeriesPoint("loss_estimate")
								.addField("state", currentState.name())
								.addField("max_loss", currentStateLossStatistics.getMax() / 256.0f)
								.addField("min_loss", currentStateLossStatistics.getMin() / 256.0f)
								.addField("avg_loss", currentStateLossStatistics.getAverage() / 256.0f)
								.addField("max_bps", currentStateBitrateStatistics.getMax())
								.addField("min_bps", currentStateBitrateStatistics.getMin())
								.addField("avg_bps", currentStateBitrateStatistics.getAverage())
								.addField("duration_ms", currentStateCumulativeDurationMs)
								.addField("consecutive_visits", currentStateConsecutiveVisits)
								.addField("bitrate_threshold", bitrate_threshold_bps_)
								.addField("low_loss_threshold", low_loss_threshold_)
								.addField("high_loss_threshold", high_loss_threshold_)
								.addField("delta_bps", bitrate_ - currentStateStartBitrateBps));
					}
				}
