...

{
	...
	/**
	 * Initializes a new <tt>JavaSoundClipImpl</tt> instance which is to play audio
	 * stored at a specific <tt>URL</tt> using <tt>java.applet.AudioClip</tt>.
	 *
	 * @param uri           the <tt>URL</tt> at which the audio is stored and which
	 *                      the new instance is to load
	 * @param audioNotifier the <tt>AudioNotifierService</tt> which is initializing
	 *                      the new instance and whose <tt>mute</tt> property/state
	 *                      is to be monitored by the new instance
	 * @throws IOException if a <tt>java.applet.AudioClip</tt> could not be
	 *                     initialized or the audio at the specified <tt>url</tt>
	 *                     could not be read
	 */
