...
public class AudioNotifierServiceImpl implements AudioNotifierService, PropertyChangeListener {
	/**
	 * The cache of <tt>SCAudioClip</tt> instances which we may reuse. The reuse is
	 * complex because a <tt>SCAudioClip</tt> may be used by a single user at a
	 * time.
	 */
	...
	/**
	 * The <tt>DeviceConfiguration</tt> which provides information about the notify
	 * and playback devices on which this instance plays <tt>SCAudioClip</tt>s.
	 */
	...
	public SCAudioClip createAudio(String uri, boolean playback) {
		...
		synchronized (audiosSyncRoot) {
			...
			/*
			 * While we want to reuse the SCAudioClip instances, they may be used by a
			 * single user at a time. That's why we'll forget about them while they are in
			 * use and we'll reclaim them when they are no longer in use.
			 */
			...
			/*
			 * Make sure that the SCAudioClip will be reclaimed for reuse when it is no
			 * longer in use.
			 */
			if (audio != null) {
				if (audios == null)
				 ...
				 * initialized because it may have become invalid if the playback or notify
				...
				audio = new SCAudioClip() {
					 ...
					 *
					 * @param loopCondition the <tt>Callable&lt;Boolean&gt;</tt> which represents
					 *                      the <tt>loopCondition</tt> to be evaluated
					...
					private Boolean evaluateLoopCondition(Callable<Boolean> loopCondition) throws Exception {
						/*
						 * SCAudioClip.play(int,Callable<Boolean>) is documented to play the SCAudioClip
						 * once only if the loopCondition is null. The same will be accomplished by
					...
					public void play() {
						/*
						 * SCAudioClip.play() is documented to behave as if loopInterval is negative
						 * and/or loopCondition is null. We have to take care that this instance does
						 * not get garbage collected until the finalAudio finishes playing so we will
						 * delegate to this instance's implementation of
					...
					public void play(int loopInterval, final Callable<Boolean> finalLoopCondition) {
						/*
						 * We have to make sure that this instance does not get garbage collected before
						 * the finalAudio finishes playing. The argument loopCondition of the method
						 * SCAudioClip.play(int,Callable<Boolean>) will live/be referenced during that
						 * time so we will use it to hold on to this instance.
						 */
	...
	/**
	 * Enables or disables the sound in the application. If <tt>false</tt>, we try
	 * to restore all looping sounds if any.
	 *
	 ...
	 * <tt>uri</tt> of the <tt>SCAudioClip</tt> with the indicator which determines
	 * whether the <tt>SCAudioClip</tt> in question uses the playback or the notify
	 * audio device.
	 */
