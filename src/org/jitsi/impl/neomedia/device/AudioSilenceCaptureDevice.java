...
class AudioSilenceCaptureDevice extends AbstractPushBufferCaptureDevice {
	...
	private static final Format[] SUPPORTED_FORMATS = new Format[] {
			new AudioFormat(AudioFormat.LINEAR, 48000, 16, 1, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
	 ...
	 * <tt>AudioMixer</tt> for the mere purposes of ticking the clock which makes
	 * <tt>AudioMixer</tt> read media from its inputs, mix it, and write it to its
	 * outputs. The preferred value is <tt>true</tt> because it causes the
	 * <tt>AudioMixer</tt> to not push media unless at least one <tt>Channel</tt> is
	 ...
	 * 
	 * @param clockOnly whether the
	 *                  {@link AudioSilenceCaptureDevice.AudioSilenceStream}s
	 *                  created by this instance are to be used only for the purpose
	 *                  of ticking the clock which makes
	 ...
	 * <tt>Format</tt>s hardcoded as supported in <tt>AudioSilenceCaptureDevice</tt>
	 * because the super looks them up by <tt>CaptureDeviceInfo</tt> and this
	...
	/**
	 * Implements a <tt>PushBufferStream</tt> which provides silence in the form of
	 * audio media.
	 */
	private static class AudioSilenceStream extends AbstractPushBufferStream<AudioSilenceCaptureDevice>
			implements Runnable {
		/**
		 * The indicator which determines whether {@link #start()} has been invoked on
		 * this instance without an intervening {@link #stop()}.
		 */
		private boolean started;

		/**
		 * The <tt>Thread</tt> which pushes available media data out of this instance to
		 * its consumer i.e. <tt>BufferTransferHandler</tt>.
		 */
		private Thread thread;

		/**
		 * The flag which determines whether this {@code AudioSilenceStream} is to be
		 * used by <tt>AudioMixer</tt> for the mere purposes of ticking the clock which
		 * makes <tt>AudioMixer</tt> read media from its inputs, mix it, and write it to
		 * its outputs.
		 */
		private final boolean clockOnly;

		/**
		 * Initializes a new <tt>AudioSilenceStream</tt> which is to be exposed by a
		 * specific <tt>AudioSilenceCaptureDevice</tt> and which is to have its
		 * <tt>Format</tt>-related information abstracted by a specific
		 * <tt>FormatControl</tt>.
		 *
		 * @param dataSource    the <tt>AudioSilenceCaptureDevice</tt> which is
		 *                      initializing the new instance and which is to expose it
		 *                      in its array of <tt>PushBufferStream</tt>s
		...
		/**
		 * Runs in {@link #thread} and pushes available media data out of this instance
		 * to its consumer i.e. <tt>BufferTransferHandler</tt>.
		 */
		@Override
		public void run() {
			try {
				 ...
				 * clock implemented by this instance uses a thread priority considered
				 * appropriate for audio processing.
				 */
				AbstractAudioRenderer.useAudioThreadPriority();

				/*
				 * The method implements a clock which ticks at a certain and regular interval
				 * of time which is not affected by the duration of the execution of, for
				 * example, the invocation of
				 * BufferTransferHandler.transferData(PushBufferStream).
				 *
				 * XXX The implementation utilizes System.currentTimeMillis() and, consequently,
				 * may be broken by run-time adjustments to the system time.
				 */
				...
				while (true) {
					...
					if (tick) {
						...
						/*
						 * The current thread has woken up too early for the next scheduled clock tick
						 * and, consequently, it should sleep until the time of the next scheduled clock
						 * tick comes.
						 */
						try {
						...
						/*
						 * The clock will not tick and spurious wakeups will be handled. However, the
						 * current thread will first check whether it is still utilized by this
					...
					synchronized (this) {
						/*
						 * If the current Thread is no longer utilized by this AudioSilenceStream, it no
						 * longer has the right to touch it. If this AudioSilenceStream has been
		 ...
		 *
		 * @throws IOException if an error has prevented the start of the transfer of
		 *                     media from this instance
		 */
		@Override
		 ...
		 *
		 * @throws IOException if an error has prevented the stopping of the transfer of
		 *                     media from this instance
		 */
		@Override
