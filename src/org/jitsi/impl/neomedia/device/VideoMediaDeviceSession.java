...
public class VideoMediaDeviceSession extends MediaDeviceSession implements RTCPFeedbackMessageCreateListener {
	 ...
	 * <tt>Component</tt> depicting the streaming of the desktop of the local peer
	 * to the remote peer.
	 */
	private static final String DESKTOP_STREAMING_ICON = "impl.media.DESKTOP_STREAMING_ICON";

	/**
	 * The <tt>Logger</tt> used by the <tt>VideoMediaDeviceSession</tt> class and
	 * its instances for logging output.
	 */
	private static final Logger logger = Logger.getLogger(VideoMediaDeviceSession.class);

	/**
	 * Gets the visual <tt>Component</tt> of a specific <tt>Player</tt> if it has
	 * one and ignores the failure to access it if the specified <tt>Player</tt> is
	 * unrealized.
	 *
	 * @param player the <tt>Player</tt> to get the visual <tt>Component</tt> of if
	 *               it has one
	 * @return the visual <tt>Component</tt> of the specified <tt>Player</tt> if it
	 *         has one; <tt>null</tt> if the specified <tt>Player</tt> does not have
	...
	/**
	 * The <tt>KeyFrameControl</tt> used by this<tt>VideoMediaDeviceSession</tt> as
	 * a means to control its key frame-related logic.
	 */
	 ...
	 * rendering the media received from the remote peer and enabling the explicit
	 * setting of the video size.
	 */
	...
	/**
	 * The list of <tt>RTCPFeedbackMessageCreateListener</tt> which will be notified
	 * when a <tt>RTCPFeedbackMessageListener</tt> is created.
	 */
	...
	/**
	 * The <tt>RTPConnector</tt> with which the <tt>RTPManager</tt> of this instance
	 * is to be or is already initialized.
	 */
	private AbstractRTPConnector rtpConnector;

	/**
	 * Use or not RTCP feedback Picture Loss Indication to request keyframes. Does
	 * not affect handling of received RTCP feedback events.
	 */
	...
	/**
	 * Adds a specific <tt>VideoListener</tt> to this instance in order to receive
	 * notifications when visual/video <tt>Component</tt>s are being added and
	 * removed.
	 * <p>
	 * Adding a listener which has already been added does nothing i.e. it is not
	 * added more than once and thus does not receive one and the same
	 * <tt>VideoEvent</tt> multiple times.
	 * </p>
	 *
	 * @param listener the <tt>VideoListener</tt> to be notified when visual/video
	 ...
	 * <tt>MediaDevice</tt> of this instance. Makes sure that its <tt>MediaType</tt>
	 * is {@link MediaType#VIDEO}.
	 *
	...
	private void controllerUpdateForCreateLocalVisualComponent(ControllerEvent ev, boolean hflip) {
		if (ev instanceof ConfigureCompleteEvent) {
			...
			/*
			 * Use SwScale for the scaling since it produces an image with better quality
			 * and add the "flip" effect to the video.
			 */
				...
				try {
					for (TrackControl trackControl : trackControls) {
						trackControl.setCodecChain(
								hflip ? new Codec[] { new HFlip(), new SwScale() } : new Codec[] { new SwScale() });
						break;
					}
			...
			/*
			 * Mostly for the sake of completeness, notify that the size of the local video
			 * has changed like we do for the remote videos.
			 */
	...
	protected DataSource createCaptureDevice() {
		/*
		 * Create our DataSource as SourceCloneable so we can use it to both display
		 * local video and stream to remote peer.
		 */
		...
		if (captureDevice != null) {
			...
			if (DeviceSystem.LOCATOR_PROTOCOL_IMGSTREAMING.equals(protocol)) {
				/*
				 * It is not clear at this time what the default frame rate for desktop
				 * streaming should be.
				 */
				frameRate = 10;
			} else {
	 ...
	 * visual/video <tt>Component</tt>. The new instance is initialized to render
	 * the media of the <tt>captureDevice</tt> of this <tt>MediaDeviceSession</tt>.
	 *
	 ...
	 * Initializes a new <tt>Player</tt> instance which is to provide the local
	 * visual/video <tt>Component</tt>. The new instance is initialized to render
	 * the media of a specific <tt>DataSource</tt>.
	 *
	...
	protected Player createLocalPlayer(DataSource captureDevice) {
		...
		if (dataSource != null) {
			...
			try {
				localPlayer = ForbiddenManager.createProcessor(dataSource);
			} catch (Exception ex) {
				exception = ex;
			}
			...
			if (exception == null) {
				if (localPlayer != null) {
					/*
					 * If a local visual Component is to be displayed for desktop sharing/streaming,
					 * do not flip it because it does not seem natural.
					 */
	...
	/**
	 * Creates the visual <tt>Component</tt> depicting the video being streamed from
	 * the local peer to the remote peer.
	 *
	 * @return the visual <tt>Component</tt> depicting the video being streamed from
	 *         the local peer to the remote peer if it was immediately created or
	 *         <tt>null</tt> if it was not immediately created and it is to be
	 *         delivered to the currently registered <tt>VideoListener</tt>s in a
	 *         <tt>VideoEvent</tt> with type {@link VideoEvent#VIDEO_ADDED} and
	 *         origin {@link VideoEvent#LOCAL}
	 */
	protected Component createLocalVisualComponent() {
		...
		/*
		 * Displaying the currently streamed desktop is perceived as unnecessary because
		 * the user sees the whole desktop anyway. Instead, a static image will be
		 * presented.
		 */
		...
		/*
		 * The visual Component to depict the video being streamed from the local peer
		 * to the remote peer is created by JMF and its Player so it is likely to take
		 * noticeably long time. Consequently, we will deliver it to the currently
		 * registered VideoListeners in a VideoEvent after returning from the call.
		 */
		...
		/*
		 * If the local visual/video Component exists at this time, it has likely been
		 * created by a previous call to this method. However, the caller may still
		 * depend on a VIDEO_ADDED event being fired for it.
		 */
	...
	/**
	 * Creates the visual <tt>Component</tt> to depict the streaming of the desktop
	 * of the local peer to the remote peer.
	 *
	 * @return the visual <tt>Component</tt> to depict the streaming of the desktop
	 *         of the local peer to the remote peer
	 */
	 ...
	 * <tt>VideoListener</tt>s of this instance that its visual <tt>Component</tt>
	 * is to no longer be used by firing a {@link VideoEvent#VIDEO_REMOVED}
	 * <tt>VideoEvent</tt>.
	 *
	...
	protected void disposeLocalVisualComponent(Component component) {
		if (component != null) {
			/*
			 * Desktop streaming does not use a Player but a Canvas with its name equal to
			 * the value of DESKTOP_STREAMING_ICON.
			 */
	 ...
	 * of its execution and prepares it to be garbage collected. If the specified
	 * <tt>Player</tt> is rendering video, notifies the <tt>VideoListener</tt>s of
	 * this instance that its visual <tt>Component</tt> is to no longer be used by
	 * firing a {@link VideoEvent#VIDEO_REMOVED} <tt>VideoEvent</tt>.
	 *
	 * @param player the <tt>Player</tt> to dispose of
	 * @see MediaDeviceSession#disposePlayer(Player)
	 */
	...
	/**
	 * Notifies the <tt>VideoListener</tt>s registered with this instance about a
	 * specific type of change in the availability of a specific visual
	 * <tt>Component</tt> depicting video.
	 *
	 * @param type            the type of change as defined by <tt>VideoEvent</tt>
	 *                        in the availability of the specified visual
	 *                        <tt>Component</tt> depicting video
	 * @param visualComponent the visual <tt>Component</tt> depicting video which
	 *                        has been added or removed in this instance
	 * @param origin          {@link VideoEvent#LOCAL} if the origin of the video is
	 *                        local (e.g. it is being locally captured);
	 *                        {@link VideoEvent#REMOTE} if the origin of the video
	 *                        is remote (e.g. a remote peer is streaming it)
	 * @param wait            <tt>true</tt> if the call is to wait till the
	 *                        specified <tt>VideoEvent</tt> has been delivered to
	 *                        the <tt>VideoListener</tt>s; otherwise, <tt>false</tt>
	...
	protected boolean fireVideoEvent(int type, Component visualComponent, int origin, boolean wait) {
		if (logger.isTraceEnabled()) {
			logger.trace("Firing VideoEvent with type " + VideoEvent.typeToString(type) + " and origin "
	...
	/**
	 * Gets the visual <tt>Component</tt>, if any, depicting the video streamed from
	 * the local peer to the remote peer.
	 *
	 * @return the visual <tt>Component</tt> depicting the local video if local
	 *         video is actually being streamed from the local peer to the remote
	 *         peer; otherwise, <tt>null</tt>
	 */
	...
	/**
	 * Returns the FMJ <tt>Format</tt> of the video we are receiving from the remote
	 * peer.
	 *
	 * @return the FMJ <tt>Format</tt> of the video we are receiving from the remote
	 *         peer or <tt>null</tt> if we are not receiving any video or the FMJ
	 *         <tt>Format</tt> of the video we are receiving from the remote peer
	 ...
	 *
	 * @param ssrc the src-id of the receive stream, which visual <tt>Component</tt>
	 *             we're looking for
	 * @return the visual <tt>Component</tt> rendering the <tt>ReceiveStream</tt>
	 *         corresponding to the given ssrc
	 */
	...
	/**
	 * Gets the visual <tt>Component</tt>s where video from the remote peer is being
	 * rendered.
	 *
	 * @return the visual <tt>Component</tt>s where video from the remote peer is
	 *         being rendered
	 */
	public List<Component> getVisualComponents() {
		...
		/*
		 * When we know (through means such as SDP) that we don't want to receive, it
		 * doesn't make sense to wait for the remote peer to acknowledge our desire. So
		 * we'll just stop depicting the video of the remote peer regardless of whether
		 * it stops or continues its sending.
		 */
	 ...
	 *
	 * @param keyFrameRequester the <tt>KeyFrameControl.KeyFrameRequester</tt> on
	 *                          which the method is invoked
	 * @return <tt>true</tt> if this <tt>KeyFrameRequester</tt> has indeed requested
	 *         a key frame from the remote peer of the associated
	...
	private boolean keyFrameRequesterRequestKeyFrame(KeyFrameControl.KeyFrameRequester keyFrameRequester) {
		...
		if (VideoMediaDeviceSession.this.useRTCPFeedbackPLI) {
			try {
				new RTCPFeedbackMessagePacket(RTCPFeedbackMessageEvent.FMT_PLI, RTCPFeedbackMessageEvent.PT_PS,
						localSSRC, remoteSSRC).writeTo(rtpConnector.getControlOutputStream());
				requested = true;
	 ...
	 * generated a <tt>ConfigureCompleteEvent</tt>.
	 *
	...
	protected void playerControllerUpdate(ControllerEvent ev) {
		 ...
		 * SizeChangeEvents of the Player do not really notify about changes in the size
		 * of the input. Besides, playerScaler will take care of the events in such a
		 * case.
		 */
	...
	/**
	 * Notifies this instance that a specific <tt>Player</tt> of remote content has
	 * generated a <tt>RealizeCompleteEvent</tt>.
	 *
	 * @param player the <tt>Player</tt> which is the source of a
	...
	protected void playerRealizeComplete(final Processor player) {
		...
		if (visualComponent != null) {
			/*
			 * SwScale seems to be very good at scaling with respect to image quality so use
			 * it for the scaling in the player replacing the scaling it does upon
			 * rendering.
			 */
	 ...
	 * @param origin           {@link VideoEvent#LOCAL} or {@link VideoEvent#REMOTE}
	 *                         which specifies the origin of the visual
	 *                         <tt>Component</tt> displaying video which is
	 *                         concerned
	 * @param width            the width reported in the event
	 * @param height           the height reported in the event
	 * @see SizeChangeEvent
	 */
	protected void playerSizeChange(final Controller sourceController, final int origin, final int width,
			final int height) {
		/*
		 * Invoking anything that is likely to change the UI in the Player thread seems
		 * like a performance hit so bring it into the event thread.
		 */
		...
		if (visualComponent != null) {
			/*
			 * The Player will notice the new size and will notify about it before it
			 * reaches the Renderer. The notification/event may as well arrive before the
			 * Renderer reflects the new size onto the preferredSize of the Component. In
			 * order to make sure that the new size is reflected on the preferredSize of the
			 * Component before the notification/event arrives to its destination/listener,
			 * reflect it as soon as possible i.e. now.
			 */
			try {
				Dimension prefSize = visualComponent.getPreferredSize();

				if ((prefSize == null) || (prefSize.width < 1) || (prefSize.height < 1)
						|| !VideoLayout.areAspectRatiosEqual(prefSize, width, height) || (prefSize.width < width)
	...
	private void playerVisualComponentResized(Processor player, ComponentEvent ev) {
		...
		/*
		 * When the visualComponent is not in a UI hierarchy, its size is not expected
		 * to be representative of what the user is seeing.
		 */
		...
		/*
		 * The size of the output video will be calculated so that it fits into the
		 * visualComponent and the video aspect ratio is preserved. The presumption here
		 * is that the inputFormat holds the video size with the correct aspect ratio.
		 */
		...
		if (playerScalerOutputSize == null)
			...
			/*
			 * If we are not going to make much of a change, do not even bother because any
			 * scaling in the Renderer will not be noticeable anyway.
			 */
	...
	/**
	 * Removes a specific <tt>VideoListener</tt> from this instance in order to have
	 * to no longer receive notifications when visual/video <tt>Component</tt>s are
	...
	/**
	 * Sets the <tt>RTPConnector</tt> that will be used to initialize some codec for
	 * RTCP feedback.
	 *
	 ...
	 *                        <tt>VideoMediaDeviceSession</tt> as a means of control
	 *                        over its key frame-related logic
	 */
	...
	protected void setProcessorFormat(Processor processor, MediaFormatImpl<? extends Format> mediaFormat) {
		...
		if ("h263-1998/rtp".equalsIgnoreCase(format.getEncoding())) {
			/*
			 * If no output size has been defined, then no SDP fmtp has been found with
			 * QCIF, CIF, VGA or CUSTOM parameters. Let's default to QCIF (176x144).
			 */
			if (outputSize == null)
		...
		/*
		 * Add a size in the output format. As VideoFormat has no setter, we recreate
		 * the object. Also check whether capture device can output such a size.
		 */
		if ((outputSize != null) && (outputSize.width > 0) && (outputSize.height > 0)) {
			...
			/*
			 * FIXME The assignment to the local variable format makes no difference because
			 * it is no longer user afterwards.
			 */
			format = new VideoFormat(videoFormat.getEncoding(), videoFormatSize, videoFormat.getMaxDataLength(),
	 ...
	 *         attempt to set the specified <tt>mediaFormat</tt> or <tt>null</tt> if
	...
	protected Format setProcessorFormat(TrackControl trackControl, MediaFormatImpl<? extends Format> mediaFormat,
		 ...
		 */
		/*
		 * The current Android video capture device system provided H.264 so it is not
		 * possible to insert an H.264 encoder in the chain. Ideally, we will want to
		 * base the decision on the format of the capture device and not on the
		 * operating system. In a perfect worlds, we will re-implement the functionality
		 * bellow using a Control interface and we will not bother with inserting
		 * customized codecs.
		 */
		if (!OSUtils.IS_ANDROID && "h264/rtp".equalsIgnoreCase(format.getEncoding())) {
			...
			// packetization-mode
			{
				...
				String packetizationMode = (formatParameters == null) ? null
						: formatParameters.get(VideoMediaFormatImpl.H264_PACKETIZATION_MODE_FMTP);
		...
		if (outputSize != null) {
			/*
			 * We have been explicitly told to use a specific output size so insert a
			 * SwScale into the codec chain which is to take care of the specified output
			 * size. However, since the video frames which it will output will be streamed
			 * to a remote peer, preserve the aspect ratio of the input.
			 */
			scaler = new SwScale(/* fixOddYuv420Size */ false, /* preserveAspectRatio */ true);
		...
		if (codecCount != 0) {
			/*
			 * Add our custom SwScale and possibly RTCP aware codec to the codec chain so
			 * that it will be used instead of default.
			 */
			try {
	...
	protected void startedDirectionChanged(MediaDirection oldValue, MediaDirection newValue) {
		...
		for (Player player : getPlayers()) {
			...
			/*
			 * The visual Component of a Player is safe to access and, respectively, report
			 * through a VideoEvent only when the Player is Realized.
			 */
	...
	/**
	 * Extends <tt>SwScale</tt> in order to provide scaling with high quality to a
	 * specific <tt>Player</tt> of remote video.
	 */
	private class PlayerScaler extends SwScale {
		...
		/**
		 * The <tt>Player</tt> into the codec chain of which this <tt>SwScale</tt> is
		 * set.
		 */
		private final Player player;

		/**
		 * Initializes a new <tt>PlayerScaler</tt> instance which is to provide scaling
		 * with high quality to a specific <tt>Player</tt> of remote video.
		 *
		 * @param player the <tt>Player</tt> of remote video into the codec chain of
		 *               which the new instance is to be set
		 */
		 ...
		 * <tt>SizeChangeVideoEvent</tt> because <tt>Player</tt> is unable to do it when
		 * this <tt>SwScale</tt> is scaling to a specific <tt>outputSize</tt>.
		 *
			...
			if (result == BUFFER_PROCESSED_OK) {
				...
				if (inputFormat != null) {
					...
					if ((size != null) && (size.height >= MIN_SWS_SCALE_HEIGHT_OR_WIDTH)
							&& (size.width >= MIN_SWS_SCALE_HEIGHT_OR_WIDTH) && !size.equals(lastSize)) {
						lastSize = size;
		...
		/**
		 * Ensures that this <tt>SwScale</tt> preserves the aspect ratio of its input
		 * video when scaling.
		 *
