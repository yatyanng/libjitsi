...
public class Video4Linux2System extends DeviceSystem {
	...
	/**
	 * The protocol of the <tt>MediaLocator</tt>s identifying <tt>CaptureDevice</tt>
	 * which implement the Video for Linux Two API Specification.
	 */
	 ...
	 * registers <tt>CaptureDevice</tt>s which implement the Video for Linux Two API
	...
	/**
	 * Discovers and registers a <tt>CaptureDevice</tt> implementing the Video for
	 * Linux Two API Specification with a specific device name with JMF.
	 *
	 * @param deviceName the device name of a candidate for a <tt>CaptureDevice</tt>
	 *                   implementing the Video for Linux Two API Specification to
	 *                   be discovered and registered with JMF
	 ...
	 * @throws Exception if anything goes wrong while discovering and registering
	 *                   the specified <tt>CaptureDevice</tt> with JMF
	 */
					...
					try {
						if ((Video4Linux2.ioctl(fd, Video4Linux2.VIDIOC_QUERYCAP, v4l2_capability) != -1)
								&& ((Video4Linux2.v4l2_capability_getCapabilities(v4l2_capability)
										& Video4Linux2.V4L2_CAP_VIDEO_CAPTURE) == Video4Linux2.V4L2_CAP_VIDEO_CAPTURE)) {
	...
	/**
	 * Registers a <tt>CaptureDevice</tt> implementing the Video for Linux Two API
	 * Specification with a specific device name, a specific <tt>open()</tt> file
	 * descriptor and a specific <tt>v4l2_capability</tt> with JMF.
	 *
	...
	private boolean register(String deviceName, int fd, long v4l2_capability) throws Exception {
		...
		CaptureDeviceManager.addDevice(new CaptureDeviceInfo(name,
				new MediaLocator(LOCATOR_PROTOCOL + ":" + deviceName), new Format[] { format }));
		return true;
	}
}
