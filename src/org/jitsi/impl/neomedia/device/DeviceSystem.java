...
public abstract class DeviceSystem extends PropertyChangeNotifier {
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>DeviceSystem</tt> class and its instances
	 * for logging output.
	 */
	 ...
	 * instance at the time its {@link #preInitialize()} method was last invoked.
	 */
	...
	/**
	 * Returns a <tt>List</tt> of <tt>CaptureDeviceInfo</tt>s which are elements of
	 * a specific <tt>List</tt> of <tt>CaptureDeviceInfo</tt>s and have a specific
	 * <tt>MediaLocator</tt> protocol.
	 *
	 * @param deviceList      the <tt>List</tt> of <tt>CaptureDeviceInfo</tt> which
	 *                        are to be filtered based on the specified
	 *                        <tt>MediaLocator</tt> protocol
	 * @param locatorProtocol the protocol of the <tt>MediaLocator</tt>s of the
	 *                        <tt>CaptureDeviceInfo</tt>s which are to be returned
	 * @return a <tt>List</tt> of <tt>CaptureDeviceInfo</tt>s which are elements of
	 *         the specified <tt>deviceList</tt> and have the specified
	 *         <tt>locatorProtocol</tt>
	 */
	protected static List<CaptureDeviceInfo> filterDeviceListByLocatorProtocol(List<CaptureDeviceInfo> deviceList,
	...
	/**
	 * Initializes the <tt>DeviceSystem</tt> instances which are to represent the
	 * supported device systems/backends such as DirectShow, PortAudio, PulseAudio,
	 * QuickTime, video4linux2. The method may be invoked multiple times. If a
	 * <tt>DeviceSystem</tt> has been initialized by a previous invocation of the
	 * method, its {@link #initialize()} method will be called again as part of the
	 * subsequent invocation only if the <tt>DeviceSystem</tt> in question returns a
	 * set of flags from its {@link #getFeatures()} method which contains the
	 * constant/flag {@link #FEATURE_REINITIALIZE}.
	 */
	public static void initializeDeviceSystems() {
		/*
		 * Detect the audio capture devices unless the configuration explicitly states
		 * that they are to not be detected.
		 */
		...
		/*
		 * Detect the video capture devices unless the configuration explicitly states
		 * that they are to not be detected.
		 */
	...
	/**
	 * Initializes the <tt>DeviceSystem</tt> instances which are to represent the
	 * supported device systems/backends which are to capable of capturing and
	 * playing back media of a specific type such as audio or video.
	 *
	 * @param mediaType the <tt>MediaType</tt> of the <tt>DeviceSystem</tt>s to be
	 *                  initialized
	 */
	public static void initializeDeviceSystems(MediaType mediaType) {
		/*
		 * The list of supported DeviceSystem implementations if hard-coded. The order
		 * of the classes is significant and represents a decreasing preference with
		 * respect to which DeviceSystem is to be picked up as the default one (for the
		 * specified mediaType, of course).
		 */
		String[] classNames;

		switch (mediaType) {
		case AUDIO:
			classNames = new String[] { OSUtils.IS_ANDROID ? ".AudioRecordSystem" : null,
					OSUtils.IS_ANDROID ? ".OpenSLESSystem" : null,
					(OSUtils.IS_LINUX || OSUtils.IS_FREEBSD) ? ".PulseAudioSystem" : null,
					OSUtils.IS_WINDOWS ? ".WASAPISystem" : null, OSUtils.IS_MAC ? ".MacCoreaudioSystem" : null,
					OSUtils.IS_ANDROID ? null : ".PortAudioSystem", ".AudioSilenceSystem", ".NoneAudioSystem" };
			break;
		case VIDEO:
			classNames = new String[] { OSUtils.IS_ANDROID ? ".MediaRecorderSystem" : null,
					OSUtils.IS_ANDROID ? ".AndroidCameraSystem" : null,
					(OSUtils.IS_LINUX || OSUtils.IS_FREEBSD) ? ".Video4Linux2System" : null,
					OSUtils.IS_MAC ? ".QuickTimeSystem" : null, OSUtils.IS_WINDOWS ? ".DirectShowSystem" : null,
					".ImgStreamingSystem" };
			break;
		default:
	...
	/**
	 * Initializes the <tt>DeviceSystem</tt> instances specified by the names of the
	 * classes which implement them. If a <tt>DeviceSystem</tt> instance has already
	 * been initialized for a specific class name, no new instance of the class in
	 * question will be initialized and rather the {@link #initialize()} method of
	 * the existing <tt>DeviceSystem</tt> instance will be invoked if the
	 * <tt>DeviceSystem</tt> instance returns a set of flags from its
	 * {@link #getFeatures()} which contains {@link #FEATURE_REINITIALIZE}.
	 *
	 * @param classNames the names of the classes which extend the
	 *                   <tt>DeviceSystem</tt> class and instances of which are to
	 *                   be initialized
	 */
	...
	/**
	 * Invokes {@link #initialize()} on a specific <tt>DeviceSystem</tt>. The method
	 * returns after the invocation returns.
	 *
	 * @param deviceSystem the <tt>DeviceSystem</tt> to invoke <tt>initialize()</tt>
	 *                     on
	 * @throws Exception if an error occurs during the initialization of
	 *                   <tt>initialize()</tt> on the specified
	 *                   <tt>deviceSystem</tt>
	 */
	 ...
	 * @param deviceSystem the <tt>DeviceSystem</tt> to invoke <tt>initialize()</tt>
	 *                     on
	 * @param asynchronous <tt>true</tt> if the invocation is to be performed in a
	...
	static void invokeDeviceSystemInitialize(final DeviceSystem deviceSystem, boolean asynchronous) throws Exception {
		if (OSUtils.IS_WINDOWS || asynchronous) {
			 ...
			 * initialization of the COM library is done per thread. However, there are
			 * multiple concurrency models which may interfere among themselves. Dedicate a
			 * new thread on which the COM library has surely not been initialized per
			...
			/*
			 * Wait for the initialize() invocation on deviceSystem to return i.e. the
			 * thread to die.
			 */
	 ...
	 * {@link #FEATURE_REINITIALIZE} indicates that this instance is able to deal
	 * with multiple consecutive invocations of its {@link #initialize()} method.
	 */
	private final int features;

	/**
	 * The protocol of the <tt>MediaLocator</tt> of the <tt>CaptureDeviceInfo</tt>s
	 * (to be) registered (with FMJ) by this <tt>DeviceSystem</tt>. The protocol is
	 * a unique identifier of a <tt>DeviceSystem</tt>.
	 */
	private final String locatorProtocol;

	/**
	 * The <tt>MediaType</tt> of this <tt>DeviceSystem</tt> i.e. the type of the
	 * media that this instance supports for capture and playback such as audio or
	 * video.
	 */
	...
	/**
	 * Initializes a new <tt>Renderer</tt> instance which is to perform playback on
	 * a device contributed by this system.
	 *
	 * @return a new <tt>Renderer</tt> instance which is to perform playback on a
	 *         device contributed by this system or <tt>null</tt>
	 */
	 ...
	 * initialization of this <tt>DeviceSystem</tt>. This instance has been prepared
	 * for initialization by an earlier call to {@link #preInitialize()} and the
	 * initialization will be completed with a subsequent call to
	 * {@link #postInitialize()}.
	 *
	 ...
	 *                   completed with a subsequent call to
	 *                   <tt>postInitialize()</tt> regardless of any
	 ...
	 *         <tt>FEATURE_XXX</tt> constants defined by the <tt>DeviceSystem</tt>
	...
	/**
	 * Gets the name of the class which implements the <tt>Renderer</tt> interface
	 * to render media on a playback or notification device associated with this
	 * <tt>DeviceSystem</tt>. Invoked by {@link #createRenderer(boolean)}.
	 *
	 ...
	 *         <tt>DeviceSystem</tt> implementation or <tt>createRenderer(boolean)
	 *         is overridden.
	 */
	 ...
	 * example, the capture devices are represented as <tt>CaptureDeviceInfo</tt>
	 * instances registered with FMJ.
	 * <p>
	...
	/**
	 * Invoked as part of the execution of {@link #initialize()} after the execution
	 * of {@link #doInitialize()} regardless of whether the latter completed
	 * successfully. The implementation of <tt>DeviceSystem</tt> fires a new
	 * <tt>PropertyChangeEvent</tt> to notify that the value of the property
	 * {@link #PROP_DEVICES} of this instance may have changed i.e. that the list of
	 * devices detected by this instance may have changed.
	 */
	protected void postInitialize() throws Exception {
		try {
			...
			if (format != null) {
				...
				/*
				 * Fire a PropertyChangeEvent but only if there is an actual change in the value
				 * of the property.
				 */
	 ...
	 * <tt>DeviceSystem</tt> removes from FMJ's <tt>CaptureDeviceManager</tt> the
	 * <tt>CaptureDeviceInfo</tt>s whose <tt>MediaLocator</tt> has the same protocol
	 * as {@link #getLocatorProtocol()} of this instance.
	 */
	...
	/**
	 * Returns a human-readable representation of this <tt>DeviceSystem</tt>. The
	 * implementation of <tt>DeviceSystem</tt> returns the protocol of the
	 * <tt>MediaLocator</tt>s of the <tt>CaptureDeviceInfo</tt>s (to be) registered
	 * by this <tt>DeviceSystem</tt>.
	 *
	 * @return a <tt>String</tt> which represents this <tt>DeviceSystem</tt> in a
	 *         human-readable form
	 */
	@Override
