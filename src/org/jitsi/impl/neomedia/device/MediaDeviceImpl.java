...
public class MediaDeviceImpl extends AbstractMediaDevice {
	...
	/**
	 * The <tt>Logger</tt> used by <tt>MediaDeviceImpl</tt> and its instances for
	 * logging output.
	 */
	 ...
	 *
	 * @param captureDevice the <tt>CaptureDevice</tt> which is to have its calls
	 *                      traced for debugging output
	...
	public static CaptureDevice createTracingCaptureDevice(CaptureDevice captureDevice, final Logger logger) {
		if (captureDevice instanceof PushBufferDataSource) {
			logger.debug("Creating Tracing Capture Device of " + MediaDeviceImpl.toString(captureDevice));
	...
	/**
	 * Returns a human-readable representation of a specific <tt>CaptureDevice</tt>
	 * in the form of a <tt>String</tt> value.
	 *
	 ...
	 *         of the specified <tt>captureDevice</tt>
	 */
	...
	/**
	 * The <tt>MediaType</tt> of this instance and the <tt>CaptureDevice</tt> that
	 * it wraps.
	 */
	private final MediaType mediaType;

	/**
	 * Initializes a new <tt>MediaDeviceImpl</tt> instance which is to provide an
	 * implementation of <tt>MediaDevice</tt> for a <tt>CaptureDevice</tt> with a
	 * specific <tt>CaptureDeviceInfo</tt> and which is of a specific
	 * <tt>MediaType</tt>.
	 *
	 * @param captureDeviceInfo the <tt>CaptureDeviceInfo</tt> of the JMF
	 *                          <tt>CaptureDevice</tt> the new instance is to
	 *                          provide an implementation of <tt>MediaDevice</tt>
	 *                          for
	...
	protected CaptureDevice createCaptureDevice() {
		...
		if (getDirection().allowsSending()) {
			...
			if (exception != null) {
				logger.error("Failed to create CaptureDevice" + " from CaptureDeviceInfo " + captureDeviceInfo,
						exception);
			} else {
				...
				if (logger.isTraceEnabled()) {
					logger.debug("About to create a tracing capture device by using "
							+ captureDevice.getClass().getName() + " " + captureDevice.hashCode());
	 ...
	 * <tt>MediaDevice</tt>. Uses the current <tt>EncodingConfiguration</tt> from
	 * the media service (i.e. the global configuration).
	 *
	 * @param sendPreset    the preset used to set some of the format parameters,
	 *                      used for video and settings.
	 * @param receivePreset the preset used to set the receive format parameters,
	 *                      used for video and settings.
	 ...
	 * and enabled in <tt>encodingConfiguration</tt>.
	 *
	 * @param sendPreset            the preset used to set some of the format
				...
				if ("h264".equalsIgnoreCase(f.getEncoding())) {
					...
					if ((captureDeviceInfo != null)
						...
						if (sendPreset != null)
							...
							/*
							 * XXX We cannot default to any video size here because we do not know how this
							 * MediaDevice instance will be used. If the caller wanted to limit the video
							 * size, she would've specified an actual sendPreset.
							 */
//                            sendSize
//                                = mediaServiceImpl
...
//                                        .getVideoSize();
					...
					advancedAttrs.put("imageattr", MediaUtils.createImageAttr(sendSize, receiveSize));
					f = mediaServiceImpl.getFormatFactory().createMediaFormat(f.getEncoding(), f.getClockRate(),
							f.getFormatParameters(), advancedAttrs);
				}
	 ...
	 *
	 * @return a <tt>String</tt> providing a human-readable representation of this
	 *         instance
	 */
	@Override
