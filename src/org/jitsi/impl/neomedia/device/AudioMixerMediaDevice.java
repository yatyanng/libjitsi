...
public class AudioMixerMediaDevice extends AbstractMediaDevice implements MediaDeviceWrapper {
	/**
	 * The <tt>Logger</tt> used by <tt>AudioMixerMediaDevice</tt> and its instances
	 * for logging output.
	 */
	 ...
	 * registered) with {@link #localUserAudioLevelDispatcher} and which delivers
	 * each of the audio level changes to {@link #localUserAudioLevelListeners}.
	 */
	...
	/**
	 * The <tt>List</tt> where we store all listeners interested in changes of the
	 * local audio level and the number of times each one of them has been added. We
	 * wrap listeners because we may have multiple subscriptions with the same
	 * listener and we would only store it once. If one of the multiple
	 * subscriptions of a particular listener is removed, however, we wouldn't want
	 * to reset the listener to <tt>null</tt> as there are others still interested,
	 * and hence the <tt>referenceCount</tt> in the wrapper.
	 * <p>
	 * <b>Note</b>: <tt>localUserAudioLevelListeners</tt> is a copy-on-write storage
	 * and access to it is synchronized by
	...
	/**
	 * The <tt>List</tt> of RTP extensions supported by this device (at the time of
	 * writing this list is only filled for audio devices and is <tt>null</tt>
	 * otherwise).
	 */
	private List<RTPExtension> rtpExtensions = null;

	/**
	 * The <tt>Map</tt> where we store audio level dispatchers and the streams they
	 * are interested in.
	 */
	...
	/**
	 * Initializes a new <tt>AudioMixerMediaDevice</tt> instance which is to enable
	 * audio mixing on a specific <tt>AudioMediaDeviceImpl</tt>.
	 *
	 * @param device the <tt>AudioMediaDeviceImpl</tt> which the new instance is to
	 *               enable audio mixing on
	 */
	public AudioMixerMediaDevice(AudioMediaDeviceImpl device) {
		/*
		 * AudioMixer is initialized with a CaptureDevice so we have to be sure that the
		 * wrapped device can provide one.
		 */
	...
	public void connect(DataSource captureDevice) throws IOException {
		...
		/*
		 * Unwrap wrappers of the captureDevice until AudioMixingPushBufferDataSource is
		 * found.
		 */
		...
		/*
		 * AudioMixingPushBufferDataSource is definitely not a CaptureDevice and does
		 * not need the special connecting defined by AbstractMediaDevice and
		 * MediaDeviceImpl.
		 */
	...
	/**
	 * Creates a <tt>DataSource</tt> instance for this <tt>MediaDevice</tt> which
	 * gives access to the captured media.
	 *
	 ...
	 * {@inheritDoc}
	 *
	 * Delegates to the {@link AbstractMediaDevice#createRenderer()} implementation
	 * of the <tt>MediaDevice</tt> on which this instance enables mixing i.e.
	 * {@link #getWrappedDevice()}.
	 */
	@Override
	...
	/**
	 * Creates a new <tt>MediaDeviceSession</tt> instance which is to represent the
	 * use of this <tt>MediaDevice</tt> by a <tt>MediaStream</tt>.
	 *
	 * @return a new <tt>MediaDeviceSession</tt> instance which is to represent the
	 *         use of this <tt>MediaDevice</tt> by a <tt>MediaStream</tt>
	...
	public synchronized MediaDeviceSession createSession() {
		if (deviceSession == null) {
			...
			logger.info(getClass().getName() + " " + hashCode() + " creates " + deviceSession.getClass().getName() + " "
					+ deviceSession.hashCode());
		}
		MediaStreamMediaDeviceSession session = new MediaStreamMediaDeviceSession(deviceSession);

		logger.info(getClass().getName() + " " + hashCode() + " returns " + session.getClass().getName() + " "
				+ session.hashCode());
	...
	/**
	 * Notifies the <tt>SimpleAudioLevelListener</tt>s registered with this instance
	 * about the new/current audio level of the local media stream.
	 *
	...
	private void fireLocalUserAudioLevelChanged(int level) {
		...
		/*
		 * XXX These events are going to happen veeery often (~50 times per sec) and
		 * we'd like to avoid creating an iterator every time.
		 */
	 ...
	 * <tt>MediaDevice</tt> (and rather the session it represents). If it still does
			...
			audioMixer = new AudioMixer(device.createCaptureDevice()) {
				...
				protected void read(PushBufferStream stream, Buffer buffer, DataSource dataSource) throws IOException {
					...
					if (dataSource == captureDevice) {
						...
						if ((streamEventDispatcher != null) && !buffer.isDiscard() && (buffer.getLength() > 0)
								&& (buffer.getData() != null)) {
						...
						if ((receiveStreamBufferListener != null) && !buffer.isDiscard() && (buffer.getLength() > 0)
								&& (buffer.getData() != null)) {
	 ...
	 * media.
	 *
	 * @return the <tt>MediaFormat</tt> in which this <tt>MediaDevice</tt> captures
	 *         media
	 ...
	 * extension descriptor indicating <tt>SENDRECV</tt> for mixer-to-client audio
	 * levels.
	 *
	...
	public List<RTPExtension> getSupportedExtensions() {
		if (rtpExtensions == null) {
			rtpExtensions = new ArrayList<RTPExtension>();
	...
	/**
	 * Set the listener which gets notified when this <tt>MediaDevice</tt> reads
	 * data from a <tt>ReceiveStream</tt>
	 *
	 ...
	 * @param sendPreset            not used
	 * @param receivePreset         not used
	 * @param encodingConfiguration the <tt>EncodingConfiguration</tt> instance to
	 *                              use
	 * @return the list of <tt>MediaFormat</tt>s supported by this
	 ...
	 * <tt>DataSourceFilter</tt> from the list of input <tt>DataSource</tt> of the
	 * <tt>AudioMixer</tt> of this <tt>AudioMixerMediaDevice</tt> from which it
	...
	private class AudioMixerMediaDeviceSession extends MediaDeviceSession {
		/**
		 * The list of <tt>MediaDeviceSession</tt>s of <tt>MediaStream</tt>s which use
		 ...
		 * represent the <tt>MediaDeviceSession</tt> of this <tt>AudioMixer</tt> with
		...
		/**
		 * Adds <tt>l</tt> to the list of listeners that are being notified of new local
		 * audio levels as they change. If <tt>l</tt> is added multiple times it would
		...
		/**
		 * Adds a specific <tt>MediaStreamMediaDeviceSession</tt> to the mix represented
		 * by this instance so that it knows when it is in use.
		 *
		 * @param mediaStreamMediaDeviceSession the
		 *                                      <tt>MediaStreamMediaDeviceSession</tt>
		 *                                      to be added to the mix represented by
		 *                                      this instance
		 */
		 ...
		 *
		 * @param playbackDataSource the <tt>DataSource</tt> providing remote audio to
		 *                           be added to the mix produced by the associated
		 *                           <tt>MediaDevice</tt>
		 */
		@Override
		public void addPlaybackDataSource(DataSource playbackDataSource) {
			/*
			 * We don't play back the contributions of the conference members separately, we
			 * have a single playback of the mix of all contributions but ours.
			 */
			super.addPlaybackDataSource(getCaptureDevice());
		}

		/**
		 * Adds a specific <tt>ReceiveStream</tt> to the list of <tt>ReceiveStream</tt>s
		 * known to this instance to be contributing audio to the mix produced by its
		 * associated <tt>AudioMixer</tt>.
		 *
		 * @param receiveStream the <tt>ReceiveStream</tt> to be added to the list of
		 ...
		 * from. Since this is the <tt>MediaDeviceSession</tt> of this
		 * <tt>AudioMixer</tt> with its <tt>MediaDevice</tt>, returns the
		 * <tt>localOutputDataSource</tt> of the <tt>AudioMixer</tt> i.e. the
		 * <tt>DataSource</tt> which represents the mix of all <tt>ReceiveStream</tt>s
		 * and excludes the captured data from the <tt>MediaDevice</tt> of the
		 * <tt>AudioMixer</tt>.
		 *
		...
		protected Player createPlayer(DataSource dataSource) {
			 ...
			 * AudioMixerMediaDeviceSession should wrap a MediaDeviceSession of that same
			 * wrapped MediaDevice.
			 */
		...
		/**
		 * Sets the <tt>VolumeControl</tt> which is to control the volume (level) of the
		 * audio (to be) played back by this instance.
		 *
		 * @param outputVolumeControl the <tt>VolumeControl</tt> which is to be control
		 *                            the volume (level) of the audio (to be) played
		 ...
		 * <tt>stream</tt>.
		 *
		 * @param stream   the stream that <tt>l</tt> would like to register as an audio
		 *                 level listener for.
		 * @param listener the listener we'd like to register for notifications from
		 *                 <tt>stream</tt>.
		 */
		 ...
		 * <tt>VolumeControl</tt> of the returned <tt>Renderer</tt> for the purposes of
		 * having call/telephony conference-specific volume (levels).
		 */
		@Override
		...
		/**
		 * Removes <tt>l</tt> from the list of listeners that are being notified of
		 * local audio levels.If <tt>l</tt> is not in the list, the method has no
		 * effect.
		 *
		...
		/**
		 * Removes a specific <tt>MediaStreamMediaDeviceSession</tt> from the mix
		 * represented by this instance. When the last
		 * <tt>MediaStreamMediaDeviceSession</tt> is removed from this instance, it is
		 * no longer in use and closes itself thus signaling to its <tt>MediaDevice</tt>
		 * that it is no longer in use.
		 *
		 * @param mediaStreamMediaDeviceSession the
		 *                                      <tt>MediaStreamMediaDeviceSession</tt>
		 *                                      to be removed from the mix represented
		 *                                      by this instance
		 */
		...
		/**
		 * Removes a specific <tt>DataSource</tt> providing remote audio from the mix
		 * produced by the associated <tt>AudioMixer</tt>.
		 *
		 * @param playbackDataSource the <tt>DataSource</tt> providing remote audio to
		 *                           be removed from the mix produced by the associated
		 *                           <tt>AudioMixer</tt>
		 */
		@Override
		 ...
		 * <tt>ReceiveStream</tt>s known to this instance to be contributing audio to
		 * the mix produced by its associated <tt>AudioMixer</tt>.
		 *
		 * @param receiveStream the <tt>ReceiveStream</tt> to be removed from the list
		 *                      of <tt>ReceiveStream</tt>s known to this instance to be
		 *                      contributing audio to the mix produced by its associated
		 *                      <tt>AudioMixer</tt>
		 */
		@Override
	...
	/**
	 * Represents the work of a <tt>MediaStream</tt> with the <tt>MediaDevice</tt>
	 * of an <tt>AudioMixer</tt> and the contribution of that <tt>MediaStream</tt>
	 * to the mix.
	 */
	private static class MediaStreamMediaDeviceSession extends AudioMediaDeviceSession
			implements PropertyChangeListener {
		/**
		 * The <tt>MediaDeviceSession</tt> of the <tt>AudioMixer</tt> that this instance
		 * exposes to a <tt>MediaStream</tt>. While there are multiple
		...
		/**
		 * We use this field to keep a reference to the listener that we've registered
		 * with the audio mixer for local audio level notifications. We use this
		 * reference so that we could unregister it if someone resets it or sets it to
		 * <tt>null</tt>.
		 */
		...
		/**
		 * We use this field to keep a reference to the listener that we've registered
		 * with the audio mixer for stream audio level notifications. We use this
		 * reference so because at the time we get it from the <tt>MediaStream</tt> it
		 * might be too early to register it with the mixer as it is like that we don't
		 * have a receive stream yet. If that's the case, we hold on to the listener and
		 * register it only when we get the <tt>ReceiveStream</tt>.
		 */
		 ...
		 * represent the work of a <tt>MediaStream</tt> with the <tt>MediaDevice</tt> of
		 * this <tt>AudioMixer</tt> and its contribution to the mix.
		 *
		 * @param audioMixerMediaDeviceSession the <tt>MediaDeviceSession</tt> of the
		 *                                     <tt>AudioMixer</tt> with its
		 *                                     <tt>MediaDevice</tt> which the new
		 *                                     instance is to delegate to in order to
		...
		protected String getName() {
			return getClass().getName() + " " + hashCode() + " which contains "
					+ audioMixerMediaDeviceSession.getClass().getName() + " " + audioMixerMediaDeviceSession.hashCode();
		}
		...
		protected Player createPlayer(DataSource dataSource) {
			/*
			 * We don't want the contribution of each conference member played back
			 * separately, we want the one and only mix of all contributions but ours to be
			 * played back once for all of them.
			 */
			return null;
		}

		/**
		 * Returns the list of SSRC identifiers that are directly contributing to the
		 * media flows that we are sending out. Note that since this is a pseudo device
		 * we would simply be delegating the call to the corresponding method of the
		 * master mixer device session.
		 *
		 * @return a <tt>long[]</tt> array of SSRC identifiers that are currently
		 *         contributing to the mixer encapsulated by this device session.
		 */
		@Override
		...
		/**
		 * The method relays <tt>PropertyChangeEvent</tt>s indicating a change in the
		 * SSRC_LIST in the encapsulated mixer device so that the <tt>MediaStream</tt>
		 * that uses this device session can update its CSRC list.
		 *
		 * @param evt that <tt>PropertyChangeEvent</tt> whose old and new value we will
		 *            be relaying to the stream.
		 */
		@Override
		...
		protected void receiveStreamAdded(ReceiveStream receiveStream) {
			...
			/*
			 * If someone registered a stream level listener, we can now add it since we
			 * have the stream that it's supposed to listen to.
			 */
		...
		/**
		 * Override it here cause we won't register effects to that stream cause we
		 * already have one.
		 *
		 ...
		 * master/audioMixerMediaDeviceSession because
		 * <tt>MediaStreamMediaDeviceSession</tt> does not perform playback/rendering.
		 */
		@Override
		...
		/**
		 * Adds <tt>listener</tt> to the list of <tt>SimpleAudioLevelListener</tt>s
		 * registered with the mixer session that this "slave session" encapsulates.
		 * This class does not keep a reference to <tt>listener</tt>.
		 *
		 * @param listener the <tt>SimpleAudioLevelListener</tt> that we are to pass to
		 *                 the mixer device session or <tt>null</tt> if we are trying to
		 *                 unregister it.
		 */
		@Override
		...
		/**
		 * Returns the last audio level that was measured by the underlying mixer for
		 * the specified <tt>csrc</tt>.
		 *
		 * @param csrc the CSRC ID whose last measured audio level we'd like to
		 *             retrieve.
		 *
		 * @return the audio level that was last measured by the underlying mixer for
		 *         the specified <tt>csrc</tt> or <tt>-1</tt> if the <tt>csrc</tt> does
		 *         not belong to neither of the conference participants.
		 */
		@Override
		 ...
		 * <tt>CaptureDevice</tt>. If we are muted we just remove the local level
	...
	/**
	 * A very lightweight wrapper that allows us to track the number of times that a
	 * particular listener was added.
	 */
	private static class SimpleAudioLevelListenerWrapper {
		...
		/**
		 * Returns <tt>true</tt> if <tt>obj</tt> is a wrapping the same listener as
		 * ours.
		 *
		 * @param obj the wrapper we'd like to compare to this instance
		 *
		 * @return <tt>true</tt> if <tt>obj</tt> is a wrapping the same listener as
		 *         ours.
		 */
		@Override
		...
		public int hashCode() {
			/*
			 * Equality is based on the listener field only so its hashCode is enough.
			 * Besides, it's the only immutable of this instance i.e. the only field
			 * appropriate for the calculation of the hashCode.
			 */
