...
public class PortAudioSystem extends AudioSystem2 {
	 ...
	 *                     retrieved for the PortAudio device with the specified
	 *                     device index as an input device or <tt>false</tt> for an
	 *                     output device
	 ...
	 * @return a sample rate supported by the PortAudio device with the specified
	...
	private static double getSupportedSampleRate(boolean input, int deviceIndex, int channelCount, long sampleFormat) {
		...
		if (deviceInfo != 0) {
			...
			if (defaultSampleRate >= MediaUtils.MAX_AUDIO_SAMPLE_RATE)
				...
				long streamParameters = Pa.StreamParameters_new(deviceIndex, channelCount, sampleFormat,
				...
				if (streamParameters == 0)
					...
					try {
						...
						boolean formatIsSupported = Pa.IsFormatSupported(inputParameters, outputParameters,
	 ...
	 *
	 * @param diagnosticsControl the <tt>DiagnosticsControl</tt> to be placed under
	 *                           monitoring of its functional health because of a
	 ...
	 * capture and playback devices by enumerating all host devices with input
	 * channels.
	 *
	...
	protected void doInitialize() throws Exception {
		/*
		 * If PortAudio fails to initialize because of, for example, a missing native
		 * counterpart, it will throw an exception here and the PortAudio Renderer will
		 * not be initialized.
		 */
		...
		for (int deviceIndex = 0; deviceIndex < deviceCount; deviceIndex++) {
			...
			/*
			 * TODO The intention of reinitialize() was to perform the initialization from
			 * scratch. However, AudioSystem was later changed to disobey. But we should at
			 * least search through both CAPTURE_INDEX and PLAYBACK_INDEX.
			 */
			...
			if (cdi == null) {
				cdi = new CaptureDeviceInfo2(name, new MediaLocator(LOCATOR_PROTOCOL + ":#" + locatorRemainder),
						new Format[] { new AudioFormat(AudioFormat.LINEAR,
								(maxInputChannels > 0)
										? getSupportedSampleRate(true, deviceIndex, channels, sampleFormat)
										: Pa.DEFAULT_SAMPLE_RATE,
								sampleSizeInBits, channels, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
			...
			/*
			 * When we perform automatic selection of capture and playback/notify devices,
			 * we would like to pick up devices from one and the same hardware because that
			 * sound like a natural expectation from the point of view of the user. In order
			 * to achieve that, we will bring the devices which support both capture and
			 * playback to the top.
			 */
		...
		/*
		 * Make sure that devices which support both capture and playback are reported
		 * as such and are preferred over devices which support either capture or
		 * playback (in order to achieve our goal to have automatic selection pick up
		 * devices from one and the same hardware).
		 */
		bubbleUpUsbDevices(captureDevices);
		bubbleUpUsbDevices(playbackDevices);
		if (!captureDevices.isEmpty() && !playbackDevices.isEmpty()) {
			/*
			 * Event if we have not been provided with the information regarding the
			 * matching of the capture and playback/notify devices from one and the same
			 * hardware, we may still be able to deduce it by examining their names.
			 */
			matchDevicesByName(captureDevices, playbackDevices);
		}
		/*
		 * Of course, of highest reliability is the fact that a specific instance
