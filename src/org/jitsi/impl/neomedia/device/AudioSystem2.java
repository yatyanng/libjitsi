...
public abstract class AudioSystem2 extends AudioSystem {
	/**
	 * The <tt>Logger</tt> used by the <tt>AudioSystem2</tt> class and its instances
	 * for logging output.
	 */
	private static final Logger logger = Logger.getLogger(AudioSystem2.class);

	/**
	 * The number of times that {@link #willOpenStream()} has been invoked without
	 * an intervening {@link #didOpenStream()} i.e. the number of API clients who
	 * are currently executing a <tt>Pa_OpenStream</tt>-like function and which are
	 * thus inhibiting <tt>updateAvailableDeviceList()</tt>.
	 */
	...
	/**
	 * The number of times that {@link #willUpdateAvailableDeviceList()} has been
	 * invoked without an intervening {@link #didUpdateAvailableDeviceList()} i.e.
	 * the number of API clients who are currently executing
	 * <tt>updateAvailableDeviceList()</tt> and who are thus inhibiting
	 * <tt>openStream</tt>.
	 */
	...
	/**
	 * The <tt>Object</tt> which ensures that this <tt>AudioSystem</tt>'s function
	 * to update the list of available devices will not be invoked concurrently. The
	 * condition should hold true on the native side but, anyway, it should not hurt
	 * (much) to enforce it on the Java side as well.
	 */
	 ...
	 * invoked.
	 * <p>
	 * <b>Note</b>: The <tt>AudioSystem2</tt> class keeps a <tt>WeakReference</tt>
	 * to the specified <tt>listener</tt> in order to avoid memory leaks.
	 * </p>
	 *
	 * @param listener the <tt>UpdateAvailableDeviceListListener</tt> to be notified
	 ...
	 * representing USB devices appear at the beginning/top of the specified list.
	 *
	 * @param devices the list of <tt>CaptureDeviceInfo2</tt>s to be sorted so that
	 ...
	 * <tt>AudioSystem</tt>'s method <tt>updateAvailableDeviceList()</tt> will be or
	 * was invoked.
	 *
				...
				if (l != null) {
					try {
						...
						if (t instanceof ThreadDeath) {
							...
							logger.error("UpdateAvailableDeviceListListener." + (will ? "will" : "did")
									+ "UpdateAvailableDeviceList failed.", t);
						}
					}
				}
			}
	 ...
	 * <tt>CaptureDeviceInfo2</tt>s so that devices from the same hardware appear at
	 * the same indices in the respective lists. The judgment with respect to the
	 * belonging to the same hardware is based on the names of the specified
	 * <tt>CaptureDeviceInfo2</tt>s. The implementation is provided as a fallback to
	 * stand in for scenarios in which more accurate relevant information is not
	 * available.
	 *
	...
	protected static void matchDevicesByName(List<CaptureDeviceInfo2> captureDevices,
		...
		Pattern pattern = Pattern.compile("array|headphones|microphone|speakers|\\p{Space}|\\(|\\)",
	...
	/**
	 * Reinitializes this <tt>AudioSystem</tt> in order to bring it up to date with
	 * possible changes in the list of available devices. Invokes
	 * <tt>updateAvailableDeviceList()</tt> to update the devices on the native side
	 * and then {@link #initialize()} to reflect any changes on the Java side.
	 * Invoked by the native side of this <tt>AudioSystem</tt> when it detects that
	...
	protected void reinitialize() throws Exception {
		...
		/*
		 * XXX We will likely minimize the risk of crashes on the native side even
		 * further by invoking initialize() with updateAvailableDeviceList() locked.
		 * Unfortunately, that will likely increase the risks of deadlocks on the Java
		 * side.
		 */
	...
	/**
	 * Waits for all API clients to finish executing a <tt>Pa_OpenStream</tt>-like
	 * function.
	 */
