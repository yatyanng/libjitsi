...
public class CoreAudioDevice {
	...
	static {
		...
		try {
			...
			/*
			 * Swallow whatever exceptions are known to be thrown by System.loadLibrary()
			 * because the class has to be loaded in order to not prevent the loading of its
			 * users and isLoaded will remain false eventually.
			 */
	...
	/**
	 * Implements a callback which gets called by the native coreaudio counterpart
	 * to notify the Java counterpart that the list of devices has changed.
	 */
