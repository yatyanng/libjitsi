...
public class DeviceConfiguration extends PropertyChangeNotifier implements PropertyChangeListener {
	...
	/**
	 * The name of the <tt>DeviceConfiguration</tt> property which represents the
	 * device used by <tt>DeviceConfiguration</tt> for audio capture.
	 */
	public static final String AUDIO_CAPTURE_DEVICE = CaptureDevices.PROP_DEVICE;

	/**
	 * The name of the <tt>DeviceConfiguration</tt> property which represents the
	 * device used by <tt>DeviceConfiguration</tt> for audio notify.
	 */
	public static final String AUDIO_NOTIFY_DEVICE = NotifyDevices.PROP_DEVICE;

	/**
	 * The name of the <tt>DeviceConfiguration</tt> property which represents the
	 * device used by <tt>DeviceConfiguration</tt> for audio playback.
	 */
	...
	private static final String[] CUSTOM_RENDERERS = new String[] {
			...
			OSUtils.IS_LINUX ? ".audio.PulseAudioRenderer" : null, OSUtils.IS_WINDOWS ? ".audio.WASAPIRenderer" : null,
	...
	/**
	 * The default value to be used for the {@link #PROP_AUDIO_ECHOCANCEL} property
	 * when it does not have a value.
	 */
	 ...
	 * {@link #PROP_AUDIO_ECHOCANCEL_FILTER_LENGTH_IN_MILLIS} property when it does
	 * not have a value. The recommended filter length is approximately the third of
	 * the room reverberation time. For example, in a small room, reverberation time
	 * is in the order of 300 ms, so a filter length of 100 ms is a good choice (800
	...
	/**
	 * The name of the <tt>long</tt> property which determines the filter length in
	 * milliseconds to be used by the echo cancellation implementation. The
	...
	/**
	 * The property we use to store the settings for maximum allowed video bandwidth
	 * (used to normalize RTP traffic, and not in codec configuration)
	 */
	...
	/**
	 * The name of the <tt>DeviceConfiguration</tt> property which represents the
	 * device used by <tt>DeviceConfiguration</tt> for video capture.
	 */
	public static final String VIDEO_CAPTURE_DEVICE = "VIDEO_CAPTURE_DEVICE";

	/**
	 * Fixes the list of <tt>Renderer</tt>s registered with FMJ in order to resolve
	...
	private static void fixRenderers() {
		...
		if (OSUtils.IS_WINDOWS) {
			if (OSUtils.IS_WINDOWS32 && (OSUtils.IS_WINDOWS_VISTA || OSUtils.IS_WINDOWS_7)) {
				...
				/*
				 * Remove the native Renderers for 64-bit Windows because native JMF libs are
				 * not available for 64-bit machines.
				 */
	 ...
	 * {@link MediaServiceImpl#DISABLE_SET_AUDIO_SYSTEM_PNAME} at the time of the
	...
	public DeviceConfiguration() {
		...
		/*
		 * Adds this instance as a PropertyChangeListener to all DeviceSystems which
		 * support reinitialization/reloading in order to be able, for example, to
		 * switch from a default/automatic selection of "None" to a DeviceSystem which
		 * has started providing at least one device at runtime.
		 */
	 ...
	 * <tt>DeviceSystem</tt>s which support reinitialization/reloading in order to
	 * be able, for example, to switch from a default/automatic selection of
	 * &quot;None&quot; to an <tt>DeviceSystem</tt> which has started providing at
	...
	private void extractConfiguredAudioCaptureDevices() {
		...
		if ((availableAudioSystems != null) && (availableAudioSystems.length != 0)) {
			...
			if (audioSystem != null) {
				 ...
				 * selection (i.e. the value of the associated ConfigurationService property) to
				 * be respected or execute too early in the life of the library/application to
...
//                }
			...
			if (audioSystem == null) {
				...
				if (cfg != null) {
					...
					if (locatorProtocol != null) {
						 ...
						 * selection of a particular AudioSystem, always use the configured AudioSystem
	...
	/**
	 * Detects capture devices configured through JMF and disable audio and/or video
	 * transmission if none were found.
	 */
	...
	private void extractConfiguredVideoCaptureDevices() {
		...
		if (NoneAudioSystem.LOCATOR_PROTOCOL.equalsIgnoreCase(videoCaptureDeviceString)) {
			...
			Format[] formats = new Format[] { new AVFrameFormat(), new VideoFormat(Constants.ANDROID_SURFACE),
					new VideoFormat(VideoFormat.RGB), new VideoFormat(VideoFormat.YUV),
	 ...
	 *
	 * @return an array of <tt>CaptureDeviceInfo</tt> describing the audio capture
	 *         devices available through this <tt>DeviceConfiguration</tt>
	 */
	 ...
	 * <tt>DeviceConfiguration</tt>, amongst which is
	 * {@link #getVideoCaptureDevice(MediaUseCase)} and represent acceptable values
	 ...
	 *                <tt>MediaUseCase</tt>
	 * @return an array of <tt>CaptureDeviceInfo</tt> describing the video capture
	 *         devices available through this <tt>DeviceConfiguration</tt>
	 */
	public List<CaptureDeviceInfo> getAvailableVideoCaptureDevices(MediaUseCase useCase) {
		Format[] formats = new Format[] { new AVFrameFormat(), new VideoFormat(Constants.ANDROID_SURFACE),
				new VideoFormat(VideoFormat.RGB), new VideoFormat(VideoFormat.YUV), new VideoFormat(Constants.H264) };
		...
		for (Format format : formats) {
			...
			if (useCase != MediaUseCase.ANY) {
				for (CaptureDeviceInfo cdi : cdis) {
					MediaUseCase cdiUseCase = DeviceSystem.LOCATOR_PROTOCOL_IMGSTREAMING.equalsIgnoreCase(
							cdi.getLocator().getProtocol()) ? MediaUseCase.DESKTOP : MediaUseCase.CALL;
	 ...
	 *
	 * @return the frame rate set on this <tt>DeviceConfiguration</tt>. The default
	 *         value is {@link #DEFAULT_VIDEO_FRAMERATE}
	 */
	 ...
	 * <tt>ConfigurationService</tt> and the <tt>DeviceSystem</tt>s which support
	 * reinitialization/reloading.
	 *
	...
	public void propertyChange(PropertyChangeEvent ev) {
		...
		if (AUDIO_CAPTURE_DEVICE.equals(propertyName) || AUDIO_NOTIFY_DEVICE.equals(propertyName)
			 ...
			 * have been selected because the user's selection may have been unavailable at
			 * the time. Make sure that the user's selection is respected if possible.
			 */
			extractConfiguredAudioCaptureDevices();

			/*
			 * The specified PropertyChangeEvent has been fired by a DeviceSystem i.e. a
			 * certain DeviceSystem is the source. Translate it to a PropertyChangeEvent
			 * fired by this instance.
			 */
			AudioSystem audioSystem = getAudioSystem();
	...
	/**
	 * Registers the custom <tt>Renderer</tt> implementations defined by class name
	 * in {@link #CUSTOM_RENDERERS} with JMF.
	 */
	private void registerCustomRenderers() {
		...
		for (String customRenderer : CUSTOM_RENDERERS) {
			...
			if ((renderers == null) || !renderers.contains(customRenderer)) {
				try {
					...
					PlugInManager.addPlugIn(customRenderer, customRendererInstance.getSupportedInputFormats(), null,
		...
		/*
		 * Just in case, bubble our JMF contributions at the top so that they are
	...
	/**
	 * Sets the device which is to be used by this <tt>DeviceConfiguration</tt> for
	 * video capture.
	 *
	 * @param device a <tt>CaptureDeviceInfo</tt> describing device to be used by
	...
	public void setVideoCaptureDevice(CaptureDeviceInfo device, boolean save) {
		if (videoCaptureDevice != device) {
			...
			if (save) {
				...
				if (cfg != null) {
					cfg.setProperty(PROP_VIDEO_DEVICE, (videoCaptureDevice == null) ? NoneAudioSystem.LOCATOR_PROTOCOL
