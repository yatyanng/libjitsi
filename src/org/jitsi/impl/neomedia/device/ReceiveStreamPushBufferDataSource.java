...
public class ReceiveStreamPushBufferDataSource extends PushBufferDataSourceDelegate<PushBufferDataSource> {
	/**
	 * Sets a <tt>BufferTransferHandler</tt> on a specific <tt>ReceiveStream</tt>
	 * which reads data as soon as possible and throws it away.
	 *
	 * @param receiveStream the <tt>ReceiveStream</tt> on which to set a
	 *                      <tt>BufferTransferHandler</tt> which reads data as soon
	 *                      as possible and throws it away
	 */
	...
	/**
	 * The <tt>ReceiveStream</tt> which has its <tt>DataSource</tt> wrapped by this
	 * instance. Currently, remembered just to be made available to callers in case
	 * they need it and not used by this instance.
	 */
	private final ReceiveStream receiveStream;

	/**
	 * The indicator which determines whether {@link DataSource#disconnect()} is to
	 * be called on the wrapped <tt>DataSource</tt> when it is called on this
	 * instance.
	 */
	private boolean suppressDisconnect;

	/**
	 * Initializes a new <tt>ReceiveStreamPushBufferDataSource</tt> instance which
	 * is to wrap a specific <tt>DataSource</tt> of a specific
	 ...
	 * is to wrap a specific <tt>DataSource</tt> of a specific
	 * <tt>ReceiveStream</tt> for the purposes of enabling explicitly control of
	 * calls to its {@link DataSource#disconnect()} and, optionally, activates the
	 * suppresses the call in question.
	 *
	 * @param receiveStream      the <tt>ReceiveStream</tt> which is to have its
	 *                           <tt>DataSource</tt>
	 * @param dataSource         the <tt>DataSource</tt> of <tt>receiveStream</tt>
	 *                           which is to be wrapped by this instance
	 * @param suppressDisconnect <tt>true</tt> if calls to
	 *                           <tt>DataSource#disconnect()</tt> on the wrapped
	 *                           <tt>dataSource</tt> are to be suppressed when there
	 *                           are such calls on the new instance; otherwise,
	 *                           <tt>false</tt>
	 */
	public ReceiveStreamPushBufferDataSource(ReceiveStream receiveStream, PushBufferDataSource dataSource,
	 ...
	 * <tt>DataSource</tt> if it has not been explicitly suppressed by setting the
	 * <tt>suppressDisconnect</tt> property of this instance.
	 */
	@Override
	...
	/**
	 * Gets the <tt>ReceiveStream</tt> which has its <tt>DataSource</tt> wrapped by
	 * this instance.
	 *
	 * @return the <tt>ReceiveStream</tt> which has its <tt>DataSource</tt> wrapped
	 *         by this instance
	 */
	 ...
	 * {@link DataSource#disconnect()} on the wrapped <tt>DataSource</tt> are to be
	 * suppressed when there are such calls on this instance.
	 *
	 * @param suppressDisconnect <tt>true</tt> to suppress calls to
	 *                           <tt>DataSource#disconnect()</tt> on the wrapped
	 *                           <tt>DataSource</tt> when there are such calls on
