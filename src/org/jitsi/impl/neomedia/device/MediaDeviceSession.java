...
//            }
            ...
            if (match.isSameEncoding(format))
            {
                ...
                if (match instanceof AudioFormat)
                {
                    if (formatSampleRate != Format.NOT_SPECIFIED)
                    {
                        ...
                        logger.debug("Format Sample Rate: "+formatSampleRate
                        		+", Match Sample Rate: "+matchSampleRate);
    ...
    protected void processorControllerUpdate(ControllerEvent ev)
    {
        ...
        if (ev instanceof ConfigureCompleteEvent)
        {
            ...
            if (processor != null)
            {
                ...
                if (format != null) {
                	logger.debug("Controller Event Update: " 
                		+ getClass().getName() + " " + hashCode()
                		+ " sets format "+format+" to "
    ...
    public void setFormat(MediaFormat format)
    {
        ...
        if (processor != null)
        {
            ...
            logger.trace(
                    "Processor's State is " + processorState
                        + " on "
                        + getClass().getName() + " " + hashCode());
            ...
            MediaFormatImpl<? extends Format> mediaFormat)
    {
        ...
        for (int trackIndex = 0;
                ...
                trackIndex++)
        {
        	logger.debug("Setting Processor Media Format in track " 
        			+ trackIndex +"/"+trackControls.length
        			+ " of "+processor.getClass().getName()+" "+processor.hashCode());
            ...
            logger.debug("The number of supported formats is " 
        			+ supportedFormats.length
        			+ " of "+trackControl.getClass().getName()+" "+trackControl.hashCode());
    ...
    protected Format setProcessorFormat(
            ...
            Format format)
    {
    	logger.debug("Start Setting Media Format "+format+" in " 
    		 +trackControl.getClass().getName()+" "+trackControl.hashCode());
    	
    	Format newFormat = trackControl.setFormat(format);
    	
    	logger.debug("Finished Setting Media Format "+format+" in " 
       		 +trackControl.getClass().getName()+" "+trackControl.hashCode());
