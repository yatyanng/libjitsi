...
import org.jitsi.util.ForbiddenManager;
...
public class MediaDeviceSession extends PropertyChangeNotifier {
	...
	/**
	 * The name of the <tt>MediaDeviceSession</tt> instance property the value of
	 * which represents the output <tt>DataSource</tt> of the
	 * <tt>MediaDeviceSession</tt> instance which provides the captured (RTP) data
	...
	/**
	 * The name of the property that corresponds to the array of SSRC identifiers
	 * that we store in this <tt>MediaDeviceSession</tt> instance and that we update
	 * upon adding and removing <tt>ReceiveStream</tt>
	 */
	public static final String SSRC_LIST = "SSRC_LIST";

	/**
	 * The JMF <tt>DataSource</tt> of {@link #device} through which this instance
	 * accesses the media captured by it.
	 */
	private DataSource captureDevice;

	/**
	 * The indicator which determines whether {@link DataSource#connect()} has been
	 * successfully executed on {@link #captureDevice}.
	 */
	...
	/**
	 * The indicator which determines whether this <tt>MediaDeviceSession</tt> is
	 * set to output "silence" instead of the actual media captured from
	 * {@link #captureDevice}.
	 */
	private boolean mute = false;

	/**
	 * The list of playbacks of <tt>ReceiveStream</tt>s and/or <tt>DataSource</tt>s
	 * performed by respective <tt>Player</tt>s on the <tt>MediaDevice</tt>
	 * represented by this instance. The (read and write) accesses to the field are
	 * to be synchronized using {@link #playbacksLock}.
	 */
	private final List<Playback> playbacks = new LinkedList<Playback>();

	/**
	 * The <tt>ReadWriteLock</tt> which is used to synchronize the (read and write)
	 * accesses to {@link #playbacks}.
	 */
	...
	private final ControllerListener playerControllerListener = new ControllerListener() {
		/**
		 * Notifies this <tt>ControllerListener</tt> that the <tt>Controller</tt> which
		 * it is registered with has generated an event.
		 *
	...
	/**
	 * The JMF <tt>Processor</tt> which transcodes {@link #captureDevice} into the
	 ...
	 * value of <tt>false</tt> does not mean that <tt>processor</tt> exists or that
	 * it is not closed, it just means that if <tt>processor</tt> failed to be
	 * initialized or it received a <tt>ControllerClosedEvent</tt>, it was at an
	 * expected time of its execution and that the fact in question was reflected,
	 * for example, by setting <tt>processor</tt> to <tt>null</tt>. If there is no
	 * <tt>processorIsPrematurelyClosed</tt> field and <tt>processor</tt> is set to
	 * <tt>null</tt> or left existing after the receipt of
	 * <tt>ControllerClosedEvent</tt>, it will either lead to not firing a
	 * <tt>PropertyChangeEvent</tt> for <tt>OUTPUT_DATA_SOURCE</tt> when it has
	 * actually changed and, consequently, cause the <tt>SendStream</tt>s of
	 * <tt>MediaStreamImpl</tt> to not be recreated or it will be impossible to
	 * detect that <tt>processor</tt> cannot have its format set and will thus be
	 * left broken even for subsequent calls to {@link #setFormat(MediaFormat)}.
	 */
	private boolean processorIsPrematurelyClosed;

	/**
	 * The list of SSRC identifiers representing the parties that we are currently
	 * handling receive streams from.
	 */
	...
	/**
	 * Whether output size has changed after latest processor config. Used for video
	 * streams.
	 */
	protected boolean outputSizeChanged = false;

	/**
	 * Initializes a new <tt>MediaDeviceSession</tt> instance which is to represent
	 * the use of a specific <tt>MediaDevice</tt> by a <tt>MediaStream</tt>.
	 *
	...
	/**
	 * Sets the indicator which determines whether this instance is to dispose of
	 * its associated player upon closing.
	 *
	 * @param disposePlayerOnClose <tt>true</tt> to have this instance dispose of
	 *                             its associated player upon closing; otherwise,
	 *                             <tt>false</tt>
	 */
	...
	/**
	 * Adds <tt>ssrc</tt> to the array of SSRC identifiers representing parties that
	 * this <tt>MediaDeviceSession</tt> is currently receiving streams from. We use
	 * this method mostly as a way of to caching SSRC identifiers during a
	 * conference call so that the streams that are sending CSRC lists could have
	 * them ready for use rather than have to construct them for every RTP packet.
	 *
	...
	/**
	 * For JPEG and H263, we know that they only work for particular sizes. So we'll
	 * perform extra checking here to make sure they are of the right sizes.
	 *
...
//            }
		...
		VideoFormat result = new VideoFormat(null, new Dimension(width, height), Format.NOT_SPECIFIED, null,
	 ...
	 * <tt>MediaDevice</tt> of this instance. Allows extenders to override and
	 * customize the check.
	 *
	...
	public void close() {
		try {
			...
			/*
			 * XXX The order of stopping the playback and capture is important here because
			 * when we use echo cancellation the capture accesses data from the playback and
			 * thus there is synchronization to avoid segfaults but this synchronization can
			 * sometimes lead to a slow stop of the playback. That is why we stop the
			 * capture first.
			 */
	...
	private void closeProcessor() {
		if (processor != null) {
			...
			/*
			 * Once the processor uses the captureDevice, the captureDevice has to be
			 * reconnected on its next use.
			 */
			disconnectCaptureDevice();
		}
	}
	...
	/**
	 * Creates the <tt>DataSource</tt> that this instance is to read captured media
	 * from.
	 *
	 * @return the <tt>DataSource</tt> that this instance is to read captured media
	 *         from
	 */
	...
	/**
	 * Creates a new <tt>Player</tt> for a specific <tt>DataSource</tt> so that it
	 * is played back on the <tt>MediaDevice</tt> represented by this instance.
	 *
	...
	protected Player createPlayer(DataSource dataSource) {
		...
		if (exception != null) {
			...
			/*
			 * We cannot wait for the Player to get configured (e.g. with waitForState)
			 * because it will stay in the Configuring state until it reads some media. In
			 * the case of a ReceiveStream not sending media (e.g. abnormally stopped), it
			 * will leave us blocked.
			 */
	...
	protected Processor createProcessor() {
		...
		if (captureDevice != null) {
			...
			try {
				processor = ForbiddenManager.createProcessor(captureDevice);
			...
			if (exception != null)
				...
				if (processorControllerListener == null)
					processorControllerListener = new ControllerListener() {
						 ...
						 * it is registered with has generated an event.
						 *
						 * @param event the <tt>ControllerEvent</tt> specifying the <tt>Controller</tt>
						 *              which is the source of the event and the very type of the event
						 * @see ControllerListener#controllerUpdate( ControllerEvent)
						 */
						@Override
						public void controllerUpdate(ControllerEvent event) {
							logger.debug("Controller Event:  " + event);
							processorControllerUpdate(event);
						}
					};
				processor.addControllerListener(processorControllerListener);

				logger.debug(processor.getClass().getSimpleName() + " " + processor.hashCode()
						+ " is waiting for Configured state!");
	 ...
	 * <tt>Processor</tt> of captured media to be sent to the remote peer. Allows
	 * extenders to override. The default implementation returns
	 * {@link ContentDescriptor#RAW_RTP}.
	 *
	 * @param processor the <tt>Processor</tt> of captured media to be sent to the
	 ...
	 * specific <tt>TrackControl</tt>. Allows extenders to override and, optionally,
	 * perform additional configuration of the returned <tt>Renderer</tt>.
	 *
	 * @param player       the <tt>Player</tt> which is to utilize the
	 *                     initialized/returned <tt>Renderer</tt>
	 * @param trackControl the <tt>TrackControl</tt> which represents the media to
	 *                     be played back (and, technically, on which the
	...
	private void disconnectCaptureDevice() {
		if (captureDevice != null) {
			/*
			 * As reported by Carlos Alexandre, stopping before disconnecting resolves a
			 * slow disconnect on Linux.
			 */
			try {
				captureDevice.stop();
			} catch (IOException ioe) {
				/*
				 * We cannot do much about the exception because we're not really interested in
				 * the stopping but rather in calling DataSource#disconnect() anyway.
				 */
	 ...
	 * {@link #playbacks} in the course of their execution and prepares them to be
	 * garbage collected.
	 */
	...
	/**
	 * Releases the resources allocated by a specific <tt>Player</tt> in the course
	 * of its execution and prepares it to be garbage collected.
	 *
	 ...
	 * <tt>Format</tt>s which matches a specific <tt>Format</tt>. The implementation
	 * considers a pair of <tt>Format</tt>s matching if they have the same encoding.
	 *
	 * @param formats the array of <tt>Format</tt>s to be searched for a match to
	 *                the specified <tt>format</tt>
	 * @param format  the <tt>Format</tt> to search for a match in the specified
	 *                <tt>formats</tt>
	 * @return the first element of <tt>formats</tt> which matches <tt>format</tt>
	 *         i.e. is of the same encoding
	 */
	private static Format findFirstMatchingFormat(Format[] formats, Format format) {
		double formatSampleRate = (format instanceof AudioFormat) ? ((AudioFormat) format).getSampleRate()
				: Format.NOT_SPECIFIED;
		...
		for (Format match : formats) {
			count++;
			logger.debug("Finding First Matching Format: " + count + "/" + formats.length);
			if (match.isSameEncoding(format)) {
				logger.debug("Format: " + format + " has same encoding as " + match);
				/*
				 * The encoding alone is, of course, not enough. For example, AudioFormats may
				 * have different sample rates (i.e. clock rates as we call them in
				 * MediaFormat).
				 */
				if (match instanceof AudioFormat) {
					if (formatSampleRate != Format.NOT_SPECIFIED) {
						...
						logger.debug(
								"Format Sample Rate: " + formatSampleRate + ", Match Sample Rate: " + matchSampleRate);
	...
	/**
	 * Gets the <tt>DataSource</tt> that this instance uses to read captured media
	 * from. If it does not exist yet, it is created.
	 *
	 ...
	 * Returns <tt>null</tt> if this instance fails to create <tt>captureDevice</tt>
	 * or to connect to it.
	 *
	 * @return {@link #captureDevice} in a connected state; <tt>null</tt> if this
	...
	protected DataSource getConnectedCaptureDevice() {
		...
		if ((captureDevice != null) && !captureDeviceIsConnected) {
			/*
			 * Give this instance a chance to set up an optimized media codec chain by
			 * setting the output Format on the input CaptureDevice.
			 */
			try {
				if (this.format != null)
	 ...
	 *
	 * @return the <tt>MediaDevice</tt> associated with this instance and the work
	...
	/**
	 * Gets the <tt>MediaFormat</tt> in which this instance captures media from its
	 * associated <tt>MediaDevice</tt>.
	 *
	 * @return the <tt>MediaFormat</tt> in which this instance captures media from
	...
	public MediaFormatImpl<? extends Format> getFormat() {
		 ...
		 * MediaDeviceSession, we'll likely run into unexpected issues so debug whether
	 ...
	 * <tt>MediaDevice</tt>.
	 *
	 * @return the <tt>MediaType</tt> of the media captured and played back by this
	 *         instance as reported by {@link MediaDevice#getMediaType()} of its
	 *         associated <tt>MediaDevice</tt>
	 */
	 ...
	 *
	 * @return the output <tt>DataSource</tt> of this instance which provides the
	 *         captured (RTP) data to be sent by <tt>MediaStream</tt> to
	 *         <tt>MediaStreamTarget</tt>
	 */
	public DataSource getOutputDataSource() {
		...
		logger.debug(
				processor.getClass().getSimpleName() + " " + processor.hashCode() + " is waiting for Realized state!");

		if ((processor == null)
				|| ((processor.getState() < Controller.Realized) && !waitForState(processor, Controller.Realized)))
			...
			/*
			 * Whoever wants the outputDataSource, they expect it to be started in accord
			 * with the previously-set direction.
			 */
	 ...
	 *
	 * @param dataSource the <tt>DataSource</tt> to get the information related to
	 *                   the playback of
	...
	/**
	 * Gets the <tt>Player</tt> rendering the <tt>ReceiveStream</tt> with a specific
	 * SSRC.
	 *
	 * @param ssrc the SSRC of the <tt>ReceiveStream</tt> to get the rendering the
	 *             <tt>Player</tt> of
	 ...
	 *
	 * @return the <tt>Player</tt>s rendering the <tt>ReceiveStream</tt>s of this
	 *         instance on its associated <tt>MediaDevice</tt>
	 */
	...
	/**
	 * Gets the JMF <tt>Processor</tt> which transcodes the <tt>MediaDevice</tt> of
	 * this instance into the format of this instance. If the <tt>Processor</tt> in
	 * question does not exist, the method will create it.
	 * <p>
	 ...
	 * instance, extreme care is to be taken when invoking the method in order to
	 * ensure that the existence of the <tt>Processor</tt> is really in accord with
	 * the rest of the state of this instance. Overall, the method is to be
	 * considered private and is to not be invoked outside the
	 * <tt>MediaDeviceSession</tt> class.
	 * </p>
	 *
	 * @return the JMF <tt>Processor</tt> which transcodes the <tt>MediaDevice</tt>
	 ...
	 * single remote party) we would rarely (if ever) have more than a single SSRC
	 * identifier returned. However, we would also be using the same method to query
	 * a device session operating over a mixer in which case we would have the SSRC
	 * IDs of all parties currently contributing to the mixing.
	 *
	 * @return a <tt>long[]</tt> array of SSRC identifiers that this device session
	 *         is handling streams from.
	 */
	...
	/**
	 * Gets the <tt>MediaDirection</tt> in which this instance has been started. For
	 * example, a <tt>MediaDirection</tt> which returns <tt>true</tt> for
	 * <tt>allowsSending()</tt> signals that this instance is capturing media from
	...
	/**
	 * Gets a list of the <tt>MediaFormat</tt>s in which this instance is capable of
	 * capturing media from its associated <tt>MediaDevice</tt>.
	 *
	 * @return a new list of <tt>MediaFormat</tt>s in which this instance is capable
	 *         of capturing media from its associated <tt>MediaDevice</tt>
	 */
	 ...
	 *
	 * @return <tt>true</tt> if this <tt>MediaDeviceSession</tt> is set to output
	...
	/**
	 * Notifies this <tt>MediaDeviceSession</tt> that a <tt>DataSource</tt> has been
	 * added for playback on the represented <tt>MediaDevice</tt>.
	 *
	 * @param playbackDataSource the <tt>DataSource</tt> which has been added for
	 ...
	 *
	 * @param playbackDataSource the <tt>DataSource</tt> which has been removed from
	 *                           playback on the represented <tt>MediaDevice</tt>
	 */
	...
	/**
	 * Notifies this <tt>MediaDeviceSession</tt> that a <tt>DataSource</tt> has been
	 * changed on the represented <tt>MediaDevice</tt>.
	 *
	 * @param playbackDataSource the <tt>DataSource</tt> which has been added for
	 *                           playback on the represented <tt>MediaDevice</tt>
	 */
	...
	/**
	 * Notifies this instance that a specific <tt>Player</tt> of remote content has
	 * generated a <tt>ConfigureCompleteEvent</tt>. Allows extenders to carry out
	 * additional processing on the <tt>Player</tt>.
	 *
	 ...
	 * {@link #playerRealizeComplete(Processor)}. In any case, extenders overriding
	 * this method should call the super implementation.
	 * </p>
	 *
	 * @param ev the <tt>ControllerEvent</tt> specifying the <tt>Controller</tt>
	 *           which is the source of the event and the very type of the event
	...
	protected void playerControllerUpdate(ControllerEvent ev) {
		if (ev instanceof ConfigureCompleteEvent) {
			...
			if (player != null) {
				...
				logger.debug(player.getClass().getSimpleName() + " " + player.hashCode() + " is about to realize!");
				player.realize();
			}
	...
	/**
	 * Notifies this instance that a specific <tt>Player</tt> of remote content has
	 * generated a <tt>RealizeCompleteEvent</tt>. Allows extenders to carry out
	 * additional processing on the <tt>Player</tt>.
	 *
	 * @param player the <tt>Player</tt> which is the source of a
	...
	protected void processorControllerUpdate(ControllerEvent ev) {
		logger.debug("Controller Event Update: " + ev.getClass().getName() + " " + ev.hashCode());
		if (ev instanceof ConfigureCompleteEvent) {
			...
			if (processor != null) {
				...
				if (format != null) {
					logger.debug("Controller Event Update: " + getClass().getName() + " " + hashCode() + " sets format "
							+ format + " to " + processor.getClass().getName() + " " + processor.hashCode());
			...
			for (AdvancedAttributesAwareCodec aaac : getAllTrackControls(AdvancedAttributesAwareCodec.class,
					processor)) {
				Map<String, String> advanceAttrs = format == null ? null : format.getAdvancedAttributes();
	...
	/**
	 * Removes <tt>ssrc</tt> from the array of SSRC identifiers representing parties
	 * that this <tt>MediaDeviceSession</tt> is currently receiving streams from.
	 *
	 * @param ssrc the SSRC identifier that we'd like to remove from the array of
	 *             <tt>ssrc</tt> identifiers stored by this session.
	 */
	...
	/**
	 * Notifies this instance that a specific <tt>ReceiveStream</tt> has been added
	 * to the list of playbacks of <tt>ReceiveStream</tt>s and/or
	 * <tt>DataSource</tt>s performed by respective <tt>Player</tt>s on the
	 * <tt>MediaDevice</tt> represented by this instance.
	 *
	 * @param receiveStream the <tt>ReceiveStream</tt> which has been added to the
	 ...
	 * <tt>MediaDevice</tt> represented by this instance.
	 *
	 * @param receiveStream the <tt>ReceiveStream</tt> which has been removed from
				...
				if (formatControl != null) {
					...
					if ((supportedFormats != null) && (supportedFormats.length != 0)) {
						if (sampleRate != Format.NOT_SPECIFIED) {
							/*
							 * As per RFC 3551.4.5.2, because of a mistake in RFC 1890 and for backward
							 * compatibility, G.722 should always be announced as 8000 even though it is
							 * wideband.
							 */
							String encoding = audioFormat.getEncoding();

							if ((Constants.G722.equalsIgnoreCase(encoding)
									|| Constants.G722_RTP.equalsIgnoreCase(encoding)) && (sampleRate == 8000)) {
								sampleRate = 16000;
							}
						}
	...
	/**
	 * Sets the <tt>ContentDescriptor</tt> which specifies the content type in which
	 * this <tt>MediaDeviceSession</tt> is to output the media captured by its
	 * <tt>MediaDevice</tt>. The default content type in which
	 * <tt>MediaDeviceSession</tt> outputs the media captured by its
	 * <tt>MediaDevice</tt> is {@link ContentDescriptor#RAW_RTP}.
	 *
	 * @param contentDescriptor the <tt>ContentDescriptor</tt> which specifies the
	 *                          content type in which this
	 *                          <tt>MediaDeviceSession</tt> is to output the media
	...
	public void setFormat(MediaFormat format) {
		...
		/*
		 * We need javax.media.Format and we know how to convert MediaFormat to it only
		 * for MediaFormatImpl so assert early.
		 */
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Set format " + this.format + " on " + getName());
		}
		...
		if (processor != null) {
			...
			logger.trace("Processor's State is " + processorState + " on " + getClass().getName() + " " + hashCode());

			if (processorState == Processor.Configured)
				setProcessorFormat(processor, this.format);
			else if (processorIsPrematurelyClosed || ((processorState > Processor.Configured)
					&& !this.format.getFormat().equals(getProcessorFormat())) || outputSizeChanged) {
	...
	protected void setProcessorFormat(Processor processor, MediaFormatImpl<? extends Format> mediaFormat) {
		...
		for (int trackIndex = 0; trackIndex < trackControls.length; trackIndex++) {
			logger.debug("Setting Processor Media Format in track " + trackIndex + "/" + trackControls.length + " of "
					+ processor.getClass().getName() + " " + processor.hashCode());
			...
			logger.debug("The number of supported formats is " + supportedFormats.length + " of "
					+ trackControl.getClass().getName() + " " + trackControl.hashCode());
			...
			case AUDIO:
				if (supportedFormats[0] instanceof AudioFormat) {
					...
					/*
					 * We've failed to find a supported format so try to use whatever we've been
					 * told and, if it fails, the caller will at least know why.
					 */
			...
			if (supportedFormat == null)
				...
				if (setFormat == null)
					logger.error("Failed to set format of track " + trackIndex + " to " + supportedFormat
							+ ". Processor is in state " + processor.getState());
				else if (setFormat != supportedFormat)
					logger.warn("Failed to change format of track " + trackIndex + " from " + setFormat + " to "
	 ...
	 * <tt>MediaDeviceSession</tt> to the remote peer. Allows extenders to override
	 * the set procedure and to detect when the JMF <tt>Format</tt> of the specified
	 * <tt>TrackControl</tt> changes.
	 *
	 * @param trackControl the <tt>TrackControl</tt> to set the JMF <tt>Format</tt>
	 *                     of
	 * @param mediaFormat  the <tt>MediaFormatImpl</tt> to be set on the specified
	 *                     <tt>TrackControl</tt>. Though <tt>mediaFormat</tt>
	 *                     encapsulates a JMF <tt>Format</tt>, <tt>format</tt> is to
	 *                     be set on the specified <tt>trackControl</tt> because it
	 *                     may be more specific. In any case, the two JMF
	 *                     <tt>Format</tt>s match. The <tt>MediaFormatImpl</tt> is
	 *                     provided anyway because it carries additional information
	 *                     such as format parameters.
	 ...
	 *                     <tt>trackControl</tt> because it may be more specific
	 ...
	 *         <tt>trackControl</tt>
	 */
	protected Format setProcessorFormat(TrackControl trackControl, MediaFormatImpl<? extends Format> mediaFormat,
			Format format) {
		logger.debug("Start Setting Media Format " + format + " in " + trackControl.getClass().getName() + " "
				+ trackControl.hashCode());

		Format newFormat = trackControl.setFormat(format);

		logger.debug("Finished Setting Media Format " + format + " in " + trackControl.getClass().getName() + " "
				+ trackControl.hashCode());
	...
	/**
	 * Sets the indicator which determines whether this <tt>MediaDeviceSession</tt>
	 * is set to output "silence" instead of the actual media fed from its
	 * <tt>CaptureDevice</tt>.
	 *
	 * @param mute <tt>true</tt> to set this <tt>MediaDeviceSession</tt> to output
	 *             "silence" instead of the actual media fed from its
	 *             <tt>CaptureDevice</tt>; otherwise, <tt>false</tt>
	 */
	 ...
	 *
	 * @param playbackDataSource the <tt>DataSource</tt> which to be added to the
	 *                           list of playbacks of <tt>ReceiveStream</tt>s and/or
	 *                           <tt>DataSource</tt>s performed by respective
	 *                           <tt>Player</tt>s on the <tt>MediaDevice</tt>
	 ...
	 * <tt>Player</tt>s on the <tt>MediaDevice</tt> represented by this instance.
	 *
	 * @param playbackDataSource the <tt>DataSource</tt> which to be removed from
	 *                           the list of playbacks of <tt>ReceiveStream</tt>s
	 *                           and/or <tt>DataSource</tt>s performed by respective
	 *                           <tt>Player</tt>s on the <tt>MediaDevice</tt>
	 *                           represented by this instance
	 */
	...
	/**
	 * Sets the JMF <tt>Processor</tt> which is to transcode {@link #captureDevice}
	...
	private void setProcessor(Processor processor) {
		if (this.processor != processor) {
			...
			/*
			 * Since the processor has changed, its output DataSource known to the public
			 * has also changed.
			 */
	 ...
	 * <tt>Player</tt>s on the <tt>MediaDevice</tt> represented by this instance.
	 *
	 * @param receiveStream the <tt>ReceiveStream</tt> which to be added to the list
			...
			if (getPlayback(receiveStream) == null) {
				...
				if (receiveStreamDataSource != null) {
					if (receiveStreamDataSource instanceof PushBufferDataSource) {
						receiveStreamDataSource = new ReceiveStreamPushBufferDataSource(receiveStream,
								(PushBufferDataSource) receiveStreamDataSource, true);
					} else {
	 ...
	 * <tt>Player</tt>s on the <tt>MediaDevice</tt> represented by this instance.
	 *
	 * @param receiveStream the <tt>ReceiveStream</tt> which to be removed from the
	 *                      list of playbacks of <tt>ReceiveStream</tt>s and/or
	 *                      <tt>DataSource</tt>s performed by respective
	 *                      <tt>Player</tt>s on the <tt>MediaDevice</tt> represented
	 *                      by this instance
	...
	/**
	 * Starts a specific <tt>Processor</tt> if this <tt>MediaDeviceSession</tt> has
	 * been started and the specified <tt>Processor</tt> is not started.
	 *
	 * @param processor the <tt>Processor</tt> to start
	 */
	protected void startProcessorInAccordWithDirection(Processor processor) {
		if (startedDirection.allowsSending() && (processor.getState() != Controller.Started)) {
			processor.start();
			if (logger.isTraceEnabled()) {
	 ...
	 * successfully entered <tt>state</tt> or <tt>false</tt> if <tt>process</tt> has
	 * failed to enter <tt>state</tt>.
	 *
	...
	/**
	 * Copies the playback part of a specific <tt>MediaDeviceSession</tt> into this
	 * instance.
	 *
	...
	public void copyPlayback(MediaDeviceSession deviceSession) {
		if (deviceSession.disposePlayerOnClose) {
			...
			/*
			 * TODO Technically, we should be synchronizing the (read and write) accesses to
			 * the playbacks fields. In practice, we are not doing it because it likely was
			 * the easiest to not bother with it at the time of its writing.
			 */
	...
	/**
	 * Represents the information related to the playback of a <tt>DataSource</tt>
	 * on the <tt>MediaDevice</tt> represented by a <tt>MediaDeviceSession</tt>. The
	 * <tt>DataSource</tt> may have an associated <tt>ReceiveStream</tt>.
	 */
	private static class Playback {
		/**
		 * The <tt>DataSource</tt> the information related to the playback of which is
		 * represented by this instance and which is associated with
		 * {@link #receiveStream}.
		 */
		public DataSource dataSource;

		/**
		 * The <tt>ReceiveStream</tt> the information related to the playback of which
		 * is represented by this instance and which is associated with
		 * {@link #dataSource}.
		 */
		...
		/**
		 * Initializes a new <tt>Playback</tt> instance which is to represent the
		 * information related to the playback of a specific <tt>DataSource</tt>.
		 *
		 * @param dataSource the <tt>DataSource</tt> the information related to the
		...
		/**
		 * Initializes a new <tt>Playback</tt> instance which is to represent the
		 * information related to the playback of a specific <tt>ReceiveStream</tt>.
		 *
		 * @param receiveStream the <tt>ReceiveStream</tt> the information related to
		 *                      the playback of which is to be represented by the new
		 *                      instance
		 */
	...
	/**
	 * Returns the <tt>Set</tt> of controls of type <tt>controlType</tt>, which are
	 * controls for some of <tt>processor</tt>'s <tt>TrackControl</tt>s.
	 *
	 * @param controlType the name of the class to search for.
	 * @param processor   the <tt>Processor</tt> whose <tt>TrackControls</tt>s will
	 *                    be searched.
	 * @return A non-null <tt>Set</tt> of all <tt>controlType</tt>s found.
	 */
