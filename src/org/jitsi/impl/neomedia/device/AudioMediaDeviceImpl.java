...
public class AudioMediaDeviceImpl extends MediaDeviceImpl {
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>AudioMediaDeviceImpl</tt> class and its
	 ...
	 * <tt>MediaDevice</tt> with <tt>MediaType</tt> <tt>AUDIO</tt> and a
	 ...
	 * implementation of <tt>MediaDevice</tt> with <tt>MediaType</tt> <tt>AUDIO</tt>
	 * to a <tt>CaptureDevice</tt> with a specific <tt>CaptureDeviceInfo</tt>.
	 *
	 * @param captureDeviceInfo the <tt>CaptureDeviceInfo</tt> of the
	 *                          <tt>CaptureDevice</tt> to which the new instance is
	 *                          to provide an implementation of <tt>MediaDevice</tt>
	 */
	...
	public void connect(DataSource captureDevice) throws IOException {
		 ...
		 * JavaSound) is 125 ms (i.e. 1/8 sec). On Mac OS X, this leads to an exception
		 * and no audio capture. A value of 30 ms for the buffer length fixes the
		 * problem and is OK when using some PSTN gateways.
		 *
		 * 2. Changing to 60 ms. When it is 30 ms, there are some issues with Asterisk
		 * and NAT (we don't start to send a/the stream and Asterisk's RTP functionality
		 * doesn't notice that we're behind NAT).
		 *
	...
	/**
	 * Creates the JMF <tt>CaptureDevice</tt> this instance represents and provides
	 * an implementation of <tt>MediaDevice</tt> for.
	 *
	 * @return the JMF <tt>CaptureDevice</tt> this instance represents and provides
	 *         an implementation of <tt>MediaDevice</tt> for; <tt>null</tt> if the
	 *         creation fails
	 */
	@Override
	 ...
	 *
	 * @param captureDevice an exclusive <tt>CaptureDevice</tt> for which sharing is
	 *                      to be enabled
	 * @return a new <tt>AudioMixer</tt> which enables the sharing of the specified
	 *         exclusive <tt>captureDevice</tt>
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * Tries to delegate the initialization of a new <tt>Renderer</tt> instance to
	 * the <tt>AudioSystem</tt> which provides the <tt>CaptureDevice</tt> of this
	 * instance. This way both the capture and the playback are given a chance to
	 * happen within the same <tt>AudioSystem</tt>. If the discovery of the delegate
	 * fails, the implementation of <tt>MediaDeviceImpl</tt> is executed and it
	 * currently leaves it to FMJ to choose a <tt>Renderer</tt> irrespective of this
	 * <tt>MediaDevice</tt>.
	 */
	@Override
	 ...
	 * extension descriptor indicating <tt>RECVONLY</tt> support for mixer-to-client
	 * audio levels.
	 *
	...
	private boolean isLessThanOrEqualToMaxAudioFormat(Format format) {
		if (format instanceof AudioFormat) {
			...
			if ((channels == Format.NOT_SPECIFIED) || (MediaUtils.MAX_AUDIO_CHANNELS == Format.NOT_SPECIFIED)
	 ...
	 * implementation of {@link MediaDeviceImpl#createCaptureDevice()}. Allows this
	 * instance to customize the very <tt>CaptureDevice</tt> which is to be possibly
	 * further wrapped by this instance.
	 *
	...
	protected CaptureDevice superCreateCaptureDevice() {
		...
		if (captureDevice != null) {
			/*
			 * Try to default the captureDevice to a Format which does not exceed the
			 * maximum quality known to MediaUtils.
			 */
			try {
					...
					for (FormatControl formatControl : formatControls) {
						...
						if (!supportedFormatToSet.matches(format)) {
							...
							if (sampleSizeInBits == Format.NOT_SPECIFIED) {
								sampleSizeInBits = MediaUtils.MAX_AUDIO_SAMPLE_SIZE_IN_BITS;
								/*
								 * TODO A great deal of the neomedia-contributed audio Codecs, CaptureDevices,
								 * DataSources and Renderers deal with 16-bit samples.
								 */
								if (sampleSizeInBits == Format.NOT_SPECIFIED)
									sampleSizeInBits = 16;
							}
							...
							if ((channels != Format.NOT_SPECIFIED) && (sampleRate != Format.NOT_SPECIFIED)
								...
								AudioFormat formatToSet = new AudioFormat(supportedFormatToSet.getEncoding(),
				 ...
				 * maximum quality known to MediaUtils and we failed but it does not mean that
				 * the captureDevice will not be successfully used.
				 */
			}
		}
