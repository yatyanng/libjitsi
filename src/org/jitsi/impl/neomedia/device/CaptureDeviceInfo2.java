...
public class CaptureDeviceInfo2 extends CaptureDeviceInfo {
	 ...
	 * <tt>CaptureDeviceInfo</tt> instance and additional information specific to
	 * the <tt>CaptureDeviceInfo2</tt> class. Because the properties of the
	 * specified <tt>captureDeviceInfo</tt> are copied into the new instance, the
	 * constructor is to be used when a <tt>CaptureDeviceInfo</tt> exists for other
	 * purposes already; otherwise, it is preferable to use
	 * {@link #CaptureDeviceInfo2(String, MediaLocator, Format[], String, String, String)}.
	 *
	 * @param captureDeiceInfo the <tt>CaptureDeviceInfo</tt> whose properties are
	 *                         to be copied into the new instance
	 * @param uid              the unique identifier of the hardware device
	 *                         (interface) which is to be represented by the new
	 *                         instance
	 ...
	 */
	public CaptureDeviceInfo2(CaptureDeviceInfo captureDeviceInfo, String uid, String transportType,
			String modelIdentifier) {
		this(captureDeviceInfo.getName(), captureDeviceInfo.getLocator(), captureDeviceInfo.getFormats(), uid,
	 ...
	 * @param locator         the <tt>MediaLocator</tt> which uniquely identifies
	 *                        the device to be described by the new instance
	 * @param formats         an array of the <tt>Format</tt>s supported by the
	 ...
	 * @param transportType   the transport type (e.g. USB) of the device to be
	 *                        represented by the new instance
	 * @param modelIdentifier the persistent identifier of the model of the hardware
	 *                        device to be represented by the new instance
	 */
	public CaptureDeviceInfo2(String name, MediaLocator locator, Format[] formats, String uid, String transportType,
			String modelIdentifier) {
	...
	/**
	 * Returns the device identifier used to save and load device preferences. It is
	 * composed by the system UID if not null. Otherwise returns the device name and
	 * (if not null) the transport type.
	 *
	 ...
	 *
	 * @param transportType the transport type to compare to the transport type of
	 *                      this instance
	 * @return <tt>true</tt> if the specified <tt>transportType</tt> is equal to/the
	 *         same as the transport type of this instance; otherwise,
	 *         <tt>false</tt>
	 */
