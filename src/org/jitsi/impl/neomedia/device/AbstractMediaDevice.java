...
public abstract class AbstractMediaDevice implements MediaDevice {
	 ...
	 * back media on this <tt>MediaDevice</tt>. Allows extenders to, for example,
	 * disable the playback on this <tt>MediaDevice</tt> by completely overriding
	 * and returning <tt>null</tt>.
	 *
	 * @param dataSource the <tt>DataSource</tt> which is to be played back by the
	 *                   new <tt>Processor</tt> instance
	 * @return a new <tt>Processor</tt> instance which is to be used to play back
	 *         the media provided by the specified <tt>dataSource</tt> or
	...
	protected Processor createPlayer(DataSource dataSource) throws Exception {
		...
		dataSource.connect();
		try {
			player = ForbiddenManager.createProcessor(dataSource);
		} finally {
			if (player == null)
	...
	/**
	 * Initializes a new <tt>Renderer</tt> instance which is to play back media on
	 * this <tt>MediaDevice</tt>. Allows extenders to initialize a specific
	 ...
	 *         <tt>MediaDevice</tt> or <tt>null</tt> if a suitable <tt>Renderer</tt>
	 *         is to be chosen irrespective of this <tt>MediaDevice</tt>
	 */
