...
public class PulseAudioSystem extends AudioSystem {
	 ...
	 * represent the automatic, default PulseAudio device to be used in the absence
	 * of a specification of a PulseAudio device.
	 */
	 ...
	 *         <tt>AudioSystem</tt> framework or <tt>null</tt> if no such system is
	...
	/**
	 * The indicator which specifies whether the method {@link #createContext()} has
	 * been executed. Used to enforce a single execution of the method in question.
	 */
	...
	/**
	 * Initializes the connection context for asynchronous communication with the
	 * PulseAudio server i.e. creates {@link #context}.
	 */
	 ...
	 * PulseAudio <tt>Renderer</tt> implementation does not follow the convention of
	 * <tt>AudioSystem</tt>.
	 */
	@Override
	...
	/**
	 * Initializes a new PulseAudio stream which is to input or output audio at a
	 * specific sample rate and with a specific number of channels. The new audio
	 * stream is to be associated with a specific human-readable name and is to have
	 * a specific PulseAudio logic role.
	 *
	 * @param sampleRate the sample rate at which the new PulseAudio stream is to
	 *                   input or output
	 ...
	 *         <tt>mediaRole</tt>
	 */
	public long createStream(int sampleRate, int channels, String mediaName, String mediaRole) throws RuntimeException {
	...
	protected synchronized void doInitialize() throws Exception {
		...
		if (!captureDeviceFormats.isEmpty()) {
			captureDevices.add(0,
					new CaptureDeviceInfo2(NULL_DEV_CAPTURE_DEVICE_INFO_NAME, new MediaLocator(LOCATOR_PROTOCOL + ":"),
	 ...
	 * thread from processing events. May be used to enforce exclusive access to all
	 * PulseAudio objects attached to the PulseAudio event loop. The lock is
	 * recursive. The method may not be called inside the PulseAudio event loop
	 * thread. Events that are dispatched from the PulseAudio event loop thread are
	 * executed with the lock held.
	 */
	 ...
	 * <tt>pa_context</tt> with properties such as the application name and version.
	 *
	 * @param proplist the <tt>pa_proplist</tt> which is to be populated with
	 *                 <tt>pa_context</tt>-related properties such as the
	 ...
	 *                   the PulseAudio server which is reporting the
	 *                   <tt>sinkInfo</tt>
	 * @param sinkInfo   the information about the sink being reported
	 * @param deviceList the list of <tt>CaptureDeviceInfo</tt>s which reperesents
	 *                   existing devices and into which the callback is represent
	 *                   the <tt>sinkInfo</tt>
	 * @param formatList the list of <tt>Format</tt>s supported by the various
	 *                   <tt>CaptureDeviceInfo</tt>s in <tt>deviceList</tt> and into
	 *                   which the callback is to represent the <tt>sinkInfo</tt>
	 */
	 ...
	 * @param deviceList the list of <tt>CaptureDeviceInfo</tt>s which reperesents
	 *                   existing devices and into which the callback is represent
	 *                   the <tt>sourceInfo</tt>
	 * @param formatList the list of <tt>Format</tt>s supported by the various
	 *                   <tt>CaptureDeviceInfo</tt>s in <tt>deviceList</tt> and into
	 *                   which the callback is to represent the <tt>sourceInfo</tt>
	 */
	private void sourceInfoListCb(long context, long sourceInfo, List<CaptureDeviceInfo2> deviceList,
			List<Format> formatList) {
		...
		if ((MediaUtils.MAX_AUDIO_CHANNELS != Format.NOT_SPECIFIED) && (MediaUtils.MAX_AUDIO_CHANNELS < channels))
			channels = MediaUtils.MAX_AUDIO_CHANNELS;
		if ((MediaUtils.MAX_AUDIO_SAMPLE_RATE != Format.NOT_SPECIFIED) && (MediaUtils.MAX_AUDIO_SAMPLE_RATE < rate))
			rate = (int) MediaUtils.MAX_AUDIO_SAMPLE_RATE;

		AudioFormat audioFormat = new AudioFormat(AudioFormat.LINEAR, rate, 16, channels, AudioFormat.LITTLE_ENDIAN,
				AudioFormat.SIGNED, Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */,
				Format.byteArray);
		...
		deviceList.add(new CaptureDeviceInfo2((description == null) ? name : description,
				new MediaLocator(LOCATOR_PROTOCOL + ":" + name), new Format[] { audioFormat }, null, null, null));
	}
	 ...
	 *                          associated with this <tt>PulseAudioSystem</tt> or a
	 *                          new PulseAudio event loop thread initialized for the
	 *                          purposes of association with this
	 ...
	 * <tt>PulseAudioSystem</tt> cleanly. Make sure to unlock the PulseAudio main
	 * loop object before calling the method.
	 */
	 ...
	 * @param stateToWaitFor the PulseAudio state of the specified <tt>context</tt>
	 *                       to wait for
	 * @return the state of the specified <tt>context</tt> which caused the method
	 *         to return
	 */
	 ...
	 * @param stateToWaitFor the PulseAudio state of the specified <tt>stream</tt>
	 *                       to wait for
	 * @return the state of the specified <tt>stream</tt> which caused the method to
	 *         return
	 */
