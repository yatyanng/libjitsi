...
public abstract class AudioSystem extends DeviceSystem {
	 ...
	 * automatic gain control (AGC) functionality between on and off. The UI will
	 * look for the presence of the flag in order to determine whether a check box
	 * is to be shown to the user to enable toggling the automatic gain control
	 * (AGC) functionality.
	 */
	 ...
	 * The constant/flag (to be) returned by {@link #getFeatures()} in order to
	 * indicate that the respective <tt>AudioSystem</tt> supports toggling its
	 * denoise functionality between on and off. The UI will look for the presence
	 * of the flag in order to determine whether a check box is to be shown to the
	 * user to enable toggling the denoise functionality.
	 */
	public static final int FEATURE_DENOISE = 1 << 1;

	/**
	 * The constant/flag (to be) returned by {@link #getFeatures()} in order to
	 * indicate that the respective <tt>AudioSystem</tt> supports toggling its echo
	 * cancellation functionality between on and off. The UI will look for the
	 * presence of the flag in order to determine whether a check box is to be shown
	 * to the user to enable toggling the echo cancellation functionality.
	 */
	 ...
	 * playback and notification audio devices. The UI, for example, will look for
	 * the presence of the flag in order to determine whether separate combo boxes
	 * are to be shown to the user to allow the configuration of the preferred
	...
	/**
	 * The (base) name of the <tt>ConfigurationService</tt> property which indicates
	 * whether automatic gain control (AGC) is to be performed for the captured
	 * audio.
	 */
	...
	/**
	 * The (base) name of the <tt>ConfigurationService</tt> property which indicates
	 * whether noise suppression is to be performed for the captured audio.
	 */
	protected static final String PNAME_DENOISE = "denoise";

	/**
	 * The (base) name of the <tt>ConfigurationService</tt> property which indicates
	 * whether noise cancellation is to be performed for the captured audio.
	 */
	...
	/**
	 * The list of devices detected by this <tt>AudioSystem</tt> indexed by their
	 * category which is among {@link #CAPTURE_INDEX}, {@link #NOTIFY_INDEX} and
	 * {@link #PLAYBACK_INDEX}.
	 */
	 ...
	 * @return a new <tt>Renderer</tt> instance which is to either perform playback
	 *         on or sound a notification through a device contributed by this
	 *         system
	 */
	public Renderer createRenderer(boolean playback) {
		...
		if (className == null) {
			/*
			 * There is no point in delegating to the super's createRenderer() because it
			 * will not have a class to instantiate.
			 */
			renderer = null;
		} else {
			Class<?> clazz;
			...
			if (clazz == null) {
				/*
				 * There is no point in delegating to the super's createRenderer() because it
				 * will fail to get the class.
				 */
				renderer = null;
			} else if (!Renderer.class.isAssignableFrom(clazz)) {
				/*
				 * There is no point in delegating to the super's createRenderer() because it
				 * will fail to cast the new instance to a Renderer.
				 */
				renderer = null;
			} else {
				...
				if (((getFeatures() & FEATURE_NOTIFY_AND_PLAYBACK_DEVICES) != 0)
					...
					try {
						...
						/*
						 * Such a constructor is optional so the failure to get it will be swallowed and
						 * the super's createRenderer() will be invoked.
						 */
	 ...
	 * <tt>AudioSystem</tt>, supports a specific flow of media data (i.e. capture,
	 * notify or playback) and is identified by a specific <tt>MediaLocator</tt>.
	 *
	 * @param dataFlow the flow of the media data supported by the
	 *                 <tt>CaptureDeviceInfo2</tt> to be returned
	 * @param locator  the <tt>MediaLocator</tt> of the <tt>CaptureDeviceInfo2</tt>
	 *                 to be returned
	 * @return a <tt>CaptureDeviceInfo2</tt> which has been contributed by this
	 *         instance, supports the specified <tt>dataFlow</tt> and is identified
	 *         by the specified <tt>locator</tt>
	 */
	 ...
	 * playback.
	 *
	 * @param dataFlow the data flow of the devices to retrieve: capture, notify or
	 *                 playback
	...
	public javax.media.format.AudioFormat getFormat(InputStream audioInputStream) {
		if ((audioInputStream instanceof AudioInputStream)) {
			...
			return new javax.media.format.AudioFormat(javax.media.format.AudioFormat.LINEAR, af.getSampleRate(),
	...
	/**
	 * Gets the (full) name of the <tt>ConfigurationService</tt> property which is
	 * associated with a (base) <tt>AudioSystem</tt>-specific property name.
	 * 
	 * @param basePropertyName the (base) <tt>AudioSystem</tt>-specific property
	 *                         name of which the associated (full)
	 *                         <tt>ConfigurationService</tt> property name is to be
	 *                         returned
	 * @return the (full) name of the <tt>ConfigurationService</tt> property which
	 *         is associated with the (base) <tt>AudioSystem</tt>-specific property
	 *         name
	 */
	 ...
	 * playback.
	 *
	 * @param dataFlow the data flow of the selected device to retrieve: capture,
	 *                 notify or playback.
	...
	/**
	 * Gets the indicator which determines whether automatic gain control (AGC) is
	 * to be performed for captured audio.
	 * 
	 * @return <tt>true</tt> if automatic gain control (AGC) is to be performed for
	 ...
	 *
	 * @return <tt>true</tt> if noise suppression is to be performed for captured
	 *         audio; otherwise, <tt>false</tt>
	 */
	 ...
	 * performed for captured audio.
	 *
	 * @return <tt>true</tt> if echo cancellation is to be performed for captured
	 *         audio; otherwise, <tt>false</tt>
	 */
	 ...
	 *
	 * @param dataFlow the data flow of the devices to perform post-initialization
	 *                 on
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * Removes any capture, playback and notification devices previously detected by
	 * this <tt>AudioSystem</tt> and prepares it for the execution of its
	 * {@link DeviceSystem#doInitialize()} implementation (which detects all devices
	 * to be provided by this instance).
	 */
	@Override
	 ...
	 * value of a specific property which had its old value modified to a specific
	 * new value. <tt>PropertyChangeNotifier</tt> does not check whether the
	 * specified <tt>oldValue</tt> and <tt>newValue</tt> are indeed different.
	 *
	...
	/**
	 * Sets the indicator which determines whether automatic gain control (AGC) is
	 * to be performed for captured audio.
	 *
	 * @param automaticGainControl <tt>true</tt> if automatic gain control (AGC) is
	 *                             to be performed for captured audio; otherwise,
	 *                             <tt>false</tt>
	 */
	 ...
	 * performed for captured audio.
	 *
	 * @param echoCancel <tt>true</tt> if echo cancellation is to be performed for
	 *                   captured audio; otherwise, <tt>false</tt>
	 */
