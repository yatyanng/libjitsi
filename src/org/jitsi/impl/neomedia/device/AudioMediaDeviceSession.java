...
public class AudioMediaDeviceSession extends MediaDeviceSession {
	/**
	 * The <tt>Logger</tt> used by the <tt>AudioMediaDeviceSession</tt> class and
	...
	/**
	 * The <tt>Effect</tt> that we will register with our output data source in
	 * order to measure the outgoing audio levels.
	 */
	...
	/**
	 * The effect that we will register with our stream in order to measure audio
	 * levels of the remote user audio.
	 */
	...
	/**
	 * Returns the last audio level that was measured by this device session for the
	 ...
	 * when the player associated with this session's <tt>ReceiveStream</tt> moves
	 * enters the <tt>Configured</tt> state, so we use the occasion to add our audio
	 * level effect.
	 *
	...
	protected void processorControllerUpdate(ControllerEvent event) {
		logger.debug("Controller Update: " + event.getClass().getName() + " " + event.hashCode());
	 ...
	 *
	 * @param processor the processor on which track control we need to register a
	 *                  level effect with.
	 */
	 ...
	 * audio level changes.
	 *
	 * @param trackControl the <tt>TrackControl</tt> where we need to register a
	 *                     level effect that would measure the audio levels of the
	 *                     <tt>ReceiveStream</tt> associated with this class.
	 *
	 * @throws UnsupportedPlugInException if we fail to add our sound level effect
	 *                                    to the track control of
	 ...
	 * notifying about changes in local audio level related information. This class
	 * only supports a single listener for audio changes per source (i.e. stream or
	 * data source). Audio changes are generally quite time intensive (~ 50 per
	 * second) so we are doing this in order to reduce the number of objects
	 * associated with the process (such as event instances listener list iterators
	 * and sync copies).
	 *
	...
	/**
	 * Sets <tt>listener</tt> as the <tt>SimpleAudioLevelListener</tt> that we are
	 * going to notify every time a change occurs in the audio level of the media
	 * that this device session is receiving from the remote party. This class only
	 * supports a single listener for audio changes per source (i.e. stream or data
	 * source). Audio changes are generally quite time intensive (~ 50 per second)
	 * so we are doing this in order to reduce the number of objects associated with
	 * the process (such as event instances listener list iterators and sync
	 * copies).
	 *
	 * @param listener the <tt>SimpleAudioLevelListener</tt> that we want notified
	 ...
	 * <tt>VolumeControl</tt> on a specific <tt>Renderer</tt> for the purposes of
	 * control over the volume (level) of the audio (to be) played back by the
	 * specified <tt>Renderer</tt>.
	 *
	 * @param renderer      the <tt>Renderer</tt> on which the specified
	 *                      <tt>volumeControl</tt> is to be set
	 * @param volumeControl the <tt>VolumeControl</tt> to be set on the specified
	 *                      <tt>renderer</tt>
	 */
			...
			if (outputAudioLevelEffect != null) {
				for (TrackControl track : processor.getTrackControls()) {
					try {
						...
						logger.warn("Failed to insert the audio level Effect. " + "Output levels will not be included. "
								+ upie);
					}
				}
			}
	...
	/**
	 * Enables or disables measuring audio levels for the output <tt>DataSource</tt>
	 * of this <tt>AudioMediaDeviceSession</tt>.
	 *
	 * Note that if audio levels are to be enabled, this method needs to be called
	 * (with <tt>enabled</tt> set to <tt>true</tt>) before the output
	 * <tt>DataSource</tt>, or the <tt>Processor</tt> are accessed (via
	 * {@link #getOutputDataSource()} and {@link #getProcessor()}). This limitation
	 * allows to not insert an <tt>Effect</tt> in the codec chain when measuring
	 * audio levels is not required (since we can only do this before the
