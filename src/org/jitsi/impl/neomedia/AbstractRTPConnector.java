...
public abstract class AbstractRTPConnector implements RTPConnector {
	/**
	 * The <tt>Logger</tt> used by the <tt>AbstractRTPConnector</tt> class and its
	 * instances to print debug information.
	 */
	private static final Logger logger = Logger.getLogger(AbstractRTPConnector.class);

	/**
	 * The pair of datagram sockets for RTP and RTCP traffic that this instance uses
	 * in the form of a <tt>StreamConnector</tt>.
	 */
	...
	/**
	 * Initializes a new <tt>AbstractRTPConnector</tt> which is to use a given pair
	...
	/**
	 * Add a stream target. A stream target is the destination address which this
	 * RTP session will send its data to. For a single session, we can add multiple
	 * SessionAddresses, and for each address, one copy of data will be sent to.
	 *
	 ...
	 *
	 * @return the <tt>StreamConnector</tt> which represents the pair of datagram
	 *         sockets for RTP and RTCP traffic used by this instance
	 */
	 ...
	 *         received from the remote targets; <tt>null</tt> if it does not exist
	 ...
	 * exist yet.
	 *
	 * @param create <tt>true</tt> to create the <tt>OutputDataStream</tt> which is
	 *               to be used to write RTCP data to be sent to the remote targets
	 *               if it does not exist yet; otherwise, <tt>false</tt>
	 * @return the <tt>OutputDataStream</tt> which is used to write RTCP data to be
	 *         sent to the remote targets; <tt>null</tt> if it does not exist yet
	...
	/**
	 * Gets the <tt>OutputDataStream</tt> which is used to write RTP data to be sent
	 * to from the remote targets and optionally creates it if it does not exist
	 * yet.
	 *
	 * @param create <tt>true</tt> to create the <tt>OutputDataStream</tt> which is
	 *               to be used to write RTP data to be sent to the remote targets
	 *               if it does not exist yet; otherwise, <tt>false</tt>
	 * @return the <tt>OutputDataStream</tt> which is used to write RTP data to be
	 *         sent to the remote targets; <tt>null</tt> if it does not exist yet
	 *         and <tt>create</tt> is <tt>false</tt>
	 * @throws IOException if creating the <tt>OutputDataStream</tt> fails
	 */
	 ...
	 * {@link RTPConnector#getReceiveBufferSize()} that always returns <tt>-1</tt>.
	 */
	@Override
	...
	/**
	 * Provides a dummy implementation to {@link RTPConnector#getSendBufferSize()}
	 * that always returns <tt>-1</tt>.
	 */
	@Override
	...
	/**
	 * Removes a target from our session. If a target is removed, there will be no
	 * data sent to that address.
	 *
	...
	/**
	 * Configures this <tt>AbstractRTPConnector</tt> to allow RTP in the specified
	 * direction. That is, enables/disables the input and output data streams
	 * according to <tt>direction</tt>.
	 *
	 * Note that the control (RTCP) streams are not affected (they are always kept
	 * enabled).
	 *
