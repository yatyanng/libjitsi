...
public class DesktopInteractImpl implements DesktopInteract {
	/**
	 * The <tt>Logger</tt> used by the <tt>DesktopInteractImpl</tt> class and its
	 ...
	 * <tt>BufferedImage</tt>. It is done in order to limit slow operation such as
	 * converting ARGB images (uint32_t) to bytes especially for big big screen. For
	 * example a 1920x1200 desktop consumes 9 MB of memory for grabbing and another
	 * 9 MB array for conversion operation.
	 *
	 * @param display index of display
	 * @param output  output buffer to store bytes in. Be sure that output length is
	 *                sufficient
	 ...
	 *
	 * @param display      index of display
	 * @param buffer       native output buffer to store bytes in. Be sure that
