...
public abstract class RTPConnectorOutputStream implements OutputDataStream {
	/**
	 * The <tt>Logger</tt> used by the <tt>RTPConnectorOutputStream</tt> class and
	 ...
	 * attempt to write a new packet in the queue will result in the first packet in
	 * the queue being dropped. Defined in order to prevent
	 * <tt>OutOfMemoryError</tt>s which may arise if the capacity of the queue is
	 * unlimited.
	 */
	...
	/**
	 * The flag which controls whether this {@link RTPConnectorOutputStream} should
	 * create its own thread which will perform the packetization (and potential
	 * transformation) and sending of packets to the targets.
	 *
	 * If {@code true}, calls to {@link #write(byte[], int, int)} will only add the
	 * given bytes to {@link #queue}. Otherwise, packetization (via
	 * {@link #packetize(byte[], int, int, Object)}) and output (via
	 * {@link #sendToTarget(RawPacket, InetSocketAddress)} will be performed by the
	...
	/**
	 * The name of the property which controls the value of
	 * {@link #USE_SEND_THREAD}.
	 */
	private static final String USE_SEND_THREAD_PNAME = RTPConnectorOutputStream.class.getName() + ".USE_SEND_THREAD";

	/**
	 * The name of the <tt>ConfigurationService</tt> and/or <tt>System</tt> integer
	 * property which specifies the value of {@link #PACKET_QUEUE_CAPACITY}.
	 */
	private static final String PACKET_QUEUE_CAPACITY_PNAME = RTPConnectorOutputStream.class.getName()
	...
	/**
	 * The name of the property which specifies the value of
	 * {@link #AVERAGE_BITRATE_WINDOW_MS}.
	 */
	private static final String AVERAGE_BITRATE_WINDOW_MS_PNAME = RTPConnectorOutputStream.class.getName()
	...
	static {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Initialized configuration. " + "Send thread: " + USE_SEND_THREAD + ". Pool capacity: "
					+ POOL_CAPACITY + ". Queue capacity: " + PACKET_QUEUE_CAPACITY + ". Avg bitrate window: "
					+ AVERAGE_BITRATE_WINDOW_MS);
	...
	/**
	 * Determines whether a <tt>RawPacket</tt> which has a specific number in the
	 * total number of sent <tt>RawPacket</tt>s is to be logged by
	 * {@link PacketLoggingService}.
	 *
	 * @param numOfPacket the number of the <tt>RawPacket</tt> in the total number
	 *                    of sent <tt>RawPacket</tt>s
	...
	/**
	 * Whether this <tt>RTPConnectorOutputStream</tt> is enabled or disabled. While
	 * the stream is disabled, it suppresses actually sending any packets via
	...
	/**
	 * The {@code PacketLoggingService} instance (to be) utilized by this instance.
	 * Cached for the sake of performance because fetching OSGi services is not
	 * inexpensive.
	 */
	...
	/**
	 * The {@code RateStatistics} instance used to calculate the sending bitrate of
	 * this output stream.
	 */
	private final RateStatistics rateStatistics = new RateStatistics(AVERAGE_BITRATE_WINDOW_MS);

	/**
	 * Initializes a new <tt>RTPConnectorOutputStream</tt> which is to send packet
	 * data out through a specific socket.
	 */
	...
	/**
	 * Creates a <tt>RawPacket</tt> element from a specific <tt>byte[]</tt> buffer
	 ...
	 *                The contents of {@code buf} starting at {@code off} with the
	 *                specified {@code len} is copied into the buffer of the
	 *                returned {@code RawPacket}.
	 ...
	 *                {@link #write(byte[], int, int, java.lang.Object)}. The
	 *                implementation of {@code RTPConnectorOutputStream} ignores the
	 *                {@code context}.
	 * @return an array with a single <tt>RawPacket</tt> containing the packet data
	 *         of the specified <tt>byte[]</tt> buffer.
	 */
	protected RawPacket[] packetize(byte[] buf, int off, int len, Object context) {
		...
		if (pktBuffer.length < len) {
			/*
			 * XXX It may be argued that if the buffer length is insufficient once, it will
			 * be insufficient more than once. That is why we recreate it without returning
			 * a packet to the pool.
			 */
			pktBuffer = new byte[len];
		}
	 ...
	 * <tt>RTPConnectorOutputStream</tt>. They should not be used by the user
	 * afterwards.
	 *
	 * @param packet the RTP packet to be sent through the <tt>DatagramSocket</tt>
	 *               of this <tt>OutputDataSource</tt>
	 * @return <tt>true</tt> if the specified <tt>packet</tt> was successfully sent
	 *         to all targets; otherwise, <tt>false</tt>.
	 */
		...
		for (InetSocketAddress target : targets) {
			try {
				...
				if (logPacket(numberOfPackets)) {
					...
					if (pktLogging != null && pktLogging.isLoggingEnabled(PacketLoggingService.ProtocolName.RTP)) {
						doLogPacket(packet, target);
					}
				}
			} catch (IOException ioe) {
				rawPacketPool.offer(packet);
				logger.error("Failed to send a packet to target " + target + ":" + ioe);
				return false;
			}
		}
		rawPacketPool.offer(packet);
	...
	/**
	 * Sends a specific <tt>RawPacket</tt> through this <tt>OutputDataStream</tt> to
	 * a specific <tt>InetSocketAddress</tt>.
	 *
	 ...
	 *                     <tt>packet</tt> through this <tt>OutputDataStream</tt> to
	 *                     the specified <tt>target</tt>
	 */
	protected abstract void sendToTarget(RawPacket packet, InetSocketAddress target) throws IOException;

	/**
	 * Enables or disables this <tt>RTPConnectorOutputStream</tt>. While the stream
	 * is disabled, it suppresses actually sending any packets via
	 * {@link #send(RawPacket)}.
	 *
	 ...
	 * <tt>OutputDataStream</tt> through its <tt>DatagramSocket</tt> per a specific
	 ...
	 * @param off the offset in {@code buf} at which the {@code byte}s to be written
	 *            into this {@code OutputDataStream} start
	 * @param len the number of {@code byte}s in {@code buf} starting at {@code off}
	 *            to be written into this {@code OutputDataStream}
	...
	/**
	 * Writes a byte[] to this {@link RTPConnectorOutputStream} synchronously ( even
	 * when {@link #USE_SEND_THREAD} is enabled).
	 *
	 * @param buf
	 * @param off
	 * @param len
	...
	/**
	 * Implements {@link OutputDataStream#write(byte[], int, int)}. Allows extenders
	 * to provide a context {@code Object} to invoked overridable methods such as
	 ...
	 * @param context the {@code Object} to provide to invoked overridable methods
	 *                such as {@link #packetize(byte[],int,int,Object)}
	 * @return the number of {@code byte}s read from {@code buf} starting at
	 *         {@code off} and not exceeding {@code len} and written into this
	 *         {@code OutputDataStream}
	 */
	protected int write(byte[] buf, int off, int len, Object context) {
		if (enabled) {
			...
			if (logger.isDebugEnabled() && targets.isEmpty()) {
				logger.debug("Write called without targets! Stream will be closed!");
				this.close();
				return -1;
			}
			if (queue != null) {
	...
	private class Queue {
		...
		/**
		 * A pool of
		 * {@link org.jitsi.impl.neomedia.RTPConnectorOutputStream.Queue.Buffer}
		 * instances.
		 */
		final ArrayBlockingQueue<Buffer> pool = new ArrayBlockingQueue<>(15);

		/**
		 * The maximum number of {@link Buffer}s to be processed by {@link #sendThread}
		 * per {@link #perNanos} nanoseconds.
		 */
		int maxBuffers = -1;

		/**
		 * The time interval in nanoseconds during which no more than
		 * {@link #maxBuffers} {@link Buffer}s are to be processed by
		 * {@link #sendThread}.
		 */
		long perNanos = -1;
		...
		/**
		 * The time stamp in nanoseconds of the start of the current <tt>perNanos</tt>
		 * interval.
		 */
		long intervalStartTimeNanos = 0;

		/**
		 * The {@link Thread} which is to read {@link Buffer}s from this {@link Queue}
		 * and send them to this {@link RTPConnectorOutputStream}'s targets.
		 */
		 ...
		 * {@link RTPConnectorOutputStream#packetize(byte[], int, int, Object)} and
		 * sends the resulting packets to this {@link RTPConnectorOutputStream}'s
		 * targets.
		 *
		 * If a pacing policy is configured, makes sure that it is respected. Note that
		 * this pacing is done on the basis of the number of {@link Buffer}s read from
		 * the queue, which technically could be different than the number of
		 * {@link RawPacket}s sent. This is done in order to keep the implementation
		 * simpler, and because in the majority of the cases (and in all current cases
		 * where pacing is enabled) the numbers do match.
		 */
		...
		/**
		 * @return a free {@link Buffer} instance with a byte array with a length of at
		 *         least {@code len}.
		 */
