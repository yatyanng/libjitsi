...
public final class Pa {
	/**
	 * Enumerates the unchanging unique identifiers of each of the supported host
	 * APIs. The type is used in the <tt>PaHostApiInfo</tt> structure. The values
	 * are guaranteed to be unique and to never change, thus allowing code to be
	 * written that conditionally uses host API specific extensions.
	 */
	public static enum HostApiTypeId {
		paAL(9), paALSA(8), paASIO(3), paAudioScienceHPI(14), paBeOS(10), paCoreAudio(5), paDirectSound(1),
		paInDevelopment(0) /* use while developing support for a new host API */, paJACK(12), paMME(2), paOSS(7),
		paSoundManager(4), paWASAPI(13), paWDMKS(11);
		 ...
		 *
		 * @param value
		 * @return the <tt>PaHostApiTypeId</tt> which has the specified <tt>value</tt>
		 *         or <tt>null</tt> if there is no such representation
		 */
	...
	/**
	 * The number of milliseconds to be read from or written to a native PortAudio
	 * stream in a single transfer of data.
	 */
	public static final int DEFAULT_MILLIS_PER_BUFFER = 20;

	/**
	 * The default value for the sample rate of the input and the output PortAudio
	 * streams with which they are to be opened if no other specific sample rate is
	 * specified to the PortAudio <tt>DataSource</tt> or <tt>PortAudioRenderer</tt>
	 * that they represent.
	 */
	 ...
	 * <tt>Pa_OpenDefaultStream()</tt> to indicate that the stream callback will
	 * accept buffers of any size.
	 */
	public static final long FRAMES_PER_BUFFER_UNSPECIFIED = 0;

	/**
	 * Used when creating new stream parameters for suggested latency to use high
	 * input/output value.
	 */
	...
	/**
	 * Used when creating new stream parameters for suggested latency to use default
	 * value.
	 */
	...
	/**
	 * The constant defined by Windows Multimedia and utilized by PortAudio's wmme
	 * host API to signal that no device driver is present.
	 */
	public static final int MMSYSERR_NODRIVER = 6;

	/**
	 * The constant defined by the native PortAudio library to signal that no device
	 * is specified.
	 */
	 ...
	 * with the <tt>paFramesPerBufferUnspecified</tt> (<tt>0</tt>) framesPerBuffer
	 * parameter. Using this flag incorrectly results in a <tt>paInvalidFlag</tt>
	 * error being returned from <tt>Pa_OpenStream</tt> and
	 * <tt>Pa_OpenDefaultStream</tt>.
	 */
	public static final long STREAM_FLAGS_NEVER_DROP_INPUT = 0x00000004;

	/**
	 * Flags used to control the behavior of a stream. They are passed as parameters
	 * to <tt>Pa_OpenStream</tt> or <tt>Pa_OpenDefaultStream</tt>.
	 */
	...
	/**
	 * Closes an audio stream. If the audio stream is active it discards any pending
	 * buffers as if <tt>Pa_AbortStream()</tt> had been called.
	 *
	 * @param stream the steam pointer.
	 * @throws PortAudioException
	 */
	...
	/**
	 * Gets the name as a <tt>byte</tt> array of the PortAudio device specified by
	 * the pointer to its <tt>PaDeviceInfo</tt> instance.
	 *
	 * @param deviceInfo the pointer to the <tt>PaDeviceInfo</tt> instance to get
	 *                   the name of
	 * @return the name as a <tt>byte</tt> array of the PortAudio device specified
	 *         by the <tt>PaDeviceInfo</tt> instance pointed to by
	 *         <tt>deviceInfo</tt>
	 */
	...
	/**
	 * Implements a callback which gets called by the native PortAudio counterpart
	 * to notify the Java counterpart that the list of PortAudio devices has
	 * changed.
	 */
	 ...
	 * <tt>deviceID</tt> is either a <tt>deviceUID</tt> or a (PortAudio device) name
	 * depending, for example, on operating system/API availability. Since at least
	 * names may not be unique, the PortAudio device to return the index of may be
	 * identified more specifically by the minimal numbers of channels to be
	 * required from the device for input and output.
	 *
	 * @param deviceID          a <tt>String</tt> identifying the PortAudio device
	 *                          to retrieve the index of. It is either a
	 *                          <tt>deviceUID</tt> or a (PortAudio device) name.
	...
	public static int getDeviceIndex(String deviceID, int minInputChannels, int minOutputChannels) {
		if (deviceID != null) {
			...
			for (int deviceIndex = 0; deviceIndex < deviceCount; ++deviceIndex) {
				...
				if (deviceID
						.equals(((deviceUID == null) || (deviceUID.length() == 0)) ? Pa.DeviceInfo_getName(deviceInfo)
								: deviceUID)) {
					/*
	 ...
	 *         waiting or, a PaErrorCode (which are always negative) if PortAudio is
		...
		if (cfg != null) {
			...
			if (suggestedLatencyString != null) {
				try {
					...
					logger.error(
							"Failed to parse configuration property " + PROP_SUGGESTED_LATENCY + " value as a double",
							nfe);
				}
			}
		}
	 ...
	 *         why the format is not supported otherwise. The constant
	 ...
	 *                         function, or the preferred block granularity for a
	 ...
	 *                         responsible for processing and filling input and
	 *                         output buffers. If <tt>null</tt>, the stream will be
	 *                         opened in 'blocking read/write' mode.
	 * @return pointer to the opened stream.
	 * @throws PortAudioException
	 */
	public static native long OpenStream(long inputParameters, long outputParameters, double sampleRate,
			long framesPerBuffer, long streamFlags, PortAudioStreamCallback streamCallback) throws PortAudioException;

	/**
	 * Read samples from an input stream. The function doesn't return until the
	 * entire buffer has been filled - this may involve waiting for the operating
	 ...
	 *                enabled or disabled
	 * @param denoise <tt>true</tt> if denoise is to be performed on the audio data
	 ...
	 *
	 * @param stream                   the (input) PortAudio stream for which the
	 *                                 number of milliseconds of echo to be canceled
	 *                                 is to be set
	 * @param echoFilterLengthInMillis the number of milliseconds of echo to be
	 *                                 canceled in the audio data provided by
	 *                                 <tt>stream</tt>
	 */
	...
	/**
	 * Terminates audio processing. It waits until all pending audio buffers have
	 * been played before it returns.
	 *
	 * @param stream pointer to the stream
	 * @throws PortAudioException
	 */
	 ...
	 *
	 * @param streamParameters the pointer to the <tt>PaStreamParameters</tt> to
	 *                         free
	 */
	 ...
	 *                         LATENCY_UNSPECIFIED - use default(default high
	 *                         input/output latency) LATENCY_HIGH - use default high
	 *                         input/output latency LATENCY_LOW - use default low
	 *                         input/output latency ... - any other value in
	 *                         milliseconds (e.g. 0.1 is acceptable)
	 * @return pointer to the params used for Pa_OpenStream.
	 */
	public static native long StreamParameters_new(int deviceIndex, int channelCount, long sampleFormat,
	 ...
	 * <p>
	 * Provides better efficiency than achieved through multiple consecutive calls
	 * to {@link #WriteStream(long, byte[], long)} with one and the same buffer
	 * because the JNI access to the bytes of the buffer which is likely to copy the
	 * whole buffer is only performed once.
	 * </p>
	 *
	 * @param stream         the pointer to the PortAudio stream to write the
	 *                       samples to
	 * @param buffer         the buffer containing the samples to be written
	 * @param offset         the byte offset in <tt>buffer</tt> at which the samples
	 *                       to be written start
	 ...
	 */
	public static native void WriteStream(long stream, byte[] buffer, int offset, long frames, int numberOfWrites)
			throws PortAudioException;
