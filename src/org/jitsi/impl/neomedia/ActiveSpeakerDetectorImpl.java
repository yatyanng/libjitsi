...
public class ActiveSpeakerDetectorImpl implements ActiveSpeakerDetector {
	 ...
	 * class name of the algorithm implementation for the detection/identification
	 * of the active/dominant speaker in a multipoint conference to be used by
	 * <tt>ActiveSpeakerDetectorImpl</tt>. The default value is <tt>null</tt>. If
	 * the specified value is <tt>null</tt> or the initialization of an instance of
	 * the specified class fails, <tt>ActiveSpeakerDetectorImpl</tt> falls back to a
	 * list of well-known algorithm implementations.
	 */
	 ...
	 * active/dominant speaker in a multipoint conference.
	 */
	private static final String[] IMPL_CLASS_NAMES = { ".DominantSpeakerIdentification",
	 ...
	 * first available algorithm from a specific list of algorithms (identified by
	 * the names of their implementing classes) for the detection/identification of
	 * the active/dominant speaker in a multipoint conference.
	 * 
	 * @param implClassNames the class names of the algorithm implementations to
	 *                       search through and in which the first available is to
	 *                       be found and used for the detection/identification of
	 *                       the active/dominant speaker in a multipoint conference
	 * @throws RuntimeException if none of the algorithm implementations specified
	 *                          by <tt>implClassNames</tt> is available
	 */
	...
	/**
	 * Gets the names of the classes known by <tt>ActiveSpeakerDetectorImpl</tt> to
	 * implement actual algorithms for the detection/identification of the
	 ...
	 *
	 * @return the names of the classes known by <tt>ActiveSpeakerDetectorImpl</tt>
	 *         to implement actual algorithms for the detection/identification of
	 *         the active/dominant speaker in a multipoint conference
	 */
	private static String[] getImplClassNames() {
		/*
		 * The user is allowed to specify the class name of the (default) algorithm
		 * implementation through the ConfigurationService.
		 */
	 ...
	 *
	 * @param className the class name to prefix with a package name if necessary
