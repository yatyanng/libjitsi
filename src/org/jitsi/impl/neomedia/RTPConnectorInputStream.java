...
public abstract class RTPConnectorInputStream<T> implements PushSourceStream {
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>RTPConnectorInputStream</tt> class and
		...
		if (priority != oldPriority) {
			...
			if (logger.isDebugEnabled()) {
				...
				if (priority != newPriority) {
					logger.debug("Did not change Thread priority from " + oldPriority + " to " + priority + ", "
							+ newPriority + " instead.");
				}
			}
		}
	...
	/**
	 * Whether this <tt>RTPConnectorInputStream</tt> is enabled or disabled. While
	 * disabled, the stream does not accept any packets.
	 */
	...
	/**
	 * Initializes a new <tt>RTPConnectorInputStream</tt> which is to receive packet
	 * data from a specific UDP socket.
	 *
	 * @param socket
	 */
	protected RTPConnectorInputStream(T socket) {
		...
		/*
		 * Adapt this PushSourceStream to the PushBufferStream interface in order to
		 * make it possible to read the Buffer flags of RawPacket.
		 */
	 ...
	 * <tt>DatagramPacket</tt> for pushing out of this <tt>PushSourceStream</tt>. In
	 * other words, determines whether <tt>p</tt> is to be
	 ...
	 * <tt>DatagramPacket</tt>s before they are converted into <tt>RawPacket</tt>s.
	 *
	 * @param datagramPacketFilter the <tt>DatagramPacketFilter</tt> which allows
	 *                             dropping <tt>DatagramPacket</tt>s before they are
	 *                             converted into <tt>RawPacket</tt>s
	 */
	...
	public synchronized void close() {
		...
		if (socket != null) {
			/*
			 * The classes DatagramSocket and Socket implement the interface Closeable since
	...
	/**
	 * Creates a new <tt>RawPacket</tt> from a specific <tt>DatagramPacket</tt> in
	 * order to have this instance receive its packet data through its
	...
	/**
	 * Provides a dummy implementation of {@link PushSourceStream#endOfStream()}
	 ...
	 * {@link PushSourceStream#getContentDescriptor()} that always returns
	 * <tt>null</tt>.
	 *
	...
	/**
	 * Provides a dummy implementation of
	 * {@link PushSourceStream#getContentLength()} that always returns
	...
	/**
	 * Provides a dummy implementation of
	 * {@link PushSourceStream#getControl(String)} that always returns
	 * <tt>null</tt>.
	 *
	...
	/**
	 * Provides a dummy implementation of {@link PushSourceStream#getControls()}
	 * that always returns <tt>EMPTY_CONTROLS</tt>.
	 *
	...
	/**
	 * Provides a dummy implementation of
	 * {@link PushSourceStream#getMinimumTransferSize()} that always returns
	 * <tt>2 * 1024</tt>.
	 *
	...
	private synchronized void maybeStartReceiveThread() {
		if (receiveThread == null) {
			if ((socket != null) && !closed && (transferHandler != null)) {
				receiveThread = new Thread(RTPConnectorInputStream.this::runInReceiveThread);
	...
	/**
	 * Pools the specified <tt>RawPacket</tt> in order to avoid future allocations
	 * and to reduce the effects of garbage collection.
	 *
	 ...
	 *               <tt>flags</tt>
	 * @param data   the <tt>byte[]</tt> that we'd like to copy the content of the
	 *               packet to.
	 ...
	 *
	 * @param buffer the <tt>byte[]</tt> that we'd like to copy the content of the
	 *               packet to.
	 * @param offset the position where we are supposed to start writing in
	 *               <tt>buffer</tt>.
	 * @param length the number of <tt>byte</tt>s available for writing in
	...
	/**
	 * Enables or disables this <tt>RTPConnectorInputStream</tt>. While the stream
	 * is disabled, it does not accept any packets.
	 *
...
//            receiverThread.setPriority(priority);
	 ...
	 *                        should be notifying when new data is available for
	 *                        reading.
	 */
	@Override
	 ...
	 * {@link #transferHandler} for each of <tt>pkts</tt> in order to consecutively
	 * push them out of/make them available outside this <tt>PushSourceStream</tt>.
	 *
		...
		for (int i = 0; i < pkts.length; i++) {
			...
			if (pkt != null) {
				if (pkt.isInvalid()) {
					...
					if (oldPkt != null) {
						/*
						 * Return oldPkt to the pool because it was made available to reading and it was
						 * not read.
						 */
