...
public abstract class MediaFormatImpl<T extends Format> implements MediaFormat {
	 ...
	 * <tt>MediaFormatImpl</tt> when no codec-specific parameters have been received
	 * via SIP/SDP or XMPP/Jingle. Explicitly defined in order to reduce unnecessary
	 * allocations.
	 */
	...
	/**
	 * Creates a new <tt>MediaFormat</tt> instance for a specific JMF
	 * <tt>Format</tt> and assigns it specific clock rate and set of format-specific
	 * parameters.
	 *
	 * @param format              the JMF <tt>Format</tt> the new instance is to
	 *                            provide an implementation of <tt>MediaFormat</tt>
	 *                            for
	...
	public static MediaFormatImpl<? extends Format> createInstance(Format format, double clockRate,
		...
		if (format instanceof AudioFormat) {
			...
			AudioFormat clockRateAudioFormat = new AudioFormat(audioFormat.getEncoding(), clockRate,
					audioFormat.getSampleSizeInBits(), audioFormat.getChannels());

			return new AudioMediaFormatImpl((AudioFormat) clockRateAudioFormat.intersects(audioFormat),
					formatParameters, advancedAttributess);
		} else if (format instanceof VideoFormat) {
			return new VideoMediaFormatImpl((VideoFormat) format, clockRate, -1, formatParameters, advancedAttributess);
		} else
			return null;
	}
	...
	/**
	 * The advanced parameters of this instance which have been received via SIP/SDP
	 * or XMPP/Jingle.
	 */
	...
	/**
	 * The codec-specific parameters of this instance which have been received via
	 * SIP/SDP or XMPP/Jingle.
	 */
	private final Map<String, String> formatParameters;

	/**
	 * Initializes a new <tt>MediaFormatImpl</tt> instance which is to provide an
	 * implementation of <tt>MediaFormat</tt> for a specific <tt>Format</tt>.
	 *
	 * @param format the JMF <tt>Format</tt> the new instance is to provide an
	 *               implementation of <tt>MediaFormat</tt> for
	 */
	...
	/**
	 * Initializes a new <tt>MediaFormatImpl</tt> instance which is to provide an
	 * implementation of <tt>MediaFormat</tt> for a specific <tt>Format</tt> and
	 * which is to have a specific set of codec-specific parameters.
	 *
	 * @param format             the JMF <tt>Format</tt> the new instance is to
	 *                           provide an implementation of <tt>MediaFormat</tt>
	 *                           for
	 ...
	 * @param advancedAttributes any parameters that have been received via SIP/SDP
	 *                           or XMPP/Jingle
	 */
	protected MediaFormatImpl(T format, Map<String, String> formatParameters, Map<String, String> advancedAttributes) {
		...
		this.format = format;
		this.formatParameters = ((formatParameters == null) || formatParameters.isEmpty()) ? EMPTY_FORMAT_PARAMETERS
	...
	/**
	 * Determines whether a specific set of advanced attributes is equal to another
	 * set of advanced attributes in the sense that they define an equal number of
	 * parameters and assign them equal values. Since the values are
	 * <tt>String</tt>s, presumes that a value of <tt>null</tt> is equal to the
	 * empty <tt>String</tt>.
	 * <p>
	...
	/**
	 * Implements MediaFormat#equals(Object) and actually compares the encapsulated
	 * JMF <tt>Format</tt> instances.
	 *
	 * @param mediaFormat the object that we'd like to compare <tt>this</tt> one to.
	 *                    8*
	 * @return <tt>true</tt> if the JMF <tt>Format</tt> instances encapsulated by
	 *         this class are equal and <tt>false</tt> otherwise.
	 */
	@Override
	public boolean equals(Object mediaFormat) {
		...
		return getFormat().equals(mediaFormatImpl.getFormat())
				&& formatParametersAreEqual(getFormatParameters(), mediaFormatImpl.getFormatParameters());
	}
	 ...
	 * attribute properties of this instance. Modifications to the returned Map do
	 * no affect the format properties of this instance.
	 *
	 * @return a copy of the attribute properties of this instance. Modifications to
	 *         the returned Map do no affect the format properties of this instance.
	 */
	@Override
	...
	/**
	 * Returns a <tt>String</tt> representation of the clock rate associated with
	 * this <tt>MediaFormat</tt> making sure that the value appears as an integer
	 * (i.e. its long-casted value is equal to its original one) unless it is
	 * actually a non integer.
	 *
	 * @return a <tt>String</tt> representation of the clock rate associated with
	 *         this <tt>MediaFormat</tt>.
	 */
	@Override
	 ...
	 *
	 * @return a reference to that JMF <tt>Format</tt> instance that this class is
	 *         wrapping.
	 */
	public T getFormat() {
		return format;
	...
	/**
	 * Implements MediaFormat#getFormatParameters(). Returns a copy of the format
	 * properties of this instance. Modifications to the returned Map do no affect
	 * the format properties of this instance.
	 *
	 * @return a copy of the format properties of this instance. Modifications to
	 *         the returned Map do no affect the format properties of this instance.
	 */
	@Override
	...
	/**
	 * Gets the encoding of the JMF <tt>Format</tt> represented by this instance as
	 * it is known to JMF (in contrast to its RFC name).
	 *
	 * @return the encoding of the JMF <tt>Format</tt> represented by this instance
	 ...
	 * associated with this <tt>MediaFormat</tt> making sure that the value appears
	 * as an integer (i.e. contains no decimal point) unless it is actually a non
	 * integer. This function corrects the problem of the G.722 codec which
	 * advertises its clock rate to be 8 kHz while 16 kHz is really used to encode
	 * the stream (that's an error noted in the respective RFC and kept for the sake
	 * of compatibility.).
	 *
	 ...
	 * known in RFC 3551 "RTP Profile for Audio and Video Conferences with Minimal
	 * Control".
	 *
	 * @return the RTP payload type of this <tt>MediaFormat</tt> if it is known in
	...
	public int hashCode() {
		/*
		 * XXX We've experienced a case of JMF's VideoFormat#hashCode() returning
		 * different values for instances which are reported equal by
		 * VideoFormat#equals(Object) which is inconsistent with the protocol covering
		 * the two methods in question and causes problems, for example, with Map. While
		 * jmfEncoding is more generic than format, it still provides a relatively good
		 * distribution given that we do not have a lot of instances with one and the
		 * same jmfEncoding.
		 */
	 ...
	 * <tt>MediaFormat</tt>, such as <tt>mediaType</tt>, <tt>encoding</tt>,
	...
	public boolean matches(MediaFormat format) {
		...
		int channels = MediaType.AUDIO.equals(mediaType) ? ((AudioMediaFormat) format).getChannels()
	 ...
	 * @return <tt>true</tt> if the specified <tt>format</tt> has specific values
	...
	public boolean matches(MediaType mediaType, String encoding, double clockRate, int channels,
		...
		if (clockRate != MediaFormatFactory.CLOCK_RATE_NOT_SPECIFIED) {
			...
			if ((formatClockRate != MediaFormatFactory.CLOCK_RATE_NOT_SPECIFIED) && (formatClockRate != clockRate))
				return false;
		}
	...
	public void setAdditionalCodecSettings(Map<String, String> settings) {
		codecSettings = ((settings == null) || settings.isEmpty()) ? EMPTY_FORMAT_PARAMETERS : settings;
	}
	...
	public String toString() {
		...
		/*
		 * If the number of channels is 1, it does not have to be mentioned because it
		 * is the default.
		 */
