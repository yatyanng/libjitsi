...
public class MediaFormatFactoryImpl implements MediaFormatFactory {
	/**
	 * The <tt>Logger</tt> used by the <tt>MediaFormatFactoryImpl</tt> class and its
	...
	public MediaFormat createUnknownMediaFormat(MediaType type) {
		...
		/*
		 * FIXME Why is a VideoFormat instance created for MediaType.AUDIO and an
		 * AudioFormat instance for MediaType.VIDEO?
		 */
	 ...
	 * which is either an <tt>AudioMediaFormat</tt> or a <tt>VideoMediaFormat</tt>
	 * instance. Otherwise, returns <tt>null</tt>.
	 *
	 * @param encoding the well-known encoding (name) to create a
	 *                 <tt>MediaFormat</tt> for
	 * @return a <tt>MediaFormat</tt> with the specified <tt>encoding</tt> which is
	 ...
	 * default clock rate and set of format parameters. If <tt>rtpPayloadType</tt>
	 * is known to this <tt>MediaFormatFactory</tt>, returns a <tt>MediaFormat</tt>
	 * which is either an <tt>AudioMediaFormat</tt> or a <tt>VideoMediaFormat</tt>
	 * instance. Otherwise, returns <tt>null</tt>.
	 *
	...
	public MediaFormat createMediaFormat(byte rtpPayloadType) {
		/*
		 * We know which are the MediaFormat instances with the specified rtpPayloadType
		 * but we cannot directly return them because they do not reflect the user's
		 * configuration with respect to being enabled and disabled.
		 */
	...
	/**
	 * Creates a <tt>MediaFormat</tt> for the specified <tt>encoding</tt> with the
	 * specified <tt>clockRate</tt> and a default set of format parameters. If
	 ...
	 *         <tt>VideoMediaFormat</tt> instance if <tt>encoding</tt> is known to
	 ...
	 * <tt>MediaFormatFactory</tt>, returns a <tt>MediaFormat</tt> which is either
	 * an <tt>AudioMediaFormat</tt> or a <tt>VideoMediaFormat</tt> instance.
	 ...
	 *                  if it makes sense for the <tt>MediaFormat</tt> with the
	 *                  specified <tt>encoding</tt>; otherwise, ignored
	...
	private MediaFormat createMediaFormat(String encoding, double clockRate, int channels, Map<String, String> fmtps) {
		for (MediaFormat format : getSupportedMediaFormats(encoding, clockRate)) {
			/*
			 * The mediaType, encoding and clockRate properties are sure to match because
			 * format is the result of the search for encoding and clockRate. We just want
			 * to make sure that the channels and the format parameters match.
			 */
	 ...
	 *                     <tt>MediaFormat</tt> for
	 * @param clockRate    the clock rate in Hz to create a <tt>MediaFormat</tt> for
	 * @param formatParams any codec specific parameters which have been received
	 ...
	 *                       for
	 * @param frameRate      the frame rate in number of frames per second to create
	 *                       a <tt>MediaFormat</tt> for
	 * @param channels       the number of available channels (1 for mono, 2 for
	 *                       stereo) if it makes sense for the <tt>MediaFormat</tt>
	 *                       with the specified <tt>encoding</tt>; otherwise,
	 *                       ignored
	 * @param formatParams   any codec specific parameters which have been received
	 *                       via SIP/SDP or XMPP/Jingle
	 * @param advancedParams any parameters which have been received via SIP/SDP or
	 *                       XMPP/Jingle
	 * @return a <tt>MediaFormat</tt> with the specified <tt>encoding</tt>,
	 *         <tt>clockRate</tt>, <tt>channels</tt> and set of format parameters
	 *         which is either an <tt>AudioMediaFormat</tt> or a
	 *         <tt>VideoMediaFormat</tt> instance if <tt>encoding</tt> is known to
	 *         this <tt>MediaFormatFactory</tt>; otherwise, <tt>null</tt>
	 * @see MediaFormatFactory#createMediaFormat(String, double, int, float, Map,
	 *      Map)
	 */
	@Override
	public MediaFormat createMediaFormat(String encoding, double clockRate, int channels, float frameRate,
		...
		/*
		 * MediaFormatImpl is immutable so if the caller wants to change the format
		 * parameters and/or the advanced attributes, we'll have to create a new
		 * MediaFormatImpl.
		 */
		...
		if ((formatParameters != null) || (advancedParameters != null)) {
			...
			case AUDIO:
				mediaFormat = new AudioMediaFormatImpl(((AudioMediaFormatImpl) mediaFormat).getFormat(),
			...
			case VIDEO:
				...
				mediaFormat = new VideoMediaFormatImpl(videoMediaFormatImpl.getFormat(),
						videoMediaFormatImpl.getClockRate(), frameRate, formatParameters, advancedParameters);
				break;
			default:
				mediaFormat = null;
			}
	 ...
	 * <tt>AudioMediaFormat</tt> or a <tt>VideoMediaFormat</tt> instance. If
	 * <tt>rtpPayloadType</tt> is not {@link MediaFormat#RTP_PAYLOAD_TYPE_UNKNOWN}
	 * and <tt>encoding</tt> is <tt>null</tt>, uses the encoding associated with
	 * <tt>rtpPayloadType</tt>.
	 *
	 * @param rtpPayloadType the RTP payload type to create a <tt>MediaFormat</tt>
	 *                       for; {@link MediaFormat#RTP_PAYLOAD_TYPE_UNKNOWN} if
	...
	public MediaFormat createMediaFormat(byte rtpPayloadType, String encoding, double clockRate, int channels,
		...
		/*
		 * If rtpPayloadType is specified, use it only to figure out encoding and/or
		 * clockRate in case either one of them is unknown.
		 */
	 ...
	 * <tt>clockRate</tt>.
	 *
	 * @param mediaFormats the <tt>MediaFormat</tt>s from which to filter out only
	 *                     the ones which have the specified <tt>encoding</tt> and,
	 ...
	 *                     retrieved; {@link #CLOCK_RATE_NOT_SPECIFIED} if any clock
	 *                     rate is acceptable
	...
	private List<MediaFormat> getMatchingMediaFormats(MediaFormat[] mediaFormats, String encoding, double clockRate) {
		/*
		 * XXX Use String#equalsIgnoreCase(String) because some clients transmit some of
		 * the codecs starting with capital letters.
		 */
		 ...
		 * wideband. So, if someone is looking for G722/16000, then: Forgive them, for
		...
		for (MediaFormat mediaFormat : mediaFormats) {
			if (mediaFormat.getEncoding().equalsIgnoreCase(encoding)
					&& ((CLOCK_RATE_NOT_SPECIFIED == clockRate) || (mediaFormat.getClockRate() == clockRate))) {
	...
	/**
	 * Gets the <tt>MediaFormat</tt>s supported by this <tt>MediaFormatFactory</tt>
	 * and the <tt>MediaService</tt> associated with it and having the specified
	 * <tt>encoding</tt> and, optionally, <tt>clockRate</tt>.
	 *
	 * @param encoding  the well-known encoding (name) of the <tt>MediaFormat</tt>s
	 *                  to be retrieved
	 ...
	 *         <tt>MediaFormatFactory</tt> and having the specified encoding and,
	 *         optionally, clock rate
	 */
	private List<MediaFormat> getSupportedMediaFormats(String encoding, double clockRate) {
		EncodingConfiguration encodingConfiguration = NeomediaServiceUtils.getMediaServiceImpl()
		...
		if (supportedMediaFormats.isEmpty())
			supportedMediaFormats = getMatchingMediaFormats(encodingConfiguration.getAllEncodings(MediaType.VIDEO),
					encoding, clockRate);
		return supportedMediaFormats;
	}
}
