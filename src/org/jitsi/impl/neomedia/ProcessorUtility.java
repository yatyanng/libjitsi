...
public class ProcessorUtility implements ControllerListener {
	...
	/**
	 * The <tt>Object</tt> used for syncing when waiting for a processor to enter a
	 * specific state.
	 */
	...
	/**
	 * Gets the <tt>Object</tt> to use for syncing when waiting for a processor to
	 * enter a specific state.
	 *
	 * @return the <tt>Object</tt> to use for syncing when waiting for a processor
	...
	/**
	 * This method is called when an event is generated by a <code>Controller</code>
	 * that this listener is registered with. We use the event to notify all waiting
	 * on our lock and record success or failure.
	 *
	...
	/**
	 * Waits until <tt>processor</tt> enters state and returns a boolean indicating
	...
	public synchronized boolean waitForState(Processor processor, int state) {
		...
		logger.debug(
				processor.getClass().getName() + " " + processor.hashCode() + " is waiting for state " + state + "!");
		...
		while ((processor.getState() < state) && !failed) {
			logger.trace(processor.getClass().getName() + " " + processor.hashCode() + " is in state "
					+ processor.getState() + " and is waiting for state " + state + "!");
...
//                    return false;
			...
			logger.trace(processor.getClass().getName() + " " + processor.hashCode() + " is now in state "
					+ processor.getState() + "!");
		}
		if (interrupted)
