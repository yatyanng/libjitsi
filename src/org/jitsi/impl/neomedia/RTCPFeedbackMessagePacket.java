...
public class RTCPFeedbackMessagePacket implements Payload {
	...
	/**
	 * The (command) sequence number of this Full Intra Request (FIR) RTCP feedback
	 * message as defined by RFC 5104 &quot;Codec Control Messages in the RTP
	 * Audio-Visual Profile with Feedback (AVPF)&quot. The sequence number space is
	 * unique for each pairing of the SSRC of command source and the SSRC of the
	 * command target. The sequence number SHALL be increased by 1 modulo 256 for
	 * each new command. A repetition SHALL NOT increase the sequence number. The
	 ...
	 * feedback message as defined by RFC 5104 &quot;Codec Control Messages in the
	 * RTP Audio-Visual Profile with Feedback (AVPF)&quot. The sequence number space
	 * is unique for each pairing of the SSRC of command source and the SSRC of the
	 * command target. The sequence number SHALL be increased by 1 modulo 256 for
	 * each new command. A repetition SHALL NOT increase the sequence number. The
	 * initial value is arbitrary.
	 *
	 * @return the (command) sequence number of this Full Intra Request (FIR) RTCP
	 *         feedback message
	 */
	...
	/**
	 * Gets the synchronization source identifier (SSRC) of the media source that
	 * this piece of feedback information is related to.
	 *
	 * @return the synchronization source identifier (SSRC) of the media source that
	 *         this piece of feedback information is related to
	 */
	 ...
	 * each new command.
	 *
	 * @param seqNr the (command) sequence number to set on this Full Intra Request
	 *              (FIR) RTCP feedback message
	 */
	...
	/**
	 * Sets the synchronization source identifier (SSRC) of the media source that
	 * this piece of feedback information is related to.
	 *
	 * @param sourceSSRC the synchronization source identifier (SSRC) of the media
	 *                   source that this piece of feedback information is related
	 *                   to
	 */
	...
	/**
	 * Writes a specific synchronization source identifier (SSRC) into a specific
	 * <tt>byte</tt> array starting at a specific offset.
	 * 
	 * @param ssrc the synchronization source identifier (SSRC) to write into
	 *             <tt>buf</tt> starting at <tt>off</tt>
	 * @param buf  the <tt>byte</tt> array to write the specified <tt>ssrc</tt> into
	 *             starting at <tt>off</tt>
	 * @param off  the offset in <tt>buf</tt> at which the writing of <tt>ssrc</tt>
	 *             is to start
	 */
	...
	public void writeTo(OutputDataStream out) {
		...
		boolean fir = pt == RTCPFeedbackMessageEvent.PT_PS && fmt == RTCPFeedbackMessageEvent.FMT_FIR;
		int rtcpPacketLength = 2;

		if (fir) {
			/*
			 * RFC 5104 "Codec Control Messages in the RTP Audio-Visual Profile with
			 * Feedback (AVPF)" defines that The length of the FIR feedback message MUST be
		...
		/*
		 * RFC 5104 "Codec Control Messages in the RTP Audio-Visual Profile with
		 * Feedback (AVPF)" defines that the "SSRC of media source" is not used by the
		 * FIR feedback message and SHALL be set to 0.
		 */
		writeSSRC(fir ? 0 : sourceSSRC, buf, off);
		off += 4;

		// FCI entries
		if (fir) {
			/*
			 * SSRC: 32 bits. The SSRC value of the media sender that is requested to send a
			...
			/*
			 * Reserved: 24 bits. All bits SHALL be set to 0 by the sender and SHALL be
