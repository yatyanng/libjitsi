...
public abstract class AbstractFormatControl implements FormatControl {
	...
	/**
	 * Gets an array of <tt>FormatControl</tt> instances from the list of controls
	 * available for a specific <tt>Controls</tt> implementation.
	 *
	...
	/**
	 * Implements {@link FormatControl#setFormat(Format)}. Not supported and just
	 * returns the currently set format if the specified <tt>Format</tt> is
	...
	/**
	 * Implements setting the <tt>Format</tt> of a specific <tt>FormatControl</tt>
	 * as documented for {@link FormatControl#setFormat(Format)} in the case of not
	 * supporting <tt>Format</tt> setting.
	 *
	 * @param formatControl the <tt>FormatControl</tt> for which the functionality
	 *                      is implemented
	 * @param format        the <tt>Format</tt> specified to be set to
	 *                      <tt>formatControl</tt> and which will be ignored in
	 *                      accord with the documentation of
	 *                      <tt>FormatControl#setFormat(Format)</tt>
	 * @return the currently set <tt>Format</tt> of <tt>formatControl</tt> if
	 *         <tt>format</tt> is supported by <tt>formatControl</tt>; <tt>null</tt>
	 *         if <tt>format</tt> is not supported by <tt>formatControl</tt>
	 */
	public static Format setFormat(FormatControl formatControl, Format format) {
		/*
		 * Determine whether the specified format is supported by this instance because
		 * we have to return null if it is not supported. Or at least that is what I
		 * gather from the respective javadoc.
		 */
		...
		/*
		 * We do not actually support setFormat so we have to return the currently set
		 * format if the specified format is supported.
		 */
