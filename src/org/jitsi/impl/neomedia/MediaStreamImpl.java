...
public class MediaStreamImpl extends AbstractMediaStream
	 ...
	 *                   representation of
	 * @return a <tt>String</tt> value which gives a human-readable representation
	 *         of the specified <tt>dataSource</tt>
	 */
	...
	/**
	 * The map of currently active <tt>RTPExtension</tt>s and the IDs that they have
	 * been assigned for the lifetime of this <tt>MediaStream</tt>.
	 */
	private final Map<Byte, RTPExtension> activeRTPExtensions = new Hashtable<>();

	/**
	 * The engine that we are using in order to add CSRC lists in conference calls,
	 * send CSRC sound levels, and handle incoming levels and CSRC lists.
	 */
	private CsrcTransformEngine csrcEngine;

	/**
	 * The session with the <tt>MediaDevice</tt> this instance uses for both capture
	 * and playback of media.
	 */
	private MediaDeviceSession deviceSession;

	/**
	 * The <tt>PropertyChangeListener</tt> which listens to {@link #deviceSession}
	 * and changes in the values of its
	...
	/**
	 * The <tt>MediaDirection</tt> in which this <tt>MediaStream</tt> is allowed to
	 * stream media.
	 */
	...
	/**
	 * The list of CSRC IDs contributing to the media that this <tt>MediaStream</tt>
	 * is sending to its remote party.
	 */
	...
	/**
	 * The indicator which determines whether this <tt>MediaStream</tt> is set to
	 * transmit "silence" instead of the actual media fed from its
	 * <tt>MediaDevice</tt>.
	 */
	 ...
	 * <tt>MediaDevice</tt>. The (read and write) accesses to the field are to be
	 * synchronized using {@link #receiveStreamsLock}.
	 */
	private final List<ReceiveStream> receiveStreams = new LinkedList<>();

	/**
	 * The <tt>ReadWriteLock</tt> which synchronizes the (read and write) accesses
	 * to {@link #receiveStreams}.
	 */
	 ...
	 *
	 * XXX(gp) I'm sure there's a reason why we do it the way we do it, but we might
	 * want to re-think about how we manage receive SSRCs. We keep track of the
	 * receive SSRC in at least 3 places, in the MediaStreamImpl (we have a
	 * remoteSourceIDs vector), in StreamRTPManager.receiveSSRCs and in
	...
	/**
	 * The <tt>RTPConnector</tt> through which this instance sends and receives RTP
	 * and RTCP traffic. The instance is a <tt>TransformConnector</tt> in order to
	 * also enable packet transformations.
	 */
	...
	/**
	 * The <tt>RTPManager</tt> which utilizes {@link #rtpConnector} and sends and
	 * receives RTP and RTCP traffic on behalf of this <tt>MediaStream</tt>.
	 */
	private StreamRTPManager rtpManager;

	/**
	 * The indicator which determines whether {@link #createSendStreams()} has been
	 * executed for {@link #rtpManager}. If <tt>true</tt>, the <tt>SendStream</tt>s
	 * have to be recreated when the <tt>MediaDevice</tt>, respectively the
	 * <tt>MediaDeviceSession</tt>, of this instance is changed.
	 */
	 ...
	 * synchronization source (SSRC) identifiers. If <tt>null</tt>, this instance
	 * will employ internal logic to generate new synchronization source (SSRC)
	 * identifiers.
	 */
	private SSRCFactory ssrcFactory = new SSRCFactoryImpl(localSourceID);

	/**
	 * The indicator which determines whether {@link #start()} has been called on
	 * this <tt>MediaStream</tt> without {@link #stop()} or {@link #close()}.
	 */
	private boolean started = false;

	/**
	 * The <tt>MediaDirection</tt> in which this instance is started. For example,
	 * {@link MediaDirection#SENDRECV} if this instances is both sending and
	 * receiving data (e.g. RTP and RTCP) or {@link MediaDirection#SENDONLY} if this
	 * instance is only sending data.
	 */
	...
	/**
	 * The <tt>TransformEngine</tt> instance that logs packets going in and out of
	...
	/**
	 * The transformer which replaces the timestamp in an abs-send-time RTP header
	 * extension.
	 */
	...
	/**
	 * The {@code RetransmissionRequesterImpl} instance for this {@code MediaStream}
	 * which will request missing packets by sending RTCP NACKs.
	 */
	private final RetransmissionRequesterImpl retransmissionRequester = createRetransmissionRequester();

	/**
	 * The engine which adds an Original Header Block header extension to incoming
	 * packets.
	 */
	...
	/**
	 * The ID of the frame markings RTP header extension. We use this field as a
	 * cache, in order to not access {@link #activeRTPExtensions} every time.
	 */
	private int frameMarkingsExtensionId = -1;

	/**
	 * The {@link TransportCCEngine} instance, if any, for this {@link MediaStream}.
	 * The instance could be shared between more than one {@link MediaStream}, if
	 * they all use the same transport.
	 */
	private TransportCCEngine transportCCEngine;

	/**
	 * Initializes a new <tt>MediaStreamImpl</tt> instance which will use the
	 * specified <tt>MediaDevice</tt> for both capture and playback of media. The
	 * new instance will not have an associated <tt>StreamConnector</tt> and it must
	 * be set later for the new instance to be able to exchange media with a remote
	 * peer.
	 *
	 ...
	 *                    <tt>StreamConnector</tt> of the new instance is to not be
	 *                    set at initialization time but specified later on
	 * @param device      the <tt>MediaDevice</tt> the new instance is to use for
	 *                    both capture and playback of media exchanged via the
	 *                    specified <tt>StreamConnector</tt>
	 * @param srtpControl an existing control instance to control the ZRTP
	 *                    operations or <tt>null</tt> if a new control instance is
	 *                    to be created by the new <tt>MediaStreamImpl</tt>
	 */
	public MediaStreamImpl(StreamConnector connector, MediaDevice device, SrtpControl srtpControl) {
		if (device != null) {
			/*
			 * XXX Set the device early in order to make sure that it is of the right type
			 * because we do not support just about any MediaDevice yet.
			 */
			setDevice(device);
		}
	 ...
	 * payload type with the specified <tt>MediaFormat</tt> in order to allow it to
	 * report <tt>rtpPayloadType</tt> in RTP flows sending and receiving media in
	 * <tt>format</tt>. Usually, <tt>rtpPayloadType</tt> will be in the range of
	 * dynamic RTP payload types.
	 *
	 * @param rtpPayloadType the RTP payload type to be associated in this
	 *                       <tt>MediaStream</tt> with the specified
	 *                       <tt>MediaFormat</tt>
	...
	public void addDynamicRTPPayloadType(byte rtpPayloadType, MediaFormat format) {
		...
		if (Constants.RED.equals(encoding)) {
			...
			if (fecTransformEngineWrapper.getWrapped() != null) {
				...
				FECTransformEngine flexFecTransformEngine = new FECTransformEngine(
						FECTransformEngine.FecType.FLEXFEC_03, rtpPayloadType, rtpPayloadType, this);
	 ...
	 * set with {@link #addDynamicRTPPayloadType(byte, MediaFormat)}. This is
	 * necessary so that we can support the RFC3264 case where the answerer has the
	 * right to declare what payload type mappings it wants to receive RTP packets
	 * with even if they are different from those in the offer. RFC3264 claims this
	 * is for support of legacy protocols such as H.323 but we've been bumping with
	 * a number of cases where multi-component pure SIP systems also need to behave
	 * this way.
	 * <p>
	 *
	 ...
	 * @return <tt>true</tt> if <tt>receiveStreams</tt> changed as a result of the
	 ...
	 *
	 * @param remoteSourceID the SSRC identifier that this stream will be using in
	 ...
	 * attribute is set to <tt>INACTIVE</tt> the mapping is removed from the local
	 * extensions table and the extension would not be transmitted or handled by
	 * this stream's <tt>RTPConnector</tt>.
	 *
	...
	private void enableRTPExtension(byte extensionID, RTPExtension rtpExtension) {
		...
		} else if (RTPExtension.TRANSPORT_CC_URN.equals(uri) && transportCCEngine != null) {
	...
	public void close() {
		/*
		 * Some statistics cannot be taken from the RTP manager and have to be gathered
		 * from the ReceiveStream. We need to do this before calling stop().
		 */
		...
		if (rtpManager != null) {
			...
			try {
				 ...
				 * RTPManager#dispose() may throw an exception after a NullPointerException has
				 * been thrown by SendStream#close() as documented in
				 * #stopSendStreams(Iterable<SendStream>, boolean). It is unknown at the time of
				 * this writing whether we can do anything to prevent the exception here but it
				 * is clear that, if we let it go through, we will not release at least one
				 * capture device (i.e. we will at least skip the MediaDeviceSession#close()
				 * bellow). For example, if the exception is thrown for the audio stream in a
				 * call, its capture device will not be released and any video stream will not
				 * get its #close() method called at all.
				 */
		 ...
		 * StreamRTPManager#dispose(). Otherwise, the latter will try to send an RTCP
		 * BYE and there will be no targets to send it to.
		 */
	 ...
	 * <tt>RTPConnectorInputStream</tt> of an <tt>RTPManager</tt> to be used by this
	 * <tt>MediaStreamImpl</tt>. Allows extenders to override.
	 *
	 * @param dataInputStream the <tt>RTPConnectorInputStream</tt> to be used by an
	...
	/**
	 * Performs any optional configuration on a specific
	 * <tt>RTPConnectorOuputStream</tt> of an <tt>RTPManager</tt> to be used by this
	 * <tt>MediaStreamImpl</tt>. Allows extenders to override.
	 *
	 * @param dataOutputStream the <tt>RTPConnectorOutputStream</tt> to be used by
	 *                         an <tt>RTPManager</tt> of this
	 ...
	 * specified <tt>RTPManager</tt> which is to be used as the <tt>RTPManager</tt>
	 * of this <tt>MediaStreamImpl</tt>. Allows extenders to override.
	 *
	 ...
	 *
	 * @return a <tt>DtmfTransformEngine</tt> if this is an audio oriented stream
	...
	protected void createSendStreams() {
		...
		/*
		 * XXX We came up with a scenario in our testing in which G.722 would work fine
		 * for the first call since the start of the application and then it would fail
		 * for subsequent calls, JMF would complain that the G.722 RTP format is unknown
		 * to the RTPManager. Since RTPManager#createSendStream(DataSource, int) is one
		 * of the cases in which the formats registered with the RTPManager are
		 * necessary, register them (again) just before we use them.
		 */
		...
		for (int streamIndex = 0; streamIndex < streamCount; streamIndex++) {
			try {
				...
				if (logger.isTraceEnabled()) {
					logger.trace("Created SendStream with hashCode " + sendStream.hashCode() + " for "
							+ toString(dataSource) + " and streamIndex " + streamIndex + " in RTPManager with hashCode "
							+ rtpManager.hashCode());
				}
				...
				logger.error("Failed to create send stream for data source " + dataSource + " and stream index "
						+ streamIndex, ioe);
			} catch (UnsupportedFormatException ufe) {
				logger.error("Failed to create send stream for data source " + dataSource + " and stream index "
						+ streamIndex + " because of failed format " + ufe.getFailedFormat(), ufe);
			}
		}
		...
		if (logger.isTraceEnabled()) {
			...
			logger.trace("Total number of SendStreams in RTPManager with hashCode " + rtpManager.hashCode() + " is "
					+ sendStreamCount);
		}
	}
	...
	/**
	 * Creates the {@link RetransmissionRequesterImpl} for this {@code MediaStream}.
	 * 
	...
	/**
	 * Creates a chain of transform engines for use with this stream. Note that this
	 * is the only place where the <tt>TransformEngineChain</tt> is and should be
	 * manipulated to avoid problems with the order of the transformers.
	 *
	...
	private TransformEngineChain createTransformEngineChain() {
		...
		// SSRC audio levels
		/*
		 * It needs to go first in the reverse transform in order to be able to prevent
		 * RTP packets from a muted audio source from being decrypted.
		 */
	 ...
	 * uses for capture and playback of media has been changed. Allows extenders to
	 * override and provide additional processing of <tt>oldValue</tt> and
	 * <tt>newValue</tt>.
	 *
	 * @param oldValue the <tt>MediaDeviceSession</tt> with the <tt>MediaDevice</tt>
	 *                 this instance used work with
	 * @param newValue the <tt>MediaDeviceSession</tt> with the <tt>MediaDevice</tt>
	 ...
	 * <tt>MediaDeviceSession</tt> has changed. Recreates the <tt>SendStream</tt>s
	 * of this instance as necessary so that it, for example, continues streaming
	 * after the change if it was streaming before the change.
	 */
	 ...
	 * needs to include in RTP packets bound to its interlocutor. The method uses
	 * the list of SSRC identifiers currently handled by our device (possibly a
	 * mixer), then removes the SSRC ID of this stream's interlocutor. If this turns
	 * out to be the only SSRC currently in the list we set the list of local CSRC
	 * identifiers to null since this is obviously a non-conf call and we don't need
	 * to be advertising CSRC lists. If that's not the case, we also add our own
	 * SSRC to the list of IDs and cache the entire list.
	 *
	...
	/**
	 * Sets the target of this <tt>MediaStream</tt> to which it is to send and from
	 * which it is to receive data (e.g. RTP) and control data (e.g. RTCP). In
	 * contrast to {@link #setTarget(MediaStreamTarget)}, sets the specified
	 * <tt>target</tt> on this <tt>MediaStreamImpl</tt> even if its current
	 * <tt>target</tt> is equal to the specified one.
	 *
	 * @param target the <tt>MediaStreamTarget</tt> describing the data (e.g. RTP)
	...
	private void doSetTarget(MediaStreamTarget target) {
		...
		/*
		 * Invoke AbstractRTPConnector#removeTargets() if the new value does actually
		 * remove an RTP or RTCP target in comparison to the old value. If the new value
		 * is equal to the oldValue or adds an RTP or RTCP target (i.e. the old value
		 * does not specify the respective RTP or RTCP target and the new value does),
		 * then removeTargets is unnecessary and would've needlessly allowed a (tiny)
		 * interval of (execution) time (between removeTargets and addTarget) without a
		 * target.
		 */
		...
		if (target == null || newDataAddr == null || connector == null) {
			...
			try {
				...
				connector.addTarget(new SessionAddress(newDataAddr.getAddress(), newDataAddr.getPort(), controlInetAddr,
						controlPort));
				targetIsSet = true;
			} catch (IOException ioe) {
				targetIsSet = false;
		...
		if (targetIsSet) {
			...
			if (logger.isTraceEnabled()) {
				logger.trace("Set target of " + getClass().getSimpleName() + " with hashCode " + hashCode() + " to "
						+ target);
			}
		}
	}
	...
	/**
	 * Returns a map containing all currently active <tt>RTPExtension</tt>s in use
	 * by this stream.
	 *
	 * @return a map containing all currently active <tt>RTPExtension</tt>s in use
	 *         by this stream.
	 */
	@Override
	...
	/**
	 * Gets the <tt>MediaDevice</tt> that this stream uses to play back and capture
	 * media.
	 *
	...
	/**
	 * Gets the <tt>MediaDirection</tt> of the <tt>device</tt> of this instance. In
	 * case there is no device, {@link MediaDirection#SENDRECV} is assumed.
	 *
	 * @return the <tt>MediaDirection</tt> of the <tt>device</tt> of this instance
	 *         if any or <tt>MediaDirection.SENDRECV</tt>
	 */
	...
	/**
	 * Gets the direction in which this <tt>MediaStream</tt> is allowed to stream
	 * media.
	 *
	 ...
	 * <tt>encoding</tt> or <tt>-1</tt> if no payload type has been negotiated for
	 * it. If multiple formats match the specified <tt>encoding</tt>, then this
	 * method would return the first one it encounters while iterating through the
	 * map.
	 *
	 * @param encoding the encoding whose payload type we are trying to obtain.
	 *
	 * @return the payload type number that has been negotiated for the specified
	 *         <tt>encoding</tt> or <tt>-1</tt> if no payload type has been
	 *         negotiated for it.
	 */
	@Override
	...
	/**
	 * Gets the existing associations in this <tt>MediaStream</tt> of RTP payload
	 * types to <tt>MediaFormat</tt>s. The returned <tt>Map</tt> only contains
	 * associations previously added in this instance with
	 * {@link #addDynamicRTPPayloadType(byte, MediaFormat)} and not globally or
	 * well-known associations reported by {@link MediaFormat#getRTPPayloadType()}.
	 *
	 * @return a <tt>Map</tt> of RTP payload type expressed as <tt>Byte</tt> to
	 *         <tt>MediaFormat</tt> describing the existing (dynamic) associations
	 *         in this instance of RTP payload types to <tt>MediaFormat</tt>s. The
	 *         <tt>Map</tt> represents a snapshot of the existing associations at
	 *         the time of the <tt>getDynamicRTPPayloadTypes()</tt> method call and
	 ...
	 * 
	 * @return a TransformEngineWrapper around a FECTransformEngine. The wrapper is
	 *         necessary as we may not have created the {@link FECTransformEngine}
	 *         in time
	 */
	...
	/**
	 * Sets the {@link FECTransformEngine} for this {@link MediaStream} By default,
	 * nothing is done with the passed engine, allowing extenders to implement it
	 * 
	 ...
	 *
	 * @return the <tt>MediaFormat</tt> that this stream is currently transmitting
	 *         in
	 ...
	 * contribute to the media that this stream is sending toward its remote counter
	 * part. In other words, the method returns the list of CSRC IDs that this
	 * stream will include in outgoing RTP packets. This method will return an
	 * <tt>null</tt> in case this stream is not part of a mixed conference call.
	 *
	 ...
	 *         sending or an <tt>null</tt> in case this <tt>MediaStream</tt> is not
	 ...
	 *
	 * @return an <tt>InetSocketAddress</tt> instance indicating the local address
	 *         that this stream is sending RTCP traffic from.
	 */
	 ...
	 *
	 * @return an <tt>InetSocketAddress</tt> instance indicating the local address
	 *         that this stream is sending RTP traffic from.
	 */
	 ...
	 * implementations can override this and return different than current default
	 * value.
	 *
	...
	/**
	 * Gets a list of the <tt>ReceiveStream</tt>s this instance plays back on its
	 * associated <tt>MediaDevice</tt>.
	 *
	 * @return a list of the <tt>ReceiveStream</tt>s this instance plays back on its
	 *         associated <tt>MediaDevice</tt>
	 */
	public Collection<ReceiveStream> getReceiveStreams() {
		 ...
		 * ReceiveStreams in this instance which are not returned by the rtpManager.
		 */
	 ...
	 *
	 * @return an <tt>InetSocketAddress</tt> instance indicating the address that
	 ...
	 *
	 * @return an <tt>InetSocketAddress</tt> instance indicating the address that
	 *         this stream is sending RTP traffic to
	 ...
	 * {@inheritDoc}
	 *
	 * Returns the last element of {@link #getRemoteSourceIDs()} which may or may
	...
	public List<Long> getRemoteSourceIDs() {
		/*
		 * TODO Returning an unmodifiable view of remoteSourceIDs prevents modifications
		 * of private state from the outside but it does not prevent
	...
	/**
	 * Gets the <tt>RTPConnector</tt> through which this instance sends and receives
	...
	/**
	 * Gets the <tt>RTPManager</tt> instance which sends and receives RTP and RTCP
	 * traffic on behalf of this <tt>MediaStream</tt>. If the <tt>RTPManager</tt>
	...
	/**
	 * Returns the target of this <tt>MediaStream</tt> to which it is to send and
	 * from which it is to receive data (e.g. RTP) and control data (e.g. RTCP).
	 *
	 * @return the <tt>MediaStreamTarget</tt> describing the data (e.g. RTP) and the
	 *         control data (e.g. RTCP) locations to which this <tt>MediaStream</tt>
	 *         is to send and from which it is to receive
	 ...
	 *
	 * @return the transport protocol (UDP or TCP) used by the streams. null if the
	 *         stream connector is not instantiated.
	 */
	@Override
	 ...
	 * instead of the media being fed from its <tt>MediaDevice</tt>. "Silence" for
	 * video is understood as video data which is not the captured video data and
	 * may represent, for example, a black image.
	 *
	 ...
	 * <tt>MediaStream</tt> without {@link #stop()} or {@link #close()} afterwards.
	 *
	 ...
	 * parameters (i.e. SDP fmtp) specified by the remote peer and to be used for
	 * the playback of <tt>ReceiveStream</tt>s. The <tt>Format</tt>s in
	 * {@link #dynamicRTPPayloadTypes} will likely represent the view of the local
	 * peer while the <tt>Format</tt> set on this <tt>MediaStream</tt> instance will
	 * likely represent the view of the remote peer. The view of the remote peer
	 * matters for the playback of <tt>ReceiveStream</tt>s.
	 *
	 * @param rtpManager the <tt>StreamRTPManager</tt> to update the registered FMJ
	...
	private void maybeUpdateDynamicRTPPayloadTypes(StreamRTPManager rtpManager) {
		...
		for (Map.Entry<Byte, MediaFormat> dynamicRTPPayloadType : getDynamicRTPPayloadTypes().entrySet()) {
			...
			@SuppressWarnings("unchecked")
			MediaFormatImpl<? extends Format> dynamicMediaFormatImpl = (MediaFormatImpl<? extends Format>) dynamicMediaFormat;
	...
	private void printFlowStatistics(StreamRTPManager rtpManager) {
		try {
			...
			buff.append("call stats for outgoing ").append(mediaTypeStr).append(" stream SSRC: ")
					.append(getLocalSourceID()).append(eol).append("bytes sent: ").append(s.getBytesSent()).append(eol)
					.append("RTP sent: ").append(s.getRTPSent()).append(eol)
					.append("remote reported min interarrival jitter: ").append(mss.getMinUploadJitterMs()).append("ms")
					.append(eol).append("remote reported max interarrival jitter: ").append(mss.getMaxUploadJitterMs())
					.append("ms").append(eol).append("local collisions: ").append(s.getLocalColls()).append(eol)
					.append("remote collisions: ").append(s.getRemoteColls()).append(eol).append("RTCP sent: ")
					.append(s.getRTCPSent()).append(eol).append("transmit failed: ").append(s.getTransmitFailed());
			...
			buff.append("call stats for incoming ").append((format == null) ? "" : format).append(" stream SSRC: ")
					.append(getRemoteSourceID()).append(eol).append("packets received: ").append(rs.getPacketsRecd())
					.append(eol).append("bytes received: ").append(rs.getBytesRecd()).append(eol)
					.append("packets lost: ").append(mss.getReceiveStats().getPacketsLost()).append(eol)
					.append("min interarrival jitter: ").append(statisticsEngine.getMinInterArrivalJitter()).append(eol)
					.append("max interarrival jitter: ").append(statisticsEngine.getMaxInterArrivalJitter()).append(eol)
					.append("RTCPs received: ").append(rs.getRTCPRecd()).append(eol).append("bad RTCP packets: ")
					.append(rs.getBadRTCPPkts()).append(eol).append("bad RTP packets: ").append(rs.getBadRTPkts())
					.append(eol).append("local collisions: ").append(rs.getLocalColls()).append(eol)
					.append("malformed BYEs: ").append(rs.getMalformedBye()).append(eol).append("malformed RRs: ")
					.append(rs.getMalformedRR()).append(eol).append("malformed SDESs: ").append(rs.getMalformedSDES())
					.append(eol).append("malformed SRs: ").append(rs.getMalformedSR()).append(eol)
					.append("packets looped: ").append(rs.getPacketsLooped()).append(eol).append("remote collisions: ")
					.append(rs.getRemoteColls()).append(eol).append("SRs received: ").append(rs.getSRRecd()).append(eol)
					.append("transmit failed: ").append(rs.getTransmitFailed()).append(eol).append("unknown types: ")
					.append(rs.getUnknownTypes());
	 ...
	 * Gets the <tt>RTPManager</tt> instance which sends and receives RTP and RTCP
	 * traffic on behalf of this <tt>MediaStream</tt>. If the <tt>RTPManager</tt>
	 * does not exist yet, it is not created.
	 *
	 * @return the <tt>RTPManager</tt> instance which sends and receives RTP and
	 *         RTCP traffic on behalf of this <tt>MediaStream</tt>
	 */
	 ...
	 * <tt>RTPManager</tt>) as necessary. For example, if there was no attempt to
	 * create the <tt>SendStream</tt>s prior to the call, does nothing. If they were
	 * created prior to the call, closes them and creates them again. If they were
	 * not started prior to the call, does not start them after recreating them.
	 */
	 ...
	 * <tt>RTPManager</tt>. Extenders should override in order to register their own
	 * customizations and should call back to this super implementation during the
	 * execution of their override in order to register the associations defined in
	 * this instance of (dynamic) RTP payload types to <tt>MediaFormat</tt>s.
	 *
	...
	protected void rtpConnectorChanged(AbstractRTPConnector oldValue, AbstractRTPConnector newValue) {
		...
		/*
		 * TODO The following is a very ugly way to expose the RTPConnector created by
		 * this instance so it may be configured from outside the class hierarchy.
		 * That's why the property in use bellow is not defined as a well-known constant
		 * and is to be considered internal and likely to be removed in a future
		 * revision.
		 */
		try {
	 ...
	 *
	 * @param inputStream the new <tt>RTPConnectorInputStream</tt> instance created
	 *                    by the <tt>rtpConnector</tt> of this instance
	...
	private void rtpConnectorInputStreamCreated(RTPConnectorInputStream<?> inputStream, boolean data) {
		/*
		 * TODO The following is a very ugly way to expose the RTPConnectorInputStreams
		 * created by the rtpConnector of this instance so they may be configured from
		 * outside the class hierarchy (e.g. to invoke addDatagramPacketFilter). That's
		 * why the property in use bellow is not defined as a well-known constant and is
		 * to be considered internal and likely to be removed in a future revision.
		 */
		try {
			firePropertyChange(
					MediaStreamImpl.class.getName() + ".rtpConnector." + (data ? "data" : "control") + "InputStream",
					null, inputStream);
		} catch (Throwable t) {
			if (t instanceof InterruptedException)
				Thread.currentThread().interrupt();
	...
	/**
	 * Sets the <tt>StreamConnector</tt> to be used by this instance for sending and
	 * receiving media.
	 *
	 * @param connector the <tt>StreamConnector</tt> to be used by this instance for
	...
	/**
	 * Sets the <tt>MediaDevice</tt> that this stream should use to play back and
	 * capture media.
	 * <p>
	 ...
	 * </p>
	 *
	 * @param device the <tt>MediaDevice</tt> that this stream should use to play
	...
	public void setDevice(MediaDevice device) {
		...
		if ((deviceSession == null) || (deviceSession.getDevice() != device)) {
			 ...
			 * MediaDeviceSession in order to prevent the recreation of the playback of the
			 * ReceiveStream(s) when just changing the MediaDevice of this MediaSteam.
			 */
			if (oldValue != null)
			...
			/*
			 * Setting a new device resets any previously-set direction. Otherwise, we risk
			 * not being able to set a new device if it is mandatory for the new device to
			 * fully cover any previously-set direction.
			 */
			direction = null;

			if (deviceSession != null) {
				if (format != null) {
					logger.debug(getClass().getName() + " " + hashCode() + " is about to set " + format + " in "
							+ deviceSession.getClass().getName() + " " + deviceSession.hashCode());
	 ...
	 * <tt>direction</tt>. If it is currently started in a direction different than
	 * the specified, directions other than the specified will be stopped.
	 *
	...
	/**
	 * Sets the <tt>MediaFormat</tt> that this <tt>MediaStream</tt> should transmit
	 * in.
	 *
	 * @param format the <tt>MediaFormat</tt> that this <tt>MediaStream</tt> should
	 *               transmit in
	...
	public void setFormat(MediaFormat format) {
		...
		if (devSess != null) {
			...
			if ((thisFormat != null) && thisFormat.equals(format) && thisFormat
					.advancedAttributesAreEqual(thisFormat.getAdvancedAttributes(), format.getAdvancedAttributes())) {
				return;
			}
		}
	 ...
	 * <tt>PropertyChangeEvent</tt>.
	 *
	 * @param localSourceID the SSRC identifier that this stream will be using in
	 *                      outgoing RTP packets from now on
	 */
	protected void setLocalSourceID(long localSourceID) {
		if (this.localSourceID != localSourceID) {
			...
			/*
			 * If ZRTP is used, then let it know about the SSRC of the new SendStream.
	 ...
	 *             "silence" instead of the actual media data that it captures from
	 *             its <tt>MediaDevice</tt>; <tt>false</tt> to transmit actual media
	 *             data captured from the <tt>MediaDevice</tt> of this
	 *             <tt>MediaStream</tt>
	...
	/**
	 * Sets the target of this <tt>MediaStream</tt> to which it is to send and from
	 * which it is to receive data (e.g. RTP) and control data (e.g. RTCP).
	 *
	 * @param target the <tt>MediaStreamTarget</tt> describing the data (e.g. RTP)
	 *               and the control data (e.g. RTCP) locations to which this
	 *               <tt>MediaStream</tt> is to send and from which it is to receive
	 ...
	 * streaming it through the local <tt>StreamConnector</tt> toward the stream's
	 * target address and port. Also puts the <tt>MediaStream</tt> in a listening
	 * state which make it play all media received from the <tt>StreamConnector</tt>
	 * on the stream's <tt>MediaDevice</tt>.
	 *
	 ...
	 *                  direction of the processing of media to be started. For
	 *                  example, {@link MediaDirection#SENDRECV} to start both
	 *                  capture and playback of media in this instance or
	...
	private void start(MediaDirection direction) {
		...
		if (direction.allowsSending() && ((startedDirection == null) || !startedDirection.allowsSending())) {
			/*
			 * The startSendStreams method will be called so the getRTPManager method will
			 * be called as part of the execution of the former.
			 */
			...
			if (logger.isInfoEnabled()) {
				...
				logger.info(
						mediaType + " codec/freq: " + stats.getEncoding() + "/" + stats.getEncodingClockRate() + " Hz");
		...
		if (direction.allowsReceiving() && ((startedDirection == null) || !startedDirection.allowsReceiving())) {
			/*
			 * The startReceiveStreams method will be called so the getRTPManager method
			 * will be called as part of the execution of the former.
			 */
			getRTPManagerForRTPTranslator = false;
	...
	/**
	 * Starts the <tt>ReceiveStream</tt>s that this instance is receiving from its
	 * remote peer. By design, a <tt>MediaStream</tt> instance is associated with a
	 * single <tt>ReceiveStream</tt> at a time. However, the <tt>ReceiveStream</tt>s
	 * are created by <tt>RTPManager</tt> and it tracks multiple
	 * <tt>ReceiveStream</tt>s. In practice, the <tt>RTPManager</tt> of this
	 * <tt>MediaStreamImpl</tt> will have a single <tt>ReceiveStream</tt> in its
	 * list.
	 */
	private void startReceiveStreams() {
		/*
		 * The ReceiveStreams originate from RtpManager, make sure that there is an
		 * actual RTPManager to initialize ReceiveStreams which are then to be started.
		 */
		getRTPManager();
		...
		for (ReceiveStream receiveStream : getReceiveStreams()) {
			try {
				...
				/*
				 * For an unknown reason, the stream DataSource can be null at the end of the
				 * Call after re-INVITEs have been handled.
				 */
				if (receiveStreamDataSource != null) {
					logger.debug(
							"Starting receive stream data source: " + receiveStreamDataSource.getClass().getName());
	...
	private void startSendStreams() {
		/*
		 * Until it's clear that the SendStreams are required (i.e. we've negotiated to
		 * send), they will not be created. Otherwise, their creation isn't only
		 * illogical but also causes the CaptureDevice to be used.
		 */
		if (!sendStreamsAreCreated)
			createSendStreams();
		...
		if (sendStreams != null) {
			for (SendStream sendStream : sendStreams) {
				try {
					...
					sendStream.start();
					logger.debug("Starting send stream Data source: " + sendStreamDataSource.getClass().getName());
	...
	/**
	 * Stops all streaming and capturing in this <tt>MediaStream</tt> and closes and
	 * releases all open/allocated devices/resources. Has no effect if this
	 ...
	 *
	 * @param direction a <tt>MediaDirection</tt> value which represents the
	 *                  direction of the processing of media to be stopped. For
	 *                  example, {@link MediaDirection#SENDRECV} to stop both
	 *                  capture and playback of media in this instance or
	...
	private void stop(MediaDirection direction) {
		...
		if ((MediaDirection.SENDRECV.equals(direction) || MediaDirection.SENDONLY.equals(direction))
				&& (MediaDirection.SENDRECV.equals(startedDirection)
						|| MediaDirection.SENDONLY.equals(startedDirection))) {
			/*
			 * XXX It is not very clear at the time of this writing whether the SendStreams
			 * are to be stopped or closed. On one hand, stopping a direction may be a
			 * temporary transition which relies on retaining the SSRC. On the other hand,
			 * it may be permanent. In which case, the respective ReceiveStream on the
			 * remote peer will timeout at some point in time. In the context of video
			 * conferences, when a member stops the streaming of their video without leaving
			 * the conference, they will stop their SendStreams. However, the other members
			 * will need respective BYE RTCP packets in order to know that they are to
			 * remove the associated ReceiveStreams from display. The initial version of the
			 * code here used to stop the SendStreams without closing them but, given the
			 * considerations above, it is being changed to close them in the case of video.
			 */
	...
	/**
	 * Stops the <tt>ReceiveStream</tt>s that this instance is receiving from its
	 * remote peer. By design, a <tt>MediaStream</tt> instance is associated with a
	 * single <tt>ReceiveStream</tt> at a time. However, the <tt>ReceiveStream</tt>s
	 * are created by <tt>RTPManager</tt> and it tracks multiple
	 * <tt>ReceiveStream</tt>s. In practice, the <tt>RTPManager</tt> of this
	...
	private void stopReceiveStreams() {
		for (ReceiveStream receiveStream : getReceiveStreams()) {
			try {
				...
				if (receiveStreamDataSource != null) {
					logger.debug(
							"Stopping receive stream data source: " + receiveStreamDataSource.getClass().getName());
	...
	/**
	 * Stops the <tt>SendStream</tt>s that this instance is sending to its remote
	 * peer and optionally closes them.
	 *
	...
	private Iterable<SendStream> stopSendStreams(Iterable<SendStream> sendStreams, boolean close) {
		...
		for (SendStream sendStream : sendStreams) {
			try {
				...
				logger.debug("Stopping send stream data source: " + sendStream.getClass().getName());
				...
				if (close) {
					try {
						 ...
						 * NullPointerException but it does not seem to be guaranteed because it does
						 * not happen while debugging and stopping at a breakpoint on
						 * SendStream#close(). One of the cases in which it appears upon call hang-up is
						 * if we do not close the "old" SendStreams upon reinvite(s). Though we are now
						 * closing such SendStreams, ignore the exception here just in case because we
						 * already ignore IOExceptions.
						 */
	...
	/**
	 * Notifies this <tt>ReceiveStreamListener</tt> that the <tt>RTPManager</tt> it
	 * is registered with has generated an event related to a
	 * <tt>ReceiveStream</tt>.
	 *
	 * @param ev the <tt>ReceiveStreamEvent</tt> which specifies the
	 *           <tt>ReceiveStream</tt> that is the cause of the event and the very
	 *           type of the event
...
//                restartZrtpControl();
	...
	public void update(RemoteEvent ev) {
		if (ev instanceof SenderReportEvent || ev instanceof ReceiverReportEvent) {
			...
			/*
			 * The level of logger used here is in accord with the level of logger used in
			 * StatisticsEngine where sent reports are logged.
			 */
			if (logger.isTraceEnabled()) {
				...
				buff.append("Received a ").append(senderReport ? "sender" : "receiver").append(" report for ")
						.append(mediaTypeStr).append(" stream SSRC:").append(getLocalSourceID()).append(" [");
				if (senderReport) {
					buff.append("packet count:").append(((SenderReport) report).getSenderPacketCount())
							.append(", bytes:").append(((SenderReport) report).getSenderByteCount());
				}

				if (feedback != null) {
					buff.append(", interarrival jitter:").append(remoteJitter).append(", lost packets:")
							.append(feedback.getNumLost()).append(", time since previous report:")
							.append((int) (feedback.getDLSR() / 65.536)).append("ms");
				}
				buff.append(" ]");
				logger.trace(buff);
			}
	...
	/**
	 * Notifies this <tt>SendStreamListener</tt> that the <tt>RTPManager</tt> it is
	 * registered with has generated an event related to a <tt>SendStream</tt>.
	 *
	 * @param ev the <tt>SendStreamEvent</tt> which specifies the
	 *           <tt>SendStream</tt> that is the cause of the event and the very
	 *           type of the event
	 ...
	 *
	 * @param ev the <tt>SessionEvent</tt> which specifies the source and the very
	 *           type of the event
	...
	public void injectPacket(RawPacket pkt, boolean data, TransformEngine after) throws TransmissionFailedException {
		try {
			...
			RTPConnectorOutputStream outputStream = data ? rtpConnector.getDataOutputStream(false)
	 ...
	 *         {@link VideoMediaStreamImpl}, but I don't want to be obliged to cast
	 *         to use this method.
	 */
	public int getTemporalID(RawPacket pkt) {
		...
		if (redBlock.getPayloadType() == vp8PT) {
			return org.jitsi.impl.neomedia.codec.video.vp8.DePacketizer.VP8PayloadDescriptor
					.getTemporalLayerIndex(redBlock.getBuffer(), redBlock.getOffset(), redBlock.getLength());
		} else if (redBlock.getPayloadType() == vp9PT) {
			return org.jitsi.impl.neomedia.codec.video.vp9.DePacketizer.VP9PayloadDescriptor
					.getTemporalLayerIndex(redBlock.getBuffer(), redBlock.getOffset(), redBlock.getLength());
		} else {
	...
	/**
	 * Returns a boolean that indicates whether or not our we're able to detect the
	 * frame boundaries for the codec of the packet that is specified as an
	 * argument.
	 *
	 * @param pkt the {@link RawPacket} that holds the RTP packet.
	 *
	 * @return true if we're able to detect the frame boundaries for the codec of
	 *         the packet that is specified as an argument, false otherwise.
	 */
	...
	public boolean isStartOfFrame(RawPacket pkt) {
		...
		if (redBlock.getPayloadType() == vp8PT) {
			...
			return org.jitsi.impl.neomedia.codec.video.vp9.DePacketizer.VP9PayloadDescriptor
					.isStartOfFrame(redBlock.getBuffer(), redBlock.getOffset(), redBlock.getLength());
		} else {
			return false;
		}
	}
	...
	public boolean isEndOfFrame(RawPacket pkt) {
		...
		if (redBlock.getPayloadType() == vp9PT) {
			return org.jitsi.impl.neomedia.codec.video.vp9.DePacketizer.VP9PayloadDescriptor
					.isEndOfFrame(redBlock.getBuffer(), redBlock.getOffset(), redBlock.getLength());
		} else {
	 ...
	 * {@inheritDoc}
	 * </p>
	 * This is absolutely terrible, but we need a RawPacket and the method is used
	 * from RTPTranslator, which doesn't work with RawPacket.
	 */
	@Override
	...
	public boolean isKeyFrame(RawPacket pkt) {
		...
		final byte vp8PT = getDynamicRTPPayloadType(Constants.VP8), h264PT = getDynamicRTPPayloadType(Constants.H264);

		if (redBlock.getPayloadType() == vp8PT) {
			return org.jitsi.impl.neomedia.codec.video.vp8.DePacketizer.isKeyFrame(redBlock.getBuffer(),
					redBlock.getOffset(), redBlock.getLength());
		} else if (redBlock.getPayloadType() == h264PT) {
			return org.jitsi.impl.neomedia.codec.video.h264.DePacketizer.isKeyFrame(redBlock.getBuffer(),
					redBlock.getOffset(), redBlock.getLength());
		} else {
			return false;
		}
	...
	/**
	 * {@inheritDoc} <br/>
	 * Note that the chain is only initialized when a {@link StreamConnector} is set
	 * for the {@link MediaStreamImpl} via {@link #setConnector(StreamConnector)} or
	 * by passing a non-null connector to the constructor. Until the chain is
	 * initialized, this method will return null.
	 */
	@Override
	...
	/**
	 * Gets the {@link REDBlock} that contains the payload of the packet passed in
	 * as a parameter.
	 *
	 * @param pkt the packet from which we want to get the primary RED block
	 * @return the {@link REDBlock} that contains the payload of the packet passed
	 *         in as a parameter, or null if the buffer is invalid.
	 */
	@Override
	...
	/**
	 * Creates the {@link DiscardTransformEngine} for this stream. Allows extenders
	 * to override.
	 */
	 ...
	 *
	 * @return the <tt>RemoteBitrateEstimator</tt> of this <tt>VideoMediaStream</tt>
