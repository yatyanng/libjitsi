...
class InSampleDesc {
	...
	/**
	 * The time stamp of <tt>inSamples</tt> to be reported in the <tt>Buffer</tt>s
	 * of the <tt>AudioMixingPushBufferStream</tt>s when mixes are read from them.
	 */
	 ...
	 *
	 * @param inSamples the set of audio samples read from <tt>inStreams</tt>
	 * @param inStreams the set of input streams from which <tt>inSamples</tt> were
	 *                  read
	...
	/**
	 * Gets the <tt>Buffer</tt> into which media data is to be read from the input
	 * streams associated with this instance.
	 *
	 ...
	 * <tt>Buffer</tt>s of the <tt>AudioMixingPushBufferStream</tt>s when mixes are
	 * read from them.
	 *
	...
	/**
	 * Sets the <tt>Buffer</tt> into which media data is to be read from the input
	 * streams associated with this instance.
	 *
	 * @param buffer the <tt>Buffer</tt> into which media data is to be read from
	 ...
	 * read from them.
	 *
	 * @param timeStamp the time stamp of <tt>inSamples</tt> to be reported in the
	 *                  <tt>Buffer</tt>s of the
	...
	public void setTimeStamp(long timeStamp) {
		if (this.timeStamp == Buffer.TIME_UNKNOWN) {
			...
			/*
			 * Setting the timeStamp more than once does not make sense because the
			 * inStreams will report different timeStamps so only one should be picked up
			 * where the very reading from inStreams takes place.
			 */
