...
public class AudioMixer {
	 ...
	 */
	private static final AudioFormat DEFAULT_OUTPUT_FORMAT = new AudioFormat(AudioFormat.LINEAR, 8000, 16, 1,
			AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED);

	/**
	 * The <tt>Logger</tt> used by the <tt>AudioMixer</tt> class and its instances
	 * for logging output.
	 */
	 ...
	 * stream data.
	 *
	 * @param dataSource the <tt>DataSource</tt> for which the <tt>Format</tt> in
	 *                   which it provides stream data is to be determined
	...
	/**
	 * Gets the <tt>Format</tt> in which a specific <tt>SourceStream</tt> provides
	 * data.
	 *
	 ...
	 * <tt>Manager.createMergingDataSource(DataSource[])</tt> requires the interface
	 * implementation for audio if it is implemented for video and it is indeed the
	 * case for our use case of <tt>AudioMixingPushBufferDataSource</tt>.
	 */
	protected final CaptureDevice captureDevice;

	/**
	 * The number of output <tt>AudioMixingPushBufferDataSource</tt>s reading from
	 * this <tt>AudioMixer</tt> which are connected. When the value is greater than
	 * zero, this <tt>AudioMixer</tt> is connected to the input <tt>DataSource</tt>s
	 * it manages.
	 */
	private int connected;

	/**
	 * The collection of input <tt>DataSource</tt>s this instance reads audio data
	 * from.
	 */
	 ...
	 * <tt>inDataSources</tt> excluding <tt>captureDevice</tt> and is thus meant for
	 * playback on the local peer in a call.
	 */
	private final AudioMixingPushBufferDataSource localOutDataSource;

	/**
	 * The output <tt>AudioMixerPushBufferStream</tt> through which this instance
	 * pushes audio sample data to <tt>AudioMixingPushBufferStream</tt>s to be
	 * mixed.
	 */
	...
	/**
	 * The number of output <tt>AudioMixingPushBufferDataSource</tt>s reading from
	 * this <tt>AudioMixer</tt> which are started. When the value is greater than
	 * zero, this <tt>AudioMixer</tt> is started and so are the input
	 ...
	 * implementation of <tt>CaptureDevice</tt> for audio if it is implemented for
	 * video and it is indeed the cause for our use case of
	 ...
	 * <tt>CaptureDevice</tt> is also added as the first input <tt>DataSource</tt>
	 * of the new instance.
	 *
	 * @param captureDevice the <tt>CaptureDevice</tt> capabilities to be provided
	 *                      to the <tt>AudioMixingPushBufferDataSource</tt>s created
	 *                      by the new instance and its first input
	 *                      <tt>DataSource</tt>
	 */
	public AudioMixer(CaptureDevice captureDevice) {
		/*
		 * AudioMixer provides PushBufferDataSources so it needs a way to push them. It
		 * does the pushing by using the pushes of its CaptureDevice i.e. it has to be a
		...
		if (logger.isTraceEnabled()) {
			logger.debug("Creating a traceable capture device by using " + captureDevice.getClass().getName() + " "
	 ...
	 * <tt>DataSource</tt>s from which this instance reads audio. If the specified
	 * <tt>DataSource</tt> indeed provides audio, the respective contributions to
	 * the mix are always included.
	 *
	 ...
	 * <tt>DataSource</tt>s from which this instance reads audio. If the specified
	 * <tt>DataSource</tt> indeed provides audio, the respective contributions to
	 * the mix will be excluded from the mix output provided through a specific
	 * <tt>AudioMixingPushBufferDataSource</tt>.
	 *
	 * @param inDataSource  a new <tt>DataSource</tt> to input audio to this
	 *                      instance
	 * @param outDataSource the <tt>AudioMixingPushBufferDataSource</tt> to not
	 *                      include the audio contributions of <tt>inDataSource</tt>
	 *                      in the mix it outputs
	 */
	void addInDataSource(DataSource inDataSource, AudioMixingPushBufferDataSource outDataSource) {
		...
		logger.trace("Adding input DataSource with hashCode " + inDataSource.hashCode() + " to AudioMixer "
				+ this.hashCode());

		boolean added = false;
		InDataSourceDesc addedInDataSourceDesc = null;
		synchronized (inDataSources) {
			...
			addedInDataSourceDesc = new InDataSourceDesc(inDataSource, outDataSource);
			added = inDataSources.add(addedInDataSourceDesc);
		}
		if (added && addedInDataSourceDesc != null) {
			logger.trace("Added input DataSource with hashCode " + inDataSource.hashCode() + " to AudioMixer "
					+ this.hashCode());
			/*
			...
			if (connected > 0) {
				try {
					addedInDataSourceDesc.connect(this);
	 ...
	 * <tt>AudioMixingPushBufferDataSource</tt> reading from it has been connected.
	 * The first of the many <tt>AudioMixingPushBufferDataSource</tt>s reading from
	 * this <tt>AudioMixer</tt> which gets connected causes it to connect to the
	...
	void connect() throws IOException {
		synchronized (inDataSources) {
			if (connected == 0) {
				...
				/*
				 * Since the media of the input streams is to be mixed, their bufferLengths have
				 * to be equal. After a DataSource is connected, its BufferControl is available
				 * and its bufferLength may change so make sure that the bufferLengths of the
				 * input streams are equal.
				 */
				if (outStream != null)
	 ...
	 * because of a specific <tt>DataSource</tt> added as an input to this instance
	 * i.e. it may be an actual input <tt>DataSource</tt> added to this instance or
	 * a <tt>DataSource</tt> transcoding an input <tt>DataSource</tt> added to this
	 * instance.
	 *
	...
	/**
	 * Notifies this <tt>AudioMixer</tt> that a specific input <tt>DataSource</tt>
	 * has finished its connecting procedure. Primarily meant for input
	 * <tt>DataSource</tt> which have their connecting executed in a separate thread
	 * as are, for example, input <tt>DataSource</tt>s which are being transcoded.
	 *
	...
	/**
	 * Creates a new <tt>InStreamDesc</tt> instance which is to describe a specific
	 * input <tt>SourceStream</tt> originating from a specific input
	 * <tt>DataSource</tt> given by its <tt>InDataSourceDesc</tt>.
	 *
	 * @param inStream         the input <tt>SourceStream</tt> to be described by
	 *                         the new instance
	 * @param inDataSourceDesc the input <tt>DataSource</tt> given by its
	 *                         <tt>InDataSourceDesc</tt> to be described by the new
	 *                         instance
	 * @return a new <tt>InStreamDesc</tt> instance which describes the specified
	 *         input <tt>SourceStream</tt> and <tt>DataSource</tt>
	 */
	...
	/**
	 * Creates a new <tt>AudioMixingPushBufferDataSource</tt> which gives access to
	 * a single audio stream representing the mix of the audio streams input into
	 * this <tt>AudioMixer</tt> through its input <tt>DataSource</tt>s. The returned
	 * <tt>AudioMixingPushBufferDataSource</tt> can also be used to include new
	 * input <tt>DataSources</tt> in this <tt>AudioMixer</tt> but have their
	 * contributions not included in the mix available through the returned
	 * <tt>AudioMixingPushBufferDataSource</tt>.
	 *
	 * @return a new <tt>AudioMixingPushBufferDataSource</tt> which gives access to
	 *         a single audio stream representing the mix of the audio streams input
	 ...
	 *
	 * @param inDataSourceDesc the <tt>InDataSourceDesc</tt> describing the input
	 *                         <tt>DataSource</tt> to be transcoded into the
	 *                         specified output <tt>Format</tt> and to receive the
	 *                         transcoding <tt>DataSource</tt>
	 ...
	 * disconnected. The last of the many <tt>AudioMixingPushBufferDataSource</tt>s
	 * reading from this <tt>AudioMixer</tt> which gets disconnected causes it to
	 * disconnect from the input <tt>DataSource</tt>s it manages.
	 */
	void disconnect() {
		synchronized (inDataSources) {
			if (connected <= 0)
			...
			if (connected == 0) {
				...
				/*
				 * XXX Make the outStream to release the inStreams. Otherwise, the
				 * PushBufferStream ones which have been wrapped into CachingPushBufferStream
				 * may remain waiting.
				 */
	 ...
	 *         <tt>AudioMixingPushBufferDataSource</tt>s if such a control is
	 ...
	 * <tt>InStreamDesc</tt>s which describes a specific <tt>SourceStream</tt>. If
	 * such an <tt>InStreamDesc</tt> does not exist, returns <tt>null</tt>.
	 *
	 * @param inStream              the <tt>SourceStream</tt> to locate an
	 *                              <tt>InStreamDesc</tt> for in
	 *                              <tt>existingInStreamDescs</tt>
	 * @param existingInStreamDescs the list of existing <tt>InStreamDesc</tt>s in
	 *                              which an <tt>InStreamDesc</tt> for
	 *                              <tt>inStream</tt> is to be located
	 * @return an <tt>InStreamDesc</tt> from <tt>existingInStreamDescs</tt> which
	 *         describes <tt>inStream</tt> if such an <tt>InStreamDesc</tt> exists;
	 ...
	 * <tt>AudioMixingPushBufferDataSource</tt>s.
	 *
	 * @return an array of <tt>FormatControl</tt>s for the <tt>CaptureDevice</tt>
	 *         this <tt>AudioMixer</tt> provides through its output
	 *         <tt>AudioMixingPushBufferDataSource</tt>s
	 */
	FormatControl[] getFormatControls() {
		/*
		 * Setting the format of the captureDevice once we've started using it is likely
		 * to wreak havoc so disable it.
		 */
	...
	/**
	 * Gets the <tt>SourceStream</tt>s (in the form of <tt>InStreamDesc</tt>) of a
	 * specific <tt>DataSource</tt> (provided in the form of
	 ...
	 *                          <tt>InDataSourceDesc</tt>) which is to be examined
	 *                          for <tt>SourceStreams</tt> producing data in the
	 *                          specified <tt>AudioFormat</tt>
	 * @param outFormat         the <tt>AudioFormat</tt> in which the collected
	 *                          <tt>SourceStream</tt>s are to produce data
	 * @param existingInStreams the <tt>InStreamDesc</tt> instances which already
	 *                          exist and which are used to avoid creating multiple
	 *                          <tt>InStreamDesc</tt>s for input
	 *                          <tt>SourceStream</tt>s which already have ones
	 * @param inStreams         the <tt>List</tt> of <tt>InStreamDesc</tt> in which
	 *                          the discovered <tt>SourceStream</tt>s are to be
	 *                          returned
	 ...
	 */
	private boolean getInStreamsFromInDataSource(InDataSourceDesc inDataSourceDesc, AudioFormat outFormat,
			InStreamDesc[] existingInStreams, List<InStreamDesc> inStreams) {
		...
		if ((inFormat != null) && !matches(inFormat, outFormat)) {
			if (inDataSource instanceof PushDataSource) {
				for (PushSourceStream inStream : ((PushDataSource) inDataSource).getStreams()) {
					...
					if (inStreamDesc == null)
						inStreamDesc = createInStreamDesc(new PushBufferStreamAdapter(inStream, inFormat),
								inDataSourceDesc);
			...
			if (inDataSource instanceof PullDataSource) {
				for (PullSourceStream inStream : ((PullDataSource) inDataSource).getStreams()) {
					...
					if (inStreamDesc == null)
						inStreamDesc = createInStreamDesc(new PullBufferStreamAdapter(inStream, inFormat),
								inDataSourceDesc);
					inStreams.add(inStreamDesc);
				}
				return true;
	 ...
	 * <tt>DataSource</tt>s from which this <tt>AudioMixer</tt> reads data which
	 * produce data in a specific <tt>AudioFormat</tt>. When an input
	 * <tt>DataSource</tt> does not have such <tt>SourceStream</tt>s, an attempt is
	 * made to transcode its tracks so that such <tt>SourceStream</tt>s can be
	 * retrieved from it after transcoding.
	 *
	 * @param outFormat         the <tt>AudioFormat</tt> in which the retrieved
	 *                          <tt>SourceStream</tt>s are to produce data
	 * @param existingInStreams the <tt>SourceStream</tt>s which are already known
	 *                          to this <tt>AudioMixer</tt>
	 ...
	 *                     <tt>SourceStream</tt>s from the input
	 *                     <tt>DataSource</tt>s
	 */
	private Collection<InStreamDesc> getInStreamsFromInDataSources(AudioFormat outFormat,
			InStreamDesc[] existingInStreams) throws IOException {
	...
	/**
	 * Gets the <tt>AudioMixingPushBufferDataSource</tt> containing the mix of all
	 * input <tt>DataSource</tt>s excluding the <tt>CaptureDevice</tt> of this
	 * <tt>AudioMixer</tt> and is thus meant for playback on the local peer in a
	 * call.
	 *
	 * @return the <tt>AudioMixingPushBufferDataSource</tt> containing the mix of
	 *         all input <tt>DataSource</tt>s excluding the <tt>CaptureDevice</tt>
	 *         of this <tt>AudioMixer</tt> and is thus meant for playback on the
	 *         local peer in a call
	 */
	 ...
	 * <tt>AudioMixer</tt> can produce data and which is to be the output
	 * <tt>Format</tt> of this <tt>AudioMixer</tt>.
	 *
	 * @return the <tt>AudioFormat</tt> in which the input <tt>DataSource</tt>s of
	 *         this <tt>AudioMixer</tt> can produce data and which is to be the
	...
	/**
	 * Gets the <tt>AudioMixerPushBufferStream</tt>, first creating it if it does
	 * not exist already, which reads data from the input <tt>DataSource</tt>s of
	 * this <tt>AudioMixer</tt> and pushes it to output
	 ...
	 *         input <tt>DataSource</tt>s of this <tt>AudioMixer</tt> and pushes it
	...
	AudioMixerPushBufferStream getOutStream() {
		synchronized (inDataSources) {
			...
			try {
				inStreams = getInStreamsFromInDataSources(outFormat,
	 ...
	 * <tt>MediaStream</tt> needs access to the codec chain used to playback one of
	 * it's <tt>ReceiveStream</tt>s.
	 *
	 ...
	 * handle varying characteristics of a certain output <tt>Format</tt>, the only
	 * requirement for the specified <tt>Format</tt>s to match is for both of them
	 * to have one and the same encoding.
	 *
	 * @param input   the <tt>Format</tt> for which it is required to determine
	 *                whether it matches a specific <tt>Format</tt>
	 * @param pattern the <tt>Format</tt> against which the specified <tt>input</tt>
	 *                is to be matched
	 ...
	 * specific <tt>DataSource</tt> into a specific output <tt>Buffer</tt>. Allows
	 * extenders to tap into the reading and monitor and customize it.
	 *
	...
	/**
	 * Removes <tt>DataSource</tt>s accepted by a specific <tt>DataSourceFilter</tt>
	 * from the list of input <tt>DataSource</tt>s of this <tt>AudioMixer</tt> from
	 * which it reads audio to be mixed.
	 *
	 * @param dataSourceFilter the <tt>DataSourceFilter</tt> which selects the
	 *                         <tt>DataSource</tt>s to be removed from the list of
	 *                         input <tt>DataSource</tt>s of this
	 *                         <tt>AudioMixer</tt> from which it reads audio to be
	 *                         mixed
	 */
	...
	/**
	 * Sets a specific <tt>AudioFormat</tt>, if possible, as the output format of
	 * the input <tt>DataSource</tt>s of this <tt>AudioMixer</tt> in an attempt to
	 * not have to perform explicit transcoding of the input <tt>SourceStream</tt>s.
	 *
				...
				if (formatControl != null) {
					...
					if ((inFormat == null) || !matches(inFormat, outFormat)) {
						...
						if (setFormat == null)
							...
							logger.warn("Failed to change format of inDataSource" + " from " + setFormat + " to "
									+ outFormat);
	 ...
	 *
	 * @param outStream  the <tt>AudioMixerPushBufferStream</tt> which requests this
	 *                   <tt>AudioMixer</tt> to start. If <tt>outStream</tt> is the
	 *                   current one and only <tt>AudioMixerPushBufferStream</tt> of
	 *                   this <tt>AudioMixer</tt>, this <tt>AudioMixer</tt> starts
	 *                   if it hasn't started yet. Otherwise, the request is
	 *                   ignored.
	 * @param generation a value generated by <tt>outStream</tt> indicating the
	 *                   order of the invocations of the <tt>start</tt> and
	 *                   <tt>stop</tt> methods performed by <tt>outStream</tt>
	 *                   allowing it to execute the said methods outside
	 *                   synchronized blocks for the purposes of reducing deadlock
	 *                   risks
	 * @throws IOException if any of the input <tt>DataSource</tt>s of this
	 *                     <tt>AudioMixer</tt> throws such an exception while
	 *                     attempting to start it
	 */
	void start(AudioMixerPushBufferStream outStream, long generation) throws IOException {
		synchronized (inDataSources) {
			/*
			 * AudioMixer has only one outStream at a time and only its current outStream
			 * knows when it has to start (and stop).
			 */
			if (this.outStream != outStream)
				return;
			/*
			 * The notion of generations was introduced in order to allow outStream to
			 * invoke the start and stop methods outside synchronized blocks. The generation
			 * value always increases in a synchronized block.
			 */
	...
	void stop(AudioMixerPushBufferStream outStream, long generation) throws IOException {
		synchronized (inDataSources) {
			/*
			 * AudioMixer has only one outStream at a time and only its current outStream
			 * knows when it has to stop (and start).
			 */
			if (this.outStream != outStream)
				return;
			/*
