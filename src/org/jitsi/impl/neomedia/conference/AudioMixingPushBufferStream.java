...
public class AudioMixingPushBufferStream extends ControlsAdapter implements PushBufferStream {
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>AudioMixingPushBufferStream</tt> class
	 ...
	 *         <tt>AudioFormat</tt>
	 * @throws UnsupportedFormatException if the specified <tt>outFormat</tt> is not
	...
	/**
	 * The total number of <tt>byte</tt>s read out of this <tt>PushBufferStream</tt>
	 * through {@link #read(Buffer)}. Intended for the purposes of debugging at the
	 ...
	 * instance and defines the input data which is to not be mixed in the output of
	...
	/**
	 * The <tt>Object</tt> which synchronizes the access to the data to be read from
	 * this <tt>PushBufferStream</tt> i.e. to {@link #inSamples},
	 ...
	 * <tt>PushBufferStream</tt> notifies its clients that new data is available for
	 * reading.
	 */
	private BufferTransferHandler transferHandler;

	/**
	 * Initializes a new <tt>AudioMixingPushBufferStream</tt> mixing the input data
	 * of a specific <tt>AudioMixerPushBufferStream</tt> and excluding from the mix
	 * the audio contributions of a specific
	 * <tt>AudioMixingPushBufferDataSource</tt>.
	 *
	 * @param audioMixerStream the <tt>AudioMixerPushBufferStream</tt> reading data
	 *                         from input <tt>DataSource</tt>s and to push it to the
	 *                         new <tt>AudioMixingPushBufferStream</tt>
	 * @param dataSource       the <tt>AudioMixingPushBufferDataSource</tt> which
	 *                         has requested the initialization of the new instance
	 *                         and which defines the input data to not be mixed in
	 *                         the output of the new instance
	 */
	 ...
	 * <tt>AudioMixerPushBufferStream</tt> because this instance is just a facet to
	 * it.
	 *
	 * @return <tt>true</tt> if this <tt>SourceStream</tt> has reached the end of
	 *         the media it makes available; otherwise, <tt>false</tt>
	 */
	@Override
	public boolean endOfStream() {
		/*
		 * TODO If the inSamples haven't been consumed yet, don't report the end of this
		 * stream even if the wrapped stream has reached its end.
		 */
	 ...
	 * wrapped <tt>AudioMixerPushBufferStream</tt> because this instance is just a
	 * facet to it.
	 *
	 * @return a <tt>ContentDescriptor</tt> which describes the content being made
	 *         available by this <tt>SourceStream</tt>
	 */
	@Override
	...
	/**
	 * Gets the <tt>AudioMixingPushBufferDataSource</tt> which created and owns this
	 * instance and defines the input data which is to not be mixed in the output of
	 * this <tt>PushBufferStream</tt>.
	 *
	 * @return the <tt>AudioMixingPushBufferDataSource</tt> which created and owns
	 *         this instance and defines the input data which is to not be mixed in
	 ...
	 *                       audio sample set is to be produced. The <tt>format</tt>
	 *                       property of the specified <tt>outBuffer</tt> is
	 *                       expected to be set to the same value but it is provided
	 *                       as a method argument in order to avoid casting from
	 *                       <tt>Format</tt> to <tt>AudioFormat</tt>.
	 * @param outSampleCount the size of the resulting mix audio sample set to be
	 *                       produced
	 * @return the resulting audio sample set of the audio mixing of the specified
	 *         input audio sample sets
	 */
	private short[] mix(short[][] inSamples, AudioFormat outFormat, int outSampleCount) {
		short[] outSamples;

		/*
		 * The trivial case of performing mixing the samples of a single stream. Then
		 * there is nothing to mix and the input becomes the output.
		 */
	...
	/**
	 * Implements {@link PushBufferStream#read(Buffer)}. If <tt>inSamples</tt> are
	 * available, mixes them and writes the mix to the specified <tt>Buffer</tt>
	 * performing the necessary data type conversions.
	 *
	...
	/**
	 * Sets the collection of audio sample sets to be mixed in the sense of audio
	 * mixing by this stream when data is read from it. Triggers a push to the
	 * clients of this stream.
	 *
	 ...
	 *                         in the specified <tt>Buffer</tt> when data is read
	 *                         from this instance
	 */
	 ...
	 * <tt>BufferTransferHandler</tt> which is to be notified by this instance when
	 * it has media available for reading.
	 *
	 * @param transferHandler the <tt>BufferTransferHandler</tt> to be notified by
	 *                        this instance when it has media available for reading
	 */
	@Override
