...
class InDataSourceDesc {
	...
	/**
	 * The <tt>Thread</tt> which currently executes {@link DataSource#connect()} on
	 * the effective input <tt>DataSource</tt> described by this instance.
	 */
	private Thread connectThread;

	/**
	 * The <tt>DataSource</tt> for which additional information is described by this
	 * instance.
	 */
	public final DataSource inDataSource;

	/**
	 * The <tt>AudioMixingPushBufferDataSource</tt> in which the mix contributions
	 * of {@link #inDataSource} are to not be included.
	 */
	 ...
	 * <tt>AudioMixer</tt>. Associates the specified <tt>DataSource</tt> with the
	 * <tt>AudioMixingPushBufferDataSource</tt> in which the mix contributions of
	 * the specified input <tt>DataSource</tt> are to not be included.
	 *
	 * @param inDataSource  a <tt>DataSource</tt> for which additional information
	 *                      is to be described by the new instance
	 * @param outDataSource the <tt>AudioMixingPushBufferDataSource</tt> in which
	 *                      the mix contributions of <tt>inDataSource</tt> are to
	 *                      not be included
	 */
	...
	/**
	 * Connects the effective input <tt>DataSource</tt> described by this instance
	 * upon request from a specific <tt>AudioMixer</tt>. If the effective input
	 * <tt>DataSource</tt> is to be asynchronously connected, the completion of the
	 * connect procedure will be reported to the specified <tt>AudioMixer</tt> by
	 * calling its {@link AudioMixer#connected(InDataSourceDesc)}.
	 *
	 * @param audioMixer the <tt>AudioMixer</tt> requesting the effective input
	 *                   <tt>DataSource</tt> described by this instance to be
	 *                   connected
	 * @throws IOException if anything wrong happens while connecting the effective
				...
				connectThread = new Thread() {
					...
					public void run() {
						try {
							...
							logger.error("Failed to connect to inDataSource " + MediaStreamImpl.toString(inDataSource),
									ioex);
						} finally {
							synchronized (InDataSourceDesc.this) {
	...
	/**
	 * Creates a <tt>DataSource</tt> which attempts to transcode the tracks of the
	 * input <tt>DataSource</tt> described by this instance into a specific output
	 * <tt>Format</tt>.
	 *
	 ...
	 *         effective input <tt>DataSource</tt> described by this instance with
	 *         the specified <tt>controlType</tt> if such a control exists;
	 ...
	 * <tt>AudioMixer</tt> directly reads in order to retrieve the mix contribution
	 * of the <tt>DataSource</tt> described by this instance.
	 *
	...
	/**
	 * Gets the <tt>SourceStream</tt>s of the effective input <tt>DataSource</tt>
	 ...
	 *
	 * @param transcodingDataSource the <tt>DataSource</tt> which transcodes the
	 *                              tracks of the input <tt>DataSource</tt>
	 *                              described by this instance in the output
	 *                              <tt>Format</tt> of the associated
	 *                              <tt>AudioMixer</tt>
	 */
	...
	/**
	 * Starts the effective input <tt>DataSource</tt> described by this instance if
	 * it is connected.
	 *
	...
	synchronized void start() throws IOException {
		if (connected) {
			logger.debug("Starting data source: " + getEffectiveInDataSource().getClass().getName());
			getEffectiveInDataSource().start();
		}
	}

	/**
	 * Stops the effective input <tt>DataSource</tt> described by this instance if
	 * it is connected.
	 *
	...
	synchronized void stop() throws IOException {
		if (connected) {
			logger.debug("Stopping data source: " + getEffectiveInDataSource().getClass().getName());
