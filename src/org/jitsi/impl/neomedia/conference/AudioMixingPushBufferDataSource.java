...
public class AudioMixingPushBufferDataSource extends PushBufferDataSource
	...
	/**
	 * The one and only <tt>PushBufferStream</tt> this <tt>PushBufferDataSource</tt>
	 * provides to its clients and containing the result of the audio mixing
	 * performed by <tt>audioMixer</tt>.
	 */
	 ...
	 * <tt>AudioMixer</tt>.
	 *
	 * @param audioMixer the <tt>AudioMixer</tt> performing audio mixing, managing
	 *                   the input <tt>DataSource</tt>s and pushing the data of the
	 *                   new output <tt>PushBufferDataSource</tt>
	 */
	 ...
	 * <tt>AudioMixer</tt> of this instance and to not have its audio contributions
	 * included in the mixing output represented by this <tt>DataSource</tt>.
	 *
	 * @param inDataSource a <tt>DataSource</tt> to be added for mixing to the
	 *                     <tt>AudioMixer</tt> associate with this instance and to
	 *                     not have its audio contributions included in the mixing
	...
	/**
	 * Implements {@link DataSource#connect()}. Lets the <tt>AudioMixer</tt> know
	 * that one of its output <tt>PushBufferDataSources</tt> has been connected and
	 * marks this <tt>DataSource</tt> as connected.
	 *
	...
	/**
	 * Implements {@link DataSource#disconnect()}. Marks this <tt>DataSource</tt> as
	 * disconnected and notifies the <tt>AudioMixer</tt> that one of its output
	 * <tt>PushBufferDataSources</tt> has been disconnected.
	 */
	@Override
	 ...
	 * Delegates to the <tt>AudioMixer</tt> because this instance is just a facet to
	 * it.
	 *
	 ...
	 * associated <tt>AudioMixer</tt> because it knows which <tt>CaptureDevice</tt>
	 * is being wrapped.
	 *
	 ...
	 * is being wrapped.
	 *
	 * @return an array of <tt>FormatControl</tt>s of the <tt>CaptureDevice</tt> of
			...
			if (audioMixerOutStream != null) {
				...
				if (started)
					try {
						...
						logger.error("Failed to start " + outStream.getClass().getSimpleName() + " with hashCode "
								+ outStream.hashCode(), ioex);
					}
			}
		}
	...
	/**
	 * Implements {@link DataSource#stop()}. Notifies the <tt>AudioMixer</tt> that
	 * one of its output <tt>PushBufferDataSources</tt> has been stopped and stops
	 * the output <tt>PushBufferStream</tt> of this <tt>DataSource</tt> (if it
	 * exists).
	 *
