...
class ShortArrayCache {
	/**
	 * The cache of <tt>short</tt> arrays managed by this instance for the purposes
	 * of reducing garbage collection.
	 */
	private SoftReference<short[][]> elements;

	/**
	 * The number of elements at the head of {@link #elements} which are currently
	 * utilized. Introduced to limit the scope of iteration.
	 */
	private int length;

	/**
	 * Allocates a <tt>short</tt> array with length/size greater than or equal to a
	 * specific number. The returned array may be a newly-initialized instance or
	 * one of the elements cached/pooled by this instance.
	 *
	...
	/**
	 * Returns a specific non-<tt>null</tt> <tt>short</tt> array into the cache/pool
	 * implemented by this instance.
	 *
	 * @param shortArray the <tt>short</tt> array to be returned into the cache/pool
	...
	/**
	 * Ensures that the <tt>data</tt> property of a specific <tt>Buffer</tt> is set
	 * to an <tt>short</tt> array with length/size greater than or equal to a
	 * specific number.
	 * 
	 * @param buffer  the <tt>Buffer</tt> the <tt>data</tt> property of which is to
	 *                be validated
	 * @param newSize the minimum length/size of the <tt>short</tt> array to be set
