 ...
 * {@link MediaStreamStats2Impl} instead, where we can manage the complexity and
 * consistency better.
 *
...
public class MediaStreamStatsImpl implements MediaStreamStats {
	...
	/**
	 * Keeps track of when a given NTP time (found in an SR) has been received. This
	 * is used to compute the correct RTT in the translator case.
	 */
	private final Map<Long, Long> emission2reception = Collections.synchronizedMap(new LRUCache<Long, Long>(100));

	/**
	 * Computes an Exponentially Weighted Moving Average (EWMA). Thus, the most
	 * recent history has a more preponderant importance in the average computed.
	 *
	 * @param nbStepSinceLastUpdate The number of step which has not been computed
	 *                              since last update. In our case the number of
	 *                              packets received since the last computation.
	 ...
	 * @param intervalMs The number of milliseconds during which <tt>nbBytes</tt>
	...
	/**
	 * The <tt>RTCPReportListener</tt> which listens to {@link #rtcpReports} about
	 * the sending and the receiving of RTCP sender/receiver reports and updates
	 * this <tt>MediaStreamStats</tt> with their feedback reports.
	 */
	 ...
	 * subsection "delay since last SR (DLSR): 32 bits"). -1 if the RTT has not been
	 * computed yet. Otherwise the RTT in ms.
	 */
	...
	/**
	 * The maximum inter arrival jitter value the other party has reported, in RTP
	 * time units.
	 */
	private long minRemoteInterArrivalJitter = -1;

	/**
	 * The minimum inter arrival jitter value the other party has reported, in RTP
	 * time units.
	 */
	private long maxRemoteInterArrivalJitter = 0;

	/**
	 * The sum of all RTP jitter values reported by the remote side, in RTP time
	 * units.
	 */
	 ...
	 */
	private final List<RTCPPacketListener> rtcpPacketListeners = Collections
			.synchronizedList(new LinkedList<RTCPPacketListener>());
	...
	private int computeRTTInMs(RTCPFeedback feedback) {
		...
		if (lsr > 0 && dlsr > 0) {
			...
			if (translator != null) {
				...
				if (receiveRTPManager != null) {
					...
					if (logger.isDebugEnabled()) {
						logger.debug("invalid_rtt,stream=" + mediaStreamImpl.hashCode() + " ssrc=" + feedback.getSSRC()
								+ ",now=" + arrivalMs + ",lsr=" + lsr + ",dlsr=" + dlsr);
					}
			...
			if (rttLong < 0 || rttLong >= 3000) {
				logger.warn("invalid_rtt,stream=" + mediaStreamImpl.hashCode() + " ssrc=" + feedback.getSSRC() + ",rtt="
						+ rttLong + ",now=" + arrivalMs + ",lsr=" + lsr + ",dlsr=" + dlsr);

				rtt = -1;
			} else {
				if (logger.isDebugEnabled()) {
					logger.debug("rtt,stream= " + mediaStreamImpl.hashCode() + " ssrc=" + feedback.getSSRC() + ",rtt="
							+ rttLong + ",now=" + arrivalMs + ",lsr=" + lsr + ",dlsr=" + dlsr);
				}
				rtt = (int) rttLong;
			}
	...
	/**
	 * Returns the number of Protocol Data Units (PDU) lost in download since the
	...
	/**
	 * Returns the download video format if this stream downloads a video, or null
	 * if not.
	 *
	 * @return the download video format if this stream downloads a video, or null
	 *         if not.
	 */
	private VideoFormat getDownloadVideoFormat() {
		...
		return (deviceSession instanceof VideoMediaDeviceSession)
				? ((VideoMediaDeviceSession) deviceSession).getReceivedVideoFormat()
				: null;
	}

	/**
	 * Returns the download video size if this stream downloads a video, or null if
	 * not.
	 *
	 * @return the download video size if this stream downloads a video, or null if
	 *         not.
	 */
	@Override
	 ...
	 * there might be multiple <tt>ReceiveStreams</tt>, returns the biggest delay
	...
	/**
	 * Returns the delay in number of packets introduced by the jitter buffer. Since
	 * there might be multiple <tt>ReceiveStreams</tt>, returns the biggest delay
	 * found in any of them.
	 *
	...
	/**
	 * Converts from RTP time units (using the assumed RTP clock rate of the media
	 * stream) to milliseconds. Returns -1D if an appropriate RTP clock rate cannot
	 * be found.
	 * 
	...
	/**
	 * Notifies this instance that an RTCP report with the given value for RTP
	 * jitter was received.
	 * 
	 ...
	 *
	 * @param streamDirection The stream direction (DOWNLOAD or UPLOAD) of the
	 *                        stream from which this function retrieve the number of
	 *                        sent/received bytes.
	...
	/**
	 * Returns the total number of Protocol Data Units (PDU) discarded by the FMJ
	 * packet queue since the beginning of the session. It's the sum over all
	...
	/**
	 * Returns the number of Protocol Data Units (PDU) discarded by the FMJ packet
	 * queue since the beginning of the session because it was full. It's the sum
	 * over all <tt>ReceiveStream</tt>s of the <tt>MediaStream</tt>
	 *
	...
	/**
	 * Returns the number of Protocol Data Units (PDU) discarded by the FMJ packet
	 * queue since the beginning of the session because they were late. It's the sum
	 * over all <tt>ReceiveStream</tt>s of the <tt>MediaStream</tt>
	 *
	...
	/**
	 * Returns the number of Protocol Data Units (PDU) discarded by the FMJ packet
	 * queue since the beginning of the session during resets. It's the sum over all
	 * <tt>ReceiveStream</tt>s of the <tt>MediaStream</tt>
	 *
	...
	/**
	 * Returns the number of Protocol Data Units (PDU) discarded by the FMJ packet
	 * queue since the beginning of the session due to shrinking. It's the sum over
	...
	/**
	 * Returns the number of packets for which FEC data was decoded. Currently this
	 * is cumulative over all <tt>ReceiveStream</tt>s.
	 *
	 * @return the number of packets for which FEC data was decoded. Currently this
	 *         is cumulative over all <tt>ReceiveStream</tt>s.
	 *
	...
	/**
	 * Returns the total number of packets that are send or receive for this stream
	 * since the stream is created.
	 * 
	...
	/**
	 * Returns the number of packets in the first <tt>JitterBufferControl</tt> found
	 * via <tt>getJitterBufferControls</tt>.
	 *
	 * @return the number of packets in the first <tt>JitterBufferControl</tt> found
	 *         via <tt>getJitterBufferControls</tt>.
	 */
	@Override
	...
	/**
	 * Returns the RTT computed with the RTCP feedback (cf. RFC3550, section 6.4.1,
	 * subsection "delay since last SR (DLSR): 32 bits").
	 *
	 * @return The RTT computed with the RTCP feedback. Returns -1 if the RTT has
	 *         not been computed yet. Otherwise the RTT in ms.
	 */
	@Override
	...
	/**
	 * Returns the upload video format if this stream uploads a video, or null if
	 * not.
	 *
	 * @return the upload video format if this stream uploads a video, or null if
	 *         not.
	 */
		...
		if (this.rttMs != rttMs) {
			...
			if (rttMs >= 0) {
				...
				if (mediaStream instanceof VideoMediaStream) {
					...
					remoteBitrateEstimator.onRttUpdate(/* avgRttMs */ rttMs, /* maxRttMs */ rttMs);

					TransportCCEngine tccEngine = mediaStream.getTransportCCEngine();

					if (tccEngine != null) {
						tccEngine.onRttUpdate(/* avgRttMs */ rttMs, /* maxRttMs */ rttMs);
					}
				}
			}
		}
	...
	private void updateJitterRTPTimestampUnits(RTCPFeedback feedback, StreamDirection streamDirection) {
		...
		MediaStreamStats2Impl extended = getExtended();
		extended.updateJitter(feedback.getSSRC(), streamDirection, rtpTimeToMs(feedback.getJitter()));
	}
	 ...
	 * @param nbSteps        The number of elapsed steps since the last number of
	 *                       loss update.
	 */
	...
	/**
	 * Updates the <tt>nbFec</tt> field with the sum of FEC-decoded packets over the
	 * different <tt>ReceiveStream</tt>s
	 */
	private void updateNbFec() {
		...
		if (devSession != null) {
			for (ReceiveStream receiveStream : devSession.getReceiveStreams()) {
				for (FECDecoderControl fecDecoderControl : devSession.getDecoderControls(receiveStream,
	...
	private void updateNbLoss(StreamDirection streamDirection, long newNbLost, long nbSteps) {
		...
		percentLoss[streamDirectionIndex] = MediaStreamStatsImpl.computeEWMA(nbSteps, percentLoss[streamDirectionIndex],
				newPercentLoss);
		// Saves the last update number download lost value.
	...
	private void updateStreamDirectionStats(StreamDirection streamDirection, long currentTimeMs) {
		...
		rateKiloBitPerSec[streamDirectionIndex] = computeEWMA(nbSteps, rateKiloBitPerSec[streamDirectionIndex],
	...
	public void srReceived(RTCPSRPacket sr) {
		if (sr != null) {
			long emisionTime = TimeUtils
					.toNtpShortFormat(TimeUtils.constuctNtp(sr.ntptimestampmsw, sr.ntptimestamplsw));
	...
	/**
	 * Notifies this instance that a specific RTCP RR or SR report was received by
	 * {@link #rtcpReports}.
	 *
	 ...
	 * 1. It is not necessary for {@link #updateStats()} to be called periodically
	 * by the user of libjitsi in order for it to return correct values. 2. The
	 * returned value is based on the average bitrate over a fixed window, as
	 * opposed to an EWMA. 3. The measurement is performed after the
	 * {@link MediaStream}'s transformations, notably after simulcast layers are
	 * dropped (i.e. closer to the network interface).
	 *
	...
	/**
	 * Notifies listeners that a transport-wide-cc packet was received. Listeners
	 * may include Remote Bitrate Estimators or Bandwidth Estimators
	 * {@param tccPacket}
	 */
