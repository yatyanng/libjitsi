...
public class RTPConnectorUDPOutputStream extends RTPConnectorOutputStream {
	...
	protected void sendToTarget(RawPacket packet, InetSocketAddress target) throws IOException {
		socket.send(new DatagramPacket(packet.getBuffer(), packet.getOffset(), packet.getLength(), target.getAddress(),
				target.getPort()));
	}
	...
	protected void doLogPacket(RawPacket packet, InetSocketAddress target) {
		...
		if (pktLogging != null) {
			pktLogging.logPacket(PacketLoggingService.ProtocolName.RTP, socket.getLocalAddress().getAddress(),
					socket.getLocalPort(), target.getAddress().getAddress(), target.getPort(),
					PacketLoggingService.TransportName.UDP, true, packet.getBuffer(), packet.getOffset(),
					packet.getLength());
		}
	}
	 ...
	 * socket.
	 *
	 * @return true if this <tt>RTPConnectorOutputStream</tt> has a valid socket,
	 *         false otherwise
	 */
	@Override
