...
public class SilenceEffect extends AbstractCodec2 implements Effect {
	...
	/**
	 * The indicator which determines whether <tt>SilenceEffect</tt> instances are
	 * to perform the copying of the data from input <tt>Buffer</tt>s to output
	...
	public static final Format[] SUPPORTED_FORMATS = new Format[] {
			new AudioFormat(AudioFormat.LINEAR, sampleRate, sampleSizeInBits, 1, // channels
	...
	/**
	 * The clock rate for the timestamps of input <tt>Buffer</tt>s (i.e. the number
	 * of units which constitute one second).
	 */
	...
	/**
	 * The timestamp (either the RTP timestamp, or the <tt>Buffer</tt>'s timestamp,
	 * according to the value of {@link #useRtpTimestamp}) of the last sample that
	 * was output by this <tt>Codec</tt>.
	 */
	...
	/**
	 * Processes <tt>inBuf</tt>, and either copies its data to <tt>outBuf</tt> or
	 * copies silence
	 * 
	 * @param inBuf  the input <tt>Buffer</tt>.
	 * @param outBuf the output <tt>Buffer</tt>.
	 * @return <tt>BUFFER_PROCESSED_OK</tt> if <tt>inBuf</tt>'s date was copied to
	 *         <tt>outBuf</tt>, and <tt>INPUT_BUFFER_NOT_CONSUMED</tt> if silence
	...
	protected int doProcess(Buffer inBuf, Buffer outBuf) {
		...
		} else // timestamp != -1 && lastOutputTimestamp != -1
		{
			...
			if (diffSamples > MAX_SAMPLES_SILENCE) {
				logger.info(
						"More than the maximum of " + MAX_SAMPLES_SILENCE + " samples of silence need to be inserted.");
	...
	/**
	 * Returns the timestamp obtained by adding <tt>samplesToAdd</tt> samples (using
	 * a sample rate of <tt>this.sampleRate</tt> per second) to timestamp (with a
	 * clock rate of <tt>this.clockRate</tt> per second).
	 * 
	 * @param oldTimestamp the timestamp to which to add.
	 * @param samplesToAdd the number of samples to add.
	 * @return the timestamp obtained by adding <tt>samplesToAdd</tt> samples (using
	 *         a sample rate of <tt>this.sampleRate</tt> per second) to timestamp
