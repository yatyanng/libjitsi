...
public class EncodingConfigurationImpl extends EncodingConfiguration {
	 ...
	 * some countries and is licensed by <a href="http://www.sipro.com">SIPRO Lab
	 * Telecom</a>.
	 */
	...
	private void initializeFormatPreferences() {
		...
		/*
		 * setEncodingPreference( "H263", VideoMediaFormatImpl.DEFAULT_CLOCK_RATE,
		 * 1000);
		 */
	...
	protected void setEncodingPreference(String encoding, double clockRate, int pref) {
		...
		/*
		 * The key in encodingPreferences associated with a MediaFormat is currently
		 * composed of the encoding and the clockRate only so it makes sense to ignore
	...
	protected int compareEncodingPreferences(MediaFormat enc1, MediaFormat enc2) {
		...
		/*
		 * If the encodings are with same priority, compare them by name. If we return
		 * equals, TreeSet will not add equal encodings.
		 */
		if (res == 0) {
			res = enc1.getEncoding().compareToIgnoreCase(enc2.getEncoding());
			/*
			 * There are formats with one and the same encoding (name) but different clock
			 * rates.
			 */
			if (res == 0) {
				 ...
				 * (name) and clock rate but different format parameters (e.g.
				 * packetization-mode).
				 */
				if (res == 0) {
					...
					if (res == 0) {
						/*
						 * The format with more parameters will be considered here to be the format with
						 * higher priority.
						 */
						...
						/*
						 * TODO Even if the number of format parameters is equal, the two formats may
						 * still be different. Consider ordering by the values of the format parameters
						 * as well.
						 */
