...
public class AVFrameFormat extends VideoFormat {
	...
	/**
	 * The <tt>DeviceSystem</tt>-specific format represented by this instance. It is
	 * necessary in the cases of multiple <tt>DeviceSystem</tt>-specific formats
	 * corresponding to one and the same native FFmpeg format in which JMF is unable
	 * to differentiate them yet the device needs to know its
	...
	/**
	 * Initializes a new <tt>AVFrameFormat</tt> instance with specific size, frame
	 * rate and FFmpeg colorspace.
	 *
	 * @param size               the <tt>Dimension</tt> of the new instance
	 * @param frameRate          the frame rate of the new instance
	 * @param pixFmt             the FFmpeg colorspace to be represented by the new
	 *                           instance
	 * @param deviceSystemPixFmt the <tt>DeviceSystem</tt>-specific colorspace to be
		...
		if ((pixFmt == NOT_SPECIFIED) && (deviceSystemPixFmt != NOT_SPECIFIED)) {
			logger.warn(
					"Specifying a device system-specific pixel format 0x"
							...
							+ " without a matching FFmpeg pixel format may" + " eventually lead to a failure.",
					new Throwable());
		}
	 ...
	 *
	 * @return a new <tt>AVFrameFormat</tt> instance which has the same properties
	 *         as this instance
	 */
	@Override
	public Object clone() {
	 ...
	 *
	 * @param f the <tt>Format</tt> the properties of which are to be copied into
	 *          this instance
	 */
	@Override
	 ...
	 *
	 * @param obj the <tt>Object</tt> to be determined whether it represents a value
	 ...
	 *         this instance and the specified <tt>format</tt>
	 */
	@Override
	...
	public boolean matches(Format format) {
		...
		if (super.matches(format)) {
			if (format instanceof AVFrameFormat) {
				...
				matches = ((pixFmt == NOT_SPECIFIED) || (avFrameFormat.pixFmt == NOT_SPECIFIED)
						|| (pixFmt == avFrameFormat.pixFmt));
			} else
				matches = true;
		} else
