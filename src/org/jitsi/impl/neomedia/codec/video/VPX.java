...
public class VPX {
	/**
	 * Operation completed without error. Corresponds to <tt>VPX_CODEC_OK</tt> from
	 * <tt>vpx/vpx_codec.h</tt>
	 */
	...
	/**
	 * The frame partitions are independently decodable by the bool decoder, meaning
	 * that partitions can be decoded even though earlier partitions have been lost.
	 * Note that intra predicition is still done over the partition boundary.
	 *
	 * To set this option for Coan encoder, enable this bit in the value passed to
	 * <tt>vpx_enc_cft_set_error_resilient</tt> for the encoder's configuration.
	 *
	...
	/**
	 * Process and return as soon as possible ('realtime' deadline) Corresponds to
	 * <tt>VPX_DL_REALTIME</tt> from <tt>vpx/vpx_encoder.h</tt>
	 */
	 ...
	 * @param flags   Flags.
	 *
	 * @return <tt>CODEC_OK</tt> on success, or an error code otherwise. The error
	 *         code can be converted to a <tt>String</tt> with
	 ...
	 * @param deadline   Soft deadline the decoder should attempt to meet, in
	 ...
	 *         {@link VPX#codec_err_to_string(int)}
	 */
	public static native int codec_decode(long context, byte[] buf, int buf_offset, int buf_size, long user_priv,
			long deadline);

	/**
	 * Gets the next frame available to display from the decoder context
	 * <tt>context</tt>. The list of available frames becomes valid upon completion
	 * of the <tt>codec_decode</tt> call, and remains valid until the next call to
	 * <tt>codec_decode</tt>.
	 *
	 * @param context The decoder context to use.
	 * @param iter    Iterator storage, initialized by setting its first element to
	 *                0.
	 *
	 * @return Pointer to a <tt>vpx_image_t</tt> describing the decoded frame, or 0
	 ...
	 * @param iface   Interface to be used. Has to be one of the
	 *                <tt>VPX.INTERFACE_*</tt> constants.
	 * @param cfg     Pointer to a pre-allocated <tt>vpx_codec_enc_cfg_t</tt>, may
	 *                be 0.
	 * @param flags   Flags.
	 *
	 * @return <tt>CODEC_OK</tt> on success, or an error code otherwise. The error
	...
	/**
	 *
	 * @param context Pointer to the codec context on which to set the confirutation
	...
	/**
	 * Encodes the frame described by <tt>img</tt>, <tt>buf</tt>, <tt>offset0</tt>,
	 * <tt>offset1</tt> and <tt>offset2</tt>.
	 *
	 * Note that <tt>buf</tt> and the offsets describe where the frames is stored,
	 * but <tt>img</tt> has to have all of its other parameters (format, dimensions,
	 * strides) already set.
	 *
	 * The reason <tt>buf</tt> and the offsets are treated differently is to allow
	 * for the encoder to operate on java memory and avoid copying the raw frame to
	 * native memory.
	 *
	 ...
	 *         {@link VPX#codec_err_to_string(int)}
	 */
	public static native int codec_encode(long context, long img, byte[] buf, int offset0, int offset1, int offset2,
			long pts, long duration, long flags, long deadline);

	/**
	 * Encoded data iterator. Iterates over a list of data packets to be passed from
	 * the encoder to the application. The kind of a packet can be determined using
	 * {@link VPX#codec_cx_pkt_get_kind} Packets of kind <tt>CODEC_CX_FRAME_PKT</tt>
	 * should be passed to the application's muxer.
	 *
	 * @param context The codec context to use.
	 * @param iter    Iterator storage, initialized by setting its first element to
	 *                0.
	 *
	 * @return Pointer to a vpx_codec_cx_pkt_t containing the output data packet, or
	 *         0 to indicate the end of available packets
	 */
	public static native long codec_get_cx_data(long context, long[] iter);

	/**
	 * Returns the <tt>kind</tt> of the <tt>vpx_codec_cx_pkt_t</tt> pointed to by
	 * <tt>pkt</tt>.
	 *
	...
	/**
	 * Returns the size of the data in the <tt>vpx_codec_cx_pkt_t</tt> pointed to by
	 * <tt>pkt</tt>. Can only be used for packets of <tt>kind</tt>
	...
	/**
	 * Returns a pointer to the data in the <tt>vpx_codec_cx_pkt_t</tt> pointed to
	 * by<tt>pkt</tt>. Can only be used for packets of <tt>kind</tt>
	 * <tt>CODEC_CX_FRAME_PKT</tt>.
	 *
	 ...
	 * for descriptor has been allocated elsewhere, and a descriptor is desired to
	 * "wrap" that storage.
	 *
	 * @param img   Pointer to a <tt>vpx_image_t</tt>.
	 ...
	 * context is not necessary. Can be used to determine if the bitstream is of the
	 * proper format, and to extract information from the stream.
	 *
	 ...
	 * @param si_ptr     Pointer to a <tt>vpx_codec_stream_info_t</tt> which will be
	...
	/**
	 * Frees memory, which has been allocated with {@link VPX#malloc(long)} or one
	 * of the <tt>*_malloc()</tt> functions.
	 *
