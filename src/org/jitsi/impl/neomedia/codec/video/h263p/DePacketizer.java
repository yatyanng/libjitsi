...
public class DePacketizer extends AbstractCodec2 {
	...
	/**
	 * Keeps track of last (input) sequence number in order to avoid inconsistent
	 * data.
	 */
	private long lastSequenceNumber = -1;

	/**
	 * The indicator which determines whether incomplete buffer packets are output
	 * from the H.263+ <tt>DePacketizer</tt> to the decoder.
	 */
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		...
		if ((lastSequenceNumber != -1) && ((sequenceNumber - lastSequenceNumber) != 1)) {
			...
			/*
			 * maybe we lost a frame somewhere or the sequence number reach its maximum
			 * number
			 */
			if (logger.isTraceEnabled())
				logger.trace("Dropped RTP packets upto sequenceNumber " + lastSequenceNumber
		...
		System.arraycopy(in, inOffset + 2 + (vBit ? 1 : 0) + plen, out, outOffset + (pBit ? 2 : 0),
				dataLength - (pBit ? 2 : 0));
	...
	/**
	 * Appends {@link #outputPaddingSize} number of bytes to <tt>out</tt> beginning
	 * at index <tt>outOffset</tt>. The specified <tt>out</tt> is expected to be
	 * large enough to accommodate the mentioned number of bytes.
	 *
	 * @param out       the buffer in which <tt>outputPaddingSize</tt> number of
