...
public class VPXEncoder extends AbstractCodec2 {
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>VPXEncoder</tt> class and its instances
	 * for logging output.
	 */
	...
	protected void doOpen() throws ResourceUnavailableException {
		...
		if (ret != VPX.CODEC_OK)
			throw new RuntimeException(
					"Failed to initialize encoder, libvpx" + " error:\n" + VPX.codec_err_to_string(ret));
	 ...
	 * {@inheritDoc}
	 *
	 * Encodes the frame in <tt>inputBuffer</tt> (in <tt>YUVFormat</tt>) into a VP8
	 * frame (in <tt>outputBuffer</tt>)
	 *
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if (leftoverPackets) {
			...
			int result = VPX.codec_encode(context, img, (byte[]) inputBuffer.getData(), offsetY, offsetU, offsetV,
					frameCount, // pts
					1, // duration
					0, // flags
					VPX.DL_REALTIME);
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new VideoFormat[] { new VideoFormat(Constants.VP8, inputVideoFormat.getSize(),
	...
	public Format setOutputFormat(Format format) {
		...
		outputFormat = new VideoFormat(videoFormat.getEncoding(), size, /* maxDataLength */ Format.NOT_SPECIFIED,
				Format.byteArray, videoFormat.getFrameRate());
