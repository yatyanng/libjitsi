...
public class DePacketizer extends AbstractCodec2 {
	/**
	 * The <tt>Logger</tt> used by the <tt>DePacketizer</tt> class and its instances
	 * for logging output.
	 */
	 ...
	 * belonging to a single VP8 compressed frame. Maps an RTP sequence number to a
	 * buffer which contains the payload.
	 */
	...
	/**
	 * Stores the last (latest) sequence number stored in <tt>data</tt>, or -1 if
	 * <tt>data</tt> is empty.
	 */
	...
	/**
	 * Stores the sum of the lengths of the data stored in <tt>data</tt>, that is
	 * the total length of the VP8 compressed frame to be constructed.
	 */
	...
	/**
	 * Re-initializes the fields which store information about the currently held
	 * data. Empties <tt>data</tt>.
	 */
	private void reinit() {
		firstSeq = lastSeq = -1;
		timestamp = -1L;
	...
	/**
	 * Checks whether the currently held VP8 compressed frame is complete (e.g all
	 * its packets are stored in <tt>data</tt>).
	 * 
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		...
		if (empty && lastSentSeq != -1 && RTPUtils.sequenceNumberComparator.compare(inSeq, lastSentSeq) != 1) {
			if (logger.isInfoEnabled())
		...
		if (!empty) {
			...
			if ((inPictureId != -1 && pictureId != -1 && inPictureId != pictureId)
				...
				if (RTPUtils.sequenceNumberComparator.compare(inSeq, firstSeq) != 1) // inSeq <= firstSeq
				{
				...
				} else // inSeq > firstSeq (and also presumably isSeq > lastSeq)
				{
					...
					if (logger.isInfoEnabled())
						logger.info("Discarding saved packets on arrival of" + " a packet for a subsequent frame: "
								+ inSeq);
		...
		if (firstSeq == -1 || (RTPUtils.sequenceNumberComparator.compare(firstSeq, inSeq) == 1))
			firstSeq = inSeq;
		if (lastSeq == -1 || (RTPUtils.sequenceNumberComparator.compare(inSeq, lastSeq) == 1))
			lastSeq = inSeq;
	...
	public static class VP8PayloadDescriptor {
		 ...
		 * partition' bit set according to <tt>startOfPartition</tt>, and all other bits
		 * set to 0.
		 * 
		...
		/**
		 * The size in bytes of the Payload Descriptor at offset <tt>offset</tt> in
		 * <tt>input</tt>. The size is between 1 and 6.
		 *
		 * @param baf the <tt>ByteArrayBuffer</tt> that holds the VP8 payload
		 *            descriptor.
		 *
		 * @return The size in bytes of the Payload Descriptor at offset <tt>offset</tt>
		 *         in <tt>input</tt>, or -1 if the input is not a valid VP8 Payload
		 *         Descriptor. The size is between 1 and 6.
		 */
		...
		/**
		 * Determines whether the VP8 payload specified in the buffer that is passed as
		 * an argument has a picture ID or not.
		 *
		...
		/**
		 * Determines whether the VP8 payload specified in the buffer that is passed as
		 * an argument has an extended picture ID or not.
		 *
		 * @param buf the byte buffer that contains the VP8 payload.
		 * @param off the offset in the byte buffer where the VP8 payload starts.
		 * @param len the length of the VP8 payload in the byte buffer.
		 * @return true if the VP8 payload contains an extended picture ID, false
		 *         otherwise.
		 */
		 ...
		 * @param input
		 * @param offset
		 * @return the value of the PictureID field of a VP8 Payload Descriptor, or -1
		 *         if the fields is not present.
		 */
		public static int getPictureId(byte[] input, int offset) {
			if (input == null || !hasPictureId(input, offset, input.length - offset)) {
				return -1;
			}
		...
		/**
		 * Sets the extended picture ID for the VP8 payload specified in the buffer that
		...
		/**
		 * Sets the TL0PICIDX field for the VP8 payload specified in the buffer that is
		...
		public static boolean setTL0PICIDX(byte[] buf, int off, int len, int val) {
			if (!isValid(buf, off, len) || (buf[off] & X_BIT) == 0 || (buf[off + 1] & L_BIT) == 0) {
				return false;
			}
		 ...
		 * @param off
		 * @param len
		 * @return true if the arguments specify a valid buffer, false otherwise.
		 */
		...
		/**
		 * Checks whether the '<tt>start of partition</tt>' bit is set in the VP8
		...
		/**
		 * Returns <tt>true</tt> if both the '<tt>start of partition</tt>' bit is set
		 * and the <tt>PID</tt> fields has value 0 in the VP8 Payload Descriptor at
		 * offset <tt>offset</tt> in <tt>input</tt>.
		 * 
		 * @param input
		 * @param offset
		 * @return <tt>true</tt> if both the '<tt>start of partition</tt>' bit is set
		 *         and the <tt>PID</tt> fields has value 0 in the VP8 Payload Descriptor
		...
		/**
		 * Returns the value of the <tt>PID</tt> (partition ID) field of the VP8 Payload
		 * Descriptor at offset <tt>offset</tt> in <tt>input</tt>.
		 * 
		 * @param input
		 * @param offset
		 * @return the value of the <tt>PID</tt> (partition ID) field of the VP8 Payload
		 *         Descriptor at offset <tt>offset</tt> in <tt>input</tt>.
		 */
	...
	/**
	 * A class that represents the VP8 Payload Header structure defined in
	 * {@link "http://tools.ietf.org/html/draft-ietf-payload-vp8-10"}
	 */
	public static class VP8PayloadHeader {
		...
		/**
		 * Returns true if the <tt>P</tt> (inverse key frame flag) field of the VP8
		 * Payload Header at offset <tt>offset</tt> in <tt>input</tt> is 0.
		 *
		 * @return true if the <tt>P</tt> (inverse key frame flag) field of the VP8
		 *         Payload Header at offset <tt>offset</tt> in <tt>input</tt> is 0,
		 *         false otherwise.
		 */
