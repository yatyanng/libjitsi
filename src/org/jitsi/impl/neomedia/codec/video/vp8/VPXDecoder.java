...
public class VPXDecoder extends AbstractCodec2 {
	...
	/**
	 * The last known width of the video output by this <tt>VPXDecoder</tt>. Used to
	 * detect changes in the output size.
	 */
	private int height;
	...
	/**
	 * The last known height of the video output by this <tt>VPXDecoder</tt>. Used
	 * to detect changes in the output size.
	 */
	private int width;
	...
	protected void doOpen() throws ResourceUnavailableException {
		...
		if (ret != VPX.CODEC_OK)
			throw new RuntimeException(
					"Failed to initialize decoder, libvpx" + " error:\n" + VPX.codec_err_to_string(ret));
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		if (leftoverFrames) {
			/*
			 * There are more decoded frames available in the context. Fill outputBuffer
			 * with the next frame.
			 */
		...
		/*
		 * outputBuffer is all setup now. Check the decoder context for more decoded
		 * frames.
		 */
		img = VPX.codec_get_frame(context, iter);
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new Format[] { new AVFrameFormat(inputVideoFormat.getSize(), inputVideoFormat.getFrameRate(),
				FFmpeg.PIX_FMT_YUV420P) };
	}

	/**
	 * Allocates a new AVFrame and set its data fields to the data fields from the
	 * <tt>vpx_image_t</tt> pointed to by <tt>img</tt>. Also set its 'linesize'
	 * according to <tt>img</tt>.
	 *
	...
	private AVFrame makeAVFrame(long img) {
		...
		FFmpeg.avframe_set_linesize(avframe.getPtr(), VPX.img_get_stride0(img), VPX.img_get_stride1(img),
