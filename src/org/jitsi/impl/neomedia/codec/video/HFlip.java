...
public class HFlip extends AbstractCodec2 implements Effect {
	/**
	 * The <tt>Logger</tt> used by the <tt>HFlip</tt> class and its instances for
	 * logging output.
	 */
	private static final Logger logger = Logger.getLogger(HFlip.class);

	/**
	 * The list of <tt>Format</tt>s supported by <tt>HFlip</tt> instances as input
	 * and output.
	 */
	...
	/**
	 * The pointer to the <tt>AVFilterContext</tt> in {@link #graph} of the FFmpeg
	 * video source with the name {@link #VSRC_BUFFER_NAME}.
	 */
	private long buffer;

	/**
	 * The pointer to the <tt>AVFilterContext</tt> in {@link #graph} of the FFmpeg
	 * video sink with the name {@link #VSINK_FFSINK_NAME}.
	 */
	private long ffsink;

	/**
	 * The pointer to the <tt>AVFilterGraph</tt> instance which contains the FFmpeg
	 * hflip filter represented by this <tt>Effect</tt>.
	 */
	private long graph = 0;

	/**
	 * The indicator which determines whether the fact that {@link #graph} is equal
	 * to zero means that an attempt to initialize it is to be made. If
	 * <tt>false</tt>, indicates that such an attempt has already been made and has
	 * failed. In other words, prevents multiple initialization attempts with the
	 * same parameters.
	 */
	...
	/**
	 * The pointer to the <tt>AVFrame</tt> instance which is the output (data) of
	 * this <tt>Effect</tt>.
	 */
	 ...
	 *
	 * @throws ResourceUnavailableException if any of the required resource cannot
	 *                                      be allocated
	...
	protected synchronized int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		long filterResult = FFmpeg.get_filtered_video_frame(inputFrame, this.width, this.height, this.pixFmt, buffer,
				ffsink, outputFrame);
		if (filterResult < 0) {
	...
	private boolean allocateFfmpegGraph(AVFrameFormat format, Dimension size, int pixFmt) {
		...
		if (graphIsPending) {
			...
			if (graph == 0)
				...
				String filters = VSRC_BUFFER_NAME + "=" + size.width + ":" + size.height + ":" + pixFmt
						+ ":1:1000000:1:1," + "hflip," + "format=pix_fmts=" + pixFmt + "," + VSINK_FFSINK_NAME;
				long log_ctx = 0;
