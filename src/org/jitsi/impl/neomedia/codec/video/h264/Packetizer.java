...
public class Packetizer extends AbstractPacketizer {
	...
	static final Format[] SUPPORTED_OUTPUT_FORMATS = {
			new ParameterizedVideoFormat(Constants.H264_RTP, VideoMediaFormatImpl.H264_PACKETIZATION_MODE_FMTP, "0"),
			new ParameterizedVideoFormat(Constants.H264_RTP, VideoMediaFormatImpl.H264_PACKETIZATION_MODE_FMTP, "1") };
	 ...
	 *                   which the index of the beginning of the
	 *                   start_code_prefix_one_3bytes of a NAL unit is to be found
	...
	/**
	 * Initializes a new <tt>Packetizer</tt> instance which is to packetize H.264
	 * encoded data/NAL units into RTP packets in accord with RFC 3984 "RTP Payload
	...
	private Format[] getMatchingOutputFormats(Format input) {
		...
		return new Format[] { new ParameterizedVideoFormat(Constants.H264_RTP, size, Format.NOT_SPECIFIED,
				Format.byteArray, frameRate,
				ParameterizedVideoFormat.toMap(VideoMediaFormatImpl.H264_PACKETIZATION_MODE_FMTP, packetizationMode)) };
	}
	...
	/**
	 * Gets the value of the <tt>packetization-mode</tt> format parameter assigned
	 * by a specific <tt>Format</tt>.
	 *
	...
	private String getPacketizationMode(Format format) {
		...
		if (format instanceof ParameterizedVideoFormat)
			packetizationMode = ((ParameterizedVideoFormat) format)
					.getFormatParameter(VideoMediaFormatImpl.H264_PACKETIZATION_MODE_FMTP);
	...
	/**
	 * Packetizes a specific NAL unit of H.264 encoded data so that it becomes ready
	 * to be sent as the payload of RTP packets. If the specified NAL unit does not
	 * fit into a single RTP packet i.e. will not become a "Single NAL Unit Packet",
	 * splits it into "Fragmentation Units (FUs)" of type FU-A.
	 *
	 * @param nal       the bytes which contain the NAL unit of H.264 encoded data
	 *                  to be packetized
	 * @param nalOffset the offset in <tt>nal</tt> at which the NAL unit of H.264
	 *                  encoded data to be packetized begins
	 * @param nalLength the length in <tt>nal</tt> beginning at <tt>nalOffset</tt>
	 *                  of the NAL unit of H.264 encoded data to be packetized
	 * @return <tt>true</tt> if at least one RTP packet payload has been packetized
	 *         i.e. prepared for sending; otherwise, <tt>false</tt>
	 */
	private boolean packetizeNAL(byte[] nal, int nalOffset, int nalLength) {
		...
		while (nalLength > 0) {
			...
			/*
			 * Tests with Asterisk suggest that the fragments of a fragmented NAL unit must
			 * be with one and the same size. There is also a similar question on the
			 * x264-devel mailing list but, unfortunately, it is unanswered.
			 */
	...
	public int process(Buffer inBuffer, Buffer outBuffer) {
		...
		if (nals.size() > 0) {
			...
			if (nals.size() > 0)
				 ...
				 * cannot be the last NALs in an access unit should probably NOT be marked
				 * anyway.
				 */
				if (nal.length > 0) {
					...
					if ((nal_unit_type == 28 /* FU-A */) && (nal.length > 1)) {
						...
						if ((fuHeader & 0x40 /* End bit */) == 0) {
							/*
							 * A FU-A without the End bit cannot possibly be the last NAL unit of an access
							 * unit.
							 */
		...
		/*
		 * We need 3 bytes for start_code_prefix_one_3bytes and at least 1 byte for the
		 * NAL unit i.e. its octet serving as the payload header.
		 */
		if (inLength < 4) {
			outBuffer.setDiscard(true);
			reset();
		 ...
		 * start_code_prefix_one_3bytes. Refer to "B.1 Byte stream NAL unit syntax and
		 * semantics" of "ITU-T Rec. H.264 Advanced video coding for generic audiovisual
		...
		if (beginIndex < endIndex) {
			...
			for (int nextBeginIndex; (beginIndex < endIndex) && ((nextBeginIndex = ff_avc_find_startcode(inData,
	...
	public Format setOutputFormat(Format format) {
		...
		/*
		 * A Packetizer translates raw media data in RTP payloads. Consequently, the
		...
		if (fmtps.get(VideoMediaFormatImpl.H264_PACKETIZATION_MODE_FMTP) == null) {
			fmtps.put(VideoMediaFormatImpl.H264_PACKETIZATION_MODE_FMTP, getPacketizationMode(inputFormat));
		}
