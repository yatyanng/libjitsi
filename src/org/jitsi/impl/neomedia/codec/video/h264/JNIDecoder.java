...
public class JNIDecoder extends AbstractCodec {
	 ...
	 */
	private static final VideoFormat[] DEFAULT_OUTPUT_FORMATS = new VideoFormat[] {
			new AVFrameFormat(FFmpeg.PIX_FMT_YUV420P) };

	/**
	 * The <tt>Logger</tt> used by the <tt>JNIDecoder</tt> class and its instances
	 * to print debug-related information.
	 */
	...
	/**
	 * The <tt>AVFrame</tt> in which the video frame decoded from the encoded media
	 * data is stored.
	 */
	...
	/**
	 * The <tt>KeyFrameControl</tt> used by this <tt>JNIDecoder</tt> to control its
	...
	/**
	 * Initializes a new <tt>JNIDecoder</tt> instance which is to decode H.264 NAL
	 * units into frames in YUV format.
	 */
	public JNIDecoder() {
		inputFormats = new VideoFormat[] {
				/*
				 * Explicitly state both ParameterizedVideoFormat (to receive any format
				 * parameters which may be of concern to this JNIDecoder) and VideoFormat (to
				 * make sure that nothing breaks because of equality and/or matching tests
				 * involving ParameterizedVideoFormat).
				 */
				new ParameterizedVideoFormat(Constants.H264), new VideoFormat(Constants.H264) };
	 ...
	 * with which this <tt>JNIDecoder</tt> has been configured. For example, takes
	 * into account the format parameter <tt>sprop-parameter-sets</tt> if it is
	 * specified by the input <tt>Format</tt>.
	 */
	private void handleFmtps() {
		try {
			...
			if (f instanceof ParameterizedVideoFormat) {
				...
				String spropParameterSets = pvf.getFormatParameter(VideoMediaFormatImpl.H264_SPROP_PARAMETER_SETS_FMTP);
				...
				if (spropParameterSets != null) {
					...
					if (nals.size() != 0) {
						...
						/*
						 * In accord with RFC 6184 "RTP Payload Format for H.264 Video", place the NAL
						 * units conveyed by sprop-parameter-sets in the NAL unit stream to precede any
						 * other NAL units in decoding order.
						 */
						FFmpeg.avcodec_decode_video(avctx, avframe.getPtr(), got_picture, nals.toByteArray(),
								nals.size());
					}
				}
			}
			...
			/*
			 * Because the handling of format parameter is new at the time of this writing
			 * and it currently handles only the format parameter sprop-parameter-sets the
			 * failed handling of which will be made visible later on anyway, do not let it
			 * kill this JNIDecoder.
			 */
		} catch (Throwable t) {
	...
	/**
	 * Decodes H.264 media data read from a specific input <tt>Buffer</tt> into a
	 * specific output <tt>Buffer</tt>.
	 *
	 ...
	 * @return the <tt>Format</tt> of the media data to be input for processing in
	 *         this <tt>Codec</tt> if <tt>format</tt> is compatible with this
	...
	/**
	 * Sets the <tt>KeyFrameControl</tt> to be used by this <tt>DePacketizer</tt> as
	 * a means of control over its key frame-related logic.
	 *
	 * @param keyFrameControl the <tt>KeyFrameControl</tt> to be used by this
	 *                        <tt>DePacketizer</tt> as a means of control over its
	 *                        key frame-related logic
	 */
