...
import static org.jitsi.impl.neomedia.codec.video.h264.H264.NAL_PREFIX;
import static org.jitsi.impl.neomedia.codec.video.h264.H264.kFuA;
import static org.jitsi.impl.neomedia.codec.video.h264.H264.kFuAHeaderSize;
import static org.jitsi.impl.neomedia.codec.video.h264.H264.kIdr;
import static org.jitsi.impl.neomedia.codec.video.h264.H264.kNalHeaderSize;
import static org.jitsi.impl.neomedia.codec.video.h264.H264.kPps;
import static org.jitsi.impl.neomedia.codec.video.h264.H264.kSei;
import static org.jitsi.impl.neomedia.codec.video.h264.H264.kSps;
import static org.jitsi.impl.neomedia.codec.video.h264.H264.kStapA;
import static org.jitsi.impl.neomedia.codec.video.h264.H264.kStapAHeaderSize;
import static org.jitsi.impl.neomedia.codec.video.h264.H264.kTypeMask;
...
public class DePacketizer extends AbstractCodec2 {
	...
	/**
	 * The indicator which determines whether incomplete NAL units are output from
	 * the H.264 <tt>DePacketizer</tt> to the decoder. It is advisable to output
	 * incomplete NAL units because the FFmpeg H.264 decoder is able to decode them.
	 * If <tt>false</tt>, incomplete NAL units will be discarded and, consequently,
	 * the video quality will be worse (e.g. if the last RTP packet of a fragmented
	 * NAL unit carrying a keyframe does not arrive from the network, the whole
	 * keyframe will be discarded and thus all NAL units upto the next keyframe will
	 * be useless).
	 */
	private static final boolean OUTPUT_INCOMPLETE_NAL_UNITS = true;

	/**
	 * The interval of time in milliseconds between two consecutive requests for a
	 * key frame from the remote peer associated with the {@link #keyFrameControl}
	 * of <tt>DePacketizer</tt>.
	 */
	private static final long TIME_BETWEEN_REQUEST_KEY_FRAME = 500;

	/**
	 * The interval of time in milliseconds from the time of the last received key
	 * frame to the time at which a key frame will be requested from the remote peer
	 * associated with the {@link #keyFrameControl} of <tt>DePacketizer</tt>. The
	 * value at the time of this writing is the default time between two consecutive
	 * key frames generated by {@link JNIDecoder} with an addition of a certain
	 * fraction of that time in the role of a leeway to prevent
	 * <tt>DePacketizer</tt> from requesting key frames from <tt>JNIEncoder</tt> in
	 * the scenario of perfect transmission.
	 */
	private static final long TIME_FROM_KEY_FRAME_TO_REQUEST_KEY_FRAME = ((JNIEncoder.DEFAULT_KEYINT * 4L)
			/ (JNIEncoder.DEFAULT_FRAME_RATE * 3L)) * 1000L;

	/**
	 * The Unspecified <tt>nal_unit_type</tt> as defined by the ITU-T Recommendation
	 * for H.264.
	 */
	 ...
	 * "Fragmentation Unit (FU)" with its Start bit set and has not encountered one
	 * with its End bit set.
	 */
	private boolean fuaStartedAndNotEnded = false;

	/**
	 * The <tt>KeyFrameControl</tt> used by this <tt>DePacketizer</tt> to control
	...
	/**
	 * The time of the last request for a key frame from the remote peer associated
	 * with {@link #keyFrameControl} performed by this <tt>DePacketizer</tt>.
	 */
	...
	/**
	 * The <tt>nal_unit_type</tt> as defined by the ITU-T Recommendation for H.264
	 * of the last NAL unit given to this <tt>DePacketizer</tt> for processing. In
	 * the case of processing a fragmentation unit, the value is equal to the
	 * <tt>nal_unit_type</tt> of the fragmented NAL unit.
	 */
	...
	/**
	 * The <tt>Thread</tt> which is to asynchronously request key frames from the
	 * remote peer associated with {@link #keyFrameControl} on behalf of this
	 * <tt>DePacketizer</tt> and in accord with {@link #requestKeyFrame}.
	 */
	...
	public DePacketizer() {
		...
		/*
		 * Apart from the generic Constants.H264_RTP VideoFormat, add the possible
		 * respective ParameterizedVideoFormats because ParameterizedVideoFormat will
		 * not match every VideoFormat due to the fact that a missing packetization-mode
		 * format parameter is interpreted as it having a value of 0.
		 */
	 ...
	 *
	 * @param in        the payload of the RTP packet from which a FU-A fragment of
	 ...
	 *                  <tt>inOffset</tt>
	 * @param outBuffer the <tt>Buffer</tt> which is to receive the extracted FU-A
	 ...
	 *         {@link #process(Buffer, Buffer)}
	 */
	private int dePacketizeSingleNALUnitPacket(int nal_unit_type, byte[] in, int inOffset, int inLength,
			Buffer outBuffer) {
		this.nal_unit_type = nal_unit_type;
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		/*
		 * We'll only be depacketizing, we'll not act as an H.264 parser. Consequently,
		 * we'll only care about the rules of packetizing/depacketizing. For example,
		 * we'll have to make sure that no packets are lost and no other packets are
		 * received when depacketizing FU-A Fragmentation Units (FUs).
		 */
		...
		if ((lastSequenceNumber != -1) && ((sequenceNumber - lastSequenceNumber) != 1)) {
			/*
			 * Even if (the new) sequenceNumber is less than lastSequenceNumber, we have to
			 * use it because the received sequence numbers may have reached their maximum
			 * value and wrapped around starting from their minimum value again.
			 */
			...
			if ((ret & OUTPUT_BUFFER_NOT_FILLED) == 0) {
				/*
				 * TODO Do we have to reset the nal_unit_type field of this DePacketizer to
				 * UNSPECIFIED_NAL_UNIT_TYPE here? If ret contains INPUT_BUFFER_NOT_CONSUMED, it
				 * seems safe to not reset it because the input Buffer will be returned for
		...
		/*
		 * Ignore the RTP time stamp reported by JMF because it is not the actual RTP
		 * packet time stamp send by the remote peer but some locally calculated JMF
		 * value.
		 */
		...
		/*
		 * NRI equal to the binary value 00 indicates that the content of the NAL unit
		 * is not used to reconstruct reference pictures for inter picture prediction.
		 * Such NAL units can be discarded without risking the integrity of the
		 * reference pictures. However, it is not the place of the DePacketizer to take
		 * the decision to discard them but of the H.264 decoder.
		 */

		/*
		 * The nal_unit_type of the NAL unit given to this DePacketizer for processing.
		 * In the case of processing a fragmentation unit, the value is equal to the
		 * nal_unit_type of the fragmentation unit, not the fragmented NAL unit and is
		 * thus in contrast with the value of the nal_unit_type field of this
		 * DePacketizer.
		 */
		...
		case 5 /* Coded slice of an IDR picture */:
			...
			/*
			 * While it seems natural to not request a key frame in the presence of 5, 7 and
			 * 8 often seem to be followed by 5 so do not request a key frame if either 7 or
			 * 8 is present.
			 */
	...
	/**
	 * Checks if a fragment of a NAL unit from a specific FU-A RTP packet payload is
	 * keyframe or not.
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * Makes sure that the format parameters of a <tt>ParameterizedVideoFormat</tt>
	 * input which are of no concern to this <tt>DePacketizer</tt> get passed on
	 * through the output to the next <tt>Codec</tt> in the codec chain (i.e.
	 * <tt>JNIDecoder</tt>).
	 */
	@Override
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		if ((matchingOutputFormats != null) && (matchingOutputFormats.length != 0)
	 ...
	 * @return <tt>true</tt> if a key frame was indeed requested in response to the
	 ...
	 * process input RTP payloads. If the specified output <tt>Buffer</tt> contains
	 * an incomplete NAL unit, its forbidden_zero_bit will be turned on and the NAL
	 * unit in question will be output by this <tt>DePacketizer</tt>.
	 *
	...
	private int reset(Buffer outBuffer) {
		/*
		 * We need the octet at the very least. Additionally, it does not make sense to
		 * output a NAL unit with zero payload because such NAL units are only given
		 * meaning for the purposes of the network and not the H.264 decoder.
		 */
	...
	private void runInRequestKeyFrameThread() {
		while (true) {
			synchronized (this) {
				...
				if (requestKeyFrame) {
					/*
					 * If we have received at least one key frame, we may receive a new one later.
					 * So allow a certain amount of time for the new key frame to arrive without
					 * DePacketizer requesting it.
					 */
					long nextKeyFrameTime = lastKeyFrameTime + TIME_FROM_KEY_FRAME_TO_REQUEST_KEY_FRAME;

					if (now >= nextKeyFrameTime) {
						/*
						 * In order to not have the requests for key frames overwhelm the remote peer,
						 * make sure two consecutive requests are separated by a certain amount of time.
						 */
						...
						if (now >= nextRequestKeyFrameTime) {
							...
							/*
							 * Too little time has passed from our last attempt to request a key frame from
							 * the remote peer. If we do not wait, we risk intruding.
							 */
							timeout = nextRequestKeyFrameTime - now;
						}
					} else {
						/*
						 * Too little time has passed from the last receipt of a key frame to make us
						 * think that the remote peer will not send a key frame without us requesting
						 * it.
						 */
					...
					/*
					 * This DePacketizer has not expressed its desire to request a key frame from
					 * the remote peer so we will have to wait until it expresses the desire in
					 * question.
					 */
					timeout = 0;
				}
				...
				if (keyFrameRequesters != null) {
					for (KeyFrameControl.KeyFrameRequester keyFrameRequester : keyFrameRequesters) {
						try {
							...
							/*
							 * A KeyFrameRequester has malfunctioned, do not let it interfere with the
							 * others.
							 */
						}
					}
	...
	/**
	 * Sets the indicator which determines whether this <tt>DePacketizer</tt> is to
	 * request a key frame from the remote peer associated with
	 * {@link #keyFrameControl}.
	 *
	 * @param requestKeyFrame <tt>true</tt> if this <tt>DePacketizer</tt> is to
	 *                        request a key frame from the remote peer associated
	 *                        with {@link #keyFrameControl}
	 */
