...
public class JNIEncoder extends AbstractCodec implements RTCPFeedbackMessageListener {
	 ...
	 */
	public static final String[] AVAILABLE_PRESETS = { "ultrafast", "superfast", "veryfast", "faster", "fast", "medium",
	...
	/**
	 * The frame rate to be assumed by <tt>JNIEncoder</tt> instances in the absence
	 * of any other frame rate indication.
	 */
	...
	/**
	 * The default maximum GOP (group of pictures) size i.e. the maximum interval
	 * between keyframes. The x264 library defaults to 250.
	 */
	public static final int DEFAULT_KEYINT = 150;

	/**
	 * The default value of the {@link #PRESET_PNAME} <tt>ConfigurationService</tt>
	 * property.
	 */
	public static final String DEFAULT_PRESET = AVAILABLE_PRESETS[0];

	/**
	 * The name of the <tt>ConfigurationService</tt> property which specifies the
	 * H.264 (encoding) profile to be used in the absence of negotiation. Though it
	 * seems that RFC 3984 "RTP Payload Format for H.264 Video" specifies the
	 * baseline profile as the default, we have till the time of this writing
	 * defaulted to the main profile and we do not currently want to change from the
	 * main to the base profile unless we really have to.
	 */
	 ...
	 * specifies the maximum GOP (group of pictures) size i.e. the maximum interval
	 * between keyframes. FFmpeg calls it <tt>gop_size</tt>, x264 refers to it as
	 * <tt>keyint</tt> or <tt>i_keyint_max</tt>.
	 */
	...
	/**
	 * The name of the <tt>ConfigurationService</tt> property which specifies the
	 * x264 preset to be used by <tt>JNIEncoder</tt>. A preset is a collection of
	 * x264 options that will provide a certain encoding speed to compression ratio.
	 * A slower preset will provide better compression i.e. quality per size.
	 */
	public static final String PRESET_PNAME = "org.jitsi.impl.neomedia.codec.video.h264.preset";

	/**
	 * The list of <tt>Formats</tt> supported by <tt>JNIEncoder</tt> instances as
	 * output.
	 */
	...
	/**
	 * The indicator which determines whether the generation of a keyframe is to be
	 * forced during a subsequent execution of {@link #process(Buffer, Buffer)}. The
	 * first frame to undergo encoding is naturally a keyframe and, for the sake of
	 * clarity, the initial value is <tt>true</tt>.
	 */
	private boolean forceKeyFrame = true;

	/**
	 * The <tt>KeyFrameControl</tt> used by this <tt>JNIEncoder</tt> to control its
	...
	/**
	 * The maximum GOP (group of pictures) size i.e. the maximum interval between
	 * keyframes (with which {@link #open()} has been invoked without an intervening
	 * {@link #close()}). FFmpeg calls it <tt>gop_size</tt>, x264 refers to it as
	 * <tt>keyint</tt> or <tt>i_keyint_max</tt>.
	 */
	private int keyint;
	...
	/**
	 * The time in milliseconds of the last request for a key frame from the remote
	 * peer to this local peer.
	 */
	 ...
	 * <tt>JNIEncoder</tt> and the associated packetizer. RFC 3984 "RTP Payload
	 * Format for H.264 Video" says that "[w]hen the value of packetization-mode is
	 * equal to 0 or packetization-mode is not present, the single NAL mode, as
	 * defined in section 6.2 of RFC 3984, MUST be used."
	 */
	...
	/**
	 * Length of the raw frame buffer. Once the dimensions are known, this is set to
	 * 3/2 * (height*width), which is the size needed for a YUV420 frame.
	 */
	private int rawFrameLen;

	/**
	 * The indicator which determines whether two consecutive frames at the
	 * beginning of the video transmission have been encoded as keyframes. The first
	 * frame is a keyframe but it is at the very beginning of the video transmission
	 * and, consequently, there is a higher risk that pieces of it will be lost on
	 * their way through the network. To mitigate possible issues in the case of
	...
	public JNIEncoder() {
		inputFormats = new Format[] {
				new YUVFormat(/* size */ null, /* maxDataLength */ Format.NOT_SPECIFIED, Format.byteArray,
						/* frameRate */ Format.NOT_SPECIFIED, YUVFormat.YUV_420, /* strideY */ Format.NOT_SPECIFIED,
						/* strideUV */ Format.NOT_SPECIFIED, /* offsetY */ Format.NOT_SPECIFIED,
						/* offsetU */ Format.NOT_SPECIFIED, /* offsetV */ Format.NOT_SPECIFIED) };
	...
	private Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		for (int index = packetizationModes.length - 1; index >= 0; index--) {
			matchingOutputFormats[index] = new ParameterizedVideoFormat(Constants.H264, size,
					/* maxDataLength */ Format.NOT_SPECIFIED, Format.byteArray, frameRate, ParameterizedVideoFormat
							.toMap(VideoMediaFormatImpl.H264_PACKETIZATION_MODE_FMTP, packetizationModes[index]));
		}
	...
	/**
	 * Determines whether the encoding of {@link #avFrame} is to produce a keyframe.
	 * The returned value will be set on <tt>avFrame</tt> via a call to
	...
	private boolean isKeyFrame() {
		...
		if (forceKeyFrame) {
			 ...
			 * transmission and, consequently, there is a higher risk that pieces of it will
			 * be lost on their way through the network. To mitigate possible issues in the
			 * case of network loss, the second frame is also a keyframe.
			 */
			if (secondKeyFrame) {
			...
			/*
			 * In order to be sure that keyint will be respected, we will implement it
			 * ourselves (regardless of the fact that we have told FFmpeg and x264 about
			 * it). Otherwise, we may end up not generating keyframes at all (apart from the
			 * two generated after open).
			 */
	...
	/**
	 * Notifies this <tt>JNIEncoder</tt> that the remote peer has requested a key
	 * frame from this local peer.
	 *
	 * @return <tt>true</tt> if this <tt>JNIEncoder</tt> has honored the request for
	 *         a key frame; otherwise, <tt>false</tt>
	 */
	...
	public synchronized void open() throws ResourceUnavailableException {
		...
		/*
		 * An Encoder translates raw media data in (en)coded media data. Consequently,
		...
		/*
		 * XXX We do not currently negotiate the profile so, regardless of the many
		 * AVCodecContext properties we have set above, force the default profile
		 * configuration.
		 */
		...
		if (intraRefresh) {
			 ...
			 * "intra-refresh" option for them. Anyway, the flag is set for the older FFmpeg
			 * versions.
			 */
		...
		if (FFmpeg.avcodec_open2(avctx, avcodec,
				/*
				 * XXX crf=0 means lossless coding which is not supported by the baseline and
				 * main profiles. Consequently, we cannot specify it because we specify either
				 * the baseline or the main profile. Otherwise, x264 will detect the
				 * inconsistency in the specified parameters/options and FFmpeg will fail.
				 */
				// "crf" /* constant quality mode, constant ratefactor */, "0",
				"intra-refresh", intraRefresh ? "1" : "0", "keyint", Integer.toString(keyint), "partitions",
				"b8x8,i4x4,p8x8", "preset", preset, "thread_type", "slice", "tune", "zerolatency") < 0) {
		...
		/*
		 * Implement the ability to have the remote peer request key frames from this
		 * local peer.
		 */
		if (keyFrameRequestee == null) {
	...
	/**
	 * Notifies this <tt>RTCPFeedbackListener</tt> that an RTCP feedback message has
	 * been received
	 *
	 * @param ev an <tt>RTCPFeedbackMessageEvent</tt> which specifies the details of
	 *           the notification event such as the feedback message type and the
	 *           payload type
	 */
	@Override
	public void rtcpFeedbackMessageReceived(RTCPFeedbackMessageEvent ev) {
		/*
		 * If RTCP message is a Picture Loss Indication (PLI) or a Full Intra-frame
		 * Request (FIR) the encoder will force the next frame to be a keyframe.
		 */
	 ...
	 *         <tt>null</tt> if the specified <tt>format</tt> is not supported by
	 *         this <tt>Codec</tt>
	 */
	@Override
	...
	/**
	 * Sets the <tt>KeyFrameControl</tt> to be used by this <tt>JNIEncoder</tt> as a
	 ...
	 * data.
	 *
	 * @param format the <tt>Format</tt> in which this <tt>Codec</tt> is to output
	 *               media data
	 * @return the <tt>Format</tt> in which this <tt>Codec</tt> is currently
	 *         configured to output media data or <tt>null</tt> if <tt>format</tt>
	 *         was found to be incompatible with this <tt>Codec</tt>
	 */
	@Override
	public Format setOutputFormat(Format format) {
		...
		outputFormat = new ParameterizedVideoFormat(videoFormat.getEncoding(), size,
				/* maxDataLength */ Format.NOT_SPECIFIED, Format.byteArray, videoFormat.getFrameRate(), fmtps);
	...
	/**
	 * Sets the packetization mode to be used for the H.264 RTP payload output by
	 * this <tt>JNIEncoder</tt> and the associated packetizer.
	 *
	 * @param packetizationMode the packetization mode to be used for the H.264 RTP
	 *                          payload output by this <tt>JNIEncoder</tt> and the
	...
	public void setPacketizationMode(String packetizationMode) {
		 ...
		 * single NAL mode, as defined in section 6.2 of RFC 3984, MUST be used."
		 */
