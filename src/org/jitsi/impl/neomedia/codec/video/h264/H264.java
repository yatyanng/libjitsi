...
public class H264 {
	/**
	 * The bytes to prefix any NAL unit to be output by this <tt>DePacketizer</tt>
	 * and given to a H.264 decoder. Includes start_code_prefix_one_3bytes.
	 * According to "B.1 Byte stream NAL unit syntax and semantics" of "ITU-T Rec.
	 * H.264 Advanced video coding for generic audiovisual services", zero_byte
	 * "shall be present" when "the nal_unit_type within the nal_unit() is equal to
	 * 7 (sequence parameter set) or 8 (picture parameter set)" or "the byte stream
	 * NAL unit syntax structure contains the first NAL unit of an access unit in
	 * decoding order".
	 */
	...
	/**
	 * Check if Single-Time Aggregation Packet (STAP-A) NAL unit is correctly
	 * formed.
	 *
