...
public class SwScale extends AbstractCodec {
	/**
	 * The <tt>Logger</tt> used by the <tt>SwScale</tt> class and its instances for
	 * logging output.
	 */
	private static final Logger logger = Logger.getLogger(SwScale.class);

	/**
	 * The minimum height and/or width of the input and/or output to be passed to
	 * <tt>sws_scale</tt> in order to prevent its crashing.
	 */
	...
	/**
	 * Gets a <tt>VideoFormat</tt> with a specific size i.e. width and height using
	 * a specific <tt>VideoFormat</tt> as a template.
	 *
	 ...
	 *               be returned
	 * @return a <tt>VideoFormat</tt> with the specified <tt>size</tt> and based on
	 *         the specified <tt>format</tt>
	 */
	private static VideoFormat setSize(VideoFormat format, Dimension size) {
		/*
		 * Since the size of the Format has changed, its size-related properties should
		 * change as well. Format#intersects doesn't seem to be cool because it
		 * preserves them and thus the resulting Format is inconsistent.
		 */
		if (format instanceof RGBFormat) {
			...
			format = new RGBFormat(size, /* maxDataLength */ Format.NOT_SPECIFIED, dataType, format.getFrameRate(),
					bitsPerPixel, rgbFormat.getRedMask(), rgbFormat.getGreenMask(), rgbFormat.getBlueMask(),
					pixelStride,
			...
			format = new YUVFormat(size, /* maxDataLength */ Format.NOT_SPECIFIED, format.getDataType(),
					format.getFrameRate(), yuvFormat.getYuvType(), /* strideY */ Format.NOT_SPECIFIED,
					/* strideUV */ Format.NOT_SPECIFIED, 0, /* offsetU */ Format.NOT_SPECIFIED,
					/* offsetV */ Format.NOT_SPECIFIED);
		} else if (format != null) {
			logger.warn("SwScale outputFormat of type " + format.getClass().getName()
	...
	/**
	 * The indicator which determines whether this scaler will attempt to keep the
	 * width and height of YUV 420 output even.
	 */
	private final boolean fixOddYuv420Size;

	/**
	 * The <tt>FrameProcessingControl</tt> of this <tt>Codec</tt> which allows JMF
	 * to instruct it to drop frames because it's behind schedule.
	 */
	 ...
	 * processed. If <tt>true</tt>, the <tt>size</tt> of the <tt>outputFormat</tt>
	 * of this instance is used to device a rectangle into which a scaled video
	 * frame should fit with the input aspect ratio preserved.
	 */
	 ...
	 */
	private VideoFormat[] supportedOutputFormats = new VideoFormat[] { new RGBFormat(),
	...
	/**
	 * Initializes a new <tt>SwScale</tt> instance which doesn't have an output size
	 * and will use a default one when it becomes necessary unless an explicit one
	 * is specified in the meantime.
	 */
	public SwScale() {
		this(false);
	}
	...
	/**
	 * Initializes a new <tt>SwScale</tt> instance which can optionally attempt to
	 * keep the width and height of YUV 420 output even.
	 *
	 * @param fixOddYuv420Size <tt>true</tt> to have the new instance keep the width
	 *                         and height of YUV 420 output even; otherwise,
	 *                         <tt>false</tt>
	 */
	...
	/**
	 * Initializes a new <tt>SwScale</tt> instance which can optionally attempt to
	 * keep the width and height of YUV 420 output even and to preserve the aspect
	 * ratio of the video frames provided to the instance as input to be processed.
	 *
	 * @param fixOddYuv420Size    <tt>true</tt> to have the new instance keep the
	 *                            width and height of YUV 420 output even;
	 *                            otherwise, <tt>false</tt>
	 * @param preserveAspectRatio <tt>true</tt> to have the new instance preserve
	 *                            the aspect ratio of the video frames provided to
	 *                            it as input to be processed; otherwise,
	 *                            <tt>false</tt>
	 */
	...
	/**
	 * Gets the <tt>Format</tt> in which this <tt>Codec</tt> is currently configured
	 * to accept input media data.
	 * <p>
	...
	public Format[] getSupportedOutputFormats(Format input) {
		...
		/*
		 * if size is set for element 0 (YUVFormat), it is also set for element 1
		 * (RGBFormat) and so on...
		 */
		Dimension size = supportedOutputFormats[0].getSize();

		/*
		 * no specified size set so return the same size as input in output format
		 * supported
		 */
		...
		return new Format[] {
				new RGBFormat(size, /* maxDataLength */ Format.NOT_SPECIFIED, /* dataType */ null, frameRate, 32,
						/* red */ Format.NOT_SPECIFIED, /* green */ Format.NOT_SPECIFIED,
						/* blue */ Format.NOT_SPECIFIED),
				new YUVFormat(size, /* maxDataLength */ Format.NOT_SPECIFIED, /* dataType */ null, frameRate,
						YUVFormat.YUV_420, /* strideY */ Format.NOT_SPECIFIED, /* strideUV */ Format.NOT_SPECIFIED,
						/* offsetY */ Format.NOT_SPECIFIED, /* offsetU */ Format.NOT_SPECIFIED,
						/* offsetV */ Format.NOT_SPECIFIED) };
	}

	/**
	 * Calculates an output size which has the aspect ratio of a specific input size
	 * and fits into a specific output size.
	 *
	 ...
	 * @return an output size which has the aspect ratio of the specified input size
	 *         and fits into the specified output size
	 */
	private Dimension preserveAspectRatio(Dimension in, Dimension out) {
		...
		/*
		 * Reduce the effects of allocation and garbage collection by caching the
		...
		if ((preserveAspectRatioCachedIn != null) && (preserveAspectRatioCachedOut != null)
				...
				&& (preserveAspectRatioCachedOut.width == outWidth) && (preserveAspectRatioCachedRet != null))
		...
		if (scale) {
			...
			/*
			 * Preserve the aspect ratio only if it is going to make noticeable differences
			 * in height and/or width; otherwise, play it safe.
			 */
	...
	/**
	 * Processes (converts color space and/or scales) an input <tt>Buffer</tt> into
	 * an output <tt>Buffer</tt>.
	 *
	...
	public int process(Buffer in, Buffer out) {
		...
		if (outFormat == null) {
			/*
			 * The format of the output Buffer is not documented to be used as input to the
			 * #process method. Anyway, we're trying to use it in case this Codec doesn't
			 * have an outputFormat set which is unlikely to ever happen.
			 */
		...
		if (src instanceof AVFrame) {
			...
			srcFmt = (inFormat instanceof YUVFormat) ? FFmpeg.PIX_FMT_YUV420P
					: getFFmpegPixelFormat((RGBFormat) inFormat);
			srcPicture = 0;
		}

		swsContext = FFmpeg.sws_getCachedContext(swsContext, inWidth, inHeight, srcFmt, outWidth, outHeight, dstFmt,
				FFmpeg.SWS_BICUBIC);
	...
	public Format setInputFormat(Format format) {
		Format inputFormat = (format instanceof VideoFormat) ? super.setInputFormat(format)
				: null /* The input must be video, a size is not required. */;

		if ((inputFormat != null) && logger.isDebugEnabled()) {
			logger.debug(
					getClass().getName() + " 0x" + Integer.toHexString(hashCode()) + " set to input in " + inputFormat);
		}
		return inputFormat;
	}
	...
	public Format setOutputFormat(Format format) {
			...
			if (YUVFormat.YUV_420 == yuvFormat.getYuvType()) {
				...
				if ((size != null) && (size.width > 2) && (size.height > 2)) {
					...
					if ((width != size.width) || (height != size.height)) {
						format = new YUVFormat(new Dimension(width, height), /* maxDataLength */ Format.NOT_SPECIFIED,
								yuvFormat.getDataType(), yuvFormat.getFrameRate(), yuvFormat.getYuvType(),
								/* strideY */ Format.NOT_SPECIFIED, /* strideUV */ Format.NOT_SPECIFIED, 0,
								/* offsetU */ Format.NOT_SPECIFIED, /* strideV */ Format.NOT_SPECIFIED);
					}
				}
			}
		}
		...
		if ((outputFormat != null) && logger.isDebugEnabled()) {
			logger.debug(getClass().getName() + " 0x" + Integer.toHexString(hashCode()) + " set to output in "
					+ outputFormat);
		}
		return outputFormat;
	}
	...
	/**
	 * Sets the size i.e. width and height of the current <tt>outputFormat</tt> of
	 * this <tt>SwScale</tt>
	 *
	...
	public void setOutputSize(Dimension size) {
		/*
		 * If the specified output size is tiny enough to crash sws_scale, do not accept
		 * it.
		 */
		if ((size != null)
