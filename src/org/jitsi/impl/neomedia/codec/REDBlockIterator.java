...
public class REDBlockIterator implements Iterator<REDBlock> {
	/**
	 * The <tt>Logger</tt> used by the <tt>REDBlockIterator</tt> class and its
	...
	public static REDBlock getPrimaryBlock(byte[] buffer, int offset, int length) {
		...
		if (isMultiBlock(buffer, offset, length)) {
			...
			if (buffer == null || offset < 0 || length < 1 || buffer.length < offset + length) {
				logger.warn(
						"Prevented an array out of bounds exception." + " offset: " + offset + ", length: " + length);
				return null;
			}
	 ...
	 * @param offset the offset in the buffer where the RED payload begins.
	 * @param length the length of the RED payload.
	 * @return {@code true if {@pkt} contains multiple RED blocks; otherwise, {@code
	 *         false}
	 */
