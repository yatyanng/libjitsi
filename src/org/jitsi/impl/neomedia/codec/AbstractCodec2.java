...
public abstract class AbstractCodec2 extends AbstractCodec {
	/**
	 * The <tt>Buffer</tt> flag which indicates that the respective <tt>Buffer</tt>
	 * contains audio data which has been decoded as a result of the operation of
	 * FEC.
	 */
	...
	/**
	 * An empty array of <tt>Format</tt> element type. Explicitly defined to reduce
	 ...
	 * mitigation techniques such as Forward Error Correction (FEC) and Packet Loss
	 * Concealment (PLC) when dealing with audio.
	 */
	...
	/**
	 * Calculates the number of sequences which have been lost i.e. which have not
	 * been received.
	 *
	 ...
	 *                  <tt>seqNo</tt> for the purposes of Codec implementations
	...
	public static int calculateLostSeqNoCount(long lastSeqNo, long seqNo) {
		...
		/*
		 * We explicitly allow the same sequence number to be received multiple times
		 * for the purposes of Codec implementations which repeatedly process one and
		 * the same input Buffer multiple times.
		 */
		if (delta == 0)
			return 0;
		else if (delta > 0)
	...
	/**
	 * Increments a specific sequence number and makes sure that the result stays
	 * within the range of valid RTP sequence number values.
	 *
	 * @param seqNo the sequence number to increment
	 * @return a sequence number which represents an increment over the specified
	 *         <tt>seqNo</tt> within the range of valid RTP sequence number values
	 */
	...
	public static YUVFormat specialize(YUVFormat yuvFormat, Class<?> dataType) {
		...
		if ((offsetV == Format.NOT_SPECIFIED) && (offsetU != Format.NOT_SPECIFIED) && (strideUV != Format.NOT_SPECIFIED)
				&& (size != null))
			offsetV = offsetU + strideUV * ((size.height + 1) / 2);

		int maxDataLength = ((strideY != Format.NOT_SPECIFIED) && (strideUV != Format.NOT_SPECIFIED)) && (size != null)
				? (strideY * size.height + 2 * strideUV * ((size.height + 1) / 2) + FFmpeg.FF_INPUT_BUFFER_PADDING_SIZE)
				: Format.NOT_SPECIFIED;

		return new YUVFormat(size, maxDataLength, (dataType == null) ? yuvFormat.getDataType() : dataType,
				yuvFormat.getFrameRate(), YUVFormat.YUV_420, strideY, strideUV, offsetY, offsetU, offsetV);
	}
	 ...
	 *
	 * @param buffer    the <tt>Buffer</tt> whose <tt>data</tt> property value is to
	 *                  be validated
	 * @param newSize   the minimum length of the array of <tt>byte</tt> which is to
	 *                  be the value of the <tt>data</tt> property of
	 *                  <tt>buffer</tt>
	 * @param arraycopy <tt>true</tt> to copy the bytes which are in the value of
	 *                  the <tt>data</tt> property of <tt>buffer</tt> at the time of
	 *                  the invocation of the method if the value of the
	 *                  <tt>data</tt> property of <tt>buffer</tt> is an array of
	 *                  <tt>byte</tt> whose length is less than <tt>newSize</tt>;
	 *                  otherwise, <tt>false</tt>
	 * @return an array of <tt>byte</tt>s which is the value of the <tt>data</tt>
	 *         property of <tt>buffer</tt> and whose length is at least
	 ...
	 * {@link #process(Buffer,Buffer)}. Introduced for the purposes of debugging at
	 ...
	 * <tt>Format</tt>s and a specific list of <tt>Format</tt>s supported as output.
	 *
	 ...
	 * @param supportedOutputFormats the list of <tt>Format</tt>s supported by the
	 *                               new instance as output
	 */
	protected AbstractCodec2(String name, Class<? extends Format> formatClass, Format[] supportedOutputFormats) {
		...
		/*
		 * An Effect is a Codec that does not modify the Format of the data, it modifies
		 * the contents.
		 */
	 ...
	 * operate. A call to {@link PlugIn#open()} on this instance will result in a
	 * call to <tt>doOpen</tt> only if {@link AbstractCodec#opened} is
	 * <tt>false</tt>. All required input and/or output formats are assumed to have
	 * been set on this <tt>Codec</tt> before <tt>doOpen</tt> is called.
	 *
	 * @throws ResourceUnavailableException if any of the resources that this
	 *                                      <tt>Codec</tt> needs to operate cannot
	 *                                      be acquired
	 */
	...
	/**
	 * Opens this <tt>PlugIn</tt> software or hardware component and acquires the
	 * resources that it needs to operate. All required input and/or output formats
	 * have to be set on this <tt>PlugIn</tt> before <tt>open</tt> is called.
	 * Buffers should not be passed into this <tt>PlugIn</tt> without first calling
	 * <tt>open</tt>.
	 *
	 * @throws ResourceUnavailableException if any of the resources that this
	 *                                      <tt>PlugIn</tt> needs to operate cannot
	 *                                      be acquired
	...
	public int process(Buffer inBuf, Buffer outBuf) {
		...
		if (((process & BUFFER_PROCESSED_FAILED) != 0) || ((process & OUTPUT_BUFFER_NOT_FILLED)) != 0) {
			outLenProcessed = 0;
		} else {
