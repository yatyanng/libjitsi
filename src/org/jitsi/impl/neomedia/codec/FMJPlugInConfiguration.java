...
//            "org.jitsi.impl.neomedia.codec.AndroidMediaCodec",
			OSUtils.IS_ANDROID ? "org.jitsi.impl.neomedia.codec.video.AndroidEncoder" : null,
			OSUtils.IS_ANDROID ? "org.jitsi.impl.neomedia.codec.video.AndroidDecoder" : null,
			...
			EncodingConfigurationImpl.G729 ? "org.jitsi.impl.neomedia.codec.audio.g729.JavaDecoder" : null,
			EncodingConfigurationImpl.G729 ? "org.jitsi.impl.neomedia.codec.audio.g729.JavaEncoder" : null,
			"org.jitsi.impl.neomedia.codec.audio.g722.JNIDecoderImpl",
			"org.jitsi.impl.neomedia.codec.audio.g722.JNIEncoderImpl",
			"org.jitsi.impl.neomedia.codec.audio.gsm.Decoder", "org.jitsi.impl.neomedia.codec.audio.gsm.Encoder",
			...
			"org.jitsi.impl.neomedia.codec.video.vp8.Packetizer", "org.jitsi.impl.neomedia.codec.video.vp8.VPXDecoder",
			"org.jitsi.impl.neomedia.codec.video.vp8.VPXEncoder", };

	/**
	 * The additional custom JMF codecs, which depend on ffmpeg and should therefore
	 * only be used when ffmpeg is enabled.
	 */
	private static final String[] CUSTOM_CODECS_FFMPEG = {
			...
			"org.jitsi.impl.neomedia.codec.video.h264.Packetizer", "org.jitsi.impl.neomedia.codec.video.SwScale",
	 ...
	 */
	private static final String[] CUSTOM_PACKAGES = { "org.jitsi.impl.neomedia.jmfext",
	 ...
	 */
	private static final String[] CUSTOM_MULTIPLEXERS = { "org.jitsi.impl.neomedia.recording.BasicWavMux" };
	...
	public static void registerCustomCodecs(boolean enableFfmpeg) {
		...
		String[] gsmCodecClasses = new String[] { "JavaDecoder", "JavaDecoder_ms", "JavaEncoder", "JavaEncoder_ms",
				"NativeDecoder", "NativeDecoder_ms", "NativeEncoder", "NativeEncoder_ms", "Packetizer" };
		for (String gsmCodecClass : gsmCodecClasses) {
			PlugInManager.removePlugIn(gsmCodecPackage + gsmCodecClass, PlugInManager.CODEC);
		}

		/*
		 * Remove FMJ's JavaSoundCodec because it seems to slow down the building of the
		 * filter graph and we do not currently seem to need it.
		 */
		...
		for (String className : customCodecs) {
			...
			/*
			 * A codec with a className of null is configured at compile time to not be
			 * registered.
			 */
			if (className == null)
				continue;
			...
			if (registeredPlugins.contains(className)) {
				...
				try {
					...
					registered = PlugInManager.addPlugIn(className, codec.getSupportedInputFormats(),
							codec.getSupportedOutputFormats(null), PlugInManager.CODEC);
				} catch (Throwable ex) {
					registered = false;
					exception = ex;
				}
		...
		/*
		 * If Jitsi provides a codec which is also provided by FMJ and/or JMF, use
		 * Jitsi's version.
		 */
		@SuppressWarnings("unchecked")
	...
	public static void registerCustomPackages() {
		...
		for (int i = CUSTOM_PACKAGES.length - 1; i >= 0; i--) {
			...
			/*
			 * Linear search in a loop but it doesn't have to scale since the list is always
			 * short.
			 */
	...
	public static void registerCustomMultiplexers() {
		...
		Collection<String> registeredMuxers = new HashSet<String>(
				PlugInManager.getPlugInList(null, null, PlugInManager.MULTIPLEXER));
		...
		for (String className : CUSTOM_MULTIPLEXERS) {
			...
			try {
				...
				registered = PlugInManager.addPlugIn(className, multiplexer.getSupportedInputFormats(),
						multiplexer.getSupportedOutputContentDescriptors(null), PlugInManager.MULTIPLEXER);
			} catch (Throwable ex) {
				registered = false;
				exception = ex;
			}
