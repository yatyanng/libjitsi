...
public class JavaEncoder extends com.ibm.media.codec.audio.AudioCodec {
	...
	public JavaEncoder() {
		...
		defaultOutputFormats = new AudioFormat[] {
				new AudioFormat(AudioFormat.ULAW, 8000, 8, 1, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED) };
	...
	private void convert(byte[] input, int inputOffset, int inputLength, byte[] outData, int outputOffset) {
		...
		for (i = inputOffset + msbOffset; i < (inputLength + inputOffset);) {
			...
			if (8 == inputSampleSize) {
				...
				if (downmix) {
					inputSample = ((inputSample & signMask)
							+ (((input[i] << 8) + (0xff & input[i + lsbOffset])) & signMask)) >> 1;
					i += 2;
				}
			}
	...
	protected Format[] getMatchingOutputFormats(Format in) {
		...
		if (channels == 2) {
			supportedOutputFormats = new AudioFormat[] {
					new AudioFormat(AudioFormat.ULAW, sampleRate, 8, 2, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED),
					new AudioFormat(AudioFormat.ULAW, sampleRate, 8, 1, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED) };
