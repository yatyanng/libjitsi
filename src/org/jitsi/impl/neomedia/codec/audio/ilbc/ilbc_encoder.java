			 ...
			 */
				...
				if (lTarget == ilbc_constants.SUBL) {
					...
					if (sInd < 0) {
						...
						if (eInd > range) {
							sInd -= (eInd - range);
							// System.out.println("on retire " + eInd + " - " + range + " pour arriver a " +
							// sInd);
							eInd = range;
						}
...
//                        pp=cbvectors+filterno*lMem-lTarget-index[stage]+filterno*base_size;
			...
			float mem[]) /* (i/o) the filter state */
	{
		...
		for (i = 0; i < len; i++) {
			...
			// * " + In[pi] + "((" + ilbc_constants.hpi_zero_coefsTbl[0] * In[pi]);
