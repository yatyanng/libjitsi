...
public class JavaEncoder extends AbstractCodec2 implements FormatParametersAwareCodec {
	...
	/**
	 * The input from previous calls to {@link #doProcess(Buffer, Buffer)} which has
	 ...
	 */
	public JavaEncoder() {
		super("iLBC Encoder", AudioFormat.class, new Format[] {
				new AudioFormat(Constants.ILBC_RTP, 8000, 16, 1, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED) });
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		...
		if (inLen >= this.inLen) {
			/*
			 * If we are about to encode from prevInput, we already have prevInputLength
			 * taken into consideration by using prevInput in the first place and we have to
			 * make sure that we will not use the same prevInput more than once.
			 */
			prevInLen = 0;
