...
public class JavaDecoder extends AbstractCodec2 {
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if (offsets.size() == 0
				&& ((inputLength > ilbc_constants.NO_OF_BYTES_20MS && inputLength != ilbc_constants.NO_OF_BYTES_30MS)
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new AudioFormat[] { new AudioFormat(AudioFormat.LINEAR, inputAudioFormat.getSampleRate(), 16, 1,
				AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED) };
	}

	/**
	 * Initializes {@link #dec} so that it processes a specific number of bytes as
	 * input.
	 *
