...
public class SpeexResampler extends AbstractCodec2 {
	 ...
	 * <tt>SpeexResampler</tt> instances.
	 */
	private static final double[] SUPPORTED_SAMPLE_RATES = new double[] { 8000, 11025, 12000, 16000, 22050, 24000,
			32000, 44100, 48000, Format.NOT_SPECIFIED };
	...
	static {
		...
		for (int i = 0; i < supportedCount; i++) {
			...
			SUPPORTED_FORMATS[j] = new AudioFormat(AudioFormat.LINEAR, SUPPORTED_SAMPLE_RATES[i],
					16 /* sampleSizeInBits */, 1 /* channels */, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
					Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */, Format.byteArray);
			SUPPORTED_FORMATS[j + 1] = new AudioFormat(AudioFormat.LINEAR, SUPPORTED_SAMPLE_RATES[i],
					16 /* sampleSizeInBits */, 1 /* channels */, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
					Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */,
					Format.shortArray);
			SUPPORTED_FORMATS[j + 2] = new AudioFormat(AudioFormat.LINEAR, SUPPORTED_SAMPLE_RATES[i],
					16 /* sampleSizeInBits */, 2 /* channels */, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
					Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */, Format.byteArray);
			SUPPORTED_FORMATS[j + 3] = new AudioFormat(AudioFormat.LINEAR, SUPPORTED_SAMPLE_RATES[i],
					16 /* sampleSizeInBits */, 2 /* channels */, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
					Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */,
					Format.shortArray);
		}
	...
	/**
	 * The pointer to the native <tt>SpeexResamplerState</tt> which is represented
	 * by this instance.
	 */
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		...
		if (inSampleRate == outSampleRate) {
			...
			/*
			 * XXX The method Speex.speex_resampler_process_interleaved_int will crash if in
			 * is null.
			 */
			if (inSampleCount == 0) {
				outSampleCount = 0;
				...
				outSampleCount = Speex.speex_resampler_process_interleaved_int(resampler, in, inOffset, inSampleCount,
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		if (inputFormat instanceof AudioFormat) {
			...
			for (Format supportedFormat : SUPPORTED_FORMATS) {
				...
				if ((Format.byteArray.equals(supportedFormat.getDataType()) && Format.byteArray.equals(inDataType))
	...
	public Format setInputFormat(Format format) {
		...
		if (inFormat != null) {
			...
			if (outputFormat == null) {
				...
				/*
				 * Conversion between data types is only supported when not resampling but
				 * rather passing through.
				 */
				if (outSampleRate != inFormat.getSampleRate())
					outDataType = inFormat.getDataType();
			}

			setOutputFormat(new AudioFormat(inFormat.getEncoding(), outSampleRate, inFormat.getSampleSizeInBits(),
					inFormat.getChannels(), inFormat.getEndian(), inFormat.getSigned(), Format.NOT_SPECIFIED,
