...
public class JNIEncoder extends AbstractCodec2 {
	 ...
	 * <tt>JNIEncoder</tt> instances.
	 */
	private static final Format[] SUPPORTED_OUTPUT_FORMATS = new Format[] { new AudioFormat(Constants.SPEEX_RTP) };
	...
	static {
		...
		for (int i = 0; i < supportedInputCount; i++) {
			SUPPORTED_INPUT_FORMATS[i] = new AudioFormat(AudioFormat.LINEAR, SUPPORTED_INPUT_SAMPLE_RATES[i], 16, 1,
					AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED,
					Format.byteArray);
		}
	}
	...
	/**
	 * The number of bytes from an input <tt>Buffer</tt> that this <tt>Codec</tt>
	 * processes in one call of its {@link #process(Buffer, Buffer)}.
	 */
	 ...
	 * because the total number of bytes was less than {@link #frameSize} and need
	 * to be prepended to a subsequent input <tt>Buffer</tt> in order to process a
	 * total of {@link #frameSize} bytes.
	 */
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		/*
		 * Make sure that the native Speex encoder which is represented by this instance
		 * is configured to work with the inputFormat.
		 */
		...
		if (state == 0) {
			long mode = Speex.speex_lib_get_mode((inputSampleRate == 16000) ? Speex.SPEEX_MODEID_WB
		...
		/*
		 * The native Speex encoder always processes frameSize bytes from the input in
		 * one call. If any specified inputBuffer is with a different length, then we'll
		 * have to wait for more bytes to arrive until we have frameSize bytes. Remember
		 * whatever is left unprocessed in previousInput and prepend it to the next
		 * inputBuffer.
		 */
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new Format[] { new AudioFormat(Constants.SPEEX_RTP, inputAudioFormat.getSampleRate(),
				Format.NOT_SPECIFIED, 1, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED,
	...
	public Format getOutputFormat() {
		...
		if ((outputFormat != null) && (outputFormat.getClass() == AudioFormat.class)) {
			...
			outputFormat = setOutputFormat(new AudioFormat(outputAudioFormat.getEncoding(),
					...
					outputAudioFormat.getChannels(), outputAudioFormat.getEndian(), outputAudioFormat.getSigned(),
					outputAudioFormat.getFrameSizeInBits(), outputAudioFormat.getFrameRate(),
	...
	public Format setInputFormat(Format format) {
		...
		if (inputFormat != null) {
			...
			if ((outputSampleRate != inputSampleRate) || (outputChannels != inputChannels)) {
				setOutputFormat(new AudioFormat(Constants.SPEEX_RTP, inputSampleRate, Format.NOT_SPECIFIED,
						inputChannels, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED,
						Format.NOT_SPECIFIED, Format.byteArray));
			}
		}
		return inputFormat;
