...
public class JNIDecoder extends AbstractCodec2 {
	...
	private static final Format[] SUPPORTED_OUTPUT_FORMATS = new Format[] {
			new AudioFormat(AudioFormat.LINEAR, Format.NOT_SPECIFIED, 16, 1, AudioFormat.LITTLE_ENDIAN,
	...
	static {
		...
		for (int i = 0; i < supportedInputCount; i++) {
			SUPPORTED_INPUT_FORMATS[i] = new AudioFormat(Constants.SPEEX_RTP, SUPPORTED_INPUT_SAMPLE_RATES[i],
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		/*
		 * Make sure that the native Speex decoder which is represented by this instance
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new Format[] {
				new AudioFormat(AudioFormat.LINEAR, inputAudioFormat.getSampleRate(), 16, 1, AudioFormat.LITTLE_ENDIAN,
						AudioFormat.SIGNED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.byteArray) };
	}
	...
	public Format setInputFormat(Format format) {
		...
		if (inputFormat != null) {
			...
			if ((outputSampleRate != inputSampleRate) || (outputChannels != inputChannels)) {
				setOutputFormat(new AudioFormat(AudioFormat.LINEAR, inputSampleRate, 16, inputChannels,
