...
public class JNIEncoder extends AbstractCodec2
		implements FormatParametersAwareCodec, PacketLossAwareEncoder, AdvancedAttributesAwareCodec {
	/**
	 * The <tt>Logger</tt> used by the <tt>JNIEncoder</tt> class and its instances
	 * for logging output.
	 */
	 ...
	 * <tt>JNIEncoder</tt> instances.
	 * <p>
	 * The implementation does support 8, 12, 16, 24 and 48kHz but the lower sample
	 * rates are not listed to prevent FMJ from defaulting to them.
	 * </p>
	 */
	 ...
	 * <tt>JNIEncoder</tt> instances.
	 */
	private static final Format[] SUPPORTED_OUTPUT_FORMATS = new Format[] { new AudioFormat(Constants.OPUS_RTP, 48000,
			/* sampleSizeInBits */ Format.NOT_SPECIFIED, 2, /* endian */ Format.NOT_SPECIFIED,
			/* signed */ Format.NOT_SPECIFIED, /* frameSizeInBits */ Format.NOT_SPECIFIED,
	...
	static {
		/*
		 * If the Opus class or its supporting JNI library are not functional, it is too
		 * late to discover the fact in #doOpen() because a JNIEncoder instance has
		 * already been initialized and it has already signaled that the Opus codec is
		 * supported.
		 */
...
//        SUPPORTED_INPUT_FORMATS = new Format[supportedInputCount*2];
		for (int i = 0; i < supportedInputCount; i++) {
			SUPPORTED_INPUT_FORMATS[i] = new AudioFormat(AudioFormat.LINEAR, SUPPORTED_INPUT_SAMPLE_RATES[i], 16, 1,
					AbstractAudioRenderer.NATIVE_AUDIO_FORMAT_ENDIAN, AudioFormat.SIGNED,
					/* frameSizeInBits */ Format.NOT_SPECIFIED, /* frameRate */ Format.NOT_SPECIFIED, Format.byteArray);
		}
		/*
		 * Using stereo input formats leads to problems (at least when used with pulse
		 * audio). It is unclear whether they are rooted in this encoder or somewhere
		 * else in the code. So stereo input formats are disabled until we make sure
...
//        }
	 ...
	 * calculated, based on {@link #frameSizeInMillis} and the <tt>inputFormat</tt>
	 * of this instance.
	 */
	private int frameSizeInBytes;

	/**
	 * The size/duration in milliseconds of an audio frame output by this instance.
	 * The possible values are: 2.5, 5, 10, 20, 40 and 60. The default value is 20.
	 */
	...
	protected void doOpen() throws ResourceUnavailableException {
		...
		if (logger.isDebugEnabled()) {
			...
			logger.debug("Encoder settings: audio bandwidth " + bw + ", bitrate " + Opus.encoder_get_bitrate(encoder)
					+ ", DTX " + Opus.encoder_get_dtx(encoder) + ", FEC " + Opus.encoder_get_inband_fec(encoder)
					+ ", VBR " + Opus.encoder_get_vbr(encoder));
		}
	}
	 ...
	 *
	 * @param inBuffer  the <tt>Buffer</tt> from which the media to be encoded is to
	 *                  be read
	 * @param outBuffer the <tt>Buffer</tt> into which the encoded media is to be
	 *                  written
	 * @return <tt>BUFFER_PROCESSED_OK</tt> if the specified <tt>inBuffer</tt> has
	...
	/**
	 * Implements {@link Control#getControlComponent()}. <tt>JNIEncoder</tt> does
	 * not provide user interface of its own.
	 *
	 * @return <tt>null</tt> to signify that <tt>JNIEncoder</tt> does not provide
	...
	public Format getOutputFormat() {
		...
		if ((f != null) && (f.getClass() == AudioFormat.class)) {
			...
			f = setOutputFormat(new AudioFormat(af.getEncoding(), af.getSampleRate(), af.getSampleSizeInBits(),
					af.getChannels(), af.getEndian(), af.getSigned(), af.getFrameSizeInBits(), af.getFrameRate(),
					af.getDataType()) {
				@Override
	 ...
	 * {@inheritDoc}
	 *
	 * Automatically tracks and calculates the size in bytes of an audio frame (to
	 * be) output by this instance.
	 */
	@Override
	...
	public void setAdvancedAttributes(Map<String, String> attributes) {
		try {
			...
			if ((s != null) && (s.length() != 0)) {
				...
				if (ptime == 3 || ptime == 5 || ptime == 10 || ptime == 20 || ptime == 40 || ptime == 60)
