...
public class JNIDecoder extends AbstractCodec2 implements FECDecoderControl {
	 ...
	 * <tt>JNIDecoder</tt> instances.
	 */
	private static final Format[] SUPPORTED_OUTPUT_FORMATS = new Format[] { new AudioFormat(AudioFormat.LINEAR, 48000,
			16, 1, AbstractAudioRenderer.NATIVE_AUDIO_FORMAT_ENDIAN, AudioFormat.SIGNED,
	...
	static {
		 ...
		 * late to discover the fact in #doOpen() because a JNIDecoder instance has
	...
	/**
	 * The size in samples per channel of the last decoded frame in the terms of the
	 * Opus library.
	 */
	...
	protected int doProcess(Buffer inBuf, Buffer outBuf) {
		...
		/*
		 * Buffer.FLAG_SILENCE is set only when the intention is to drop the specified
		 * input Buffer but to note that it has not been lost.
		 */
		...
		/*
		 * Detect the lost Buffers/packets and decode FEC/PLC. When no in-band forward
		 * error correction data is available, the Opus decoder will operate as if PLC
		 * has been specified.
		 */
		boolean decodeFEC = ((lostSeqNoCount > 0) && (lostSeqNoCount <= MAX_AUDIO_SEQUENCE_NUMBERS_TO_PLC)
				&& (lastFrameSizeInSamplesPerChannel != 0));
		...
		if (decodeFEC) {
			...
			byte[] out = validateByteArraySize(outBuf, outOffset + lastFrameSizeInSamplesPerChannel * outputFrameSize,
					outOffset != 0);
			int frameSizeInSamplesPerChannel = Opus.decode(decoder, in, inOffset, inLength, out, outOffset,
					lastFrameSizeInSamplesPerChannel, /* decodeFEC */ 1);
			...
			if (frameSizeInSamplesPerChannel > 0) {
				...
				outBuf.setFlags(
						outBuf.getFlags() | (((in == null) || (inLength == 0)) ? BUFFER_FLAG_PLC : BUFFER_FLAG_FEC));
			...
			byte[] out = validateByteArraySize(outBuf, outOffset + frameSizeInSamplesPerChannel * outputFrameSize,
					outOffset != 0);

			frameSizeInSamplesPerChannel = Opus.decode(decoder, in, inOffset, inLength, out, outOffset,
					frameSizeInSamplesPerChannel, /* decodeFEC */ 0);
			if (frameSizeInSamplesPerChannel > 0) {
				int frameSizeInBytes = frameSizeInSamplesPerChannel * outputFrameSize;
				...
				/*
				 * When we encounter a lost frame, we will presume that it was of the same
				 * duration as the last received frame.
				 */
		...
		if (outLength > 0) {
			...
			outBuf.setOffset(0);
			/*
			 * The sequence number is not likely to be important after the depacketization
			 * and the decoding but BasicFilterModule will copy them from the input Buffer
			 * into the output Buffer anyway so it makes sense to keep the sequence number
			 * straight for the sake of completeness.
			 */
	...
	/**
	 * Implements {@link Control#getControlComponent()}. <tt>JNIDecoder</tt> does
	 * not provide user interface of its own.
	 *
	 * @return <tt>null</tt> to signify that <tt>JNIDecoder</tt> does not provide
	 ...
	 * {@inheritDoc}
	 *
	 * Makes sure that the <tt>outputFormat</tt> of this instance is in accord with
	 * the <tt>inputFormat</tt> of this instance.
	 */
	@Override
