...
public class JNIEncoder extends FFmpegAudioEncoder {
	 ...
	 */
	static final AudioFormat[] SUPPORTED_INPUT_FORMATS = { new AudioFormat(AudioFormat.LINEAR, 16000, 16, 1,
	...
	static final AudioFormat[] SUPPORTED_OUTPUT_FORMATS = {
			new AudioFormat(Constants.AMR_WB_RTP, 16000, /* sampleSizeInBits */ Format.NOT_SPECIFIED, 1),
			new AudioFormat(Constants.AMR_WB, 16000, /* sampleSizeInBits */ Format.NOT_SPECIFIED, 1) };
	...
	/**
	 * The indicator which determines whether this <tt>JNIEncoder</tt> is to perform
	 * RTP packetization.
	 */
	...
	protected int doProcess(Buffer inBuf, Buffer outBuf) {
		...
		if (this.packetize && ((doProcess & BUFFER_PROCESSED_FAILED) == 0)
	...
	private int packetize(Buffer buf) {
		...
		for (int srcI = srcOff + 1, srcEnd = srcOff + srcLen, dstI = 1; srcI < srcEnd; srcI++) {
			int s = 0xFF & src[srcI];
			int d = 0xC0 & dst[dstI];
		...
		buf.setData(dst);
		buf.setDuration(20L /* milliseconds */
	 ...
	 * {@inheritDoc}
	 *
	 * Additionally, determines whether this <tt>JNIEncoder</tt> is to perform RTP
	 * packetization.
	 */
	@Override
