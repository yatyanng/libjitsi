...
public class JNIDecoder extends FFmpegAudioDecoder {
	...
	/**
	 * The indicator which determines whether this <tt>JNIDecoder</tt> is to perform
	 * RTP depacketization.
	 */
	...
	private int depacketize(Buffer buf) {
		...
		for (int inI = inOff + 1, inEnd = inOff + inLen, outI = 1; inI < inEnd; outI++) {
			int i = (0x3F & in[inI]) << 2;
	...
	protected int doProcess(Buffer inBuf, Buffer outBuf) {
		if (this.depacketize) {
			...
			if (((depacketize & BUFFER_PROCESSED_FAILED) != 0) || ((depacketize & OUTPUT_BUFFER_NOT_FILLED) != 0)) {
				return depacketize;
			}
		}
	 ...
	 * {@inheritDoc}
	 *
	 * Additionally, determines whether this <tt>JNIDecoder</tt> is to perform RTP
	 * depacketization.
	 */
	@Override
