...
public class JavaEncoder extends AbstractCodec2 implements AdvancedAttributesAwareCodec {
	...
	/**
	 * The count of the output frames to packetize. By default we packetize 2 audio
	 * frames in one G729 packet.
	 */
	...
	/**
	 * The previous input if it was less than the input frame size and which is to
	 * be prepended to the next input in order to form a complete input frame.
	 */
	private byte[] prevIn;

	/**
	 * The length of the previous input if it was less than the input frame size and
	 * which is to be prepended to the next input in order to form a complete input
	 * frame.
	 */
	 ...
	 * nanosecond. We packetize 2 audio frames in one G729 packet by default.
	 */
	...
	public JavaEncoder() {
		super("G.729 Encoder", AudioFormat.class,
				new AudioFormat[] { new AudioFormat(AudioFormat.G729_RTP, 8000, Format.NOT_SPECIFIED, 1) });
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		...
		byte[] output = validateByteArraySize(outBuffer,
				outBuffer.getOffset() + OUTPUT_FRAMES_COUNT * OUTPUT_FRAME_SIZE_IN_BYTES, true);

		packetize(serial, output, outBuffer.getOffset() + OUTPUT_FRAME_SIZE_IN_BYTES * outFrameCount);
