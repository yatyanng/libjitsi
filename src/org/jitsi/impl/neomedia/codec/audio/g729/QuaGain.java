...
class QuaGain {
	...
	/*
	 * File : QUA_GAIN.C Used for the floating point version of both G.729 main body
	 * and G.729A
	 */
	 ...
	 */
	int qua_gain(float code[], float[] g_coeff, int l_subfr, FloatReference gain_pit, FloatReference gain_code,
			int tameflag) {
		 ...
		 * removed). * An initial predicted gain, g_0, is first determined and the
		 * correction * factor alpha = gain / g_0 is quantized. * The pitch gain and the
		 * correction factor are vector quantized and the * mean-squared weighted error
		 * criterion is used in the quantizer search. * CS Codebook , fast pre-selection
		 * version *
		 */
	 ...
	 * Presearch for gain codebook
	 *
	 * @param best_gain input : [0] unquantized pitch gain [1] unquantized code gain
