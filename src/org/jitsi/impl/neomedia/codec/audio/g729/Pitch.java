...
class Pitch {
	...
	/*
	 * File : PITCH.C Used for the floating point version of G.729 main body (not
	 * for G.729A)
	 */
	 ...
	 *
	 * @param signal        input : signal to compute pitch s[-PIT_MAX : l_frame-1]
	 ...
	 * @return lag found
	 */
	private static int lag_max(float[] signal, int signal_offset, int l_frame, int lagmax, int lagmin,
	...
	/**
	 * Find the normalized correlation between the target vector and the filtered
	 * past excitation.
	 *
	 * @param exc              input : excitation buffer
	 * @param exc_offset       input : excitation buffer offset
	 ...
	 * @param corr_norm        output: normalized correlation (correlation between
	 *                         target and filtered excitation divided by the square
	 *                         root of energy of filtered excitation)
	 * @param corr_norm_offset input: normalized correlation offset
	 */
	private static void norm_corr(float[] exc, int exc_offset, float xn[], float h[], int l_subfr, int t_min, int t_max,
			float corr_norm[], int corr_norm_offset) {
	 ...
	 */
	static int enc_lag3(int T0, int T0_frac, IntReference T0_min, IntReference T0_max, int pit_min, int pit_max,
			int pit_flag) {
		int index;
