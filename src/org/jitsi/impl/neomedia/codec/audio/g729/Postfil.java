...
class Postfil extends Ld8k {
	...
	/*
	 * File : POSTFIL.C Used for the floating point version of G.729 main body (not
	 * for G.729A)
	 */
	 ...
	 */
	int post(int t0, float[] signal_ptr, int signal_ptr_offset, float[] coeff, int coeff_offset, float[] sig_out,
			int sig_out_offset) {
	...
	private int pst_ltp(int t0, float[] ptr_sig_in, int ptr_sig_in_offset, float[] ptr_sig_pst0,
		...
		if (num_gltp == 0.f) {
			...
			filt_plt(ptr_sig_in, ptr_sig_in_offset, ptr_y_up, ptr_y_up_offset, ptr_sig_pst0, ptr_sig_pst0_offset,
					gain_plt);
		}
		return vo;
	}
	 ...
	 * @param off_yup
	 */
	private void search_del(int t0, float[] ptr_sig_in, int ptr_sig_in_offset, IntReference ltpdel, IntReference phase,
			FloatReference num_gltp, FloatReference den_gltp, float[] y_up, IntReference off_yup) {
	 ...
	 */
	private void filt_plt(float[] s_in, int s_in_offset, float[] s_ltp, int s_ltp_offset, float[] s_out,
			int s_out_offset, float gain_plt) {
	 ...
	 */
	private void compute_ltp_l(float[] s_in, int s_in_offset, int ltpdel, int phase, float[] y_up, int y_up_offset,
			FloatReference num, FloatReference den) {
	...
	/**
	 * Tilt filtering with : (1 + mu z-1) * (1/1-|mu|). computes y[n] = (1/1-|mu|)
	 * (x[n]+mu*x[n-1])
	 *
	...
	/**
	 * Control of the subframe gain. gain[n] = AGC_FAC * gain[n-1] + (1 - AGC_FAC)
	 * g_in/g_out
	 *
