...
class Util {
	...
	/*
	 * File : UTIL.C Used for the floating point version of both G.729 main body and
	 * G.729A
	 */
	...
	/**
	 * Assigns the value zero to element of the specified array of floats. The
	 * number of components set to zero equal to the length argument. The components
	 * at positions offset through offset+length-1 in the array are set to zero.
	 *
	 * @param x      (o) : vector to clear
	 ...
	 * destination array. A subsequence of array components are copied from the
	 * source array referenced by x to the destination array referenced by y. The
	 * number of components copied is equal to the length argument. The components
	 * at positions x_offset through x_offset+length-1 in the source array are
	 * copied into positions 0 through length-1, respectively, of the destination
	 * array.
	 *
	 * @param x        (i) : input vector
	 ...
	 * of array components are copied from the source array referenced by x to the
	 * destination array referenced by y. The number of components copied is equal
	 * to the length argument. The components at positions x_offset through
	 * x_offset+length-1 in the source array are copied into positions y_offset
	 * through y_offset+length-1, respectively, of the destination array.
	 *
	 * @param x        (i) : input vector
	 * @param x_offset (i) : input vector offset
	 * @param y        (o) : output vector
