...
class Lpc {
	...
	/*
	 * File : LPC.C Used for the floating point version of G.729 main body (not for
	 * G.729A)
	 */
	 ...
	 * The found LSPs are in the cosine domain with values in the range from 1 down
	 * to -1. The table grid[] contains the points (in the cosine domain) at which
	 * the polynomials are evaluated. The table corresponds to NO_POINTS frequencies
	 * uniformly spaced between 0 and pi.
	 *
	 ...
	 *
	 * The polynomial order is n = m/2 (m is the prediction order) The polynomial is
	 * given by C(x) = T_n(x) + f(1)T_n-1(x) + ... +f(n-1)T_1(x) + f(n)/2
	 *
