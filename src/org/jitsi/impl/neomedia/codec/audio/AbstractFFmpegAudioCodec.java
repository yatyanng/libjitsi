...
public abstract class AbstractFFmpegAudioCodec extends AbstractCodec2 {
	...
	/**
	 * The <tt>AVCodecContext</tt> which performs the actual encoding/decoding and
	 * which is the native counterpart of this open
	...
	/**
	 * Initializes a new <tt>AbstractFFmpegAudioCodec</tt> instance with a specific
	 * <tt>PlugIn</tt> name, a specific <tt>AVCodecID</tt>, and a specific list of
	 * <tt>Format</tt>s supported as output.
	 *
	 * @param name                   the <tt>PlugIn</tt> name of the new instance
	 * @param codecID                the <tt>AVCodecID</tt> of the FFmpeg codec to
	...
	/**
	 * Configures the <tt>AVCodecContext</tt> initialized in {@link #doOpen()} prior
	 * to invoking one of the FFmpeg functions in the <tt>avcodec_open</tt> family.
	 * Allows extenders to override and provide additional, optional configuration.
	 *
	...
	protected synchronized void doOpen() throws ResourceUnavailableException {
		...
		if (avctx == 0) {
			throw new ResourceUnavailableException("Could not open FFmpeg codec " + codecIDToString(codecID) + "!");
		}
	}

	/**
	 * Finds an <tt>AVCodec</tt> with a specific <tt>AVCodecID</tt>. The method is
	 * invoked by {@link #doOpen()} in order to (eventually) open a new
	...
	/**
	 * Gets the <tt>AudioFormat</tt> with which {@link #avctx} is to be configured
	 * and opened by {@link #doOpen()}.
	 *
