...
import static org.jitsi.impl.neomedia.codec.audio.silk.Typedef.SKP_int16_MAX;
...
public class DecodeCore {
	...
	static void SKP_Silk_decode_core(SKP_Silk_decoder_state psDec, /* I/O Decoder state */
		...
		for (k = 0; k < NB_SUBFR; k++) {
			...
			if (psDec.lossCnt != 0 && psDec.prev_sigtype == SIG_TYPE_VOICED && psDecCtrl.sigtype == SIG_TYPE_UNVOICED
					&& k < (NB_SUBFR >> 1)) {
			...
			if (sigtype == SIG_TYPE_VOICED) {
				...
				if ((k & (3 - (NLSF_interpolation_flag << 1))) == 0) {
					...
					Arrays.fill(FiltState, 0, psDec.LPC_order,
							0); /* Not really necessary, but Valgrind and Coverity will complain otherwise */
					MA.SKP_Silk_MA_Prediction(psDec.outBuf, start_idx + k * (psDec.frame_length >> 2), A_Q12, 0,
							FiltState, sLTP, start_idx, psDec.frame_length - start_idx, psDec.LPC_order);
			...
			if (sigtype == SIG_TYPE_VOICED) {
				...
				for (i = 0; i < psDec.subfr_length; i++) {
					...
					pres_Q10[pres_Q10_offset + i] = (pexc_Q10[pexc_Q10_offset + i]
			...
			SKP_Silk_decode_short_term_prediction(vec_Q10, pres_Q10, pres_Q10_offset, psDec.sLPC_Q14, A_Q12_tmp,
					psDec.LPC_order, psDec.subfr_length);

			/* Scale with Gain */
			for (i = 0; i < psDec.subfr_length; i++) {
				pxq[pxq_offset + i] = (short) SigProcFIX
						.SKP_SAT16(SigProcFIX.SKP_RSHIFT_ROUND(SKP_SMULWW(vec_Q10[i], Gain_Q16), 10));
			}
	...
	private static void SKP_Silk_decode_short_term_prediction(int[] vec_Q10, int[] pres_Q10, int pres_Q10_offset,
			int[] sLPC_Q14, short[] A_Q12_tmp, int LPC_order, int subfr_length) {
		int i;
		int LPC_pred_Q10;
		int j;
