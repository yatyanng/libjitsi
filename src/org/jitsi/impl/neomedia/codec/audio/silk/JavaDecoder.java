...
public class JavaDecoder extends AbstractCodec2 {
	/**
	 * A private class, an instance of which is registered via <tt>addControl</tt>.
	 * This instance will be used by outside classes to access decoder statistics.
	 */
	...
	/**
	 * The maximum number of frames encoded into a single payload as defined by the
	 * SILK standard.
	 */
	...
	/**
	 * The length of an output frame as determined by {@link #FRAME_DURATION} and
	 * the <tt>inputFormat</tt> of this <tt>JavaDecoder</tt>.
	 */
	private short frameLength;

	/**
	 * The number of frames decoded from the last input <tt>Buffer</tt> which has
	...
	/**
	 * Temporary buffer used to hold the lbrr data when decoding FEC. Defined here
	 * to avoid using <tt>new</tt> in <tt>doProcess</tt>.
	 */
	private byte[] lbrrData = new byte[JavaEncoder.MAX_BYTES_PER_FRAME];

	/**
	 * The <tt>Logger</tt> used by this <tt>JavaDecoder</tt> instance for logging
	 * output.
	 */
	private final Logger logger = Logger.getLogger(JavaDecoder.class);

	/**
	 * Number of packets which: were missing, the following packet was available and
	 * it contained FEC data.
	 */
	private int nbFECDecoded = 0;

	/**
	 * Number of packets which: were missing, the next packet was available, but it
	 * did not contain FEC data.
	 */
	...
	protected int doProcess(Buffer inBuf, Buffer outBuf) {
		...
		/*
		 * Check whether a packet has been lost. If a packet has more than one frame, we
		 * go through each frame in a new call to the process method so having the same
		 * sequence number as on the previous pass is fine.
		 */
		...
		{
			lbrrBytes[0] = 0;
			DecAPI.SKP_Silk_SDK_search_for_LBRR(in, inOffset, (short) inLength, /* lost_offset */ lostSeqNoCount,
			...
			if (lbrrBytes[0] == 0) {
				...
				lostFlag = 1;
			} else if (DecAPI.SKP_Silk_SDK_Decode(decState, decControl, 0, lbrrData, 0, lbrrBytes[0], out, outOffset,
					outLength) == 0) {
		...
		{
			outLength[0] = frameLength;
			if (DecAPI.SKP_Silk_SDK_Decode(decState, decControl, lostFlag, in, inOffset, inLength, out, outOffset,
					outLength) == 0) {
				outBuf.setDuration(FRAME_DURATION * 1000000);
				outBuf.setLength(outLength[0]);
				outBuf.setOffset(outOffset);
