...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MAX_NB_SHELL_BLOCKS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MAX_PULSES;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.N_RATE_LEVELS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SHELL_CODEC_FRAME_LENGTH;
...
public class EncodePulses {
	...
	static void SKP_Silk_encode_pulses(SKP_Silk_range_coder_state psRC, /* I/O Range coder state */
		...
		RangeCoder.SKP_Silk_range_encoder(psRC, RateLevelIndex, TablesPulsesPerBlock.SKP_Silk_rate_levels_CDF[sigtype],
				0);
		...
		for (i = 0; i < iter; i++) {
			if (nRshifts[i] > 0) {
				...
				nLS = nRshifts[i] - 1;
				for (k = 0; k < SHELL_CODEC_FRAME_LENGTH; k++) {
					abs_q = pulses_ptr[pulses_ptr_offset + k] > 0 ? pulses_ptr[pulses_ptr_offset + k]
