...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.DECISION_DELAY;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.DECISION_DELAY_MASK;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.DEL_DEC_STATES_MAX;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.HARM_SHAPE_FIR_TAPS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.LTP_ORDER;
...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.NSQ_LPC_BUF_LENGTH;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SHAPE_LPC_ORDER_MAX;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SIG_TYPE_VOICED;
import static org.jitsi.impl.neomedia.codec.audio.silk.Macros.SKP_SMLABB;
import static org.jitsi.impl.neomedia.codec.audio.silk.Macros.SKP_SMLAWB;
import static org.jitsi.impl.neomedia.codec.audio.silk.Macros.SKP_SMLAWT;
...
public class NSQDelDec {
	static void SKP_Silk_NSQ_del_dec(SKP_Silk_encoder_state psEncC, /* I/O Encoder State */
		...
		// TODO: use a local copy of the parameter short x[], which is supposed to be
		// input;
		short[] x_tmp = x.clone();
		int x_tmp_offset = 0;
...
//                                psDD.Gain_Q16[ last_smple_idx ] ), 10 ) );
							pxq[pxq_offset + i
									- decisionDelay] = (short) SigProcFIX.SKP_SAT16(SigProcFIX.SKP_RSHIFT_ROUND(
											SKP_SMULWW(psDD.Xq_Q10[last_smple_idx], psDD.Gain_Q16[last_smple_idx]),
											10));
...
//                    SKP_memset( FiltState, 0, psEncC.predictLPCOrder * sizeof( SKP_int32 ) );
					...
					MA.SKP_Silk_MA_Prediction(NSQ.xq, start_idx + k * psEncC.subfr_length, A_Q12, A_Q12_offset,
							FiltState, sLTP, start_idx, psEncC.frame_length - start_idx, psEncC.predictLPCOrder);
			...
			SKP_Silk_nsq_del_dec_scale_states(NSQ, psDelDec, x_tmp, x_tmp_offset, x_sc_Q10, subfr_length, sLTP,
					sLTP_Q16, k, psEncC.nStatesDelayedDecision, smpl_buf_idx, LTP_scale_Q14, Gains_Q16,
					psEncCtrlC.pitchL);

			int smpl_buf_idx_ptr[] = new int[1];
			smpl_buf_idx_ptr[0] = smpl_buf_idx;
			SKP_Silk_noise_shape_quantizer_del_dec(NSQ, psDelDec, psEncCtrlC.sigtype, x_sc_Q10, q_tmp, q_tmp_offset,
					pxq, pxq_offset, sLTP_Q16, A_Q12, A_Q12_offset, B_Q14, B_Q14_offset, AR_shp_Q13, AR_shp_Q13_offset,
					lag, HarmShapeFIRPacked_Q14, Tilt_Q14[k], LF_shp_Q14[k], Gains_Q16[k], Lambda_Q10, offset_Q10,
		...
		for (i = 0; i < decisionDelay; i++) {
			...
			pxq[pxq_offset + i - decisionDelay] = (short) SigProcFIX.SKP_SAT16(SigProcFIX
					.SKP_RSHIFT_ROUND(SKP_SMULWW(psDD.Xq_Q10[last_smple_idx], psDD.Gain_Q16[last_smple_idx]), 10));
			NSQ.sLTP_shp_Q10[NSQ.sLTP_shp_buf_idx - decisionDelay + i] = psDD.Shape_Q10[last_smple_idx];
...
//TODO: copy back the q_tmp to the output parameter q;
	...
	static void SKP_Silk_noise_shape_quantizer_del_dec(SKP_Silk_nsq_state NSQ, /* I/O NSQ state */
		...
		/*
		 * psSampleState is an two-dimension array of reference, which should be created
		 * manually.
		 */
		{
...
//                n_AR_Q10 = SKP_RSHIFT( n_AR_Q10, 1 );           /* Q11 -> Q10 */
				...
				if (r_Q10 < -1536) {
					...
					rd1_Q10 = (rd1_Q10 + SigProcFIX.SKP_SUB_RSHIFT32(SigProcFIX.SKP_ADD_LSHIFT32(Lambda_Q10, r_Q10, 1),
							rr_Q20, 9));
					q1_Q10 = -1024;
					q2_Q10 = 0;
				}
...
//                SKP_memcpy( &psSampleState[ RDmax_ind ][ 0 ], &psSampleState[ RDmin_ind ][ 1 ], sizeof( NSQ_sample_struct ) );
			...
			if (subfr > 0 || i >= decisionDelay) {
				...
				xq[xq_offset + i - decisionDelay] = (short) SigProcFIX.SKP_SAT16(SigProcFIX
						.SKP_RSHIFT_ROUND(SKP_SMULWW(psDD.Xq_Q10[last_smple_idx], psDD.Gain_Q16[last_smple_idx]), 10));
...
//                psSS                                     = &psSampleState[ k ][ 0 ];
	...
	static void SKP_Silk_nsq_del_dec_scale_states(SKP_Silk_nsq_state NSQ, /* I/O NSQ state */
		...
		if (inv_gain_Q16 != NSQ.prev_inv_gain_Q16) {
			...
			scale_length = Math.max(scale_length,
					lag + LTP_ORDER); /* make sure to scale whole pitch period if voiced */
