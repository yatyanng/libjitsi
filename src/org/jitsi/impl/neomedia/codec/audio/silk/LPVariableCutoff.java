...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.TRANSITION_INT_NUM;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.TRANSITION_INT_STEPS_DOWN;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.TRANSITION_INT_STEPS_UP;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.TRANSITION_NA;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.TRANSITION_NB;
...
public class LPVariableCutoff {
	 ...
	 * @param fac_Q16
	 */
	static void SKP_Silk_LP_interpolate_filter_taps(int[] B_Q28, int[] A_Q28, final int ind, final int fac_Q16) {
		int nb, na;

		if (ind < TRANSITION_INT_NUM - 1) {
			if (fac_Q16 > 0) {
				if (fac_Q16 == SigProcFIX.SKP_SAT16(fac_Q16)) { /* fac_Q16 is in range of a 16-bit int */
					/* Piece-wise linear interpolation of B and A */
					for (nb = 0; nb < TRANSITION_NB; nb++) {
						B_Q28[nb] = SKP_SMLAWB(TablesOther.SKP_Silk_Transition_LP_B_Q28[ind][nb],
								TablesOther.SKP_Silk_Transition_LP_B_Q28[ind + 1][nb]
										- TablesOther.SKP_Silk_Transition_LP_B_Q28[ind][nb],
								fac_Q16);
					}
					for (na = 0; na < TRANSITION_NA; na++) {
						A_Q28[na] = SKP_SMLAWB(TablesOther.SKP_Silk_Transition_LP_A_Q28[ind][na],
								TablesOther.SKP_Silk_Transition_LP_A_Q28[ind + 1][na]
										- TablesOther.SKP_Silk_Transition_LP_A_Q28[ind][na],
								fac_Q16);
					}
				} else if (fac_Q16 == (1 << 15)) { /*
													 * Neither fac_Q16 nor ( ( 1 << 16 ) - fac_Q16 ) is in range of a
													 * 16-bit int
													 */
					...
					/* Piece-wise linear interpolation of B and A */
					for (nb = 0; nb < TRANSITION_NB; nb++) {
						B_Q28[nb] = SKP_SMLAWB(TablesOther.SKP_Silk_Transition_LP_B_Q28[ind + 1][nb],
								TablesOther.SKP_Silk_Transition_LP_B_Q28[ind][nb]
										- TablesOther.SKP_Silk_Transition_LP_B_Q28[ind + 1][nb],
								(1 << 16) - fac_Q16);
					}
					for (na = 0; na < TRANSITION_NA; na++) {
						A_Q28[na] = SKP_SMLAWB(TablesOther.SKP_Silk_Transition_LP_A_Q28[ind + 1][na],
								TablesOther.SKP_Silk_Transition_LP_A_Q28[ind][na]
										- TablesOther.SKP_Silk_Transition_LP_A_Q28[ind + 1][na],
								(1 << 16) - fac_Q16);
					}
	...
	static void SKP_Silk_LP_variable_cutoff(SKP_Silk_LP_state psLP, /* I/O LP filter state */
