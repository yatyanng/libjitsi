...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.NLSF_MSVQ_FLUCTUATION_REDUCTION;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.NLSF_MSVQ_MAX_CB_STAGES;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.NLSF_MSVQ_SURV_MAX_REL_RD;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.NLSF_MSVQ_TREE_SEARCH_MAX_VECTORS_EVALUATED;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.NLSF_MSVQ_TREE_SEARCH_MAX_VECTORS_EVALUATED_LC_MODE;
...
public class NLSFMSVQEncodeFLP {
	...
	static void SKP_Silk_NLSF_MSVQ_encode_FLP(int[] NLSFIndices, /* O Codebook path vector [ CB_STAGES ] */
		...
		for (s = 0; s < psNLSF_CB_FLP.nStages; s++) {
			...
			NLSFVQRateDistortionFLP.SKP_Silk_NLSF_VQ_rate_distortion_FLP(pRateDist, pCurrentCBStage, pRes, pW, pRate,
					NLSF_mu, prev_survivors, LPC_order);
			...
			/*
			 * Update accumulated codebook contributions for the 'cur_survivors' best
			 * codebook indices
			 */
			for (k = 0; k < cur_survivors; k++) {
				...
				/*
				 * Write the current stage indices for the 'cur_survivors' to the best path
				 * matrix
				 */
		...
		if (NLSF_MSVQ_FLUCTUATION_REDUCTION) {
			...
			if (deactivate_fluc_red != 1) {
				...
				for (s = 0; s < cur_survivors; s++) {
					...
					NLSFMSVQDecodeFLP.SKP_Silk_NLSF_MSVQ_decode_FLP(pNLSF, psNLSF_CB_FLP, pPath_new,
