		...
		for (k = 0; k < NB_SUBFR; k++) {
			...
			if (psEncCtrlC.sigtype == SIG_TYPE_VOICED) {
				...
				if ((k & (3 - (LSF_interpolation_flag << 1))) == 0) {
					...
					MA.SKP_Silk_MA_Prediction(NSQ.xq, start_idx + k * (psEncC.frame_length >> 2), A_Q12, A_Q12_offset,
			...
			SKP_Silk_nsq_scale_states(NSQ, x_tmp, x_tmp_offset, x_sc_Q10, psEncC.subfr_length, sLTP, sLTP_Q16, k,
			...
			SKP_Silk_noise_shape_quantizer(NSQ, psEncCtrlC.sigtype, x_sc_Q10, q_tmp, q_tmp_offset, pxq, pxq_offset,
					sLTP_Q16, A_Q12, A_Q12_offset, B_Q14, B_Q14_offset, AR_shp_Q13, AR_shp_Q13_offset, lag,
					HarmShapeFIRPacked_Q14, Tilt_Q14[k], LF_shp_Q14[k], Gains_Q16[k], Lambda_Q10, offset_Q10,
...
//            SKP_assert( ( (SKP_int64)AR_shp_Q13 & 3 ) == 0 );   /* check that array starts at 4-byte aligned address */
			...
			xq[xq_offset + i] = (short) SigProcFIX
					.SKP_SAT16(SigProcFIX.SKP_RSHIFT_ROUND(SKP_SMULWW(xq_Q10, Gain_Q16), 10));
	...
	static void SKP_Silk_nsq_scale_states(SKP_Silk_nsq_state NSQ, /* I/O NSQ state */
		...
		scale_length = scale_length
				- SKP_SMULBB(NB_SUBFR - (subfr + 1), length); /* subtract samples that will be too old in next frame */
		scale_length = SigProcFIX.SKP_max_int(scale_length,
