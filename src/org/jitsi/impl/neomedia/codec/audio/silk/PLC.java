		...
		if (psDecCtrl.sigtype == SIG_TYPE_VOICED) {
			...
			for (j = 0; j * psDec.subfr_length < psDecCtrl.pitchL[NB_SUBFR - 1]; j++) {
				...
				if (temp_LTP_Gain_Q14 > LTP_Gain_Q14) {
					...
					System.arraycopy(psDecCtrl.LTPCoef_Q14, SKP_SMULBB(NB_SUBFR - 1 - j, LTP_ORDER), psPLC.LTPCoef_Q14,
							0, LTP_ORDER);
	...
	static void SKP_Silk_PLC_conceal(SKP_Silk_decoder_state psDec, /* I/O Decoder state */
		...
		for (k = (NB_SUBFR >> 1); k < NB_SUBFR; k++) {
			for (i = 0; i < psDec.subfr_length; i++) {
				exc_buf_ptr[exc_buf_ptr_offset + i] = (short) (SKP_SMULWW(psDec.exc_Q10[i + k * psDec.subfr_length],
		...
		for (k = 0; k < NB_SUBFR; k++) {
			...
			for (i = 0; i < psDec.subfr_length; i++) {
				...
				LPC_exc_Q10 = (SKP_SMULWB(rand_ptr[rand_ptr_offset + idx],
						rand_scale_Q14) << 2); /* Random noise part */
		...
		for (i = 0; i < psDec.frame_length; i++) {
			signal[signal_offset + i] = (short) SigProcFIX.SKP_SAT16(
					SigProcFIX.SKP_RSHIFT_ROUND(SKP_SMULWW(sig_Q10[i], psPLC.prevGain_Q16[NB_SUBFR - 1]), 10));
		}
