...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.HIGH_PASS_INPUT;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.LA_PITCH_MAX;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.LBRR_IDX_MASK;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MAX_ARITHM_BYTES;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MAX_CONSECUTIVE_DTX;
...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.NO_SPEECH_FRAMES_BEFORE_DTX;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.NO_VOICE_ACTIVITY;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.N_LEVELS_QGAIN;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SKP_SILK_ADD_LBRR_TO_PLUS1;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SKP_SILK_ADD_LBRR_TO_PLUS2;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SKP_SILK_LAST_FRAME;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SKP_SILK_LBRR_VER1;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SKP_SILK_LBRR_VER2;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SKP_SILK_MORE_FRAMES;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SWITCH_TRANSITION_FILTERING;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.VOICE_ACTIVITY;
...
///*TEST END****************************************************************************/
		...
		/*******************************************/
		SigProcFLP.SKP_short2float_array(x_frame, x_frame_offset + psEnc.sCmn.la_shape, pIn_HP_LP, 0,
				psEnc.sCmn.frame_length);
...
//        frame_cnt++;
		...
		/****************************************/
		/* Update input buffer */
		System.arraycopy(psEnc.x_buf, psEnc.x_buf_offset + psEnc.sCmn.frame_length, psEnc.x_buf, psEnc.x_buf_offset,
		...
		if (psEnc.sCmn.nFramesInPayloadBuf * FRAME_LENGTH_MS >= psEnc.sCmn.PacketSize_ms) {
			...
			if (pnBytesOut[0] >= nBytes[0]) {
				...
				if (frame_terminator > SKP_SILK_MORE_FRAMES
					...
					System.arraycopy(psEnc.sCmn.LBRR_buffer[LBRR_idx].payload, 0, pCode, pCode_offset + nBytes[0],
				...
				/* Update FEC buffer */
				System.arraycopy(LBRRpayload, 0, psEnc.sCmn.LBRR_buffer[psEnc.sCmn.oldest_LBRR_idx].payload, 0,
						nBytesLBRR[0]);
		...
		if (psEnc.speech_activity > DefineFLP.WB_DETECT_ACTIVE_SPEECH_LEVEL_THRES) {
			psEnc.sCmn.sSWBdetect.ActiveSpeech_ms = SigProcFIX.SKP_ADD_POS_SAT32(psEnc.sCmn.sSWBdetect.ActiveSpeech_ms,
					FRAME_LENGTH_MS);
		}
...
//TODO:use clone rather than memory copy.
					...
					psEncCtrl.sCmn.GainsIndices[0] = SigProcFIX.SKP_LIMIT(psEncCtrl.sCmn.GainsIndices[0], 0,
							N_LEVELS_QGAIN - 1);
				}
				/* Decode to get Gains in sync with decoder */
				int LBRRprevLastGainIndex_ptr[] = new int[1];
				LBRRprevLastGainIndex_ptr[0] = psEnc.sCmn.LBRRprevLastGainIndex;
				GainQuant.SKP_Silk_gains_dequant(Gains_Q16, psEncCtrl.sCmn.GainsIndices, LBRRprevLastGainIndex_ptr,
						psEnc.sCmn.nFramesInPayloadBuf);
				psEnc.sCmn.LBRRprevLastGainIndex = LBRRprevLastGainIndex_ptr[0];

				/*
				 * Overwrite unquantized gains with quantized gains and convert back to Q0 from
				 * Q16
				 */
			...
			/* Encode Parameters */
			/****************************************/
			EncodeParameters.SKP_Silk_encode_parameters(psEnc.sCmn, psEncCtrl.sCmn, psEnc.sCmn.sRC_LBRR,
					psEnc.sCmn.q_LBRR);
			/****************************************/
			/* Encode Parameters */
			/****************************************/
