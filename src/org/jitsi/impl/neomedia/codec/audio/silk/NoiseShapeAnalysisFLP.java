...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SHAPE_LPC_WIN_MAX;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SHAPE_LPC_WIN_MS;
...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.USE_HARM_SHAPING;
...
public class NoiseShapeAnalysisFLP {
	 ...
	 */
	static void SKP_Silk_noise_shape_analysis_FLP(SKP_Silk_encoder_state_FLP psEnc, /* I/O Encoder state FLP */
		...
		SNR_adj_dB = psEncCtrl.current_SNR_dB - PerceptualParametersFLP.BG_SNR_DECR_dB * psEncCtrl.coding_quality
		...
		for (k = 0; k < NB_SUBFR; k++) {
			/* Apply window */
			ApplySineWindowFLP.SKP_Silk_apply_sine_window_FLP(x_windowed, 0, x_ptr, x_ptr_offset, 0,
					SHAPE_LPC_WIN_MS * psEnc.sCmn.fs_kHz);
			...
			AutocorrelationFLP.SKP_Silk_autocorrelation_FLP(auto_corr, 0, x_windowed, 0,
					SHAPE_LPC_WIN_MS * psEnc.sCmn.fs_kHz, psEnc.sCmn.shapingLPCOrder + 1);
			 ...
			 */
			nrg = LevinsondurbinFLP.SKP_Silk_levinsondurbin_FLP(psEncCtrl.AR2, k * SHAPE_LPC_ORDER_MAX, auto_corr,
					psEnc.sCmn.shapingLPCOrder);

			/* Bandwidth expansion for synthesis filter shaping */
			BwexpanderFLP.SKP_Silk_bwexpander_FLP(psEncCtrl.AR2, k * SHAPE_LPC_ORDER_MAX, psEnc.sCmn.shapingLPCOrder,
					BWExp2);
...
//                psEnc->sCmn.shapingLPCOrder * sizeof( SKP_float ) );
			...
			BwexpanderFLP.SKP_Silk_bwexpander_FLP(psEncCtrl.AR1, k * SHAPE_LPC_ORDER_MAX, psEnc.sCmn.shapingLPCOrder,
					BWExp1);
			...
			LPCInvPredGainFLP.SKP_Silk_LPC_inverse_pred_gain_FLP(pre_nrg_djinnaddress, psEncCtrl.AR2,
					k * SHAPE_LPC_ORDER_MAX, psEnc.sCmn.shapingLPCOrder);
			pre_nrg = pre_nrg_djinnaddress[0];
			float[] nrg_djinnaddress = { nrg };
			LPCInvPredGainFLP.SKP_Silk_LPC_inverse_pred_gain_FLP(nrg_djinnaddress, psEncCtrl.AR1,
					k * SHAPE_LPC_ORDER_MAX, psEnc.sCmn.shapingLPCOrder);
		...
		if (psEncCtrl.input_tilt <= 0.0f && psEncCtrl.sCmn.sigtype == SIG_TYPE_UNVOICED) {
			...
			if (psEnc.sCmn.fs_kHz == 24) {
				gain_mult *= (float) Math.pow(2.0f,
						-0.16f * PerceptualParametersFLP.DE_ESSER_COEF_SWB_dB * essStrength);
		...
		if (psEncCtrl.sCmn.sigtype == SIG_TYPE_VOICED) {
			...
			Tilt = -PerceptualParametersFLP.HP_NOISE_COEF - (1 - PerceptualParametersFLP.HP_NOISE_COEF)
