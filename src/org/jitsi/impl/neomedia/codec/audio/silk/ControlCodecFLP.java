...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.ACCUM_BITS_DIFF_THRESHOLD;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.DEL_DEC_STATES_MAX;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.FIND_PITCH_LPC_ORDER_MAX;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.FIND_PITCH_LPC_WIN_MS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.FRAME_LENGTH_MS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.INBAND_FEC_MIN_RATE_BPS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.LA_PITCH_MS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.LA_SHAPE_MAX;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.LA_SHAPE_MS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.LBRR_LOSS_THRES;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.LOW_COMPLEXITY_ONLY;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MAX_API_FS_KHZ;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MAX_FRAME_LENGTH;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MAX_LBRR_DELAY;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MAX_LPC_ORDER;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MAX_NLSF_MSVQ_SURVIVORS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MAX_NLSF_MSVQ_SURVIVORS_LC_MODE;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MAX_NLSF_MSVQ_SURVIVORS_MC_MODE;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MB2NB_BITRATE_BPS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MB2WB_BITRATE_BPS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MIN_LPC_ORDER;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MIN_TARGET_RATE_MB_BPS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MIN_TARGET_RATE_NB_BPS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MIN_TARGET_RATE_SWB_BPS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MIN_TARGET_RATE_WB_BPS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.NB2MB_BITRATE_BPS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.NB_SUBFR;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.PITCH_EST_COMPLEXITY_HC_MODE;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.PITCH_EST_COMPLEXITY_LC_MODE;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.PITCH_EST_COMPLEXITY_MC_MODE;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SHAPE_LPC_ORDER_MAX;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SIG_TYPE_UNVOICED;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SKP_SILK_ADD_LBRR_TO_PLUS1;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SKP_SILK_NO_LBRR;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SWB2WB_BITRATE_BPS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.SWITCH_TRANSITION_FILTERING;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.TARGET_RATE_TAB_SZ;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.TRANSITION_FRAMES_DOWN;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.TRANSITION_FRAMES_UP;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.USE_LBRR;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.WB2MB_BITRATE_BPS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.WB2SWB_BITRATE_BPS;
import static org.jitsi.impl.neomedia.codec.audio.silk.Typedef.SKP_int32_MAX;
...
public class ControlCodecFLP {
	 ...
	 * Control encoder SNR.
	 *
	 * @param psEnc               Pointer to Silk encoder state FLP
	 * @param API_fs_Hz           External (API) sampling rate (Hz).
	 * @param max_internal_fs_kHz Maximum internal sampling rate (kHz).
	 * @param PacketSize_ms       Packet length (ms).
	 * @param TargetRate_bps      Target max bitrate (if SNR_dB == 0).
	 * @param PacketLoss_perc     Packet loss rate (in percent).
	 * @param INBandFEC_enabled   Enable (1) / disable (0) inband FEC.
	 * @param DTX_enabled         Enable / disable DTX.
	 * @param InputFramesize_ms   Inputframe in ms.
	 * @param Complexity          Complexity (0->low; 1->medium; 2->high).
	 * @return
	 */
	...
	static int SKP_Silk_control_encoder_FLP(SKP_Silk_encoder_state_FLP psEnc, /*
		...
		if (API_fs_Hz == 8000 || fs_kHz == 0 || API_fs_Hz < fs_kHz * 1000 || fs_kHz > max_internal_fs_kHz) {
			/*
			 * Switching is not possible, encoder just initialized, internal mode higher
			 * than external,
			 */
			...
			/*
			 * Accumulate the difference between the target rate and limit for switching
			 * down
			 */
			...
			if (psEnc.speech_activity < 0.5f
					&& psEnc.sCmn.nFramesInPayloadBuf == 0) { /* Low speech activity and payload buffer empty */
					...
					if ((psEnc.sCmn.sLP.transition_frame_no == 0) && /* Transition phase not active */
							(psEnc.sCmn.bitrateDiff <= -ACCUM_BITS_DIFF_THRESHOLD || /* Bitrate threshold is met */
									(psEnc.sCmn.sSWBdetect.WB_detected
											* psEnc.sCmn.fs_kHz == 24))) { /* Forced down-switching due to WB input */
						psEnc.sCmn.sLP.transition_frame_no = 1; /* Begin transition phase */
						psEnc.sCmn.sLP.mode = 0; /* Switch down */
					} else if ((psEnc.sCmn.sLP.transition_frame_no >= TRANSITION_FRAMES_DOWN)
				...
				if (SWITCH_TRANSITION_FILTERING != 0) {
					if (((psEnc.sCmn.fs_kHz * 1000 < API_fs_Hz) && (TargetRate_bps >= psEnc.sCmn.bitrate_threshold_up)
							...
							&& (psEnc.sCmn.sLP.transition_frame_no == 0)) { /*
																			 * No transition phase running, ready to
																			 * switch
																			 */
...
//    //    #else
		...
		if (SWITCH_TRANSITION_FILTERING != 0) {
			...
			if ((psEnc.sCmn.sLP.mode == 1) && (psEnc.sCmn.sLP.transition_frame_no >= TRANSITION_FRAMES_UP)
		...
		if (psEnc.sCmn.fs_kHz != fs_kHz || psEnc.sCmn.prev_API_fs_Hz != API_fs_Hz) {
			/*
			 * Allocate space for worst case temporary upsampling, 8 to 48 kHz, so a factor
			 * 6
			 */
			...
			if (fs_kHz * 1000 < API_fs_Hz && psEnc.sCmn.fs_kHz != 0) {
				...
				ret += Resampler.SKP_Silk_resampler(temp_resampler_state, x_buf_API_fs_Hz, 0, x_bufFIX, 0,
						nSamples_temp);
			...
			if (1000 * fs_kHz != API_fs_Hz) {
				/*
				 * Correct resampler state (unless resampling by a factor 1) by resampling
				 * buffered data from API_fs_Hz to fs_kHz
				 */
				ret += Resampler.SKP_Silk_resampler(psEnc.sCmn.resampler_state, x_bufFIX, 0, x_buf_API_fs_Hz, 0,
						nSamples_temp);
			}
...
//            psEnc.NoiseShapingQuantizer            = SKP_Silk_NSQ_del_dec;
		...
		if (TargetRate_bps != psEnc.sCmn.TargetRate_bps) {
			...
			for (k = 1; k < TARGET_RATE_TAB_SZ; k++) {
				...
				if (TargetRate_bps < rateTable[k]) {
					...
					psEnc.SNR_dB = 0.5f * (TablesOther.SNR_table_Q1[k - 1]
							+ frac * (TablesOther.SNR_table_Q1[k] - TablesOther.SNR_table_Q1[k - 1]));
					break;
				}
			}
		}
		...
		/* Set packet size */
		if ((PacketSize_ms != 20) && (PacketSize_ms != 40) && (PacketSize_ms != 60) && (PacketSize_ms != 80)
				&& (PacketSize_ms != 100)) {
	...
	static void SKP_Silk_LBRR_ctrl_FLP(SKP_Silk_encoder_state_FLP psEnc, /* I/O Encoder state FLP */
		...
		if (psEnc.sCmn.LBRR_enabled != 0) {
			...
			if (psEnc.speech_activity > DefineFLP.LBRR_SPEECH_ACTIVITY_THRES
					&& psEnc.sCmn.PacketLoss_perc > LBRR_LOSS_THRES) { // nb! maybe multiply loss prob and speech
																		// activity
...
class NSQImplNSQ implements NoiseShapingQuantizerFP {
	@Override
	public void NoiseShapingQuantizer(SKP_Silk_encoder_state psEnc, SKP_Silk_encoder_control psEncCtrl,
			SKP_Silk_nsq_state nsq, short[] x, byte[] q, int arg6, short[] arg7, short[] arg8, short[] arg9,
...
class NSQImplNSQDelDec implements NoiseShapingQuantizerFP {
	...
	public void NoiseShapingQuantizer(SKP_Silk_encoder_state psEnc, SKP_Silk_encoder_control psEncCtrl,
			...
			int[] arg10, int[] arg11, int[] arg12, int[] arg13, int arg14, int arg15) {
		// TODO Auto-generated method stub
		NSQDelDec.SKP_Silk_NSQ_del_dec(psEnc, psEncCtrl, NSQ, x, q, arg6, arg7, arg8, arg9, arg10, arg11, arg12, arg13,
				arg14, arg15);
	}
}
