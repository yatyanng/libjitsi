...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.MAX_MATRIX_SIZE;

public class SolveLSFLP {
	/**
	 * Function to solve linear equation Ax = b, when A is a MxM symmetric square
	 * matrix - using LDL factorisation.
	 * 
...
//TODO:change L from two dimension to one dimension.
		...
		/***************************************************
		 * Factorize A by LDL such that A = L*D*(L^T), where L is lower triangular with
		 * ones on diagonal
		 ****************************************************/
...
//        SKP_Silk_SolveWithLowerTriangularWdiagOnes_FLP( &L[ 0 ][ 0 ], M, b, T );
		...
		/****************************************************
		 * D*(L^T)*x = T <=> (L^T)*x = inv(D)*T, because D is diagonal just multiply
		 * with 1/d_i
		 ****************************************************/
		for (i = 0; i < M; i++) {
			T[i] = T[i] * Dinv[i];
...
//            ptr1 =  matrix_adr( L, 0, i, M );
	...
	/**
	 * Function to solve linear equation Ax = b, when A is a MxM lower triangular
...
//            ptr1 =  matrix_adr( L, i, 0, M );
	...
	static void SKP_Silk_LDL_FLP(float[] A, /* (I/O) Pointer to Symetric Square Matrix */
		 ...
		 * diag_min_value; SKP_float v[ MAX_MATRIX_SIZE ], D[ MAX_MATRIX_SIZE ]; // temp
		 * arrays
		 */
