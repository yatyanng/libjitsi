...
public class Decimate2CoarseFLP {
	...
	static void SKP_Silk_decimate2_coarse_FLP(float[] in, /* I: 16 kHz signal [2*len] */
		...
		/* allpass filters */
		AllpassIntFLP.SKP_Silk_allpass_int_FLP(scratch, scratch_offset, S, S_offset, A21c_FLP[0], scratch,
				scratch_offset + 2 * len, len);
		AllpassIntFLP.SKP_Silk_allpass_int_FLP(scratch, scratch_offset + 2 * len, S, S_offset + 1, A21c_FLP[1], scratch,
				scratch_offset, len);

		AllpassIntFLP.SKP_Silk_allpass_int_FLP(scratch, scratch_offset + len, S, S_offset + 2, A20c_FLP[0], scratch,
				scratch_offset + 2 * len, len);
		AllpassIntFLP.SKP_Silk_allpass_int_FLP(scratch, scratch_offset + 2 * len, S, S_offset + 3, A20c_FLP[1], scratch,
				scratch_offset + len, len);
