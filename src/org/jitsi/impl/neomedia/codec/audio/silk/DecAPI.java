...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.NO_LBRR_THRES;
...
import static org.jitsi.impl.neomedia.codec.audio.silk.Typedef.SKP_assert;
...
public class DecAPI {
	 ...
	 * @return the returned value carries the error message. 0 indicates OK; other
	 *         indicates error.
	 */
	static int SKP_Silk_SDK_Decode(Object decState, /* I/O: State */
		...
		ret += DecodeFrame.SKP_Silk_decode_frame(psDec, samplesOut, samplesOut_offset, nSamplesOut, inData,
				inData_offset, nBytesIn, lostFlag, used_bytes_ptr);
		...
		if (psDec.fs_kHz * 1000 != decControl.API_sampleRate) {
			...
			ret += Resampler.SKP_Silk_resampler(psDec.resampler_state, samplesOut, samplesOut_offset, samplesOut_tmp, 0,
					nSamplesOut[0]);
		...
		/*
		 * Copy all parameters that are needed out of internal structure to the control
		 * stucture
		 */
	...
	static void SKP_Silk_SDK_search_for_LBRR(byte[] inData, /* I: Encoded input vector */
		...
		SKP_Silk_decoder_state sDec = new SKP_Silk_decoder_state(); // Local decoder state to avoid interfering with
																	// running decoder */
...
//          };
			if (((sDec.FrameTermination - 1) & lost_offset) != 0 && sDec.FrameTermination > 0 && sDec.nBytesLeft >= 0) {
				...
				System.arraycopy(inData, inData_offset + nBytesIn - sDec.nBytesLeft, LBRRData, LBRRData_offset + 0,
						sDec.nBytesLeft);
				break;
			}
