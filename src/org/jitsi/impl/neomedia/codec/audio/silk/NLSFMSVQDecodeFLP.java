...
public class NLSFMSVQDecodeFLP {
	...
	static void SKP_Silk_NLSF_MSVQ_decode_FLP(float[] pNLSF, /* O Decoded output vector [ LPC_ORDER ] */
		...
		assert (0 <= NLSFIndices[NLSFIndices_offset + 0]
				&& NLSFIndices[NLSFIndices_offset + 0] < psNLSF_CB_FLP.CBStages[0].nVectors);
		...
		for (s = 1; s < psNLSF_CB_FLP.nStages; s++) {
			/* Check that each index is within valid range */
			assert (0 <= NLSFIndices[NLSFIndices_offset + s]
					&& NLSFIndices[NLSFIndices_offset + s] < psNLSF_CB_FLP.CBStages[s].nVectors);
