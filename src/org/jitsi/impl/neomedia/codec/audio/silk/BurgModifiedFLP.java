...
public class BurgModifiedFLP {
	 ...
	 * @param subfr_length   input signal subframe length (including D preceeding
	 *                       samples).
	...
	static float SKP_Silk_burg_modified_FLP( /* O returns residual energy */
			...
			int x_offset, final int subfr_length, /* I input signal subframe length (including D preceeding samples) */
		...
		float[] x_ptr;
		int x_ptr_offset;
		double C_first_row[] = new double[SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC],
				C_last_row[] = new double[SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC];
		double CAf[] = new double[SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC + 1],
				CAb[] = new double[SigProcFIXConstants.SKP_Silk_MAX_ORDER_LPC + 1];
		...
		for (s = 0; s < nb_subfr; s++) {
			...
			for (n = 1; n < D + 1; n++) {
				C_first_row[n - 1] += InnerProductFLP.SKP_Silk_inner_product_FLP(x_ptr, x_ptr_offset, x_ptr,
