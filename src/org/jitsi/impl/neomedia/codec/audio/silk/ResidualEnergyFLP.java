...
//        LPC_res_ptr = LPC_res + LPC_order;
		...
		nrgs[0] = (float) (gains[0] * gains[0]
				* EnergyFLP.SKP_Silk_energy_FLP(LPC_res_ptr, LPC_res_ptr_offset + 0 * shift, subfr_length));
		nrgs[1] = (float) (gains[1] * gains[1]
				* EnergyFLP.SKP_Silk_energy_FLP(LPC_res_ptr, LPC_res_ptr_offset + 1 * shift, subfr_length));

		LPCAnalysisFilterFLP.SKP_Silk_LPC_analysis_filter_FLP(LPC_res, a[1], x, 0 + 2 * shift, 2 * shift, LPC_order);
		nrgs[2] = (float) (gains[2] * gains[2]
				* EnergyFLP.SKP_Silk_energy_FLP(LPC_res_ptr, LPC_res_ptr_offset + 0 * shift, subfr_length));
		nrgs[3] = (float) (gains[3] * gains[3]
				* EnergyFLP.SKP_Silk_energy_FLP(LPC_res_ptr, LPC_res_ptr_offset + 1 * shift, subfr_length));
	}
}
