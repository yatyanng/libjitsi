...
public class EncAPI {
	...
	static int SKP_Silk_SDK_Encode(Object encState, /* I/O: State */
		...
		if ((ret = ControlCodecFLP.SKP_Silk_control_encoder_FLP(psEnc, API_fs_Hz, max_internal_fs_kHz, PacketSize_ms,
				TargetRate_bps, PacketLoss_perc, UseInBandFEC, UseDTX, input_ms, Complexity)) != 0) {
			assert (false);
			return (ret);
		}
		/* Detect energy above 8 kHz */
		if (Math.min(API_fs_Hz, 1000 * max_internal_fs_kHz) == 24000 && psEnc.sCmn.sSWBdetect.SWB_detected == 0
		...
		while (true) {
			...
			if (API_fs_Hz == SKP_SMULBB(1000, psEnc.sCmn.fs_kHz)) {
				...
				/* Copy to buffer */
				System.arraycopy(samplesIn, samplesIn_offset, psEnc.sCmn.inputBuf, psEnc.sCmn.inputBufIx,
						nSamplesFromInput);
			} else {
				...
				ret += Resampler.SKP_Silk_resampler(psEnc.sCmn.resampler_state, psEnc.sCmn.inputBuf,
						psEnc.sCmn.inputBufIx, samplesIn, samplesIn_offset, nSamplesFromInput);
...
//                            MaxBytesOut_ptr, psEnc.sCmn.inputBuf, psEnc.sCmn.inputBufIx ) ) != 0 )
					if ((ret = EncodeFrameFLP.SKP_Silk_encode_frame_FLP(psEnc, outData, outData_offset, MaxBytesOut_ptr,
...
//                            nBytesOut, psEnc.sCmn.inputBuf, psEnc.sCmn.inputBufIx) ) != 0 )
					if ((ret = EncodeFrameFLP.SKP_Silk_encode_frame_FLP(psEnc, outData, outData_offset, nBytesOut,
							psEnc.sCmn.inputBuf, 0)) != 0) {
						assert (false);
					}
