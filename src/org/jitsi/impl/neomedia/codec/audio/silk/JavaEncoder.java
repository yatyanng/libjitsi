...
public class JavaEncoder extends AbstractCodec2 implements PacketLossAwareEncoder {
	/**
	 * The <tt>Logger</tt> used by this <tt>JavaEncoder</tt> instance for logging
	 * output.
	 */
	...
	/**
	 * If <tt>alwaysExpectPacketLoss</tt> is <tt>true</tt> the expected packet loss
	 * will always be set at or above this threshold.
	 */
	...
	/**
	 * Whether to always assume packet loss and set the encoder's expected packet
	 * loss over <tt>MIN_PACKET_LOSS_PERCENTAGE</tt>.
	 */
	...
	static {
		...
		for (int i = 0; i < supportedCount; i++) {
			...
			SUPPORTED_INPUT_FORMATS[i] = new AudioFormat(AudioFormat.LINEAR, supportedSampleRate, 16, 1,
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if (EncAPI.SKP_Silk_SDK_Encode(encState, encControl, inputData, inputOffset, inputLength, outputData,
	...
	static Format[] getMatchingOutputFormats(Format inputFormat, Format[] supportedInputFormats,
		...
		if (inputFormat == null)
			...
			if (matchingInputFormat == null)
				...
				Format outputFormat = new AudioFormat(null /* encoding */, matchingInputAudioFormat.getSampleRate(),
	...
	/**
	 * Stub. Only added in order to implement the <tt>PacketLossAwareEncoder</tt>
	 * interface.
	 *
	 * @return null
	 */
