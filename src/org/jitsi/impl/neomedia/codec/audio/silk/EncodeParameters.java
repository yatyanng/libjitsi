...
public class EncodeParameters {
	 ...
	 */
	static void SKP_Silk_encode_parameters(SKP_Silk_encoder_state psEncC, /* I/O Encoder state */
		...
		if (psEncCtrlC.sigtype == SIG_TYPE_VOICED) {
			...
			/* lag index */
			if (psEncC.fs_kHz == 8) {
				RangeCoder.SKP_Silk_range_encoder(psRC, psEncCtrlC.lagIndex, TablesPitchLag.SKP_Silk_pitch_lag_NB_CDF,
						0);
			} else if (psEncC.fs_kHz == 12) {
				RangeCoder.SKP_Silk_range_encoder(psRC, psEncCtrlC.lagIndex, TablesPitchLag.SKP_Silk_pitch_lag_MB_CDF,
						0);
			} else if (psEncC.fs_kHz == 16) {
				RangeCoder.SKP_Silk_range_encoder(psRC, psEncCtrlC.lagIndex, TablesPitchLag.SKP_Silk_pitch_lag_WB_CDF,
						0);
			} else {
				RangeCoder.SKP_Silk_range_encoder(psRC, psEncCtrlC.lagIndex, TablesPitchLag.SKP_Silk_pitch_lag_SWB_CDF,
						0);
			}
		...
		/*********************************************/
		EncodePulses.SKP_Silk_encode_pulses(psRC, psEncCtrlC.sigtype, psEncCtrlC.QuantOffsetType, q,
				psEncC.frame_length);
