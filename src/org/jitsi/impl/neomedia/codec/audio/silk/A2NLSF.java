...
public class A2NLSF extends A2NLSF_constants {
	 ...
	 * coefficients. If not all roots are found, the a_Q16 coefficients are
	 * bandwidth expanded until convergence.
	 * 
	 ...
	 */
	static void SKP_Silk_A2NLSF(int[] NLSF, /* O Normalized Line Spectral Frequencies, Q15 (0 - (2^15-1)), [d] */
		...
		while (true) {
			...
			if ((ylo <= 0 && yhi >= 0) || (ylo >= 0 && yhi <= 0)) {
				...
				if (OVERSAMPLE_COSINE_TABLE)
					b = (k > 2 * SigProcFIXConstants.LSF_COS_TAB_SZ_FIX);
				else
					b = (k > SigProcFIXConstants.LSF_COS_TAB_SZ_FIX);
				if (b) {
					i++;
