...
public class FindLTPFLP {
	...
	static void SKP_Silk_find_LTP_FLP(float b[], /* O LTP coefs */
		...
		for (k = 0; k < NB_SUBFR; k++) {
			...
			CorrMatrixFLP.SKP_Silk_corrMatrix_FLP(lag_ptr, lag_ptr_offset, subfr_length, LTP_ORDER, WLTP_ptr,
					WLTP_ptr_offset);
			CorrMatrixFLP.SKP_Silk_corrVector_FLP(lag_ptr, lag_ptr_offset, r_ptr, r_ptr_offset, subfr_length, LTP_ORDER,
					Rr);
			...
			RegularizeCorrelationsFLP.SKP_Silk_regularize_correlations_FLP(WLTP_ptr, WLTP_ptr_offset, rr, k, regu,
					LTP_ORDER);
			SolveLSFLP.SKP_Silk_solve_LDL_FLP(WLTP_ptr, WLTP_ptr_offset, LTP_ORDER, Rr, b_ptr, b_ptr_offset);

			/* Calculate residual energy */
			nrg[k] = ResidualEnergyFLP.SKP_Silk_residual_energy_covar_FLP(b_ptr, b_ptr_offset, WLTP_ptr,
					WLTP_ptr_offset, Rr, rr[k], LTP_ORDER);
