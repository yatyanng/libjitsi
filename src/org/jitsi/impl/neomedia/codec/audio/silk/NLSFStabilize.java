...
public class NLSFStabilize {
	 ...
	 * @param NDeltaMin_Q15   Normalized delta min vector in Q15, NDeltaMin_Q15[L]
	 *                        must be >= 1 [L+1].
	 * @param L               Number of NLSF parameters in the input vector.
	 */
	static void SKP_Silk_NLSF_stabilize(int[] NLSF_Q15, /* I/O: Unstable/stabilized normalized LSF vector in Q15 [L] */
			int NLSF_Q15_offset, int[] NDeltaMin_Q15, /*
														 * I: Normalized delta min vector in Q15, NDeltaMin_Q15[L] must
														 * be >= 1 [L+1]
														 */
		...
		for (loops = 0; loops < MAX_LOOPS; loops++) {
			...
			if (I == 0) {
				...
				center_freq_Q15 = SigProcFIX
						.SKP_LIMIT_32(
								SigProcFIX.SKP_RSHIFT_ROUND(
										NLSF_Q15[NLSF_Q15_offset + I - 1] + NLSF_Q15[NLSF_Q15_offset + I], 1),
		...
		if (loops == MAX_LOOPS) {
			...
			for (i = 1; i < L; i++)
				NLSF_Q15[NLSF_Q15_offset + i] = SigProcFIX.SKP_max_int(NLSF_Q15[NLSF_Q15_offset + i],
						NLSF_Q15[NLSF_Q15_offset + i - 1] + NDeltaMin_Q15[i]);

			/* Last NLSF should be no higher than 1 - NDeltaMin[L] */
			NLSF_Q15[NLSF_Q15_offset + L - 1] = SigProcFIX.SKP_min_int(NLSF_Q15[NLSF_Q15_offset + L - 1],
					(1 << 15) - NDeltaMin_Q15[L]);

			/* Keep NDeltaMin distance between the NLSFs */
			for (i = L - 2; i >= 0; i--)
				NLSF_Q15[NLSF_Q15_offset + i] = SigProcFIX.SKP_min_int(NLSF_Q15[NLSF_Q15_offset + i],
						NLSF_Q15[NLSF_Q15_offset + i + 1] - NDeltaMin_Q15[i + 1]);
		}
	}
	...
	static void SKP_Silk_NLSF_stabilize_multi(int[] NLSF_Q15, /*
																 * I/O: Unstable/stabilized normalized LSF vectors in
																 * Q15 [LxN]
																 */
