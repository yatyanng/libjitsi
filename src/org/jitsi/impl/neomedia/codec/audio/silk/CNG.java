...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.CNG_BUF_MASK_MAX;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.CNG_GAIN_SMTH_Q16;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.CNG_NLSF_SMTH_Q16;
...
public class CNG {
	...
	static void SKP_Silk_CNG_exc(short residual[], /* O CNG residual signal Q0 */
		...
		for (i = 0; i < length; i++) {
			...
			residual[residual_offset + i] = (short) SigProcFIX
					.SKP_SAT16(SigProcFIX.SKP_RSHIFT_ROUND(SKP_SMULWW(exc_buf_Q10[idx], Gain_Q16), 10));
		}
		rand_seed[0] = seed;
	}
	...
	static void SKP_Silk_CNG(SKP_Silk_decoder_state psDec, /* I/O Decoder state */
		...
		if (psDec.lossCnt == 0 && psDec.vadFlag == NO_VOICE_ACTIVITY) {
			...
			/* Smoothing of LSF's */
			for (i = 0; i < psDec.LPC_order; i++) {
				psCNG.CNG_smth_NLSF_Q15[i] += SKP_SMULWB(psDec.prevNLSF_Q15[i] - psCNG.CNG_smth_NLSF_Q15[i],
						CNG_NLSF_SMTH_Q16);
			}
			...
			/* Smooth gains */
			for (i = 0; i < NB_SUBFR; i++) {
				psCNG.CNG_smth_Gain_Q16 += SKP_SMULWB(psDecCtrl.Gains_Q16[i] - psCNG.CNG_smth_Gain_Q16,
						CNG_GAIN_SMTH_Q16);
			}
		}
		...
		if (psDec.lossCnt != 0) {// || psDec.vadFlag == NO_VOICE_ACTIVITY ) {
			...
			if (psDec.LPC_order == 16) {
				LPCSynthesisOrder16.SKP_Silk_LPC_synthesis_order16(CNG_sig, LPC_buf, Gain_Q26, psCNG.CNG_synth_state,
						CNG_sig, length);
			} else {
				LPCSynthesisFilter.SKP_Silk_LPC_synthesis_filter(CNG_sig, LPC_buf, Gain_Q26, psCNG.CNG_synth_state,
						CNG_sig, length, psDec.LPC_order);
			}
			/* Mix with signal */
			for (i = 0; i < length; i++) {
