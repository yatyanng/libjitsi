...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.BWE_AFTER_LOSS_Q16;
...
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.NLSF_MSVQ_MAX_CB_STAGES;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.RANGE_CODER_ILLEGAL_SAMPLING_RATE;
import static org.jitsi.impl.neomedia.codec.audio.silk.Define.RANGE_CODER_READ_BEYOND_BUFFER;
...
public class DecodeParameters {
	...
	static void SKP_Silk_decode_parameters(SKP_Silk_decoder_state psDec, /* I/O State */
		...
		if (psDec.nFramesDecoded == 0) {
			RangeCoder.SKP_Silk_range_decoder(Ix_ptr, 0, psRC, TablesOther.SKP_Silk_SamplingRates_CDF, 0,
		...
		if (psDec.nFramesDecoded == 0) {
			...
			RangeCoder.SKP_Silk_range_decoder(Ix_ptr, 0, psRC, TablesTypeOffset.SKP_Silk_type_offset_CDF, 0,
		...
		if (psDec.nFramesDecoded == 0) {
			/* first frame in packet: independent coding */
			RangeCoder.SKP_Silk_range_decoder(GainsIndices, 0, psRC, TablesGain.SKP_Silk_gain_CDF[psDecCtrl.sigtype], 0,
					TablesGain.SKP_Silk_gain_CDF_offset);
		} else {
			/* condidtional coding */
			RangeCoder.SKP_Silk_range_decoder(GainsIndices, 0, psRC, TablesGain.SKP_Silk_delta_gain_CDF, 0,
					TablesGain.SKP_Silk_delta_gain_CDF_offset);
		}

		/* remaining subframes */
		for (i = 1; i < NB_SUBFR; i++) {
			RangeCoder.SKP_Silk_range_decoder(GainsIndices, i, psRC, TablesGain.SKP_Silk_delta_gain_CDF, 0,
					TablesGain.SKP_Silk_delta_gain_CDF_offset);
		}
		...
		RangeCoder.SKP_Silk_range_decoder_multi(NLSFIndices, psRC, psNLSF_CB.StartPtr, psNLSF_CB.MiddleIx,
				psNLSF_CB.nStages);
		...
		if (psDecCtrl.sigtype == SIG_TYPE_VOICED) {
			...
			/* Get lag index */
			if (psDec.fs_kHz == 8) {
				RangeCoder.SKP_Silk_range_decoder(Ixs, 0, psRC, TablesPitchLag.SKP_Silk_pitch_lag_NB_CDF, 0,
						TablesPitchLag.SKP_Silk_pitch_lag_NB_CDF_offset);
			} else if (psDec.fs_kHz == 12) {
				RangeCoder.SKP_Silk_range_decoder(Ixs, 0, psRC, TablesPitchLag.SKP_Silk_pitch_lag_MB_CDF, 0,
						TablesPitchLag.SKP_Silk_pitch_lag_MB_CDF_offset);
			} else if (psDec.fs_kHz == 16) {
				RangeCoder.SKP_Silk_range_decoder(Ixs, 0, psRC, TablesPitchLag.SKP_Silk_pitch_lag_WB_CDF, 0,
						TablesPitchLag.SKP_Silk_pitch_lag_WB_CDF_offset);
			} else {
				RangeCoder.SKP_Silk_range_decoder(Ixs, 0, psRC, TablesPitchLag.SKP_Silk_pitch_lag_SWB_CDF, 0,
			...
			if (psDec.fs_kHz == 8) {
				...
				RangeCoder.SKP_Silk_range_decoder(Ixs, 1, psRC, TablesPitchLag.SKP_Silk_pitch_contour_NB_CDF, 0,
...
//                SKP_Silk_range_decoder( &Ixs[ 1 ], psRC, SKP_Silk_pitch_contour_CDF, SKP_Silk_pitch_contour_CDF_offset );
				RangeCoder.SKP_Silk_range_decoder(Ixs, 1, psRC, TablesPitchLag.SKP_Silk_pitch_contour_CDF, 0,
			...
			/* Decode LTP scaling */
			/**********************/
			RangeCoder.SKP_Silk_range_decoder(Ix_ptr, 0, psRC, TablesOther.SKP_Silk_LTPscale_CDF, 0,
		...
		RangeCoder.SKP_Silk_range_decoder(vadFlag_ptr, 0, psRC, TablesOther.SKP_Silk_vadflag_CDF, 0,
		...
		RangeCoder.SKP_Silk_range_decoder(FrameTermination_ptr, 0, psRC, TablesOther.SKP_Silk_FrameTermination_CDF, 0,
