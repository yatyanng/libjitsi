...
import static org.jitsi.impl.neomedia.codec.audio.silk.CommonPitchEstDefines.PITCH_EST_D_SRCH_LENGTH;
import static org.jitsi.impl.neomedia.codec.audio.silk.CommonPitchEstDefines.PITCH_EST_FRAME_LENGTH_MS;
import static org.jitsi.impl.neomedia.codec.audio.silk.CommonPitchEstDefines.PITCH_EST_MAX_DECIMATE_STATE_LENGTH;
import static org.jitsi.impl.neomedia.codec.audio.silk.CommonPitchEstDefines.PITCH_EST_MAX_FRAME_LENGTH;
import static org.jitsi.impl.neomedia.codec.audio.silk.CommonPitchEstDefines.PITCH_EST_MAX_LAG;
import static org.jitsi.impl.neomedia.codec.audio.silk.CommonPitchEstDefines.PITCH_EST_MAX_LAG_MS;
import static org.jitsi.impl.neomedia.codec.audio.silk.CommonPitchEstDefines.PITCH_EST_MIN_LAG_MS;
import static org.jitsi.impl.neomedia.codec.audio.silk.CommonPitchEstDefines.PITCH_EST_NB_CBKS_STAGE2;
import static org.jitsi.impl.neomedia.codec.audio.silk.CommonPitchEstDefines.PITCH_EST_NB_CBKS_STAGE2_EXT;
import static org.jitsi.impl.neomedia.codec.audio.silk.CommonPitchEstDefines.PITCH_EST_NB_CBKS_STAGE3_MAX;
import static org.jitsi.impl.neomedia.codec.audio.silk.CommonPitchEstDefines.PITCH_EST_NB_STAGE3_LAGS;
import static org.jitsi.impl.neomedia.codec.audio.silk.CommonPitchEstDefines.PITCH_EST_NB_SUBFR;
import static org.jitsi.impl.neomedia.codec.audio.silk.Macros.SKP_SMULBB;
import static org.jitsi.impl.neomedia.codec.audio.silk.PitchEstDefinesFLP.PITCH_EST_FLP_FLATCONTOUR_BIAS;
import static org.jitsi.impl.neomedia.codec.audio.silk.PitchEstDefinesFLP.PITCH_EST_FLP_PREVLAG_BIAS;
import static org.jitsi.impl.neomedia.codec.audio.silk.PitchEstDefinesFLP.PITCH_EST_FLP_SHORTLAG_BIAS;
...
public class PitchAnalysisCoreFLP {
	...
	static int SKP_Silk_pitch_analysis_core_FLP( /* O voicing estimate: 0 voiced, 1 unvoiced */
		...
		float[][] C = new float[PITCH_EST_NB_SUBFR][(PITCH_EST_MAX_LAG >> 1)
				+ 5]; /* use to be +2 but then valgrind reported errors for SWB */
		...
		if (Fs_kHz == 12) {
			...
			if (complexity == SigProcFIXConstants.SKP_Silk_PITCH_EST_MAX_COMPLEX) {
				...
				Decimate2CoarseFLP.SKP_Silk_decimate2_coarse_FLP(signal, 0, filt_state, 0, signal_8kHz, 0, scratch_mem,
						0, frame_length_8kHz);
			} else {
		...
		if (complexity == SigProcFIXConstants.SKP_Silk_PITCH_EST_MAX_COMPLEX) {
			...
			for (int i_djinn = 0; i_djinn < 4; i_djinn++)
				filt_state[i_djinn] = 0;
			Decimate2CoarseFLP.SKP_Silk_decimate2_coarse_FLP(signal_8kHz, 0, filt_state, 0, signal_4kHz, 0, scratch_mem,
					0, frame_length_4kHz);
		} else {
			assert (2 <= PITCH_EST_MAX_DECIMATE_STATE_LENGTH);
			for (int i_djinn = 0; i_djinn < 4; i_djinn++)
				filt_state[i_djinn] = 0;
			Decimate2CoarsestFLP.SKP_Silk_decimate2_coarsest_FLP(signal_8kHz, 0, filt_state, 0, signal_4kHz, 0,
					scratch_mem, 0, frame_length_4kHz);
		}
		...
		for (k = 0; k < 2; k++) {
			...
			cross_corr = InnerProductFLP.SKP_Silk_inner_product_FLP(target_ptr, target_ptr_offset, basis_ptr,
...
//                        basis_ptr[ sf_length_8kHz ] * basis_ptr[ sf_length_8kHz ];
		...
		SortFLP.SKP_Silk_insertion_sort_decreasing_FLP(C[0], min_lag_4kHz, d_srch, max_lag_4kHz - min_lag_4kHz + 1,
				length_d_srch);
		...
		for (k = 0; k < PITCH_EST_NB_SUBFR; k++) {
			...
			for (j = 0; j < length_d_comp; j++) {
				...
				cross_corr = InnerProductFLP.SKP_Silk_inner_product_FLP(basis_ptr, basis_ptr_offset, target_ptr,
						target_ptr_offset, sf_length_8kHz);
		...
		if (Fs_kHz == 8 && complexity > SigProcFIXConstants.SKP_Silk_PITCH_EST_MIN_COMPLEX) {
		...
		for (k = 0; k < length_d_srch; k++) {
			...
			if (prevLag > 0) {
				...
				CCmax_new_b -= PITCH_EST_FLP_PREVLAG_BIAS * PITCH_EST_NB_SUBFR * LTPCorr[0] * delta_lag_log2_sqr
	 ...
	 */
	static void SKP_P_Ana_calc_corr_st3(float[][][] cross_corr_st3, float signal[], /* I vector to correlate */
	 ...
	 **********************************************************************/
	{
		...
		for (k = 0; k < PITCH_EST_NB_SUBFR; k++) {
			...
			for (j = PitchEstTables.SKP_Silk_Lag_range_stage3[complexity][k][0]; j <= PitchEstTables.SKP_Silk_Lag_range_stage3[complexity][k][1]; j++) {
				...
				scratch_mem[lag_counter] = (float) InnerProductFLP.SKP_Silk_inner_product_FLP(target_ptr,
						target_ptr_offset, basis_ptr, basis_ptr_offset, sf_length);
				lag_counter++;
			}
	 ...
	 * @param complexity    Complexity setting.
	 */
	static void SKP_P_Ana_calc_energy_st3(float[][][] energies_st3, float signal[], /* I vector to correlate */
			int signal_offset, int start_lag, /* I start lag */
			int sf_length, /* I sub frame length */
			int complexity /* I Complexity setting */
	)
	/****************************************************************
	 * Calculate the energies for first two subframes. The energies are calculated
	 * recursively.
	 ****************************************************************/
	{
		float[] target_ptr, basis_ptr;
