...
public class FindLPCFLP {
	...
	static void SKP_Silk_find_LPC_FLP(float NLSF[], /* O NLSFs */
		...
		if (useInterpNLSFs == 1) {
			 ...
			 */
			/*
			 * adding it to the residual energy of the first 10 ms in each iteration of the
			 * search below
			 */
			res_nrg -= BurgModifiedFLP.SKP_Silk_burg_modified_FLP(a_tmp, x, (NB_SUBFR / 2) * subfr_length, subfr_length,
					NB_SUBFR / 2, DefineFLP.FIND_LPC_COND_FAC, LPC_order);
			...
			for (k = 3; k >= 0; k--) {
				...
				LPCAnalysisFilterFLP.SKP_Silk_LPC_analysis_filter_FLP(LPC_res, a_tmp, x, 0, 2 * subfr_length,
						LPC_order);
				res_nrg_interp = EnergyFLP.SKP_Silk_energy_FLP(LPC_res, LPC_order, subfr_length - LPC_order)
