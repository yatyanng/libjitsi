...
public class CachingPushBufferStream implements PushBufferStream {
	 ...
	 * <tt>CachingPushBufferStream</tt>. Generally, defined to a relatively large
	 * value which allows large buffering and yet tries to prevent
	 * <tt>OutOfMemoryError</tt>.
	 */
	private static final int MAX_CACHE_SIZE = 1024;

	/**
	 * The <tt>BufferControl</tt> of this <tt>PushBufferStream</tt> which allows the
	...
	/**
	 * The list of <tt>Buffer</tt>s in which this instance stores the data it reads
	 * from the wrapped <tt>PushBufferStream</tt> and from which it reads in chunks
	 * later on when its {@link #read(Buffer)} method is called.
	 */
	 ...
	 * <tt>#read(Buffer)</tt> method of the wrapped stream and to be thrown by this
	 * stream on the earliest call of its <tt>#read(Buffer)</tt> method.
	 */
	private IOException readException;

	/**
	 * The <tt>PushBufferStream</tt> being paced by this instance with respect to
	 * the maximum number of data units it provides in a single push.
	 */
	...
	/**
	 * Initializes a new <tt>CachingPushBufferStream</tt> instance which is to pace
	 * the number of per-push data units a specific <tt>PushBufferStream</tt>
	 * provides.
	 *
	 * @param stream the <tt>PushBufferStream</tt> to be paced with respect to the
	 ...
	 * acceptable given the maximum size of the <tt>cache</tt> and the length of the
	 * media currently available in it.
	 *
	 * @return <tt>true</tt> if adding a new <tt>Buffer</tt> to <tt>cache</tt> is
	 *         acceptable; otherwise, <tt>false</tt> which means that the reading
	...
	private boolean canWriteInCache() {
		synchronized (cache) {
			...
			/*
			 * Obviously, if there's nothing in the cache, we desperately want something to
			 * be written into it.
			 */
			if (cacheSize < 1)
				return true;
			...
			/*
			 * There is no bufferLength specified by a BufferControl so don't buffer
			 * anything.
			 */
			if (bufferLength < 1)
				return false;
			/*
			 * Having Buffers in the cache and yet not having their length in milliseconds
			 * is weird so don't buffer anything.
			 */
			if (cacheLengthInMillis < 1)
				return false;
			/*
			 * Of course, if the media in the cache hasn't reached the specified buffer
			 * length, write more to the cache.
			 */
	 ...
	 *
	 * @return <tt>true</tt> if this <tt>PushBufferStream</tt> has reached the end
	 *         of the content it makes available; otherwise, <tt>false</tt>
	 */
	@Override
	public boolean endOfStream() {
		/*
		 * TODO If the cache is still not exhausted, don't report the end of this stream
	 ...
	 * allows the adjustment of the size of the buffering it performs. If it does
	 * not exist yet, it is created.
	 *
	 * @return the <tt>BufferControl</tt> of this <tt>PushBufferStream</tt> which
	 *         allows the adjustment of the size of the buffering it performs
	 */
	 ...
	 *
	 * @return a <tt>ContentDescriptor</tt> which describes the type of the content
	 *         made available by the wrapped <tt>PushBufferStream</tt>
	 */
	@Override
	...
	/**
	 * Implements {@link javax.media.Controls#getControl(String)}. Delegates to the
	 * wrapped <tt>PushBufferStream</tt> and gives access to the
	 ...
	 *                    control of the wrapped <tt>PushBufferStream</tt> to be
	 *                    retrieved
	 * @return an <tt>Object</tt> which represents the control of the wrapped
	 *         <tt>PushBufferStream</tt> with the specified type if such a control
	 ...
	 * wrapped <tt>PushBufferStream</tt> and adds the <tt>BufferControl</tt> of this
	 * instance if the wrapped <tt>PushBufferStream</tt> does not have a control of
	 * such type available.
	 *
	 * @return an array of <tt>Object</tt>s which represent the control available
	 *         for the wrapped <tt>PushBufferStream</tt>
	 */
	@Override
	...
	/**
	 * Gets the length in milliseconds of the media in a specific <tt>Buffer</tt>
	 ...
	 * @return the length in milliseconds of the media in <tt>buffer</tt> if there
	 *         actually is media in <tt>buffer</tt> and its length in milliseconds
	...
	/**
	 * Implements {@link PushBufferStream#read(Buffer)}. If an <tt>IOException</tt>
	 * has been thrown by the wrapped stream when data was last read from it,
	 * re-throws it. If there has been no such exception, reads from the cache of
	 * this instance.
	 *
	 * @param buffer the <tt>Buffer</tt> to receive the read media data
	 * @throws IOException if the wrapped stream has thrown such an exception when
	 *                     data was last read from it
	 */
	@Override
	...
	/**
	 * Reads data from a specific input <tt>Buffer</tt> (if such data is available)
	 * and writes the read data into a specific output <tt>Buffer</tt>. The input
	 * <tt>Buffer</tt> will be modified to reflect the number of read data units. If
	 * the output <tt>Buffer</tt> has allocated an array for storing the read data
	 * and the type of this array matches that of the input <tt>Buffer</tt>, it will
	 * be used and thus the output <tt>Buffer</tt> may control the maximum number of
	 * data units to be read into it.
	 *
	 * @param in        the <tt>Buffer</tt> to read data from
	 * @param out       the <tt>Buffer</tt> into which to write the data read from
	 *                  the specified <tt>in</tt>
	 ...
	 *                     including if either of the formats of <tt>in</tt> and
	 *                     <tt>out</tt> are not supported
	 */
	private int read(Buffer in, Buffer out, int outOffset) throws IOException {
		...
		if (outData != null) {
			...
			if (inData.getClass().equals(dataType) && dataType.equals(byte[].class)) {
				...
				/*
				 * If we're currently continuing a concatenation, the parameters of the first
				 * read from input are left as the parameters of output. Mostly done at least
				 * for timeStamp.
				 */
				if (out.getOffset() == outOffset) {
					 ...
					 * Buffer.TIME_UNKNOWN is possibly the calculation of the output duration as the
					 * input duration multiplied by the ratio between the current output length and
					 * the initial input length.
					 */
		...
		/*
		 * If we were supposed to continue a concatenation and we discovered that it
		 * could not be continued, flush whatever has already been written to the
		 * caller.
		 */
		if (out.getOffset() == outOffset) {
			out.copy(in);
	...
	/**
	 * Implements
	 * {@link PushBufferStream#setTransferHandler(BufferTransferHandler)}. Delegates
	 * to the wrapped <tt>PushBufferStream<tt> but wraps the specified
	 ...
	 * calls in question.
	 *
	 * @param transferHandler the <tt>BufferTransferHandler</tt> to be notified by
	 *                        this <tt>PushBufferStream</tt> when media data is
	...
	/**
	 * Reads data from the wrapped/input <tt>PushBufferStream</tt> into the cache of
	 * this stream if the cache accepts it. If the cache does not accept a new read,
	 * blocks the calling thread until the cache accepts a new read and data is read
	 * from the wrapped <tt>PushBufferStream</tt> into the cache.
	 *
	...
	protected void transferData(BufferTransferHandler transferHandler) {
		/*
		 * Obviously, we cannot cache every Buffer because we will run out of memory. So
		 * wait for room to appear within cache (or for this instance to be stopped, of
		 * course).
		 */
		...
		if (interrupted) {
			...
			/*
			 * The protocol of PushBufferStream's #read(Buffer) method is that it does not
			 * block. The underlying implementation may be flawed though so we would better
			 * not take any chances. Besides, we have a report at the time of this writing
			 * which suggests that we may really be hitting a rogue implementation in a
			 * real-world scenario.
			 */
			...
			if (readException == null) {
				if (!buffer.isDiscard() && (buffer.getLength() != 0) && (buffer.getData() != null)) {
					/*
					 * Well, we risk disagreeing with #canWriteInCache() because we have temporarily
					 * released the cache but we have read a Buffer from the stream so it is
					 * probably better to not throw it away.
					 */
					synchronized (cache) {
						cache.add(buffer);
						cacheLengthInMillis += getLengthInMillis(buffer);
	...
	private static class BufferControlImpl implements BufferControl {
		...
		/**
		 * The minimum threshold in milliseconds for the buffering performed by the
		 ...
		 * milliseconds of the buffering performed by the owner of this instance.
		 *
		 * @return the length in milliseconds of the buffering performed by the owner of
		 *         this instance; {@link BufferControl#DEFAULT_VALUE} if it is up to the
		 *         owner of this instance to decide the length in milliseconds of the
		 *         buffering to perform if any
		 */
		@Override
		 ...
		 * <tt>Component</tt> representing this instance and exported by the owner of
		 * this instance. Returns <tt>null</tt>.
		 *
		 * @return the UI <tt>Component</tt> representing this instance and exported by
		 *         the owner of this instance if such a <tt>Component</tt> is available;
		...
		/**
		 * Implements {@link BufferControl#getEnabledThreshold()}. Gets the indicator
		 * which determines whether threshold calculations are enabled.
		 *
		 * @return <tt>true</tt> if threshold calculations are enabled; otherwise,
		 *         <tt>false</tt>
		 */
		@Override
		 ...
		 * threshold in milliseconds for the buffering performed by the owner of this
		 * instance.
		 *
		 * @return the minimum threshold in milliseconds for the buffering performed by
		...
		/**
		 * Implements {@link BufferControl#setBufferLength(long)}. Sets the length in
		 * milliseconds of the buffering to be performed by the owner of this instance
		 * and returns the value actually in effect after attempting to set it to the
		 * specified value.
		 *
		 * @param bufferLength the length in milliseconds of the buffering to be
		 *                     performed by the owner of this instance
		 * @return the length in milliseconds of the buffering performed by the owner of
		 *         this instance that is actually in effect after the attempt to set it
		 *         to the specified <tt>bufferLength</tt>
		 */
		@Override
		...
		/**
		 * Implements {@link BufferControl#setEnabledThreshold(boolean)}. Sets the
		 * indicator which determines whether threshold calculations are enabled.
		 *
		 * @param enabledThreshold <tt>true</tt> if threshold calculations are to be
		 ...
		 * threshold in milliseconds for the buffering to be performed by the owner of
		 * this instance and returns the value actually in effect after attempting to
		 * set it to the specified value.
		 *
		 * @param minimumThreshold the minimum threshold in milliseconds for the
		 *                         buffering to be performed by the owner of this
		 *                         instance
		 * @return the minimum threshold in milliseconds for the buffering performed by
		 *         the owner of this instance that is actually in effect after the
		 *         attempt to set it to the specified <tt>minimumThreshold</tt>
		 */
		@Override
