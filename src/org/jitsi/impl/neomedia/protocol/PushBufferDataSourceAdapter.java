...
public class PushBufferDataSourceAdapter extends PushBufferDataSourceDelegate<PullBufferDataSource> {
	...
	private static class PushBufferStreamAdapter implements PushBufferStream {
		...
		/**
		 * The indicator which determines whether {@link #buffer} contains media data
		 * read by this instance from {@link #stream} and not returned by this
		 * implementation of {@link PushBufferStream#read(Buffer)} yet.
		 */
		private boolean bufferIsWritten = false;

		/**
		 * The indicator which determined whether {@link #start()} has been called
		 ...
		 * {@link PullBufferStream#read(Buffer)} on {@link #stream} and which still
		 * hasn't been rethrown by this implementation of
		 * {@link PushBufferStream#read(Buffer)}.
		 */
		private IOException streamReadException;

		/**
		 * The <tt>Thread</tt> which currently reads media data from {@link #stream}
		 * into {@link #buffer}.
		 */
		 ...
		 * <tt>PushBufferStream</tt> notifies its user that media data is available for
		 * reading.
		 */
		private BufferTransferHandler transferHandler;

		/**
		 * Initializes a new <tt>PushBufferStreamAdapter</tt> instance which is to
		 * implement <tt>PushBufferStream</tt> for a specific <tt>PullBufferStream</tt>.
		 *
		...
		/**
		 * Disposes of this <tt>PushBufferStreamAdapter</tt>. Afterwards, this instance
		 * is not guaranteed to be operation and considered to be available for garbage
		 * collection.
		 */
		void close() {
			stop();
		 ...
		 * <tt>PullBufferStream</tt>.
		 *
		 * @return <tt>true</tt> if the wrapped <tt>PullBufferStream</tt> has reached
		 *         the end of the media data; otherwise, <tt>false</tt>
		 */
		@Override
		 ...
		 * <tt>PullBufferStream</tt>.
		 *
		 * @return the length of the content the wrapped <tt>PullBufferStream</tt> gives
		 *         access to
		 */
		@Override
		 ...
		 * wrapped <tt>PullBufferStream</tt>.
		 *
		 * @param controlType a <tt>String</tt> value which specifies the type of the
		 ...
		 *         <tt>PushBufferStream</tt> has such a control; <tt>null</tt> if the
		 *         wrapped <tt>PushBufferStream</tt> does not have a control of the
		 *         specified type
		 */
		@Override
		 ...
		 * @throws IOException if anything wrong happens while reading media data from
		...
		private void runInStreamReadThread() {
			...
			synchronized (buffer) {
				...
				/*
				 * If an exception has been thrown by the stream's read method, it may be better
				 * to give the stream's underlying implementation (e.g. PortAudio) a little time
				 * to possibly get its act together.
				 */
		 ...
		 * means through which this <tt>PushBufferStream</tt> is to notify its user that
		 * media data is available for reading.
		 *
		 * @param transferHandler the <tt>BufferTransferHandler</tt> through which
		 *                        <tt>PushBufferStream</tt> is to notify its user that
		 *                        media data is available for reading
		 */
		@Override
		...
		/**
		 * Starts the reading of media data of this <tt>PushBufferStreamAdapter</tt>
		 * from the wrapped <tt>PullBufferStream</tt>.
		 */
		void start() {
		...
		/**
		 * Stops the reading of media data of this <tt>PushBufferStreamAdapter</tt> from
				...
				if (STRICT_STOP) {
					...
					while (streamReadThread != null)
						try {
							...
							logger.info(getClass().getSimpleName() + " interrupted while waiting for"
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>PushBufferDataSourceAdapter</tt> class
	 * and its instances for logging output.
	 */
	private static final Logger logger = Logger.getLogger(PushBufferDataSourceAdapter.class);

	/**
	 * The indicator which determines whether the <tt>PushBufferStreamAdapater</tt>
	 * instances should wait for their
	 ...
	 * this <tt>DataSource</tt> without a subsequent call to {@link #stop()}.
	 */
	private boolean started = false;
	 ...
	 * <tt>PushBufferDataSource</tt> capabilities for a specific
	 * <tt>PullBufferDataSource</tt>.
	 *
	 * @param dataSource the <tt>PullBufferDataSource</tt> the new instance is to
	 ...
	 * <tt>PushBufferStreamAdapter</tt>s which wrap the <tt>PullBufferStream</tt>s
	 * of the <tt>PullBufferDataSource</tt> wrapped by this instance.
	 */
	@Override
	 ...
	 * <tt>PushBufferStream</tt>s through which this <tt>PushBufferDataSource</tt>
	 * gives access to its media data.
	 *
	...
	public PushBufferStream[] getStreams() {
		synchronized (streams) {
			...
			/*
			 * I don't know whether dataSource returns a copy of its internal storage so I'm
			 * not sure if it's safe to modify dataSourceStreams.
			 */
			...
			/*
			 * Dispose of the PushBufferStreamAdapters which adapt PullBufferStreams which
			 * are no longer returned by dataSource.
			 */
			Iterator<PushBufferStreamAdapter> streamIter = streams.iterator();
			...
			/*
			 * Create PushBufferStreamAdapters for the PullBufferStreams returned by
			 * dataSource which are not adapted yet.
			 */
			for (int dataSourceStreamIndex = 0; dataSourceStreamIndex < dataSourceStreamCount; dataSourceStreamIndex++) {
	...
	public void start() throws IOException {
		logger.debug("Starting Data source deletgate: " + super.getClass().getName());
		super.start();
	...
	public void stop() throws IOException {
		logger.debug("Stopping Data source deletgate: " + super.getClass().getName());
		synchronized (streams) {
			started = false;
