...
public class TranscodingDataSource extends DataSource {
	private static final Logger logger = Logger.getLogger(TranscodingDataSource.class);
	/**
	...
	/**
	 * The <tt>Processor</tt> which carries out the actual transcoding of the tracks
	 * of <tt>inputDataSource</tt>.
	 */
	private Processor transcodingProcessor;

	/**
	 * Initializes a new <tt>TranscodingDataSource</tt> instance to transcode the
	 * tracks of a specific <tt>DataSource</tt> into a specific output
	 * <tt>Format</tt>.
	 *
	 * @param inputDataSource the <tt>DataSource</tt> which is to have its tracks
	 *                        transcoded in a specific output <tt>Format</tt>
	...
	/**
	 * Implements {@link DataSource#connect()}. Sets up the very transcoding process
	 * and just does not start it i.e. creates a <tt>Processor</tt> on the
	 * <tt>inputDataSource</tt>, sets <tt>outputFormat</tt> on its tracks (which
	 * support a <tt>Format</tt> compatible with <tt>outputFormat</tt>) and connects
	 * to its <tt>output DataSource</tt>.
	 *
	 * @throws IOException if creating the transcoding <tt>Processor</tt>, setting
	 *                     its <tt>Format</tt> or connecting to it fails
	 */
	@Override
	public synchronized void connect() throws IOException {
		...
		/*
		 * Manager#createProcessor(DataSource) requires the specified DataSource to be
		 * connected.
		 */
		...
		try {
			processor = ForbiddenManager.createProcessor(inputDataSource);
	...
	/**
	 * Implements {@link DataSource#disconnect()}. Stops and undoes the whole setup
	 * of the very transcoding process i.e. disconnects from the output
	 ...
	 *
	 * @return a <tt>String</tt> value which describes the type of the content made
	 ...
	 * @return an <tt>Object</tt> which represents the control of this instance with
	 *         the specified type if such a control is available; otherwise,
	 *         <tt>null</tt>
	 */
	@Override
	public synchronized Object getControl(String controlType) {
		/*
		 * The Javadoc of DataSource#getControl(String) says it's an error to call the
	...
	/**
	 * Implements {@link DataSource#getControls()}. Delegates to the actual output
	 * of the transcoding.
	 *
	...
	/**
	 * Implements {@link DataSource#getDuration()}. Delegates to the actual output
	 * of the transcoding.
	 *
	...
	/**
	 * Gets the output streams that this instance provides. Some of them may be the
	 * result of transcoding the tracks of the input <tt>DataSource</tt> of this
	 * instance in the output <tt>Format</tt> of this instance.
	 *
	 * @return an array of <tt>SourceStream</tt>s which represents the collection of
	 *         output streams that this instance provides
	 */
	...
	/**
	 * Implements {@link DataSource#start()}. Starts the actual transcoding process
	 * already set up with {@link #connect()}.
	 *
	...
	public synchronized void start() throws IOException {
		/*
		 * The Javadoc of DataSource#start() says it's an error to call the method
		 * without being connected and by that time we should have the outputDataSource.
		 */
		logger.debug("Starting data source: " + outputDataSource.getClass().getName());
		outputDataSource.start();
		logger.debug("Starting transcoding processor: " + transcodingProcessor.getClass().getName());
		transcodingProcessor.start();
	}

	/**
	 * Implements {@link DataSource#stop()}. Stops the actual transcoding process if
	 * it has already been set up with {@link #connect()}.
	 *
	...
	public synchronized void stop() throws IOException {
		if (outputDataSource != null) {
			logger.debug("Stopping transcoding processor: " + transcodingProcessor.getClass().getName());
			transcodingProcessor.stop();
			logger.debug("Stopping data source: " + outputDataSource.getClass().getName());
