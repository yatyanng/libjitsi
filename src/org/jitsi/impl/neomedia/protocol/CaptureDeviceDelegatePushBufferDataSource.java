...
public class CaptureDeviceDelegatePushBufferDataSource extends PushBufferDataSource implements CaptureDevice {
	private static final Logger logger = Logger.getLogger(CaptureDeviceDelegatePushBufferDataSource.class);
	/**
	 * The constant which represents an empty array with <tt>PushBufferStream</tt>
	 * element type. Explicitly defined in order to reduce unnecessary allocations.
	 */
	protected static final PushBufferStream[] EMPTY_STREAMS = new PushBufferStream[0];

	/**
	 * The <tt>CaptureDevice</tt> this instance delegates to in order to implement
	 * its <tt>CaptureDevice</tt> functionality.
	 */
	protected final CaptureDevice captureDevice;

	/**
	 * Initializes a new <tt>CaptureDeviceDelegatePushBufferDataSource</tt> instance
	 * which delegates to a specific <tt>CaptureDevice</tt> in order to implement
	 * its <tt>CaptureDevice</tt> functionality.
	 *
	 * @param captureDevice the <tt>CaptureDevice</tt> the new instance is to
	 *                      delegate to in order to provide its
	 ...
	 * <tt>null</tt>.
	 *
	 * @return the <tt>CaptureDeviceInfo</tt> of the wrapped <tt>CaptureDevice</tt>
	 *         if available; otherwise, <tt>null</tt>
	 */
	@Override
	 ...
	 *
	 * @return the array of controls for the wrapped <tt>CaptureDevice</tt> if it
	 *         implements <tt>DataSource</tt>; otherwise, an empty array with
	 ...
	 *
	 * @return the duration of the wrapped <tt>CaptureDevice</tt> as returned by its
	 *         implementation of <tt>DataSource</tt> if any; otherwise, returns
	...
	public void start() throws IOException {
		if (captureDevice != null) {
			logger.debug("Starting Capture Device: " + captureDevice.getClass().getName());
			captureDevice.start();
		}
	}
	...
	public void stop() throws IOException {
		if (captureDevice != null) {
			logger.debug("Stopping Capture Device: " + captureDevice.getClass().getName());
			captureDevice.stop();
		}
	}
}
