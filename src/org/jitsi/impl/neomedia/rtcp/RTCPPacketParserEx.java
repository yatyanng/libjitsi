...
public class RTCPPacketParserEx extends net.sf.fmj.media.rtp.RTCPPacketParser {
	 ...
	 *                     serialization/writing of the binary representation of the
	 *                     specified {@code rtcp}
	 */
	public static RawPacket toRawPacket(RTCPPacket rtcp) throws IOException {
	 ...
	 * @param in        the binary representation from which the new instance is to
	 *                  be initialized, excluding the first 4 bytes.
	 * @return
	...
	private RTCPFBPacket parseRTCPFBPacket(RTCPCompoundPacket base, int firstbyte, int type, int length,
		...
		if (logger.isTraceEnabled()) {
			...
			if (detailStr == null)
				detailStr = "";
			logger.trace("SSRC of packet sender: 0x" + Long.toHexString(senderSSRC) + " (" + senderSSRC
					+ "), SSRC of media source: 0x" + Long.toHexString(sourceSSRC) + " (" + sourceSSRC
					+ "), Payload type (PT): " + ptStr + ", Feedback message type (FMT): " + fmtStr + detailStr);
		}
