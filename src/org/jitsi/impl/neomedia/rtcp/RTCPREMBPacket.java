...
public class RTCPREMBPacket extends RTCPFBPacket {
	...
	/**
	 * The exponential scaling of the mantissa for the maximum total media bit rate
	 * value, ignoring all packet overhead.
	 */
	public int exp;

	/**
	 * The mantissa of the maximum total media bit rate (ignoring all packet
	 * overhead) that the sender of the REMB estimates. The BR is the estimate of
	 * the traveled path for the SSRCs reported in this message.
	 */
	public int mantissa;
	...
	public void assemble(DataOutputStream dataoutputstream) throws IOException {
		...
		/*
		 * version (V): (2 bits): This field identifies the RTP version. The current
		 * version is 2. padding (P) (1 bit): If set, the padding bit indicates that the
		 * packet contains additional padding octets at the end that are not part of the
		 * control information but are included in the length field. Always 0. Feedback
		 * message type (FMT) (5 bits): This field identifies the type of the FB message
		 * and is interpreted relative to the type (transport layer, payload- specific,
		 * or application layer feedback). Always 15, application layer feedback
		 * message.
		 */
		buf[off++] = (byte) (0x8F);

		/*
		 * Payload type (PT) (8 bits): This is the RTCP packet type that identifies the
		 * packet as being an RTCP FB message. Always PSFB (206), Payload-specific FB
		 * message.
		 */
		buf[off++] = (byte) (0xCE);
	 ...
	 * {@link ByteArrayBuffer} that is passed in the first argument is an RTCP REMB
	 * packet.
	 *
	 * @param baf the {@link ByteArrayBuffer} that holds the RTCP packet.
	 * @return true if the packet specified in the {@link ByteArrayBuffer} that is
	 *         passed in the first argument is an RTCP REMB packet, otherwise false.
	 */
