...
public class RTCPTCCPacket extends RTCPFBPacket {
	/**
	 * The maximum number of packets (including missing packets) to include in an
	 * {@link RTCPTCCPacket} being constructed for a list of packets.
	 */
	 ...
	 *
	 *         Warning: the timestamps are represented in the 250µs format used by
	 *         the on-the-wire format, and don't represent local time. This is
	 *         different than the timestamps expected as input when constructing a
	 *         packet with
	 ...
	 *
	 *         The format is 32 bits with 250µs resolution. Note that the format in
	 *         the transport-wide cc draft is 24bit with 2^6ms resolution. The
	 *         change in the unit facilitates the arrival time computations, as the
	 *         deltas have 250µs resolution.
	 */
	...
	/**
	 * @return the packets represented in the FCI portion of an RTCP transport-cc
	 *         feedback packet.
	 *
	 *         Warning: the timestamps are represented in the 250µs format used by
	 *         the on-the-wire format, and don't represent local time. This is
	 ...
	 *         {@link RTCPTCCPacket#RTCPTCCPacket(long, long, PacketMap, byte, DiagnosticContext)}.
	 *
	 * @param fciBuffer          the buffer which contains the FCI portion of the
	 *                           RTCP feedback packet.
	 * @param includeNotReceived whether the returned map should include the packets
	 *                           described in the feedback packet as lost. Note that
	 *                           the RLE encoding allows ~2^16 packets to be
	 *                           described as lost in just a few bytes, so when
	 *                           parsing packets coming over the network it is wise
	 *                           to not blindly set this option to {@code true}.
	 */
	...
	/**
	 * @return the type of a Packet Status Chunk contained in {@code buf} at offset
	 *         {@code off}.
	 ...
	 * contained in {@code buf} at offset {@code off}. Returns -1 if the index is
	 * found to be invalid (although the validity check is not performed for RLE
	 * chunks).
	 *
	 * @param buf the buffer which contains the Packet Status Chunk.
	 * @param off the offset in {@code buf} at which the Packet Status Chunk begins.
	 ...
	 * in the buffer {@code buf} at offset {@code off}. Note that this may not
	 * necessarily match with the number of packets that we want to read from the
	 * chunk. E.g. if a feedback packet describes 3 packets (indicated by the value
	 * "3" in the "packet status count" field), and it contains a Vector Status
	 * Chunk which can describe 7 packets (long symbols), then we want to read only
	 * 3 packets (but this method will return 7).
	 *
	 * @param buf the buffer which contains the Packet Status Chunk
	...
	/**
	 * The {@link Logger} used by the {@link RTCPTCCPacket} class and its instances
	 * for logging output.
	 */
	...
	/**
	 * The value of the {@code T} bit of a Packet Status Chunk, which identifies it
	 * as a Vector chunk.
	 */
	...
	/**
	 * The value of the {@code T} bit of a Packet Status Chunk, which identifies it
	 * as a Run Length Encoding chunk.
	 */
	private static final int CHUNK_TYPE_RLE = 0;

	/**
	 * The value of the {@code S} bit og a Status Vector Chunk, which indicates
	 * 1-bit (short) symbols.
	 */
	private static final int SYMBOL_TYPE_SHORT = 0;

	/**
	 * The value of the {@code S} bit of a Status Vector Chunk, which indicates
	 * 2-bit (long) symbols.
	 */
	...
	/**
	 * The minimum length of the FCI field of a valid transport-cc RTCP feedback
	 * message. 8 bytes for the fixed fields + 2 bytes for one packet status chunk.
	 */
	...
	/**
	 * The offset of the first packet status chunk relative to the start of the FCI.
	 */
	 ...
	 * sender SSRC" and "media source SSRC" values, and which describes a specific
	 ...
	 *                          timestamps which this packet is to describe. Note
	 *                          that missing sequence numbers, as well as those
	 *                          mapped to a negative will be interpreted as missing
	 ...
	 *
	 *                          Warning: The timestamps for the packets are expected
	 *                          to be in millisecond increments, which is different
	 *                          than the output map produced after parsing a packet!
	 *
	 *                          Note: this implementation is not optimized and might
	 *                          not always use the minimal possible number of bytes
	 *                          to describe a given set of packets.
	 */
	public RTCPTCCPacket(long senderSSRC, long sourceSSRC, PacketMap packets, byte fbPacketCount,
		...
		byte[] buf = packetCount % 7 == 0 ? new byte[(packetCount / 7) * 2 + 8]
			...
			if (ts == null || ts < 0) {
				...
				if (tsDelta >= 0 && tsDelta <= 63) {
					...
					if (logger.isTraceEnabled()) {
						logger.trace(diagnosticContext.makeTimeSeriesPoint("small_delta").addField("seq", seq)
								.addField("arrival_time_ms", ts).addField("ref_time_ms", nextReferenceTime)
					...
					if (logger.isTraceEnabled()) {
						logger.trace(diagnosticContext.makeTimeSeriesPoint("large_delta").addField("seq", seq)
								.addField("arrival_time_ms", ts).addField("ref_time_ms", nextReferenceTime)
								.addField("delta", tsDelta));
					}
				} else {
	...
	/**
	 * Logs a message message about a packet in debug level (if debug logging is
	 * enabled).
	 */
	...
	/**
	 * An ordered collection which maps sequence numbers to timestamps, the order is
