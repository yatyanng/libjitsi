...
public class RTCPReceiverFeedbackTermination extends PeriodicRunnable implements TransformEngine {
	 ...
	 */
	private static final RTCPReportBlock[] MIN_RTCP_REPORT_BLOCKS_ARRAY = new RTCPReportBlock[MIN_RTCP_REPORT_BLOCKS];

	/**
	 * The {@link Logger} used by the {@link RTCPReceiverFeedbackTermination} class
	 ...
	 * <tt>MediaStream</tt> has and make <tt>RTCPReportBlock</tt>s for all of them.
	 *
	 * @return
	 */
		...
		for (ReceiveStream receiveStream : receiveStreams) {
			...
			if (!info.ours && info.sender) {
				...
				if (logger.isTraceEnabled()) {
					logger.trace(stream.getDiagnosticContext().makeTimeSeriesPoint("created_report_block")
							.addField("rtcp_termination", hashCode()).addField("ssrc", reportBlock.getSSRC())
	...
	private RTCPREMBPacket makeREMB(long senderSSRC) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Estimated bitrate (bps): " + bitrate + ", dest: " + Arrays.toString(dest) + ", time (ms): "
	...
	class RTCPTransformer extends SinglePacketTransformerAdapter {
		...
		private RawPacket doTransform(RawPacket pkt, boolean send) {
			...
			while (it.hasNext()) {
				...
				if (pt == RTCPPacket.RR || RTCPREMBPacket.isREMBPacket(baf) || RTCPTCCPacket.isTCCPacket(baf)) {
					it.remove();
					continue;
				}
				...
				if (!send && pt > -1) {
					...
					if ((pt == RTCPFeedbackMessageEvent.PT_PS && fmt == RTCPFeedbackMessageEvent.FMT_PLI)
							|| (pt == RTCPFeedbackMessageEvent.PT_PS && fmt == RTCPFeedbackMessageEvent.FMT_FIR)) {
						...
						((RTPTranslatorImpl) stream.getRTPTranslator()).getRtcpFeedbackMessageSender()
