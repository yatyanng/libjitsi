...
public class AudioLevelEffect extends ControlsAdapter implements Effect {
	/**
	 * The indicator which determines whether <tt>AudioLevelEffect</tt> instances
	 * are to perform the copying of the data from input <tt>Buffer</tt>s to output
	 * <tt>Buffer</tt>s themselves (e.g. using
	...
	public AudioLevelEffect() {
		supportedAudioFormats = new Format[] { new AudioFormat(AudioFormat.LINEAR, Format.NOT_SPECIFIED, 16, 1,
				AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, 16, Format.NOT_SPECIFIED, Format.byteArray) };
	}

	/**
	 * Sets (or unsets if <tt>listener</tt> is <tt>null</tt>), the listener that is
	 * going to be notified of audio level changes detected by this effect. Given
	 * the semantics of the {@link AudioLevelEventDispatcher} this effect would do
	 * no real work if no listener is set or if it is set to <tt>null</tt>.
	 *
	 * @param listener the <tt>SimpleAudioLevelListener</tt> that we'd like to
	 *                 receive level changes or <tt>null</tt> if we'd like level
	...
	public Format[] getSupportedOutputFormats(Format input) {
		return new Format[] { new AudioFormat(AudioFormat.LINEAR, ((AudioFormat) input).getSampleRate(), 16, 1,
				AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, 16, Format.NOT_SPECIFIED, Format.byteArray) };
	}
	 ...
	 *                     processed.
	 * @param outputBuffer The <tt>Buffer</tt> in which to store the processed media
	 *                     data.
	...
	public int process(Buffer inputBuffer, Buffer outputBuffer) {
		/*
		 * In accord with what an Effect is generally supposed to do, copy the data from
		 * the inputBuffer into outputBuffer.
		 */
		...
		/*
		 * At long last, do the job which this AudioLevelEffect exists for i.e. deliver
		 * the data to eventDispatcher so that its audio level gets calculated and
		 * delivered to audioEventListener.
		 */
	 ...
	 * Opens this effect.
	 *
	 * @throws ResourceUnavailableException If all of the required resources cannot
	 *                                      be acquired.
	 */
	@Override
