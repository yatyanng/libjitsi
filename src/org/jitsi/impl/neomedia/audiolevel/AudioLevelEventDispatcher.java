...
public class AudioLevelEventDispatcher {
	 ...
	 * AudioLevelEventDispatcher is to idly wait for data to be provided for audio
	 * level calculation before it exits.
	 */
	...
	/**
	 * Initializes a new <tt>AudioLevelEventDispatcher</tt> instance which is to use
	 * a specific name for its <tt>Thread</tt> which is to run the actual audio
	 * level calculations and to dispatch to its <tt>SimpleAudioLevelListener</tt>
	 *
	 * @param threadName
	 */
	...
	private void run() {
		...
		while (true) {
			...
			synchronized (this) {
				...
				ssrc = this.ssrc;
				/*
				 * If no one is interested in the audio level, do not even keep the Thread
				 * waiting.
				 */
			...
			/*
			 * In order to try to mitigate the issue with allocating data, try to return the
			 * one which we have just calculated the audio level of.
			 */
			synchronized (this) {
	 ...
	 *
	 * @param listener the listener that we will be notifying or <tt>null</tt> if we
	 *                 are to remove it.
	 */
	...
	/**
	 * Sets an <tt>AudioLevelMap</tt> that this dispatcher could use to cache levels
	 * it's measuring in addition to simply delivering them to a listener.
	 *
	 * @param cache the <tt>AudioLevelMap</tt> where this dispatcher should cache
	 *              measured results.
	...
	/**
	 * Starts the <tt>Thread</tt> which is to run the audio level calculations and
	 * to dispatch to {@link #listener} if necessary or notifies it about a change
	 * it the state on which it depends.
	 */
			...
			public void run() {
				try {
					...
					synchronized (AudioLevelEventDispatcher.this) {
						 ...
						 */
						if ((thread == null) && ((listener != null) || ((cache != null) && (ssrc != -1)))
