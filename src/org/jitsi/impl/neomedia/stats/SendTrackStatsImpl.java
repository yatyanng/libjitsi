...
public class SendTrackStatsImpl extends AbstractTrackStats implements SendTrackStats {
	...
	/**
	 * Notifies this instance that an RTP packet with a particular sequence number
	 ...
	 * {@inheritDoc}
	 *
	 * Returns an estimation of the loss rate based on the most recent RTCP Receiver
	 * Report that we received, and the rate of "non-sent" packets (i.e. in the case
	 * of jitsi-videobridge the loss rate from the sender to the bridge).
	 */
	@Override
	public double getLossRate() {
		...
		double fractionNotSent = (packetsSent + packetsNotSent > 0) ? (packetsNotSent / (packetsNotSent + packetsSent))
				: 0;
	...
	/**
	 * Notifies this instance that an RTCP packet with a given length in bytes was
	 * sent (or is about to be sent).
	 * 
	 * @param length
	 */
	...
	/**
	 * Notifies this instance that an RTCP Receiver Report with a given value for
	 * the "fraction lost" field was received.
	 * 
