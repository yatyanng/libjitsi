...
public abstract class AbstractTrackStats implements TrackStats {
	 ...
	 * subsection "delay since last SR (DLSR): 32 bits"). {@code -1} if the RTT has
	 * not been computed yet. Otherwise, the RTT in milliseconds.
	 */
	private long rtt = -1;
	...
	/**
	 * The total number of RTP packets. This excludes RTCP packets, because the
	 * value is used to calculate the number of lost RTP packets.
	 */
	...
	/**
	 * Number of bytes for packets which were requested and found in the cache, but
	...
	/**
	 * Number of packets which were requested and found in the cache, but were
	 * intentionally not retransmitted.
	 */
	protected final AtomicLong packetsNotRetransmitted = new AtomicLong();

	/**
	 * The number of packets for which retransmission was requested, but they were
	...
	/**
	 * The length of the interval over which the average bitrate, packet rate and
	 * packet loss rate are computed.
	 */
	 ...
	 * @param now    the time at which the packet was processed (passed in order to
	 *               avoid calling {@link System#currentTimeMillis()}).
	 ...
	 *
	 * @param jitter the new value to set on this instance as the last sent/received
	...
	/**
	 * Gets the number of bytes for packets which were requested and found in the
	 * cache, but were intentionally not retransmitted.
	 *
	 * @return the number of bytes for packets which were requested and found in the
	 *         cache, but were intentionally not retransmitted.
	 */
	@Override
	...
	/**
	 * Gets the number of packets which were requested and found in the cache, but
	 * were intentionally not retransmitted.
	 *
	 * @return the number of packets which were requested and found in the cache,
	 *         but were intentionally not retransmitted.
	 */
	@Override
	...
	/**
	 * Gets the number of packets for which retransmission was requested, but they
	 * were missing from the cache.
	 * 
	 * @return the number of packets for which retransmission was requested, but
	 ...
	 * retransmitted (that is, the remote endpoint requested it, and it was found in
	 * the local cache, but it was not retransmitted).
	 * 
	...
	/**
	 * Notifies this instance that the remote endpoint requested retransmission of a
	 * packet, and it was not found in the local cache.
	 */
