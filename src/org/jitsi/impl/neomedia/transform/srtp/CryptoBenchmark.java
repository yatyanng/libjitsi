...
//                    System.err.println(service);
		...
		for (Digest digest : digests) {
			...
			System.err.println(
					digest.getClass().getName() + ": digestSize " + digestSize + ", byteLength " + byteLength + ".");
		}
		for (MessageDigest messageDigest : messageDigests) {
			...
			System.err
					.println(messageDigest.getProvider().getClass().getName() + ": digestLength " + digestLength + ".");
		}
		...
		Mac[] macs = { new HMac(new SHA1Digest()), new OpenSSLHMAC(OpenSSLHMAC.SHA1) };
...
//                        blockCipher.reset();
			...
			for (Mac mac : macs) {
				...
				System.err.println(mac.getClass().getName() + ": ratio " + String.format("%.2f", time / (double) time0)
						+ ", time " + time + ", out "
						/* + byteEncoder.encodeToString(out) */ + ".");
			}
		}
