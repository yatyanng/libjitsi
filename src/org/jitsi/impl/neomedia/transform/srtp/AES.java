...
public class AES {
	...
	/**
	 * The simple name of the <tt>BlockCipherFactory</tt> class/interface which is
	 * used as a class name suffix by the well-known <tt>BlockCipherFactory</tt>
	 * implementations.
	 */
	...
	/**
	 * The <tt>BlockCipherFactory</tt> implementation which is (to be) used by the
	 * class <tt>AES</tt> to initialize <tt>BlockCipher</tt>s.
	 */
	 ...
	 * which specifies the name of the class to instantiate as a
	 ...
	 * implementations.
	 */
	private static final Class<?>[] FACTORY_CLASSES = { BouncyCastleBlockCipherFactory.class,
			SunJCEBlockCipherFactory.class, SunPKCS11BlockCipherFactory.class, };
	...
	/**
	 * The class to instantiate as a <tt>BlockCipherFactory</tt> implementation to
	 * be used to initialized <tt>BlockCipher</tt>s.
	 *
	...
	/**
	 * Benchmarks a specific array/list of <tt>BlockCipherFactory</tt> instances and
			...
			if (factory == null) {
				try {
					...
					if (AES.factory != factory) {
						...
						logger.info("Will employ AES implemented by " + getSimpleClassName(factory) + ".");
					}
				}
			}
		}
	...
	/**
	 * Initializes the <tt>BlockCipherFactory</tt> instances to be benchmarked by
	 * the class <tt>AES</tt> and among which the fastest-performing one is to be
	 * selected.
	 * 
	 * @return the <tt>BlockCipherFactory</tt> instances to be benchmarked by the
	 *         class <tt>AES</tt> and among which the fastest-performing one is to
	 *         be selected
	 */
				...
				if (add) {
					try {
						...
						if (t instanceof InterruptedException) {
							...
							logger.warn("Failed to employ class " + factoryClassName + " as an AES implementation: "
									+ t.getLocalizedMessage());
						}
					}
				}
	...
	/**
	 * Gets a <tt>BlockCipherFactory</tt> instance to be used by the <tt>AES</tt>
	 * class to initialize <tt>BlockCipher</tt>s.
	 *
	 * <p>
	 ...
	 * @param keySize AES key size (16, 24, 32 bytes)
	 *
	 * @return a <tt>BlockCipherFactory</tt> instance to be used by the <tt>AES</tt>
	 *         class to initialize <tt>BlockCipher</tt>s
	 */
	 ...
	 * <tt>BlockCipherFactory</tt> to be used for display purposes of brevity and
	 * readability.
	 *
	 * @param factory the <tt>BlockCipherFactory</tt> for which a simple class name
	 *                is to be returned
	...
	public static class SunPKCS11BlockCipherFactory extends SecurityProviderBlockCipherFactory {
		 ...
		 * <tt>true</tt>, an attempt will be made to initialize a
		 ...
		 * (optimized) AES implementation.
		 *
		 * @return the <tt>java.security.Provider</tt> instance (to be) employed for an
		 *         (optimized) AES implementation
		 */
