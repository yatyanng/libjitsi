...
public class SRTPCryptoContext extends BaseSRTPCryptoContext {
	 ...
	 * indicates whether protection against replay attacks is to be activated. The
	 * default value is <tt>true</tt>.
	 */
	public static final String CHECK_REPLAY_PNAME = SRTPCryptoContext.class.getName() + ".checkReplay";

	/**
	 * The indicator which determines whether protection against replay attacks is
	 * to be activated. The default value is <tt>true</tt>.
	 */
	 ...
	 * {@link #readConfigurationServicePropertiesOnce()} is to read the values of
	 * certain <tt>ConfigurationService</tt> properties of concern to
	 ...
	 * concern to <tt>SRTPCryptoContext</tt> once during the initialization of the
	 * first instance.
	 */
	 ...
	 * valid during the execution of {@link #reverseTransformPacket(RawPacket)}
	 * only.) RFC 3711 refers to it by the name <tt>v</tt>.
	 */
	...
	/**
	 * RFC 3711: for the receiver only, a 16-bit sequence number <tt>s_l</tt>, which
	...
	/**
	 * Constructs an empty SRTPCryptoContext using ssrc. The other parameters are
	 ...
	 *
	 * @param sender            <tt>true</tt> if the new instance is to be used by
	 *                          an SRTP sender; <tt>false</tt> if the new instance
	 *                          is to be used by an SRTP receiver
	 * @param ssrc              the RTP SSRC that this SRTP cryptographic context
	 *                          protects.
	 * @param roc               the initial Roll-Over-Counter according to RFC 3711.
	 *                          These are the upper 32 bit of the overall 48 bit
	 *                          SRTP packet index. Refer to chapter 3.2.1 of the
	 *                          RFC.
	 * @param keyDerivationRate the key derivation rate defines when to recompute
	 *                          the SRTP session keys. Refer to chapter 4.3.1 in the
	 *                          RFC.
	 * @param masterK           byte array holding the master key for this SRTP
	 *                          cryptographic context. Refer to chapter 3.2.1 of the
	 *                          RFC about the role of the master key.
	 * @param masterS           byte array holding the master salt for this SRTP
	 *                          cryptographic context. It is used to computer the
	 *                          initialization vector that in turn is input to
	 *                          compute the session key, session authentication key
	 *                          and the session salt.
	 * @param policy            SRTP policy for this SRTP cryptographic context,
	 *                          defined the encryption algorithm, the authentication
	 *                          algorithm, etc
	 */
	@SuppressWarnings("fallthrough")
	public SRTPCryptoContext(boolean sender, int ssrc, int roc, long keyDerivationRate, byte[] masterK, byte[] masterS,
			SRTPPolicy policy) {
		...
		if (delta > 0) {
			...
			if (sender) {
				logger.error("Discarding RTP packet with sequence number " + seqNo + ", SSRC "
						+ Long.toString(0xFFFFFFFFL & ssrc) + " because it is outside the replay window! (roc " + roc
						+ ", s_l " + s_l + ", guessedROC " + guessedROC);
			}
			return false; // Packet too old.
		} else if (((replayWindow >> (-delta)) & 0x1) != 0) {
			if (sender) {
				logger.error("Discarding RTP packet with sequence number " + seqNo + ", SSRC "
						+ Long.toString(0xFFFFFFFFL & ssrc) + " because it has been received already! (roc " + roc
						+ ", s_l " + s_l + ", guessedROC " + guessedROC);
			}
	 ...
	 * new SRTPCryptoContext initialized with the data of this SRTPCryptoContext.
	 * Replacing the SSRC, Roll-over-Counter, and the key derivation rate the
	 * application cab use this SRTPCryptoContext to encrypt/decrypt a new stream
	 * (Synchronization source) inside one RTP session. Before the application can
	 * use this SRTPCryptoContext it must call the deriveSrtpKeys method.
	 *
	...
	synchronized public void deriveSrtpKeys(long index) {
		...
		if (authKey != null) {
			...
			case SRTPPolicy.SKEIN_AUTHENTICATION:
				...
				mac.init(new ParametersForSkein(new KeyParameter(authKey), ParametersForSkein.Skein512,
						tagStore.length * 8));
				break;
			}
	 ...
	 * @return <tt>true</tt> if the packet can be accepted; <tt>false</tt> if the
	 *         packet failed authentication or failed replay check
	 */
	synchronized public boolean reverseTransformPacket(RawPacket pkt) {
		if (logger.isDebugEnabled()) {
			logger.debug("Reverse transform for SSRC " + this.ssrc + " SeqNo=" + pkt.getSequenceNumber() + " s_l=" + s_l
					+ " seqNumSet=" + seqNumSet + " guessedROC=" + guessedROC + " roc=" + roc);
		}
	 ...
	 * include: encryption, using either Counter Mode encryption, or F8 Mode
	 ...
	 * SRTPPolicy used in this SRTPCryptoContext is requires no encryption and no
