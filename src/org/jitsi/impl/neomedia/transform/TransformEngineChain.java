...
public class TransformEngineChain implements TransformEngine {
	/**
	 * The sequence of <tt>TransformEngine</tt>s whose <tt>PacketTransformer</tt>s
	 * this engine chain will be applying to RTP and RTCP packets. Implemented as
	 * copy-on-write storage for the purposes of performance.
	 */
	...
	/**
	 * The sequence of <tt>PacketTransformer</tt>s that this engine chain will be
	 * applying to RTCP packets.
	 */
	private PacketTransformerChain rtcpTransformChain;

	/**
	 * The sequence of <tt>PacketTransformer</tt>s that this engine chain will be
	 * applying to RTP packets.
	 */
	private PacketTransformerChain rtpTransformChain;

	/**
	 * Creates a new <tt>TransformEngineChain</tt> using the <tt>engineChain</tt>
	 * array. Engines will be applied in the order specified by the
	 * <tt>engineChain</tt> array for outgoing packets and in the reverse order for
	 * incoming packets.
	 *
	 * @param engineChain an array containing <tt>TransformEngine</tt>s in the order
	...
	/**
	 * Creates a new <tt>TransformEngineChain</tt> without initializing the array of
	 * transformers to be used. Allows extending classes to initialize the array on
	 * their own.
	 */
	 ...
	 * @return {@code true} if the engine was added, and {@code false} if the engine
	 *         was not added because it is already a member of the chain.
	 */
	 ...
	 *
	 * @return a <tt>PacketTransformerChain</tt> over all RTCP transformers in this
	 *         engine chain.
	 */
	@Override
	public PacketTransformer getRTCPTransformer() {
		 ...
		 * initialization of their PacketTransformer until it is requested for the first
		 * time AND may have to send packets at that very moment, not earlier (e.g.
		 * DTLS-SRTP may have to send Client Hello). Make sure that, when the
		 * PacketTransformer of this TransformEngine is requested for the first time,
		 * the same method will be invoked on each of the TransformEngines in
		 * engineChain.
		 */
	 ...
	 * <tt>TransformEngineChain</tt>.
	 *
	 * @return a <tt>PacketTransformerChain</tt> over all RTP transformers in this
	 *         engine chain.
	 */
	@Override
	 ...
	 * {@code PacketTransformer}s this engine chain will be applying to RTP and RTCP
	 * packets.
	 *
	 * @param engineChain the sequence of {@code TransformEngine}s whose
	 *                    {@code PacketTransformer}s this engine chain will be
	 *                    applying to RTP and RTCP packets
	 */
	...
	/**
	 * A <tt>PacketTransformerChain</tt> is a meta <tt>PacketTransformer</tt> that
	 * applies all transformers present in this engine chain. The class respects the
	 * order of the engine chain for outgoing packets and reverses it for incoming
	 * packets.
	 */
	public class PacketTransformerChain implements PacketTransformer {
		/**
		 * Indicates whether this transformer will be dealing with RTP or, in other
		 * words, whether it will transform packets via the RTP transformers in this
		 * chain rather than the RTCP ones.
		 */
		private final boolean rtp;

		/**
		 * Creates an instance of this packet transformer and prepares it to deal with
		 * RTP or RTCP according to the <tt>isRtp</tt> argument.
		 *
		 * @param rtp <tt>true</tt> if this transformer will be dealing with RTP (i.e.
		 *            will transform packets via the RTP transformers in this chain
		...
		public void close() {
			for (TransformEngine engine : engineChain) {
				PacketTransformer pTransformer = rtp ? engine.getRTPTransformer() : engine.getRTCPTransformer();
		...
		/**
		 * Transforms the specified {@code pkts} using the {@code TransformEngine}s in
		 * the chain in order starting after a specific {@code TransformEngine}.
		 *
		 * @param pkts  the array of {@code RawPacket}s to transform
		 * @param after the {@code TransformEngine} in the chain after which the
		 *              transformation is to begin. If {@code after} is not in the
		 *              chain, the transformation executes through the whole chain.
		...
		public RawPacket[] transform(RawPacket[] pkts, TransformEngine after) {
			...
			for (TransformEngine engine : engineChain) {
				...
				// Transform.
				PacketTransformer transformer = rtp ? engine.getRTPTransformer() : engine.getRTCPTransformer();
