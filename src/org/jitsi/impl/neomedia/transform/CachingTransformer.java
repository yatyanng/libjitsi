...
public class CachingTransformer extends SinglePacketTransformerAdapter implements TransformEngine, RecurringRunnable {
	/**
	 * The <tt>Logger</tt> used by the <tt>CachingTransformer</tt> class and its
	...
	/**
	 * Whether caching packets is enabled or disabled. Note that the default value
	 * is {@code false}.
	 */
	...
	public long getTimeUntilNextRun() {
		return (lastUpdateTime < 0L) ? 0L : lastUpdateTime + PROCESS_INTERVAL_MS - System.currentTimeMillis();
	}
	...
	public void setEnabled(boolean enabled) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug((enabled ? "Enabling" : "Disabling") + " CachingTransformer " + hashCode());
		}
	}
