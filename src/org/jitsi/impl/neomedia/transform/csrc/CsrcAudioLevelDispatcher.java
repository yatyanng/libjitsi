...
public class CsrcAudioLevelDispatcher implements Runnable {
	...
	/**
	 * The levels added to this instance (by the <tt>reverseTransform</tt> method of
	 * a <tt>PacketTransformer</tt> implementation) last.
	 */
	private long[] levels;

	/**
	 * The <tt>AudioMediaStreamImpl</tt> which listens to this event dispatcher. If
	 * <tt>null</tt>, this event dispatcher is stopped. If non-<tt>null</tt>, this
	 * event dispatcher is started.
	 */
	...
	/**
	 * Initializes a new <tt>CsrcAudioLevelDispatcher</tt> to dispatch events to a
	 * specific <tt>AudioMediaStreamImpl</tt>.
	 *
	...
	/**
	 * A level matrix that we should deliver to our media stream and its listeners
	...
	/**
	 * Waits for new levels to be reported via the <tt>addLevels()</tt> method and
	...
	public void setMediaStream(AudioMediaStreamImpl mediaStream) {
		synchronized (this) {
			if (this.mediaStream != mediaStream) {
				...
				/*
				 * If the mediaStream changes, it is unlikely that the (audio) levels are
				 * associated with it.
				 */
				this.levels = null;
