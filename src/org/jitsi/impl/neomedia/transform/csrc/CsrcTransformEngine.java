...
public class CsrcTransformEngine extends SinglePacketTransformer implements TransformEngine {
	...
	/**
	 * The number currently assigned to CSRC audio level extensions or <tt>-1</tt>
	 * if no such ID has been set and audio level extensions should not be
	 * transmitted.
	 */
	 ...
	 * discarded in all packets. The CSRC count will be 0 as well. The default value
	 * is <tt>false</tt>.
	 */
	 ...
	 * which indicates whether the list of CSRC identifiers are to be discarded from
	 * all packets. The default value is <tt>false</tt>.
	 */
	private static final String DISCARD_CONTRIBUTING_SRCS_PNAME = CsrcTransformEngine.class.getName()
	...
	/**
	 * Creates an engine instance that will be adding CSRC lists to the specified
	 * <tt>stream</tt>.
	 *
	...
	public CsrcTransformEngine(MediaStreamImpl mediaStream) {
		...
		/*
		 * Take into account that RTPExtension.CSRC_AUDIO_LEVEL_URN may have already
		 * been activated.
		 */
	...
	/**
	 * Creates a audio level extension buffer containing the level extension header
	 * and the audio levels corresponding to (and in the same order as) the
	 * <tt>CSRC</tt> IDs in the <tt>csrcList</tt>
	 *
	 * @param csrcList the list of CSRC IDs whose level we'd like the extension to
	 *                 contain.
	 * @return the extension buffer in the form that it should be added to the RTP
	 *         packet.
	 */
	 ...
	 *
	 * @param ensureCapacity the minimum length that we need the returned buffer to
	 *                       have.
	 * @return a reusable <tt>byte[]</tt> array guaranteed to have a length equal to
	 *         or greater than <tt>ensureCapacity</tt>.
	 */
	 ...
	 * <tt>MediaStream</tt> associated with this engine. Other than that the method
	 * does not do any transformations since CSRC lists are part of RFC 3550 and
	 * they shouldn't be disrupting the rest of the application.
	 *
	 * @param pkt the RTP <tt>RawPacket</tt> that we are to extract a CSRC list
	 *            from.
	 *
	 * @return the same <tt>RawPacket</tt> that was received as a parameter since we
	 *         don't need to worry about hiding the CSRC list from the rest of the
	 *         RTP stack.
	 */
	@Override
	...
	/**
	 * Sets the ID that this transformer should be using for audio level extensions
	 * or disables audio level extensions if <tt>extID</tt> is <tt>-1</tt>.
	 *
	 ...
	 * contributing to the media being sent by the <tt>MediaStream</tt> associated
	 * with this engine and (unless the list is empty) encodes them into the
	 * <tt>RawPacket</tt>.
	 *
	 * @param pkt the RTP <tt>RawPacket</tt> that we need to add a CSRC list to.
	 *
	 * @return the updated <tt>RawPacket</tt> instance containing the list of CSRC
	 *         identifiers.
	 */
	@Override
