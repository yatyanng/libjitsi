...
public class REDFilterTransformEngine extends SinglePacketTransformerAdapter implements TransformEngine {
	...
	/**
	 * A boolean flag determining whether or not this transformer should strip RED
	 * from outgoing packets.
	 */
	private boolean enabled = false;

	/**
	 * The RED payload type. This, of course, should be dynamic but in the context
	 * of this transformer this doesn't matter.
	 */
	private final byte redPayloadType;

	/**
	 * Initializes a new <tt>REDFilterTransformEngine</tt> with the given payload
	 * type number for RED.
	 * 
	...
	public RawPacket transform(RawPacket pkt) {
		...
		if (!enabled || redPayloadType == -1 || pkt == null || pkt.getPayloadType() != redPayloadType) {
			return pkt;
		}
