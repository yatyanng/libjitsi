...
public class DebugTransformEngine implements TransformEngine {
	/**
	 * The <tt>Logger</tt> used by the <tt>DebugTransformEngine</tt> class and its
	...
	/**
	 * The {@code PacketLoggingService} (to be) used by this instance. Cached for
	 * the sake of performance because fetching OSGi services is not inexpensive.
	 */
	...
	/**
	 * Creates and returns a new <tt>DebugTransformEngine</tt> if logging is enabled
	 * for <tt>PacketLoggingService.ProtocolName.ARBITRARY</tt>.
	 *
	 * @param mediaStream the <tt>MediaStream</tt> that will own the newly created
	 *                    <tt>DebugTransformEngine</tt>.
	 ...
	 * @param sender {@code true} if {@code pkt} has originated from the local peer
	 *               and is to be sent to the remote peer or {@code false} if
	 *               {@code pkt} has originated from the remote peer and has been
	...
	private RawPacket transform(RawPacket pkt, boolean data, boolean sender) {
		...
		pktLogging.logPacket(PacketLoggingService.ProtocolName.ARBITRARY, src.getAddress().getAddress(), src.getPort(),
				dst.getAddress().getAddress(), dst.getPort(), PacketLoggingService.TransportName.UDP, sender,
				pkt.getBuffer().clone(), pkt.getOffset(), len);
	...
	class MyRTPPacketTransformer extends SinglePacketTransformer {
		...
		public RawPacket reverseTransform(RawPacket pkt) {
			return DebugTransformEngine.this.transform(pkt, /* data */ true, /* sender */ false);
		}

		@Override
		public RawPacket transform(RawPacket pkt) {
			return DebugTransformEngine.this.transform(pkt, /* data */ true, /* sender */ true);
		}
	}

	/**
	 * Logs RTCP packets that go in and out of the <tt>MediaStream</tt> that owns
	 * this instance.
	 */
	class MyRTCPPacketTransformer extends SinglePacketTransformer {
		@Override
		public RawPacket reverseTransform(RawPacket pkt) {
			return DebugTransformEngine.this.transform(pkt, /* data */ false, /* sender */ false);
		}

		@Override
		public RawPacket transform(RawPacket pkt) {
			return DebugTransformEngine.this.transform(pkt, /* data */ false, /* sender */ true);
		}
	}
}
