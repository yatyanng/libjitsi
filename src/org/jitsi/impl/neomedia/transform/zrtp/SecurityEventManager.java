...
public class SecurityEventManager extends ZrtpUserCallback {
	...
	public void showMessage(ZrtpCodes.MessageSeverity sev, EnumSet<?> subCode) {
		...
		if (msgCode instanceof ZrtpCodes.InfoCodes) {
			...
			message = "Indicates compatibility problems like for example:"
					+ "unsupported protocol version, unsupported hash type," + "cypher type, SAS scheme, etc.";
			i18nMessage = getI18NString("impl.media.security.ZRTP_GENERIC_MSG", msgCode.toString());
		} else {
			// Other warnings are internal only, no user action required
			sendEvent = false;
		...
		if (logger.isDebugEnabled()) {
			logger.debug(sessionTypeToString(sessionType) + ": ZRTP message: severity: " + sev + ", sub code: "
	...
	public void zrtpNotSuppOther() {
		if (logger.isDebugEnabled()) {
			logger.debug(sessionTypeToString(sessionType) + ": Other party does not support ZRTP key negotiation"
	 ...
	 *                    <tt>String</tt> for the purposes of this
