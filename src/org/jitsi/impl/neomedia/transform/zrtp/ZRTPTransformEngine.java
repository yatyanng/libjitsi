...
public class ZRTPTransformEngine extends SinglePacketTransformer implements SrtpControl.TransformEngine, ZrtpCallback {
	 ...
	 *
	 * This very simple timeout provider can handle one timeout request at one time
	 * only. A second request would overwrite the first one and would lead to
	 * unexpected results.
	 *
	 * @author Werner Dittmann <Werner.Dittmann@t-online.de>
	 */
	...
	/**
	 * Sometimes we need to start muted so we will discard any packets during some
	 * time after the start of the transformer. This is needed when for this time we
	 * can receive encrypted packets but we hadn't established a secure
	 * communication. This happens when a secure stream is recreated.
	 */
	 ...
	 *
	 * The Paranoid mode controls the behaviour and handling of the SAS verify flag.
	 * If Panaoid mode is set to flase then ZRtp applies the normal handling. If
	 * Paranoid mode is set to true then the handling is:
	 *
	 * <ul>
	 * <li>Force the SAS verify flag to be false at srtpSecretsOn() callback. This
	 * gives the user interface (UI) the indication to handle the SAS as <b>not
	 * verified</b>. See implementation note below.</li>
	 * <li>Don't set the SAS verify flag in the <code>Confirm</code> packets, thus
	 * the other also must report the SAS as <b>not verified</b>.</li>
	 * <li>ignore the <code>SASVerified()</code> function, thus do not set the SAS
	 * to verified in the ZRTP cache.</li>
	 ...
	 * particular it computes, compares, uses, and stores the retained secrets. This
	 * avoids unnecessary warning messages. The user may enable or disable the
	 * Paranoid mode on a call-by-call basis without breaking the key continuity
	 * data.
	 *
	 * <b>Implementation note:</b></br>
	 * An application shall always display the SAS code if the SAS verify flag is
	 * <code>false</code>. The application shall also use mechanisms to remind the
	 * user to compare the SAS code, for example useing larger fonts, different
	 * colours and other display features.
	 */
	...
	/**
	 * Count successfully decrypted SRTP packets. Need to decide if RS2 will become
	 * valid.
	 *
	 * @see stopZrtp
	 */
	 ...
	 * {@link #getRTCPTransformer()} to initialize a new <tt>ZRTCPTransformer</tt>
	 * instance which cannot possibly be correctly used after the disposal of this
	 * instance anyway.
	 */
	...
	/**
	 * Custom engine initialization method. This allows to explicit specify if the
	 * engine starts with auto-sensing or not.
	 *
	 * @param zidFilename The ZID file name
	...
	/**
	 *
	 * @param startMuted whether to be started as muted if no secure communication
	 *                   is established
	 */
	 ...
	 * Stop ZRTP engine.
	 *
	 * The ZRTP stack stores RS2 without the valid flag. If we close the ZRTP stream
	 * then check if we need to set RS2 to vaild. This is the case if we received
	 * less than 10 good SRTP packets. In this case we enable RS2 to make sure the
	 * ZRTP self-synchronization is active.
	 *
	...
	public RawPacket reverseTransform(RawPacket pkt) {
		...
		/*
		 * If the incoming packet is not a ZRTP packet, treat it as a normal RTP packet
		...
		/*
		 * If ZRTP is enabled, process it. In any case return null because ZRTP packets
		...
		if (enableZrtp && started) {
			...
			// Check if it is really a ZRTP packet, if not don't process it
				...
				byte[] extHeader = zPkt.readRegion(extHeaderOffset,
						RawPacket.EXT_HEADER_SIZE + zPkt.getExtensionLength() + zPkt.getPayloadLength());
	...
	/**
	 * The callback method required by the ZRTP implementation. First allocate space
	 * to hold the complete ZRTP packet, copy the message part in its place, the
	 * initialize the header, counter, SSRC and crc.
	 *
	...
	public boolean srtpSecretsReady(ZrtpSrtpSecrets secrets, EnableSecurity part) {
		...
		if (part == EnableSecurity.ForSender) {
			...
			if (secrets.getRole() == Role.Initiator) {
				srtpPolicy = new SRTPPolicy(cipher, secrets.getInitKeyLen() / 8, // key length
						authn, authKeyLen, // auth key length
						secrets.getSrtpAuthTagLen() / 8, // auth tag length
						secrets.getInitSaltLen() / 8 /* salt length */);

				SRTPContextFactory engine = new SRTPContextFactory(true /* sender */, secrets.getKeyInitiator(),
						secrets.getSaltInitiator(), srtpPolicy, srtpPolicy);

				srtpOutTransformer = new SRTPTransformer(engine);
				getRTCPTransformer().setSrtcpOut(new SRTCPTransformer(engine));
			} else {
				srtpPolicy = new SRTPPolicy(cipher, secrets.getRespKeyLen() / 8, // key length
						authn, authKeyLen, // auth key length
						secrets.getSrtpAuthTagLen() / 8, // auth taglength
						secrets.getRespSaltLen() / 8 /* salt length */);

				SRTPContextFactory engine = new SRTPContextFactory(true /* sender */, secrets.getKeyResponder(),
						secrets.getSaltResponder(), srtpPolicy, srtpPolicy);
			...
			if (secrets.getRole() == Role.Initiator) {
				...
				SRTPContextFactory engine = new SRTPContextFactory(false /* receiver */, secrets.getKeyResponder(),
						secrets.getSaltResponder(), srtpPolicy, srtpPolicy);
				...
				SRTPContextFactory engine = new SRTPContextFactory(false /* receiver */, secrets.getKeyInitiator(),
						secrets.getSaltInitiator(), srtpPolicy, srtpPolicy);
	 ...
	 * @param verified if <code>verified</code> is true then SAS was verified by
	 *                 both parties during a previous call.
	 *
	...
	/**
	 * Method called when the user requests through GUI to switch a secured call to
	 * unsecure mode. Just forwards the request to the Zrtp class.
	 */
...
//            zrtpEngine.requestGoSecure();
	 ...
	 *
	 *
	 * @param index Hello hash of the Hello packet identified by index. Index must
	 ...
	 *
	 * @param index Hello hash of the Hello packet identfied by index. Index must be
	 ...
	 *         immediately after class instantiation. Returns <code>null</code> if
	 ...
	 * data as a string.
	 *
	 * @return a String containing the Hello hash value as hex-digits. Peer Hello
	 *         hash is available after we received a Hello packet from our peer. If
	 *         peer's hello hash is not available return null.
	 */
	 ...
	 *
	 * If true then this ZRTP session acts as MitM, usually enabled by a PBX based
	 * client (user agent)
	 *
	 ...
	 *
	 * If MitM mode is set to true this ZRTP session acts as MitM, usually enabled
	 * by a PBX based client (user agent).
	 *
	 ...
	 *
	 * For further explanation of paranoid mode refer to the documentation of ZRtp
	 * class.
	 *
	 ...
	 *
	 * If true then we will set the enrollment flag (E) in the confirm packets and
	 * performs the enrollment actions. A MitM (PBX) enrollment service sets this
	 * flag.
	 *
	 ...
	 *
	 * If true then we will set the enrollment flag (E) in the confirm packets and
	 * perform the enrollment actions. A MitM (PBX) enrollment service must set this
	 * mode to true.
	 *
	...
	/**
	 * Method called by the Zrtp class as result of a GoClear request from the other
	 * peer. An explicit user confirmation is needed before switching to unsecured
	 * mode. This is asked through the user callback.
	 */
	@Override
	 ...
	 *
	 * This functions returns the other party's ZID that was receivied during ZRTP
	 * processing.
	 *
	 * The ZID data can be retrieved after ZRTP receive the first Hello packet from
	 * the other party. The application may call this method for example during SAS
	 * processing in showSAS(...) user callback method.
	 *
