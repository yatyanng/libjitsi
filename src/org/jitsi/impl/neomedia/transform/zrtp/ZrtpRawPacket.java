...
public class ZrtpRawPacket extends RawPacket {
	 ...
	 *
	 * The method checks if the first byte of the received data matches the defined
	 * ZRTP pattern 0x10
	 *
	...
	/**
	 * Checks whether extension bit is set and if so is the extension header an zrtp
	 * one.
	 * 
	...
	protected boolean hasMagic() {
		return (readByte(4) == ZRTP_MAGIC[0]) && (readByte(5) == ZRTP_MAGIC[1]) && (readByte(6) == ZRTP_MAGIC[2])
