...
public class ULPFECReceiver extends AbstractFECReceiver {
	/**
	 * The <tt>Logger</tt> used by the <tt>ULPFECReceiver</tt> class and its
	...
	/**
	 * A class that allows the recovery of a <tt>RawPacket</tt> given a set of media
	 * packets and an ulpfec packet.
	 *
	...
	private static class Reconstructor {
		/**
		 * Subset of the media packets which is needed for recovery, given a specific
		 * value of <tt>fecPacket</tt>.
		 */
		...
		/**
		 * Number of media packet which are needed for recovery (given a specific value
		 * of <tt>fecPacket</tt>) which are not available. If the value is <tt>0</tt>,
		 * this indicates that all media packets referenced in <tt>fecPacket</tt> *are*
		 * available, and so no recovery is needed.
		 */
		...
		/**
		 * Sets the ulpfec packet to be used for recovery and also updates the values of
		 * <tt>numMissing</tt>, <tt>sequenceNumber</tt> and <tt>neededPackets</tt>.
		 * 
		...
		/**
		 * Recovers a media packet using the ulpfec packet <tt>fecPacket</tt> and the
		 * packets in <tt>neededPackets</tt>.
		 * 
