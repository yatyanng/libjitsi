...
public class FlexFec03Mask {
	...
	/**
	 * The size of the packet mask in a flexfec packet is dynamic. This method
	 * determines the size (in bytes) of the mask in the given packet buffer and
	 * returns it
	 * 
	 ...
	 * @param sortedProtectedSeqNums the sequence numbers this mask should mark as
	 *                               protected. NOTE: this list MUST be in sorted
	 *                               order
	 * @return the size, in bytes, of the mask that is needed to convey the given
	...
	/**
	 * Extract the mask containing just the protected sequence number bits (not the
	 * k bits) from the given buffer
	 * 
	 * @param maskWithKBits the mask with the k bits included
	 * @return a {@link FlexFec03BitSet} which contains the bits of the packet mask
	 *         WITHOUT the k bits (the packet location bits are 'collapsed' so that
	 *         their bit index correctly represents the delta from baseSeqNum)
	 */
	...
	/**
	 * Initialize this maskWithoutKBits with a received buffer. buffer + maskOffset
	 * should point to the location of the start of the mask
	 * 
	...
	/**
	 * Create a mask from a base sequence number and a list of protected sequence
	 * numbers
	 * 
	 * @param baseSeqNum       the base sequence number to use for the mask
	 * @param protectedSeqNums the sequence numbers this mask should mark as
	 *                         protected
	 */
	...
	/**
	 * Get the list of media packet sequence numbers which are marked as protected
	 * in this mask
	 * 
	 * @return a list of sequence numbers of the media packets marked as protected
	 *         by this mask
	 */
