...
/**
 * A bit-set class which is similar to a standard bitset, but with 2
 * differences: 1) The size of this set is preserved. Unlike the standard
 * bitset, which will not include leading 0's in its size (which doesn't work
 * well for a bitmask), this one will always report the size it was allocated
 * for 2) When reading (valueOf) and writing (toByteArray) it inverts the order
...
public class FlexFec03BitSet {
	...
	/**
	 * Add a bit with the given value in the given position. Existing bits will be
	 * moved to the right. No loss of bits will occur.
	 * 
	...
	/**
	 * Remove a bit from the given position. Existing bits will be moved to the
	 * left.
	 * 
	 * @param bitIndex
	 */
	 ...
	 * @return a FlexFec03BitSet representing the chunk of the given buffer based on
	...
	/**
	 * Writes this bitset to a byte array, where the rightmost bit is treated as the
