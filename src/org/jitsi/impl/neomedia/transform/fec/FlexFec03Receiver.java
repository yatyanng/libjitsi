...
public class FlexFec03Receiver extends AbstractFECReceiver {
	/**
	 * The <tt>Logger</tt> used by the <tt>FlexFec03Receiver</tt> class and its
		...
		for (Map.Entry<Integer, RawPacket> entry : fecPackets.entrySet()) {
			...
			if (reconstructor.canRecover()) {
				...
				if (recovered != null) {
					...
					logger.error("Recovery of packet " + reconstructor.missingSequenceNumber + " failed even"
	...
	private static class Reconstructor {
		...
		/**
		 * We can only recover a single missing packet, so when we check for how many
		 * are missing, we'll keep track of the first one we find
		 */
		int missingSequenceNumber = -1;

		/**
		 * The number of missing media packets we've detected for the set FlexFEC packet
		 */
		int numMissing = -1;
		 ...
		 * 
		 * @param mediaPackets the currently available media packets. Note that this is
		 *                     a reference so it will remain up to date as the map is
		...
		/**
		 * Set the {@link FlexFec03Packet} to be used for this reconstruction, and check
		 * if any media packets protected by this fec packet are missing and can be
		 * recovered
		 * 
		...
		public void setFecPacket(FlexFec03Packet p) {
			...
			numMissing = 0;
			if (logger.isDebugEnabled()) {
				logger.debug("Reconstructor checking if recovery is " + "possible: fec packet " + p.getSequenceNumber()
						+ " protects packets:\n" + p.getProtectedSequenceNumbers());
			}

			for (Integer protectedSeqNum : fecPacket.getProtectedSequenceNumbers()) {
				if (logger.isDebugEnabled()) {
					logger.debug("Checking if we've received media " + "packet " + protectedSeqNum);
				}
			...
			if (logger.isDebugEnabled()) {
				logger.debug("There were " + numMissing + " missing media " + "packets for flexfec "
						+ p.getSequenceNumber());
			}
			if (numMissing > 1) {
		...
		/**
		 * Initialize the given RawPacket with the RTP header information and payload
		 * from fecPacket
		 * 
		 * @param fecPacket       the FlexFEC packet being used for recovery
		 * @param recoveredPacket the blank RawPacket we're recreating the recovered
		 *                        packet in
		...
		private boolean startPacketRecovery(FlexFec03Packet fecPacket, RawPacket recoveredPacket) {
			...
			// starting after that)
			System.arraycopy(fecPacket.getBuffer(), fecPacket.getFlexFecHeaderOffset(), recoveredPacket.getBuffer(), 0,
					RawPacket.FIXED_HEADER_SIZE);

			// Copy over the recovery rtp payload data from the fec packet
			System.arraycopy(fecPacket.getBuffer(),
					fecPacket.getFlexFecHeaderOffset() + fecPacket.getFlexFecHeaderSize(), recoveredPacket.getBuffer(),
					RawPacket.FIXED_HEADER_SIZE, fecPacket.getFlexFecPayloadLength());
		 ...
		 * @param sourceOffset  the offset in the source packet at which the payload
		 *                      begins. Note that this is not necessarily the location
		 *                      at which the RTP payload starts...for the purpose of
		 *                      FlexFEC, everything after the fixed RTP header is
		 *                      considered the 'payload'
		...
		/**
		 * Do the final work when recovering an RTP packet (set the RTP version, the
		 * length, the sequence number, and the ssrc)
		 * 
		...
		/**
		 * Recover a media packet based on the given FlexFEC packet and received media
		 * packets
		 * 
		...
		private RawPacket recover() {
			...
			for (Integer protectedSeqNum : fecPacket.getProtectedSequenceNumbers()) {
				if (protectedSeqNum != missingSequenceNumber) {
					...
					xorPayloads(mediaPacket.getBuffer(), mediaPacket.getOffset() + RawPacket.FIXED_HEADER_SIZE,
							recoveredPacket.getBuffer(), RawPacket.FIXED_HEADER_SIZE,
