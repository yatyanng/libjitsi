...
public abstract class AbstractFECReceiver implements PacketTransformer {
	/**
	 * The <tt>Logger</tt> used by the <tt>AbstractFECReceiver</tt> class and its
	...
	/**
	 * The SSRC of the fec stream NOTE that for ulpfec this might be the same as the
	 * associated media stream, whereas for flexfec it will be different
	 */
	protected long ssrc;
	 ...
	 * select the oldest one to discard when the buffer is full (when the map has
	 * more than <tt>MEDIA_BUFF_SIZE</tt> entries).
	 *
	 * We keep them in a <tt>Map</tt> so that we can easily search for a packet with
	 * a specific sequence number.
	 *
	 * Note: This might turn out to be inefficient, especially with increased buffer
	 * sizes. In the vast majority of cases (e.g. on every received packet) we do an
	 * insert at one end and a delete from the other -- this can be optimized. We
	 * very rarely (when we receive a packet out of order) need to insert at an
	 * arbitrary location. FIXME: Look at using the existing packet cache instead of
	 * our own here
	 */
	protected final SortedMap<Integer, RawPacket> mediaPackets = new TreeMap<Integer, RawPacket>(
			RTPUtils.sequenceNumberComparator);
	 ...
	 * We keep them ordered by their RTP sequence numbers, so that we can easily
	 * select the oldest one to discard when the buffer is full (when the map has
	 * more than <tt>FEC_BUFF_SIZE</tt> entries.
	 *
	 * We keep them in a <tt>Map</tt> so that we can easily search for a packet with
	 * a specific sequence number.
	 *
	 ...
	 * our own here
	 */
	protected final SortedMap<Integer, RawPacket> fecPackets = new TreeMap<Integer, RawPacket>(
			RTPUtils.sequenceNumberComparator);

	/**
	 * Initialize the FEC receiver
	 * 
	 * @param ssrc        the ssrc of the stream on which fec packets will be
	 *                    received
	...
	/**
	 * Saves <tt>p</tt> into <tt>fecPackets</tt>. If the size of <tt>fecPackets</tt>
	 * has reached <tt>FEC_BUFF_SIZE</tt> discards the oldest packet from it.
	 * 
	 ...
	 * 
	 * @param payloadType the payload type. FIXME(brian): do we need both this and
	 *                    the ability to pass the payload type in the ctor? Can we
	 *                    get rid of this or get rid of the arg in the ctor?
	 */
	...
	public void close() {
		if (logger.isInfoEnabled()) {
			logger.info("Closing AbstractFECReceiver for SSRC=" + ssrc + ". Received " + statistics.numRxFecPackets
					+ " fec packets, recovered " + statistics.numRecoveredPackets + " media packets. Recovery "
	 ...
	 * @return a RawPacket[] containing the given media packets as well as any media
	 *         packets that were recovered
	 */
