...
/**
 * Appends an Original Header Block packet extension to incoming packets. Note
 * that we currently do NOT follow the PERC format, but rather an extended
...
public class OriginalHeaderBlockTransformEngine extends SinglePacketTransformerAdapter implements TransformEngine {
	/**
	 * The <tt>Logger</tt> used by the {@link OriginalHeaderBlockTransformEngine}
	...
	public RawPacket transform(RawPacket pkt) {
		...
		/*
		 * if (extensionID != -1) { RawPacket.HeaderExtension ohb =
		 * pkt.getHeaderExtension(extensionID); if (ohb != null) { rebuildOhb(pkt, ohb);
		 * } } return pkt;
		 */
	}
	...
	/**
	 * @return the length of the OHB extension, given the fields which differ from
	 *         the original packet.
	...
	/**
	 * Adds an abs-send-time RTP header extension with an ID of {@link #extensionID}
	 * and value derived from the current system time to the packet {@code pkt}.
	 * 
