...
public abstract class SinglePacketTransformer implements PacketTransformer {
	 ...
	 * {@link #reverseTransform(RawPacket)} and {@link #transform(RawPacket)}, the
	 * logging may be overwhelming.
	 */
	private static final int EXCEPTIONS_TO_LOG = 1000;

	/**
	 * The <tt>Logger</tt> used by the <tt>SinglePacketTransformer</tt> class and
	 ...
	 * associated with a <tt>Predicate</tt> so that they only process packets that
	 * they're supposed to process. For example, transformers that transform RTP
	 * packets should not transform RTCP packets, if, by mistake, they happen to be
	 * passed RTCP packets.
	 */
	 ...
	 * inheritors will use the parametrized ctor. Also, we might want to move this
	 * check inside the <tt>TransformEngineChain</tt> so that we only make the check
	 * once per packet: The RTCP transformer is only supposed to (reverse) transform
	 * RTCP packets and the RTP transformer is only supposed to modify RTP packets.
	 */
	...
	public RawPacket[] reverseTransform(RawPacket[] pkts) {
		return transformArray(pkts, this::reverseTransform, exceptionsInReverseTransform, "reverseTransform");
	}
	 ...
	 * {@inheritDoc}
	 *
	 * Transforms an array of packets by calling {@link #transform(RawPacket)} on
	 * each one.
	 */
	@Override
	public RawPacket[] transform(RawPacket[] pkts) {
		return transformArray(pkts, this::transform, exceptionsInTransform, "transform");
	}
	 ...
	 * @param transformFunction the function to apply to each (non-null) element of
	 *                          the array.
			...
			for (int i = 0; i < pkts.length; i++) {
				...
				if (pkt != null && (packetPredicate == null || packetPredicate.test(pkt))) {
					try {
						...
						if ((exceptionCounter.get() % EXCEPTIONS_TO_LOG) == 0 || exceptionCounter.get() == 1) {
