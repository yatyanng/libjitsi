...
public class DtmfTransformEngine extends SinglePacketTransformer implements TransformEngine {
	/**
	 * The <tt>AudioMediaStreamImpl</tt> that this transform engine was created by
	 * and that it's going to deliver DTMF packets for.
	 */
	...
	private enum ToneTransmissionState {
		...
		/**
		 * Indicates that we are currently in the process of sending a DTMF tone, and we
		 * have already sent at least one packet.
		 */
		SENDING,

		/**
		 * Indicates that the user has requested that DTMF transmission be stopped but
		 * we haven't acted upon that request yet (i.e. we have yet to send a single
		 * retransmission)
		 */
		END_REQUESTED,

		/**
		 * Indicates that the user has requested that DTMF transmission be stopped we
		 * have already sent a retransmission of the final packet.
		 */
	 ...
	 */
	private static final DTMFRtpTone[] supportedTones = new DTMFRtpTone[] { DTMFRtpTone.DTMF_0, DTMFRtpTone.DTMF_1,
			DTMFRtpTone.DTMF_2, DTMFRtpTone.DTMF_3, DTMFRtpTone.DTMF_4, DTMFRtpTone.DTMF_5, DTMFRtpTone.DTMF_6,
			DTMFRtpTone.DTMF_7, DTMFRtpTone.DTMF_8, DTMFRtpTone.DTMF_9, DTMFRtpTone.DTMF_A, DTMFRtpTone.DTMF_B,
			DTMFRtpTone.DTMF_C, DTMFRtpTone.DTMF_D, DTMFRtpTone.DTMF_SHARP, DTMFRtpTone.DTMF_STAR };
	...
	/**
	 * The number of tone for which we have received a stop request. This is used to
	 * signal that stop is already received for "currentTone" not yet sent.
	 */
	private int nbToneToStop = 0;

	/**
	 * A mutex used to control the start and the stop of a tone and thereby to
	 * control concurrent modification access to "currentTone", "nbToneToStop" and
	...
	/**
	 * The maximal DTMF tone dration. The default value is -1 telling to stop only
	 * when the user asks to. This can be changed by using the
	 ...
	 * ones upon request.
	 *
	 * @param stream the <tt>AudioMediaStream</tt> whose RTP packets we are going to
	 *               be replacing with DTMF.
	 */
	 ...
	 *
	 * @param pkt an incoming packet that we need to parse and handle in case we
	 *            determine it to be DTMF.
	 *
	 * @return the <tt>pkt</tt> if it is not a DTMF tone and <tt>null</tt> otherwise
	 *         since we will be handling the packet ourselves and their's no point
	 *         in feeding it to the application.
	 */
	@Override
	...
	public RawPacket transform(RawPacket pkt) {
		...
		if (currentDtmfPayload == -1)
			throw new IllegalStateException(
					"Can't send DTMF when no payload " + "type has been negotiated for DTMF events.");

		DtmfRawPacket dtmfPkt = new DtmfRawPacket(pkt.getBuffer(), pkt.getOffset(), pkt.getLength(),
				currentDtmfPayload);
	...
	/**
	 * DTMF sending stub: this is where we should set the transformer in the proper
	 * state so that it would start replacing packets with dtmf codes.
	 *
	...
	public void stopSendingDTMF() {
		synchronized (startStopToneMutex) {
			...
			int stoppingTone = (toneTransmissionState == ToneTransmissionState.END_REQUESTED
					|| toneTransmissionState == ToneTransmissionState.END_SEQUENCE_INITIATED) ? 1 : 0;
	...
	private class DTMFDispatcher implements Runnable {
		...
		/**
		 * Waits for new tone to be reported via the <tt>addTonePacket()</tt> method and
		 * then delivers them to the <tt>AudioMediaStream</tt> that we are associated
		 * with.
		 */
		@Override
		public void run() {
		...
		/**
		 * A packet that we should convert to tone and deliver to our media stream and
