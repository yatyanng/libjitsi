...
public class RetransmissionRequesterDelegate implements RecurringRunnable {
	/**
	 * If more than <tt>MAX_MISSING</tt> consecutive packets are lost, we will not
	 * request retransmissions for them, but reset our state instead.
	 */
	public static final int MAX_MISSING = 100;

	/**
	 * The maximum number of retransmission requests to be sent for a single RTP
	 * packet.
	 */
	public static final int MAX_REQUESTS = 10;

	/**
	 * The interval after which another retransmission request will be sent for a
	 * packet, unless it arrives. Ideally this should not be a constant, but should
	 * be based on the RTT to the endpoint.
	 */
	public static final int RE_REQUEST_AFTER_MILLIS = 150;

	/**
	 * The interval we'll ask the {@link RecurringRunnableExecutor} to check back in
	 * if there is no current work TODO(brian): i think we should actually be able
	 * to get rid of this and just rely on scheduled work and the 'work ready now'
	 * callback
	 */
	...
	/**
	 * Maps an SSRC to the <tt>Requester</tt> instance corresponding to it. TODO:
	 * purge these somehow (RTCP BYE? Timeout?)
	 */
	...
	/**
	 * The SSRC which will be used as Packet Sender SSRC in NACK packets sent by
	 * this {@code RetransmissionRequesterDelegate}.
	 */
	...
	public long getTimeUntilNextRun() {
		...
		if (nextDueRequester == null) {
			...
			if (logger.isTraceEnabled()) {
				logger.trace(hashCode() + ": Next nack is scheduled for ssrc " + nextDueRequester.ssrc + " at "
						+ Math.max(nextDueRequester.nextRequestAt, 0) + ".  (current time is " + now + ")");
			}
	...
	/**
	 * Get a list of the requesters (not necessarily in sorted order) which are due
	 * to request as of the given time
	 *
	 * @param currentTime the current time
	 * @return a list of the requesters (not necessarily in sorted order) which are
	 *         due to request as of the given time
	 */
		...
		synchronized (requesters) {
			for (Requester requester : requesters.values()) {
				if (requester.isDue(currentTime)) {
					if (logger.isTraceEnabled()) {
						logger.trace(hashCode() + " requester for ssrc " + requester.ssrc + " has work due at "
								+ requester.nextRequestAt + " (now = " + currentTime + ") and is missing packets: "
		...
		for (Requester dueRequester : dueRequesters) {
			synchronized (dueRequester) {
				...
				if (!missingPackets.isEmpty()) {
					if (logger.isTraceEnabled()) {
						logger.trace(hashCode() + " Sending nack with packets " + missingPackets + " for ssrc "
								+ dueRequester.ssrc);
	...
	private class Requester {
		 ...
		 * @return true if this {@link Requester} is due to send a nack, false otherwise
		 */
		 ...
		 *
		 * @return true if there is work for this requester ready to be done now, false
		 *         otherwise
		 */
		synchronized private boolean received(int seq) {
			...
			if (diff <= 0) {
				...
				if (r != null && logger.isDebugEnabled()) {
					...
					if (rtt > 0) {
						...
						long delta = firstRequestSentAt > 0 ? timeProvider.currentTimeMillis() - r.firstRequestSentAt
								: 0;
			...
			} else // if (diff > MAX_MISSING)
			{
				...
				lastReceivedSeq = seq;
				if (logger.isDebugEnabled()) {
					logger.debug("Resetting retransmission requester state. " + "SSRC: " + ssrc + ", last received: "
							+ lastReceivedSeq + ", current: " + seq + ". Removing " + requests.size()
		...
		/**
		 * Returns a set of RTP sequence numbers which are considered still MIA, and for
		 * which a retransmission request needs to be sent. Assumes that the returned
		 * set of sequence numbers will be requested immediately and updates the state
		 * accordingly (i.e. increments the timesRequested counters and sets the time of
		 * next request).
		 *
		 * @return a set of RTP sequence numbers which are considered still MIA, and for
		 *         which a retransmission request needs to be sent.
		 */
		...
		public synchronized void notifyNackCreated(long time, Collection<Integer> sequenceNumbers) {
			for (Integer seqNum : sequenceNumbers) {
				...
				if (request.timesRequested == MAX_REQUESTS) {
					if (logger.isDebugEnabled()) {
						logger.debug("Generated the last NACK for SSRC=" + ssrc + " seq=" + request.seq + ". "
								+ "Time since the first request: " + (time - request.firstRequestSentAt));
					}
	...
	private static class Request {
		...
		/**
		 * The system time at the moment a retransmission request for this packet was
		 * first sent.
		 */
		long firstRequestSentAt = -1;

		/**
		 * The number of times that a retransmission request for this packet has been
		 * sent.
		 */
		int timesRequested = 0;

		/**
		 * Initializes a new <tt>Request</tt> instance with the given RTP sequence
		 * number.
		 * 
