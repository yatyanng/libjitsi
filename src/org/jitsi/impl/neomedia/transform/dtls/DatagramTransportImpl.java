...
public class DatagramTransportImpl implements DatagramTransport {
	/**
	 * The <tt>Logger</tt> used by the <tt>DatagramTransportImpl</tt> class and its
	...
	/**
	 * The queue of <tt>RawPacket</tt>s which have been received from the network
	 * are awaiting to be received by the application through this
	 ...
	 *                    data/RTP packets or {@link Component#RTCP} if the new
	 ...
	 *
	 * @param cause the <tt>Throwable</tt> which would have been thrown if the bug
	 *              did not exist
	 */
	private void breakOutOfDTLSReliableHandshakeReceiveMessage(Throwable cause) {
		for (StackTraceElement stackTraceElement : cause.getStackTrace()) {
			if ("org.bouncycastle.crypto.tls.DTLSReliableHandshake".equals(stackTraceElement.getClassName())
	...
	public int getSendLimit() throws IOException {
		...
		if (sendLimit <= 0) {
			/*
			 * XXX The estimation bellow is wildly inaccurate and hardly related but we have
			 * to start somewhere.
			 */
	...
	/**
	 * Queues a packet received from the network to be received by the application
	 * through this <tt>DatagramTransport</tt>.
	 *
	 * @param buf the array of <tt>byte</tt>s which contains the packet to be queued
	 * @param off the offset within <tt>buf</tt> at which the packet to be queued
	 *            starts
	...
	public int receive(byte[] buf, int off, int len, int waitMillis) throws IOException {
		...
		/*
		 * If this DatagramTransportImpl is to be received from, then what is to be
		 * received may be a response to a request that was earlier scheduled for send.
		 */
		/*
		 * XXX However, it may unnecessarily break up a flight into multiple datagrams.
		 * Since we have implemented the recognition of the end of flights, it should be
		 * fairly safe to rely on it alone.
		 */
//        flush();

		/*
		 * If no datagram is received at all and the specified waitMillis expires, a
		 * negative value is to be returned in order to have the outbound flight
		 * retransmitted.
		 */
		int received = -1;
			...
			synchronized (receiveQ) {
				...
				if (pkt != null) {
					...
					if (toReceiveIsPositive) {
						...
						if (toReceiveIsPositive) {
							/*
							 * The specified buf has received toReceive bytes and we do not concatenate
							 * RawPackets.
							 */
							break;
						}
	...
	public void send(byte[] buf, int off, int len) throws IOException {
		...
		if (len >= DtlsPacketTransformer.DTLS_RECORD_HEADER_LENGTH) {
			...
			case ContentType.handshake:
				short msg_type = TlsUtils.readUint8(buf, off + 1 /* type */
						+ 2 /* version */
						+ 2 /* epoch */
						+ 6 /* sequence_number */
						+ 2 /* length */);
	 ...
	 * 
	 * @param connector the <tt>RTPConnector</tt> which represents and implements
	 *                  the actual <tt>DatagramSocket</tt> to be adapted by this
	 *                  instance
	 */
