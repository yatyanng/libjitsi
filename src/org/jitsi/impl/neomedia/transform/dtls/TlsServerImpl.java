...
public class TlsServerImpl extends DefaultTlsServer {
	...
	private final CertificateRequest certificateRequest = new CertificateRequest(
			new short[] { ClientCertificateType.rsa_sign }, /* supportedSignatureAlgorithms */ null,
			/* certificateAuthorities */ null);

	/**
	 * The <tt>SRTPProtectionProfile</tt> negotiated between this DTLS-SRTP server
	 * and its client.
	 */
	 ...
	 *
	 * @param packetTransformer the <tt>PacketTransformer</tt> which is initializing
	 *                          the new instance
	 */
	 ...
	 *
	 * @return the <tt>SRTPProtectionProfile</tt> negotiated between this DTLS-SRTP
	 *         server and its client
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * Overrides the super implementation to explicitly specify cipher suites which
	 * we know to be supported by Bouncy Castle and provide Perfect Forward Secrecy.
	 */
	@Override
	protected int[] getCipherSuites() {
		return new int[] {
				/* core/src/main/java/org/bouncycastle/crypto/tls/DefaultTlsServer.java */
				CipherSuite.TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384, CipherSuite.TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
	 ...
	 * initialized.
	 *
	 * @return the <tt>TlsContext</tt> with which this <tt>TlsServer</tt> has been
	 *         initialized
	 */
	 ...
	 * does not work with the Bouncy Castle Crypto APIs at the time of this writing.
	 */
	@Override
	 ...
	 * {@inheritDoc}
	 *
	 * Depending on the <tt>selectedCipherSuite</tt>, <tt>DefaultTlsServer</tt> will
	 * require either <tt>rsaEncryptionCredentials</tt> or
	...
	protected TlsEncryptionCredentials getRSAEncryptionCredentials() throws IOException {
		if (rsaEncryptionCredentials == null) {
			...
			rsaEncryptionCredentials = new DefaultTlsEncryptionCredentials(context, certificateInfo.getCertificate(),
	...
	protected TlsSignerCredentials getRSASignerCredentials() throws IOException {
		if (rsaSignerCredentials == null) {
			...
			// the certificate.
			rsaSignerCredentials = new DefaultTlsSignerCredentials(context, certificateInfo.getCertificate(),
					certificateInfo.getKeyPair().getPrivate(),
	...
	public Hashtable getServerExtensions() throws IOException {
		...
		if (TlsSRTPUtils.getUseSRTPExtension(serverExtensions) == null) {
			...
			int chosenProtectionProfile = DtlsControlImpl
					.chooseSRTPProtectionProfile(useSRTPData.getProtectionProfiles());
	...
	/**
	 * FIXME: If Client Hello does not include points format extensions then we will
	 * end up with alert 47 failure caused by NPE on serverECPointFormats. It was
	 * causing JitsiMeet to fail with Android version of Chrome.
	 *
	 * The fix has been posted upstream and this method should be removed once it is
	 * published.
	 */
	@SuppressWarnings("rawtypes")
	private Hashtable getServerExtensionsOverride() throws IOException {
		...
		if (TlsECCUtils.isECCCipherSuite(selectedCipherSuite)) {
			 ...
			 */
			serverECPointFormats = new short[] { ECPointFormat.uncompressed, ECPointFormat.ansiX962_compressed_prime,
	 ...
	 * {@inheritDoc}
	 *
	 * Overrides the super implementation as a simple means of detecting that the
	 * security-related negotiations between the local and the remote enpoints are
	 * starting. The detection carried out for the purposes of
	 * <tt>SrtpListener</tt>.
	 */
	@Override
	...
	/**
	 * Determines whether this {@code TlsServerImpl} is to operate in pure DTLS mode
	 * without SRTP extensions or in DTLS/SRTP mode.
	 *
	 ...
	 * {@inheritDoc}
	 *
	 * Makes sure that the DTLS extended client hello contains the <tt>use_srtp</tt>
	 * extension.
	 */
	@Override
	@SuppressWarnings("rawtypes")
