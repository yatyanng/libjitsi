...
public class CompoundPacketEngine implements TransformEngine, PacketTransformer {
	/**
	 * The maximum number of individual RTCP packets contained in an RTCP compound
	 * packet. If an input packet (seems to) contain more than this, it will remain
	 * unchanged.
	 */
	private final static int MAX_INDIVIDUAL = 20;

	/**
	 * Returns the length in bytes of the RTCP packet contained in <tt>buf</tt> at
	 * offset <tt>off</tt>. Assumes that <tt>buf</tt> is valid at least until index
	 * <tt>off</tt>+3.
	 * 
	 * @return the length in bytes of the RTCP packet contained in <tt>buf</tt> at
	 *         offset <tt>off</tt>.
	 */
	...
	/**
	 * Returns a reference to this class since it is performing RTCP transformations
	 * in here.
	 *
	 ...
	 * {@inheritDoc}
	 *
	 * The implementation of <tt>CompoundPacketEngine</tt> does not transform when
	 * sending RTCP packets because the only purpose is to split received compound
	 * RTCP packets.
	 */
	@Override
