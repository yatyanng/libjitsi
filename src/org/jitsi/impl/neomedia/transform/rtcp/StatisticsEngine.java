...
public class StatisticsEngine extends SinglePacketTransformer implements TransformEngine {
	...
	/**
	 * Determines whether <tt>buf</tt> appears to contain an RTCP packet starting at
	 * <tt>off</tt> and spanning at most <tt>len</tt> bytes. Returns the length in
	 * bytes of the RTCP packet if it was determined that there indeed appears to be
	 * such an RTCP packet; otherwise, <tt>-1</tt>.
	 *
	 * @param buf
	 * @param off
	 * @param len
	 * @return the length in bytes of the RTCP packet in <tt>buf</tt> starting at
	 *         <tt>off</tt> and spanning at most <tt>len</tt> bytes if it was
	...
	/**
	 * The <tt>MediaType</tt> of {@link #mediaStream}. Cached for the purposes of
	 * performance.
	 */
	...
	/**
	 * The {@link RTCPPacketParserEx} which this instance will use to parse RTCP
	 * packets.
	 */
	 ...
	 *
	 * @param pkt            the <tt>RawPacket</tt> into which
	 *                       <tt>extendedReport</tt> is to be added
	 * @param extendedReport the RTCP XR packet to add into <tt>pkt</tt>
	 * @return <tt>true</tt> if <tt>extendedReport</tt> was added into <tt>pkt</tt>;
	...
	private boolean addRTCPExtendedReport(RawPacket pkt, RTCPExtendedReport extendedReport) {
		...
		while (off < end) {
			...
			if (pt == RTCPPacket.BYE) {
				...
				if ((sc < 0) || (rtcpPktLen < ((1 + sc) * 4))) {
					...
					for (int i = 0, ssrcOff = off + 4; i < sc; ++i, ssrcOff += 4) {
		...
		if (off <= end) {
			...
			DataOutputStream dataoutputstream = new DataOutputStream(
					new ByteBufferOutputStream(buf, off, extendedReportLen));
	...
	/**
	 * Adds RTP Control Protocol Extended Report (RTCP XR) packets to <tt>pkt</tt>
	 * if <tt>pkt</tt> contains RTCP SR or RR packets.
	 *
	 * @param pkt       the <tt>RawPacket</tt> to which RTCP XR packets are to be
	 *                  added
	 * @param sdpParams
	 * @return a list of <tt>RTCPExtendedReport</tt> packets added to <tt>pkt</tt>
	 *         or <tt>null</tt> or an empty list if no RTCP XR packets were added to
	 *         <tt>pkt</tt>
	 */
	 ...
	 * @param sourceSSRCs the SSRCs of the RTP data packet sources to be reported
	 *                    upon by the new RTCP XR packet
	 * @param sdpParams
	...
	private RTCPExtendedReport createRTCPExtendedReport(int senderSSRC, int[] sourceSSRCs, String sdpParams) {
		...
		if ((sourceSSRCs != null) && (sourceSSRCs.length != 0) && (sdpParams != null)
				&& sdpParams.contains(RTCPExtendedReport.VoIPMetricsReportBlock.SDP_PARAMETER)) {
			xr = new RTCPExtendedReport();
			for (int sourceSSRC : sourceSSRCs) {
				RTCPExtendedReport.VoIPMetricsReportBlock reportBlock = createVoIPMetricsReportBlock(senderSSRC,
						sourceSSRC);
	...
	/**
	 * Initializes a new RTCP XR &quot;VoIP Metrics Report Block&quot; as defined by
	 * RFC 3611.
	 *
	 * @param senderSSRC
	 ...
	 *
	 * @param senderSSRC
	 * @param receiveStream the <tt>ReceiveStream</tt> to be reported upon by the
	 *                      new instance
	 * @return a new <tt>RTCPExtendedReport.VoIPMetricsReportBlock</tt> instance
	 *         reporting upon <tt>receiveStream</tt>
	 */
	private RTCPExtendedReport.VoIPMetricsReportBlock createVoIPMetricsReportBlock(int senderSSRC,
		...
		// end system delay
		/*
		 * Defined as the sum of the total sample accumulation and encoding delay
		 * associated with the sending direction and the jitter buffer, decoding, and
		 * playout buffer delay associated with the receiving direction. Collectively,
		 * these cover the whole path from the network to the very playback of the
		 * audio. We cannot guarantee latency pretty much anywhere along the path and,
		 * consequently, the metric will be "extremely variable".
		 */

		// signal level
		// noise level
		/*
		 * The computation of noise level requires the notion of silent period which we
		 * do not have (because, for example, we do not voice activity detection).
		 */

		// residual echo return loss (RERL)
		/*
		 * WebRTC, which is available and default on OS X, appears to be able to provide
		 * the residual echo return loss. Speex, which is available and not default on
		 * the supported operating systems, and WASAPI, which is available and default
		 * on Windows, do not seem to be able to provide the metric. Taking into account
		 * the availability of the metric and the distribution of the users according to
		 * operating system, the support for the metric is estimated to be
		 * insufficiently useful. Moreover, RFC 3611 states that RERL for "PC softphone
		 * or speakerphone" is "extremely variable, consider reporting "undefined"
		 * (127)".
		 */
		 ...
		 * impairments, and others that we do not know how to define at the time of this
		 * writing.
		 */
		// ext. R factor
		/*
		 * The external R factor is a voice quality metric describing the segment of the
		 * call that is carried over a network segment external to the RTP segment, for
		 * example a cellular network. We implement the RTP segment only and we do not
		 * have a notion of a network segment external to the RTP segment.
		 */
		// MOS-LQ
		/*
		 * TODO It is unclear at the time of this writing from RFC 3611 how MOS-LQ is to
		 * be calculated.
		 */
		// MOS-CQ
		/*
		 * The metric may be calculated by converting an R factor determined according
		 * to ITU-T G.107 or ETSI TS 101 329-5 into an estimated MOS using the equation
		 * specified in G.107. However, we do not have R factor.
		 */

		// receiver configuration byte (RX config)
		// packet loss concealment (PLC)
		/*
		 * We insert silence in place of lost packets by default and we have FEC and/or
		 * PLC for OPUS and SILK.
		 */
		...
		if (jbc == null) {
			voipMetrics
					.setJitterBufferAdaptive(RTCPExtendedReport.VoIPMetricsReportBlock.UNKNOWN_JITTER_BUFFER_ADAPTIVE);
		} else {
			// discard rate
			if (expectedPacketCount > 0) {
			...
			if (jbc.isAdaptiveBufferEnabled()) {
				...
				voipMetrics.setJitterBufferAdaptive(
						RTCPExtendedReport.VoIPMetricsReportBlock.NON_ADAPTIVE_JITTER_BUFFER_ADAPTIVE);
			...
			/*
			 * TODO We do not know how to calculate it at the time of this writing. It very
			 * likely is to be calculated in JitterBufferBehaviour because
			 * JitterBufferBehaviour implements the adaptiveness of a jitter buffer
			 * implementation.
			 */
		}
	...
	private long getFECDecodedPacketCount(ReceiveStream receiveStream) {
		...
		if (devSession != null) {
			Iterable<FECDecoderControl> decoderControls = devSession.getDecoderControls(receiveStream,
	...
	/**
	 * Returns a reference to this class since it is performing RTP transformations
	 * in here.
	 *
	...
	/**
	 * Initializes a new SR or RR <tt>RTCPReport</tt> instance from a specific byte
	 * array.
	 *
	 ...
	 *                     <tt>pkt</tt> into a new SR or RR <tt>RTCPReport</tt>
	 *                     instance.
	 */
	private RTCPReport parseRTCPReport(int type, byte[] buf, int off, int len) throws IOException {
		switch (type) {
		case RTCPPacket.RR:
	...
	public RawPacket reverseTransform(RawPacket pkt) {
		...
		if (RTCPUtils.isRtcp(pkt.getBuffer(), pkt.getOffset(), pkt.getLength())) {
			...
			if (compound == null || compound.packets == null || compound.packets.length == 0) {
				logger.info("Failed to parse an incoming RTCP packet: " + (ex == null ? "null" : ex.getMessage()));
	...
	/**
	 * Processes the {@link RTCPPacket}s from {@code in} as received RTCP packets
	 * and updates the {@link MediaStreamStats}. Adds to {@code out} the ones which
	 * were not consumed and should be output from this instance.
	 * 
			...
			case RTCPFBPacket.PSFB:
				if (rtcp instanceof RTCPREMBPacket) {
					...
					if (logger.isTraceEnabled()) {
						logger.trace("remb_received,stream=" + mediaStream.hashCode() + " bps=" + remb.getBitrate()
								+ ",dest=" + Arrays.toString(remb.getDest()));
					}
	...
	/**
	 * Transfers RTCP sender report feedback as new information about the download
	 * stream for the MediaStreamStats. Finds the info needed for statistics in the
	 * packet and stores it, then returns the same packet as
	...
	/**
	 * Transfers RTCP sender/receiver report feedback as new information about the
	 * download stream for the <tt>MediaStreamStats</tt>.
	 *
			...
			if (!feedbackReports.isEmpty()) {
				...
				if (logger.isTraceEnabled()) {
					...
					if (numberOfRTCPReports % 4 == 1) {
						...
						buff.append("Sending a report for ").append(mediaTypeStr).append(" stream SSRC:").append(ssrc)
								.append(" [");
						// SR includes sender info, RR does not.
						if (r instanceof RTCPSenderReport) {
							RTCPSenderReport sr = (RTCPSenderReport) r;

							buff.append("packet count:").append(sr.getSenderPacketCount()).append(", bytes:")
									.append(sr.getSenderByteCount()).append(", ");
						}
						buff.append("interarrival jitter:").append(jitter).append(", lost packets:")
	...
	/**
	 * Computes the sum of the values of a specific {@code Map} with {@code Long}
	 * values.
	 *
	 * @param map the {@code Map} with {@code Long} values to sum up. Note that we
	...
	/**
	 * Utility method to return a value from a map and perform unboxing only if the
