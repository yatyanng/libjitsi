...
public class PayloadTypeTransformEngine extends SinglePacketTransformerAdapter implements TransformEngine {
	 ...
	 * nothing, packets are passed through without modification. Maps source payload
	 * to target payload.
	 */
	...
	/**
	 * Checks if there are any override mappings, if no setting just pass through
	 * the packet. If the <tt>RawPacket</tt> payload has entry in mappings to
	 * override, we override packet payload type.
	 *
	...
	public RawPacket transform(RawPacket pkt) {
		if (mappingOverridesCopy == null || mappingOverridesCopy.isEmpty() || pkt.getVersion() != RTPHeader.VERSION) {
			return pkt;
		}
	...
	/**
	 * Adds an additional RTP payload type mapping used to override the payload type
	 * of outgoing RTP packets. If an override for <tt>originalPT<tt/>, was already
	 * being overridden, this call is simply going to update the override to the new
	 * one.
	 * <p>
