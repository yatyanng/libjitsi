...
public class MediaUtils {
	...
	/**
	 * An empty array with <tt>MediaFormat</tt> element type. Explicitly defined in
	...
	/**
	 * The <tt>Map</tt> of JMF-specific encodings to well-known encodings as defined
	 * in RFC 3551.
	 */
	...
	/**
	 * The <tt>MediaFormat</tt>s which do not have RTP payload types assigned by RFC
	 * 3551 and are thus referred to as having dynamic RTP payload types.
	 */
	...
	static {
		addMediaFormats((byte) SdpConstants.PCMU, "PCMU", MediaType.AUDIO, AudioFormat.ULAW_RTP, 8000);

		/*
		 * Some codecs depend on JMF native libraries which are only available on 32-bit
		...
		if (OSUtils.IS_LINUX32 || OSUtils.IS_WINDOWS32) {
			...
			addMediaFormats((byte) SdpConstants.G723, "G723", MediaType.AUDIO, AudioFormat.G723_RTP, g723FormatParams,
					null, 8000);
		}

		addMediaFormats((byte) SdpConstants.GSM, "GSM", MediaType.AUDIO, AudioFormat.GSM_RTP, 8000);
		addMediaFormats((byte) SdpConstants.PCMA, "PCMA", MediaType.AUDIO, Constants.ALAW_RTP, 8000);
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, "iLBC", MediaType.AUDIO, Constants.ILBC_RTP, 8000);
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, Constants.SPEEX, MediaType.AUDIO, Constants.SPEEX_RTP,
				8000, 16000, 32000);
		addMediaFormats((byte) SdpConstants.G722, "G722", MediaType.AUDIO, Constants.G722_RTP, 8000);
		if (EncodingConfigurationImpl.G729) {
			Map<String, String> g729FormatParams = new HashMap<>();
			g729FormatParams.put("annexb", "no");

			addMediaFormats((byte) SdpConstants.G729, "G729", MediaType.AUDIO, AudioFormat.G729_RTP, g729FormatParams,
					null, 8000);
		}
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, "telephone-event", MediaType.AUDIO,
				Constants.TELEPHONE_EVENT, 8000);
		...
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, Constants.RED, MediaType.VIDEO, Constants.RED);
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, Constants.ULPFEC, MediaType.VIDEO, Constants.ULPFEC);
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, Constants.FLEXFEC_03, MediaType.VIDEO,
		...
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, "SILK", MediaType.AUDIO, Constants.SILK_RTP,
				silkFormatParams, null, 8000, 12000, 16000, 24000);
		...
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, Constants.OPUS, MediaType.AUDIO, Constants.OPUS_RTP, 2,
				opusFormatParams, opusAdvancedParams, 48000);

		// Adaptive Multi-Rate Wideband (AMR-WB)
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, Constants.AMR_WB, MediaType.AUDIO, Constants.AMR_WB_RTP,
				16000);
		 ...
		 * VideoFormat.JPEG_RTP); addMediaFormats( (byte) SdpConstants.H263, "H263",
		 * MediaType.VIDEO, VideoFormat.H263_RTP); addMediaFormats( (byte)
		 * SdpConstants.H261, "H261", MediaType.VIDEO, VideoFormat.H261_RTP);
		 */
		...
		if (h264Enabled || cfg.getBoolean(MediaService.ENABLE_H264_FORMAT_PNAME, false)) {
			...
			/*
			 * Disable PLI because the periodic intra-refresh feature of FFmpeg/x264 is
			 * used.
			 */
			// h264AdvancedAttributes.put("rtcp-fb", "nack pli");

			/*
			 * XXX The initialization of MediaServiceImpl is very complex so it is wise to
			 * not reference it at the early stage of its initialization.
			 */
			...
			/*
			 * if ((cfg == null) || cfg .getString(
			 * "net.java.sip.communicator.impl.neomedia" +
			 * ".codec.video.h264.defaultProfile", JNIEncoder.MAIN_PROFILE)
			 * .equals(JNIEncoder.MAIN_PROFILE)) { // main profile, common features, HD
			 * capable level 3.1 h264FormatParams.put("profile-level-id", "4DE01f"); } else
			 */
			{
			...
			if ((cfg == null) || cfg.getBoolean(
					"net.java.sip.communicator.impl.neomedia" + ".codec.video.h264.packetization-mode-1.enabled",
					true)) {
				// packetization-mode=1
				h264FormatParams.put(packetizationMode, "1");
				addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, Constants.H264, MediaType.VIDEO,
						Constants.H264_RTP, h264FormatParams, h264AdvancedAttributes);
			}
			// packetization-mode=0
			/*
			 * XXX At the time of this writing,
			 * EncodingConfiguration#compareEncodingPreferences(MediaFormat, MediaFormat) is
			 * incomplete and considers two MediaFormats to be equal if they have an equal
			 * number of format parameters (given that the encodings and clock rates are
			 * equal, of course). Either fix the method in question or don't add a format
			 * parameter for packetization-mode 0 (which is equivalent to having
			 * packetization-mode explicitly defined as 0 anyway, according to the
			 * respective RFC).
			 */
			h264FormatParams.remove(packetizationMode);
			addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, Constants.H264, MediaType.VIDEO, Constants.H264_RTP,
					h264FormatParams, h264AdvancedAttributes);
		}

		/*
		 * H263+ Map<String, String> h263FormatParams = new HashMap<String, String>();
		 * Map<String, String> h263AdvancedAttributes = new LinkedHashMap<String,
		 * String>();
		 * 
		 * // The maximum resolution we can receive is the size of our screen // device.
		 * if (res != null) h263FormatParams.put("CUSTOM", res.width + "," + res.height
		 * + ",2"); h263FormatParams.put("VGA", "2"); h263FormatParams.put("CIF", "1");
		...
		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, Constants.VP8, MediaType.VIDEO, Constants.VP8_RTP, null,
				null);

		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, Constants.VP9, MediaType.VIDEO, Constants.VP9_RTP, null,
				null);

		addMediaFormats(MediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, Constants.RTX, MediaType.VIDEO, Constants.RTX_RTP, null,
				null);
	 ...
	 * <tt>MediaFormat</tt>s of a specific <tt>MediaType</tt>, with a specific JMF
	 * encoding and, optionally, with specific clock rates.
	 *
	 * @param rtpPayloadType the RTP payload type to be associated with a list of
	 *                       <tt>MediaFormat</tt>s
	 * @param encoding       the well-known encoding (name) corresponding to
	 *                       <tt>rtpPayloadType</tt> (in contrast to the
	 *                       JMF-specific encoding specified by
	 *                       <tt>jmfEncoding</tt>)
	 ...
	 */
	private static void addMediaFormats(byte rtpPayloadType, String encoding, MediaType mediaType, String jmfEncoding,
			double... clockRates) {
	 ...
	 *                           JMF-specific encoding specified by
	 *                           <tt>jmfEncoding</tt>)
	 * @param mediaType          the <tt>MediaType</tt> of the <tt>MediaFormat</tt>s
	...
	private static void addMediaFormats(byte rtpPayloadType, String encoding, MediaType mediaType, String jmfEncoding,
			int channels, Map<String, String> formatParameters, Map<String, String> advancedAttributes,
			double... clockRates) {
		...
		if (mediaFormats.size() > 0) {
			...
			jmfEncodingToEncodings.put(((MediaFormatImpl<? extends Format>) mediaFormats.get(0)).getJMFEncoding(),
					encoding);
		}
	}
	 ...
	 */
	private static void addMediaFormats(byte rtpPayloadType, String encoding, MediaType mediaType, String jmfEncoding,
			Map<String, String> formatParameters, Map<String, String> advancedAttributes, double... clockRates) {
		addMediaFormats(rtpPayloadType, encoding, mediaType, jmfEncoding, 1 /* channel */, formatParameters,
				advancedAttributes, clockRates);
	}
	...
	public static String createImageAttr(java.awt.Dimension sendSize, java.awt.Dimension maxRecvSize) {
		...
		if (sendSize != null) {
			 ...
			 * img.append(",y="); img.append((int)sendSize.getHeight()); img.append("]");
			 */
	 ...
	 * represents a specific JMF <tt>Format</tt>. If there is no such representing
	 * <tt>MediaFormat</tt> in <tt>MediaUtils</tt>, returns <tt>null</tt>.
	 *
	 ...
	 *
	 * @param encoding  the well-known encoding (name) of the <tt>MediaFormat</tt>
	 *                  to get
	 * @param clockRate the clock rate of the <tt>MediaFormat</tt> to get
	 * @return the <tt>MediaFormat</tt> known to <tt>MediaUtils</tt> and having the
	 *         specified <tt>encoding</tt> and <tt>clockRate</tt>
	 */
	 ...
	 * @return the <tt>MediaFormat</tt> known to <tt>MediaUtils</tt> and having the
	 *         specified <tt>encoding</tt> (name), <tt>clockRate</tt> and matching
	 *         format parameters
	 */
	 ...
	 * internal storage of <tt>MediaUtils</tt>. Since the index is in the internal
	 * storage which may or may not be one and the same for the various
	 * <tt>MediaFormat</tt> instances and which may or may not be searched for the
	 * purposes of determining the index, the index is not to be used as a way to
	 * determine whether <tt>MediaUtils</tt> knows the specified
	 * <tt>mediaFormat</tt>
	 *
	 ...
	 *
	 * @param rtpPayloadType the RTP payload type to retrieve the corresponding
	 *                       <tt>MediaFormat</tt>s for
	 * @return an array of <tt>MediaFormat</tt>s corresponding to the specified RTP
	 *         payload type
	 */
	...
	/**
	 * Gets the <tt>MediaFormat</tt>s known to <tt>MediaUtils</tt> and being of the
	 * specified <tt>MediaType</tt>.
	 *
	 * @param mediaType the <tt>MediaType</tt> of the <tt>MediaFormat</tt>s to get
	 * @return the <tt>MediaFormat</tt>s known to <tt>MediaUtils</tt> and being of
	 ...
	 * specific well-known encoding (name) as defined by RFC 3551 "RTP Profile for
		...
		if (jmfEncoding != null) {
			for (MediaFormat[] rtpPayloadTypeMediaFormats : rtpPayloadTypeStrToMediaFormats.values()) {
				for (MediaFormat rtpPayloadTypeMediaFormat : rtpPayloadTypeMediaFormats) {
					if (((MediaFormatImpl<? extends Format>) rtpPayloadTypeMediaFormat).getJMFEncoding()
							.equals(jmfEncoding)) {
			...
			if (mediaFormats.size() < 1) {
				for (MediaFormat rtpPayloadTypelessMediaFormat : rtpPayloadTypelessMediaFormats) {
					if (((MediaFormatImpl<? extends Format>) rtpPayloadTypelessMediaFormat).getJMFEncoding()
							.equals(jmfEncoding)) {
	...
	/**
	 * Gets the RTP payload type corresponding to a specific JMF encoding and clock
	 * rate.
	 *
	 * @param jmfEncoding the JMF encoding as returned by
	 *                    {@link Format#getEncoding()} or the respective
	 *                    <tt>AudioFormat</tt> and <tt>VideoFormat</tt> encoding
	 *                    constants to get the corresponding RTP payload type of
	 * @param clockRate   the clock rate to be taken into account in the search for
	 *                    the RTP payload type if the JMF encoding does not uniquely
	 *                    identify it
	 * @return the RTP payload type corresponding to the specified JMF encoding and
	 *         clock rate if known in RFC 3551 "RTP Profile for Audio and Video
	...
	/**
	 * Gets the well-known encoding (name) as defined in RFC 3551 "RTP Profile for
	 * Audio and Video Conferences with Minimal Control" corresponding to a given
	 * JMF-specific encoding.
	 *
	 * @param jmfEncoding the JMF encoding to get the corresponding well-known
	 *                    encoding of
	 * @return the well-known encoding (name) as defined in RFC 3551 "RTP Profile
	 *         for Audio and Video Conferences with Minimal Control" corresponding
	 *         to <tt>jmfEncoding</tt> if any; otherwise, <tt>null</tt>
	 */
