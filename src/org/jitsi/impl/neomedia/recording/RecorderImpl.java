...
public class RecorderImpl implements Recorder {
	...
	public static final String[] SUPPORTED_FORMATS = new String[] {
			/*
			 * Disables formats currently not working SoundFileUtils.aif, SoundFileUtils.au,
			 * SoundFileUtils.gsm,
			 */
			SoundFileUtils.wav, SoundFileUtils.mp3 };

	/**
	 * The <tt>AudioMixerMediaDevice</tt> which is to be or which is already being
	 * recorded by this <tt>Recorder</tt>.
	 */
	private final AudioMixerMediaDevice device;

	/**
	 * The <tt>RecorderEventHandler</tt> which this <tt>Recorder</tt> should notify
	 * when events related to recording (such as start/end of a recording) occur.
	 */
	...
	/**
	 * The <tt>List</tt> of <tt>Recorder.Listener</tt>s interested in notifications
	...
	/**
	 * The indicator which determines whether this <tt>Recorder</tt> is set to skip
	 * media from mic.
	 */
	 ...
	 *
	 * @param device device that can create a session that provides the output data
	 *               source
	 */
	...
	/**
	 * Adds a new <tt>Recorder.Listener</tt> to the list of listeners interested in
	 ...
	 * @throws IllegalArgumentException if the specified <tt>format</tt> is not a
	...
	/**
	 * Gets a list of the formats in which this <tt>Recorder</tt> supports recording
	 * media.
	 *
	 ...
	 *
	 * @param listener the existing <tt>Recorder.Listener</tt> to be removed from
	 *                 the list of listeners interested in notifications from this
	 *                 <tt>Recorder</tt>
	 ...
	 * (e.g. the media being sent and received in a <tt>Call</tt>) into a file with
	 * a specific name.
	 *
	...
	public void start(String format, String filename) throws IOException, MediaException {
		if (this.sink == null) {
			...
			/*
			 * A file without an extension may not only turn out to be a touch more
			 * difficult to play but is suspected to also cause an exception inside of JMF.
			 */
			...
			try {
				...
				/*
				 * This RecorderImpl will use deviceSession to get a hold of the media being set
				 * to the remote peers associated with the same AudioMixerMediaDevice i.e. this
	...
	/**
	 * Stops the recording of the media associated with this <tt>Recorder</tt> (e.g.
	 * the media being sent and received in a <tt>Call</tt>) if it has been started
	 * and prepares this <tt>Recorder</tt> for garbage collection.
	 *
	 * @see Recorder#stop()
	 */
	@Override
	public void stop() {
		...
		if (sink != null) {
			...
			/*
			 * RecorderImpl creates the sink upon start() and it does it only if it is null
			 * so this RecorderImpl has really stopped only if it has managed to close() the
			 * (existing) sink. Notify the registered listeners.
			 */
	...
	/**
	 * Put the recorder in mute state. It won't record the local input. This is used
	 * when the local call is muted and we don't won't to record the local input.
	 *
	...
	/**
	 * Returns the filename we are last started or stopped recording to, null if not
	 * started.
	 *
	 * @return the filename we are last started or stopped recording to, null if not
	 *         started.
	 */
	@Override
