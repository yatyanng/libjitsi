...
/**
 * A <tt>TransformEngine</tt> and <tt>PacketTransformer</tt> which implement a
 * fixed-size buffer. The class is specific to video recording. Buffered are
 * only VP8 RTP packets, and they are places in different buffers according to
 * their SSRC.
 *
 * @author Boris Grozev
 */
public class PacketBuffer implements TransformEngine, PacketTransformer {
	/**
	 * A <tt>Comparator</tt> implementation for RTP sequence numbers. Compares the
	 * sequence numbers <tt>a</tt> and <tt>b</tt> of <tt>pkt1</tt> and
	 * <tt>pkt2</tt>, taking into account the wrap at 2^16.
	 *
	 * IMPORTANT: This is a valid <tt>Comparator</tt> implementation only if used
	...
	/**
	 * Implements {@link PacketTransformer#reverseTransform(RawPacket[])}.
	 *
	...
	/**
	 * Checks whether a particular <tt>RawPacket</tt> will be buffered or not by
	 * this instance. Currently we only buffer VP8 packets, recognized by their
	 * payload type number.
	 * 
	...
	/**
	 * Resets the buffer for a particular SSRC (effectively re-enabling it if it was
	 * disabled).
	 * 
	 * @param ssrc
	 */
	...
	/**
	 * Gets the <tt>Buffer</tt> instance responsible for buffering packets with SSRC
	 * <tt>ssrc</tt>. Creates it if necessary, always returns non-null.
	 * 
	...
	/**
	 * Empties the <tt>Buffer</tt> for a specific SSRC, and returns its contents as
	 * an ordered (by RTP sequence number) array.
	 * 
	 * @param ssrc the SSRC for which to empty the <tt>Buffer</tt>.
	 * @return the contents of the <tt>Buffer</tt> for SSRC, or an empty array, if
	 *         there is no buffer for SSRC.
	 */
	...
	private static class Buffer {
		...
		/**
		 * Whether this buffer is disabled or not. If disabled, it will drop incoming
		 * packets, and output 'null'.
		 */
		...
		/**
		 * Inserts a specific <tt>RawPacket</tt> in this <tt>Buffer</tt>. If, after the
		 * insertion, the number of elements stored in the buffer is more than
		 * <tt>this.capacity</tt>, removes from the buffer and returns the 'first'
		 * packet in the buffer. Otherwise, return null.
		 *
		 * @param pkt the packet to insert.
		 * @return Either the 'first' packet in the buffer, or null, according to
		 *         whether the buffer capacity has been reached after the insertion of
		 *         <tt>pkt</tt>.
		 */
