...
public class RecorderEventHandlerJSONImpl implements RecorderEventHandler {
	/**
	 * The <tt>Logger</tt> used by the <tt>RecorderEventHandlerJSONImpl</tt> class
	...
	public synchronized boolean handleEvent(RecorderEvent ev) {
		...
		/*
		 * For a RECORDING_ENDED event without a valid instant, find it's associated
		 * (i.e. with the same SSRC) RECORDING_STARTED event and compute the
		 * RECORDING_ENDED instance based on its duration.
		 */
		if (RecorderEvent.Type.RECORDING_ENDED.equals(type) && ev.getInstant() == -1 && duration != -1) {
			List<RecorderEvent> events = MediaType.AUDIO.equals(mediaType) ? audioEvents : videoEvents;
