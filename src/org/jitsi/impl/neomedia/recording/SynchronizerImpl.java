...
public class SynchronizerImpl implements Synchronizer {
	/**
	 * Whether the CNAME identifier from RTCP SDES packets should be use as an
	 * endpoint identifier.
	 *
	 * If set to true, RTCP SDES packets fed through
	 * {@link #addRTCPPacket(RawPacket)} will be searched for CNAME items, and the
	 * values will be used as endpoint identifiers.
	 */
	...
	/**
	 * Maps an SSRC to the <tt>SSRCDesc</tt> structure containing information about
	 * it.
	 */
	...
	/**
	 * Adds an RTCP packet to this instance. Time mappings are extracted and used by
	 * this instance.
	 * 
	...
	/**
	 * Returns the <tt>SSRCDesc</tt> instance mapped to the SSRC <tt>ssrc</tt>. If
	 * no instance is mapped to <tt>ssrc</tt>, create one and inserts it in the map.
	 ...
	 * 
	 * @return the string with length <tt>len</tt>read from <tt>buf</tt> at offset
	 *         <tt>off</tt>.
	 */
	 ...
	 * <tt>pkt</tt>. Assumes that <tt>pkt</tt> is a valid RTCP packet (at least as
	 * reported by {@link #isValidRTCP(RawPacket)}).
	 * 
	 ...
	 * between a wallclock at the endpoint and a time we chose on the local system
	 * clock to correspond to it.
	 */
