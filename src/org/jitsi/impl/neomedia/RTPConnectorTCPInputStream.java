...
public class RTPConnectorTCPInputStream extends TransformInputStream<Socket> {
	 ...
	 * data from a specific TCP socket.
	 *
	...
	protected void doLogPacket(DatagramPacket p) {
		...
		if (pktLogging != null) {
			pktLogging.logPacket(PacketLoggingService.ProtocolName.RTP,
					(p.getAddress() != null) ? p.getAddress().getAddress() : new byte[] { 0, 0, 0, 0 }, p.getPort(),
					socket.getLocalAddress().getAddress(), socket.getLocalPort(),
					PacketLoggingService.TransportName.TCP, false, p.getData(), p.getOffset(), p.getLength());
		}
	}
