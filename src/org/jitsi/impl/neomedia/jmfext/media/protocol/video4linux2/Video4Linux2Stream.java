...
public class Video4Linux2Stream extends AbstractVideoPullBufferStream<DataSource> {
	 ...
	 * media data captured by the Video for Linux Two API Specification device out
	 * of this instance through the <tt>Buffer</tt>s specified in its
	 * {@link #read(Buffer)}.
	 */
	...
	/**
	 * The file descriptor of the Video for Linux Two API Specification device read
	 ...
	 * through which the Video for Linux Two API Specification device provides the
	 * captured media data to this instance when {@link #requestbuffersMemory} is
	 * equal to <tt>V4L2_MEMORY_MAP</tt>.
	 */
	private int[] mmapLengths;

	/**
	 * The buffers through which the Video for Linux Two API Specification device
	 * provides the captured media data to this instance when
	 * {@link #requestbuffersMemory} is equal to <tt>V4L2_MEMORY_MAP</tt>. These are
	 * mapped in the application's address space.
	 */
	private long[] mmaps;
	...
	/**
	 * The number of buffers through which the Video for Linux Two API Specification
	 * device provides the captured media data to this instance when
	 * {@link #requestbuffersMemory} is equal to <tt>V4L2_MEMORY_MMAP</tt>.
	 */
	private int requestbuffersCount = 0;

	/**
	 * The input method negotiated by this instance with the Video for Linux Two API
	...
	/**
	 * The <tt>v4l2_buffer</tt> instance via which captured media data is fetched
	 * from the Video for Linux Two API Specification device to this instance in
	 * {@link #read(Buffer)}.
	 */
	private long v4l2_buffer;

	/**
	 * Initializes a new <tt>Video4Linux2Stream</tt> instance which is to have its
	...
	/**
	 * Gets the <tt>Format</tt> of this <tt>PullBufferStream</tt> as directly known
	 * by it.
	 *
	 * @return the <tt>Format</tt> of this <tt>PullBufferStream</tt> as directly
	 *         known by it or <tt>null</tt> if this <tt>PullBufferStream</tt> does
	 *         not directly know its <tt>Format</tt> and it relies on the
	 ...
	 *                     <tt>PullBufferStream</tt> into the specified
	 *                     <tt>buffer</tt>
	...
	/**
	 * Gets the <tt>Format</tt> of the media data captured by the Video for Linux
	 * Two API Specification device represented by the <tt>fd</tt> of this instance.
	 *
	 * @return the <tt>Format</tt> of the media data captured by the Video for Linux
				...
				try {
					if (Video4Linux2.ioctl(fd, Video4Linux2.VIDIOC_G_FMT, v4l2_format) != -1) {
						...
						if (FFmpeg.PIX_FMT_NONE != ffmpegPixFmt) {
							...
							format = new AVFrameFormat(new Dimension(width, height), Format.NOT_SPECIFIED, ffmpegPixFmt,
									pixelformat);
						}
					}
				} finally {
	...
	/**
	 * Unmaps the buffers through which the Video for Linux Two API Specification
	 * device provides the captured media data to this instance when
	 * {@link #requestbuffersMemory} is equal to <tt>V4L2_MEMORY_MMAP</tt> i.e.
	 * breaks the buffers' mappings between the driver's and the application's
	 * address spaces.
	 */
	private void munmap() {
		try {
	...
	/**
	 * Negotiates the input method with the Video for Linux Two API Specification
	 * device represented by the <tt>fd</tt> of this instance.
	 *
	 * @throws IOException if anything goes wrong while negotiating the input method
		...
		try {
			...
			try {
				for (int i = 0; i < requestbuffersCount; i++) {
					...
					long mmap = Video4Linux2.mmap(0, length, Video4Linux2.PROT_READ | Video4Linux2.PROT_WRITE,
	...
	/**
	 * Sets the file descriptor of the Video for Linux Two API Specification device
	 * which is to be read through this <tt>PullBufferStream</tt>.
	 *
	 * @param fd the file descriptor of the Video for Linux Two API Specification
	 *           device which is to be read through this <tt>PullBufferStream</tt>
	 * @throws IOException if anything goes wrong while setting the file descriptor
	 *                     of the Video for Linux Two API Specification device which
	 *                     is to be read through this <tt>PullBufferStream</tt>
	 */
	void setFd(int fd) throws IOException {
		if (this.fd != fd) {
			...
			/*
			 * Before a Video for Linux Two API Specification device can be read, an attempt
			 * to set its format must be made and its cropping must be reset. We can only
			 * learn about the format to be set from formatControl. But since this
			 * AbstractPullBufferStream exists already, formatControl will ask it about its
			 * format. So pretend that there is no device prior to asking formatControl
			 * about the format in order to get the format that has been set by the user.
			 */
			this.fd = -1;
			this.capabilities = 0;
	...
	/**
	 * Sets the crop of the Video for Linux Two API Specification device represented
	 * by the <tt>fd</tt> of this instance to its default value so that this
	 * <tt>PullBufferStream</tt> reads media data without cropping.
	 */
	...
	/**
	 * Sets the <tt>Format</tt> in which the Video for Linux Two API Specification
	 * device represented by the <tt>fd</tt> of this instance is to capture media
	 * data.
	 *
	 ...
	 * @throws IOException if anything goes wrong while setting the <tt>Format</tt>
	 *                     of the media data to be captured by the Video for Linux
	 *                     Two API Specification device represented by the
	 *                     <tt>fd</tt> of this instance
	 */
	 ...
	 * @throws IOException if anything goes wrong while setting the native format of
	 *                     the media data to be captured by the Video for Linux Two
	 *                     API Specification device represented by the <tt>fd</tt>
	 *                     of this instance
	 */
	private void setFdFormat(long v4l2_format, long fmtPix, Dimension size, int pixelformat) throws IOException {
		...
		if (Video4Linux2.ioctl(fd, Video4Linux2.VIDIOC_S_FMT, v4l2_format) == -1) {
			throw new IOException("ioctl: request= VIDIOC_S_FMT"
					+ ((size == null) ? "" : (", size= " + size.width + "x" + size.height)) + ", pixelformat= "
					+ pixelformat);
	...
	public void start() throws IOException {
		...
		/*
		 * we will start capture in read() method (i.e do the VIDIOC_STREAMON ioctl)
		 * because for some couple of fps/resolution the captured image will be weird
		 * (shift, not a JPEG for JPEG/MJPEG format, ...) if it is done here. Maybe it
		 * is due because sometime JMF do the sequence start/stop/start too quickly...
		 */
		startInRead = true;
	}
