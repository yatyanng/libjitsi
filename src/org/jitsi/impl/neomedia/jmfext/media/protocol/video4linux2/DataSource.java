...
public class DataSource extends AbstractVideoPullBufferCaptureDevice {
	...
	protected void doDisconnect() {
		try {
			/*
			 * Letting the Video4Linux2Stream know that the fd is going to be closed is
			 * necessary at least because AbstractPullBufferStream#close() is not
			 * guaranteed.
			 */
			synchronized (getStreamSyncRoot()) {
	 ...
	 *
	 * @return the name of the Video for Linux Two API Specification device which
	 *         represents the media source of this <tt>DataSource</tt>
	 */
	private String getDeviceName() {
		MediaLocator locator = getLocator();

		return ((locator != null) && DeviceSystem.LOCATOR_PROTOCOL_VIDEO4LINUX2.equalsIgnoreCase(locator.getProtocol()))
	 ...
	 *
	 * @param v4l2PixFmt the FFmpeg pixel format to get the matching Video for Linux
	 *                   Two API Specification pixel format of
	 * @return the Video for Linux Two API Specification pixel format matching the
	 *         specified FFmpeg format
	 */
	 ...
	 *
	 * @param ffmpegPixFmt the Video for Linux Two API Specification pixel format to
	 *                     get the matching FFmpeg pixel format of
	 * @return the FFmpeg pixel format matching the specified Video for Linux Two
	 *         API Specification pixel format
	 */
