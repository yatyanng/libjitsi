...
public class DataSource extends AbstractPullBufferCaptureDevice {
	...
	/**
	 * The indicator which determines whether <tt>DataSource</tt> instances apply
	 * audio volume levels on the audio data to be renderer or leave the task to
	 * PulseAudio.
	 */
...
//                    }
	...
	private class PulseAudioStream extends AbstractPullBufferStream<DataSource> {
		/**
		 * The <tt>PulseAudioSystem</tt> instance which provides the capture device and
		 * allows creating {@link #stream}.
		 */
		...
		/**
		 * The number of channels of audio data this <tt>PulseAudioStream</tt> is
		 * configured to input.
		 */
		private int channels;

		/**
		 * The indicator which determines whether {@link #stream}'s input is paused or
		 * resumed.
		 */
		...
		/**
		 * The <tt>GainControl</tt> which specifies the volume level to be applied to
		 * the audio data input through this <tt>PulseAudioStream</tt>.
		 */
		private final GainControl gainControl;

		/**
		 * The volume level specified by {@link #gainControl} which has been set on
		 * {@link #stream}.
		 */
		...
		/**
		 * Connects this <tt>PulseAudioStream</tt> to the configured source and prepares
		 * it to input audio data in the configured FMJ <tt>Format</tt>.
		 *
		 * @throws IOException if this <tt>PulseAudioStream</tt> fails to connect to the
		 *                     configured source
		 */
		...
		/**
		 * Connects this <tt>PulseAudioStream</tt> to the configured source and prepares
		 * it to input audio data in the configured FMJ <tt>Format</tt>. The method
		 ...
		 *
		 * @param b <tt>true</tt> to pause the input of audio data or <tt>false</tt> to
		 *          resume it
		 */
		...
		/**
		 * Disconnects this <tt>PulseAudioStream</tt> and its <tt>DataSource</tt> from
		...
		private void setStreamVolume(long stream, float level) {
			...
			long o = PA.context_set_source_output_volume(audioSystem.getContext(), PA.stream_get_index(stream), cvolume,
					null);
		...
		/**
		 * Pauses the input of audio data performed by {@link #stream}. The method
		 * executes with the assumption that the PulseAudio event loop object is locked
		 * by the executing thread.
		 */
	...
	/**
	 * Returns the name of the PulseAudio source that this <tt>DataSource</tt> is
	 * configured to input audio data from.
	 *
	 * @return the name of the PulseAudio source that this <tt>DataSource</tt> is
	 *         configured to input audio data from
	 */
