...
public class AudioCaptureClient {
	 ...
	 *                            retrieve the number of frames in the next data
	 *                            packet
	 * @return the number of frames in the next data packet in the capture endpoint
	 *         buffer associated with the specified <tt>iAudioCaptureClient</tt>. If
	 *         the function/method <tt>IAudioCaptureClient_GetNextPacketSize</tt>
	 *         throws an <tt>HResultException</tt>, return <tt>0</tt>.
	 */
	 ...
	 * associated <tt>IAudioCaptureClient</tt> by the instance and awaits to be read
	 * out of this instance via {@link #read(byte[], int, int)}.
	 */
	private byte[] available;

	/**
	 * The number of bytes in {@link #available} which represent valid audio data
	 * read from the associated <tt>IAudioCaptureClient</tt> by this instance.
	 */
	private int availableLength;

	/**
	 * The number of audio frames to be filled in a <tt>byte[]</tt> in an invocation
	 * of {@link #read(byte[], int, int)}. The method
	...
	/**
	 * The size/length in bytes of the <tt>byte[]</tt> to be filled in an invocation
	 ...
	 * instance, {@link #iAudioClient} and {@link #iAudioCaptureClient} is busy and,
	 * consequently, its state should not be modified. For example, the audio stream
	 * is busy during the execution of {@link #read(byte[], int, int)}.
	 */
	...
	/**
	 * The frame size in bytes of the audio data made available by this instance. It
	 * is the product of {@link #dstSampleSize} and {@link #dstChannels}.
	 */
	...
	/**
	 * The WASAPI <tt>IAudioCaptureClient</tt> obtained from {@link #iAudioClient}
	 * which enables this instance to read input data from the capture endpoint
	 * buffer.
	 */
	 ...
	 * <tt>AudioCaptureClient</tt> to create and initialize an audio stream between
	 * this instance and the audio engine of the associated audio endpoint device.
	 */
	...
	/**
	 * The indicator which reports whether audio samples have been read out of this
	 * instance since the indicator has been set to <tt>false</tt>. For example,
	 * determines whether audio samples have been read out of this instance since
	 * this instance has given control to its associated
	 * <tt>BufferTransferHandler</tt> (if any) and thus resolves a condition similar
	 * to a busy wait which arises if the associated <tt>BufferTransferHandler</tt>
	...
	/**
	 * The indicator which determines whether this <tt>AudioCaptureClient</tt> is
	...
	/**
	 * The <tt>BufferTransferHandler</tt> which is to be invoked when this instance
	 * has made audio data available to be read via {@link #read(byte[], int, int)}.
	 * {@link BufferTransferHandler#transferData(PushBufferStream)} will be called
	 * with a <tt>null</tt> argument because <tt>AudioCaptureClient</tt> does not
	 * implement <tt>PushBufferStream</tt> and has rather been refactored out of a
	 * <tt>PushBufferStream</tt> implementation (i.e. <tt>WASAPIStream</tt>).
	 */
	 ...
	 *                          endpoint device to be opened and read by the new
	 *                          instance
	 * @param dataFlow          the <tt>AudioSystem.DataFlow</tt> of the audio
	 *                          endpoint device identified by <tt>locator</tt>. If
	 *                          <tt>AudioSystem.DataFlow.PLAYBACK</tt> and
	 *                          <tt>streamFlags</tt> includes
	 *                          {@link WASAPI#AUDCLNT_STREAMFLAGS_LOOPBACK}, allows
	 *                          opening a render endpoint device in loopback mode
	 *                          and inputing the data that is being written on that
	 ...
	 *                          <tt>IAudioCaptureClient</tt> to be represented by
	 *                          the new instance may be initialized with a different
	 *                          <tt>AudioFormat</tt> in which case the new instance
	 *                          will automatically transcode the data input from the
	 *                          <tt>IAudioCaptureClient</tt> into the specified
	 *                          <tt>outFormat</tt>.
	 * @param transferHandler   the <tt>BufferTransferHandler</tt> to be invoked
	 *                          when the new instance has made data available to be
	 *                          read via {@link #read(byte[], int, int)}
	 * @throws Exception if the initialization of the new instance fails
	 */
	public AudioCaptureClient(WASAPISystem audioSystem, MediaLocator locator, AudioSystem.DataFlow dataFlow,
			int streamFlags, long hnsBufferDuration, AudioFormat outFormat, BufferTransferHandler transferHandler)
			throws Exception {
		...
		try {
			 ...
			 */
			long iAudioClient = audioSystem.initializeIAudioClient(locator, dataFlow, streamFlags, eventHandle,
					hnsBufferDuration, formats);

			if (iAudioClient == 0) {
				throw new ResourceUnavailableException("Failed to initialize IAudioClient" + " for MediaLocator "
						+ locator + " and AudioSystem.DataFlow " + dataFlow);
			}
			try {
				/*
	...
	/**
	 * Releases the resources acquired by this instance throughout its lifetime and
	 ...
	 *
	 * @param buffer the <tt>byte</tt> array into which the audio data read from the
	 *               internal buffer of this instance is to be written
	 ...
	 * @return the number of bytes read from the internal buffer of this instance
	 *         and written into the specified <tt>buffer</tt>
	 * @throws IOException if the reading from the internal buffer of this instance
	 *                     or writing into the specified <tt>buffer</tt> fails
	 */
	 ...
	 * associated <tt>IAudioCaptureClient</tt> by this instance and are available to
	 * be read out of this instance via {@link #read(byte[], int, int)}.
	 *
	 * @return the number of bytes of audio samples which have been read from the
	...
	/**
	 * Pops a specific number of bytes from {@link #available}. For example, because
	 * such a number of bytes have been read from <tt>available</tt> and written
	 * into a <tt>Buffer</tt>.
	 *
	 ...
	 * @param offset the offset in <tt>buffer</tt> at which the writing of the audio
	 *               data is to start
	 * @param length the maximum number of bytes in <tt>buffer</tt> starting at
	 *               <tt>offset</tt> to be written
	 * @return the number of bytes read from this instance and written into the
	 *         specified <tt>buffer</tt>
	 * @throws IOException if the reading from this instance or the writing into the
	...
	/**
	 * Reads audio data from this instance into a specific <tt>IMediaBuffer</tt> or
	 * a specific <tt>byte</tt> array.
	 *
	 * @param iMediaBuffer the <tt>IMediaBuffer</tt> into which the audio data read
	 *                     from this instance is to be written. If <tt>null</tt>,
	 *                     the writing occurs on <tt>buffer</tt> starting at
	 *                     <tt>offset</tt>. If non-<tt>null</tt>, <tt>buffer</tt>
	 *                     and <tt>offset</tt> are ignored.
	 * @param buffer       the <tt>byte</tt> array into which the audio data read
	 *                     from this instance is to be written if
	 *                     <tt>iMediaBuffer</tt> is <tt>null</tt>; otherwise,
	 *                     <tt>buffer</tt> and <tt>offset</tt> are ignored
	 * @param offset       the offset in <tt>buffer</tt> at which the writing of the
	 *                     audio data is to start if <tt>iMediaBuffer</tt> is
	 *                     <tt>null</tt>; otherwise, <tt>buffer</tt> and
	 *                     <tt>offset</tt> are ignored
	 * @param length       the maximum number of bytes to be read from this instance
	 *                     into the specified <tt>iMediaBuffer</tt> or the specified
	 ...
	 *
	 * @param iMediaBuffer the <tt>IMediaBuffer</tt> into which the audio data read
	 *                     from this instance is to be written
	 * @param length       the maximum number of bytes to read from this instance
	 *                     and write into the specified <tt>iMediaBuffer</tt>
	 * @return the number of bytes read from this instance and written into the
	 *         specified <tt>iMediaBuffer</tt>
	 * @throws IOException if the reading of audio data from this instance or the
	...
	/**
	 * Reads from {@link #iAudioCaptureClient} into {@link #available} and returns a
	 * non-<tt>null</tt> <tt>BufferTransferHandler</tt> if this instance is to push
	 * audio data.
	 *
	 * @return a <tt>BufferTransferHandler</tt> if this instance is to push audio
	 *         data; otherwise, <tt>null</tt>
	 */
	private BufferTransferHandler readInEventHandleCmd() {
		/*
		 * Determine the size in bytes of the next data packet in the capture endpoint
		 * buffer.
		 */
		...
		if (numFramesInNextPacket != 0) {
			...
			try {
				int read = IAudioCaptureClient_Read(iAudioCaptureClient, available, availableLength, toRead,
	...
	/**
	 * Starts the transfer of media from the <tt>IAudioCaptureClient</tt> identified
	 * by the <tt>MediaLocator</tt> with which this instance has been initialized.
	 *
	...
	/**
	 * Stops the transfer of media from the <tt>IAudioCaptureClient</tt> identified
	 * by the <tt>MediaLocator</tt> with which this instance has been initialized.
	 *
