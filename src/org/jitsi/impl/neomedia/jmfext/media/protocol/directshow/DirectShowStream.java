...
public class DirectShowStream extends AbstractPushBufferStream<DataSource> {
	 ...
	 * <p>
	 * <b>Note</b>: If the method returns <tt>true</tt>, an actual attempt to set
	 * the specified <tt>format</tt> on an specific <tt>DirectShowStream</tt>
	 * instance may still fail but that will be because the finer-grained properties
	 * of the <tt>format</tt> are not supported by that <tt>DirectShowStream</tt>
	 * instance.
	 * </p>
	 *
	 ...
	 *               instances
	 * @return <tt>true</tt> if the specified <tt>format</tt> appears to be suitable
	 *         for attempts to be set on <tt>DirectShowStream</tt> instance;
	...
	static boolean isSupportedFormat(Format format) {
		if (format instanceof AVFrameFormat) {
			...
			if (pixFmt != -1) {
				 ...
				 * connect-disconnect-connect sequence of the native capture device may reorder
				 * its formats in a different way. Consequently, in the absence of further calls
				 * to setFormat() by JMF, a crash may occur later (typically, during scaling)
	...
	/**
	 * The indicator which determines whether {@link #delegate} automatically drops
	 * late frames. If <tt>false</tt>, we have to drop them ourselves because
	 * DirectShow will buffer them all and the video will be late.
	 */
	...
	/**
	 * The <tt>DSCaptureDevice</tt> which identifies the DirectShow video capture
	 ...
	 * latter becomes is consumed. Thus prepares this <tt>DirectShowStream</tt> to
	 * provide the latest available frame and not wait for DirectShow to capture a
	 * new one.
	 */
	 ...
	 * <tt>DirectShowStream</tt> so that the call is not made in DirectShow and we
	 * can drop late frames when {@link #automaticallyDropsLateVideoFrames} is
	 * <tt>false</tt>.
	 */
	...
	/**
	 * Connects this <tt>SourceStream</tt> to the DirectShow video capture device
	 * identified by {@link #device}.
	 *
	 * @throws IOException if anything goes wrong while this <tt>SourceStream</tt>
	 *                     connects to the DirectShow video capture device
	 ...
	 *
	 * @throws IOException if anything goes wrong while this <tt>SourceStream</tt>
	 *                     disconnects from the DirectShow video capture device it
	 *                     has previously connected to during the execution of
	 *                     <tt>connect()</tt>
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * Overrides the super implementation to enable setting the <tt>Format</tt> of
	 * this <tt>DirectShowStream</tt> after the <tt>DataSource</tt> which provides
	 ...
	 * inside {@link #transferDataThread} so that the call is not made in DirectShow
	 * and we can drop late frames in the meantime.
	 */
	...
	private void SampleCB(long source, long ptr, int length) {
		...
		synchronized (dataSyncRoot) {
			...
			if (data != null) {
				int copiedLength = DSCaptureDevice.samplecopy(source, ptr, data.getPtr(), length);
				if (copiedLength == 0) {
					data.free();
					data = null;
				} else {
	 ...
	 * DirectShow video capture device this <tt>SourceStream</tt> is to capture data
	 * from.
	 *
	 ...
	 * instance.
	 *
	 * @param format the <tt>Format</tt> to set on the <tt>DSCaptureDevice</tt> of
	 *               this instance
	...
	private void setDeviceFormat(Format format) throws IOException {
		if (format == null)
			...
			if (size == null)
				...
				int hresult = device
						.setFormat(new DSFormat(size.width, size.height, avFrameFormat.getDeviceSystemPixFmt()));
	 ...
	 *
	 * @param hresult the <tt>HRESUlT</tt> to be described by the detail message of
