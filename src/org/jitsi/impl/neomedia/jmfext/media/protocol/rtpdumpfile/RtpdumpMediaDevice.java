...
public class RtpdumpMediaDevice {
	/**
	 * Create a new video <tt>MediaDevice</tt> instance which will read the rtpdump
	 * file located at <tt>filePath</tt>, and which will have the encoding format
	 * <tt>encodingConstant</tt>.
	 * 
	 * @param filePath            the location of the rtpdump file
	 * @param rtpEncodingConstant the format this <tt>MediaDevice</tt> will have.
	 *                            You can find the list of possible format in the
	 *                            class <tt>Constants</tt> of libjitsi (ex :
	 *                            Constants.VP8_RTP).
	 * @param format              the <tt>MediaFormat</tt> of the data contained in
	 *                            the payload of the recorded rtp packet in the
	 *                            rtpdump file.
	 * @return a <tt>MediaDevice</tt> that will read the rtpdump file given.
	 */
	public static MediaDevice createRtpdumpVideoMediaDevice(String filePath, String rtpEncodingConstant,
			MediaFormat format) {
		/*
		 * NOTE: The RtpdumpStream instance needs to know the RTP clock rate, to
		 * correctly interpret the RTP timestamps. We use the frameRate field of
		...
		return new MediaDeviceImpl(
				new CaptureDeviceInfo("Video rtpdump file", new MediaLocator("rtpdumpfile:" + filePath),
						new Format[] { new VideoFormat(rtpEncodingConstant, /* Encoding */
								null, /* Dimension */
	...
	/**
	 * Create a new audio <tt>MediaDevice</tt> instance which will read the rtpdump
	 * file located at <tt>filePath</tt>, and which will have the encoding format
	 * <tt>format</tt>.
	 *
	 * Note: for proper function, <tt>format</tt> has to implement correctly the
	 * <tt>computeDuration(long)</tt> method, because FMJ insists on using this to
	 * compute its own RTP timestamps.
	 *
	 * Note: The RtpdumpStream instance needs to know the RTP clock rate to
	 ...
	 *
	 * @param filePath the location of the rtpdump file
	 * @param format   the <tt>AudioFormat</tt> of the data contained in the payload
	...
	private static class MyAudioMediaDeviceImpl extends AudioMediaDeviceImpl {
		 ...
		 * <tt>AudioLevelEffect</tt>, because this causes media to be re-encoded (as
		 * <tt>AudioLevelEffect</tt> only works with raw audio formats).
		 */
		@Override
