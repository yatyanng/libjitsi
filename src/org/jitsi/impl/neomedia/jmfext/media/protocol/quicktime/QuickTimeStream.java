...
public class QuickTimeStream extends AbstractPushBufferStream<DataSource> {
	...
	/**
	 * The indicator which determines whether {@link #captureOutput} automatically
	 * drops late frames. If <tt>false</tt>, we have to drop them ourselves because
	 * QuickTime/QTKit will buffer them all and the video will be late.
	 */
	 ...
	 * media data captured by {@link #captureOutput} out of this instance through
	 * the <tt>Buffer</tt>s specified in its {@link #read(Buffer)}.
	 */
	...
	/**
	 * The captured media data to become the value of {@link #data} as soon as the
	 * latter becomes is consumed. Thus prepares this <tt>QuickTimeStream</tt> to
	 * provide the latest available frame and not wait for QuickTime/QTKit to
	 * capture a new one.
	 */
	 ...
	 * <tt>QuickTimeStream</tt> so that the call is not made in QuickTime/QTKit and
	 * we can drop late frames when {@link #automaticallyDropsLateVideoFrames} is
	 * <tt>false</tt>.
	 */
	...
	QuickTimeStream(DataSource dataSource, FormatControl formatControl) {
		...
		automaticallyDropsLateVideoFrames = captureOutput.setAutomaticallyDropsLateVideoFrames(true);
		captureOutput.setDelegate(new QTCaptureDecompressedVideoOutput.Delegate() {

			/**
			 * Notifies this <tt>Delegate</tt> that the <tt>QTCaptureOutput</tt> to which it
			 * is set has output a specific <tt>CVImageBuffer</tt> representing a video
			 * frame with a specific <tt>QTSampleBuffer</tt>.
			 *
			 * @param videoFrame   the <tt>CVImageBuffer</tt> which represents the output
			 *                     video frame
			 * @param sampleBuffer the <tt>QTSampleBuffer</tt> which represents additional
	 ...
	 * specific <tt>CVImageBuffer</tt> representing a video frame with a specific
	 * <tt>QTSampleBuffer</tt>.
	 *
	 * @param captureOutput the <tt>QTCaptureOutput</tt> which has output a video
	 *                      frame
	 * @param videoFrame    the <tt>CVImageBuffer</tt> which represents the output
	 *                      video frame
	 * @param sampleBuffer  the <tt>QTSampleBuffer</tt> which represents additional
	...
	private void captureOutputDidOutputVideoFrameWithSampleBuffer(QTCaptureOutput captureOutput,
		...
		synchronized (dataSyncRoot) {
			if (!automaticallyDropsLateVideoFrames && (data != null)) {
				...
				if (nextData != null) {
					nextData.setLength(pixelBuffer.getBytes(nextData.getPtr(), nextData.getCapacity()));
	...
	/**
	 * Gets the <tt>Format</tt> of this <tt>PushBufferStream</tt> as directly known
	 * by it.
	 *
	 * @return the <tt>Format</tt> of this <tt>PushBufferStream</tt> as directly
	 *         known by it or <tt>null</tt> if this <tt>PushBufferStream</tt> does
		...
		if (this.format == null) {
			...
			if (format == null)
				...
				if (videoFormat.getSize() != null)
					...
					Dimension defaultSize = NeomediaServiceUtils.getMediaServiceImpl().getDeviceConfiguration()
							.getVideoSize();

					format = videoFormat.intersects(
							new VideoFormat(/* encoding */ null, new Dimension(defaultSize.width, defaultSize.height),
									/* maxDataLength */ Format.NOT_SPECIFIED, /* dataType */ null,
		...
		if (pixelBufferAttributes != null) {
			...
			case CVPixelFormatType.kCVPixelFormatType_32ARGB:
				if (captureOutputFormat instanceof AVFrameFormat) {
					return new AVFrameFormat(((width == 0) && (height == 0) ? null : new Dimension(width, height)),
							/* frameRate */ Format.NOT_SPECIFIED, FFmpeg.PIX_FMT_ARGB,
							CVPixelFormatType.kCVPixelFormatType_32ARGB);
				} else {
					return new RGBFormat(((width == 0) && (height == 0) ? null : new Dimension(width, height)),
			...
			case CVPixelFormatType.kCVPixelFormatType_420YpCbCr8Planar:
				if ((width == 0) && (height == 0)) {
					...
					return new AVFrameFormat(new Dimension(width, height), /* frameRate */ Format.NOT_SPECIFIED,
							FFmpeg.PIX_FMT_YUV420P, CVPixelFormatType.kCVPixelFormatType_420YpCbCr8Planar);
				} else {
					int strideY = width;
					...
					return new YUVFormat(new Dimension(width, height), /* maxDataLength */ Format.NOT_SPECIFIED,
							Format.byteArray, /* frameRate */ Format.NOT_SPECIFIED, YUVFormat.YUV_420, strideY,
							strideUV, offsetY, offsetU, offsetV);
				}
			}
		}
		return null;
	...
	/**
	 * Gets the output frame rate of the <tt>QTCaptureDecompressedVideoOutput</tt>
	...
	/**
	 * Gets the <tt>Format</tt> of the media data made available by this
	 * <tt>PushBufferStream</tt> as indicated by a specific <tt>CVPixelBuffer</tt>.
	 *
	 * @param videoFrame the <tt>CVPixelBuffer</tt> which provides details about the
	...
	private Format getVideoFrameFormat(CVPixelBuffer videoFrame) {
		...
		if ((size == null) || ((size.width == 0) && (size.height == 0))) {
			format = format.intersects(
					new VideoFormat(/* encoding */ null, new Dimension(videoFrame.getWidth(), videoFrame.getHeight()),
							/* maxDataLength */ Format.NOT_SPECIFIED, /* dataType */ null,
							/* frameRate */ Format.NOT_SPECIFIED));
		}
		return format;
	 ...
	 *
	 * @param format the <tt>Format</tt> of the media data made available by this
	...
	private void setCaptureOutputFormat(Format format) {
		...
		if (VideoFormat.RGB.equalsIgnoreCase(encoding)) {
			...
			if (pixelBufferAttributes == null)
				pixelBufferAttributes = new NSMutableDictionary();
			pixelBufferAttributes.setIntForKey(CVPixelFormatType.kCVPixelFormatType_420YpCbCr8Planar,
					CVPixelBufferAttributeKey.kCVPixelBufferPixelFormatTypeKey);
		} else
	...
	/**
	 * Sets the output frame rate of the <tt>QTCaptureDecompressedVideoOutput</tt>
	 * represented by this <tt>QuickTimeStream</tt>.
	 *
	 * @param frameRate the output frame rate to be set on the
	 *                  <tt>QTCaptureDecompressedVideoOutput</tt> represented by
