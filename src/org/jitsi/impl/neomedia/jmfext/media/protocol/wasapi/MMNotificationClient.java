...
public class MMNotificationClient {
	/**
	 * The <tt>Logger</tt> used by the <tt>MMNotificationClient</tt> class to log
	 * debug information.
	 */
			...
			for (IMMNotificationClient pNotify : pNotifySet) {
				try {
					...
					if (t instanceof ThreadDeath)
						...
						logger.error("An IMMNotificationClient failed to normally" + " complete the handling of an"
	...
	/**
	 * Registers a specific <tt>IMMNotificationClient</tt> to be notified when an
	 * audio endpoint device is added or removed, when the state or properties of an
	 * endpoint device change, or when there is a change in the default role
	 * assigned to an endpoint device.
	 *
	...
	/**
	 * Deletes the registration of a specific <tt>IMMNotificationClient</tt> that
	 * the client registered in a previous call to
	 * {@link #RegisterEndpointNotificationCallback(IMMNotificationClient)}.
	 *
	 * @param pNotify the <tt>IMMNotificationClient</tt> to delete the registration
	 *                of
	 */
	public static void UnregisterEndpointNotificationCallback(IMMNotificationClient pNotify) {
		...
		synchronized (MMNotificationClient.class) {
			/*
			 * XXX The implementation bellow is hardly optimal because it consecutively
			 * employs the contains and remove Collection methods each of which performs a
			 * linear search for one and the same element in effectively the same set of
			 * elements. Anyway, the unregistering of IMMNotificationClients will very occur
			 * much less often than notification deliveries.
			 */
