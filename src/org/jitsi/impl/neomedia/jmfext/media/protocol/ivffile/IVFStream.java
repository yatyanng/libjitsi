...
public class IVFStream extends AbstractVideoPullBufferStream<DataSource> {
	...
	/**
	 * The timestamp of the frame (of its header) read during the last call to
	 * doRead. It's used to determine if enough time has passed during the last
	 * frame and the next frame to be returned.
	 */
	private long lastFrameTimestamp = 0;

	/**
	 * The VP8Frame that is filled by the VP8 frame and header read in the IVF file.
	 * We use the same VP8Frame object (set with the new frame each time of course)
	 * to avoid the allocation of a new VP8Frame each time.
	 */
	 ...
	 * 
	 * an IVF file doesn't have a well-defined "framerate" : each frame has its own
	 * timestamp. The two fields in the IVF file header, "rate" and "scale" are used
	 * to specify the "timebase", which controls how the frames' timestamps are to
	 * be interpreted. A frame timestamp of "X" means "X * scale / rate" seconds.
	 * 
	...
	/**
	 * Initializes a new <tt>IVFStream</tt> instance which is to have a specific
	 * <tt>FormatControl</tt>
	 *
	...
	protected void doRead(Buffer buffer) throws IOException {
		...
		/*
		 * We just check if the time that has passed since the last call to doRead took
		 * more or less milliseconds & nanoseconds than the number of milliseconds &
		 * nanoseconds between 2 consecutive frames (based on their timestamps and the
		 * the timebase). If it's less, we wait the remaining time (and if its more, we
		 * directly return).
		 */
		...
		/*
		 * Even if we loop the IVF file, there won't be any problem with millis when the
		 * last frame of the file was reached and the current frame is the first one :
		 * (frame.getTimestamp() - lastFrameTimestamp) will be negative so we won't
		 * sleep this time.
		 */
		if (nanos > 0) {
			try {
