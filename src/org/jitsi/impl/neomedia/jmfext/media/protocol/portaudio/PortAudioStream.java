...
public class PortAudioStream extends AbstractPullBufferStream<DataSource> {
	...
	/**
	 * The indicator which determines whether audio quality improvement is enabled
	 * for this <tt>PortAudioStream</tt> in accord with the preferences of the user.
	 */
	...
	/**
	 * The device identifier (the device UID, or if not available, the device name)
	 * of the PortAudio device read through this <tt>PullBufferStream</tt>.
	 */
	private String deviceID;

	/**
	 * The <tt>DiagnosticsControl</tt> implementation of this instance which allows
	 * the diagnosis of the functional health of <tt>Pa_ReadStream</tt>.
	 */
	private final DiagnosticsControl diagnosticsControl = new DiagnosticsControl() {
		/**
		 * {@inheritDoc}
		 *
		 * <tt>PortAudioStream</tt>'s <tt>DiagnosticsControl</tt> implementation does
		...
		public String toString() {
			...
			if (deviceID != null) {
				int index = Pa.getDeviceIndex(id, /* minInputChannels */ 1, /* minOutputChannels */ 0);
	...
	/**
	 * The <tt>GainControl</tt> through which the volume/gain of captured media is
	 * controlled.
	 */
	...
	private final UpdateAvailableDeviceListListener paUpdateAvailableDeviceListListener = new UpdateAvailableDeviceListListener() {
		/**
		 * The device ID (could be deviceUID or name but that is not really of concern
		 * to PortAudioStream) used before and after (if still available) the update.
		 */
	 ...
	 * <tt>FormatControl</tt>.
	 *
	 * @param dataSource              the <tt>DataSource</tt> which is creating the
	 *                                new instance so that it becomes one of its
	 *                                <tt>streams</tt>
	 * @param formatControl           the <tt>FormatControl</tt> which is to
	 *                                abstract the <tt>Format</tt>-related
	 *                                information of the new instance
	 * @param audioQualityImprovement <tt>true</tt> to enable audio quality
	 *                                improvement for the new instance in accord
	 *                                with the preferences of the user or
	 *                                <tt>false</tt> to completely disable audio
	 *                                quality improvement
	 */
	public PortAudioStream(DataSource dataSource, FormatControl formatControl, boolean audioQualityImprovement) {
		...
		/*
		 * XXX We will add a UpdateAvailableDeviceListListener and will not remove it
	...
	private void connect() throws IOException {
		int deviceIndex = Pa.getDeviceIndex(deviceID, /* minInputChannels */ 1, /* minOutputChannels */ 0);
		...
		try {
			...
			stream = Pa.OpenStream(inputParameters, 0 /* outputParameters */, sampleRate, framesPerBuffer,
		...
		/*
		 * Know the Format in which this PortAudioStream will output audio data so that
		 * it can report it without going through its DataSource.
		 */
	...
	public void read(Buffer buffer) throws IOException {
		...
		synchronized (this) {
			...
			if (message != null) {
				/*
				 * There is certainly a problem but it is other than a malfunction in
				 * Pa_ReadStream.
				 */
				if (readIsMalfunctioningSince != NEVER)
					setReadIsMalfunctioning(false);
		...
		try {
			...
			/*
			 * If a timeout has occurred in the method Pa.ReadStream, give the application a
			 * little time to allow it to possibly get its act together. The same treatment
			 * sounds appropriate on Windows as soon as the wmme host API starts reporting
			 * that no device driver is present.
			 */
			boolean yield = false;
	 ...
	 *                 PortAudioStream. This String contains the deviceUID, or if
	 *                 not available, the device name. If set to null, then there
	 *                 was no device used before the update.
	 *
	 * @throws IOException if input/output error occurred
	 */
	synchronized void setDeviceID(String deviceID) throws IOException {
		/*
		 * We should better not short-circuit because the deviceID may be the same but
		 * it eventually resolves to a deviceIndex and may have changed after
		 * hotplugging.
		 */
		...
		if (this.deviceID != null) {
			/*
			 * Just to be on the safe side, make sure #read(Buffer) is not currently
			 * executing.
			 */
			waitWhileStreamIsBusy();
			...
			if (stream != 0) {
				/*
				 * For the sake of completeness, attempt to stop this instance before
				 * disconnecting it.
				 */
				if (started) {
					try {
				...
				try {
					...
					if ((errorCode == Pa.paTimedOut) || (Pa.HostApiTypeId.paMME.equals(pae.getHostApiType())
					...
					if (closed) {
						 ...
						 * which it is supposed to output audio data the next time it is opened instead
