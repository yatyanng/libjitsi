...
public class MacCoreaudioStream extends AbstractPullBufferStream<DataSource> {
	 ...
	 * for this <tt>MacCoreaudioStream</tt> in accord with the preferences of the
	 * user.
	 */
	...
	/**
	 * The buffer which stores the outgoing data before sending them to the RTP
	 * stack.
	 */
	 ...
	 * of the MacCoreaudio device read through this <tt>PullBufferStream</tt>.
	 */
	...
	/**
	 * A list of already allocated and filled buffers, ready to be send throw the
	 * network.
	 */
	...
	/**
	 * A mutual eclusion used to avoid conflict when starting / stoping the stream
	 * for this stream;
	 */
	...
	private final UpdateAvailableDeviceListListener updateAvailableDeviceListListener = new UpdateAvailableDeviceListListener() {
		 ...
		 * to MacCoreaudioStream) used before and after (if still available) the update.
		 */
	...
	/**
	 * Initializes a new <tt>MacCoreaudioStream</tt> instance which is to have its
	...
	public MacCoreaudioStream(DataSource dataSource, FormatControl formatControl, boolean audioQualityImprovement) {
		...
		// WeakReference.
		AudioSystem2 audioSystem = (AudioSystem2) AudioSystem.getAudioSystem(AudioSystem.LOCATOR_PROTOCOL_MACCOREAUDIO);
	 ...
	 *                 MacCoreaudioStream. This String contains the deviceUID, or if
	...
	public void start() throws IOException {
		synchronized (startStopMutex) {
			if (stream == 0 && deviceUID != null) {
				...
				try {
					stream = MacCoreAudioDevice.startStream(deviceUID, this, (float) format.getSampleRate(),
							format.getChannels(), format.getSampleSizeInBits(), false,
