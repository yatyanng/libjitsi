...
public class ByteBufferPool {
	...
	/**
	 * Drains this <tt>ByteBufferPool</tt> i.e. frees the <tt>ByteBuffer</tt>s that
	 * it contains.
	 */
	...
	/**
	 * Gets a <tt>ByteBuffer</tt> out of this pool of <tt>ByteBuffer</tt>s which is
	 * capable to receiving at least <tt>capacity</tt> number of bytes.
	 *
	 * @param capacity the minimal number of bytes that the returned
	 *                 <tt>ByteBuffer</tt> is to be capable of receiving
	 * @return a <tt>ByteBuffer</tt> which is ready for writing captured media data
	 *         into and which is capable of receiving at least <tt>capacity</tt>
	 *         number of bytes
	 */
	...
	/**
	 * Implements a <tt>ByteBuffer</tt> which is pooled in a <tt>ByteBufferPool</tt>
	 * in order to reduce the numbers of allocations and deallocations of
	 * <tt>ByteBuffer</tt>s and their respective native memory.
	 */
	private static class PooledByteBuffer extends ByteBuffer {
		/**
		 * The <tt>ByteBufferPool</tt> in which this instance is pooled and in which it
		 * should returns upon {@link #free()}.
		 */
		...
		/**
		 * Invokes {@link ByteBuffer#free()} i.e. does not make any attempt to return
		 * this instance to the associated <tt>ByteBufferPool</tt> and frees the native
		 ...
		 * {@inheritDoc}
		 *
		 * Returns this <tt>ByteBuffer</tt> and, respectively, the native memory that it
		 * represents to the associated <tt>ByteBufferPool</tt>. If the
		 * <tt>ByteBufferPool</tt> has already been finalized by the garbage collector,
