...
public class DataSource extends AbstractPushBufferCaptureDevice {
	 ...
	 * mere purposes of ticking the clock which makes <tt>AudioMixer</tt> read media
	 * from its inputs, mix it, and write it to its outputs. The preferred value is
	 * <tt>true</tt> because it causes the <tt>AudioMixer</tt> to not push media
	 * unless at least one <tt>Channel</tt> is receiving actual media.
	 */
	...
	/**
	 * The optional {@link MediaLocator} remainder to indicate to {@code DataSource}
	 * and its associated {@code AudioSilenceStream} that
	 * {@code BufferTransferHandler.transferData(PushBufferStream)} is to not be
	 * invoked. If {@code true}, then the {@code DataSource} is a dummy suitable for
	 * scenarios in which an capature device is required but no audio samples from
	 * it are necessary such as negotiating signaling for audio but actually RTP
	 * translating other participants/peers' audio.
	 */
	 ...
	 * audio media.
	 */
	private static class AudioSilenceStream extends AbstractPushBufferStream<DataSource> implements Runnable {
		/**
		 ...
		 * {@code BufferTransferHandler.transferData(PushBufferStream)} and, thus, tick
		 * the media clock that it represents. If {@code false}, then it is a dummy
		 * suitable for scenarios in which an audio capture device is required but no
		 * audio samples from it are necessary such as negotiating signaling for audio
		 * but actually RTP translating other participants/peers' audio.
		 */
		 ...
		 *                      {@code BufferTransferHandler.transferData(PushBufferStream)}
		 *                      and, thus, tick the media clock that it represents;
