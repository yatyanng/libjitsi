...
abstract class AbstractBufferStream<T extends DataSource> extends AbstractControls implements SourceStream {
	/**
	 * The <tt>Logger</tt> used by the <tt>AbstractBufferStream</tt> class and its
	 * instances.
	 */
	private static final Logger logger = Logger.getLogger(AbstractBufferStream.class);

	/**
	 * The (default) <tt>ContentDescriptor</tt> of the <tt>AbstractBufferStream</tt>
	 * instances.
	 */
	private static final ContentDescriptor CONTENT_DESCRIPTOR = new ContentDescriptor(ContentDescriptor.RAW);

	/**
	 * The <tt>DataSource</tt> which has created this instance and which contains it
	 * as one of its <tt>streams</tt>.
	 */
	protected final T dataSource;

	/**
	 * The <tt>FormatControl</tt> which gives access to the <tt>Format</tt> of the
	 * media data provided by this <tt>SourceStream</tt> and which, optionally,
	 * allows setting it.
	 */
	...
	/**
	 * Gets the <tt>Format</tt> of this <tt>AbstractBufferStream</tt> as directly
	 * known by it. Allows extenders to override the <tt>Format</tt> known to the
	 * <tt>DataSource</tt> which created this instance and possibly provide more
	 * details on the currently set <tt>Format</tt>.
	 *
	 * @return the <tt>Format</tt> of this <tt>AbstractBufferStream</tt> as directly
	 *         known by it or <tt>null</tt> if this <tt>AbstractBufferStream</tt>
	 *         does not directly know its <tt>Format</tt> and it relies on the
	 *         <tt>DataSource</tt> which created it to report its <tt>Format</tt>
	 */
	...
	/**
	 * Determines whether the end of this <tt>SourceStream</tt> has been reached.
	 * The <tt>AbstractBufferStream</tt> implementation always returns
	 * <tt>false</tt>.
	 *
