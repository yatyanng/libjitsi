...
public class DataSource extends AbstractVideoPushBufferCaptureDevice {
	...
	/**
	 * The <tt>QTCaptureSession</tt> which captures from {@link #device} and pushes
	 * media data to the <tt>PushBufferStream</tt>s of this
	...
	/**
	 * The list of <tt>Format</tt>s to be reported by <tt>DataSource</tt> instances
	 ...
	 * provide a <tt>FrameRateControl</tt> which gets and sets the frame rate of the
	 * <tt>QTCaptureDecompressedVideoOutput</tt> represented by the
	...
	protected FrameRateControl createFrameRateControl() {
		return new FrameRateControlAdapter() {
			/**
			 * The output frame rate to be managed by this <tt>FrameRateControl</tt> when
			 * there is no <tt>QuickTimeStream</tt> to delegate to.
			 */
	 ...
	 * <tt>Format</tt>s with respect to video size. The implementation tries to come
	 * up with sane video sizes (for example, by looking for codecs which accept the
	 * encodings of the specified generic <tt>Format</tt>s and using their sizes if
	 * any).
	 *
	 * @param genericFormats the <tt>Format</tt>s from which more specific are to be
	 *                       derived
	...
	private static synchronized Format[] getSupportedFormats(Format[] genericFormats) {
		...
		for (Format genericFormat : genericFormats) {
			...
			if (genericVideoFormat.getSize() == null) {
				...
				Vector<String> codecs = PlugInManager.getPlugInList(new VideoFormat(genericVideoFormat.getEncoding()),
						...
						if (supportedInputFormat instanceof VideoFormat) {
							...
							if (size != null)
								specificFormats.add(genericFormat.intersects(
										new VideoFormat(null, size, Format.NOT_SPECIFIED, null, Format.NOT_SPECIFIED)));
						}
				}
			}
	 ...
	 * <tt>DataSource</tt>.
	 *
	 * @param device the <tt>QTCaptureDevice</tt> which represents the media source
	 *               of this <tt>DataSource</tt>
	 */
	 ...
	 * at the time of the attempt to set its <tt>Format</tt>.
	 *
	 ...
	 * <tt>DataSource</tt>.
	 *
	 * @param locator the <tt>MediaLocator</tt> which specifies the media source of
