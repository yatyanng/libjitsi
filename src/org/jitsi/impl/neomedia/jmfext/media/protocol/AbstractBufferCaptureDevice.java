 ...
 *        through which this <tt>AbstractBufferCaptureDevice</tt> is to give
...
public abstract class AbstractBufferCaptureDevice<AbstractBufferStreamT extends AbstractBufferStream<?>>
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>AbstractBufferCaptureDevice</tt> class
	 ...
	 * <tt>FormatControl</tt>s. Explicitly defined in order to reduce unnecessary
	 * allocations.
	 */
	 ...
	 * specified by the <tt>MediaLocator</tt> of this <tt>DataSource</tt> has been
	 * opened.
	 */
	...
	/**
	 * The array of <tt>FormatControl</tt> instances each one of which can be used
	 * before {@link #connect()} to get and set the capture <tt>Format</tt> of each
	 * one of the capture streams.
	 */
	 ...
	 * <p>
	 * Warning: Caution is advised when directly using the field and access to it is
	 * to be synchronized with synchronization root <tt>this</tt>.
	 * </p>
	 */
	 ...
	 *
	 * @throws IOException if anything goes wrong while opening the connection to
	 *                     the media source of this
	 *                     <tt>AbstractBufferCaptureDevice</tt>
	 */
	@Override
	...
	/**
	 * Creates a new <tt>FormatControl</tt> instance which is to be associated with
	 * a <tt>PushBufferStream</tt> at a specific zero-based index in the list of
	 * streams of this <tt>PushBufferDataSource</tt>. As the <tt>FormatControl</tt>s
	 * of a <tt>PushBufferDataSource</tt> can be requested before
	 * {@link #connect()}, its <tt>PushBufferStream</tt>s may not exist at the time
	 * of the request for the creation of the <tt>FormatControl</tt>.
	 *
	 * @param streamIndex the zero-based index of the <tt>PushBufferStream</tt> in
	 *                    the list of streams of this <tt>PushBufferDataSource</tt>
	 *                    which is to be associated with the new
	 *                    <tt>FormatControl</tt> instance
	 * @return a new <tt>FormatControl</tt> instance which is to be associated with
	 *         a <tt>PushBufferStream</tt> at the specified <tt>streamIndex</tt> in
	 *         the list of streams of this <tt>PushBufferDataSource</tt>
	 */
	protected FormatControl createFormatControl(final int streamIndex) {
		return new AbstractFormatControl() {
			/**
			 * The <tt>Format</tt> of this <tt>FormatControl</tt> and, respectively, of the
			 * media data of its owner.
			 */
			...
			/**
			 * Gets the <tt>Format</tt>s in which the owner of this <tt>FormatControl</tt>
			 ...
			 * <tt>Format</tt> in which the owner of this <tt>FormatControl</tt> is to
			 * provide media data.
			 *
			 ...
			 *         instance if <tt>format</tt> is supported by this instance and
	 ...
	 * <tt>AbstractBufferCaptureDevice</tt>. The <tt>Format</tt>-related information
	 * of the new instance is to be abstracted by a specific <tt>FormatControl</tt>.
	 *
	 * @param streamIndex   the zero-based index of the
	 *                      <tt>AbstractBufferStream</tt> in the list of streams of
	...
	/**
	 * Provides the default implementation of <tt>AbstractBufferCaptureDevice</tt>
	 * for {@link #doStart()}.
	 *
	...
	/**
	 * Provides the default implementation of <tt>AbstractBufferCaptureDevice</tt>
	 * for {@link #getFormat(int, Format)}.
	 *
	 * @param streamIndex the zero-based index of the <tt>AbstractBufferStream</tt>
	 ...
	 *                    <tt>streamIndex</tt>
	 * @return the <tt>Format</tt> to be reported by the <tt>FormatControl</tt> of
	 *         the <tt>AbstractBufferStream</tt> at the specified
	 *         <tt>streamIndex</tt> in the list of streams of this
	 *         <tt>AbstractBufferCaptureDevice</tt>
	...
	/**
	 * Provides the default implementation of <tt>AbstractBufferCaptureDevice</tt>
	 * for {@link #getSupportedFormats(int)}.
	 *
	 * @param streamIndex the zero-based index of the <tt>AbstractBufferStream</tt>
	 *                    for which the specified <tt>FormatControl</tt> is to
	 *                    report the list of supported <tt>Format</tt>s
	 ...
	 *         in the list of streams of this <tt>AbstractBufferCaptureDevice</tt>
	 */
	 ...
	 * sure that there will be no request to open a connection if the connection has
	 * already been opened.
	 *
	 * @throws IOException if anything goes wrong while opening the connection to
	 *                     the media source of this
	 ...
	 * <tt>AbstractBufferCaptureDevice</tt>. Allows extenders to override and be
	 * sure that there will be no request to close a connection if the connection
	 ...
	 * <tt>AbstractBufferCaptureDevice</tt>. Allows extenders to override and be
	 * sure that there will be no request to start the transfer of media data if it
	 ...
	 * <tt>AbstractBufferCaptureDevice</tt>. Allows extenders to override and be
	 * sure that there will be no request to stop the transfer of media data if it
	 ...
	 * describes it.
	 *
	 * @return the <tt>CaptureDeviceInfo</tt> of this <tt>CaptureDevice</tt> which
	 *         describes it
	 */
	@Override
	...
	/**
	 * Gets the <tt>CaptureDeviceInfo</tt> of a specific <tt>CaptureDevice</tt> by
	 * locating its registration in JMF using its <tt>MediaLocator</tt>.
	 *
	...
	public static CaptureDeviceInfo getCaptureDeviceInfo(DataSource captureDevice) {
		/*
		 * TODO The implemented search for the CaptureDeviceInfo of this CaptureDevice
		 * by looking for its MediaLocator is inefficient.
		 */
	 ...
	 *
	 * @param controlType the type of the control available for this instance to be
	 *                    retrieved
	 * @return an <tt>Object</tt> which represents the control of the specified type
	 *         available for this instance if such a control is indeed available;
	 ...
	 * <tt>AbstractBufferStream</tt> at a specific zero-based index in the list of
	 * streams of this <tt>AbstractBufferCaptureDevice</tt>. The
	 * <tt>AbstractBufferStream</tt> may not exist at the time of requesting its
	 * <tt>Format</tt>. Allows extenders to override the default behavior which is
	 * to report any last-known format or the first <tt>Format</tt> from the list of
	 * supported formats as defined in the JMF registration of this
	 * <tt>CaptureDevice</tt>.
	 *
	 * @param streamIndex the zero-based index of the <tt>AbstractBufferStream</tt>
	 *                    the <tt>Format</tt> of which is to be retrieved
	 ...
	 *                    <tt>streamIndex</tt>
	 * @return the <tt>Format</tt> to be reported by the <tt>FormatControl</tt> of
	 *         the <tt>PushBufferStream</tt> at the specified <tt>streamIndex</tt>
	...
	/**
	 * Gets an array of <tt>FormatControl</tt> instances each one of which can be
	 * used before {@link #connect()} to get and set the capture <tt>Format</tt> of
	 * each one of the capture streams.
	 *
	 * @return an array of <tt>FormatControl</tt> instances each one of which can be
	 *         used before {@link #connect()} to get and set the capture
	 ...
	 * streams of this <tt>PushBufferDataSource</tt>. The <tt>PushBufferStream</tt>
	 * may not exist at the time of requesting its <tt>Format</tt>.
	 *
	 * @param streamIndex the zero-based index of the <tt>PushBufferStream</tt> the
	...
	/**
	 * Gets an array of <tt>FrameRateControl</tt> instances which can be used to get
	 * and/or set the output frame rate of this
	 * <tt>AbstractBufferCaptureDevice</tt>.
	 *
	 * @return an array of <tt>FrameRateControl</tt> instances which can be used to
	 ...
	 * @return the <tt>Format</tt> to be reported by the <tt>FormatControl</tt> of
	 *         the <tt>PushBufferStream</tt> at the specified <tt>streamIndex</tt>
	 *         in the list of streams of this <tt>PushBufferStream</tt> or
	 *         <tt>null</tt> if the attempt to set the <tt>Format</tt> did not
	 *         success and any last-known <tt>Format</tt> is to be left in effect
	 */
	 ...
	 * zero-based index in the list of streams of this
	 * <tt>AbstractBufferCaptureDevice</tt>. The <tt>AbstractBufferStream</tt> does
	 * not exist at the time of the attempt to set its <tt>Format</tt>. Allows
	 * extenders to override the default behavior which is to not attempt to set the
	 * specified <tt>Format</tt> so that they can enable setting the <tt>Format</tt>
	 * prior to creating the <tt>AbstractBufferStream</tt>. If setting the
	 * <tt>Format</tt> of an existing <tt>AbstractBufferStream</tt> is desired,
	 * <tt>AbstractBufferStream#doSetFormat(Format)</tt> should be overridden
	 * instead.
	 *
	 * @param streamIndex the zero-based index of the <tt>AbstractBufferStream</tt>
	 ...
	 *         the <tt>AbstractBufferStream</tt> at the specified
	 *         <tt>streamIndex</tt> in the list of streams of this
	 *         <tt>AbstractBufferStream</tt> or <tt>null</tt> if the attempt to set
	 ...
	 * <tt>AbstractBufferCaptureDevice</tt> gives access to its media data.
	 *
	 * @return the internal array of <tt>AbstractBufferStream</tt>s through which
	 *         this <tt>AbstractBufferCaptureDevice</tt> gives access to its media
	 *         data
	 */
