...
public class DataSource extends AbstractVideoPushBufferCaptureDevice {
	 ...
	 */
	private static final int[] DS_TO_FFMPEG_PIX_FMTS = new int[] { DSFormat.RGB24, FFmpeg.PIX_FMT_RGB24, DSFormat.RGB32,
			FFmpeg.PIX_FMT_RGB32, DSFormat.ARGB32, FFmpeg.PIX_FMT_ARGB, DSFormat.YUY2, FFmpeg.PIX_FMT_YUYV422,
			DSFormat.MJPG, FFmpeg.PIX_FMT_YUVJ422P, DSFormat.UYVY, FFmpeg.PIX_FMT_UYVY422, DSFormat.Y411,
			FFmpeg.PIX_FMT_UYYVYY411, DSFormat.Y41P, FFmpeg.PIX_FMT_YUV411P, DSFormat.NV12, FFmpeg.PIX_FMT_NV12,
	 ...
	 *
	 * @param dsPixFmt the DirectShow pixel format to get the matching FFmpeg pixel
	 *                 format of
		...
		if (logger.isTraceEnabled()) {
			...
			if (device != null) {
				...
				for (DSFormat supportedFormat : supportedFormats) {
					logger.trace("width= " + supportedFormat.getWidth() + ", height= " + supportedFormat.getHeight()
	...
	protected void doConnect() throws IOException {
		...
		try {
			...
			if (!connected) {
				 ...
				 * is prepared to deal with a partially modified state and will restore it to
				 * its pristine form.
				 */
				doDisconnect();
			}
	...
	protected Format setFormat(int streamIndex, Format oldValue, Format newValue) {
		...
		return DirectShowStream.isSupportedFormat(newValue) ? newValue
