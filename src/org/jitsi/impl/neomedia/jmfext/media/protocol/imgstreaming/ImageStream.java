...
public class ImageStream extends AbstractVideoPullBufferStream<DataSource> {
	/**
	 * The <tt>Logger</tt> used by the <tt>ImageStream</tt> class and its instances
	 * for logging output.
	 */
	 ...
	 *
	 * @param buffer the <tt>Buffer</tt> this <tt>PullBufferStream</tt> is to read
	 *               into
	 * @throws IOException if an I/O error occurs while this
	 *                     <tt>PullBufferStream</tt> reads into the specified
	 *                     <tt>Buffer</tt>
	...
	protected void doRead(Buffer buffer) throws IOException {
		/*
		 * Determine the Format in which we're expected to output. We cannot rely on the
		 * Format always being specified in the Buffer because it is not its
		 * responsibility, the DataSource of this ImageStream knows the output Format.
		 */
	...
	public byte[] readScreen(byte[] output, Dimension dim) {
		...
		/*
		 * OK native grabber failed or is not available, try with AWT Robot and convert
		 * it to the right format
		 *
		 * Note that it is very memory consuming since memory are allocated to capture
		 * screen (via Robot) and then for converting to raw bytes Moreover support for
		 * multiple display has not yet been investigated
		 *
		 * Normally not of our supported platform (Windows (x86, x64), Linux (x86,
		 * x86-64), Mac OS X (i386, x86-64, ppc) and FreeBSD (x86, x86-64) should go
		 * here.
		 */
	...
	private ByteBuffer readScreenNative(Dimension dim) {
		...
		try {
			b = desktopInteract.captureScreen(displayIndex, x, y, dim.width, dim.height, data.getPtr(),
					data.getLength());
		} catch (Throwable t) {
