...
public abstract class AbstractPushBufferCaptureDevice extends PushBufferDataSource implements CaptureDevice {
	/**
	 * The <tt>AbstractBufferCaptureDevice</tt> which provides the implementation of
	...
	private final AbstractBufferCaptureDevice<AbstractPushBufferStream<?>> impl = new AbstractBufferCaptureDevice<AbstractPushBufferStream<?>>() {
		...
		/**
		 * Overrides {@link AbstractBufferCaptureDevice#getControls()} to add controls
		 * specific to this <tt>AbstractPushBufferCaptureDevice</tt>.
		 *
		 * {@inheritDoc}
		 */
		@Override
	...
	/**
	 * Initializes a new <tt>AbstractPushBufferCaptureDevice</tt> instance from a
	...
	/**
	 * Opens a connection to the media source specified by the <tt>MediaLocator</tt>
	 * of this <tt>DataSource</tt>.
	 *
	 * @throws IOException if anything goes wrong while opening the connection to
	 *                     the media source specified by the <tt>MediaLocator</tt>
	 *                     of this <tt>DataSource</tt>
	 */
	@Override
	 ...
	 * <tt>PushBufferDataSource</tt>. The <tt>Format</tt>-related information of the
	 * new instance is to be abstracted by a specific <tt>FormatControl</tt>.
	 *
	 ...
	 *         <tt>PushBufferDataSource</tt> and which has its
	 ...
	 * <tt>MediaLocator</tt> of this <tt>DataSource</tt>. If such a connection has
	 * not been opened, the call is ignored.
	 */
	@Override
	...
	/**
	 * Opens a connection to the media source specified by the <tt>MediaLocator</tt>
	 * of this <tt>DataSource</tt>. Allows extenders to override and be sure that
	 * there will be no request to open a connection if the connection has already
	 * been opened.
	 *
	 * @throws IOException if anything goes wrong while opening the connection to
	 *                     the media source specified by the <tt>MediaLocator</tt>
	 ...
	 * override and be sure that there will be no request to close a connection if
	 * the connection has not been opened yet.
	 */
	protected void doDisconnect() {
		/*
		 * While it is not clear whether the streams can be released upon disconnect,
		 * com.imb.media.protocol.SuperCloneableDataSource gets the streams of the
		 * DataSource it adapts (i.e. this DataSource when SourceCloneable support is to
		 * be created for it) before #connect(). Unfortunately, it means that it isn't
		 * clear when the streams are to be disposed.
		 */
	}

	/**
	 * Starts the transfer of media data from this <tt>DataSource</tt>. Allows
	 * extenders to override and be sure that there will be no request to start the
	 * transfer of media data if it has already been started.
	 *
	 ...
	 * extenders to override and be sure that there will be no request to stop the
	 * transfer of media data if it has not been started yet.
	 *
	...
	/**
	 * Implements {@link javax.media.protocol.DataSource#getControls()}. Gets the
	 ...
	 * may not exist at the time of requesting its <tt>Format</tt>. Allows extenders
	 * to override the default behavior which is to report any last-known format or
	 * the first <tt>Format</tt> from the list of supported formats as defined in
	 * the JMF registration of this <tt>CaptureDevice</tt>.
	 *
	 ...
	 * <tt>FormatControl</tt> as supported formats for a <tt>PushBufferStream</tt>
	 * at a specific zero-based index in the list of streams of this
	 * <tt>PushBufferDataSource</tt>.
	 *
	 * @param streamIndex the zero-based index of the <tt>PushBufferStream</tt> for
	 *                    which the specified <tt>FormatControl</tt> is to report
	 *                    the list of supported <tt>Format</tt>s
	 * @return an array of <tt>Format</tt>s to be reported by a
	 *         <tt>FormatControl</tt> as the supported formats for the
	 *         <tt>PushBufferStream</tt> at the specified <tt>streamIndex</tt> in
	 ...
	 * zero-based index in the list of streams of this
	 * <tt>PushBufferDataSource</tt>. The <tt>PushBufferStream</tt> does not exist
	 * at the time of the attempt to set its <tt>Format</tt>. Allows extenders to
	 * override the default behavior which is to not attempt to set the specified
	 * <tt>Format</tt> so that they can enable setting the <tt>Format</tt> prior to
	 * creating the <tt>PushBufferStream</tt>. If setting the <tt>Format</tt> of an
	 * existing <tt>PushBufferStream</tt> is desired,
	 * <tt>AbstractPushBufferStream#doSetFormat(Format)</tt> should be overridden
	 * instead.
	 *
	 * @param streamIndex the zero-based index of the <tt>PushBufferStream</tt> the
	...
	/**
	 * Gets the internal array of <tt>AbstractPushBufferStream</tt>s through which
	 * this <tt>AbstractPushBufferCaptureDevice</tt> gives access to its media data.
	 *
	 * @return the internal array of <tt>AbstractPushBufferStream</tt>s through
	 *         which this <tt>AbstractPushBufferCaptureDevice</tt> gives access to
	 *         its media data
	 */
