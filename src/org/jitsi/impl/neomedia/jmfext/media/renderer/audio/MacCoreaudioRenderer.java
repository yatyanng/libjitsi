...
public class MacCoreaudioRenderer extends AbstractAudioRenderer<MacCoreaudioSystem> {
	...
	/**
	 * A mutual exclusion used to avoid conflict when starting / stopping the stream
	 * for this renderer;
	 */
	...
	/**
	 * The list of the sample rates supported by <tt>MacCoreaudioRenderer</tt> as
	 * input.
	 */
	...
	/**
	 * The <tt>UpdateAvailableDeviceListListener</tt> which is to be notified before
	 * and after MacCoreaudio's native function <tt>UpdateAvailableDeviceList()</tt>
	 * is invoked. It will close {@link #stream} before the invocation in order to
	 * mitigate memory corruption afterwards and it will attempt to restore the
	 * state of this <tt>Renderer</tt> after the invocation.
	 */
	...
	/**
	 * Initializes a new <tt>MacCoreaudioRenderer</tt> instance which is to either
		...
		if (supportedInputFormats == null) {
			...
			if (deviceUID == null) {
				...
				for (Format supportedInputFormat : SUPPORTED_INPUT_FORMATS) {
					getSupportedInputFormats(supportedInputFormat, minOutputChannels, maxOutputChannels,
	...
	private void getSupportedInputFormats(Format format, int minOutputChannels, int maxOutputChannels,
			List<Format> supportedInputFormats) {
		AudioFormat audioFormat = (AudioFormat) format;
		int sampleSizeInBits = audioFormat.getSampleSizeInBits();
		double sampleRate = audioFormat.getSampleRate();
		boolean isEchoCancel = audioSystem.isEchoCancel();
		float minRate = MacCoreAudioDevice.getMinimalNominalSampleRate(deviceUID, true, isEchoCancel);
		float maxRate = MacCoreAudioDevice.getMaximalNominalSampleRate(deviceUID, true, isEchoCancel);

		for (int channels = minOutputChannels; channels <= maxOutputChannels; channels++) {
			if (sampleRate >= minRate && sampleRate <= maxRate) {
				supportedInputFormats.add(new AudioFormat(audioFormat.getEncoding(), sampleRate, sampleSizeInBits,
						channels, audioFormat.getEndian(), audioFormat.getSigned(), Format.NOT_SPECIFIED, // frameSizeInBits
	...
	/**
	 * Opens the MacCoreaudio device and output stream represented by this instance
	...
	public int process(Buffer buffer) {
		synchronized (startStopMutex) {
			if (stream != 0 && !isStopping) {
				...
				if (gainControl != null) {
					BasicVolumeControl.applyGain(gainControl, (byte[]) buffer.getData(), buffer.getOffset(),
							buffer.getLength());
				}
				...
				System.arraycopy(buffer.getData(), buffer.getOffset(), this.buffer, nbBufferData, length);
				nbBufferData += length;
			}
		}
	 ...
	 *                the MacCoreaudio device to be used by this instance for
	 *                rendering
	 */
	@Override
	 ...
	 * resources associated with this <tt>MacCoreaudioRenderer</tt> will begin being
	 * rendered.
	 */
	@Override
	public void start() {
		...
		synchronized (startStopMutex) {
			if (stream == 0 && deviceUID != null) {
				...
				try {
					 ...
					 */
					boolean isEchoCancel = AudioSystem.DataFlow.PLAYBACK.equals(dataFlow) && audioSystem.isEchoCancel();

					stream = MacCoreAudioDevice.startStream(deviceUID, this, (float) inputFormat.getSampleRate(),
							nbChannels, inputFormat.getSampleSizeInBits(), false,
							inputFormat.getEndian() == AudioFormat.BIG_ENDIAN, false, false, isEchoCancel);
				} finally {
					audioSystem.didOpenStream();
				}
			}
	...
	/**
	 * Writes the data received to the buffer give in arguments, which is provided
	 * by the CoreAudio library.
	 *
	...
	/**
	 * Increases the buffer length if necessary: if the new length is greater than
