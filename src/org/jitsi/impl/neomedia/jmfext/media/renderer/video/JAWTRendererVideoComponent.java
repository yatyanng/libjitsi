...
public class JAWTRendererVideoComponent extends Canvas {
	 ...
	 * <tt>Component</tt> to be delivered. For example, after the native counterpart
	 * has been able to acquire the native handle of the AWT <tt>Component</tt>, it
	 * may be able to determine when the native handle needs painting without
	 * waiting for AWT to call <tt>paint</tt> on the <tt>Component</tt>. In such a
	 * scenario, the native counterpart may indicate with <tt>false</tt> that it
	 * does not need further <tt>paint</tt> deliveries.
	 */
	 ...
	 * determines whether the native counterpart of this <tt>JAWTRenderer</tt> wants
	 * <tt>paint</tt> calls on its AWT <tt>Component</tt> to be delivered.
	 */
	@Override
			...
			if ((handle = getHandle()) != 0) {
				...
				try {
					...
					if (OSUtils.IS_MAC) {
						...
						setBounds(x - SwScale.MIN_SWS_SCALE_HEIGHT_OR_WIDTH, y - SwScale.MIN_SWS_SCALE_HEIGHT_OR_WIDTH,
	...
	/**
	 * Gets the handle of the native counterpart of the <tt>JAWTRenderer</tt> which
	 * paints in this <tt>AWTVideoComponent</tt>.
	 *
	...
	/**
	 * Overrides {@link Canvas#paint(Graphics)} to paint this <tt>Component</tt> in
	 * the native counterpart of its associated <tt>JAWTRenderer</tt>.
	 */
	@Override
