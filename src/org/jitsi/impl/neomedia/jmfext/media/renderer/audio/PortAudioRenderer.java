...
public class PortAudioRenderer extends AbstractAudioRenderer<PortAudioSystem> {
	 ...
	 * <tt>PortAudioRenderer</tt> without an intervening {@link #close()}. The state
	 * it represents is from the public point of view. The private point of view is
	 * represented by {@link #stream}.
	 */
	private static final byte FLAG_OPEN = 1;

	/**
	 * The flag which indicates that {@link #start()} has been called on a
	 * <tt>PortAudioRenderer</tt> without an intervening {@link #stop()}. The state
	 * it represents is from the public point of view. The private point of view is
	 * represented by {@link #started}.
	 */
	 ...
	 * input.
	 */
	private static final double[] SUPPORTED_INPUT_SAMPLE_RATES = new double[] { 8000, 11025, 16000, 22050, 32000, 44100,
			48000 };
	...
	static {
		...
		for (int i = 0; i < count; i++) {
			SUPPORTED_INPUT_FORMATS[i] = new AudioFormat(AudioFormat.LINEAR, SUPPORTED_INPUT_SAMPLE_RATES[i],
					16 /* sampleSizeInBits */, Format.NOT_SPECIFIED /* channels */, AudioFormat.LITTLE_ENDIAN,
	 ...
	 * always written, the number of the unwritten audio samples is always less than
	 * that.
	 */
	...
	/**
	 * The number of bytes to write to the native PortAudio stream represented by
	 * this instance with a single invocation. Based on {@link #framesPerBuffer}.
	 */
	private int bytesPerBuffer;

	/**
	 * The <tt>DiagnosticsControl</tt> implementation of this instance which allows
	 * the diagnosis of the functional health of <tt>Pa_WriteStream</tt>.
	 */
	private final DiagnosticsControl diagnosticsControl = new DiagnosticsControl() {
		/**
		 * {@inheritDoc}
		 *
		 * <tt>PortAudioRenderer</tt>'s <tt>DiagnosticsControl</tt> implementation does
		 * not provide its own user interface and always returns <tt>null</tt>.
		 */
		@Override
		...
		public String toString() {
			...
			if (locator != null) {
				...
				if (id != null) {
					int index = Pa.getDeviceIndex(id, /* minInputChannels */ 0, /* minOutputChannels */ 1);
	...
	/**
	 * The flags which represent certain state of this <tt>PortAudioRenderer</tt>.
	 * Acceptable values are among the <tt>FLAG_XXX</tt> constants defined by the
	 * <tt>PortAudioRenderer</tt> class. For example, {@link #FLAG_OPEN} indicates
	 * that from the public point of view {@link #open()} has been invoked on this
	 * <tt>Renderer</tt> without an intervening {@link #close()}.
	 */
	private byte flags = 0;

	/**
	 * The number of frames to write to the native PortAudio stream represented by
	 * this instance with a single invocation.
	 */
	 ...
	 * {@link #stream} before the invocation in order to mitigate memory corruption
	 * afterwards and it will attempt to restore the state of this <tt>Renderer</tt>
	...
	private final UpdateAvailableDeviceListListener paUpdateAvailableDeviceListListener = new UpdateAvailableDeviceListListener() {
		...
		public void didUpdateAvailableDeviceList() throws Exception {
			synchronized (PortAudioRenderer.this) {
				...
				/*
				 * PortAudioRenderer's field flags represents its open and started state from
				 * the public point of view. We will automatically open and start this Renderer
				 * i.e. we will be modifying the state from the private point of view only and,
		...
		public void willUpdateAvailableDeviceList() throws Exception {
			synchronized (PortAudioRenderer.this) {
				...
				/*
				 * PortAudioRenderer's field flags represents its open and started state from
				 * the public point of view. We will automatically close this Renderer i.e. we
	...
	public PortAudioRenderer(boolean enableVolumeControl) {
		...
		/*
		 * XXX We will add a PaUpdateAvailableDeviceListListener and will not remove it
		 * because we will rely on PortAudioSystem's use of WeakReference.
		 */
		if (audioSystem != null) {
	...
	public Format[] getSupportedInputFormats() {
		if (supportedInputFormats == null) {
			...
			if ((locator == null) || ((deviceID = DataSource.getDeviceID(locator)) == null) || (deviceID.length() == 0)
					|| ((deviceIndex = Pa.getDeviceIndex(deviceID, /* minInputChannels */ 0,
				...
				for (Format supportedInputFormat : SUPPORTED_INPUT_FORMATS) {
					getSupportedInputFormats(supportedInputFormat, deviceIndex, minOutputChannels, maxOutputChannels,
	...
	private void getSupportedInputFormats(Format format, int deviceIndex, int minOutputChannels, int maxOutputChannels,
			...
			if (outputParameters != 0) {
				try {
					if (Pa.IsFormatSupported(0, outputParameters, sampleRate)) {
						supportedInputFormats.add(new AudioFormat(audioFormat.getEncoding(), sampleRate,
	 ...
	 *
	 * @throws ResourceUnavailableException if the PortAudio device or output stream
	...
	public synchronized void open() throws ResourceUnavailableException {
		try {
			...
			/*
			 * Log the problem because FMJ may swallow it and thus make debugging harder
			 * than necessary.
			 */
	...
	private void doOpen() throws ResourceUnavailableException {
		if (stream == 0) {
			...
			int deviceIndex = Pa.getDeviceIndex(deviceID, /* minInputChannels */ 0, /* minOutputChannels */ 1);
			...
			try {
				...
				stream = Pa.OpenStream(0 /* inputParameters */, outputParameters, sampleRate, framesPerBuffer,
						Pa.STREAM_FLAGS_CLIP_OFF | Pa.STREAM_FLAGS_DITHER_OFF, null /* streamCallback */);
			} catch (PortAudioException paex) {
	 ...
	 *
	 * @param ev a <tt>PropertyChangeEvent</tt> which specifies details about the
	 *           change such as the name of the property and its old and new values
	 */
	@Override
	protected synchronized void playbackDevicePropertyChange(PropertyChangeEvent ev) {
		 ...
		 * change in the selection with respect to the playback device.
		 */

		waitWhileStreamIsBusy();

		/*
		 * From the public point of view, the state of this PortAudioRenderer remains
		 * the same.
		 */
	...
	public int process(Buffer buffer) {
		...
		try {
			...
			/*
			 * If a timeout has occurred in the method Pa.WriteStream, give the application
			 * a little time to allow it to possibly get its act together. The same
			 * treatment sounds appropriate on Windows as soon as the wmme host API starts
			 * reporting that no device driver is present.
			 */
			boolean yield = false;
	...
	private void process(byte[] buffer, int offset, int length) throws PortAudioException {
		...
		/*
		 * If there are audio samples left unwritten from a previous write, prepend them
		 * to the specified buffer. If it's possible to write them now, do it.
		 */
		if ((bufferLeft != null) && (bufferLeftLength > 0)) {
			int numberOfBytesInBufferLeftToBytesPerBuffer = bytesPerBuffer - bufferLeftLength;
			int numberOfBytesToCopyToBufferLeft = (numberOfBytesInBufferLeftToBytesPerBuffer < length)
	 ...
	 *
	 * @param locator a <tt>MediaLocator</tt> which specifies the device index of
	 ...
	 * resources associated with this <tt>PortAudioRenderer</tt> will begin being
	 * rendered.
	 */
	@Override
	...
	/**
	 * Waits on this instance while {@link #streamIsBusy} is equal to <tt>true</tt>
	 * i.e. until it becomes <tt>false</tt>. The method should only be called by a
	 * thread that is the owner of this object's monitor.
	 */
