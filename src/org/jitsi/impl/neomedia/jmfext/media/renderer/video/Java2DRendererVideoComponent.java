...
public class Java2DRendererVideoComponent extends Canvas {
	...
	public void paint(Graphics g) {
		if (this.mis != null) {
			g.drawImage(this.createImage(mis), 0, 0, getWidth(), getHeight(), null);
		}
	}
	...
	void process(Buffer buffer, Dimension size) {
		if (mis == null || !this.size.equals(size)) {
			this.size = size;
			mis = new MemoryImageSource(size.width, size.height, (int[]) buffer.getData(), buffer.getOffset(),
					size.width);
		} else {
			mis.newPixels((int[]) buffer.getData(), ColorModel.getRGBdefault(), buffer.getOffset(), size.width);
		}
