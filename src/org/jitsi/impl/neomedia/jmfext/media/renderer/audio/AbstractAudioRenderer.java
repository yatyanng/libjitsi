...
public abstract class AbstractAudioRenderer<T extends AudioSystem> extends AbstractRenderer<AudioFormat> {
	...
	/**
	 * The native byte order of the hardware upon which this Java virtual machine is
	 * running expressed in the <tt>endian</tt> term of {@link AudioFormat}.
	 */
	public static final int NATIVE_AUDIO_FORMAT_ENDIAN = (ByteOrder.nativeOrder() == ByteOrder.BIG_ENDIAN)
	...
	/**
	 * The <tt>GainControl</tt> through which the volume/gain of the media rendered
	 * by this instance is (to be) controlled.
	 */
	private GainControl gainControl;

	/**
	 * The <tt>MediaLocator</tt> which specifies the playback device to be used by
	 * this <tt>Renderer</tt>.
	 */
	 ...
	 * the properties of {@link #audioSystem}.
	 */
	 ...
	 *
	 * @param audioSystem the <tt>AudioSystem</tt> which is to provide the playback
	 ...
	 *                    notification or playback devices to be used by the new
	 *                    instance
	 ...
	 *                    provided by the specified <tt>audioSystem</tt> are to be
	 *                    used by the new instance. Must be either
	 *                    {@link AudioSystem.DataFlow#NOTIFY} or
	 *                    {@link AudioSystem.DataFlow#PLAYBACK}.
	 * @throws IllegalArgumentException if the specified <tt>dataFlow</tt> is
	 *                                  neither <tt>AudioSystem.DataFlow.NOTIFY</tt>
	...
	protected AbstractAudioRenderer(T audioSystem, AudioSystem.DataFlow dataFlow) {
		...
		if (AudioSystem.DataFlow.PLAYBACK.equals(dataFlow)) {
			/*
			 * XXX The Renderer implementations are probed for their supportedInputFormats
			 * during the initialization of MediaServiceImpl so the latter may not be
			 * available at this time. Which is not much of a problem given than the
			 * GainControl is of no interest during the probing of the
	 ...
	 *                        <tt>AudioSystem</tt> which is to provide the playback
	 *                        devices to be used by the new instance
	 */
	 ...
	 * @param locatorProtocol the protocol of the <tt>MediaLocator</tt>s of the
	 *                        <tt>CaptureDeviceInfo</tt> registered by the
	 *                        <tt>AudioSystem</tt> which is to provide the
	 *                        notification or playback devices to be used by the new
	 *                        instance
	 * @param dataFlow        the flow of the media data to be implemented by the
	 *                        new instance i.e. whether notification or playback
	 *                        devices provided by the specified <tt>audioSystem</tt>
	 *                        are to be used by the new instance. Must be either
	 *                        {@link AudioSystem.DataFlow#NOTIFY} or
	 *                        {@link AudioSystem.DataFlow#PLAYBACK}.
	 * @throws IllegalArgumentException if the specified <tt>dataFlow</tt> is
	 *                                  neither <tt>AudioSystem.DataFlow.NOTIFY</tt>
	 ...
	 *
	 * @return an array of <tt>Object</tt>s which represent the available controls
	 *         over this instance
	 */
	@Override
	...
	/**
	 * Gets the <tt>GainControl</tt>, if any, which controls the volume level of the
	 * audio (to be) played back by this <tt>Renderer</tt>.
	 *
	 * @return the <tt>GainControl</tt>, if any, which controls the volume level of
	...
	/**
	 * Gets the <tt>MediaLocator</tt> which specifies the playback device to be used
	 * by this <tt>Renderer</tt>.
	 *
	 * @return the <tt>MediaLocator</tt> which specifies the playback device to be
	...
	public Format[] getSupportedInputFormats() {
		/*
		 * XXX If the AudioSystem (class) associated with this Renderer (class and its
		 * instances) fails to initialize, the following may throw a
	...
	public void open() throws ResourceUnavailableException {
		 ...
		 * MediaLocator, it will use the default playback device (of its associated
		 * AudioSystem). In the case of using the default playback device, change the
		 * playback device used by this instance upon changes of the default playback
		 * device.
		 */
		if ((this.locator == null) && (audioSystem != null)) {
			/*
			 * We actually want to allow the user to switch the playback and/or notify
			 * device to none mid-stream in order to disable the playback. If an extender
			 * does not want to support that behavior, they will throw an exception and/or
			 * not call this implementation anyway.
			 */
	...
	/**
	 * Notifies this instance that the value of the property of {@link #audioSystem}
	 * which identifies the default notification or playback (as determined by
	 * {@link #dataFlow}) device has changed. The default implementation does
	 * nothing so extenders may safely not call back to their
	...
	/**
	 * Sets the <tt>MediaLocator</tt> which specifies the playback device to be used
	 * by this <tt>Renderer</tt>.
	 *
	 * @param locator the <tt>MediaLocator</tt> which specifies the playback device
