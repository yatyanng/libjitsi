...
public class PulseAudioRenderer extends AbstractAudioRenderer<PulseAudioSystem> {
	/**
	 * The human-readable <tt>PlugIn</tt> name of the <tt>PulseAudioRenderer</tt>
	 * instances.
	 */
	private static final String PLUGIN_NAME = "PulseAudio Renderer";

	/*
	 * FIXME The control of the volume through the native PulseAudio API has been
	 * reported to maximize the system-wide volume of the source with flat volumes
	 * i.e. https://java.net/jira/browse/JITSI-1050 (Pulseaudio changes volume to
	 * maximum values).
	 */
	 ...
	 */
	private static final Format[] SUPPORTED_INPUT_FORMATS = new Format[] { new AudioFormat(AudioFormat.LINEAR,
			Format.NOT_SPECIFIED /* sampleRate */, 16, Format.NOT_SPECIFIED /* channels */, AudioFormat.LITTLE_ENDIAN,
	...
	/**
	 * The indicator which determines whether {@link #stream}'s playback is paused
	 * or resumed.
	 */
	 ...
	 * <tt>PulseAudioRenderer</tt> to set the per-channel volume of {@link #stream}.
	 */
	...
	/**
	 * The level of the volume specified by the <tt>GainControl</tt> associated with
	 * this <tt>PulseAudioRenderer</tt> which has been applied to {@link #stream}.
	 */
	...
	/**
	 * The PulseAudio stream which performs the actual rendering of audio data for
	 * this <tt>PulseAudioRenderer</tt>.
	 */
	private long stream;

	/**
	 * The PulseAudio callback which notifies this <tt>PulseAudioRenderer</tt> that
	 * {@link #stream} requests audio data to play back.
	 */
	...
	public PulseAudioRenderer(String mediaRole) {
		super(PulseAudioSystem.getPulseAudioSystem(),
				((mediaRole == null) || PulseAudioSystem.MEDIA_ROLE_PHONE.equals(mediaRole))
	 ...
	 *
	 * @param gainControl the <tt>GainControl</tt> which specifies the volume to set
	 *                    on <tt>data</tt>
	 ...
	 *
	 * @param b <tt>true</tt> to pause the playback of audio data or <tt>false</tt>
	 *          to resume it
	 */
	...
	/**
	 * Returns the name of the sink this <tt>PulseAudioRenderer</tt> is configured
	 * to connect {@link #stream} to.
	 *
	 * @return the name of the sink this <tt>PulseAudioRenderer</tt> is configured
	 *         to connect {@link #stream} to
	 */
	...
	/**
	 * Opens this <tt>PulseAudioRenderer</tt> i.e. initializes the PulseAudio stream
	 * which is to play audio data back. The method executes with the assumption
	...
	private void openWithMainloopLock() throws ResourceUnavailableException {
		...
		if ((sampleRate == Format.NOT_SPECIFIED) && (MediaUtils.MAX_AUDIO_SAMPLE_RATE != Format.NOT_SPECIFIED))
	...
	protected void playbackDevicePropertyChange(PropertyChangeEvent ev) {
		/*
		 * FIXME Disabled due to freezes reported by Vincent Lucas and Kertesz Laszlo on
		 * the dev mailing list.
		 */
		audioSystem.lockMainloop();
		try {
			/*
			 * If the stream is not open, changes to the default playback device do not
			 * really concern this Renderer because it will pick them up when it gets open.
			 */
			boolean open = (stream != 0);

			if (open) {
				/*
				 * One and the same name of the sink that stream is connected to in the server
				 * may come from different MediaLocator instances.
				 */
	 ...
	 * {@link #stream}. The method executes with the assumption that the PulseAudio
	 * event loop object is locked by the executing thread.
	 *
	 * @param buffer the FMJ <tt>Buffer</tt> which specifies the audio data to play
	 *               back
	 * @return <tt>BUFFER_PROCESSED_OK</tt> if the specified <tt>buffer</tt> was
	 *         successfully sumbitted for playback through {@link #stream};
	 *         otherwise, <tt>BUFFER_PROCESSED_FAILED</tt>
	 */
	...
	/**
	 * Sets the volume of a specific PulseAudio <tt>stream</tt> to a level specified
	 * by the <tt>GainControl</tt> associated with this <tt>PulseAudioRenderer</tt>.
	 *
	...
	/**
	 * Pauses the playback of audio data performed by {@link #stream}. The method
	 * executes with the assumption that the PulseAudio event loop object is locked
