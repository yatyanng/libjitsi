...
public class WASAPIRenderer extends AbstractAudioRenderer<WASAPISystem> {
	...
	/**
	 * Attempts to initialize and open a new <tt>Codec</tt> to resample media data
	 * from a specific input <tt>AudioFormat</tt> into a specific output
	 ...
	 *                  output media data
	 * @return a new <tt>Codec</tt> which is able to resample media data from the
	 *         specified <tt>inFormat</tt> into the specified <tt>outFormat</tt> if
	...
	/**
	 * The indicator which determines whether the audio stream represented by this
	 * instance, {@link #iAudioClient} and {@link #iAudioRenderClient} is busy and,
	 * consequently, its state should not be modified. For example, the audio stream
	 * is busy during the execution of {@link #process(Buffer)}.
	 */
	...
	/**
	 * The value of {@link #devicePeriod} expressed in terms of numbers of frames
	 * (i.e. takes the sample rate into account).
	 */
	...
	/**
	 * The event handle that the system signals when an audio buffer is ready to be
	 * processed by the client.
	 */
	private long eventHandle;

	/**
	 * The <tt>Runnable</tt> which is scheduled by this <tt>WASAPIRenderer</tt> and
	 * executed by {@link #eventHandleExecutor} and waits for {@link #eventHandle}
	 * to be signaled.
	 */
	...
	/**
	 * The WASAPI <tt>IAudioRenderClient</tt> obtained from {@link #iAudioClient}
	 * which enables this <tt>Renderer</tt> to write output data to the rendering
	 * endpoint buffer.
	 */
	 ...
	 * property of this instance was equal to null when this <tt>Renderer</tt> was
	 * opened. Indicates that this <tt>Renderer</tt> should successfully process
	 * media data without actually rendering to any render endpoint device.
	 */
	...
	/**
	 * The number of channels of the audio signal output by {@link #resampler}. It
	 * may differ from {@link #dstChannels}.
	 */
	private int resamplerChannels;

	/**
	 * The data which has remained unwritten during earlier invocations of
	 * {@link #runInEventHandleCmd(Runnable)} because it represents frames which are
	 * few enough to be accepted on their own for writing by
	 ...
	 * {@link #resamplerChannels} and {@link #resamplerSampleSize} and cached in
	 * order to reduce calculations.
	 */
	private int resamplerFrameSize;

	/**
	 * The <tt>Buffer</tt> which provides the input to {@link #resampler}.
	 * Represents a unit of {@link #srcBuffer} to be processed in a single call to
	 * <tt>resampler</tt>.
	 */
	...
	/**
	 * The data which has remained unwritten during earlier invocations of
	 * {@link #process(Buffer)} because it represents frames which are few enough to
	 * be accepted on their own for writing by {@link #iAudioRenderClient}.
	 */
	private byte[] srcBuffer;

	/**
	 * The number of bytes in {@link #srcBuffer} which represent valid audio data to
	 * be written by {@link #iAudioRenderClient}.
	 */
	...
	/**
	 * The frame size in bytes with which this <tt>Renderer</tt> has been opened. It
	 * is the product of {@link #srcSampleSize} and {@link #srcChannels}.
	 */
	...
	/**
	 * The indicator which determines whether this <tt>Renderer</tt> is started i.e.
	 * there has been a successful invocation of {@link #start()} without an
	 * intervening invocation of {@link #stop()}.
	 */
	private boolean started;

	/**
	 * The time in milliseconds at which the writing to the render endpoint buffer
	 * has started malfunctioning. For example, {@link #srcBuffer} being full from
	 * the point of view of {@link #process(Buffer)} for an extended period of time
	 * may indicate abnormal functioning.
	 */
	private long writeIsMalfunctioningSince = DiagnosticsControl.NEVER;

	/**
	 * The maximum interval of time in milliseconds that the writing to the render
	 * endpoint buffer is allowed to be under suspicion that it is malfunctioning.
	 * If it remains under suspicion after the maximum interval of time has elapsed,
	 * the writing to the render endpoint buffer is to be considered malfunctioning
	 * for real.
	 */
	...
	/**
	 * Initializes a new <tt>WASAPIRenderer</tt> instance which is to either perform
	 * playback or sound a notification.
	 *
	 * @param dataFlow {@link AudioSystem.DataFlow#PLAYBACK} if the new instance is
	 ...
	 * playback or sound a notification.
	 *
	 * @param playback <tt>true</tt> if the new instance is to perform playback or
	 ...
	 * <tt>inputFormat</tt> with which an attempt is to be made to initialize a new
	...
	private AudioFormat[] getFormatsToInitializeIAudioClient() {
		...
		if (inputFormat == null)
			 ...
			 * non-integer quotient may result in audio glitches. Try to minimize the risks
			 * of having to use any of these two when unnecessary.
			 */
				...
				Collections.sort(formats, new Comparator<AudioFormat>() {
					...
					private int computeSampleRateDistance(AudioFormat af) {
						...
						if (min == 0)
							...
							/*
							 * Prefer AudioFormats which will cause upsampling to AudioFormats which will
							 * cause downsampling.
							 */
							if (downsample) {
								l = Short.MAX_VALUE - l;
	 ...
	 * {@inheritDoc}
	 *
	 * Overrides the super implementation to handle the case in which the user has
	 * selected "none" for the playback/notify device.
	 */
	@Override
	public Format[] getSupportedInputFormats() {
		if (getLocator() == null) {
			/*
			 * XXX We toyed with the idea of calculating a list of common Formats supported
			 * by all devices (of the dataFlow of this AbstractAudioRenderer, of course) but
			 * that turned out to be monstrous in code, inefficient at least in terms of
			 * garbage collection and with questionable suitability. The following approach
			 * will likely have a comparable suitability with better efficiency achieved
			...
			return WASAPISystem.getFormatsToInitializeIAudioClient(
					new AudioFormat(AudioFormat.LINEAR, sampleRate, sampleSizeInBits, channels,
							AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, /* frameSizeInBits */ Format.NOT_SPECIFIED,
	 ...
	 * @param offset        the offset in <tt>data</tt> at which the bytes of the
	 *                      audio samples to be written into the render endpoint
	 *                      buffer begin
	 * @param length        the number of the bytes in <tt>data</tt> beginning at
	 *                      <tt>offset</tt> of the audio samples to be written into
	 *                      the render endpoint buffer
	 * @param srcSampleSize the size in bytes of an audio sample in <tt>data</tt>
	 * @param srcChannels   the number of channels of the audio signal provided in
	 *                      <tt>data</tt>
	 * @return the number of bytes from <tt>data</tt> (starting at <tt>offset</tt>)
	 *         which have been written into the render endpoint buffer or <tt>0</tt>
	 *         upon <tt>HResultException</tt>
	 */
	private int maybeIAudioRenderClientWrite(byte[] data, int offset, int length, int srcSampleSize, int srcChannels) {
		int written;

		try {
			written = IAudioRenderClient_Write(iAudioRenderClient, data, offset, length, srcSampleSize, srcChannels,
	...
	private void maybeOpenResampler() {
		...
		if (outFormat.getChannels() != channels) {
			outFormat = new AudioFormat(outFormat.getEncoding(), outFormat.getSampleRate(),
	...
	/**
	 * Processes audio samples from {@link #srcBuffer} through {@link #resampler}
	 * i.e. resamples them in order to produce media data in {@link #resamplerData}
	 * to be written into the render endpoint buffer.
	 *
	 * @param numFramesRequested the number of audio frames in the units of
	 *                           {@link #dstFormat} requested by the rendering
	 *                           endpoint
	 */
	private void maybeResample(int numFramesRequested) {
		...
		/*
		 * Do not resample if there is enough resampled audio to satisfy the request of
		...
		if (outLength < numFramesRequested * resamplerFrameSize) {
			...
			if (outFrames > 0) {
				...
				if (inLength > 0) {
					...
					if (resampled > 0) {
						/*
						 * How many bytes have actually been consumed from the input during the sample
						 * rate conversion?
						 */
	...
	public synchronized void open() throws ResourceUnavailableException {
		...
		try {
			...
			if (locatorIsNull = (locator == null)) {
				...
				try {
					long iAudioClient = audioSystem.initializeIAudioClient(locator, dataFlow, /* streamFlags */ 0,
							eventHandle, WASAPISystem.DEFAULT_BUFFER_DURATION, formats);
					...
					try {
						...
						if (iAudioRenderClient == 0) {
							throw new ResourceUnavailableException(
									"IAudioClient_GetService" + "(IID_IAudioRenderClient)");
						}
						try {
							...
							/*
							 * The remainder/residue in frames of IAudioRenderClient_Write cannot be more
							 * than the maximum capacity of the endpoint buffer.
							 */
							...
							if (resampler == null) {
								...
								/*
								 * The units of srcBuffer are based on srcFormat, the units of numBufferFrames
								 * are based on dstFormat.
								 */
								int srcSampleRate = (int) srcFormat.getSampleRate();
	...
	protected synchronized void playbackDevicePropertyChange(PropertyChangeEvent ev) {
		/*
		 * Stop, close, re-open and re-start this Renderer (performing whichever of
		 * these in order to bring it into the same state) in order to reflect the
		 * change in the selection with respect to the playback or notify device.
		 */
	...
	/**
	 * Pops a specific number of bytes from {@link #srcBuffer}. For example, because
	 * such a number of bytes have been read from <tt>srcBuffer</tt> and written
	 * into the rendering endpoint buffer.
	 *
	...
	public int process(Buffer buffer) {
		...
		try {
			...
			if (ret != BUFFER_PROCESSED_FAILED) {
				...
				if (numFramesRequested == 0) {
					if (eventHandle == 0) {
						/*
						 * There is NO available space in the rendering endpoint buffer into which this
						 * Renderer can write data.
						 */
						ret |= INPUT_BUFFER_NOT_CONSUMED;
						sleep = devicePeriod;
						/*
						 * The writing to the render endpoint buffer may or may not be malfunctioning,
						 * it depends on the interval of time that the state remains unchanged.
						 */
						if (writeIsMalfunctioningSince == DiagnosticsControl.NEVER)
							setWriteIsMalfunctioning(true);
					} else {
						/*
						 * The process method will write into srcBuffer, the runInEventHandleCmd will
						 * read from srcBuffer and write into the rendering endpoint buffer.
						 */
						...
						if (toCopy > 0) {
							...
							/*
							 * Writing from the input Buffer into srcBuffer has occurred so it does not look
							 * like the writing to the render endpoint buffer is malfunctioning.
							 */
							if (writeIsMalfunctioningSince != DiagnosticsControl.NEVER)
							...
							sleep = devicePeriod;
							/*
							 * No writing from the input Buffer into srcBuffer has occurred so it is
							 * possible that the writing to the render endpoint buffer is malfunctioning.
							 */
							if (writeIsMalfunctioningSince == DiagnosticsControl.NEVER)
								setWriteIsMalfunctioning(true);
						}
					...
					if (srcBufferLength > 0) {
						...
						/*
						 * There is no remainder/residue from earlier invocations of the method. This
						 * Renderer will feed iAudioRenderClient from data.
						 */
						effectiveData = data;
					...
					if ((toWrite / srcFrameSize) == 0)
						...
						try {
							written = IAudioRenderClient_Write(iAudioRenderClient, effectiveData, effectiveOffset,
									toWrite, srcSampleSize, srcChannels, dstSampleSize, dstChannels);
						} catch (HResultException hre) {
							written = 0;
							ret = BUFFER_PROCESSED_FAILED;
							logger.error("IAudioRenderClient_Write", hre);
					...
					if (ret != BUFFER_PROCESSED_FAILED) {
						if (effectiveData == data) {
							...
							if (written == 0) {
								/*
								 * The available number of frames appear to be too few for IAudioRenderClient to
								 * accept. They will have to be prepended to the next input Buffer.
								 */
				...
				if (((ret & INPUT_BUFFER_NOT_CONSUMED) == INPUT_BUFFER_NOT_CONSUMED)
					...
					if (writeIsMalfunctioningDuration > writeIsMalfunctioningTimeout) {
						/*
						 * The writing to the render endpoint buffer has taken too long so whatever is
						 * in srcBuffer is surely out-of-date.
						 */
						srcBufferLength = 0;
						ret = BUFFER_PROCESSED_FAILED;
		...
		/*
		 * If there was no available space in the rendering endpoint buffer, we will
		 * want to wait a bit for such space to be made available.
		 */
		if (((ret & INPUT_BUFFER_NOT_CONSUMED) == INPUT_BUFFER_NOT_CONSUMED) && (sleep > 0)) {
			...
			synchronized (this) {
				/*
				 * Spurious wake-ups should not be a big issue here. While this Renderer may
				 * check for available space in the rendering endpoint buffer more often than
				 * practically necessary (which may very well classify as a case of performance
				 * loss), the ability to unblock this Renderer is considered more important.
				 */
				try {
					wait(sleep);
	 ...
	 *                       wait for <tt>eventHandle</tt> to be signaled and in
	 *                       whose associated thread the method is invoked
	 */
		...
		try {
			...
			do {
				...
				try {
					...
					/*
					 * If there is no available space in the rendering endpoint buffer, wait for the
					 * system to signal when an audio buffer is ready to be processed by the client.
					 */
					if (numFramesRequested > 0) {
						byte[] buf;
						int bufChannels;
	 ...
	 * malfunctioning. Keeps track of the time at which the malfunction has started.
	 *
	 * @param writeIsMalfunctioning <tt>true</tt> if the writing to the render
	 *                              endpoint buffer is (believed to be)
	...
	public synchronized void start() {
		if (iAudioClient == 0) {
			...
			if (srcBuffer != null) {
				 ...
				 * make room for silence at the beginning.
				 */
	...
	/**
	 * Gets a human-readable representation of a specific <tt>MediaLocator</tt> for
	 ...
	 * @return a human-readable representation of the specified <tt>locator</tt> for
	 *         the purposes of testing/debugging
	 */
	private String toString(MediaLocator locator) {
		...
		if (locator == null)
			...
			s = null;
			/*
			 * Try to not throw any exceptions because the purpose is to produce at least
			 * some identification of the specified MediaLocator even if not the most
			 * complete.
			 */
			try {
