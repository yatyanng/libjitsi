...
public class JAWTRenderer extends AbstractRenderer<VideoFormat> implements VideoRenderer {
	 ...
	 * <tt>JAWTRenderer</tt>s before video frames with actual sizes are processed.
	 * Introduced to mitigate multiple failures to realize the actual video frame
	 * size and/or to properly scale the visual/video <tt>Component</tt>s.
	 */
	private static final int DEFAULT_COMPONENT_HEIGHT_OR_WIDTH = 16;

	/**
	 * The <tt>Logger</tt> used by the <tt>JAWTRenderer</tt> class and its instances
	 * for logging output.
	 */
	 ...
	 */
	private static final Format[] SUPPORTED_INPUT_FORMATS = new Format[] { OSUtils.IS_LINUX
			? new YUVFormat(null /* size */, Format.NOT_SPECIFIED /* maxDataLength */, Format.intArray,
					Format.NOT_SPECIFIED /* frameRate */, YUVFormat.YUV_420, Format.NOT_SPECIFIED /* strideY */,
					Format.NOT_SPECIFIED /* strideUV */, Format.NOT_SPECIFIED /* offsetY */,
					Format.NOT_SPECIFIED /* offsetU */, Format.NOT_SPECIFIED /* offsetV */)
			: OSUtils.IS_ANDROID
					? new RGBFormat(null, Format.NOT_SPECIFIED, Format.intArray, Format.NOT_SPECIFIED, 32, 0x000000ff,
							0x0000ff00, 0x00ff0000)
					: new RGBFormat(null, Format.NOT_SPECIFIED, Format.intArray, Format.NOT_SPECIFIED, 32, 0x00ff0000,
							0x0000ff00, 0x000000ff) };

	static {
		JNIUtils.loadLibrary("jnawtrenderer", JAWTRenderer.class.getClassLoader());
	}
	 ...
	 * handle as returned by {@link #open(Component)} and rendering into a specific
	 * AWT <tt>Component</tt>. Releases the resources which the specified native
	 * counterpart has retained during its execution and its handle is considered to
	 * be invalid afterwards.
	 *
	 * @param handle    the handle to the native counterpart of a
	 *                  <tt>JAWTRenderer</tt> as returned by
	 *                  {@link #open(Component)} which is to be closed
	 * @param component the AWT <tt>Component</tt> into which the
	 *                  <tt>JAWTRenderer</tt> and its native counterpart are
	 *                  drawing. The platform-specific info of <tt>component</tt> is
	 *                  not guaranteed to be valid.
	 */
	private static native void close(long handle, Component component);

	/**
	 * Opens a handle to a native counterpart of a <tt>JAWTRenderer</tt> which is to
	 * draw into a specific AWT <tt>Component</tt>.
	 *
	 * @param component the AWT <tt>Component</tt> into which a
	 *                  <tt>JAWTRenderer</tt> and the native counterpart to be
	 *                  opened are to draw. The platform-specific info of
	 *                  <tt>component</tt> is not guaranteed to be valid.
	 * @return a handle to a native counterpart of a <tt>JAWTRenderer</tt> which is
	 *         to draw into the specified AWT <tt>Component</tt>
	 ...
	 *
	 * @param handle    the handle to the native counterpart of a
	 *                  <tt>JAWTRenderer</tt> which is to draw into the specified
	 *                  AWT <tt>Component</tt>
	 * @param component the AWT <tt>Component</tt> into which the
	 *                  <tt>JAWTRenderer</tt> and its native counterpart specified
	 *                  by <tt>handle</tt> are to draw. The platform-specific info
	 *                  of <tt>component</tt> is guaranteed to be valid only during
	 *                  the execution of <tt>paint</tt>.
	 * @param g         the <tt>Graphics</tt> context into which the drawing is to
	 *                  be performed
	 * @param zOrder
	 * @return <tt>true</tt> if the native counterpart of a <tt>JAWTRenderer</tt>
	 *         wants to continue receiving the <tt>paint</tt> calls on the AWT
	 *         <tt>Component</tt>; otherwise, false. For example, after the native
	...
	/**
	 * Processes the data provided in a specific <tt>int</tt> array with a specific
	 * offset and length and renders it to the output device represented by a
	 * <tt>JAWTRenderer</tt> specified by the handle to it native counterpart.
	 *
	 * @param handle    the handle to the native counterpart of a
	 *                  <tt>JAWTRenderer</tt> to process the specified data and
	 *                  render it
	 * @param component the <tt>AWT</tt> component into which the specified
	 *                  <tt>JAWTRenderer</tt> and its native counterpart draw
	 * @param data      an <tt>int</tt> array which contains the data to be
	 *                  processed and rendered
	 * @param offset    the index in <tt>data</tt> at which the data to be processed
	 *                  and rendered starts
	 ...
	 */
	static native boolean process(long handle, Component component, int[] data, int offset, int length, int width,
			int height);
	...
	/**
	 * Closes this <tt>PlugIn</tt> and releases the resources it has retained during
	 * its execution. No more data will be accepted by this <tt>PlugIn</tt>
	 * afterwards. A closed <tt>PlugIn</tt> can be reinstated by calling
	 * <tt>open</tt> again.
	 */
	@Override
	 ...
	 *
	 * @return the region in the component of this <tt>VideoRenderer</tt> where the
	 *         video is rendered; <tt>null</tt> if the entire component is used
	 */
	@Override
	...
	/**
	 * Gets the <tt>Object</tt> which synchronizes the access to the handle to the
	 * native counterpart of this <tt>JAWTRenderer</tt>.
	 *
	 * @return the <tt>Object</tt> which synchronizes the access to the handle to
	...
	/**
	 * Processes the data provided in a specific <tt>Buffer</tt> and renders it to
	 * the output device represented by this <tt>Renderer</tt>.
	 *
	...
	public synchronized int process(Buffer buffer) {
		...
		if ((format != null) && (format != this.inputFormat) && !format.equals(this.inputFormat)
		...
		if (handle == 0)
			...
			if ((size.width >= SwScale.MIN_SWS_SCALE_HEIGHT_OR_WIDTH)
				...
				boolean repaint = process(handle, component, (int[]) buffer.getData(), buffer.getOffset(), bufferLength,
	...
	/**
	 * Sets properties of the AWT <tt>Component</tt> of this <tt>Renderer</tt> which
	 * depend on the properties of the <tt>inputFormat</tt> of this
	 * <tt>Renderer</tt>. Makes sure that the procedure is executed on the AWT event
	 * dispatching thread because an AWT <tt>Component</tt>'s properties (such as
	 * <tt>preferredSize</tt>) should be accessed in the AWT event dispatching
	 * thread.
	 */
		...
		if ((component != null) && (width > 0) && (height > 0)) {
			...
			if (component.isPreferredSizeSet() && (component.getParent() == null)) {
				...
				if ((size.width < 1) || (size.height < 1)
						|| !VideoLayout.areAspectRatiosEqual(size, prefSize.width, prefSize.height)) {
	...
	/**
	 * Sets the AWT <tt>Component</tt> into which this <tt>VideoRenderer</tt> is to
	 * draw. <tt>JAWTRenderer</tt> cannot draw into any other AWT <tt>Component</tt>
	 * but its own so it always returns <tt>false</tt>.
	 *
	 ...
	 *         <tt>component</tt> as the AWT <tt>Component</tt> into which it is to
	 *         draw; <tt>false</tt>, otherwise
	 */
	@Override
	 ...
	 *         this <tt>Renderer</tt>. Typically, it is the supported input
	 *         <tt>Format</tt> which most closely matches the specified
	 *         <tt>Format</tt>.
	 */
	@Override
