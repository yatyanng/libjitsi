...
public class AudioMediaStreamImpl extends MediaStreamImpl implements AudioMediaStream, PropertyChangeListener {
	...
	private static final AudioFormat[] CUSTOM_CODEC_FORMATS = new AudioFormat[] {
			 ...
			 */
			new AudioFormat(Constants.ALAW_RTP, 8000, 8, 1, Format.NOT_SPECIFIED, AudioFormat.SIGNED),
			new AudioFormat(Constants.G722_RTP, 8000, Format.NOT_SPECIFIED /* sampleSizeInBits */, 1) };

	/**
	 * The <tt>Logger</tt> used by the <tt>AudioMediaStreamImpl</tt> class and its
	 ...
	 * especially for those generated after the <tt>AudioSystem</tt> has changed.
	 */
	...
	/**
	 * The listener which has been set on this instance to get notified of changes
	 * in the levels of the audio that the local peer/user is sending to the remote
	 * peer(s).
	 */
	 ...
	 * (level) of the audio received in/by this <tt>AudioMediaStream</tt> and played
	 * back.
	 */
	...
	/**
	 * The listener which has been set on this instance to get notified of changes
	 * in the levels of the audios that the local peer/user is receiving from the
	 * remote peer(s).
	 */
	...
	/**
	 * Initializes a new <tt>AudioMediaStreamImpl</tt> instance which will use the
	 * specified <tt>MediaDevice</tt> for both capture and playback of audio
	 ...
	 *                    both capture and playback of audio exchanged via the
	...
	/**
	 * Adds a <tt>DTMFListener</tt> to this <tt>AudioMediaStream</tt> which is to
	 * receive notifications when the remote party starts sending DTMF tones to us.
	 *
	 * @param listener the <tt>DTMFListener</tt> to register for notifications about
	 ...
	 * rather than in the super class is that CSRC levels only make sense for audio
	 * streams so we don't want them enabled in any other type.
	 *
	 * @param extensionID  the ID assigned to <tt>rtpExtension</tt> for the lifetime
	 *                     of this stream.
	...
	/**
	 * Delivers the <tt>audioLevels</tt> map to whoever is interested. This method
	 * is meant for use primarily by the transform engine handling incoming RTP
	 * packets (currently <tt>CsrcTransformEngine</tt>).
	 *
	 * @param audioLevels an array mapping CSRC IDs to audio levels in consecutive
	 *                    elements.
	 */
	...
	protected void configureRTPManagerBufferControl(StreamRTPManager rtpManager, BufferControl bufferControl) {
		...
		/*
		 * There isn't a particular reason why we'd choose 100 or 120. It may be that
		 * 120 is divided by 30 (which is used by iLBC, for example) and 100 isn't.
		 * Anyway, what matters most is that it's proportional to the latency of the
		 * playback.
		 */
		...
		/*
		 * The threshold should better be half of the bufferLength rather than equal to
		 * it (as it used to be before). Whatever it is, FMJ/JMF doesn't take it into
		 * account anyway.
		 */
	...
	protected DtmfTransformEngine createDtmfTransformEngine() {
		if (dtmfTransformEngine == null) {
			ConfigurationService cfg = LibJitsi.getConfigurationService();
			if (cfg == null || !cfg.getBoolean(AudioMediaStream.DISABLE_DTMF_HANDLING_PNAME, false)) {
	...
	protected void deviceSessionChanged(MediaDeviceSession oldValue, MediaDeviceSession newValue) {
		try {
			...
			if (newValue != null) {
				 ...
				 * outputVolumeControl of this instance (of course). The output volume (level)
				 * of the oldValue will continue to be controlled by the outputVolumeControl of
				 * this instance (as well). The latter behaviour should not present a problem
				 * and keeps the design and implementation as simple as possible.
				 */
	...
	/**
	 * Delivers the <tt>DTMF</tt> tones. The method is meant for use primarily by
	 * the transform engine handling incoming RTP packets (currently
	 * <tt>DtmfTransformEngine</tt>).
	 *
	 * @param tone the new tone
	 * @param end  <tt>true</tt> if the tone is to be ended or <tt>false</tt> to be
	 *             started
	 */
	...
	/**
	 * Receives and reacts to property change events: if the selected device (for
	 * capture, playback or notifications) has changed, then create or recreate the
	 * streams in order to use it. We want to listen to these events, especially for
	 * those generated after the audio system has changed.
	 *
	...
	public void propertyChange(PropertyChangeEvent ev) {
		/*
		 * FIXME It is very wrong to do the following upon every PropertyChangeEvent
		 * fired by MediaServiceImpl. Moreover, it does not seem right that we'd want to
		 * start this MediaStream upon a PropertyChangeEvent (regardless of its
		 * specifics).
		 */
	...
	protected void registerCustomCodecFormats(StreamRTPManager rtpManager) {
		...
		for (AudioFormat format : CUSTOM_CODEC_FORMATS) {
			...
			/*
			 * NOTE (mkoch@rowa.de): com.sun.media.rtp.RtpSessionMgr.addFormat leaks memory,
			 * since it stores the Format in a static Vector. AFAIK there is no easy way
			 * around it, but the memory impact should not be too bad.
			 */
	...
	/**
	 * Removes <tt>listener</tt> from the list of <tt>DTMFListener</tt>s registered
	 * with this <tt>AudioMediaStream</tt> to receive notifications about incoming
	 * DTMF tones.
	 *
	 * @param listener the <tt>DTMFListener</tt> to no longer be notified by this
	...
	/**
	 * Registers <tt>listener</tt> as the <tt>CsrcAudioLevelListener</tt> that will
	 * receive notifications for changes in the levels of conference participants
	 * that the remote party could be mixing.
	 *
	 * @param listener the <tt>CsrcAudioLevelListener</tt> that we'd like to
	 *                 register or <tt>null</tt> if we'd like to stop receiving
	 *                 notifications.
	 */
	@Override
	...
	/**
	 * Sets <tt>listener</tt> as the <tt>SimpleAudioLevelListener</tt> registered to
	 * receive notifications from our device session for changes in the levels of
	 * the audio that this stream is sending out.
	 *
	 * @param listener the <tt>SimpleAudioLevelListener</tt> that we'd like to
	 *                 register or <tt>null</tt> if we want to stop local audio
	 *                 level measurements.
	 */
	@Override
	 ...
	 * Sets <tt>listener</tt> as the <tt>SimpleAudioLevelListener</tt> registered to
	 * receive notifications from our device session for changes in the levels of
	 * the party that's at the other end of this stream.
	 *
	 * @param listener the <tt>SimpleAudioLevelListener</tt> that we'd like to
	 ...
	 * <tt>stopSendingDTMF()</tt> method is called (Excepts for INBAND DTMF, which
	 * stops by itself this is why where there is no need to call the
	 ...
	 * corresponding stop method is called. Also, calling this method successively
	 * without invoking the corresponding stop method between the calls will simply
	 * replace the <tt>DTMFTone</tt> from the first call with that from the second.
	 *
	 ...
	 *                                  {@link DTMFMethod#INBAND_DTMF},
	 *                                  {@link DTMFMethod#RTP_DTMF}, and
	 *                                  {@link DTMFMethod#SIP_INFO_DTMF}
	 * @see AudioMediaStream#startSendingDTMF( DTMFTone, DTMFMethod, int, int, int)
	 */
	@Override
	public void startSendingDTMF(DTMFTone tone, DTMFMethod dtmfMethod, int minimalToneDuration, int maximalToneDuration,
			int volume) {
		switch (dtmfMethod) {
		case INBAND_DTMF:
