...
public class FileAccessServiceImpl implements FileAccessService {
	/**
	 * The <tt>Logger</tt> used by the <tt>FileAccessServiceImpl</tt> class and its
	...
	/**
	 * The indicator which determines whether {@link #initialize()} has been invoked
	 * on this instance. Introduced to delay the initialization of the state of this
	 * instance until it is actually necessary.
	 */
	...
	/**
	 * This method returns a created temporary file. After you close this file it is
	 * not guaranteed that you will be able to open it again nor that it will
	...
	public File getTemporaryDirectory() throws IOException {
		...
		if (!file.delete()) {
			throw new IOException(
					"Could not create temporary directory, " + "because: could not delete temporary file.");
		}
		if (!file.mkdirs()) {
	...
	/**
	 * This method returns a file specific to the current user. It may not exist,
	 * but it is guaranteed that you will have the sufficient rights to create it.
	 *
	 * This file should not be considered secure because the implementor may return
	 * a file accessible to everyone. Generally it will reside in current user's
	 * homedir, but it may as well reside in a shared directory.
	 *
	 * Note: DO NOT store unencrypted sensitive information in this file
	 *
		...
		try {
			...
			if (file == null) {
				throw new SecurityException(
						"Insufficient rights to access " + "this file in current user's home directory: "
	 ...
	 * return a directory accessible to everyone. Generally it will reside in
	 ...
	 * @throws Exception Thrown if there is no suitable location for the persistent
	 *                   directory.
	 */
	@Override
	public File getPrivatePersistentDirectory(String dirName, FileCategory category) throws Exception {
		...
		if (dir.exists()) {
			if (!dir.isDirectory()) {
				throw new RuntimeException("Could not create directory " + "because: A file exists with this name:"
						+ dir.getAbsolutePath());
			}
		} else {
			if (!dir.mkdirs()) {
	...
	/**
	 * Returns the full path corresponding to a file located in the sip-communicator
	 * config home and carrying the specified name.
	 * 
	...
	/**
	 * Checks if a file exists and if it is writable or readable. If not - checks if
	 * the user has a write privileges to the containing directory.
	 *
	...
	public File getDefaultDownloadDirectory() throws IOException {
		...
		if (OSUtils.IS_WINDOWS) {
			if (getMajorOSVersion() < 6) {
				...
				int hResult = Shell32.INSTANCE.SHGetFolderPath(null, Shell32.CSIDL_MYDOCUMENTS, null,
				...
				int hResult = Shell32.INSTANCE.SHGetKnownFolderPath(g, Shell32.KF_FLAG_INIT | Shell32.KF_FLAG_CREATE,
						null, pszPath);
	...
	/**
	 * Gets the major version of the executing operating system as defined by the
	 * <tt>os.version</tt> system property.
	 *
	 * @return the major version of the executing operating system as defined by the
	 *         <tt>os.version</tt> system property
	 */
	private static int getMajorOSVersion() {
		...
		if ((osVersion != null) && (osVersion.length() > 0)) {
			...
			String majorOSVersionString = (majorOSVersionEnd > -1) ? osVersion.substring(0, majorOSVersionEnd)
					: osVersion;
	...
	/**
	 * Creates a failsafe transaction which can be used to safely store informations
	 * into a file.
	 *
	...
	/**
	 * Initializes this instance if it has not been initialized yet i.e. acts as a
	 * delayed constructor of this instance. Introduced because this
	 * <tt>FileAccessServiceImpl</tt> queries <tt>System</tt> properties that may
	 * not be set yet at construction time and, consequently, throws an
