...
public class LibJitsiImpl extends LibJitsi {
	...
	private static class ServiceLock {
		 ...
		 *
		 * @param           <T>
		 * @param className the {@code name} of {@code clazz} which has already been
		 *                  retrieved from {@code clazz}
		...
		private static <T> T initializeService(String className, Class<T> clazz) {
			...
			if (exception != null && LOGGER.isInfoEnabled()) {
				LOGGER.info("Failed to initialize service implementation " + implClassName + ".", exception);
			}
