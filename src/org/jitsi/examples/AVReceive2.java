...
public class AVReceive2 {
	/**
	 * The port which is the target of the transmission i.e. on which the media is
	 * to be received.
	 *
	...
	/**
	 * The <tt>InetAddress</tt> of the host which is the target of the receipt i.e.
	 * from which the media is to be received.
	 *
	...
	/**
	 * The port which is the target of the receipt i.e. from which the media is to
	 * be received.
	 *
	...
	/**
	 * Initializes a new <tt>AVReceive2</tt> instance which is to receive audio and
	 ...
	 *
	 * @return <tt>true</tt> if this instance has been successfully initialized to
	 *         receive audio and video
	 * @throws Exception if anything goes wrong while initializing this instance for
	...
	/**
	 * The name of the command-line argument which specifies the port on which the
	 * media is to be received. The command-line argument value will be used as the
	 * port to receive the audio RTP on, the next port after it will be used to
	 * receive the audio RTCP on. Respectively, the subsequent ports ports will be
	 * used to transmit the video RTP and RTCP on."
	 */
	private static final String LOCAL_PORT_BASE_ARG_NAME = "--local-port-base=";

	/**
	 * The name of the command-line argument which specifies the name of the host
	 * from which the media is to be received.
	 */
	 ...
	 * media is to be received. The command-line argument value will be used as the
	 * port to receive the audio RTP from, the next port after it will be to receive
	 * the audio RTCP from. Respectively, the subsequent ports will be used to
	 * receive the video RTP and RTCP from."
	 */
	...
	private static final String[][] ARGS = {
			...
			{ REMOTE_HOST_ARG_NAME, "The name of the host from which the media is to be received." },
	...
	public static void main(String[] args) throws Exception {
		...
		// -Drun.example.arg.line="--local-port-base=10000 --remote-host=129.130.131.132
		// --remote-port-base=5000"
		if (args.length < 3) {
			prUsage();
		} else {
			...
			try {
				AVReceive2 avReceive = new AVReceive2(argMap.get(LOCAL_PORT_BASE_ARG_NAME),
						argMap.get(REMOTE_HOST_ARG_NAME), argMap.get(REMOTE_PORT_BASE_ARG_NAME));

				if (avReceive.initialize()) {
					try {
						/*
						 * Wait for the media to be received and played back. AVTransmit2 transmits for
						 * 1 minute so AVReceive2 waits for 2 minutes to allow AVTransmit2 to start the
	...
	/**
	 * Outputs human-readable description about the usage of the <tt>AVReceive2</tt>
	 * application and the command-line arguments it accepts as valid.
	 */
