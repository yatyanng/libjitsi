...
public class AVTransmit2 {
	/**
	 * The port which is the source of the transmission i.e. from which the media is
	 * to be transmitted.
	 *
	...
	/**
	 * The <tt>MediaStream</tt> instances initialized by this instance indexed by
	 * their respective <tt>MediaType</tt> ordinal.
	 */
	private MediaStream[] mediaStreams;

	/**
	 * The <tt>InetAddress</tt> of the host which is the target of the transmission
	...
	/**
	 * The port which is the target of the transmission i.e. to which the media is
	 * to be transmitted.
	 *
	...
	/**
	 * Initializes a new <tt>AVTransmit2</tt> instance which is to transmit audio
	 * and video to a specific host and a specific port.
	 *
	 * @param localPortBase  the port which is the source of the transmission i.e.
	 *                       from which the media is to be transmitted
	 * @param remoteHost     the name of the host which is the target of the
	 *                       transmission i.e. to which the media is to be
	 *                       transmitted
	 * @param remotePortBase the port which is the target of the transmission i.e.
	 *                       to which the media is to be transmitted
	...
	/**
	 * Starts the transmission. Returns null if transmission started ok. Otherwise
	 * it returns a string with the reason why the setup failed.
	 */
	private String start() throws Exception {
		/*
		 * Prepare for the start of the transmission i.e. initialize the MediaStream
		 * instances.
		 */
		...
		for (MediaType mediaType : mediaTypes) {
			/*
			 * The default MediaDevice (for a specific MediaType) is configured (by the user
			 * of the application via some sort of UI) into the ConfigurationService. If
			 * there is no ConfigurationService instance known to LibJitsi, the first
			 * available MediaDevice of the specified MediaType will be chosen by
			 * MediaService.
			 */
			...
			// direction
			/*
			 * The AVTransmit2 example sends only and the AVReceive2 receives only. In a
			 * call, the MediaStream's direction will most commonly be set to SENDRECV.
			 */
			...
			double clockRate;
			/*
			 * The AVTransmit2 and AVReceive2 examples use the H.264 video codec. Its RTP
			 * transmission has no static RTP payload type number assigned.
			 */
			...
			case VIDEO:
				...
				/*
				 * The dymanic RTP payload type numbers are usually negotiated in the signaling
				 * functionality.
				 */
			...
			if (encoding != null) {
				...
				/*
				 * The MediaFormat instances which do not have a static RTP payload type number
				 * association must be explicitly assigned a dynamic RTP payload type number.
				 */
			...
			// target
			/*
			 * The AVTransmit2 and AVReceive2 examples follow the common practice that the
			 * RTCP port is right after the RTP port.
			 */
			...
			// name
			/*
			 * The name is completely optional and it is not being used by the MediaStream
			 * implementation at this time, it is just remembered so that it can be
			 * retrieved via MediaStream#getName(). It may be integrated with the signaling
		...
		/*
		 * Do start the transmission i.e. start the initialized MediaStream instances.
		 */
	...
	/**
	 * The name of the command-line argument which specifies the port from which the
	 * media is to be transmitted. The command-line argument value will be used as
	 * the port to transmit the audio RTP from, the next port after it will be to
	 * transmit the audio RTCP from. Respectively, the subsequent ports will be used
	 * to transmit the video RTP and RTCP from."
	 */
	private static final String LOCAL_PORT_BASE_ARG_NAME = "--local-port-base=";

	/**
	 * The name of the command-line argument which specifies the name of the host to
	 * which the media is to be transmitted.
	 */
	private static final String REMOTE_HOST_ARG_NAME = "--remote-host=";

	/**
	 * The name of the command-line argument which specifies the port to which the
	 * media is to be transmitted. The command-line argument value will be used as
	 * the port to transmit the audio RTP to, the next port after it will be to
	 * transmit the audio RTCP to. Respectively, the subsequent ports will be used
	 * to transmit the video RTP and RTCP to."
	 */
	...
	private static final String[][] ARGS = {
			{ LOCAL_PORT_BASE_ARG_NAME,
					"The port which is the source of the transmission i.e. from"
							...
							+ " subsequent ports will be used to transmit the video RTP" + " and RTCP from." },
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length < 2) {
			...
			try {
				...
				AVTransmit2 at = new AVTransmit2(argMap.get(LOCAL_PORT_BASE_ARG_NAME), argMap.get(REMOTE_HOST_ARG_NAME),
	...
	/**
	 * Parses the arguments specified to the <tt>AVTransmit2</tt> application on the
	 * command line.
	 *
	 * @param args the arguments specified to the <tt>AVTransmit2</tt> application
	 *             on the command line
	 ...
	 * <tt>AVTransmit2</tt> application and the command-line arguments it accepts as
	 * valid.
	 */
