 ...
 * i.e. asynchronous tasks which determine by themselves the intervals (the
 * lengths of which may vary) at which they are to be invoked.
 *
...
public class RecurringRunnableExecutor implements Executor {
	/**
	 * The <tt>Logger</tt> used by the <tt>RecurringRunnableExecutor</tt> class and
	 * its instances to print debug information.
	 */
	private static final Logger logger = Logger.getLogger(RecurringRunnableExecutor.class);

	/**
	 * The {@code RecurringRunnable}s registered with this instance which are to be
	 * invoked in {@link #thread}.
	 */
	private final List<RecurringRunnable> recurringRunnables = new LinkedList<>();

	/**
	 * The (background) {@code Thread} which invokes {@link RecurringRunnable#run()}
	 * on {@link #recurringRunnables} (in accord with their respective
	 * {@link RecurringRunnable#getTimeUntilNextRun()}).
	 */
	private Thread thread;

	/**
	 * A {@code String} which will be added to the name of {@link #thread}. Meant to
	 * facilitate debugging.
	 */
	private final String name;

	/**
	 * Whether this {@link RecurringRunnableExecutor} is closed. When it is closed,
	 * it should stop its thread(s).
	 */
	...
	/**
	 * De-registers a {@code RecurringRunnable} from this {@code Executor} so that
	 * its {@link RecurringRunnable#run()} is no longer invoked (by this instance).
	 *
	 * @param recurringRunnable the {@code RecurringRunnable} to de-register from
	 *                          this instance
	 ...
	 * Invokes {@link RecurringRunnable#run()} on all {@link #recurringRunnables}
	 * which are at or after the time at which they want the method in question
	 * called. TODO(brian): worth investigating if we can invoke the ready runnables
	 * outside the scope of the {@link RecurringRunnableExecutor#recurringRunnables}
	 * lock so we can get rid of a possible deadlock scenario when invoking a
	 * runnable which needs to signal to the executor that it has work ready
	 *
	 * @return {@code true} to continue with the next iteration of the loop
	 *         implemented by {@link #runInThread()} or {@code false} to break (out
	 *         of) the loop
	 */
	private boolean run() {
				...
				if (timeToNext < 1L) {
					try {
						...
						if (t instanceof InterruptedException) {
							...
							logger.error("The invocation of the method " + recurringRunnable.getClass().getName()
	...
	/**
	 * Registers a {@code RecurringRunnable} with this {@code Executor} so that its
	 * {@link RecurringRunnable#run()} is invoked (by this instance).
	 *
	 * @param recurringRunnable the {@code RecurringRunnable} to register with this
	 *                          instance
	 * @return {@code true} if the list of {@code RecurringRunnable}s of this
	 *         instance changed because of the method call; otherwise, {@code false}
	 */
	...
	/**
	 * Starts or notifies {@link #thread} depending on and in accord with the state
	 * of this instance.
	 */
	...
	/**
	 * Closes this {@link RecurringRunnableExecutor}, signalling its thread to stop
	 * and de-registering all registered runnables.
	 */
	public void close() {
		synchronized (recurringRunnables) {
			closed = true;
