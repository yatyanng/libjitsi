...
public class VideoLayout extends FitLayout {
	...
	/**
	 * Creates an instance of <tt>VideoLayout</tt> by also indicating if this video
	 * layout is dedicated to a conference interface.
	 *
	 * @param conference <tt>true</tt> if the new instance will be dedicated to a
	...
	/**
	 * Determines whether the aspect ratio of a specific <tt>Dimension</tt> is to be
	 * considered equal to the aspect ratio of specific <tt>width</tt> and
	 * <tt>height</tt>.
	 *
	 * @param size   the <tt>Dimension</tt> whose aspect ratio is to be compared to
	 *               the aspect ratio of <tt>width</tt> and <tt>height</tt>
	 * @param width  the width which defines in combination with <tt>height</tt> the
	 *               aspect ratio to be compared to the aspect ratio of
	 *               <tt>size</tt>
	 * @param height the height which defines in combination with <tt>width</tt> the
	 *               aspect ratio to be compared to the aspect ratio of
	 *               <tt>size</tt>
	 ...
	 *
	 * @param remotes the remote visual/video <tt>Component</tt>s to be displayed in
	 *                a grid
	 * @return the number of columns to use for the grid display of the specified
	 *         remote visual/video <tt>Component</tt>s
	 */
	...
	/**
	 * Lays out the specified <tt>Container</tt> (i.e. the <tt>Component</tt>s it
	 * contains) in accord with the logic implemented by this
	 * <tt>LayoutManager</tt>.
	 *
	...
	public void layoutContainer(Container parent) {
		...
		/*
		 * When there are multiple remote visual/video Components, the local one will be
		 * displayed as if it is a remote one i.e. in the same grid, not on top of a
		 * remote one. The same layout will be used when this instance is dedicated to a
		...
		if (!conference && (remoteCount == 1)) {
			/*
			 * If the videos are to be laid out as in a one-to-one call, the remote video
			 * has to fill the parent and the local video will be placed on top of the
			 * remote video. The remote video will be laid out now and the local video will
			 * be laid out later/further bellow.
			 */
			...
			Rectangle bounds = new Rectangle(0, 0,
					/*
					 * HGAP is the horizontal gap between the Components being laid out by this
					 * VideoLayout so the number of HGAPs will be with one less than the number of
					 * columns and that horizontal space cannot be allocated to the bounds of the
					 * Components.
					 */
			...
			for (int i = 0; i < remoteCount; i++) {
				...
				/*
				 * On the x axis, the first column starts at zero and each subsequent column
				 * starts relative to the end of its preceding column.
				 */
				if (column == 0) {
					bounds.x = 0;
					/*
					 * Eventually, there may be empty cells in the last row. Center the non-empty
					 * cells horizontally.
					 */
					if (row == rowsMinus1) {
		...
		if (local == null) {
			/*
			 * It is plain wrong to display a close button for the local video if there is
			 * no local video.
			 */
			if (closeButton != null)
			...
			/*
			 * If the local visual/video Component is not displayed as if it is a remote
			 * one, it will be placed on top of a remote one.
			 */
			if (!remotes.contains(local)) {
				...
				/*
				 * XXX The remote Component being a JLabel is meant to signal that there is no
				 * remote video and the remote is the photoLabel.
				 */
				...
				super.layoutComponent(local, new Rectangle(localX, localY, width, height), alignmentX,
			...
			if (closeButton != null) {
				/*
				 * XXX We may be overwriting the visible property set by our client (who has
				 * initialized the close button) but it is wrong to display a close button for
				 * the local video if the local video is not visible.
				 */
				closeButton.setVisible(local.isVisible());

				super.layoutComponent(closeButton,
						new Rectangle(local.getX() + local.getWidth() - closeButton.getWidth(), local.getY(),
		...
		/*
		 * The video canvas will get the locations of the other components to paint so
		 * it has to cover the parent completely.
		 */
		if (canvas != null)
	...
	public Dimension preferredLayoutSize(Container parent) {
		...
		if (local != null) {
			/*
			 * If the local visual/video Component is not displayed as if it is a remote
			 * one, it will be placed on top of a remote one. Then for the purposes of the
			 * preferredLayoutSize method it needs to be considered only if there is no
			 * remote video whatsoever.
			 */
			...
			/*
			 * The closeButton has to be on top of the local video. Consequently, the
			 * preferredLayoutSize method does not have to consider it. Well, maybe if does
			 * if the local video is smaller than the closeButton... but that's just not
			 * cool anyway.
			 */
		}
		...
		/*
		 * The video canvas will get the locations of the other components to paint so
		 * it has to cover the parent completely. In other words, the
