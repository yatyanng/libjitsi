...
public class VideoContainer extends TransparentPanel {
	 ...
	 * {@link VideoLayout#CENTER_REMOTE} when no other <tt>Component</tt> has been
	 * added to it to be displayed there. For example, the avatar of the remote peer
	 * may be displayed in place of the remote video when the remote video is not
	 * available.
	 */
	 ...
	 * {@link #doLayout()}, {@link #repaint()} and/or {@link #validate()} are to be
	 * invoked (as soon as {@link #inAddOrRemove} decreases from a positive number
	 * to zero).
	 */
	private boolean validateAndRepaint;

	/**
	 * Initializes a new <tt>VideoContainer</tt> with a specific <tt>Component</tt>
	 * to be displayed when no remote video is available.
	 *
	 * @param noVideoComponent the component to be displayed when no remote video is
	 *                         available
	 * @param conference       <tt>true</tt> to dedicate the new instance to a
	 *                         telephony conferencing user interface; otherwise,
	 *                         <tt>false</tt>
	 */
	...
	/**
	 * Adds the given component at the {@link VideoLayout#CENTER_REMOTE} position in
	 * the default video layout.
	 *
	...
	public void add(Component comp, Object constraints, int index) {
		enterAddOrRemove();
		try {
			if (VideoLayout.CENTER_REMOTE.equals(constraints) && (noVideoComponent != null)
					&& !noVideoComponent.equals(comp)
					|| (comp.equals(noVideoComponent) && noVideoComponent.getParent() != null)) {
	...
	/**
	 * Notifies this instance that a specific <tt>Component</tt> has been added to
	 * or removed from this <tt>Container</tt>.
	 *
	...
	private void onContainerEvent(ContainerEvent ev) {
		try {
			...
			/*
			 * If an explicit background color is to be displayed by this Component, make
			 * sure that its opaque property i.e. transparency does not interfere with that
			 * display.
			 */
			if (DEFAULT_BACKGROUND_COLOR != null) {
	 ...
	 *
	 * @param ev a <tt>PropertyChangeEvent</tt> which details the specifics of the
	 *           notification such as the name of the property whose value changed
	 *           and the <tt>Component</tt> which fired the notification
	 */
	private void propertyChange(PropertyChangeEvent ev) {
		if (PREFERRED_SIZE_PROPERTY_NAME.equals(ev.getPropertyName()) && SwingUtilities.isEventDispatchThread()) {
			/*
			 * The goal is to invoke doLayout, repaint and/or validate. These methods and
			 * the specifics with respect to avoiding unnecessary calls to them are already
			 * dealt with by enterAddOrRemove, exitAddOrRemove and validateAndRepaint.
			 */
			synchronized (syncRoot) {
				enterAddOrRemove();
				validateAndRepaint = true;
	...
	/**
	 * Overrides the default remove behavior in order to add the default no video
	 * component when the remote video is removed.
	 *
		...
		try {
			...
			for (Component c : components) {
				if (!c.equals(noVideoComponent)
						&& VideoLayout.CENTER_REMOTE.equals(videoLayout.getComponentConstraints(c))) {
	 ...
	 * ContainerEvent#COMPONENT_REMOVED when there is no other Component left in the
	 * Container will cause an infinite loop because Container#removeAll() will
	 * detect that a new Component has been added while dispatching the event and
	 * will then try to remove the new Component.
	 */
	@Override
