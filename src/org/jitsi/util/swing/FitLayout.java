...
public class FitLayout implements LayoutManager {
	 ...
	 * Introduced to mitigate issues arising from the fact that a <tt>Component</tt>
	 * zero height and/or width.
	 */
	 ...
	 * <tt>Component</tt> of the parent <tt>Container</tt> and thus doesn't need any
	 * <tt>String</tt> associations.
	 */
	@Override
	...
	/**
	 * Gets the first <tt>Component</tt> of a specific <tt>Container</tt> if there
	 * is such a <tt>Component</tt>.
	 *
	 * @param parent the <tt>Container</tt> to retrieve the first <tt>Component</tt>
	 *               of
	...
	protected void layoutComponent(Component component, Rectangle bounds, float alignmentX, float alignmentY) {
		...
		/*
		 * XXX The following (mostly) represents a quick and dirty hack for the purposes
		 * of video conferencing which adds transparent JPanels to VideoContainer and
		 * does not want them fitted because they contain VideoContainers themselves and
		 * the videos get fitted in them.
		 */
		if (((component instanceof JPanel) && !component.isOpaque()
				&& (((Container) component).getComponentCount() > 1)) || (component instanceof VideoContainer)
		/*
		 * If the specified component does not have a preferredSize, we cannot know its
		 * aspect ratio and we are left with no choice but to stretch it within the
		 * complete bounds.
		 */
		...
		/*
		 * Why would one fit a Component into a rectangle with zero width and height?
		 */
		if (size.height < 1)
			size.height = 1;
		if (size.width < 1)
		...
		component.setBounds(bounds.x + Math.round((bounds.width - size.width) * alignmentX),
				bounds.y + Math.round((bounds.height - size.height) * alignmentY), size.width, size.height);
	}

	/*
	 * Scales the first Component if its preferred size is larger than the size of
	 * its parent Container in order to display the Component in its entirety and
	 * then centers it within the display area of the parent.
	 */
	@Override
	...
	protected void layoutContainer(Container parent, float componentAlignmentX) {
		...
		if (component != null) {
			layoutComponent(component, new Rectangle(parent.getSize()), componentAlignmentX,
					Component.CENTER_ALIGNMENT);
		}
	}

	/*
	 * Since this LayoutManager lays out only the first Component of the specified
	 * parent Container, the minimum size of the Container is the minimum size of
	 ...
	 * {@inheritDoc}
	 *
	 * Since this <tt>LayoutManager</tt> lays out only the first <tt>Component</tt>
	 * of the specified parent <tt>Container</tt>, the preferred size of the
	 * <tt>Container</tt> is the preferred size of the mentioned <tt>Component</tt>.
	 */
	@Override
