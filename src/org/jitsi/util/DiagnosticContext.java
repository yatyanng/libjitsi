...
public class DiagnosticContext extends ConcurrentHashMap<String, Object> {
	/**
	 * Makes a new time series point without a timestamp. This is recommended for
	 * time series where the exact timestamp value isn't important and can be
	 * deduced via other means (i.e. Java logging timestamps).
	 *
	...
	/**
	 * Makes a new time series point with a timestamp. This is recommended for time
	 * series where it's important to have the exact timestamp value.
	 *
	 * @param timeSeriesName the name of the time series
	...
	public TimeSeriesPoint makeTimeSeriesPoint(String timeSeriesName, long tsMs) {
		return makeTimeSeriesPoint(timeSeriesName, Instant.ofEpochMilli(tsMs));
	}
	...
	public TimeSeriesPoint makeTimeSeriesPoint(String timeSeriesName, Instant instant) {
		return new TimeSeriesPoint(this).addField("series", timeSeriesName).addField("time",
