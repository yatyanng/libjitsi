...
public class RTCPUtils {
	 ...
	 * @return the number of bytes that were written to the byte buffer, or -1 in
	 *         case of an error.
	 */
	...
	/**
	 * Gets the RTCP packet length in bytes as specified by the length field of the
	 * RTCP packet (does not verify that the buffer is actually large enough).
	 *
	 * @param buf the byte buffer that contains the RTCP header.
	 * @param off the offset in the byte buffer where the RTCP header starts.
	 * @param len the number of bytes in buffer which constitute the actual data.
	...
	/**
	 * Checks whether the RTCP header is valid or not (note that a valid header does
	 * not necessarily imply a valid packet). It does so by checking the RTCP header
	 * version and makes sure the buffer is at least 8 bytes long.
	 *
	 * @param buf the byte buffer that contains the RTCP header.
	 * @param off the offset in the byte buffer where the RTCP header starts.
	 * @param len the number of bytes in buffer which constitute the actual data.
	 ...
	 *         {@link ByteArrayBuffer} that is passed in as a parameter, or -1 in
	 *         case of an error.
	 */
	 ...
	 * @param off the offset in the byte buffer where the RTCP header starts.
	 * @param len the number of bytes in buffer which constitute the actual data.
	 * @return the report count field of the RTCP packet specified in the byte
	 *         buffer that is passed in as a parameter, or -1 in case of an error.
	 */
	...
	/**
	 * Checks whether the buffer described by the parameters looks like an RTCP
	 * packet. It only checks the Version and Packet Type fields, as well as a
	 * minimum length. This method returning {@code true} does not necessarily mean
	 * that the given packet is a valid RTCP packet, but it should be parsed as RTCP
