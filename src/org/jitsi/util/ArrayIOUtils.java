...
public class ArrayIOUtils {
	...
	/**
	 * Reads an integer from a specific series of bytes starting the reading at a
	 * specific offset in it.
	 *
	 * @param in       the series of bytes to read an integer from
	 * @param inOffset the offset in <tt>in</tt> at which the reading of the integer
	 *                 is to start
	 * @return an integer read from the specified series of bytes starting at the
	 *         specified offset in it
	 */
	public static int readInt(byte[] in, int inOffset) {
		return (in[inOffset + 3] << 24) | ((in[inOffset + 2] & 0xFF) << 16) | ((in[inOffset + 1] & 0xFF) << 8)
				| (in[inOffset] & 0xFF);
	}

	/**
	 * Reads a short integer from a specific series of bytes starting the reading at
	 * a specific offset in it. The difference with {@link #readShort(byte[], int)}
	 * is that the read short integer is an <tt>int</tt> which has been formed by
	 * reading two bytes, not a <tt>short</tt>.
	 *
	 * @param in       the series of bytes to read the short integer from
	 * @param inOffset the offset in <tt>in</tt> at which the reading of the short
	 *                 integer is to start
	 * @return a short integer in the form of <tt>int</tt> read from the specified
	 *         series of bytes starting at the specified offset in it
	 */
	 ...
	 * @param inOffset the offset in <tt>in</tt> at which the reading of the short
	 *                 integer is to start
	 * @return a short integer in the form of <tt>short</tt> read from the specified
	 *         series of bytes starting at the specified offset in it
	 */
	 ...
	 * specific output array of bytes starting the writing at a specific offset in
	 * it.
	 *
	...
	/**
	 * Converts a short integer to a series of bytes and writes the result into a
	 * specific output array of bytes starting the writing at a specific offset in
	 * it. The difference with {@link #writeShort(short, byte[], int)} is that the
	 * input is an <tt>int</tt> and just two bytes of it are written.
	 *
	 ...
	 *                  contained in only two of the four bytes made available by
	 *                  the integer
