...
public class SctpNotification {
	/******** Notifications **************/
	/*
	 * union sctp_notification { struct sctp_tlv { uint16_t sn_type; uint16_t
	 * sn_flags; uint32_t sn_length; } sn_header; struct sctp_assoc_change
	 * sn_assoc_change; struct sctp_paddr_change sn_paddr_change; struct
	 * sctp_remote_error sn_remote_error; struct sctp_shutdown_event
	 * sn_shutdown_event; struct sctp_adaptation_event sn_adaptation_event; struct
	 * sctp_pdapi_event sn_pdapi_event; struct sctp_authkey_event sn_auth_event;
	 * struct sctp_sender_dry_event sn_sender_dry_event; struct
	 * sctp_send_failed_event sn_send_failed_event; struct sctp_stream_reset_event
	 * sn_strreset_event; struct sctp_assoc_reset_event sn_assocreset_event; struct
	...
	/**
	 * When the SCTP implementation has no user data anymore to send or retransmit
	...
	/**
	 * Association change event struct sctp_assoc_change { uint16_t sac_type;
	 * uint16_t sac_flags; uint32_t sac_length; uint16_t sac_state; uint16_t
	 * sac_error; uint16_t sac_outbound_streams; uint16_t sac_inbound_streams;
	 * sctp_assoc_t sac_assoc_id; //uint32_t uint8_t sac_info[]; // not available
	 * yet };
	 */
	...
	/**
	 * Address event struct sctp_paddr_change { uint16_t spc_type; uint16_t
	 * spc_flags; uint32_t spc_length; struct sockaddr_storage spc_aaddr; uint32_t
	 * spc_state; uint32_t spc_error; sctp_assoc_t spc_assoc_id; //uint32_t uint8_t
	 * spc_padding[4]; };
	 *
	 */
	 ...
	 *
	 * struct sctp_send_failed_event { uint16_t ssfe_type; uint16_t ssfe_flags;
	 * uint32_t ssfe_length; uint32_t ssfe_error; struct sctp_sndinfo ssfe_info;
	 * sctp_assoc_t ssfe_assoc_id; uint8_t ssfe_data[]; };
	 *
	 * struct sctp_sndinfo { uint16_t snd_sid; uint16_t snd_flags; uint32_t
	 * snd_ppid; uint32_t snd_context; sctp_assoc_t snd_assoc_id; // uint32 };
	 *
	 */
	 ...
	 *
	 * struct sctp_sender_dry_event { uint16_t sender_dry_type; uint16_t
	 * sender_dry_flags; uint32_t sender_dry_length; sctp_assoc_t
	 * sender_dry_assoc_id; };
	 */
	 ...
	 * Stream reset event
	 *
	 * struct sctp_stream_reset_event { uint16_t strreset_type; uint16_t
	 * strreset_flags; uint32_t strreset_length; sctp_assoc_t strreset_assoc_id;
