...
public class SctpSocket {
	...
	private static long bytes_to_long(byte[] buffer, int offset) {
		int fByte = (0x000000FF & (buffer[offset]));
		int sByte = (0x000000FF & (buffer[offset + 1]));
		int tByte = (0x000000FF & (buffer[offset + 2]));
		int foByte = (0x000000FF & (buffer[offset + 3]));
		return (fByte << 24 | sByte << 16 | tByte << 8 | foByte) & 0xFFFFFFFFL;
	}
	 ...
	 * @throws java.io.IOException if this socket is closed or an error occurs while
	 ...
	 *
	 * @param callback the callback that will be fired when new data is received.
	 */
	 ...
	 *
	 * @param link <tt>NetworkLink</tt> that will be used by this instance to send
	 *             network packets.
	 */
